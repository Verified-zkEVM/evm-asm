...
> Snapshot: `2026-06-29` @ [`02d7e2796`](https://github.com/Verified-zkEVM/evm-asm/commit/02d7e27966c929437a2a7e4e422d596611be2771)
...
| 🟡 ADDMOD | partial | `evm_addmod_partial_domain_named_regs_owned_live_stack_spec_within` | — | addmod_correct proven; zero-modulus stack spec done for arbitrary b; no-overflow skeleton composed through legacy MOD no-NOP body proof; current partial theorem has named domain plus a final live-stack post at sp+64, with consumed operand cells and all leftover registers in the current frame weakened to ownership and scratch resources framed explicitly; zero-modulus and no-overflow branches are exposed directly as evm_addmod_n0_return_owned_live_stack_spec_within and evm_addmod_no_overflow_return_owned_live_stack_spec_within |
...
| 🟡 EXP | partial | `evm_exp_headroom_frame_regs_owned_leftover_live_stack_spec_within` | — | exp_correct proven; pure executable stack transition names EvmWord.exp; headroom RV64 full-loop composition now reaches a Spec-level named-pre theorem that splits the final live stack at evmSp+32; consumed operand/scratch cells and leftover headroom stack cells are weakened to ownership; leftover frame/scratch registers x2, x9, and x5 are also owned in the leftover frame, with remaining returned resources framed explicitly; public evm_exp_stack_spec_within wrapper still pending |
