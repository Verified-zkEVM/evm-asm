...
> Snapshot: `2026-06-29` @ [`80d9b19fc`](https://github.com/Verified-zkEVM/evm-asm/commit/80d9b19fc29d1e9959ea2afb1dee1afb43db1b9e)
...
| 🟡 EXP | partial | `evm_exp_headroom_public_stack_shape_base_aligned_spec_within` | — | exp_correct proven; pure executable stack transition names EvmWord.exp; headroom RV64 full-loop composition now reaches a Spec-level theorem with ordinary entry-base alignment, old register/scratch values and the internal scratch base existentially packaged, and the final live stack split at evmSp+32; consumed operand/scratch cells and leftover headroom stack cells are weakened to ownership; leftover frame/scratch registers x0, x2, x9, and x5 are also owned in the leftover frame, with remaining returned resources framed explicitly; public evm_exp_stack_spec_within wrapper still pending |
