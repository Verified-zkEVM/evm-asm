...
> Snapshot: `2026-08-06` @ [`5ca2c4dc5`](https://github.com/Verified-zkEVM/evm-asm/commit/5ca2c4dc5fe6882f44b1ef387919a7d0b9069689)
...
| **looser** | 0 |
| no counterpart | 2 |
| *(rows with a `SpecRef` port defect — an orthogonal axis, not a verdict)* | 1 |
| n/a — unproven | 3 |
...
| ported | 4 | machine-checked tie to a SpecRef port that has no differential; requires a port-fidelity clause table |
...
| `header` | guest | `header_extract_number` | `header_number_of_decode` | domain-restricted **+ port defect (#11513)** | ported | the `number` field of `_decode_header` (SpecRef/Stateless.lean:75, stateless.py:244) |
...
- Programs in `EvmAsm/Codegen/Programs.lean` registry: **1020
0**
