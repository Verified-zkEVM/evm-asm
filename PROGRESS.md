...
> Snapshot: `2026-06-29` @ [`68f8c4e0a`](https://github.com/Verified-zkEVM/evm-asm/commit/68f8c4e0a48d8f6c77b6dc4db88a1acc6e2c0abd)
...
| 🟡 EXP | partial | `evm_exp_headroom_owned_leftover_live_stack_spec_within` | — | exp_correct proven; pure executable stack transition names EvmWord.exp; headroom RV64 full-loop composition now reaches a Spec-level named-pre theorem that splits the final live stack at evmSp+32; consumed operand/scratch cells and leftover headroom stack cells are weakened to ownership, with the remaining returned registers framed explicitly; public evm_exp_stack_spec_within wrapper still pending |
