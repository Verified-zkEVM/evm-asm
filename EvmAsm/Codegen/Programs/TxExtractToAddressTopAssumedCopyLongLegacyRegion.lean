...
theorem nFrontCopyStepsLongLegacyRegion_le_nExtract (lol : Nat) (hlol : lol ≤ 8) :
    nFrontCopyStepsLongLegacyRegion lol ≤ nExtractSteps := by
  simp only [nFrontCopyStepsLongLegacyRegion, nExtractSteps, nTypeSteps]
  omega
  -- lol ≤ 8 ⇒ 7*lol+25 ≤ 81 (= short full walk_init budget)

set_option maxRecDepth 8000 in
theorem extractAssumed_copy_concrete_long_legacy_region
    ...
    (hlol : lol ≤ 8)
    (hE2E : cpsTripleWithin (nFrontCopyStepsLongLegacyRegion lol) E s.ra extractLinkedCode
  ...
  have h1 := cpsTripleWithin_mono_nSteps (nFrontCopyStepsLongLegacyRegion_le_nExtract lol hlol) hE2E
...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_temps_long_legacy_region
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr contentPtr w2 : Word)
    (txBytes : List (BitVec 8))
    (lol : Nat)
    ...
    (hE2E : ∀ (old5 old6 old7 old14 old15 old16 : Word),
      cpsTripleWithin (nFrontCopyStepsLongLegacyRegion lol) E s.ra extractLinkedCode
        (creationE2EPre sp0 spC s txBase lenW toBuf isCreationPtr
          old5 old6 old7 old14 old15 old16 txBytes)
        (copyE2EPostRegion sp0 s txBase toBuf isCreationPtr contentPtr w2
          txBytes)) :
  ...
  have htemps : cpsTripleWithin nExtractSteps E s.ra extractLinkedCode
    ...
    have hc := extractAssumed_copy_concrete_long_legacy_region sp0 spC s txBase lenW
...
theorem extractAssumed_copy_of_front_long_concrete_legacy_region
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr : Word)
    (txBytes : List (BitVec 8))
    (srcOff0 srcOff1 srcOff2 srcOff3 q : Nat)
    (hspC : spC = sp0 + signExtend12 (-80 : BitVec 12))
    ...
    (hnext_content : ∀ (next3 len3 : Word),
      rlpItemDecode txBytes srcOff3 (txBase + BitVec.ofNat 64 srcOff3)
        (longWalkEnd txBase (lenW - (teerTxTypeDispatch txBytes).2.2)
          (teerTxTypeDispatch txBytes).2.2.toNat) next3 len3 →
        next3 = txBase + BitVec.ofNat 64 (8 * q) + (20 : Word))
  ...
  let lol := ((txBytes[(teerTxTypeDispatch txBytes).2.2.toNat]'hoff).zeroExtend 64 -
      (0xf7 : Word)).toNat
  refine extractAssumed_copy_temps_long_legacy_region sp0 spC s txBase lenW toBuf
    isCreationPtr contentPtr w2 txBytes lol hspC hlol
    (fun old5 old6 old7 old14 old15 old16 => ?_)
  have hE := extractFrontCopy_then_epi_of_decode_long_concrete_legacy_region
    sp0 spC s txBase lenW toBuf isCreationPtr txBytes
    srcOff0 srcOff1 srcOff2 srcOff3 q
    hoff
...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_fullCode_of_decode_long_concrete_legacy_region
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr : Word)
    (txBytes : List (BitVec 8))
    (srcOff0 srcOff1 srcOff2 srcOff3 q : Nat)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_copy_of_front_long_concrete_legacy_region sp0 spC s
      txBase lenW toBuf isCreationPtr txBytes
      srcOff0 srcOff1 srcOff2 srcOff3 q
      hspC hret hwi_off hcur htype0 hsalign
      hoff0 hover0 hvalid0 hss0 hls0 hll0 hdec0 hinb0
      hoff1 hover1 hvalid1 hss1 hls1 hll1 hdec1 hinb1
      hoff2 hover2 hvalid2 hss2 hls2 hll2 hdec2 hinb2
      hoff3 hover3 hvalid3 hss3 hls3 hll3 hdec3 hinb3
...
#print axioms extractAssumed_copy_of_front_long_concrete_legacy_region
#print axioms extractAssumed_copy_fullCode_of_decode_long_concrete_legacy_region
