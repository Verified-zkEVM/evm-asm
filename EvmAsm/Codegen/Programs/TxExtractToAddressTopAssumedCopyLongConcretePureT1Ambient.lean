/-
  Ambient dual: long t1 copy pure honesty (typeW=1, fields 0..4, content@4).
...
theorem extractAssumed_copy_longConcrete_pure_t1_ambient
  ...
  have h_match :
    ...
    exact h
  have hhoff :=
    extractSuccess_copy_t1_hoff_srcOff_long slice hsuccess hcopyFlag htype1
      items hdecL hlong
  have hhover :=
    extractSuccess_copy_t1_hover_srcOff_long slice loadPtr hsuccess hcopyFlag htype1
      items hdecL hlong hover_sl
  have hlenItems :=
    extractSuccess_copy_t1_items_length_long slice hsuccess hcopyFlag htype1
      items hdecL hlong
  ...
  have hfields03 :=
    extractSuccess_copy_t1_hnext_fields03_long slice hsuccess hcopyFlag htype1
      items hdecL hlong
  have hoverEnd :
      regionBase.toNat +
  ...
  have hhnext :=
    extractSuccess_copy_t1_hnext_hlen20_srcOff_long slice loadPtr hsuccess
      hcopyFlag htype1 items hdecL hlong
  ...
  have hlen20 : ∀ (next4 len4 : Word),
    ...
    simpa [absOff4, srcOff4, listOff, endW, absListOff] using
      hlen20_ambient_of_slice_long regionBase loadPtr bs off len listOff items 4 endW
        hptr hbound hencInner hlong hn4 hitem4 hoff4_sl hspan4a hroom4
        (fun n l hd => hhnext.2.2.2.2 endW n l hd)
  have hq_abs : ambientAbsOff off srcOff4 + 1 = 8 * q := by
    simpa [srcOff4, listOff, ambientAbsOff] using hq_align
  have hnext_sl_content : ∀ (next4 len4 : Word),
      rlpItemDecode slice srcOff4 (loadPtr + BitVec.ofNat 64 srcOff4) endW
        next4 len4 →
      next4 = loadPtr + BitVec.ofNat 64 srcOff4 + (1 : Word) + (20 : Word) := by
    intro n l hd
    let contentPtrSl := loadPtr + BitVec.ofNat 64 srcOff4 + (1 : Word)
    have hcontentPtr : contentPtrSl =
        loadPtr + BitVec.ofNat 64 srcOff4 + (1 : Word) := rfl
    have hsrc : srcOff4 = longListSrcOff listOff items 4 := rfl
    have hc := extractSuccess_copy_t1_hnext_content_long slice loadPtr contentPtrSl
      srcOff4 hsuccess hcopyFlag htype1 items hdecL hlong hsrc hcontentPtr
  ...
  have hnext_content : ∀ (next4 len4 : Word),
      rlpItemDecode bs absOff4 (regionBase + BitVec.ofNat 64 absOff4) endW
        next4 len4 →
      next4 = regionBase + BitVec.ofNat 64 (8 * q) + (20 : Word) := by
    simpa [absOff4, srcOff4, listOff, endW, absListOff] using
      hnext_content_ambient_of_slice_long regionBase loadPtr bs off len listOff items 4 q endW
        hptr hbound hencInner hlong hn4 hitem4 hoff4_sl hspan4a hroom4 hq_abs hcover20
        hnext_sl_content
  have hinb0 :=
...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_longConcrete_pure_t1_ambient_fullCode
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (items : List EL.RLP.RLPItem)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_copy_longConcrete_pure_t1_ambient sp0 spC s
