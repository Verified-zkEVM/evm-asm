/-
  EvmAsm.Codegen.Programs.ReceiptExtractLogsBloomSpec

  The whole-program caller contract `receiptExtractLogsBloom_spec_within` for
  `receiptExtractLogsBloom_prog` (`Bloom.lean`, 46 instrs).  The routine extracts
  the receipt's `logs_bloom` field (RLP field index 2, a 256-byte value) via one
  ...
  Byte-identical to the `header_extract_logs_bloom` sibling up to the field index
  (2-vs-6), the base address, and the scratch-cell names, so this module is a
  copy-adapt of `HeaderExtractLogsBloomSpec`.  It reuses:
    - `ReceiptExtractLogsBloomCallSAsm.receiptExtractLogsBloom_call_spec_within`
      for the single K20 `rlp_list_nth_item` call (already proven);
    - the field-extractors' `copyIntoRegion` content-tie machinery
...
namespace EvmAsm.Codegen.ReceiptExtractLogsBloomSpec
...
abbrev relbBase : Word := (Codegen.GuestAddrs.receipt_extract_logs_bloom : Word)
/-- Guest entry of `rlp_list_nth_item`. -/
abbrev K20B : Word := (Codegen.GuestAddrs.rlp_list_nth_item : Word)
/-- The global scratch offset cell. -/
abbrev relbOffAddr : Word := (Codegen.GuestAddrs.relb_offset : Word)
/-- The global scratch length cell. -/
abbrev relbLenAddr : Word := (Codegen.GuestAddrs.relb_length : Word)

theorem program_length : Codegen.receiptExtractLogsBloom_prog.length = 46 := by decide

/-- The `receipt_extract_logs_bloom` body at its linked guest address. -/
def wrapperCode : CodeReq := CodeReq.ofProg relbBase Codegen.receiptExtractLogsBloom_prog
...
theorem relbMem (prog : List Instr) (hprog : prog = Codegen.receiptExtractLogsBloom_prog)
    (A : Word) (k : Nat) (ins : Instr)
    (hk : k < prog.length)
    (hA : A = relbBase + BitVec.ofNat 64 (4 * k))
    (hins : prog[k]'hk = ins) :
  ...
  exact CodeReq.ofProg_mem_at relbBase A Codegen.receiptExtractLogsBloom_prog k ins hA hk hins
    (by rw [program_length]; norm_num) a i hs

/-! ## The nth-item call ([15], `relbBase+60 → relbBase+64`)

    The single K20 `rlp_list_nth_item` call on field index 2 is provided directly
    by `ReceiptExtractLogsBloomCallSAsm.receiptExtractLogsBloom_call_spec_within`
    (already proven), used at the composition point in the main theorem below. -/

/-! ## `la` materialize helpers (address-specific AUIPC hi/lo) -/

/-- `la x13, relb_offset` at [11]-[12] (`relbBase+44 → relbBase+52`). -/
theorem relbLaOff44 (v : Word) :
    cpsTripleWithin 2 (relbBase + 44) (relbBase + 52) fullCode
      (.x13 ↦ᵣ v) (.x13 ↦ᵣ relbOffAddr) := by
  have hau := relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 44) 11
    (.AUIPC .x13 (Codegen.laHi Codegen.GuestAddrs.relb_offset
      (Codegen.GuestAddrs.receipt_extract_logs_bloom + 44)))
    (by rw [program_length]; norm_num) (by bv_omega) rfl
  have had := relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 48) 12
    (.ADDI .x13 .x13 (Codegen.laLo Codegen.GuestAddrs.relb_offset
      (Codegen.GuestAddrs.receipt_extract_logs_bloom + 44)))
    (by rw [program_length]; norm_num) (by bv_omega) rfl
  have h := la_materialize_within .x13 v (relbBase + 44) relbOffAddr
    (by decide) (by decide) hau had
  rw [show (relbBase + 44 : Word) + 8 = relbBase + 52 from by bv_omega] at h
  exact h

/-- `la x14, relb_length` at [13]-[14] (`relbBase+52 → relbBase+60`). -/
theorem relbLaLen52 (v : Word) :
    cpsTripleWithin 2 (relbBase + 52) (relbBase + 60) fullCode
      (.x14 ↦ᵣ v) (.x14 ↦ᵣ relbLenAddr) := by
  have hau := relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 52) 13
    (.AUIPC .x14 (Codegen.laHi Codegen.GuestAddrs.relb_length
      (Codegen.GuestAddrs.receipt_extract_logs_bloom + 52)))
    (by rw [program_length]; norm_num) (by bv_omega) rfl
  have had := relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 56) 14
    (.ADDI .x14 .x14 (Codegen.laLo Codegen.GuestAddrs.relb_length
      (Codegen.GuestAddrs.receipt_extract_logs_bloom + 52)))
    (by rw [program_length]; norm_num) (by bv_omega) rfl
  have h := la_materialize_within .x14 v (relbBase + 52) relbLenAddr
    (by decide) (by decide) hau had
  rw [show (relbBase + 52 : Word) + 8 = relbBase + 60 from by bv_omega] at h
  exact h

/-! ## Prologue [0]-[14] (`relbBase → relbBase+60`)

    Allocate the 32-byte frame, save `ra/s0/s1/s2`, marshal the ABI inputs into
    the K20 argument registers (`x10=rlp ptr`, `x11=len`, `x12=2`), and
    materialize the two scratch-cell addresses (`x13=relb_offset`,
    `x14=relb_length`).  The post is exactly the K20 call precondition
    (`callEntryRest`), plus the saved frame and the pass-through output region. -/
set_option maxRecDepth 8000 in
theorem relbPrologue
    (sp0 listBase listLenW outPtr : Word) (fsaved : HeaderFieldsSpec.Saved)
    (s3 s4 s5 v13 v14 oldOffset oldLen : Word)
    (receiptBytes outBytes : List (BitVec 8)) (newSp : Word)
    (h_newSp : newSp = sp0 + signExtend12 (-32 : BitVec 12)) :
    cpsTripleWithin 15 relbBase (relbBase + 60) fullCode
      ((.x2 ↦ᵣ sp0) ** regsAt HeaderFieldsSpec.hxFrame
        ...
        bytesRegion listBase receiptBytes ** bytesRegion outPtr outBytes **
        stackFree newSp 8 ** (relbOffAddr ↦ₘ oldOffset) ** (relbLenAddr ↦ₘ oldLen))
      (((.x1 ↦ᵣ fsaved.ra) **
        callEntryRest newSp listBase listLenW (2 : Word) relbOffAddr relbLenAddr
          oldOffset oldLen
          { ra := fsaved.ra, s0 := listBase, s1 := listLenW, s2 := outPtr,
            s3 := s3, s4 := s4, s5 := s5 } receiptBytes) **
        HeaderFieldsSpec.savedFrame newSp fsaved ** bytesRegion outPtr outBytes) := by
  -- [0] addi sp, sp, -32
  have ha0 := addi_spec_gen_same_within .x2 sp0 (-32 : BitVec 12) relbBase (by decide)
  rw [← h_newSp] at ha0
  have ha := cpsTripleWithin_extend_code
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl relbBase 0
      (.ADDI .x2 .x2 (-32 : BitVec 12)) (by rw [program_length]; norm_num) (by bv_omega) rfl) ha0
  rw [show relbBase + 4 = relbBase + 4 from rfl] at ha
  ...
  -- [1]-[4] store sequence
  have hstoreMono : ∀ a i,
      CodeReq.ofProg (relbBase + 4) (storeProg HeaderFieldsSpec.hxFrame) a = some i →
        fullCode a = some i := by
    intro a i h_mem
    unfold fullCode
    apply CodeReq.union_mono_left
    exact CodeReq.ofProg_mono_sub relbBase (relbBase + 4)
      Codegen.receiptExtractLogsBloom_prog (storeProg HeaderFieldsSpec.hxFrame) 1
  ...
  have hs0 := storeSeq_spec HeaderFieldsSpec.hxFrame newSp
    (HeaderFieldsSpec.savedVals fsaved) (relbBase + 4) (by decide)
  have hs := cpsTripleWithin_extend_code hstoreMono hs0
  rw [show relbBase + 4 + BitVec.ofNat 64 (4 * HeaderFieldsSpec.hxFrame.length) = relbBase + 20 from by
  ...
  -- [5]-[9] register moves
  have hm := HeaderFieldsSpec.hfSetupMoves5 (code := fullCode) relbBase listBase listLenW outPtr
    fsaved.s0 fsaved.s1 fsaved.s2
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 20) 5 (.MV .x8 .x10)
      (by rw [program_length]; norm_num) (by bv_omega) rfl)
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 24) 6 (.MV .x9 .x11)
      (by rw [program_length]; norm_num) (by bv_omega) rfl)
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 28) 7 (.MV .x18 .x12)
      (by rw [program_length]; norm_num) (by bv_omega) rfl)
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 32) 8 (.MV .x10 .x8)
      (by rw [program_length]; norm_num) (by bv_omega) rfl)
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 36) 9 (.MV .x11 .x9)
      (by rw [program_length]; norm_num) (by bv_omega) rfl)
  ...
  -- [10] li x12, 6
  have hli := li_spec_gen_within .x12 outPtr (2 : Word) (relbBase + 40) (by decide)
  have hlie := cpsTripleWithin_extend_code
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 40) 10 (.LI .x12 (2 : Word))
      (by rw [program_length]; norm_num) (by bv_omega) rfl) hli
  rw [show (relbBase + 40 : Word) + 4 = relbBase + 44 from by bv_omega] at hlie
  ...
  -- [11]-[12] la x13, relb_offset ; [13]-[14] la x14, relb_length
  have hla13 := relbLaOff44 v13
  have hla13F := cpsTripleWithin_frameR
    ((.x2 ↦ᵣ newSp) ** (.x1 ↦ᵣ fsaved.ra) ** HeaderFieldsSpec.savedFrame newSp fsaved **
      (.x8 ↦ᵣ listBase) ** (.x9 ↦ᵣ listLenW) ** (.x18 ↦ᵣ outPtr) **
      (.x10 ↦ᵣ listBase) ** (.x11 ↦ᵣ listLenW) ** (.x12 ↦ᵣ (2 : Word)) **
  ...
  have hla14 := relbLaLen52 v14
  have hla14F := cpsTripleWithin_frameR
    ((.x2 ↦ᵣ newSp) ** (.x1 ↦ᵣ fsaved.ra) ** HeaderFieldsSpec.savedFrame newSp fsaved **
      (.x8 ↦ᵣ listBase) ** (.x9 ↦ᵣ listLenW) ** (.x18 ↦ᵣ outPtr) **
      (.x10 ↦ᵣ listBase) ** (.x11 ↦ᵣ listLenW) ** (.x12 ↦ᵣ (2 : Word)) **
      (.x13 ↦ᵣ relbOffAddr) ** (.x19 ↦ᵣ s3) ** (.x20 ↦ᵣ s4) ** (.x21 ↦ᵣ s5) **
      regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** regOwn .x28 ** regOwn .x29 **
      regOwn .x30 ** regOwn .x31 ** (.x0 ↦ᵣ (0 : Word)) **
      bytesRegion listBase receiptBytes ** bytesRegion outPtr outBytes **
      stackFree newSp 8 ** (relbOffAddr ↦ₘ oldOffset) ** (relbLenAddr ↦ₘ oldLen))
...
/-! ## The shared return postcondition
    ...
    on success (a0=0, len=256) the output holds the 256 field-2 content bytes
    (`copyIntoRegion`); on a0=2 the field-2 length was ≠256 (output unchanged);
    on a0=1 the RLP parse failed. -/
def relbRetPost (newSp listBase outPtr : Word) (fsaved : HeaderFieldsSpec.Saved)
    (receiptBytes outBytes : List (BitVec 8)) (listLen : Nat) : Assertion :=
  fun h => ∃ (a0v : Word) (finalOut : List (BitVec 8)) (fo len : Word) (junk : Assertion),
    ((((.x10 ↦ᵣ a0v) ** (.x2 ↦ᵣ (newSp + 32)) ** (.x1 ↦ᵣ fsaved.ra) **
      ...
      bytesRegion listBase receiptBytes ** bytesRegion outPtr finalOut ** junk) **
     ⌜(a0v = (0 : Word) ∧ Success receiptBytes listBase listLen 2 fo len ∧
          len = (256 : Word) ∧
          finalOut = copyIntoRegion outBytes receiptBytes 0 fo.toNat 256) ∨
       (a0v = (2 : Word) ∧ Success receiptBytes listBase listLen 2 fo len ∧
          len ≠ (256 : Word) ∧ finalOut = outBytes) ∨
       (a0v = (1 : Word) ∧ Failure receiptBytes listBase listLen 2)⌝) h

/-! ## Epilogue [40]-[45] (`relbBase+160 → return`)

    Restore `ra/s0/s1/s2`, deallocate the 32-byte frame, `ret`.  The status `a0`
    and the framed rest `Fr` are carried untouched. -/
set_option maxRecDepth 8000 in
theorem relbEpilogue (newSp a0v v1 v8 v9 v18 : Word) (fsaved : HeaderFieldsSpec.Saved)
    (Fr : Assertion) (hFr : Fr.pcFree) :
    cpsTripleWithin 6 (relbBase + 160) (fsaved.ra &&& ~~~(1 : Word)) fullCode
  ...
  have hc0 := relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 160) 40
    (.LD .x1 .x2 (0 : BitVec 12)) (by rw [program_length]; norm_num) (by bv_omega) rfl
  have hc1 := relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 164) 41
    (.LD .x8 .x2 (8 : BitVec 12)) (by rw [program_length]; norm_num) (by bv_omega) rfl
  have hc2 := relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 168) 42
    (.LD .x9 .x2 (16 : BitVec 12)) (by rw [program_length]; norm_num) (by bv_omega) rfl
  have hc3 := relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 172) 43
    (.LD .x18 .x2 (24 : BitVec 12)) (by rw [program_length]; norm_num) (by bv_omega) rfl
  have hc4 := relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 176) 44
    (.ADDI .x2 .x2 (32 : BitVec 12)) (by rw [program_length]; norm_num) (by bv_omega) rfl
  have hc5 := relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 180) 45
    (.JALR .x0 .x1 (0 : BitVec 12)) (by rw [program_length]; norm_num) (by bv_omega) rfl
  -- [ld ra, 0(sp)]
  have hl0 := ld_spec_gen_within .x1 .x2 newSp v1 fsaved.ra (0 : BitVec 12) (relbBase + 160) (by decide)
  ...
  -- [ld s0, 8(sp)]
  have hl1 := ld_spec_gen_within .x8 .x2 newSp v8 fsaved.s0 (8 : BitVec 12) (relbBase + 164) (by decide)
  rw [show newSp + signExtend12 (8 : BitVec 12) = newSp + 8 from by
        rw [show signExtend12 (8 : BitVec 12) = (8 : Word) from by decide],
      show (relbBase + 164 : Word) + 4 = relbBase + 168 from by bv_omega] at hl1
  ...
  -- [ld s1, 16(sp)]
  have hl2 := ld_spec_gen_within .x9 .x2 newSp v9 fsaved.s1 (16 : BitVec 12) (relbBase + 168) (by decide)
  rw [show newSp + signExtend12 (16 : BitVec 12) = newSp + 16 from by
        rw [show signExtend12 (16 : BitVec 12) = (16 : Word) from by decide],
      show (relbBase + 168 : Word) + 4 = relbBase + 172 from by bv_omega] at hl2
  ...
  -- [ld s2, 24(sp)]
  have hl3 := ld_spec_gen_within .x18 .x2 newSp v18 fsaved.s2 (24 : BitVec 12) (relbBase + 172) (by decide)
  rw [show newSp + signExtend12 (24 : BitVec 12) = newSp + 24 from by
        rw [show signExtend12 (24 : BitVec 12) = (24 : Word) from by decide],
      show (relbBase + 172 : Word) + 4 = relbBase + 176 from by bv_omega] at hl3
  ...
  -- [addi sp, sp, 32]
  have haddi := addi_spec_gen_same_within .x2 newSp (32 : BitVec 12) (relbBase + 176) (by decide)
  rw [show newSp + signExtend12 (32 : BitVec 12) = newSp + 32 from by
      rw [show signExtend12 (32 : BitVec 12) = (32 : Word) from by decide],
    show (relbBase + 176 : Word) + 4 = relbBase + 180 from by bv_omega] at haddi
  ...
  -- [jalr x0, 0(x1)]
  have hjalr := jalr_x0_spec_gen_within .x1 fsaved.ra (0 : BitVec 12) (relbBase + 180)
...
private theorem relbCopyBody5 (srcBase dstBase x31old : Word)
    ...
    cpsTripleWithin 5 (relbBase + 116) (relbBase + 136) fullCode
  ...
  -- [29] lbu x31, 0(x28)
  have hlbu := bytesRegion_lbu_within .x31 .x28 srcBase x31old (relbBase + 116)
    srcBytes (srcOff + i) (by decide) h_src_align h_src_lt (by omega)
    (h_src_valid (srcOff + i) h_src_lt)
  rw [show (relbBase + 116 : Word) + 4 = relbBase + 120 from by bv_omega, ← hbval] at hlbu
  have hlbue := cpsTripleWithin_extend_code
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 116) 29
  ...
  -- [30] sb x31, 0(x29)
  have hsb := bytesRegion_sb_within .x29 .x31 dstBase (bval.zeroExtend 64) (relbBase + 120)
  ...
  rw [htrunc, ← hstep, show (relbBase + 120 : Word) + 4 = relbBase + 124 from by bv_omega] at hsb
  have hsbe := cpsTripleWithin_extend_code
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 120) 30
  ...
  have h3e := cpsTripleWithin_extend_code
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 124) 31
  ...
  have h4e := cpsTripleWithin_extend_code
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 128) 32
...
set_option maxRecDepth 8000 in
/-- The while-copy loop closure ([28]-[34], `relbBase+112 → relbBase+140`):
...
private theorem relbCopyLoop (srcBase dstBase x31old : Word)
    ...
    (h_dst_valid : ∀ k, k < dstBytes.length →
      isValidByteAccess (dstBase + BitVec.ofNat 64 k) = true) :
    cpsTripleWithin (7 * n + 1) (relbBase + 112) (relbBase + 140) fullCode
  ...
  have ha_taken : (relbBase + 112 : Word) + signExtend13 (28 : BitVec 13) = relbBase + 140 := by
    rw [show signExtend13 (28 : BitVec 13) = (28 : Word) from by decide]; bv_omega
  have ha_fall : (relbBase + 112 : Word) + 4 = relbBase + 116 := by bv_omega
  have hbeqMem : ∀ a i', CodeReq.singleton (relbBase + 112) (.BEQ .x30 .x0 (28 : BitVec 13)) a = some i'
      → fullCode a = some i' :=
    relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 112) 28
  ...
  | succ k ih =>
    ...
    have hbody := relbCopyBody5 srcBase dstBase x31old srcBytes dstBytes srcOff dstOff i k
      h_src_align h_dst_align (by omega) (by omega) h_src_over h_dst_over h_src_valid h_dst_valid
    have hjal := jal_x0_spec_gen_within (-24 : BitVec 21) (relbBase + 136)
    rw [show (relbBase + 136 : Word) + signExtend21 (-24 : BitVec 21) = relbBase + 112 from by
      decide] at hjal
    have hjale := cpsTripleWithin_extend_code
      (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 136) 34
...
/-- `la x5, relb_length` at [17]-[18] (`relbBase+68 → relbBase+76`). -/
theorem relbLaLen68 (v : Word) :
    cpsTripleWithin 2 (relbBase + 68) (relbBase + 76) fullCode
      (.x5 ↦ᵣ v) (.x5 ↦ᵣ relbLenAddr) := by
  have hau := relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 68) 17
    (.AUIPC .x5 (Codegen.laHi Codegen.GuestAddrs.relb_length
      (Codegen.GuestAddrs.receipt_extract_logs_bloom + 68)))
    (by rw [program_length]; norm_num) (by bv_omega) rfl
  have had := relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 72) 18
    (.ADDI .x5 .x5 (Codegen.laLo Codegen.GuestAddrs.relb_length
      (Codegen.GuestAddrs.receipt_extract_logs_bloom + 68)))
    (by rw [program_length]; norm_num) (by bv_omega) rfl
  have h := la_materialize_within .x5 v (relbBase + 68) relbLenAddr
    (by decide) (by decide) hau had
  rw [show (relbBase + 68 : Word) + 8 = relbBase + 76 from by bv_omega] at h
  exact h

/-- `la x5, relb_offset` at [22]-[23] (`relbBase+88 → relbBase+96`). -/
theorem relbLaOff88 (v : Word) :
    cpsTripleWithin 2 (relbBase + 88) (relbBase + 96) fullCode
      (.x5 ↦ᵣ v) (.x5 ↦ᵣ relbOffAddr) := by
  have hau := relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 88) 22
    (.AUIPC .x5 (Codegen.laHi Codegen.GuestAddrs.relb_offset
      (Codegen.GuestAddrs.receipt_extract_logs_bloom + 88)))
    (by rw [program_length]; norm_num) (by bv_omega) rfl
  have had := relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 92) 23
    (.ADDI .x5 .x5 (Codegen.laLo Codegen.GuestAddrs.relb_offset
      (Codegen.GuestAddrs.receipt_extract_logs_bloom + 88)))
    (by rw [program_length]; norm_num) (by bv_omega) rfl
  have h := la_materialize_within .x5 v (relbBase + 88) relbOffAddr
    (by decide) (by decide) hau had
...
private theorem relbTail0 (v10old newSp v1 v8 v9 v18 : Word) (fsaved : HeaderFieldsSpec.Saved)
    (Fr : Assertion) (hFr : Fr.pcFree) :
    cpsTripleWithin (2 + 6) (relbBase + 140) (fsaved.ra &&& ~~~(1 : Word)) fullCode
      (((.x10 ↦ᵣ v10old) ** (.x2 ↦ᵣ newSp) ** (.x1 ↦ᵣ v1) ** (.x8 ↦ᵣ v8) **
        (.x9 ↦ᵣ v9) ** (.x18 ↦ᵣ v18) ** HeaderFieldsSpec.savedFrame newSp fsaved) ** Fr)
      (((.x10 ↦ᵣ (0 : Word)) ** (.x2 ↦ᵣ (newSp + 32)) ** (.x1 ↦ᵣ fsaved.ra) **
        (.x8 ↦ᵣ fsaved.s0) ** (.x9 ↦ᵣ fsaved.s1) ** (.x18 ↦ᵣ fsaved.s2) **
        HeaderFieldsSpec.savedFrame newSp fsaved) ** Fr) := by
  have hli := li_spec_gen_within .x10 v10old (0 : Word) (relbBase + 140) (by decide)
  have hlie := cpsTripleWithin_extend_code
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 140) 35 (.LI .x10 (0 : Word))
      (by rw [program_length]; norm_num) (by bv_omega) rfl) hli
  rw [show (relbBase + 140 : Word) + 4 = relbBase + 144 from by bv_omega] at hlie
  have hliF := cpsTripleWithin_frameR
  ...
  have hjal := jal_x0_spec_gen_within (16 : BitVec 21) (relbBase + 144)
  rw [show (relbBase + 144 : Word) + signExtend21 (16 : BitVec 21) = relbBase + 160 from by decide] at hjal
  have hjale := cpsTripleWithin_extend_code
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 144) 36 (.JAL .x0 (16 : BitVec 21))
  ...
  rw [sepConj_emp_left'] at hjalf
  have hepi := relbEpilogue newSp (0 : Word) v1 v8 v9 v18 fsaved Fr hFr
...
private theorem relbTail1 (v10old newSp v1 v8 v9 v18 : Word) (fsaved : HeaderFieldsSpec.Saved)
    (Fr : Assertion) (hFr : Fr.pcFree) :
    cpsTripleWithin (2 + 6) (relbBase + 148) (fsaved.ra &&& ~~~(1 : Word)) fullCode
      (((.x10 ↦ᵣ v10old) ** (.x2 ↦ᵣ newSp) ** (.x1 ↦ᵣ v1) ** (.x8 ↦ᵣ v8) **
        (.x9 ↦ᵣ v9) ** (.x18 ↦ᵣ v18) ** HeaderFieldsSpec.savedFrame newSp fsaved) ** Fr)
      (((.x10 ↦ᵣ (1 : Word)) ** (.x2 ↦ᵣ (newSp + 32)) ** (.x1 ↦ᵣ fsaved.ra) **
        (.x8 ↦ᵣ fsaved.s0) ** (.x9 ↦ᵣ fsaved.s1) ** (.x18 ↦ᵣ fsaved.s2) **
        HeaderFieldsSpec.savedFrame newSp fsaved) ** Fr) := by
  have hli := li_spec_gen_within .x10 v10old (1 : Word) (relbBase + 148) (by decide)
  have hlie := cpsTripleWithin_extend_code
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 148) 37 (.LI .x10 (1 : Word))
      (by rw [program_length]; norm_num) (by bv_omega) rfl) hli
  rw [show (relbBase + 148 : Word) + 4 = relbBase + 152 from by bv_omega] at hlie
  have hliF := cpsTripleWithin_frameR
    (((.x2 ↦ᵣ newSp) ** (.x1 ↦ᵣ v1) ** (.x8 ↦ᵣ v8) ** (.x9 ↦ᵣ v9) ** (.x18 ↦ᵣ v18) **
      HeaderFieldsSpec.savedFrame newSp fsaved) ** Fr)
    (by unfold HeaderFieldsSpec.savedFrame
        repeat' first | exact hFr | exact pcFree_regIs | exact pcFree_memIs | apply pcFree_sepConj) hlie
  have hjal := jal_x0_spec_gen_within (8 : BitVec 21) (relbBase + 152)
  rw [show (relbBase + 152 : Word) + signExtend21 (8 : BitVec 21) = relbBase + 160 from by decide] at hjal
  have hjale := cpsTripleWithin_extend_code
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 152) 38 (.JAL .x0 (8 : BitVec 21))
      (by rw [program_length]; norm_num) (by bv_omega) rfl) hjal
  have hjalf := cpsTripleWithin_frameR
    ...
    (by unfold HeaderFieldsSpec.savedFrame
        repeat' first | exact hFr | exact pcFree_regIs | exact pcFree_memIs | apply pcFree_sepConj) hjale
  rw [sepConj_emp_left'] at hjalf
  have hepi := relbEpilogue newSp (1 : Word) v1 v8 v9 v18 fsaved Fr hFr
  have s1 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hliF hjalf
  have s2 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) s1 hepi
  exact cpsTripleWithin_mono_nSteps (by omega)
    (cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp) (fun _ hp => by xperm_hyp hp) s2)
...
private theorem relbTail2 (v10old newSp v1 v8 v9 v18 : Word) (fsaved : HeaderFieldsSpec.Saved)
    (Fr : Assertion) (hFr : Fr.pcFree) :
    cpsTripleWithin (1 + 6) (relbBase + 156) (fsaved.ra &&& ~~~(1 : Word)) fullCode
      (((.x10 ↦ᵣ v10old) ** (.x2 ↦ᵣ newSp) ** (.x1 ↦ᵣ v1) ** (.x8 ↦ᵣ v8) **
        (.x9 ↦ᵣ v9) ** (.x18 ↦ᵣ v18) ** HeaderFieldsSpec.savedFrame newSp fsaved) ** Fr)
      (((.x10 ↦ᵣ (2 : Word)) ** (.x2 ↦ᵣ (newSp + 32)) ** (.x1 ↦ᵣ fsaved.ra) **
        (.x8 ↦ᵣ fsaved.s0) ** (.x9 ↦ᵣ fsaved.s1) ** (.x18 ↦ᵣ fsaved.s2) **
        HeaderFieldsSpec.savedFrame newSp fsaved) ** Fr) := by
  have hli := li_spec_gen_within .x10 v10old (2 : Word) (relbBase + 156) (by decide)
  have hlie := cpsTripleWithin_extend_code
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 156) 39 (.LI .x10 (2 : Word))
      (by rw [program_length]; norm_num) (by bv_omega) rfl) hli
  rw [show (relbBase + 156 : Word) + 4 = relbBase + 160 from by bv_omega] at hlie
  have hliF := cpsTripleWithin_frameR
    (((.x2 ↦ᵣ newSp) ** (.x1 ↦ᵣ v1) ** (.x8 ↦ᵣ v8) ** (.x9 ↦ᵣ v9) ** (.x18 ↦ᵣ v18) **
      HeaderFieldsSpec.savedFrame newSp fsaved) ** Fr)
    (by unfold HeaderFieldsSpec.savedFrame
        repeat' first | exact hFr | exact pcFree_regIs | exact pcFree_memIs | apply pcFree_sepConj) hlie
  have hepi := relbEpilogue newSp (2 : Word) v1 v8 v9 v18 fsaved Fr hFr
...
private theorem cpsTripleWithin_callReturn_pre
    ...
    (h : ∀ status offset len v11 v12,
        ...
        cpsTripleWithin N (relbBase + 64) ret fullCode
          (((.x1 ↦ᵣ X) **
...
/-! ## Post-call: length load ([17]-[20], `relbBase+68 → relbBase+84`)

    `la x5, relb_length ; ld x6, 0(x5) ; li x7, 256`. -/
private theorem relbLenBlock (len v5old v6old v7old : Word) :
    cpsTripleWithin 4 (relbBase + 68) (relbBase + 84) fullCode
      ((.x5 ↦ᵣ v5old) ** (.x6 ↦ᵣ v6old) ** (.x7 ↦ᵣ v7old) ** (relbLenAddr ↦ₘ len))
      ((.x5 ↦ᵣ relbLenAddr) ** (.x6 ↦ᵣ len) ** (.x7 ↦ᵣ (256 : Word)) ** (relbLenAddr ↦ₘ len)) := by
  have f0 := cpsTripleWithin_frameR
    ((.x6 ↦ᵣ v6old) ** (.x7 ↦ᵣ v7old) ** (relbLenAddr ↦ₘ len))
    (by repeat' first | exact pcFree_regIs | exact pcFree_memIs | apply pcFree_sepConj)
    (relbLaLen68 v5old)
  have h1 := ld_spec_gen_within .x6 .x5 relbLenAddr v6old len (0 : BitVec 12)
    (relbBase + 76) (by decide)
  rw [signExtend12_0, show (relbLenAddr + 0 : Word) = relbLenAddr from by bv_omega,
      show (relbBase + 76 : Word) + 4 = relbBase + 80 from by bv_omega] at h1
  have e1 := cpsTripleWithin_extend_code
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 76) 19
      (.LD .x6 .x5 (0 : BitVec 12)) (by rw [program_length]; norm_num) (by bv_omega) rfl) h1
  have f1 := cpsTripleWithin_frameR ((.x7 ↦ᵣ v7old))
    pcFree_regIs e1
  have h2 := li_spec_gen_within .x7 v7old (256 : Word) (relbBase + 80) (by decide)
  rw [show (relbBase + 80 : Word) + 4 = relbBase + 84 from by bv_omega] at h2
  have e2 := cpsTripleWithin_extend_code
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 80) 20
      (.LI .x7 (256 : Word)) (by rw [program_length]; norm_num) (by bv_omega) rfl) h2
  have f2 := cpsTripleWithin_frameR
    ((.x5 ↦ᵣ relbLenAddr) ** (.x6 ↦ᵣ len) ** (relbLenAddr ↦ₘ len))
...
/-! ## Post-call: offset load + content-pointer setup ([22]-[27],
    `relbBase+88 → relbBase+112`)

    `la x5, relb_offset ; ld x6, 0(x5) ; add x28, x8, x6 ; mv x29, x18 ;
     li x30, 256`. -/
private theorem relbOffsetSetup (offset listBase outPtr v5old v6old v28old v29old v30old : Word) :
    cpsTripleWithin 6 (relbBase + 88) (relbBase + 112) fullCode
      ((.x5 ↦ᵣ v5old) ** (.x6 ↦ᵣ v6old) ** (.x8 ↦ᵣ listBase) ** (.x18 ↦ᵣ outPtr) **
       (.x28 ↦ᵣ v28old) ** (.x29 ↦ᵣ v29old) ** (.x30 ↦ᵣ v30old) ** (relbOffAddr ↦ₘ offset))
      ((.x5 ↦ᵣ relbOffAddr) ** (.x6 ↦ᵣ offset) ** (.x8 ↦ᵣ listBase) ** (.x18 ↦ᵣ outPtr) **
       (.x28 ↦ᵣ (listBase + offset)) ** (.x29 ↦ᵣ outPtr) ** (.x30 ↦ᵣ (256 : Word)) **
       (relbOffAddr ↦ₘ offset)) := by
  have f0 := cpsTripleWithin_frameR
    ((.x6 ↦ᵣ v6old) ** (.x8 ↦ᵣ listBase) ** (.x18 ↦ᵣ outPtr) ** (.x28 ↦ᵣ v28old) **
     (.x29 ↦ᵣ v29old) ** (.x30 ↦ᵣ v30old) ** (relbOffAddr ↦ₘ offset))
    (by repeat' first | exact pcFree_regIs | exact pcFree_memIs | apply pcFree_sepConj)
    (relbLaOff88 v5old)
  have h1 := ld_spec_gen_within .x6 .x5 relbOffAddr v6old offset (0 : BitVec 12)
    (relbBase + 96) (by decide)
  rw [signExtend12_0, show (relbOffAddr + 0 : Word) = relbOffAddr from by bv_omega,
      show (relbBase + 96 : Word) + 4 = relbBase + 100 from by bv_omega] at h1
  have e1 := cpsTripleWithin_extend_code
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 96) 24
      (.LD .x6 .x5 (0 : BitVec 12)) (by rw [program_length]; norm_num) (by bv_omega) rfl) h1
  ...
  have h2 := add_spec_gen_within .x28 .x8 .x6 listBase offset v28old (relbBase + 100) (by decide)
  rw [show (relbBase + 100 : Word) + 4 = relbBase + 104 from by bv_omega] at h2
  have e2 := cpsTripleWithin_extend_code
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 100) 25
      (.ADD .x28 .x8 .x6) (by rw [program_length]; norm_num) (by bv_omega) rfl) h2
  have f2 := cpsTripleWithin_frameR
    ((.x5 ↦ᵣ relbOffAddr) ** (.x18 ↦ᵣ outPtr) ** (.x29 ↦ᵣ v29old) ** (.x30 ↦ᵣ v30old) **
     (relbOffAddr ↦ₘ offset))
    (by repeat' first | exact pcFree_regIs | exact pcFree_memIs | apply pcFree_sepConj) e2
  have h3 := mv_spec_gen_within .x29 .x18 outPtr v29old (relbBase + 104) (by decide)
  rw [show (relbBase + 104 : Word) + 4 = relbBase + 108 from by bv_omega] at h3
  have e3 := cpsTripleWithin_extend_code
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 104) 26
      (.MV .x29 .x18) (by rw [program_length]; norm_num) (by bv_omega) rfl) h3
  have f3 := cpsTripleWithin_frameR
    ((.x5 ↦ᵣ relbOffAddr) ** (.x6 ↦ᵣ offset) ** (.x8 ↦ᵣ listBase) **
     (.x28 ↦ᵣ (listBase + offset)) ** (.x30 ↦ᵣ v30old) ** (relbOffAddr ↦ₘ offset))
    (by repeat' first | exact pcFree_regIs | exact pcFree_memIs | apply pcFree_sepConj) e3
  have h4 := li_spec_gen_within .x30 v30old (256 : Word) (relbBase + 108) (by decide)
  rw [show (relbBase + 108 : Word) + 4 = relbBase + 112 from by bv_omega] at h4
  have e4 := cpsTripleWithin_extend_code
    (relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 108) 27
      (.LI .x30 (256 : Word)) (by rw [program_length]; norm_num) (by bv_omega) rfl) h4
  have f4 := cpsTripleWithin_frameR
    ((.x5 ↦ᵣ relbOffAddr) ** (.x6 ↦ᵣ offset) ** (.x8 ↦ᵣ listBase) ** (.x18 ↦ᵣ outPtr) **
     (.x28 ↦ᵣ (listBase + offset)) ** (.x29 ↦ᵣ outPtr) ** (relbOffAddr ↦ₘ offset))
...
private theorem relbCopyThenTail0
    ...
    cpsTripleWithin ((7 * 256 + 1) + (2 + 6)) (relbBase + 112) (fsaved.ra &&& ~~~(1 : Word)) fullCode
      ...
      (((.x10 ↦ᵣ (0 : Word)) ** (.x2 ↦ᵣ (newSp + 32)) ** (.x1 ↦ᵣ fsaved.ra) **
       ...
       ((.x30 ↦ᵣ (0 : Word)) ** (.x28 ↦ᵣ (listBase + BitVec.ofNat 64 (offset.toNat + 256))) **
        ...
        bytesRegion listBase receiptBytes **
        bytesRegion outPtr (copyIntoRegion outBytes receiptBytes 0 offset.toNat 256) ** Fr)) := by
  have hcopy := relbCopyLoop listBase outPtr x31old receiptBytes outBytes offset.toNat 0 256 0
  ...
  have htail := relbTail0 a0old newSp v1 v8 v9 outPtr fsaved
    ((.x30 ↦ᵣ (0 : Word)) ** (.x28 ↦ᵣ (listBase + BitVec.ofNat 64 (offset.toNat + 256))) **
     (.x29 ↦ᵣ (outPtr + (256 : Word))) ** regOwn .x31 ** (.x0 ↦ᵣ (0 : Word)) **
     bytesRegion listBase receiptBytes **
     bytesRegion outPtr (copyIntoRegion outBytes receiptBytes 0 offset.toNat 256) ** Fr)
    (by repeat' first
      | exact hFr | exact pcFree_regIs | exact pcFree_regOwn | exact pcFree_memIs
...
/-! ## Post-call: the a0=0 success continuation ([22]-[36]+epilogue,
    `relbBase+88 → return`) -/
set_option maxRecDepth 8000 in
private theorem relbSuccessContinue
    ...
    (h_dst_valid : ∀ k, k < outBytes.length →
      isValidByteAccess (outPtr + BitVec.ofNat 64 k) = true) :
    cpsTripleWithin (6 + ((7 * 256 + 1) + (2 + 6))) (relbBase + 88) (fsaved.ra &&& ~~~(1 : Word))
      fullCode
      ((.x5 ↦ᵣ v5old) ** (.x6 ↦ᵣ v6old) ** (.x8 ↦ᵣ listBase) ** (.x18 ↦ᵣ outPtr) **
       (.x28 ↦ᵣ v28old) ** (.x29 ↦ᵣ v29old) ** (.x30 ↦ᵣ v30old) ** (.x31 ↦ᵣ x31old) **
       (.x0 ↦ᵣ (0 : Word)) ** (relbOffAddr ↦ₘ offset) **
       bytesRegion listBase receiptBytes ** bytesRegion outPtr outBytes **
      ...
      (((.x10 ↦ᵣ (0 : Word)) ** (.x2 ↦ᵣ (newSp + 32)) ** (.x1 ↦ᵣ fsaved.ra) **
       ...
       ((.x30 ↦ᵣ (0 : Word)) ** (.x28 ↦ᵣ (listBase + BitVec.ofNat 64 (offset.toNat + 256))) **
        (.x29 ↦ᵣ (outPtr + (256 : Word))) ** regOwn .x31 ** (.x0 ↦ᵣ (0 : Word)) **
        bytesRegion listBase receiptBytes **
        bytesRegion outPtr (copyIntoRegion outBytes receiptBytes 0 offset.toNat 256) **
        ((.x5 ↦ᵣ relbOffAddr) ** (.x6 ↦ᵣ offset) ** (relbOffAddr ↦ₘ offset) ** Fr))) := by
  have hsetup := relbOffsetSetup offset listBase outPtr v5old v6old v28old v29old v30old
  have hsetupF := cpsTripleWithin_frameR
    ((.x31 ↦ᵣ x31old) ** (.x0 ↦ᵣ (0 : Word)) ** bytesRegion listBase receiptBytes **
  ...
  have hctf := relbCopyThenTail0 offset listBase outPtr newSp x31old listBase v9 a0old v1 fsaved
    receiptBytes outBytes ((.x5 ↦ᵣ relbOffAddr) ** (.x6 ↦ᵣ offset) ** (relbOffAddr ↦ₘ offset) ** Fr)
    (by repeat' first
...
/-! ## Post-call: length-check dispatch ([21]→ret, `relbBase+84`)
    ...
    (a0=2).  Both arms carry the abstract `Success` fact into `relbRetPost`. -/
set_option maxRecDepth 8000 in
private theorem relbLenDispatch
    (offset len listBase outPtr newSp v5old v28old v29old v30old x31old a0old v1 v9 : Word)
    (fsaved : HeaderFieldsSpec.Saved) (receiptBytes outBytes : List (BitVec 8)) (listLen : Nat)
    (Fr : Assertion) (hFr : Fr.pcFree)
    (h_src_align : listBase.toNat % 8 = 0)
    (h_dst_align : outPtr.toNat % 8 = 0)
    (h_src_bound : offset.toNat + 256 ≤ receiptBytes.length)
    ...
    (h_dst_valid : ∀ k, k < outBytes.length →
      isValidByteAccess (outPtr + BitVec.ofNat 64 k) = true)
    (hSucc : Success receiptBytes listBase listLen 2 offset len) :
    cpsTripleWithin (1 + (6 + ((7 * 256 + 1) + (2 + 6)))) (relbBase + 84)
      ...
      ((.x6 ↦ᵣ len) ** (.x7 ↦ᵣ (256 : Word)) ** (.x5 ↦ᵣ v5old) ** (.x8 ↦ᵣ listBase) **
       ...
       (.x31 ↦ᵣ x31old) ** (.x0 ↦ᵣ (0 : Word)) ** (relbOffAddr ↦ₘ offset) **
       (relbLenAddr ↦ₘ len) ** bytesRegion listBase receiptBytes ** bytesRegion outPtr outBytes **
       (.x10 ↦ᵣ a0old) ** (.x2 ↦ᵣ newSp) ** (.x1 ↦ᵣ v1) ** (.x9 ↦ᵣ v9) **
       HeaderFieldsSpec.savedFrame newSp fsaved ** Fr)
      (relbRetPost newSp listBase outPtr fsaved receiptBytes outBytes listLen) := by
  have ha_t : (relbBase + 84 : Word) + signExtend13 (72 : BitVec 13) = relbBase + 156 := by
    rw [show signExtend13 (72 : BitVec 13) = (72 : Word) from by decide]; bv_omega
  have ha_f : (relbBase + 84 : Word) + 4 = relbBase + 88 := by bv_omega
  have hmono : ∀ a i', CodeReq.singleton (relbBase + 84) (.BNE .x6 .x7 (72 : BitVec 13)) a = some i'
      → fullCode a = some i' :=
    relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 84) 21
      (.BNE .x6 .x7 (72 : BitVec 13)) (by rw [program_length]; norm_num) (by bv_omega) rfl
  have hbne := bne_spec_gen_within .x6 .x7 (72 : BitVec 13) len (256 : Word) (relbBase + 84)
  ...
  · -- fall through: success path (a0=0)
    ...
    have hntF := cpsTripleWithin_frameR
      ((.x5 ↦ᵣ v5old) ** (.x8 ↦ᵣ listBase) ** (.x18 ↦ᵣ outPtr) ** (.x28 ↦ᵣ v28old) **
       ...
       (relbOffAddr ↦ₘ offset) ** (relbLenAddr ↦ₘ len) ** bytesRegion listBase receiptBytes **
       bytesRegion outPtr outBytes ** (.x10 ↦ᵣ a0old) ** (.x2 ↦ᵣ newSp) ** (.x1 ↦ᵣ v1) **
       (.x9 ↦ᵣ v9) ** HeaderFieldsSpec.savedFrame newSp fsaved ** Fr)
      (by unfold HeaderFieldsSpec.savedFrame; repeat' first
        | exact hFr | exact pcFree_regIs | exact pcFree_regOwn | exact pcFree_memIs
        | exact bytesRegion_pcFree _ _ | apply pcFree_sepConj) hnt
    have hsucc := relbSuccessContinue offset listBase outPtr newSp v5old len v28old v29old v30old
      x31old v9 a0old v1 fsaved receiptBytes outBytes
      ((.x7 ↦ᵣ (256 : Word)) ** (relbLenAddr ↦ₘ len) ** Fr)
      (by repeat' first
        | exact hFr | exact pcFree_regIs | exact pcFree_memIs | apply pcFree_sepConj)
      h_src_align h_dst_align h_src_bound h_dst_bound h_src_over h_dst_over h_src_valid h_dst_valid
    ...
    refine cpsTripleWithin_mono_nSteps (by omega)
      (cpsTripleWithin_weaken (fun _ hp => by xperm_chunked hp)
        (fun h hq => ?_) s)
    refine ⟨(0 : Word), copyIntoRegion outBytes receiptBytes 0 offset.toNat 256, offset, len,
      (.x30 ↦ᵣ (0 : Word)) ** (.x28 ↦ᵣ (listBase + BitVec.ofNat 64 (offset.toNat + 256))) **
      (.x29 ↦ᵣ (outPtr + (256 : Word))) ** regOwn .x31 ** (.x0 ↦ᵣ (0 : Word)) **
      (.x5 ↦ᵣ relbOffAddr) ** (.x6 ↦ᵣ offset) ** (relbOffAddr ↦ₘ offset) **
      (.x7 ↦ᵣ (256 : Word)) ** (relbLenAddr ↦ₘ len) ** Fr, ?_⟩
  ...
  · -- branch taken: length-mismatch tail (a0=2)
    ...
    have htail := relbTail2 a0old newSp v1 listBase v9 outPtr fsaved
...
private theorem relbOkArm
    ...
    (h_dst_valid : ∀ k, k < outBytes.length →
      isValidByteAccess (outPtr + BitVec.ofNat 64 k) = true)
    (hSucc : Success receiptBytes listBase listLen 2 offset len) :
    cpsTripleWithin (1 + (4 + (1 + (6 + ((7 * 256 + 1) + (2 + 6)))))) (relbBase + 64)
      (fsaved.ra &&& ~~~(1 : Word)) fullCode
      ((.x10 ↦ᵣ (0 : Word)) ** (.x0 ↦ᵣ (0 : Word)) ** (.x5 ↦ᵣ v5) ** (.x6 ↦ᵣ v6) **
       (.x7 ↦ᵣ v7) ** (.x8 ↦ᵣ listBase) ** (.x18 ↦ᵣ outPtr) ** (.x28 ↦ᵣ v28) **
       (.x29 ↦ᵣ v29) ** (.x30 ↦ᵣ v30) ** (.x31 ↦ᵣ v31) ** (relbOffAddr ↦ₘ offset) **
       (relbLenAddr ↦ₘ len) ** bytesRegion listBase receiptBytes ** (.x2 ↦ᵣ newSp) **
       (.x1 ↦ᵣ v1) ** (.x9 ↦ᵣ v9) ** HeaderFieldsSpec.savedFrame newSp fsaved **
       bytesRegion outPtr outBytes ** Fr)
      (relbRetPost newSp listBase outPtr fsaved receiptBytes outBytes listLen) := by
  -- [16] bne x10, x0, +84 : not taken (x10 = 0)
  have ha_t : (relbBase + 64 : Word) + signExtend13 (84 : BitVec 13) = relbBase + 148 := by
    rw [show signExtend13 (84 : BitVec 13) = (84 : Word) from by decide]; bv_omega
  have ha_f : (relbBase + 64 : Word) + 4 = relbBase + 68 := by bv_omega
  have hmono : ∀ a i', CodeReq.singleton (relbBase + 64) (.BNE .x10 .x0 (84 : BitVec 13)) a = some i'
      → fullCode a = some i' :=
    relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 64) 16
      (.BNE .x10 .x0 (84 : BitVec 13)) (by rw [program_length]; norm_num) (by bv_omega) rfl
  have hbne := bne_spec_gen_within .x10 .x0 (84 : BitVec 13) (0 : Word) (0 : Word) (relbBase + 64)
  rw [ha_t, ha_f] at hbne
  have hbnee := cpsBranchWithin_extend_code hmono hbne
  have hnt := cpsBranchWithin_ntakenStripPure2 hbnee (fun hp hQt => by
    obtain ⟨_, _, _, _, _, hQ⟩ := hQt
  ...
  have hntF := cpsTripleWithin_frameR
    ...
    (by unfold HeaderFieldsSpec.savedFrame; repeat' first
      ...
      | exact bytesRegion_pcFree _ _ | apply pcFree_sepConj) hnt
  -- [17]-[20] length load
  have hlb := relbLenBlock len v5 v6 v7
  have hlbF := cpsTripleWithin_frameR
    ((.x10 ↦ᵣ (0 : Word)) ** (.x0 ↦ᵣ (0 : Word)) ** (.x8 ↦ᵣ listBase) ** (.x18 ↦ᵣ outPtr) **
     (.x28 ↦ᵣ v28) ** (.x29 ↦ᵣ v29) ** (.x30 ↦ᵣ v30) ** (.x31 ↦ᵣ v31) **
     (relbOffAddr ↦ₘ offset) ** bytesRegion listBase receiptBytes **
     (.x2 ↦ᵣ newSp) ** (.x1 ↦ᵣ v1) ** (.x9 ↦ᵣ v9) ** HeaderFieldsSpec.savedFrame newSp fsaved **
     bytesRegion outPtr outBytes ** Fr)
    (by unfold HeaderFieldsSpec.savedFrame; repeat' first
      | exact hFr | exact pcFree_regIs | exact pcFree_regOwn | exact pcFree_memIs
  ...
  have hdisp := relbLenDispatch offset len listBase outPtr newSp relbLenAddr v28 v29 v30 v31
    (0 : Word) v1 v9 fsaved receiptBytes outBytes listLen Fr hFr
    h_src_align h_dst_align h_src_bound h_dst_bound h_src_over h_dst_over h_src_valid h_dst_valid
    hSucc
...
/-! ## Post-call outcome ([16]→ret, `relbBase+64`)
...
private theorem relbPostCallOutcome
    ...
    (h_dst_valid : ∀ k, k < outBytes.length →
      isValidByteAccess (outPtr + BitVec.ofNat 64 k) = true)
    (hbound : ∀ fo ln, Success receiptBytes listBase listLen 2 fo ln →
      fo.toNat + 256 ≤ receiptBytes.length) :
    cpsTripleWithin (1 + (4 + (1 + (6 + ((7 * 256 + 1) + (2 + 6)))))) (relbBase + 64)
      (fsaved.ra &&& ~~~(1 : Word)) fullCode
      (((.x1 ↦ᵣ (relbBase + 64)) **
        callReturnResult newSp listBase indexW relbOffAddr relbLenAddr oldOffset oldLen csaved
          receiptBytes listLen 2) **
       (HeaderFieldsSpec.savedFrame newSp fsaved ** bytesRegion outPtr outBytes))
      (relbRetPost newSp listBase outPtr fsaved receiptBytes outBytes listLen) := by
  apply cpsTripleWithin_callReturn_pre newSp listBase indexW relbOffAddr relbLenAddr
    oldOffset oldLen csaved receiptBytes listLen 2
    ...
    refine cpsTripleWithin_weaken (fun h hp => by
      ...
      (cpsTripleWithin_of_forall_regIs_to_regOwn7
        (P := (.x10 ↦ᵣ (0 : Word)) ** (.x0 ↦ᵣ (0 : Word)) ** (.x8 ↦ᵣ listBase) **
          (.x18 ↦ᵣ outPtr) ** (relbOffAddr ↦ₘ offset) ** (relbLenAddr ↦ₘ len) **
          bytesRegion listBase receiptBytes ** (.x2 ↦ᵣ newSp) ** (.x1 ↦ᵣ (relbBase + 64)) **
        ...
        (fun v5 v6 v7 v28 v29 v30 v31 =>
          cpsTripleWithin_weaken (fun h hp => by xperm_chunked hp) (fun _ hq => hq)
            (relbOkArm offset len listBase outPtr newSp v5 v6 v7 v28 v29 v30 v31 (relbBase + 64)
              csaved.s1 fsaved receiptBytes outBytes listLen
              ((.x11 ↦ᵣ v11) ** (.x12 ↦ᵣ v12) ** regOwn .x13 ** regOwn .x14 **
               (.x19 ↦ᵣ csaved.s3) ** (.x20 ↦ᵣ csaved.s4) ** (.x21 ↦ᵣ csaved.s5) **
               stackFree newSp 8)
              (by repeat' first
  ...
  | fail hFail =>
    ...
    have ha_f : (relbBase + 64 : Word) + 4 = relbBase + 68 := by bv_omega
    have hmono : ∀ a i',
        CodeReq.singleton (relbBase + 64) (.BNE .x10 .x0 (84 : BitVec 13)) a = some i'
        → fullCode a = some i' :=
      relbMem Codegen.receiptExtractLogsBloom_prog rfl (relbBase + 64) 16
        (.BNE .x10 .x0 (84 : BitVec 13)) (by rw [program_length]; norm_num) (by bv_omega) rfl
    have hbne := bne_spec_gen_within .x10 .x0 (84 : BitVec 13) (1 : Word) (0 : Word) (relbBase + 64)
    rw [ha_t, ha_f] at hbne
    have hbnee := cpsBranchWithin_extend_code hmono hbne
    have htk := cpsBranchWithin_takenStripPure2 hbnee (fun hp hQf => by
      obtain ⟨_, _, _, _, _, hQ⟩ := hQf
    ...
    have htkF := cpsTripleWithin_frameR
      ((.x1 ↦ᵣ (relbBase + 64)) ** (.x2 ↦ᵣ newSp) ** stackFree newSp 8 ** (.x8 ↦ᵣ listBase) **
       ...
       regOwn .x31 ** bytesRegion listBase receiptBytes ** (relbOffAddr ↦ₘ oldOffset) **
       (relbLenAddr ↦ₘ oldLen) ** HeaderFieldsSpec.savedFrame newSp fsaved **
       bytesRegion outPtr outBytes)
      (by unfold HeaderFieldsSpec.savedFrame; repeat' first
        | exact pcFree_regIs | exact pcFree_regOwn | exact pcFree_memIs | exact pcFree_stackFree _ _
        | exact bytesRegion_pcFree _ _ | apply pcFree_sepConj) htk
    have htail := relbTail1 (1 : Word) newSp (relbBase + 64) listBase csaved.s1 outPtr fsaved
      ((.x0 ↦ᵣ (0 : Word)) ** stackFree newSp 8 ** (.x19 ↦ᵣ csaved.s3) ** (.x20 ↦ᵣ csaved.s4) **
       (.x21 ↦ᵣ csaved.s5) ** regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** (.x11 ↦ᵣ v11) **
       (.x12 ↦ᵣ v12) ** regOwn .x13 ** regOwn .x14 ** regOwn .x28 ** regOwn .x29 ** regOwn .x30 **
       regOwn .x31 ** bytesRegion listBase receiptBytes ** (relbOffAddr ↦ₘ oldOffset) **
       (relbLenAddr ↦ₘ oldLen) ** bytesRegion outPtr outBytes)
      (by repeat' first
        | exact pcFree_regIs | exact pcFree_regOwn | exact pcFree_memIs | exact pcFree_stackFree _ _
        | exact bytesRegion_pcFree _ _ | apply pcFree_sepConj)
    have s := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_chunked hp) htkF htail
    ...
    refine ⟨(1 : Word), outBytes, oldOffset, oldLen,
      ...
      (.x21 ↦ᵣ csaved.s5) ** regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** (.x11 ↦ᵣ v11) **
      (.x12 ↦ᵣ v12) ** regOwn .x13 ** regOwn .x14 ** regOwn .x28 ** regOwn .x29 ** regOwn .x30 **
      regOwn .x31 ** (relbOffAddr ↦ₘ oldOffset) ** (relbLenAddr ↦ₘ oldLen), ?_⟩
    exact (sepConj_pure_right _).2 ⟨by xperm_chunked hq, Or.inr (Or.inr ⟨rfl, hFail⟩)⟩

/-! ## Whole-program caller contract

    `relbPrologue [0]-[14] ;; the K20 nth-item call [15] ;; the post-call outcome
    [16]→ret`, composed into one raw-pinned `cpsTripleWithin` from `relbBase` to
    the function return (`fsaved.ra &&& ~~~1`).  On the a0=0 success path the
    output region holds the genuine 256-byte field-2 content
    (`copyIntoRegion outBytes receiptBytes 0 offset.toNat 256`), tied to the K20
    `Success … 2 offset 256` fact; a0=2 means the field length was ≠256; a0=1
    means the RLP parse failed. -/
set_option maxRecDepth 8000 in
theorem receiptExtractLogsBloom_spec_within
    (sp0 newSp listBase outPtr : Word) (fsaved : HeaderFieldsSpec.Saved)
    (s3 s4 s5 v13 v14 oldOffset oldLen : Word)
    (receiptBytes outBytes : List (BitVec 8)) (listLen : Nat)
    (h_src_align : listBase.toNat % 8 = 0)
    (h_dst_align : outPtr.toNat % 8 = 0)
    (h_slack : listLen + 9 ≤ receiptBytes.length)
    (h_src_over : listBase.toNat + receiptBytes.length < 2 ^ 64)
    (h_dst_over : outPtr.toNat + outBytes.length < 2 ^ 64)
    (h_dst_bound : 256 ≤ outBytes.length)
    (h_src_valid : ∀ k, k < receiptBytes.length →
    ...
    (h_newSp : newSp = sp0 + signExtend12 (-32 : BitVec 12)) :
    cpsTripleWithin
      ((15 + (1 + ((12 + ((85 + 93 * (2 + 2)) + 6)) + 9))) +
        (1 + (4 + (1 + (6 + ((7 * 256 + 1) + (2 + 6)))))))
      relbBase (fsaved.ra &&& ~~~(1 : Word)) fullCode
      ((.x2 ↦ᵣ sp0) ** regsAt HeaderFieldsSpec.hxFrame
          (HeaderFieldsSpec.savedVals fsaved) **
        frameSlotsOwn HeaderFieldsSpec.hxFrame newSp **
        ...
        stackFree newSp 8 ** (relbOffAddr ↦ₘ oldOffset) ** (relbLenAddr ↦ₘ oldLen))
      (relbRetPost newSp listBase outPtr fsaved receiptBytes outBytes listLen) := by
  have hpro := relbPrologue sp0 listBase (BitVec.ofNat 64 listLen) outPtr fsaved s3 s4 s5 v13 v14
    oldOffset oldLen receiptBytes outBytes newSp h_newSp
  have hcall := ReceiptExtractLogsBloomCallSAsm.receiptExtractLogsBloom_call_spec_within
    newSp listBase (BitVec.ofNat 64 listLen)
    (2 : Word) oldOffset oldLen fsaved.ra
    { ra := fsaved.ra, s0 := listBase, s1 := BitVec.ofNat 64 listLen, s2 := outPtr,
      s3 := s3, s4 := s4, s5 := s5 } receiptBytes listLen 2
    (HeaderFieldsSpec.savedFrame newSp fsaved ** bytesRegion outPtr outBytes)
    (by repeat' first
  ...
  have hpost := relbPostCallOutcome listBase outPtr newSp (2 : Word) oldOffset oldLen
    { ra := relbBase + 64, s0 := listBase, s1 := BitVec.ofNat 64 listLen, s2 := outPtr,
      s3 := s3, s4 := s4, s5 := s5 } fsaved receiptBytes outBytes listLen
