/-
  EvmAsm.Codegen.Programs.Secp256k1PointZero64SAsm

  Verified SAsm port of `secp256k1_point_zero64`: zero a 64-byte secp256k1
  affine point buffer using the emitted alignment-free byte loop.
-/
...
namespace Secp256k1PointZero64SAsm
...
#guard (secp256k1PointZero64_verified : List Instr).length = 6
#guard (secp256k1PointZero64Body 0 []).flatten 0 =
  (secp256k1PointZero64Body 0 []).flatten 0x80000000
#guard (secp256k1PointZero64Body 0 []).flatten 0 ++ [Instr.JALR .x0 .x1 0] =
  secp256k1PointZero64_prog

theorem secp256k1PointZero64Fn_spec (dst : Word) (orig : List (BitVec 8))
    (hwf : RwRegion.wf ⟨dst, 64⟩) (base : Word) :
    (secp256k1PointZero64Fn dst orig).Spec base := by
  have hbase : (secp256k1PointZero64Fn dst orig).rw.base = dst := rfl
  vcgen
