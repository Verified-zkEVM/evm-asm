/-
  ExtractAssumed packaging under short concrete of_decode E2E (no ∀endPtr).
-/
...
/-- Assumed creation short concrete of_decode (no ∀endPtr on hcur/hdec/hinb). -/
theorem extractAssumed_creation_under_honesty_of_decode_short_concrete
  ...
  have hShort := extractFrontCreation_then_epi_of_decode_short_concrete sp0 spC s
  ...
  have hle : nFrontCreationStepsShort ≤ nFrontCreationSteps := by
    simp only [nFrontCreationStepsShort, nFrontCreationSteps, nTypeSteps]
    omega
  have h := cpsTripleWithin_mono_nSteps hle hShort
...
theorem extractAssumed_creation_fullCode_of_decode_short_concrete
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr : Word)
    (txBytes : List (BitVec 8))
    (srcOff0 srcOff1 srcOff2 srcOff3 srcOff4 srcOff5 : Nat)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_creation_under_honesty_of_decode_short_concrete sp0 spC s
      txBase lenW toBuf isCreationPtr txBytes
      srcOff0 srcOff1 srcOff2 srcOff3 srcOff4 srcOff5
      hspC hret hcur hne0 hne1 hsalign
      hoff0 hover0 hvalid0 hss0 hls0 hll0 hdec0 hinb0
...
#print axioms extractAssumed_creation_under_honesty_of_decode_short_concrete
#print axioms extractAssumed_creation_fullCode_of_decode_short_concrete
