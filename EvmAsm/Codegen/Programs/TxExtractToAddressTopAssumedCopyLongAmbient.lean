/-
  Ambient Assumed packaging for type234 long 20B copy (regionBase/loadPtr).
  Reuses copyE2EPostAmbient / assumed Pre/Post Ambient.
-/
...
theorem nFrontCopyStepsLongRegion_le_nExtract (lol : Nat) (hlol : lol ≤ 8) :
    nFrontCopyStepsLongRegion lol ≤ nExtractSteps := by
  simp only [nFrontCopyStepsLongRegion, nExtractSteps, nTypeSteps]
  omega
  -- lol ≤ 8 ⇒ 7*lol+25 ≤ 81 (= short full walk_init budget)

set_option maxRecDepth 8000 in
theorem extractAssumed_copy_concrete_long_region_ambient
    ...
    (hlol : lol ≤ 8)
    (hE2E : cpsTripleWithin (nFrontCopyStepsLongRegion lol) E s.ra extractLinkedCode
  ...
  have h1 := cpsTripleWithin_mono_nSteps (nFrontCopyStepsLongRegion_le_nExtract lol hlol) hE2E
...
private theorem of_forall_regOwn6_long_amb
...
private def assumedCoreCopyLongAmbient (sp0 : Word) (s : ExtractSaved)
...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_temps_long_region_ambient
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr contentPtr w2 : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (lol : Nat)
    ...
    (hE2E : ∀ (old5 old6 old7 old14 old15 old16 : Word),
      cpsTripleWithin (nFrontCopyStepsLongRegion lol) E s.ra extractLinkedCode
        (creationE2EPreAmbient sp0 spC s regionBase loadPtr lenW toBuf isCreationPtr
          old5 old6 old7 old14 old15 old16 bs)
        (copyE2EPostAmbient sp0 s regionBase toBuf isCreationPtr contentPtr w2
          bs off len)) :
  ...
  let Q := extractAssumedPostAmbient s.ra sp0
        s.s0 s.s1 s.s2 s.s3 s.s4 s.s5 s.s6 s.s7
        regionBase toBuf isCreationPtr bs
  let Core := assumedCoreCopyLongAmbient sp0 s regionBase loadPtr lenW toBuf isCreationPtr bs
  have htemps : cpsTripleWithin nExtractSteps E s.ra extractLinkedCode
    ...
    refine of_forall_regOwn6_long_amb (r1 := .x5) (r2 := .x6) (r3 := .x7)
      (r4 := .x14) (r5 := .x15) (r6 := .x16) (fun old5 old6 old7 old14 old15 old16 => ?_)
    have hc := extractAssumed_copy_concrete_long_region_ambient sp0 spC s regionBase loadPtr lenW
    ...
    refine cpsTripleWithin_weaken (fun _ hp => by
      dsimp [Core, assumedCoreCopyLongAmbient, assumedPreConcreteAmbient] at hp ⊢
      xperm_hyp hp) (fun _ hq => by
      dsimp [Q] at hq ⊢; exact hq) hc
  refine cpsTripleWithin_weaken (fun _ hp => by
    simp only [extractAssumedPreAmbient] at hp ⊢
    dsimp [Core, assumedCoreCopyLongAmbient] at hp ⊢
    xperm_hyp hp) (fun _ hq => by
    dsimp [Q] at hq ⊢; exact hq) htemps

set_option maxRecDepth 8000 in
/-- Wire Assumed bare under long concrete copy E2E of_decode region. -/
theorem extractAssumed_copy_of_front_long_concrete_region_ambient
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (bs : List (BitVec 8)) (off len : Nat)
  ...
  let lol := ((bs[ambientAbsOff off (teerTxTypeDispatch (txSlice bs off len)).2.2.toNat]'hoff).zeroExtend 64 -
      (0xf7 : Word)).toNat
  refine extractAssumed_copy_temps_long_region_ambient sp0 spC s regionBase loadPtr lenW toBuf
    isCreationPtr contentPtr w2 bs off len lol hspC hlol
    (fun old5 old6 old7 old14 old15 old16 => ?_)
  have hE := extractFrontCopy_then_epi_of_decode_long_concrete_region_ambient
...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_fullCode_of_decode_long_concrete_region_ambient
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (absOff0 absOff1 absOff2 absOff3 absOff4 absOff5 q : Nat)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_copy_of_front_long_concrete_region_ambient sp0 spC s
      regionBase loadPtr lenW toBuf isCreationPtr bs off len
      absOff0 absOff1 absOff2 absOff3 absOff4 absOff5 q
      hspC hret hwi_off hcur hne0 hne1 hsalign
      hoff0 hover0 hvalid0 hss0 hls0 hll0 hdec0 hinb0
      hoff1 hover1 hvalid1 hss1 hls1 hll1 hdec1 hinb1
      hoff2 hover2 hvalid2 hss2 hls2 hll2 hdec2 hinb2
      hoff3 hover3 hvalid3 hss3 hls3 hll3 hdec3 hinb3
...
#print axioms extractAssumed_copy_of_front_long_concrete_region_ambient
#print axioms extractAssumed_copy_fullCode_of_decode_long_concrete_region_ambient
