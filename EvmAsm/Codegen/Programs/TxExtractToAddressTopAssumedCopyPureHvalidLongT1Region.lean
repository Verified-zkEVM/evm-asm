...
/-- Bare Assumed long t1 copy under pure honesty + dword-aligned content.
    `hq_align : longListSrcOff listOff items 4 + 1 = 8 * q`. classical-3. -/
theorem extractAssumed_copy_longConcrete_pureHvalid_t1_region
    ...
    (hq_align : longListSrcOff (teerTxTypeDispatch txBytes).2.2.toNat items 4 + 1 = 8 * q)
    ...
    (hitem0 : (encode (items[0]'(by
        have := extractSuccess_copy_t1_items_length_long txBytes hsuccess
  ...
  have hhoff :=
    extractSuccess_copy_t1_hoff_srcOff_long txBytes hsuccess hcopyFlag htype1
      items hdecL hlong
  have hhover :=
    extractSuccess_copy_t1_hover_srcOff_long txBytes txBase hsuccess hcopyFlag htype1
      items hdecL hlong hover
  have hlenItems :=
    extractSuccess_copy_t1_items_length_long txBytes hsuccess hcopyFlag htype1
      items hdecL hlong
  ...
  have hfields03 :=
    extractSuccess_copy_t1_hnext_fields03_long txBytes hsuccess hcopyFlag htype1
      items hdecL hlong
  have hendEq :
  ...
  have hcur :
    ...
    simpa [longWalkCursor, srcOff0, listOff] using
      longWalkCursor_eq_srcOff0 txBytes txBase listOff items hencInner hlong hbound
        hoff hoverC
  have hhnext :=
    extractSuccess_copy_t1_hnext_hlen20_srcOff_long txBytes txBase hsuccess
  ...
  have hnext_content : ∀ (next4 len4 : Word),
    ...
    intro n l hd
    have hc := extractSuccess_copy_t1_hnext_content_long txBytes txBase contentPtr
      srcOff4 hsuccess hcopyFlag htype1 items hdecL hlong rfl hcontentPtr endW n l hd
    simpa [contentPtr] using hc
  have hitem4 :
      (encode (items[4]'hn4)).length ≤ 55 := by
    have hf := extractSuccess_copy_t1_field4_pfx94_long txBytes hsuccess hcopyFlag htype1
      items hdecL hlong
  ...
  have hv :=
    extractSuccess_copy_t1_hvalid_srcOff_long txBytes txBase hsuccess
      hcopyFlag htype1 items hdecL hlong hge6 hvalidBuf
  have hss0 :=
    hss_of_long_list_item txBytes txBase listOff items 0 hencInner hlong hn0 hitem0
      hhoff.1 hover (Or.inl hfields03.1) hv.2.1
  have hss1 :=
    hss_of_long_list_item txBytes txBase listOff items 1 hencInner hlong hn1 hitem1
      hhoff.2.1 hover (Or.inl hfields03.2.1) hv.2.2.2.1
  have hss2 :=
    hss_of_long_list_item txBytes txBase listOff items 2 hencInner hlong hn2 hitem2
      hhoff.2.2.1 hover (Or.inl hfields03.2.2.1) hv.2.2.2.2.2.1
  have hss3 :=
    hss_of_long_list_item txBytes txBase listOff items 3 hencInner hlong hn3 hitem3
      hhoff.2.2.2.1 hover (Or.inl hfields03.2.2.2) hv.2.2.2.2.2.2.2.1
  have hss4 :=
  ...
  exact extractAssumed_copy_of_front_long_concrete_t1_region
    sp0 spC s txBase lenW toBuf isCreationPtr txBytes
    srcOff0 srcOff1 srcOff2 srcOff3 srcOff4 q
    hspC hret hoff hcur htype1 halign
...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_longConcrete_pureHvalid_t1_region_fullCode
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr : Word)
    (txBytes : List (BitVec 8))
    (items : List EL.RLP.RLPItem)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_copy_longConcrete_pureHvalid_t1_region
      sp0 spC s txBase lenW toBuf isCreationPtr txBytes items q
      hspC hret htalign htover htvalid hq_align hq hcover hcvalid
      hlen hsuccess hcopyFlag htype1 hdecL hlong
      halign hover hvalidBuf
      hitem0 hitem1 hitem2 hitem3 hge6)

#print axioms extractAssumed_copy_longConcrete_pureHvalid_t1_region
#print axioms extractAssumed_copy_longConcrete_pureHvalid_t1_region_fullCode

/-- Path refinements for long t1 copy + dword-aligned content. -/
def extractCopyT1LongPathRegion
  ...
  extractSuccess txBytes ∧
    ...
    6 ≤ items.length ∧
    longListSrcOff (teerTxTypeDispatch txBytes).2.2.toNat items 4 + 1 = 8 * q ∧
    8 * q + 16 < txBytes.length

set_option maxRecDepth 8000 in
/-- Bare Assumed footprint under long t1 copy + aligned content. classical-3. -/
theorem extractAssumed_success_flat_copy_t1_long
    ...
    (hcvalid : isValidMemAccess
      (txBase + BitVec.ofNat 64 (8 * q) + (16 : Word)) = true)
    (hpath : extractCopyT1LongPathRegion txBytes items q)
    (hitem0 : (encode (items[0]'(by
  ...
  simpa only [hsra, hs0, hs1, hs2, hs3, hs4, hs5, hs6, hs7] using
    extractAssumed_copy_longConcrete_pureHvalid_t1_region_fullCode
...
#print axioms extractAssumed_success_flat_copy_t1_long
