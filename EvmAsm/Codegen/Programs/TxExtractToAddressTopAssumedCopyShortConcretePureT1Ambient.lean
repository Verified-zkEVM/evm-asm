...
/-- Ambient short concrete Assumed creation with pure-discharged hcur/hnext/hlen20/hnext_content/hinb/
    hoff/hover/hls/hll/hne and short walk guards. Residual: hvalid*/hvalid1_*/hdec*. -/
theorem extractAssumed_copy_shortConcrete_pure_t1_region_ambient
  ...
  have hfields03 :=
    extractSuccess_copy_t1_hnext_fields03 slice hsuccess hcopyFlag htype1
  ...
  have hhnext :=
    extractSuccess_copy_t1_hnext_hlen20_srcOff slice loadPtr hsuccess
  ...
  have hlen20 : ∀ (next4 len4 : Word),
    ...
    simpa [absOff4, srcOff4, listOff, endW, absListOff] using
      hlen20_ambient_of_slice regionBase loadPtr bs off len listOff items 4 endW
  ...
  have hnext_sl_content : ∀ (next4 len4 : Word),
    ...
    intro n l hd
    have hsrc : srcOff4 =
        (teerTxTypeDispatch slice).2.2.toNat + 1 + encodeItemsPrefixLen items 4 := by
      simp only [srcOff4, shortListSrcOff, listOff]
    have hc := extractSuccess_copy_t1_hnext_content slice loadPtr
      (loadPtr + BitVec.ofNat 64 srcOff4 + (1 : Word)) srcOff4
      hsuccess hcopyFlag htype1 items hdecL hshort hsrc rfl
  ...
  have hnext_content : ∀ (next4 len4 : Word),
    ...
    simpa [absOff4, srcOff4, listOff, endW, absListOff] using
      hnext_content_ambient_of_slice regionBase loadPtr bs off len listOff items 4 q endW
        hptr hbound hencInner hshort hn4 hoff4_sl hspan4a hroom4 hq_abs hcover20
        hnext_sl_content
  have hinb0 :=
  ...
  have hss4 :=
    hss_ambient_of_short_list_field regionBase loadPtr bs off len listOff items 4
      hptr hbound hencInner hshort hn4 hoff4_sl hover (Or.inl (by omega : (4 : Nat) + 1 < items.length))
      hvalid1_4 hoff4
  have hls0 :
      ¬ BitVec.ult ((bs[absOff0]'hoff0).zeroExtend 64) (0xb8 : Word) = true →
        BitVec.ult ((bs[absOff0]'hoff0).zeroExtend 64) (0xc0 : Word) = true →
  ...
  exact extractAssumed_copy_under_honesty_of_decode_short_concrete_t1_region_ambient
...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_shortConcrete_pure_t1_region_ambient_fullCode
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (items : List EL.RLP.RLPItem)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_copy_shortConcrete_pure_t1_region_ambient sp0 spC s
...
#print axioms extractAssumed_copy_shortConcrete_pure_t1_region_ambient
#print axioms extractAssumed_copy_shortConcrete_pure_t1_region_ambient_fullCode
