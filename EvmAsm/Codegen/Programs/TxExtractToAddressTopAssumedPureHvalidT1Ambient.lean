/-
  Ambient short t1 creation: discharge hdec*/hvalid* via slice pure + bridges +
  validByteRange on regionBase. Residual path flags (creation/t1/short/hge6).
-/
...
/-- Ambient short concrete Assumed creation; hdec+hvalid from pure+validByteRange.
    Residual path: creation/t1/short/hge6/hdecL. -/
theorem extractAssumed_creation_shortConcrete_pureHvalid_t1_ambient
  ...
  simpa [absOff0, absOff1, absOff2, absOff3, absOff4, absListOff,
    ...
    extractAssumed_creation_shortConcrete_pure_t1_ambient
...
set_option maxRecDepth 8000 in
theorem extractAssumed_creation_shortConcrete_pureHvalid_t1_ambient_fullCode
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (items : List EL.RLP.RLPItem)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_creation_shortConcrete_pureHvalid_t1_ambient
      sp0 spC s regionBase loadPtr lenW toBuf isCreationPtr bs off len items
      hspC hret htalign htover htvalid hlen hptr hbound hsuccess hcreFlag htype1 hdecL hshort
      hsalign hover hvalidBuf hge6)

/-- Path refinements for ambient short t1 creation arm. -/
def extractCreationT1ShortPathAmbient
...
set_option maxRecDepth 8000 in
/-- Ambient Assumed footprint under short t1 creation path (statics + path).
    classical-3. Residual: other extractSuccess arms (copy/t1/long). -/
theorem extractAssumed_success_flat_creation_t1_short_ambient
  ...
  simpa only [hsra, hs0, hs1, hs2, hs3, hs4, hs5, hs6, hs7] using
    extractAssumed_creation_shortConcrete_pureHvalid_t1_ambient_fullCode
      spVal spC s regionBase loadPtr lenW toBuf isCreationPtr bs off len items
      hspC hret htalign htover htvalid hlen hptr hbound hsuccess hcreFlag htype1 hdecL hshort
      hsalign hover hvalidBuf hge6

#print axioms extractAssumed_creation_shortConcrete_pureHvalid_t1_ambient
#print axioms extractAssumed_creation_shortConcrete_pureHvalid_t1_ambient_fullCode
#print axioms extractAssumed_success_flat_creation_t1_short_ambient
