/-
  legacy/t1 HaveField copy under midOwned with content inside bytesRegion.
-/
  ...
  have hCopy :
      cpsTripleWithin
        ...
        ((.x1 ↦ᵣ s.ra) ** (.x2 ↦ᵣ sp0) **
          ...
          regOwn .x28 ** regOwn .x29 ** regOwn .x30) := by
    let Pcore : Assertion :=
      haveFieldCopyAmbientNoBytes txBase lenW (0 : Word) innerW endPtr cursor **
    ...
    have htemps :
      ...
      refine cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp) (fun _ hq => hq)
        (cpsTripleWithin_of_forall_memIs_to_memOwn (a := toBuf + 16)
          ...
          (fun old16' => ?_))
      have h := extractHaveFieldCopy_then_epi_region sp0 spC s txBase lenW (0 : Word)
        innerW endPtr cursor toBuf isCreationPtr t2Old t1Old t0Old next old16'
        LinkLegacyWalk3 s7 txBytes q hspC hret hq halign hcover hcvalid
    ...
    exact cpsTripleWithin_weaken (fun _ hp => by
      dsimp only [Pcore, midOwned, joinStackAmbient, haveFieldCopyAmbientNoBytes,
        extractToBufOwn, legStable, contentPtr] at hp ⊢
...
set_option maxRecDepth 8000 in
theorem extractT1HaveFieldCopy_then_epi_region
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW innerW endPtr next toBuf isCreationPtr s7 : Word)
  ...
  have hCopy :
      cpsTripleWithin
        ...
        ((.x1 ↦ᵣ s.ra) ** (.x2 ↦ᵣ sp0) **
          ...
          regOwn .x28 ** regOwn .x29 ** regOwn .x30) := by
    let Pcore : Assertion :=
      haveFieldCopyAmbientNoBytes txBase lenW (1 : Word) innerW endPtr cursor **
        joinStackAmbient spC s **
    ...
    have htemps :
      ...
      refine cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp) (fun _ hq => hq)
        (cpsTripleWithin_of_forall_memIs_to_memOwn (a := toBuf + 16)
          (P := Pcore ** (.x7 ↦ᵣ t2Old) ** (.x6 ↦ᵣ t1Old) ** (.x5 ↦ᵣ t0Old))
          (fun old16' => ?_))
      have h := extractHaveFieldCopy_then_epi_region sp0 spC s txBase lenW (1 : Word)
        innerW endPtr cursor toBuf isCreationPtr t2Old t1Old t0Old next old16'
        LinkT1Walk4 s7 txBytes q hspC hret hq halign hcover hcvalid
        htalign htover htvalid
      have hF := cpsTripleWithin_frameR
        (regOwn .x28 ** regOwn .x29 ** regOwn .x30)
        (by
    ...
    exact cpsTripleWithin_weaken (fun _ hp => by
      dsimp only [Pcore, midOwned, joinStackAmbient, haveFieldCopyAmbientNoBytes,
        extractToBufOwn, t1Stable, contentPtr] at hp ⊢
      xperm_hyp hp) (fun _ hq => hq) htemps
  have hTo2 :
...
#print axioms extractLegacyHaveFieldCopy_then_epi_region
#print axioms extractT1HaveFieldCopy_then_epi_region
