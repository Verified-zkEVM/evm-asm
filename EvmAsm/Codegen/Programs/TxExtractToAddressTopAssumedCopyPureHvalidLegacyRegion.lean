/-
  Wire pure honesty into short concrete bare Assumed legacy copy path (region).
  Gates content dword-alignment: shortListSrcOff 3 + 1 = 8*q.
-/
...
theorem extractAssumed_copy_shortConcrete_pureHvalid_legacy_region
    ...
    (hq_align : shortListSrcOff (teerTxTypeDispatch txBytes).2.2.toNat items 3 + 1 = 8 * q)
  ...
  have hhoff :=
    extractSuccess_copy_legacy_hoff_srcOff txBytes hsuccess hcopyFlag htype0
      items hdecL hshort
  have hhover :=
    extractSuccess_copy_legacy_hover_srcOff txBytes txBase hsuccess hcopyFlag htype0
      items hdecL hshort hover
  have hlenItems :=
    extractSuccess_copy_legacy_items_length txBytes hsuccess hcopyFlag htype0
      items hdecL hshort
  have hhnext :=
    extractSuccess_copy_legacy_hnext_hlen20_srcOff txBytes txBase hsuccess
      hcopyFlag htype0 items hdecL hshort
  ...
  have hnext_content : ∀ (next3 len3 : Word),
    ...
    intro n l hd
    have hc := extractSuccess_copy_legacy_hnext_content txBytes txBase contentPtr srcOff3
      hsuccess hcopyFlag htype0 items hdecL hshort rfl hcontentPtr endW n l hd
  ...
  have hss2 :=
    hss_of_short_list_item txBytes txBase listOff items 2 hencInner hshort hn2
      hhoff.2.2.1 hover (Or.inl hfields.2.2) hv.2.2.2.2.2.1
  have hss3 :=
  ...
  exact extractAssumed_copy_of_front_short_concrete_legacy_region
...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_shortConcrete_pureHvalid_legacy_region_fullCode
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr : Word)
    (txBytes : List (BitVec 8))
    (items : List EL.RLP.RLPItem)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_copy_shortConcrete_pureHvalid_legacy_region
...
#print axioms extractAssumed_copy_shortConcrete_pureHvalid_legacy_region
#print axioms extractAssumed_copy_shortConcrete_pureHvalid_legacy_region_fullCode
