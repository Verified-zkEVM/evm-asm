...
#guard GuestAddrs.blsg2_encode = 0x80033BA0
#guard GuestAddrs.blsg_le_to_be = 0x8002EF08
...
/-- The verification `CodeReq`: the caller at its guest address plus the
    ...
    call). -/
def encCr : CodeReq :=
  (CodeReq.ofProg (0x80033BA0 : Word) blsg2Encode_prog).union
    (CodeReq.ofProg (0x8002EF08 : Word) blsgLeToBe_prog)
...
theorem blsgLeToBeFlat_spec (ret srci dsti : Word) (inb ob : List (BitVec 8))
  ...
  have had := Fn.retSpecFlat (blsgLeToBeFn srci dsti inb ob)
    (0x8002EF08 : Word)
    (blsgLeToBeFn_spec srci dsti inb ob hwfR hwfW hilen (0x8002EF08 : Word))
  ...
  rw [show (blsgLeToBeFn srci dsti inb ob).programRet (0x8002EF08 : Word)
...
/-- One loop pass (`0x80033BC0 → 0x80033BE0`): compute the 48·i offset,
...
private theorem encStep_spec (i : Nat) (hi : i < 4) (v1 : Word)
    ...
    cpsTripleWithin ((blsgLeToBeFn 0 0 [] []).body.steps + 10)
      (0x80033BC0 : Word) (0x80033BE0 : Word) encCr
      ...
      ((.x18 ↦ᵣ BitVec.ofNat 64 (i + 1)) ** (.x5 ↦ᵣ BitVec.ofNat 64 4)
        ** (((.x1 : Reg) ↦ᵣ (0x80033BD8 : Word)) ** (.x8 ↦ᵣ src) ** (.x9 ↦ᵣ dst)
          ** regOwn .x6 ** regOwn .x10 ** regOwn .x11 ** regOwns encRest
  ...
  rw [show (0x80033BD4 : Word) + 4 = (0x80033BD8 : Word) from by decide] at hcall
  ...
  have ha := addi_spec_gen_same_within .x18 (BitVec.ofNat 64 i) (1 : BitVec 12)
    (0x80033BD8 : Word) (by decide)
  rw [cnt_step_up i (by omega),
      show (0x80033BD8 : Word) + 4 = (0x80033BDC : Word) from by decide] at ha
  have haC := liftCode (cr' := encCr) ha (by code_mem)
  have hli := li_spec_gen_own_within .x5 (4 : Word) (0x80033BDC : Word) (by decide)
  rw [show (0x80033BDC : Word) + 4 = (0x80033BE0 : Word) from by decide,
  ...
  have hliF := cpsTripleWithin_frameR
    ((.x18 ↦ᵣ BitVec.ofNat 64 (i + 1)) ** ((.x1 : Reg) ↦ᵣ (0x80033BD8 : Word))
      ** (.x8 ↦ᵣ src) ** (.x9 ↦ᵣ dst)
...
def encInv (ret src dst : Word)
  ...
  (if i = 0 then ((.x1 : Reg) ↦ᵣ ret) else ((.x1 : Reg) ↦ᵣ (0x80033BD8 : Word)))
  ** (.x8 ↦ᵣ src) ** (.x9 ↦ᵣ dst)
  ** regOwn .x6 ** regOwn .x10 ** regOwn .x11 ** regOwns encRest
...
private theorem encLoopBody_spec
  ...
  · -- i = 0: chunk 0, entry `ra`.
    ...
    · norm_num [encInv] at hq ⊢
      xperm_hyp hq
  · -- i = 1: chunk 1, link `ra`.
    have h := hstep 1 (by omega) (0x80033BD8 : Word) in1 o1 hi1 ho1
      (by omega) (by omega)
    ...
    · norm_num [encInv] at hq ⊢
      xperm_hyp hq
  · -- i = 2: chunk 2.
    have h := hstep 2 (by omega) (0x80033BD8 : Word) in2 o2 hi2 ho2
      (by omega) (by omega)
    ...
    · norm_num [encInv] at hq ⊢
      xperm_hyp hq
  · -- i = 3: chunk 3.
    have h := hstep 3 (by omega) (0x80033BD8 : Word) in3 o3 hi3 ho3
      (by omega) (by omega)
...
def encVals' (src dst : Word) : Reg → Word :=
  fun r => match r with
  | .x1 => (0x80033BD8 : Word) | .x8 => src | .x9 => dst
...
theorem blsg2Encode_spec (sp0 ret src dst arb8 arb9 arb18 : Word)
  ...
  have hm1 := mv_spec_gen_within .x8 .x10 src arb8 (0x80033BB4 : Word) (by decide)
  rw [show (0x80033BB4 : Word) + 4 = (0x80033BB8 : Word) from by decide] at hm1
  have hm1C := liftCode (cr' := encCr) hm1 (by code_mem)
  have hm2 := mv_spec_gen_within .x9 .x11 dst arb9 (0x80033BB8 : Word) (by decide)
  rw [show (0x80033BB8 : Word) + 4 = (0x80033BBC : Word) from by decide] at hm2
  have hm2C := liftCode (cr' := encCr) hm2 (by code_mem)
  have hm3 := li_spec_gen_within .x18 arb18 (0 : Word) (0x80033BBC : Word) (by decide)
  rw [show (0x80033BBC : Word) + 4 = (0x80033BC0 : Word) from by decide,
      show (0 : Word) = BitVec.ofNat 64 0 from rfl] at hm3
  have hm3C := liftCode (cr' := encCr) hm3 (by code_mem)
  -- ---- the count-up loop ----
  have hloop := countupLoopBottom_spec encCr (0x80033BC0 : Word) (0x80033BE0 : Word)
  ...
  rw [show (0x80033BE0 : Word) + 4 = (0x80033BE4 : Word) from by decide] at hloop
  ...
  have hbody : cpsTripleWithin
      (3 + 4 * ((blsgLeToBeFn 0 0 [] []).body.steps + 10 + 1))
      ((0x80033BA0 : Word) + BitVec.ofNat 64 (4 * (1 + encFrame.length)))
      ((0x80033BA0 : Word)
      ...
      ((.x2 ↦ᵣ (sp0 + signExtend12 (-40 : BitVec 12)))
        ...
        ** (regOwn .x10 ** regOwn .x11
          ...
          ** bytesRegion (src + BitVec.ofNat 64 96) in2
          ** bytesRegion (src + BitVec.ofNat 64 144) in3)) := by
    have hentry : (0x80033BA0 : Word) + BitVec.ofNat 64 (4 * (1 + encFrame.length))
        = (0x80033BB4 : Word) := by decide
    have hexit : (0x80033BA0 : Word)
          + BitVec.ofNat 64 (4 * (1 + encFrame.length + encBody.length))
        = (0x80033BE4 : Word) := by decide
    rw [hentry, hexit]
    ...
    have hq1 : (((.x5 : Reg) ↦ᵣ (4#64 : Word))
        ** ((.x18 ↦ᵣ (4#64 : Word)) ** ((.x1 : Reg) ↦ᵣ (2147695576 : Word))
          ** (.x8 ↦ᵣ src) ** (.x9 ↦ᵣ dst)
          ** regOwn .x6 ** regOwn .x10 ** regOwn .x11 ** regOwns encRest
          ** bytesRegion dst (blsgLeToBeBytes in0)
