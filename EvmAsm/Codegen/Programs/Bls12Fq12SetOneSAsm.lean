...
#guard GuestAddrs.blq_zero = 0x80034174
#guard GuestAddrs.blq_set_one = 0x8003418c
...
/-- The verification `CodeReq`: both adjacent routines, at their guest
    addresses. -/
def blqCr : CodeReq :=
  CodeReq.ofProg (0x80034174 : Word) (blqZero_prog ++ blqSetOne_prog)
...
theorem blqZeroFlat_spec (ret dst : Word) (vs : List Word)
    ...
    cpsTripleWithin (blqZeroSteps dst vs) (0x80034174 : Word) ret blqCr
  ...
  rw [show (Bls12Fq12Zero576SAsm.blqZeroFn dst (vs.flatMap dwordBytes)).programRet (0x80034174 : Word)
...
def setOneVals' (dst : Word) : Reg → Word :=
  fun r => match r with | .x1 => (0x800341a0 : Word) | .x8 => dst | _ => 0
...
theorem blqSetOneFrame_spec (sp0 ret dst arb8 v5 v7 : Word) (vs : List Word)
  ...
  -- mv s0, a0
  have hmv := mv_spec_gen_within .x8 .x10 dst arb8 (0x80034198 : Word) (by decide)
  rw [show (0x80034198 : Word) + 4 = (0x8003419c : Word) from by decide] at hmv
  ...
  have hcallee := blqZeroFlat_spec ((0x8003419c : Word) + 4) dst vs hlen hwf (by decide)
  have hcall := callWithin_spec (0x8003419c : Word) (0x80034174 : Word) ret
    (jalOff GuestAddrs.blq_zero (GuestAddrs.blq_set_one + 16)) (blqZeroSteps dst vs)
    (by decide) (by code_mem) (by pcf) hcallee
  rw [show (0x8003419c : Word) + 4 = (0x800341a0 : Word) from by decide] at hcall
  ...
  have hli := li_spec_gen_own_within .x5 (1 : Word) (0x800341a0 : Word) (by decide)
  rw [show (0x800341a0 : Word) + 4 = (0x800341a4 : Word) from by decide] at hli
  have hliC := liftCode (cr' := blqCr) hli (by code_mem)
  have hliF := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ (0x800341a0 : Word)) ** (.x8 ↦ᵣ dst)
  ...
  have hsd := sd_spec_gen_within .x8 .x5 dst (1 : Word)
    ((List.replicate 72 (0 : Word)).getD 0 0) (0 : BitVec 12) (0x800341a4 : Word)
  rw [add_sext0] at hsd
  rw [show (0x800341a4 : Word) + 4 = (0x800341a8 : Word) from by decide] at hsd
  have hsdC := liftCode (cr' := blqCr) hsd (by code_mem)
  have hsdF := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ (0x800341a0 : Word))
      ** (.x10 ↦ᵣ (dst + BitVec.ofNat 64 576))
      ** regOwn .x7 ** regOwns blqRiders
  ...
  have hbody : cpsTripleWithin (1 + (1 + blqZeroSteps dst vs) + 1 + 1)
      ((0x8003418c : Word) + BitVec.ofNat 64 (4 * (1 + setOneFrame.length)))
      ((0x8003418c : Word)
      ...
      ((.x2 ↦ᵣ (sp0 + signExtend12 (-16 : BitVec 12)))
        ...
        ** ((.x10 ↦ᵣ (dst + BitVec.ofNat 64 576)) ** regOwn .x5 ** regOwn .x7
          ...
          ** (Reg.x0 ↦ᵣ (0 : Word))
          ** dwordsIs dst ((1 : Word) :: List.replicate 71 (0 : Word)))) := by
    have hentry : (0x8003418c : Word) + BitVec.ofNat 64 (4 * (1 + setOneFrame.length))
        = (0x80034198 : Word) := by decide
    have hexit : (0x8003418c : Word)
          + BitVec.ofNat 64 (4 * (1 + setOneFrame.length + setOneBody.length))
        = (0x800341a8 : Word) := by decide
    rw [hentry, hexit]
