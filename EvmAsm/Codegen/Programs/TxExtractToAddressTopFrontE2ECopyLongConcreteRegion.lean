/-
  Front E → AfterSave long concrete → 20B copy → ret of_decode (no ∀endPtr).
-/
...
/-- E → ret type234 20B copy long+of_decode with concrete endPtr (no ∀endPtr). -/
theorem extractFrontCopy_then_epi_of_decode_long_concrete_region
  ...
  have hC := extractFrontAfterSaveCopy_then_epi_of_decode_long_region sp0 spC s
...
#print axioms extractFrontCopy_then_epi_of_decode_long_concrete_region
