...
/-- Overflow test of the in-place subtraction on the nonzero decrease arm:
    `bne a0, x0, +52` at K73 + 224 sends a nonzero borrow to the shared
    failure exit (li x10, 1 at K73 + 276) and falls through to the successful
    `li x10, 0` at K73 + 228 otherwise.  Value-generalized over the borrowed
...
/-- Deployed multiply contract specialized to the K73 call site: the callee
    runs at the deployed address, returns to `K73 + 92`, and its assertion
...
theorem k73_mul_callee_at_callsite
    ...
    cpsTripleWithin 3850 (GuestAddrs.u256_mul_u64_be : Word) (K73 + 92) mulCode
      (EvmAsm.Codegen.U256MulU64Be.mulWholePre F spOld (K73 + 92)
      ...
      (EvmAsm.Codegen.U256MulU64Be.mulWholeBodyPost
        (spOld + Rv64.signExtend12 (-48 : BitVec 12)) (K73 + 92)
  ...
  exact mulWhole_spec F hF baseBytes
    ...
    hout spOld (K73 + 92) v8 v9 v18 delta v20 aPtr delta outPtr outPtr
...
/-- Nonzero decrease entry composed onto the multiply call-and-status stage:
    ...
    the shared two-way branch exits at K73 + 96 / K73 + 276. -/
...
theorem k73_decrease_mul_fall_to_sub_borrow_spec_within
  ...
  have hret1 : (((K73 + 108) + 4 : Word) &&& ~~~(1 : Word)) = (K73 + 108) + 4 :=
    EvmAsm.Rv64.BitAux.word_add_even_andn_one (by decide) (by decide)
  have hret2 : (((K73 + 124) + 4 : Word) &&& ~~~(1 : Word)) = (K73 + 124) + 4 :=
    EvmAsm.Rv64.BitAux.word_add_even_andn_one (by decide) (by decide)
  have hretS : (((K73 + 220) + 4 : Word) &&& ~~~(1 : Word)) = K73 + 220 + 4 :=
    EvmAsm.Rv64.BitAux.word_add_even_andn_one (by decide) (by decide)
  have hFframe :
  ...
  have hdivpair := k73_decrease_div_pair_spec_within
    outPtr target (target - gasUsed) (K73 + 92) outBytes
    (k73DecreaseDivPairFrame spH raIn basePtr outPtr target (target - gasUsed)
...
private theorem k73_decr_exit_to_tail_pre
    ...
    {baseBytes accBytes outBytes : List (BitVec 8)} (H : Assertion) :
    ∀ u : PartialState,
      ((.x0 ↦ᵣ (0 : Word)) ** (.x1 ↦ᵣ (K73 + 224)) ** (.x8 ↦ᵣ basePtr) **
  ...
  have c1 : ((.x0 ↦ᵣ (0 : Word)) ** regOwn .x1 ** (.x8 ↦ᵣ basePtr) **
    ...
    decr_under_id (B := (.x0 : Reg) ↦ᵣ (0 : Word))
      (decr_sep_pin_lift (r := .x1) (v := K73 + 224)) u hu
  have c2 : ((.x0 ↦ᵣ (0 : Word)) **
      regOwn .x1 **
      regOwn .x8 **
      (.x9 ↦ᵣ outPtr) **
...
/-- Full nonzero-decrease run from the divider entry to a symbolic return:
    the fall leg reaches the borrow test at K73 + 224; a zero borrow falls
...
theorem k73_decrease_mul_fall_to_return_spec_within
  ...
  have hRestI :
      ((.x1 ↦ᵣ (K73 + 224)) ** (.x8 ↦ᵣ basePtr) ** (.x9 ↦ᵣ outPtr) **
