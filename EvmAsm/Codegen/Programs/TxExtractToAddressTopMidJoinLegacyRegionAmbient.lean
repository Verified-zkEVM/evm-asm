/-
  Ambient legacy MidJoin bare copy region: AfterSave → copy → ret (split loadPtr/regionBase).
-/
...
theorem leg3Exists_to_copy_region_ambient
  ...
  have hpure :
      cpsTripleWithin
        ...
        (⌜rlpItemDecode bs absOff3 (regionBase + BitVec.ofNat 64 absOff3)
            endPtr next3 len3⌝ **
          (leg3OkRegsAmbient loadPtr regionBase lenW innerW endPtr next3 len3
            bs absOff3 **
    ...
    have hleaf := extractLegacyHaveFieldCopy_then_epi_region_ambient sp0 spC s loadPtr regionBase
      lenW innerW endPtr next3 toBuf isCreationPtr s7 bs absOff3 q
      hspC hret (by simpa only [contentPtr] using hnext) hq halign hcover
      hcvalid htalign htover htvalid
    refine cpsTripleWithin_weaken (fun _ hp => by
      simp only [hlen, leg3OkConcreteAmbient] at hp ⊢
...
theorem extractLegacyAfterSaveCopy_then_epi_of_decode_region_ambient
    (sp0 spC : Word) (s : ExtractSaved)
    ...
    cpsTripleWithin
      ...
      (afterSaveFrameTyAmbient loadPtr regionBase lenW (0 : Word) innerW cursor endPtr bs **
      ...
      ((.x1 ↦ᵣ s.ra) ** (.x2 ↦ᵣ sp0) **
        ...
        regOwn .x28 ** regOwn .x29 ** regOwn .x30) := by
  intro contentPtr w2
  have hW := extractLegacyToWalk3Ok_owned_of_decode_ambient spC s loadPtr regionBase lenW innerW
  ...
  have hC := leg3Exists_to_copy_region_ambient sp0 spC s loadPtr regionBase lenW innerW
    endPtr toBuf isCreationPtr s7 bs absOff3 q
    hspC hret hlen20 hnext_content hq hsalign hcover hcvalid htalign htover htvalid
  exact cpsTripleWithin_seq_same_cr hW hC

#print axioms leg3Exists_to_copy_region_ambient
#print axioms extractLegacyAfterSaveCopy_then_epi_of_decode_region_ambient
