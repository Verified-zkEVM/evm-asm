...
#guard GuestAddrs.secp256k1_point_double = 0x80020408
#guard GuestAddrs.secf_is_zero32 = 0x8001fc2c
#guard GuestAddrs.secf_zero32 = 0x8001fb58
#guard GuestAddrs.secf_be_to_le = 0x8001fb6c
#guard GuestAddrs.secf_le_to_be = 0x8001fbbc
#guard GuestAddrs.secc_le_p1 = 0xa3c055c8

/-- The staging-point base (`secc_le_p1`): a 64-byte LE point image
    `x || y`, four u64 limbs per coordinate. -/
def arenaB : Word := 0xa3c055c8
...
def pdCr : CodeReq :=
  (CodeReq.ofProg (0x80020408 : Word) secp256k1PointDouble_prog).union
    ((CodeReq.ofProg (0x8001fc2c : Word) secfIsZero32_prog).union
      ((CodeReq.ofProg (0x8001fb58 : Word) secfZero32_prog).union
        ((CodeReq.ofProg (0x8001fb6c : Word) secfBeToLe_prog).union
          (CodeReq.ofProg (0x8001fbbc : Word) secfLeToBe_prog))))
...
theorem secfIsZero32Flat_spec (ret ptr : Word) (bs : List (BitVec 8))
  ...
  have had := Fn.retSpecFlat (secfIsZero32Fn ptr bs)
    (0x8001fc2c : Word)
    (secfIsZero32Fn_spec ptr bs hwfR (0x8001fc2c : Word))
    ...
    (fun rf' ws' hlen' hpost' hp hh => by
      ...
      exact sepConj_mono_right (regAtomsOf_to_regOwns (fun r => rf' r) a0Rest)
        hp hh)
  rw [show (secfIsZero32Fn ptr bs).programRet (0x8001fc2c : Word)
...
theorem secfZero32Flat_spec (ret dst : Word) (ob : List (BitVec 8))
  ...
  have had := Fn.retSpecFlat (secfZero32Fn dst ob)
    (0x8001fb58 : Word)
    (secfZero32Fn_spec dst ob hrww (0x8001fb58 : Word))
  ...
  rw [show (secfZero32Fn dst ob).programRet (0x8001fb58 : Word)
...
theorem secfBeToLeFlat_spec (ret srci dsti : Word) (inb ob : List (BitVec 8))
  ...
  have had := Fn.retSpecFlat (secfBeToLeFn srci dsti inb ob)
    (0x8001fb6c : Word)
    (secfBeToLeFn_spec srci dsti inb ob hwfR hrww hilen (0x8001fb6c : Word))
    ...
    (fun rf' ws' hlen' hpost' hp hh => by
      ...
      xperm_hyp hpure)
  rw [show (secfBeToLeFn srci dsti inb ob).programRet (0x8001fb6c : Word)
...
theorem secfLeToBeFlat_spec (ret srci dsti : Word) (inb ob : List (BitVec 8))
  ...
  have had := Fn.retSpecFlat (secfLeToBeFn srci dsti inb ob)
    (0x8001fbbc : Word)
    (secfLeToBeFn_spec srci dsti inb ob hwfR hrww hilen (0x8001fbbc : Word))
    ...
    (fun rf' ws' hlen' hpost' hp hh => by
      ...
      have hpure : (⌜beBytesToNat ws' = Accel.leLimbsToNat
        ...
        (sepConj_pure_left hp).mpr ⟨⟨hpost'.1, hlen'⟩, hh2⟩
      xperm_hyp hpure)
  rw [show (secfLeToBeFn srci dsti inb ob).programRet (0x8001fbbc : Word)
...
theorem curveStep_spec (img : List (BitVec 8))
    ...
    cpsTripleWithin 1 (0x80020470 : Word) (0x80020474 : Word) pdCr
      (((.x5 : Reg) ↦ᵣ (0xa3c055c8 : Word)) ** regOwns csrsRest
  ...
  refine cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp) (fun _ hq => hq)
    (cpsTripleWithin_peel_regOwns csrsRest (by decide)
      (P := ((.x5 : Reg) ↦ᵣ (0xa3c055c8 : Word)) ** bytesRegion arenaB img)
      (fun vf => ?_))
  have hcs := csrs_curveDbl_spec_within .secp256k1 (0x80020470 : Word) .x5
    (by decide) arenaB 64 img
    (fun r => if r = .x5 then (0xa3c055c8 : Word) else vf r)
    hlen (by decide) hvalid
    0
    (by
      show RegFile.get _ .x5 = arenaB + BitVec.ofNat 64 0
      rw [RegFile.get, if_neg (by decide : (Reg.x5 : Reg) ≠ .x0)]
      show (if (Reg.x5 : Reg) = .x5 then (0xa3c055c8 : Word) else vf .x5) = _
      rw [if_pos rfl]
      decide)
    ⟨0, rfl⟩ (by decide)
  ...
  rw [show (0x80020470 : Word) + 4 = (0x80020474 : Word) from by decide,
  ...
  rw [regFileIs_eq_regAtoms, regAtoms_eq_regAtomsOf _ _ (by decide),
    exposedRegs_split5,
    show (if (Reg.x5 : Reg) = .x5 then (0xa3c055c8 : Word) else vf .x5)
      = (0xa3c055c8 : Word) from if_pos rfl,
    regAtomsOf_congr
      (fun r => if r = .x5 then (0xa3c055c8 : Word) else vf r) vf csrsRest
      (fun r hr => by
        show (if r = .x5 then (0xa3c055c8 : Word) else vf r) = vf r
