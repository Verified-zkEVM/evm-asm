/-
  Front E → AfterSave long concrete → creation → ret of_decode.
-/
...
/-- Long-path E2E steps: walk_init uses 1+(7*lol+25). -/
def nFrontCreationStepsLong (lol : Nat) : Nat :=
...
/-- E → ret type234 creation long+of_decode with concrete longWalkCursor/End. -/
theorem extractFrontCreation_then_epi_of_decode_long_concrete
  ...
  have hC := extractFrontAfterSaveCreation_then_epi_of_decode_long sp0 spC s
...
#print axioms extractFrontCreation_then_epi_of_decode_long_concrete
