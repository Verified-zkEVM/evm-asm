...
def balStorageReadsInExecLogFunction : String :=
  ...
  "  mv s1, a3                    # log base\n" ++
  "  mv s2, a4                    # log length\n" ++
  "  mv s6, a1                    # AccountChanges ptr (offsets are relative to it)\n" ++
  ...
  "  la t1, bsr_koff; ld t1, 0(t1); add t1, s3, t1     # content ptr (BE, MSB first)\n" ++
  "  la t2, bsr_klen; ld t2, 0(t2)                     # content length\n" ++
