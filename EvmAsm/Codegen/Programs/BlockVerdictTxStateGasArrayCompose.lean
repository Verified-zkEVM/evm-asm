...
structure A4gbrResiduals where
  /-- Extract assumed still residual; type_dispatch discharged.
      Path packaging: creation/copy short bare for type234+legacy+t1 (`extract_discharge_*_short_*_available`). Long-list pure guards + Call_long + leaf_hyps packaging DONE classical-3; Front/Assumed long packaging residual (validByteRange/hlover wire + AfterSave). -/
