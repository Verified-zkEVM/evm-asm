              ...
              **concrete short AfterSave→creation of_decode DONE**
               ...
               **short concrete E→ret + Assumed DONE**
                ...
                **copy Assumed pure wire DONE**
                ...
                Assumed**contentDwords). **type234 short creation bare Assumed path Prop DONE**; **type234 short copy Assumed**content path Prop DONE**. **legacy of_decode AfterSave→creation DONE** (`TopMidSeq/Chain/JoinLegacyDecode`). **legacy short creation bare Assumed path Prop DONE** (`extractAssumed_success_flat_creation_legacy_short` under `extractCreationLegacyShortPath`; classical-3). **t1 of_decode AfterSave→creation DONE** (`TopMidSeq/Chain/JoinT1Decode`; classical-3). Residual: t1 Front E2E+pureHvalid+path Prop bare Assumed; long + content-from-bytesRegion for copy; multi-tx ambient Option A; Teer prover1; gate a4gbr.1.
