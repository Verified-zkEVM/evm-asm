...
structure A4gbrResiduals where
  ...
  /-- Teer AuthLoop…AtRecover(E+936) + recover Call(Assumed)+BNE+prior zeros→E+968 classical-3; success empty skip→E+1116 + bal_find Setup/Call(Assumed)/BNE→E+1156 classical-3; bal_finals Setup/Call(Assumed)/BNE→E+1196 classical-3; acct_absent zero + records empty skip→AtSvfTxCount(E+1276) classical-3; svf_tx_count=1 fallthrough→AfterSvfTxCountBne(E+1296) classical-3; code_at Setup/Call(Assumed)/BNE→AfterCodeAtBne(E+1388) classical-3; cahsr_code_length empty-skip→AtSvfTxCountSkip(E+1848) + len=23→AfterCahsrLenEq23(E+1412) + codePtr+0xef0100 prefix compose→AtSvfTxCountSkip(E+1848) classical-3 (teerCahsrPrefixEf0100Skip / LenEq23ThenPrefix); bal_nonce Setup+Call(Assumed)+BEQ status0→AfterBalNonceBeq0(E+2016) classical-3; nonce-join→AfterNonceEq(E+2100) + acct/finals/rolled→E+2168 + prior≠0 skip→AfterPriorJoin(E+2384) classical-3 (teerPriorNezSkip); OR-zero setup+Assumed+BEQ zero→AtSuccessCount(E+2708) classical-3; epi restore loadSeq+ADDI+JALR@E+2920 classical-3 (teerEpilogueRestore); wouldbe store+rolled=0 BEQ→EpiRestore classical-3 (teerWouldbeToEpi_rolled0); wouldbe+epi→ret compose classical-3 (teerWouldbeToRet_rolled0); auth-loop exit/empty→ret classical-3 (teerAuthLoopExitToRet_rolled0 / teerEmptyAuthToRet_rolled0); TeerCalleeAssumptions + TeerFrontToAuthLoopAssumed
            * FrontEarly: prologue+scratch+bal≠0 under applied (E→AfterBalCheck) DONE + teerEmptyAuth_front_then_exit(_mono) classical-3 (empty-auth→ret a0=0 under front Assumed); stackFree20/epi_a5 discharge + ExitFrame top-6+a5 packaging classical; empty-exit→applied_flat post + teerAssumed_empty_applied_flat structure fill classical-3 under FrontToAuthLoopAssumed+hteer0 (TxEip7702TeerDischarge); FrontEarly teerPrologue_applied + teerScratchZero_fullScratch classical-3 (E→AfterAbiMoves; scratch peel under full teerScratchOwn); residual: scratch-zero+bal under applied; discharge FrontToAuthLoopAssumed (Spec..AuthLoopStart compose); honest hteer0 empty-auth model; PriorZero/SuccessWrite bodies; status=1/account_at; non-empty loop; TeerCodeAtAssumed named hyp; TeerRecover/BalFind/BalFinals/OrZero/AuthSenderMatch Assumed named hyps; leaf modulo its remaining input callees (prover1 scope). -/
