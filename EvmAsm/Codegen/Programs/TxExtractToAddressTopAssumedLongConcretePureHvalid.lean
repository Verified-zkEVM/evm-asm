/-
  Discharge packaging `hvalid*` / `hvalid1_*` / `hlover` / `hlvalid`
  via buffer-wide `validByteRange` for long-list creation.
-/
...
/-- Long concrete Assumed creation; hvalid*/hlover/hlvalid from `validByteRange`.
    Residual path: long + hitem0..5 short-encode + hge7. -/
theorem extractAssumed_creation_longConcrete_pureHvalid
  ...
  have hv :=
    extractSuccess_creation_type234_hvalid_srcOff_long txBytes txBase hsuccess
      hcreFlag hge items hdecL hlong hge7 hvalidBuf
  have hll :=
    extractSuccess_long_hlover_hlvalid txBytes txBase hsuccess items hdecL hlong
      hover hvalidBuf
  exact extractAssumed_creation_longConcrete_pureHdec
    ...
    hitem3 hv.2.2.2.2.2.2.1 hv.2.2.2.2.2.2.2.1
    hitem4 hv.2.2.2.2.2.2.2.2.1 hv.2.2.2.2.2.2.2.2.2.1
    hitem5 hv.2.2.2.2.2.2.2.2.2.2.1 hv.2.2.2.2.2.2.2.2.2.2.2
    hge7

set_option maxRecDepth 8000 in
theorem extractAssumed_creation_longConcrete_pureHvalid_fullCode
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr : Word)
    (txBytes : List (BitVec 8))
    (items : List EL.RLP.RLPItem)
...
#print axioms extractAssumed_creation_longConcrete_pureHvalid
#print axioms extractAssumed_creation_longConcrete_pureHvalid_fullCode
