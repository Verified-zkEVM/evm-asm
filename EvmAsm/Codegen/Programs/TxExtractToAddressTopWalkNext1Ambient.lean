/-
  Ambient dual of type234 walk_next1 (prep + call outcome + OkNested).
...
def wn1StableAmbient (loadPtr lenW typeW innerW endPtr cursor : Word) : Assertion :=
...
theorem wn1StableAmbient_pcFree (loadPtr lenW typeW innerW endPtr cursor : Word) :
    (wn1StableAmbient loadPtr lenW typeW innerW endPtr cursor).pcFree := by
  unfold wn1StableAmbient; pcf

def wn1CommonAmbient (regionBase : Word) (bs : List (BitVec 8)) : Assertion :=
  regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** regOwn .x28 ** regOwn .x29 **
    regOwn .x30 ** regOwn .x31 ** (.x0 ↦ᵣ (0 : Word)) ** (.x1 ↦ᵣ LinkWalkNext1) **
    bytesRegion regionBase bs

def wn1OkRegsAmbient (loadPtr regionBase lenW typeW innerW endPtr next len : Word)
...
def wn1OkConcreteAmbient (loadPtr regionBase lenW typeW innerW endPtr next len : Word)
    (bs : List (BitVec 8)) (absOff : Nat) : Assertion :=
  wn1OkRegsAmbient loadPtr regionBase lenW typeW innerW endPtr next len bs absOff **
    ⌜rlpItemDecode bs absOff (regionBase + BitVec.ofNat 64 absOff)
      endPtr next len⌝

private theorem of_forall_regOwn7_wn1
...
theorem extractWalkNext1Prep_framed_ambient
    ...
    cpsTripleWithin (1 + (1 + 1)) AfterWalkNext0Bne WalkNext1JalPc extractLinkedCode
      ...
      (wn1StableAmbient loadPtr lenW typeW innerW endPtr next **
        ...
        (.x1 ↦ᵣ LinkWalkNext0) ** bytesRegion regionBase bs) := by
  ...
  have hCore :
      cpsTripleWithin (1 + (1 + 1)) AfterWalkNext0Bne WalkNext1JalPc extractLinkedCode
        (wn0OkRegsAmbient loadPtr regionBase lenW typeW innerW endPtr next len
          bs absOff0)
        (wn1StableAmbient loadPtr lenW typeW innerW endPtr next **
          (.x10 ↦ᵣ next) ** (.x11 ↦ᵣ endPtr) ** (.x12 ↦ᵣ len) **
          regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** regOwn .x28 ** regOwn .x29 **
    ...
    refine cpsTripleWithin_weaken (fun _ hp => by
      simp only [wn0OkRegsAmbient, wn0StableAmbient, wn0CommonAmbient] at hp ⊢
      xperm_hyp hp) (fun _ hq => by
      simp only [wn1StableAmbient] at hq ⊢
      xperm_hyp hq) hF
  refine cpsTripleWithin_weaken (fun st hp => by
    simp only [wn0OkConcreteAmbient] at hp
    exact (sepConj_pure_right st).mp hp |>.1) (fun _ hq => hq) hCore

theorem extractWalkNext1Post_to_commonOutcome_ambient
    (regionBase endPtr : Word) (bs : List (BitVec 8)) (absOff : Nat) :
    ∀ h, extractWalkNext1Post regionBase endPtr bs absOff h →
      (wn1CommonAmbient regionBase bs **
        wn0Outcome regionBase endPtr bs absOff) h := by
  intro h hp
  simp only [extractWalkNext1Post] at hp
  obtain ⟨h1, h2, hd, hu, hcom, hout⟩ := hp
  have hcom' : wn1CommonAmbient regionBase bs h1 := by
    simp only [wn1CommonAmbient]
    xperm_hyp hcom
  exact ⟨h1, h2, hd, hu, hcom', hout⟩

set_option maxRecDepth 8000 in
theorem extractWalkNext1Call_type234_a2_outcome_ambient
  ...
  let Pcore : Assertion :=
    wn1StableAmbient loadPtr lenW typeW innerW endPtr cursor **
  ...
  have htemps :
      cpsTripleWithin (1 + 87) WalkNext1JalPc LinkWalkNext1 extractLinkedCode
        ...
        Qassumed := by
    refine of_forall_regOwn7_wn1 (r1 := .x5) (r2 := .x6) (r3 := .x7)
      (r4 := .x28) (r5 := .x29) (r6 := .x30) (r7 := .x31)
      (fun t0 t1 t2 t3 t4 t5 t6 => ?_)
    have hleaf := extractWalkNext1Call_ambient regionBase endPtr a2Old
      t0 t1 t2 t3 t4 t5 t6 bs absOff LinkWalkNext0
      hsalign hoff hover hvalid hss hls hll
    have hF := cpsTripleWithin_frameR
      (wn1StableAmbient loadPtr lenW typeW innerW endPtr cursor)
      (wn1StableAmbient_pcFree _ _ _ _ _ _) hleaf
    refine cpsTripleWithin_weaken (fun _ hp => by
      dsimp only [Pcore, wn1StableAmbient, extractWalkNextPrest] at hp ⊢
      xperm_hyp hp) (fun h hq => by
      dsimp only [Qassumed] at hq ⊢
      have hq' :
          (wn1StableAmbient loadPtr lenW typeW innerW endPtr cursor **
            extractWalkNext1Post regionBase endPtr bs absOff) h := by
        xperm_hyp hq
      obtain ⟨hA, hP, hd, hu, hamb, hpost⟩ := hq'
      have hnorm := extractWalkNext1Post_to_commonOutcome_ambient
...
set_option maxRecDepth 8000 in
theorem extractWalkNext1BneOk_framed_ambient
    (loadPtr regionBase lenW typeW innerW endPtr next len : Word)
    (bs : List (BitVec 8)) (absOff : Nat) :
  ...
  have hF := cpsTripleWithin_frameR
    ...
    (by pcf) h0
  refine cpsTripleWithin_weaken (fun _ hp => by
    simp only [wn1StableAmbient, wn1CommonAmbient] at hp ⊢
    xperm_hyp hp) (fun _ hq => by
    simp only [wn1StableAmbient, wn1CommonAmbient] at hq ⊢
    xperm_hyp hq) hF

set_option maxRecDepth 8000 in
theorem extractWalkNext1OkNested_bne_ambient
    ...
    (P := ⌜rlpItemDecode bs absOff cursor endPtr next len⌝ **
      (((.x11 ↦ᵣ (0 : Word)) ** (.x0 ↦ᵣ (0 : Word))) **
        (wn1StableAmbient loadPtr lenW typeW innerW endPtr cursor **
          ...
          (.x10 ↦ᵣ next) ** (.x12 ↦ᵣ len))))
    (fun h hp => by
      simp only [wn1CommonAmbient] at hp
      xperm_hyp hp)
    (fun _ hq => hq) ?_
  refine cpsTripleWithin_pure_pre (fun hdec => ?_)
  have h0 := extractWalkNext1BneOk_framed_ambient loadPtr regionBase
    lenW typeW innerW endPtr next len bs absOff
  refine cpsTripleWithin_weaken (fun h hp => by
    simp only [wn1CommonAmbient] at hp ⊢
    xperm_hyp hp) (fun h hq => by
    refine ⟨next, len, ?_⟩
    simp only [wn1OkConcreteAmbient, wn1OkRegsAmbient]
    exact (sepConj_pure_right h).mpr ⟨hq, hdec⟩) h0

#print axioms extractWalkNext1Prep_framed_ambient
#print axioms extractWalkNext1Call_type234_a2_outcome_ambient
#print axioms extractWalkNext1OkNested_bne_ambient
