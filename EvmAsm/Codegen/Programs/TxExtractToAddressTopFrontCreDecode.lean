/-
  Front AfterSave → creation of_decode (no universal hok).
-/
...
theorem extractFrontAfterSaveCreation_then_epi_of_decode
  ...
  have hEx :
    ...
    exact (extractType234AfterSaveCreation_then_epi_of_decode sp0 spC s
      txBase lenW
      (teerTxTypeDispatch txBytes).2.1
      (teerTxTypeDispatch txBytes).2.2
      ...
      (hnext1 endPtr) (hnext2 endPtr) (hnext3 endPtr) (hnext4 endPtr) (hnext5 endPtr) (hcre endPtr))
  exact cpsTripleWithin_weaken
    (fun h hp => frontAfterSave_to_midJoinPre spC s txBase lenW
      toBuf isCreationPtr txBytes h hp)
    (fun _ hq => hq) hEx

#print axioms extractFrontAfterSaveCreation_then_epi_of_decode
