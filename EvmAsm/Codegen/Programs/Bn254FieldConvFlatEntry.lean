/-
  EvmAsm.Codegen.Programs.Bn254FieldConvFlatEntry

  Whole-routine flat triples for the two BN254 base-field BE↔LE converters,
  anchored at their OWN single-program `CodeReq` — the `GuestImageEntries`
  pairing — rather than a caller's union (#12244).

  ## Why this module exists

  This is the same defect `Secp256k1FieldConvFlatEntry` closed for the
  secp256k1 pair, one curve over. Flat contracts for `bnf_be_to_le` /
  `bnf_le_to_be` already existed, twice each, inside the two callers:

  | theorem | `CodeReq` | programs it requires loaded |
  |---|---|---|
  | `Bn254FieldAddModPSAsm.bnfBeToLeFlat_spec` | `addCr` | 3 |
  | `Bn254FieldMulModPSAsm.bnfBeToLeFlat_spec` | `mulCr` | 3 |
  | `Bn254FieldAddModPSAsm.bnfLeToBeFlat_spec` | `addCr` | 3 |
  | `Bn254FieldMulModPSAsm.bnfLeToBeFlat_spec` | `mulCr` | 3 |
  ...
  claim `GuestImageEntries.lean` makes (`(GuestAddrs.bnf_be_to_le,
  bnfBeToLe_prog)`), so none of those four theorems is the image claim, and
  rowing one would attach a registry row whose hypothesis the image does not
  discharge.

  ⚠️ The two callers' copies were **byte-identical modulo the `CodeReq` name**:
  186 lines duplicated verbatim, `addCr` against `mulCr`. That is the signature
  of this class — the union is the only thing that differs, because the union is
  the only thing that was ever caller-specific.
  ...
  and then widened it with `liftCode (cr' := addCr) … (by code_mem)` on the very
  next line. The own-`CodeReq` triple was the load-bearing step all along; it
  was simply never named.

  So this module states it once and the four caller-side theorems become
  one-line `liftCode` corollaries. That REMOVES the duplication rather than
  adding to it, and the direction is the sound one: `⊆`-monotonicity gives
  caller triples from the entry triple, never the reverse.

  ## Geometry

  Both converters are the both-regions-non-empty case:

  | | `region` (read-only) | `rw` (writable) | ambient | adapter |
  |---|---|---|---|---|
  | `bnf_be_to_le` | non-empty (`srci`, 32 B) | non-empty (`dsti`, 32 B) | empty | `retSpecFlat` |
  | `bnf_le_to_be` | non-empty (`srci`, 32 B) | non-empty (`dsti`, 32 B) | empty | `retSpecFlat` |

  Two live windows is what forces `hdisj`: with both a source and a destination
  in the footprint, the separating conjunction needs them apart, so these triples
  are NOT total over their argument types. `hdisj` is a genuine domain
  restriction, discharged at each call site by the arena layout (`arenaB` /
  `arenaM`) — not a representability guard.
...
/-- **`bnf_be_to_le`, whole-routine flat triple at the guest entry.**
    ...
    Anchored over `CodeReq.ofProg (GuestAddrs.bnf_be_to_le) bnfBeToLe_prog`,
    ...
    ⚠️ Distinct from `Bn254FieldAddModPSAsm.bnfBeToLeFlat_spec` and
    `Bn254FieldMulModPSAsm.bnfBeToLeFlat_spec`, which agree on entry, exit, pre
    and post but are anchored over `addCr` / `mulCr`. Both are now corollaries of
    this theorem; neither is the image claim. -/
theorem bnfBeToLeFlatEntry_spec (ret srci dsti : Word) (inb ob : List (BitVec 8))
    ...
    cpsTripleWithin ((bnfBeToLeFn srci dsti inb ob).body.steps + 1)
      (GuestAddrs.bnf_be_to_le : Word) ret
      (CodeReq.ofProg (GuestAddrs.bnf_be_to_le : Word) bnfBeToLe_prog)
  ...
  have had := Fn.retSpecFlat (bnfBeToLeFn srci dsti inb ob)
    ...
    (fun rf' ws' hlen' hpost' hp hh => by
      ...
      xperm_hyp hpure)
  -- ⛔ NO `liftCode` here. That is the whole point of this module: the triple
  ...
  rw [show (bnfBeToLeFn srci dsti inb ob).region = (⟨srci, inb⟩ : Region) from rfl,
      show (bnfBeToLeFn srci dsti inb ob).rw.base = dsti from rfl] at had
...
/-- **`bnf_le_to_be`, whole-routine flat triple at the guest entry.**
    ...
    Anchored over `CodeReq.ofProg (GuestAddrs.bnf_le_to_be) bnfLeToBe_prog`,
...
theorem bnfLeToBeFlatEntry_spec (ret srci dsti : Word) (inb ob : List (BitVec 8))
    (hilen : inb.length = 32) (holen : ob.length = 32)
    (hwfR : Region.wf ⟨srci, inb⟩) (hrww : RwRegion.wf ⟨dsti, 32⟩)
    (hso : srci.toNat + 32 < 2 ^ 64) (hdo : dsti.toNat + 32 < 2 ^ 64)
    (hdisj : srci.toNat + 32 ≤ dsti.toNat ∨ dsti.toNat + 32 ≤ srci.toNat)
    ...
    cpsTripleWithin ((bnfLeToBeFn srci dsti inb ob).body.steps + 1)
      (GuestAddrs.bnf_le_to_be : Word) ret
  ...
  rw [show (bnfLeToBeFn srci dsti inb ob).region = (⟨srci, inb⟩ : Region) from rfl,
      show (bnfLeToBeFn srci dsti inb ob).rw.base = dsti from rfl] at had
  rw [regFileIs_eq_regAtoms, regAtoms_eq_regAtomsOf _ _ (by decide),
    exposedRegs_split2,
    show (if (Reg.x10 : Reg) = .x10 then srci else
        if (Reg.x10 : Reg) = .x11 then dsti else vf .x10) = srci from if_pos rfl,
