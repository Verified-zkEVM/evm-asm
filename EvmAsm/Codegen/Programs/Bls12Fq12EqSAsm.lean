...
#guard GuestAddrs.blq_eq = 0x8003455C
...
#guard (blqEqBody 0 0 [] []).flatten 0x8003455C = blqEq_prog
...
theorem blqEqBody_flatten_eq (ptr1 ptr2 : Word) (bs1 bs2 : List (BitVec 8)) :
    (blqEqBody ptr1 ptr2 bs1 bs2).flatten (0x8003455C : Word) = blqEq_prog := rfl
...
theorem blqEq_spec (ptr1 ptr2 ret : Word) (bs1 bs2 : List (BitVec 8))
    ...
    cpsTripleWithin (blqEqBody ptr1 ptr2 bs1 bs2).steps
      (0x8003455C : Word) ret
      (CodeReq.ofProg (0x8003455C : Word) blqEq_prog)
  ...
  rw [show (DualReadScan.scanBody .x5 .x6 .x7 .x10 .x11 ptr1 ptr2 bs1 bs2
      72).flatten (0x8003455C : Word) = blqEq_prog from rfl] at h
  exact h
