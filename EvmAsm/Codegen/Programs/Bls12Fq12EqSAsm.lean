...
#guard GuestAddrs.blq_eq = 0x8003495c
...
#guard (blqEqBody 0 0 [] []).flatten GuestAddrs.blq_eq = blqEq_prog
...
theorem blqEqBody_flatten_eq (ptr1 ptr2 : Word) (bs1 bs2 : List (BitVec 8)) :
    (blqEqBody ptr1 ptr2 bs1 bs2).flatten (GuestAddrs.blq_eq : Word) = blqEq_prog := rfl
...
theorem blqEq_spec (ptr1 ptr2 ret : Word) (bs1 bs2 : List (BitVec 8))
    ...
    cpsTripleWithin (blqEqBody ptr1 ptr2 bs1 bs2).steps
      (GuestAddrs.blq_eq : Word) ret
      (CodeReq.ofProg (GuestAddrs.blq_eq : Word) blqEq_prog)
