/-
  MidJoin of_decode: AfterSave → creation/copy under pure decode (no universal hok).
-/
...
/-- AfterSave → creation → ret under pure decode (no universal hok). -/
theorem extractType234AfterSaveCreation_then_epi_of_decode
    ...
    cpsTripleWithin
      ...
      (fun h => ∃ next5 : Word,
        ((.x1 ↦ᵣ s.ra) ** (.x2 ↦ᵣ sp0) **
          ...
          (.x31 ↦ᵣ (next5 - (0 : Word))) **
          creExtraTemps) h) := by
  have hW := extractType234ToWn5Ok_owned_of_decode spC s txBase lenW typeW innerW
...
set_option maxRecDepth 8000 in
/-- AfterSave → 20B copy → ret under pure decode (no universal hok). -/
theorem extractType234AfterSaveCopy_then_epi_of_decode
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW typeW innerW cursor endPtr toBuf isCreationPtr s7 : Word)
    (txBytes : List (BitVec 8))
    (srcOff0 srcOff1 srcOff2 srcOff3 srcOff4 srcOff5 : Nat)
  ...
  have hW0 := extractType234ToWn5Ok_owned_of_decode spC s txBase lenW typeW innerW
    cursor endPtr toBuf isCreationPtr s7 txBytes
    srcOff0 srcOff1 srcOff2 srcOff3 srcOff4 srcOff5
    hcur hne0 hne1 hsalign
    hoff0 hover0 hvalid0 hss0 hls0 hll0 hdec0 hinb0
...
#print axioms extractType234AfterSaveCreation_then_epi_of_decode
#print axioms extractType234AfterSaveCopy_then_epi_of_decode
