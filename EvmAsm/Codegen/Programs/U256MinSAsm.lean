...
-- Address anchor.
#guard GuestAddrs.u256_min = 0x80024be8
...
private theorem pickA_spec (w : Word) (hcond : minSel as bs) :
    cpsTripleWithin 2 (0x80024c14 : Word) (0x80024c20 : Word)
      (CodeReq.ofProg (0x80024be8 : Word) u256Min_prog)
  ...
  have hmv := liftCode (cr' := CodeReq.ofProg (0x80024be8 : Word) u256Min_prog)
    (mv_spec_gen_within .x5 .x10 aPtr w (0x80024c14 : Word) (by decide))
    (by code_mem)
  rw [show (0x80024c14 : Word) + 4 = (0x80024c18 : Word) from by decide] at hmv
  have hjal := liftCode (cr' := CodeReq.ofProg (0x80024be8 : Word) u256Min_prog)
    (jal_x0_spec_gen_within (8 : BitVec 21) (0x80024c18 : Word))
    (by code_mem)
  rw [show (0x80024c18 : Word) + signExtend21 (8 : BitVec 21) = (0x80024c20 : Word)
...
private theorem pickB_spec (w : Word) (hcond : ¬ minSel as bs) :
    cpsTripleWithin 1 (0x80024c1c : Word) (0x80024c20 : Word)
      (CodeReq.ofProg (0x80024be8 : Word) u256Min_prog)
      (((.x5 : Reg) ↦ᵣ w) ** ((.x31 : Reg) ↦ᵣ (32 : Word)) ** ((.x10 : Reg) ↦ᵣ aPtr) **
      ((.x11 : Reg) ↦ᵣ bPtr) ** ((.x12 : Reg) ↦ᵣ outPtr) **
      ((.x1 : Reg) ↦ᵣ ret) **
      ...
      bytesRegion aPtr as ** bytesRegion bPtr bs ** bytesRegion outPtr os) := by
  have hmv := liftCode (cr' := CodeReq.ofProg (0x80024be8 : Word) u256Min_prog)
    (mv_spec_gen_within .x5 .x11 bPtr w (0x80024c1c : Word) (by decide))
    (by code_mem)
  rw [show (0x80024c1c : Word) + 4 = (0x80024c20 : Word) from by decide] at hmv
  have hmvF := cpsTripleWithin_frameR
...
private theorem scanLoop_spec
    ...
    (M i : Nat) (hMi : i + M = 32)
    (hpref : ∀ j, j < i → as.getD j 0 = bs.getD j 0) :
    cpsTripleWithin (9 * M + 8) (0x80024bf0 : Word) (0x80024c20 : Word)
      (CodeReq.ofProg (0x80024be8 : Word) u256Min_prog)
  ...
  | zero =>
      ...
      have hbr := cpsBranchWithin_frameR
        ...
        (cpsBranchWithin_extend_code
          (cr' := CodeReq.ofProg (0x80024be8 : Word) u256Min_prog)
          (h := beq_spec_gen_within .x5 .x31 (36 : BitVec 13)
            (BitVec.ofNat 64 32) (32 : Word) (0x80024bf0 : Word))
          (hmono := by code_mem))
      rw [show (0x80024bf0 : Word) + signExtend13 (36 : BitVec 13)
            = (0x80024c14 : Word) from by decide,
          show (0x80024bf0 : Word) + 4 = (0x80024bf4 : Word) from by decide] at hbr
  ...
  | succ n ih =>
      ...
      set CR := CodeReq.ofProg (0x80024be8 : Word) u256Min_prog with hCR
      ...
      have hadd6 := liftCode (cr' := CR)
        (add_spec_gen_within .x6 .x10 .x5 aPtr (BitVec.ofNat 64 i) (vf .x6)
          (0x80024bf4 : Word) (by decide))
        (by rw [hCR]; code_mem)
      rw [show (0x80024bf4 : Word) + 4 = (0x80024bf8 : Word) from by decide] at hadd6
      have hadd7 := liftCode (cr' := CR)
        (add_spec_gen_within .x7 .x11 .x5 bPtr (BitVec.ofNat 64 i) (vf .x7)
          (0x80024bf8 : Word) (by decide))
        (by rw [hCR]; code_mem)
      rw [show (0x80024bf8 : Word) + 4 = (0x80024bfc : Word) from by decide] at hadd7
      have hlbuA := liftCode (cr' := CR)
        (bytesRegion_lbu_within .x28 .x6 aPtr (vf .x28) (0x80024bfc : Word)
          as i (by decide) halignA hia (by omega) (hvalidA i hiN))
        (by rw [hCR]; code_mem)
      rw [show (0x80024bfc : Word) + 4 = (0x80024c00 : Word) from by decide] at hlbuA
      have hlbuB := liftCode (cr' := CR)
        (bytesRegion_lbu_within .x29 .x7 bPtr (vf .x29) (0x80024c00 : Word)
          bs i (by decide) halignB hib (by omega) (hvalidB i hiN))
        (by rw [hCR]; code_mem)
      rw [show (0x80024c00 : Word) + 4 = (0x80024c04 : Word) from by decide] at hlbuB
      ...
      have hbrHdr := cpsBranchWithin_frameR
        ...
        (cpsBranchWithin_extend_code (cr' := CR)
          (h := beq_spec_gen_within .x5 .x31 (36 : BitVec 13)
            (BitVec.ofNat 64 i) (32 : Word) (0x80024bf0 : Word))
          (hmono := by rw [hCR]; code_mem))
      rw [show (0x80024bf0 : Word) + signExtend13 (36 : BitVec 13)
            = (0x80024c14 : Word) from by decide,
          show (0x80024bf0 : Word) + 4 = (0x80024bf4 : Word) from by decide] at hbrHdr
      ...
      have hbr1 := cpsBranchWithin_frameR
        ...
        (cpsBranchWithin_extend_code (cr' := CR)
          (h := bltu_spec_gen_within .x28 .x29 (16 : BitVec 13) aByte bByte
            (0x80024c04 : Word))
          (hmono := by rw [hCR]; code_mem))
      rw [show (0x80024c04 : Word) + signExtend13 (16 : BitVec 13)
            = (0x80024c14 : Word) from by decide,
          show (0x80024c04 : Word) + 4 = (0x80024c08 : Word) from by decide] at hbr1
      ...
      have hbr2 := cpsBranchWithin_frameR
        ...
        (cpsBranchWithin_extend_code (cr' := CR)
          (h := bltu_spec_gen_within .x29 .x28 (20 : BitVec 13) bByte aByte
            (0x80024c08 : Word))
          (hmono := by rw [hCR]; code_mem))
      rw [show (0x80024c08 : Word) + signExtend13 (20 : BitVec 13)
            = (0x80024c1c : Word) from by decide,
          show (0x80024c08 : Word) + 4 = (0x80024c0c : Word) from by decide] at hbr2
      ...
      rw [show (BitVec.ofNat 64 i : Word) + signExtend12 (1 : BitVec 12)
          ...
          omega,
          show (0x80024c0c : Word) + 4 = (0x80024c10 : Word) from by decide] at haddi
      have hjal := liftCode (cr' := CR)
        (jal_x0_spec_gen_within (-32 : BitVec 21) (0x80024c10 : Word))
        (by rw [hCR]; code_mem)
      rw [show (0x80024c10 : Word) + signExtend21 (-32 : BitVec 21)
            = (0x80024bf0 : Word) from by decide] at hjal
      -- ---- continue segment (both bytes equal): ADDI ; JAL ; IH ----
      have hcont : ¬ BitVec.ult aByte bByte → ¬ BitVec.ult bByte aByte →
          cpsTripleWithin (9 * n + 10) (0x80024c0c : Word)
            (0x80024c20 : Word) CR
      ...
      have hstation2 : ¬ BitVec.ult aByte bByte →
          cpsTripleWithin (1 + (9 * n + 10)) (0x80024c08 : Word)
            (0x80024c20 : Word) CR
            ((.x28 ↦ᵣ aByte) ** (.x29 ↦ᵣ bByte) **
          ((.x5 : Reg) ↦ᵣ BitVec.ofNat 64 i) **
          (.x6 ↦ᵣ (aPtr + BitVec.ofNat 64 i)) **
        ...
        exact cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp)
          (fun _ hq => hq) hst
      -- ---- station 1: BLTU x28 x29 → pick a ----
      have hstation1 : cpsTripleWithin (1 + (1 + (9 * n + 10))) (0x80024c04 : Word)
          (0x80024c20 : Word) CR
          ((.x28 ↦ᵣ aByte) ** (.x29 ↦ᵣ bByte) **
          ((.x5 : Reg) ↦ᵣ BitVec.ofNat 64 i) **
          (.x6 ↦ᵣ (aPtr + BitVec.ofNat 64 i)) **
        ...
        exact cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp)
          (fun _ hq => hq) hst
      -- ---- body ; station 1 ----
      have hbody : cpsTripleWithin (9 * n + 16) (0x80024bf4 : Word)
          (0x80024c20 : Word) CR
          (((.x5 : Reg) ↦ᵣ BitVec.ofNat 64 i) **
...
theorem u256Min_spec (aPtr bPtr outPtr ret : Word) (as bs os : List (BitVec 8))
    ...
    cpsTripleWithin 308 (0x80024be8 : Word) ret
      (CodeReq.ofProg (0x80024be8 : Word) u256Min_prog)
      (((.x10 : Reg) ↦ᵣ aPtr) ** ((.x11 : Reg) ↦ᵣ bPtr) **
        ((.x12 : Reg) ↦ᵣ outPtr) ** ((.x1 : Reg) ↦ᵣ ret) **
  ...
  have hli5 := liftCode (cr' := CR)
    (li_spec_gen_own_within .x5 (0 : Word) (0x80024be8 : Word) (by decide))
    (by rw [hCR]; code_mem)
  rw [show (0x80024be8 : Word) + 4 = (0x80024bec : Word) from by decide,
      show (0 : Word) = BitVec.ofNat 64 0 from rfl] at hli5
  have hli31 := liftCode (cr' := CR)
    (li_spec_gen_own_within .x31 (32 : Word) (0x80024bec : Word) (by decide))
    (by rw [hCR]; code_mem)
  rw [show (0x80024bec : Word) + 4 = (0x80024bf0 : Word) from by decide] at hli31
  -- ---- the byte-walk scan ----
  ...
  have hcopy : cpsTripleWithin 8 (0x80024c20 : Word) (0x80024c40 : Word) CR
      ((((.x5 : Reg) ↦ᵣ (if minSel as bs then aPtr else bPtr)) **
        ((.x12 : Reg) ↦ᵣ outPtr) **
        bytesRegion aPtr as ** bytesRegion bPtr bs ** bytesRegion outPtr os)
        ** regOwn .x6)
    ...
    · rw [if_pos hc, if_pos hc]
      have h := cpsTripleWithin_extend_code
        (hmono := CodeReq.ofProg_mono_sub (0x80024be8 : Word) (0x80024c20 : Word)
          u256Min_prog (dwordCopyProgFrom .x5 .x12 .x6 0 4) 14
          (by decide) (by decide) (by decide) (by decide))
        (selectedDwordCopy_spec .x5 .x12 .x6 (by decide) aPtr outPtr tv as os
          0 4 (by omega) (by omega) (by decide) (0x80024c20 : Word))
      rw [show (0x80024c20 : Word) + BitVec.ofNat 64 (4 * (2 * 4))
            = (0x80024c40 : Word) from by decide,
  ...
  have hli10 := liftCode (cr' := CR)
    (li_spec_gen_within .x10 aPtr (0 : Word) (0x80024c40 : Word) (by decide))
    (by rw [hCR]; code_mem)
  rw [show (0x80024c40 : Word) + 4 = (0x80024c44 : Word) from by decide] at hli10
  have hret := liftCode (cr' := CR)
    (EvmAsm.Evm64.ret_spec_within' (0x80024c44 : Word) ret)
    (by rw [hCR]; code_mem)
