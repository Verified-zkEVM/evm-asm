/-
  Ambient long t1 copy: discharge hdec*/hvalid*/hlover/hlvalid via slice pure +
  bridges + validByteRange. Path Prop + flat success (hitem0..3; hitem4 derived).
-/
...
theorem extractAssumed_copy_longConcrete_pureHvalid_t1_ambient
  ...
  have hdec4 :=
    hdec_ambient_field_long' regionBase loadPtr bs off len listOff items 4 endW
      hptr hbound hencInner hlong hn4 hitem4 hhoff.2.2.2.2 hspan4 hoverEnd_sl hoverEnd_abs hendEq
  ...
  have hlvalid : ∀ k, k < longListLol items →
    ...
    exact isValidByteAccess_of_validByteRange regionBase _ (absListOff + 1 + k)
      hvalidBuf hk_bs
  simpa [absOff0, absOff1, absOff2, absOff3, absOff4, absListOff,
    srcOff0, srcOff1, srcOff2, srcOff3, srcOff4, listOff, slice] using
    extractAssumed_copy_longConcrete_pure_t1_ambient
...
theorem extractAssumed_copy_longConcrete_pureHvalid_t1_ambient_fullCode
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_copy_longConcrete_pureHvalid_t1_ambient
      sp0 spC s regionBase loadPtr lenW toBuf isCreationPtr bs off len items q
      hq_align hq hcover hcvalid
      hspC hret htalign htover htvalid hlen hptr hbound hsuccess hcopyFlag htype1 hdecL hlong
...
set_option maxRecDepth 8000 in
/-- Ambient Assumed footprint under long t1 copy path (statics + path + hitem).
    classical-3. Residual: success_flat case-split.
    classical-3. Residual: long copy arms. -/
theorem extractAssumed_success_flat_copy_t1_long_ambient
  ...
  have hitem4 :
    ...
    have hf := extractSuccess_copy_t1_field4_pfx94_long (txSlice bs off len)
      hsuccess hcopyFlag htype1 items hdecL hlong
    obtain ⟨_, ⟨content, hget, hlen20⟩, _⟩ := hf
    have hval : items[4]'hn4 = .bytes content := (List.getElem?_eq_some_iff.mp hget).2
    have henc : encode (items[4]'hn4) = BitVec.ofNat 8 0x94 :: content := by
      rw [hval, encode_bytes_len20_pfx content hlen20]
    have hlenE : (encode (items[4]'hn4)).length = 21 := by
      simp [henc, hlen20]
    omega
  let s : ExtractSaved :=
  ...
  simpa only [hsra, hs0, hs1, hs2, hs3, hs4, hs5, hs6, hs7] using
    extractAssumed_copy_longConcrete_pureHvalid_t1_ambient_fullCode
