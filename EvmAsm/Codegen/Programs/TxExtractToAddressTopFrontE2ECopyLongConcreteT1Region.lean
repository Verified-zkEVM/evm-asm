/-
  Front E → AfterSave long concrete → t1 20B copy → ret bare region.
-/
...
/-- Long-path E2E t1 copy steps: walk_init 15; 4 walks. -/
def nFrontCopyStepsLongT1Region (lol : Nat) : Nat :=
...
theorem extractFrontCopy_then_epi_of_decode_long_concrete_t1_region
    ...
    cpsTripleWithin (nFrontCopyStepsLongT1Region
        (((txBytes[(teerTxTypeDispatch txBytes).2.2.toNat]'hoff).zeroExtend 64 -
          (0xf7 : Word)).toNat))
  ...
  have hC := extractFrontAfterSaveCopy_then_epi_of_decode_long_t1_region sp0 spC s
...
#print axioms extractFrontCopy_then_epi_of_decode_long_concrete_t1_region
