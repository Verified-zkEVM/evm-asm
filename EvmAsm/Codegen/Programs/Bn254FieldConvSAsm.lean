...
/-- The LE→BE converter body: `init` prologue, then the outer limb `doWhile`
    ...
    The functional spec (`bnfLeToBeFn`) states the genuine inverse relation;
    its VC proof is deferred to a follow-up (it requires a fresh nested-loop
    argument mirroring `bnfBeToLe`'s but for byte-dispersal via `SB`/`SRLI`
    instead of byte-assembly via `LBU`/`OR`/`SLLI`). -/
def bnfLeToBeBody (_src _dst : Word) (_inBytes : List (BitVec 8)) : Stmt :=
  .block "init" [.LI .x5 (0 : Word)] ;;;
  .doWhile "outer" (.bne .x5 .x6) 3 (fun _ _ _ _ => True)
    ( .block "setup"
        [ .SLLI .x6 .x5 (3 : BitVec 6),
          ...
          .ADD .x6 .x11 .x6,
          .LI .x29 (8 : Word) ] ;;;
      .doWhileS "inner" (.bne .x29 .x0) 7 (fun _ _ _ _ _ _ _ => True)
        (.block "body"
...
/-- The LE→BE converter as an `Fn`.  The post is the genuine inverse relation
    ...
    equals the little-endian decode of the four input u64 limbs.  The VC
    proof is deferred (see `bnfLeToBeBody` docstring above). -/
