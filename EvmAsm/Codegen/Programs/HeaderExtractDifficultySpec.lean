/-
  Caller contract for the 8-instruction `header_extract_difficulty` wrapper.

  `headerExtractDifficulty_prog` allocates a 16-byte frame, saves `ra`, shuffles
  its arguments (`a3 := a2` output pointer, `a2 := 7` field index), tail-calls
  ...
  big-endian decode of the real field-7 content (via K34's `Result`).

  The Program still carries a layout placeholder call PC (`2147483664` =
  `0x80000010` = wrapper base + 16) rather than a linked `GuestAddrs` entry —
  `header_extract_difficulty` is not yet in the guest symbol table.  The proof
  therefore pins `H` to that placeholder base so the baked `jalOff` matches.
-/
...
namespace EvmAsm.Codegen.HeaderExtractDifficultySpec
...
/-! ## Base addresses and linked code
    ...
    Header field 7 (`difficulty`) is `Uint` in the reference model; this
    wrapper uses the strict K34 path for canonicality, matching the number
    extractor. -/

/-- Placeholder wrapper base matching the Program's baked call PC
    `2147483664 = H + 16`. Not a linked guest address. -/
abbrev H : Word := (2147483648 : Word)  -- 0x80000000

theorem hdr_length : headerExtractDifficulty_prog.length = 8 := by decide

/-- The wrapper's own re-emitted instructions at the placeholder base. -/
def hdrCode : CodeReq := CodeReq.ofProg H headerExtractDifficulty_prog
...
/-- Success return: `a0 = 0` (or the empty join encoded in K34's `Result`),
    ...
    the field-7 content (pinned inside `successPayload`'s
    `Result`). -/
def hdrSuccess
    ...
    ((.x1 ↦ᵣ raIn) ** (spH ↦ₘ raIn) ** (.x2 ↦ᵣ sp0) **
      ((.x8 ↦ᵣ outer.s0) ** (.x9 ↦ᵣ outer.s1) ** savedFrame newSp outer **
       successPayload newSp listBase offset len v12 x5 scalarStatus
         wrapperStatus outputValue saved bytes listLen 7)) h
...
theorem hdrPrologue
    ...
    cpsTripleWithin 4 H (H + 16) fullCode
      ...
      ((.x1 ↦ᵣ raIn) ** (spH ↦ₘ raIn) **
       flatPre spH newSp listBase listLenW (7 : Word) outputPtr oldOut
  ...
  have h0' := cpsTripleWithin_extend_code (cr' := hdrCode)
    (CodeReq.ofProg_mem_at H H headerExtractDifficulty_prog 0
  ...
  have h1' := cpsTripleWithin_extend_code (cr' := hdrCode)
    (CodeReq.ofProg_mem_at H (H + 4) headerExtractDifficulty_prog 1
  ...
  have h2' := cpsTripleWithin_extend_code (cr' := hdrCode)
    (CodeReq.ofProg_mem_at H (H + 8) headerExtractDifficulty_prog 2 (.MV .x13 .x12)
      (by bv_omega) (by rw [hdr_length]; decide) rfl
      (by rw [hdr_length]; decide)) h2
  -- [3] LI x12 8 : x12 := 8
  have h3 := li_spec_gen_within .x12 outputPtr (7 : Word) (H + 12) (by decide)
  have h3' := cpsTripleWithin_extend_code (cr' := hdrCode)
    (CodeReq.ofProg_mem_at H (H + 12) headerExtractDifficulty_prog 3
      (.LI .x12 (7 : Word)) (by bv_omega) (by rw [hdr_length]; decide) rfl
...
theorem epiCore (sp0 spH raIn : Word) (G : Assertion) (hG : G.pcFree)
  ...
  have h5' := cpsTripleWithin_extend_code hdr_mono
    (cpsTripleWithin_extend_code (cr' := hdrCode)
      (CodeReq.ofProg_mem_at H (H + 20) headerExtractDifficulty_prog 5
  ...
  have h6' := cpsTripleWithin_extend_code hdr_mono
    (cpsTripleWithin_extend_code (cr' := hdrCode)
      (CodeReq.ofProg_mem_at H (H + 24) headerExtractDifficulty_prog 6
  ...
  have h7' := cpsTripleWithin_extend_code hdr_mono
    (cpsTripleWithin_extend_code (cr' := hdrCode)
      (CodeReq.ofProg_mem_at H (H + 28) headerExtractDifficulty_prog 7
...
theorem hdrEpilogue
    ...
    cpsTripleWithin 3 (H + 20) raIn fullCode
      ((.x1 ↦ᵣ (H + 20)) ** (spH ↦ₘ raIn) **
       flatPost spH newSp listBase oldOffset oldLen outer saved bytes listLen 7)
  ...
  have tripleS : cpsTripleWithin 3 (H + 20) raIn fullCode
      ((.x1 ↦ᵣ (H + 20)) ** (spH ↦ₘ raIn) **
       flatSuccessReturned spH newSp listBase outer saved bytes listLen 7)
      (hdrPost sp0 spH newSp raIn listBase oldOffset oldLen outer saved bytes
        listLen) :=
    ...
    (fun h' (hh : ((spH ↦ₘ raIn) **
        (fun h'' => flatSuccessReturned spH newSp listBase outer saved bytes
          ...
          flatFailureReturned spH newSp listBase oldOffset oldLen outer saved
            bytes listLen 7 h'')) h') => sepConj_or_split h' hh) h hp
...
set_option maxRecDepth 8000 in
/-- **`header_extract_difficulty` caller contract.**  The 8-instruction wrapper =
    prologue ;; `rlpFieldToU64_flat_spec_within` (field index 7) ;; epilogue.
    On `a0 = 0` the caller's output cell holds the big-endian decode
    of the real field-7 content (pinned inside `successPayload`'s `Result`);
    `a0 = 1` is an RLP parse failure and `a0 = 2` a field wider than 8 bytes,
    both with a zeroed output cell. -/
theorem header_extract_difficulty_spec_within
    (sp0 raIn oldRaSlot spH newSp listBase listLenW outputPtr old13 old14
      oldOut oldOffset oldLen s0In s1In s2 s3 s4 s5 : Word)
    (bytes : List (BitVec 8)) (listLen : Nat)
    (hspH : spH = sp0 + signExtend12 (-16 : BitVec 12))
    ...
    let saved : EvmAsm.Codegen.RlpListNthItemSAsm.Saved :=
      { ra := B + 48, s0 := listBase, s1 := outputPtr, s2 := s2, s3 := s3,
        s4 := s4, s5 := s5 }
    let callSteps := 1 + ((12 + ((85 + 93 * (7 + 2)) + 6)) + 9)
  ...
  have hflat := rlpFieldToU64_flat_spec_within spH newSp listBase listLenW
    (7 : Word) outputPtr oldOut oldOffset oldLen old14 outer s2 s3 s4 s5 bytes
    listLen 7 hnewSp hlistLenW (by decide) (by decide) hsalign hslack hover
  ...
  have hcall := callWithin_spec (H + 16) B raIn
    (jalOff GuestAddrs.rlp_field_to_u64_strict 2147483664)
    n34 (by
      change BitVec.ofNat 64 2147483664 +
          signExtend21 (jalOff GuestAddrs.rlp_field_to_u64_strict 2147483664) =
        BitVec.ofNat 64 GuestAddrs.rlp_field_to_u64_strict
      exact jalOff_correct GuestAddrs.rlp_field_to_u64_strict 2147483664
        (by decide))
    (fun a i hi => hdr_mono a i
      (CodeReq.ofProg_mem_at H (H + 16) headerExtractDifficulty_prog 4
        (.JAL .x1 (jalOff GuestAddrs.rlp_field_to_u64_strict 2147483664))
        (by bv_omega) (by rw [hdr_length]; decide) rfl
        (by rw [hdr_length]; decide) a i hi))
  ...
  have hcall' : cpsTripleWithin (1 + n34) (H + 16) (H + 20) fullCode
      ((.x1 ↦ᵣ raIn) ** (spH ↦ₘ raIn) **
        flatPre spH newSp listBase listLenW (7 : Word) outputPtr oldOut oldOffset
...
end EvmAsm.Codegen.HeaderExtractDifficultySpec
