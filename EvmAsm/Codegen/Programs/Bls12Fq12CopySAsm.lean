...
namespace Bls12Fq12CopySAsm

def frameOk576 (src dst : Word) : Prop :=
  src.toNat + 576 < 2 ^ 64 ∧ dst.toNat + 576 < 2 ^ 64 ∧
    (src.toNat + 576 ≤ dst.toNat ∨ dst.toNat + 576 ≤ src.toNat)

def copyWin576 (srcBytes orig : List (BitVec 8)) (i : Nat) : List (BitVec 8) :=
  srcBytes.take (8 * i) ++ orig.drop (8 * i)

theorem copyWin576_zero (srcBytes orig : List (BitVec 8)) :
    copyWin576 srcBytes orig 0 = orig := by
  simp [copyWin576]

theorem copyWin576_72_eq (srcBytes orig : List (BitVec 8))
    (hs : srcBytes.length = 576) (ho : orig.length = 576) :
    copyWin576 srcBytes orig 72 = srcBytes := by
  simp only [copyWin576, Nat.reduceMul]
  rw [List.take_of_length_le (by omega), List.drop_eq_nil_of_le (by omega), List.append_nil]

theorem length_copyWin576 (srcBytes orig : List (BitVec 8)) (i : Nat)
    (hs : srcBytes.length = 576) (ho : orig.length = 576) (hi : i ≤ 72) :
    (copyWin576 srcBytes orig i).length = 576 := by
  simp only [copyWin576, List.length_append, List.length_take, List.length_drop, hs, ho]
  omega

theorem copyWin576_step (srcBytes orig : List (BitVec 8)) (i : Nat)
    (hs : srcBytes.length = 576) (ho : orig.length = 576) (hi : i < 72) :
    setBytes (copyWin576 srcBytes orig i) (8 * i)
      (dwordBytes (packBytes ((srcBytes.drop (8 * i)).take 8))) =
    copyWin576 srcBytes orig (i + 1) := by
...
private theorem src_miss8 (src dst : Word) (ws : List (BitVec 8)) (k : Nat)
    (hk : k < 72) (hws : ws.length = 576) (hfr : frameOk576 src dst) :
...
private theorem copy_engine (src dst : Word) (srcBytes : List (BitVec 8))
    (rf : RegFile) (ws : List (BitVec 8)) (i : Nat) (hi : i < 72)
    (hx10 : rf.get .x10 = src + BitVec.ofNat 64 (8 * i))
    (hx11 : rf.get .x11 = dst + BitVec.ofNat 64 (8 * i))
    (hws : ws.length = 576) (hfr : frameOk576 src dst) :
...
private theorem copy_blockVCs (src dst : Word) (srcBytes : List (BitVec 8))
    ...
    (hx10 : rf.get .x10 = src + BitVec.ofNat 64 (8 * i))
    (hx11 : rf.get .x11 = dst + BitVec.ofNat 64 (8 * i))
    (hws : ws.length = 576) (hs : srcBytes.length = 576) (hfr : frameOk576 src dst) :
...
def copyInv (src dst : Word) (srcBytes orig : List (BitVec 8)) :
  ...
  fun i rf ws A =>
    ...
    i < 72 ∧ srcBytes.length = 576 ∧ orig.length = 576 ∧ frameOk576 src dst ∧
    ws = copyWin576 srcBytes orig (i + 1) ∧ A = empAssertion

def blqCopyBody (src dst : Word) (srcBytes orig : List (BitVec 8)) : Stmt :=
  .block "init" [.LI .x7 (72 : Word)] ;;;
  .doWhile "loop" (.bne .x7 .x0) 71 (copyInv src dst srcBytes orig)
    (.block "copy" copyStepBlock)

def blqCopyFn (src dst : Word) (srcBytes orig : List (BitVec 8)) : Fn where
  name := "blqCopy"
  region := ⟨src, srcBytes⟩
  rw := ⟨dst, 576⟩
  pre := fun rf ws A =>
    rf.get .x10 = src ∧ rf.get .x11 = dst ∧ ws = orig ∧
    srcBytes.length = 576 ∧ orig.length = 576 ∧ frameOk576 src dst ∧ A = empAssertion
  post := fun _ ws A => ws = srcBytes ∧ A = empAssertion
  body := blqCopyBody src dst srcBytes orig

def blqCopy_verified : Program :=
  (blqCopyBody 0 0 [] []).flatten 0

#guard (blqCopy_verified : List Instr).length = 7
#guard (blqCopyBody 0 0 [] []).flatten 0 = (blqCopyBody 0 0 [] []).flatten 0x80000000
#guard (blqCopyBody 0 0 [] []).flatten 0 ++ [Instr.JALR .x0 .x1 0] = blqCopy_prog

theorem blqCopyFn_spec (src dst : Word) (srcBytes orig : List (BitVec 8))
    (hwf : (Region.mk src srcBytes).wf) (hrww : RwRegion.wf ⟨dst, 576⟩) (base : Word) :
    (blqCopyFn src dst srcBytes orig).Spec base := by
  have hbase : (blqCopyFn src dst srcBytes orig).rw.base = dst := rfl
  vcgen
  case region => exact ⟨hwf, hrww⟩
  case blqCopy.loop.inv_init =>
    rintro rf' ws' A' h
    ...
    dsimp only [blqCopyFn] at hbase ⊢
    have hx10Init : (execBlock ⟨src, srcBytes⟩ dst rfInit ws0
        [Instr.LI Reg.x7 72]).1.get .x10 = src := by
    ...
    · change setBytes ws0 (8 * 0)
        ...
        copyWin576 srcBytes ws0 (0 + 1)
      simpa [copyWin576_zero srcBytes ws0] using
        copyWin576_step srcBytes ws0 0 hs ho (by omega)
  case blqCopy.loop.inv_step =>
    rintro i hi rf' ws' A' ⟨rf₀, ws₀, -, ⟨⟨hx10, hx11, hx7, hlt, hs, ho, hfr, hws₀, hA⟩, hcond⟩, rfl, rfl⟩
    dsimp only [blqCopyFn] at hbase ⊢
    have hwsLen : ws₀.length = 576 := by
      rw [hws₀]
      exact length_copyWin576 srcBytes orig (i + 1) hs ho (by omega)
    ...
    · rw [hws₀, copyWin576_step srcBytes orig (i + 1) hs ho (by omega)]
  case blqCopy.loop.exhausted =>
    rintro rf ws A ⟨-, -, hx7, -, -, -, -, -, -⟩
    simp only [Cond.holds, hx7, not_not, RegFile.get_x0]
    decide
  case blqCopy.loop.body.copy.mem =>
    ...
    · rcases hpre with ⟨rfInit, wsInit, -, ⟨hx10, hx11, rfl, hs, ho, hfr, hA⟩, rfl, rfl⟩
      ...
      exact copy_blockVCs src dst srcBytes _ ws 0 (by omega)
        ...
        (by
          simp only [execBlock_cons, execBlock_nil, execInstrRF, aluSem, RegFile.get_set_ne,
            ne_eq, reduceCtorEq, not_false_eq_true, hx11]
          bv_omega) hlen576 hs hfr
    · rcases hloop with ⟨i, hi, ⟨hx10, hx11, hx7, hlt, hs, ho, hfr, hws, hA⟩, hcond⟩
      have hlen576 : ws.length = 576 := by
        change ws.length = 576 at hlen
        exact hlen
      exact copy_blockVCs src dst srcBytes rf ws (i + 1) (by omega) hx10 hx11 hlen576 hs hfr
  case blqCopy.post =>
    ...
    have hi71 : i = 71 := by
      ...
      rfl
    subst hi71
    rw [hws, copyWin576_72_eq srcBytes orig hs ho]
    exact ⟨rfl, hA⟩

/-! ## Flat linked-entry contract -/

def blqCopyCr : CodeReq :=
  CodeReq.ofProg (GuestAddrs.blq_copy : Word) blqCopy_prog

def blqCopyScratch : List Reg :=
...
private theorem exposedRegs_split_copy (vf : Reg → Word) :
    regAtomsOf vf exposedRegs
      = ((.x10 ↦ᵣ vf .x10) ** (.x11 ↦ᵣ vf .x11) **
          regAtomsOf vf blqCopyScratch) := by
  show regAtomsOf vf
      [.x5, .x6, .x7, .x28, .x29, .x30, .x31,
       .x10, .x11, .x12, .x13, .x14, .x15, .x16, .x17] = _
  simp only [blqCopyScratch, regAtomsOf_cons, regAtomsOf_nil]
  xperm

private theorem copy_args_notin_scratch :
    ∀ r ∈ blqCopyScratch, r ≠ (.x10 : Reg) ∧ r ≠ (.x11 : Reg) := by
  decide

theorem blqCopyFlat_spec (ret src dst : Word)
    (srcBytes orig : List (BitVec 8))
    (hwf : (Region.mk src srcBytes).wf)
    (hrww : RwRegion.wf ⟨dst, 576⟩)
    (hs : srcBytes.length = 576) (ho : orig.length = 576)
    (hfr : frameOk576 src dst)
    (hsz : 4 * ((blqCopyFn src dst srcBytes orig).body.size + 1) ≤ 2 ^ 64)
    (halign : (ret &&& ~~~(1 : Word)) = ret) :
    cpsTripleWithin
      ((blqCopyFn src dst srcBytes orig).body.steps + 1)
      (GuestAddrs.blq_copy : Word) ret blqCopyCr
      (((.x1 : Reg) ↦ᵣ ret) ** (.x10 ↦ᵣ src) ** (.x11 ↦ᵣ dst) **
        regOwns blqCopyScratch ** bytesRegion dst orig ** bytesRegion src srcBytes)
      (((.x1 : Reg) ↦ᵣ ret) ** regOwns exposedRegs **
        bytesRegion dst srcBytes ** bytesRegion src srcBytes) := by
  refine cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp) (fun _ hq => hq)
    (cpsTripleWithin_peel_regOwns blqCopyScratch (by decide)
      (P := ((.x1 : Reg) ↦ᵣ ret) ** (.x10 ↦ᵣ src) ** (.x11 ↦ᵣ dst) **
        bytesRegion dst orig ** bytesRegion src srcBytes)
      (fun vf => ?_))
  have hpre : (blqCopyFn src dst srcBytes orig).pre
  ...
  have had := Fn.retSpecFlatAmbient
    (blqCopyFn src dst srcBytes orig)
    (GuestAddrs.blq_copy : Word)
    (blqCopyFn_spec src dst srcBytes orig hwf hrww
      (GuestAddrs.blq_copy : Word))
    (by simpa [blqCopyFn] using hsz) ret halign
    (fun r => if r = .x10 then src else if r = .x11 then dst else vf r)
    orig empAssertion pcFree_emp (by simpa [blqCopyFn] using ho) hpre
    (fun _ _ _ hpost => hpost.2)
    (Q := regOwns exposedRegs ** bytesRegion dst srcBytes)
    (fun rf' ws' hlen' hpost' hp hh => by
      obtain ⟨hws', -⟩ := hpost'
      rw [hws', show (blqCopyFn src dst srcBytes orig).rw.base = dst from rfl] at hh
  ...
  rw [show (blqCopyFn src dst srcBytes orig).programRet
      (GuestAddrs.blq_copy : Word) = blqCopy_prog from rfl] at had
  have hadC := had
  rw [show (blqCopyFn src dst srcBytes orig).rw.base = dst from rfl,
    show (blqCopyFn src dst srcBytes orig).region = Region.mk src srcBytes from rfl] at hadC
  simp only [sepConj_emp_right'] at hadC
  rw [regFileIs_eq_regAtoms, regAtoms_eq_regAtomsOf _ _ (by decide),
    exposedRegs_split_copy,
