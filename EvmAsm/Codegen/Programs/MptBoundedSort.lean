...
def mptBoundedBuildMissingSubtreeFunction : String :=
  ...
  ".Lmbms_finish:\n  beqz s6, .Lmbms_deleted; li t0, 2; bgeu s6, t0, .Lmbms_branch; li t0, 1; bne s6, t0, .Lmbms_fail; slli t0, s7, 5; slli t1, s7, 3; add t0, t0, t1; add t0, s4, t0; addi t1, t0, 8; ld t2, 0(t0); sd t1, " ++ toString bsrMptFrameExtensionChildPtrOffset ++ "(s4); sd t2, " ++ toString bsrMptFrameExtensionChildLenOffset ++ "(s4); sb s7, " ++ toString bsrMptFrameExtensionPathOffset ++ "(s4); li t0, 1; sd t0, " ++ toString bsrMptFrameExtensionPathLenOffset ++ "(s4); mv a0, s4; la a1, bsr_builder_node; la a2, bsr_builder_result_ref; la a3, bsr_builder_result_len; jal ra, mpt_bounded_encode_extension; j .Lmbms_ret\n" ++
