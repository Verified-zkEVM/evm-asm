/-
  Wire pure honesty into ambient short concrete Assumed creation for legacy (type=0).
...
theorem extractAssumed_creation_shortConcrete_pure_legacy_ambient
  ...
  have hfields02 :=
    extractSuccess_creation_legacy_hnext_fields02 slice hsuccess hcreFlag htype0
  ...
  have hhnext :=
    extractSuccess_creation_legacy_hnext_hcre_srcOff slice loadPtr hsuccess
  ...
  have hcre : ∀ (next3 len3 : Word),
    ...
    simpa [absOff3, srcOff3, listOff, endW, absListOff] using
      hcre_ambient_of_slice regionBase loadPtr bs off len listOff items 3 endW
        hptr hbound hencInner hshort hn3 hoff3_sl hspan3a hroom3
  ...
  have hss0 :=
    hss_ambient_of_short_list_field regionBase loadPtr bs off len listOff items 0
      hptr hbound hencInner hshort hn0 hoff0_sl hover (Or.inl hfields02.1)
      hvalid1_0 hoff0
  have hss1 :=
    hss_ambient_of_short_list_field regionBase loadPtr bs off len listOff items 1
      hptr hbound hencInner hshort hn1 hoff1_sl hover (Or.inl hfields02.2.1)
      hvalid1_1 hoff1
  have hss2 :=
    hss_ambient_of_short_list_field regionBase loadPtr bs off len listOff items 2
      hptr hbound hencInner hshort hn2 hoff2_sl hover (Or.inl hfields02.2.2)
      hvalid1_2 hoff2
  have hss3 :=
  ...
  exact extractAssumed_creation_under_honesty_of_decode_short_concrete_legacy_ambient
...
set_option maxRecDepth 8000 in
theorem extractAssumed_creation_shortConcrete_pure_legacy_ambient_fullCode
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (items : List EL.RLP.RLPItem)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_creation_shortConcrete_pure_legacy_ambient sp0 spC s
...
#print axioms extractAssumed_creation_shortConcrete_pure_legacy_ambient
#print axioms extractAssumed_creation_shortConcrete_pure_legacy_ambient_fullCode
