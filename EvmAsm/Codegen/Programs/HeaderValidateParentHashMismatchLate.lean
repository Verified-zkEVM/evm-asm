/-
  EvmAsm.Codegen.Programs.HeaderValidateParentHashMismatchLate

  Dword-1/2/3-mismatch residuals for `header_validate_parent_hash`
  (status = 2, first differing compare round at k = 1, 2, 3).  Same
  namespace as `HeaderValidateParentHashSpec`; the chain mirrors
  `HeaderValidateParentHashMismatch` (the k = 0 residual) with the
  `hvphCompareRound{j}Eq` fall-through rounds prepended, so the unified
  whole-routine theorem can state its status-2 outcome as "first
  differing dword k" for ANY k < 4.

  Cost arithmetic per k: setup 4 ;; mismatch-exit (10 + 3*k) gives
  `14 + 3*k` from `H+72`; keccak leg `(5 + nK) + (14 + 3*k) =
  19 + 3*k + nK` from `H+52`; headers leg `20 + 3*k + nK` from `H+40`.
-/
...
/-- Concrete-scratch helper for `hvphFromCompareSetupMismatch1`. -/
theorem hvphFromCompareSetupMismatch1_vals
    ...
    cpsTripleWithin 17 (H + 72) (saved .x1) hvphCode
      ((.x2 ↦ᵣ spC) ** (.x1 ↦ᵣ link) **
  ...
  have hexit := hvphCompareMismatch1Exit sp0 spC ret saved cur
    claimedBytes computedBytes v7 v28 o10 G hG hspC
    (by simpa [saved, hvphFrameVals] using hret) hclen hcdlen h0 h_ne
...
set_option maxRecDepth 8000 in
/-- From `H+72`: `la` ptrs ;; eq0 ;; ne1 mismatch ;; status-2. Cost `17`.
    Scratch `x5/x6/x7/x28` are `regOwn`. -/
theorem hvphFromCompareSetupMismatch1
    (sp0 spC ret link parentPtr parentLen : Word) (vals : Reg → Word)
    (claimedBytes computedBytes : List (BitVec 8))
    (o10 : Word)
    (G : Assertion) (hG : G.pcFree)
  ...
  refine cpsTripleWithin_weaken
    ...
    (fun _ hp => by xperm_hyp hp) (fun _ hq => hq) ?_
  simpa [saved] using
    hvphFromCompareSetupMismatch1_vals sp0 spC ret link parentPtr parentLen vals
      claimedBytes computedBytes old5 old6 v7 v28 o10 G hG hspC hret
      hclen hcdlen h0 h_ne

set_option maxRecDepth 8000 in
/-- Keccak setup+call (`H+52`) ;; dword-1 mismatch exit. Cost `22+nK`. -/
theorem hvphKeccakThenMismatch1
    ...
    (h_ne : dwordAt claimedBytes 1 ≠ dwordAt (keccakBodyDigest parentBytes N rem) 1) :
    ...
    let saved := hvphFrameVals ret vals
    cpsTripleWithin (22 + nKeccak N rem) (H + 52) (saved .x1) fullCode
      ((.x2 ↦ᵣ spC) ** (.x1 ↦ᵣ (H + 40)) **
        (.x8 ↦ᵣ parentPtr) ** (.x9 ↦ᵣ parentLen) ** (.x18 ↦ᵣ vals .x18) **
      ...
      ((.x10 ↦ᵣ (2 : Word)) ** (.x1 ↦ᵣ saved .x1) ** (.x2 ↦ᵣ sp0) **
        ...
        frameSlotsSaved hvphFrame spC saved **
        (.x5 ↦ᵣ Claimed) ** (.x6 ↦ᵣ Computed) **
        (.x7 ↦ᵣ dwordAt claimedBytes 1) ** (.x28 ↦ᵣ dwordAt digest 1) **
  ...
  have hG : Gmm.pcFree := by
    ...
    refine pcFree_sepConj ?_ (pcFree_sepConj ?_ (pcFree_sepConj (bytesRegion_pcFree _ _)
      ...
      <;> pcf
  have hmm0 := hvphFromCompareSetupMismatch1 sp0 spC ret (H + 72) parentPtr parentLen vals
    claimedBytes digest (0 : Word) Gmm hG hspC hret hclen hcdlen h0 h_ne
  ...
  have hn : (5 + nKeccak N rem) + 17 = 22 + nKeccak N rem := by omega
  rw [← hn]
...
set_option maxRecDepth 8000 in
/-- Concrete-scratch helper: `H+40` beq-ok ;; keccak ;; dword-1-mismatch. Cost `23+nK`. -/
theorem hvphFromHeadersMismatch1_vals
    (sp0 spC ret parentPtr parentLen : Word) (vals : Reg → Word)
    ...
    cpsTripleWithin (23 + nKeccak N rem) (H + 40) (saved .x1) fullCode
  ...
  have hbeqF := cpsTripleWithin_frameR
    ...
    (by
      ...
      · pcf
      · pcf) hbeq
  have hkm := hvphKeccakThenMismatch1 sp0 spC ret parentPtr parentLen vals
  ...
  have hn : 1 + (22 + nKeccak N rem) = 23 + nKeccak N rem := by omega
  rw [← hn]
  exact cpsTripleWithin_weaken (fun _ hp => by
    simp only [Amb] at hp ⊢
    xperm_hyp hp) (fun _ hq => by
...
set_option maxRecDepth 8000 in
/-- From `H+40` with status 0: beq-ok ;; keccak ;; dword-1-mismatch. Cost `23+nK`. -/
theorem hvphFromHeadersMismatch1
    (sp0 spC ret parentPtr parentLen : Word) (vals : Reg → Word)
    (v20 : Word)
    (parentBytes claimedBytes : List (BitVec 8)) (N rem : Nat)
    (os : List (BitVec 8))
  ...
  simpa [digest, kvals, out0, saved, Amb] using
    hvphFromHeadersMismatch1_vals sp0 spC ret parentPtr parentLen vals
...
set_option maxRecDepth 8000 in
/-- Concrete-scratch helper for `hvphFromCompareSetupMismatch2`. -/
theorem hvphFromCompareSetupMismatch2_vals
    (sp0 spC ret link parentPtr parentLen : Word) (vals : Reg → Word)
    (claimedBytes computedBytes : List (BitVec 8))
    (old5 old6 v7 v28 o10 : Word)
    (G : Assertion) (hG : G.pcFree)
    ...
    let saved := hvphFrameVals ret vals
    cpsTripleWithin 20 (H + 72) (saved .x1) hvphCode
      ((.x2 ↦ᵣ spC) ** (.x1 ↦ᵣ link) **
        (.x8 ↦ᵣ parentPtr) ** (.x9 ↦ᵣ parentLen) ** (.x18 ↦ᵣ vals .x18) **
        (.x5 ↦ᵣ old5) ** (.x6 ↦ᵣ old6) **
        (.x7 ↦ᵣ v7) ** (.x28 ↦ᵣ v28) ** (.x10 ↦ᵣ o10) **
  ...
  have hsetup := cpsTripleWithin_frameR
    ...
    (by refine pcFree_sepConj ?_ (pcFree_sepConj ?_ (pcFree_sepConj ?_
          (pcFree_sepConj ?_ (pcFree_sepConj ?_ hG))))
        <;> first | exact bytesRegion_pcFree _ _ | pcf) hsetup0
  have hexit := hvphCompareMismatch2Exit sp0 spC ret saved cur
    claimedBytes computedBytes v7 v28 o10 G hG hspC
    (by simpa [saved, hvphFrameVals] using hret) hclen hcdlen h0 h1 h_ne
  have hregs : regsAt hvphFrame cur =
      ((.x1 ↦ᵣ link) ** (.x8 ↦ᵣ parentPtr) ** (.x9 ↦ᵣ parentLen) **
        (.x18 ↦ᵣ vals .x18)) := by
    simp [cur, hvphFrame, regsAt, sepConj_emp_right']
...
/-- From `H+72`: `la` ptrs ;; eq0 ;; ne1 mismatch ;; status-2. Cost `17`.
    Scratch `x5/x6/x7/x28` are `regOwn`. -/
theorem hvphFromCompareSetupMismatch2
    (sp0 spC ret link parentPtr parentLen : Word) (vals : Reg → Word)
    (claimedBytes computedBytes : List (BitVec 8))
    (o10 : Word)
    (G : Assertion) (hG : G.pcFree)
  ...
  refine cpsTripleWithin_weaken
    ...
    (fun _ hp => by xperm_hyp hp) (fun _ hq => hq) ?_
  simpa [saved] using
    hvphFromCompareSetupMismatch2_vals sp0 spC ret link parentPtr parentLen vals
      claimedBytes computedBytes old5 old6 v7 v28 o10 G hG hspC hret
      hclen hcdlen h0 h1 h_ne

set_option maxRecDepth 8000 in
/-- Keccak setup+call (`H+52`) ;; dword-2 mismatch exit. Cost `22+nK`. -/
theorem hvphKeccakThenMismatch2
    (sp0 spC ret parentPtr parentLen : Word) (vals : Reg → Word)
    (old10 old11 old12 v20 v28 v29 : Word)
    (parentBytes claimedBytes : List (BitVec 8)) (N rem : Nat)
    (os : List (BitVec 8))
    ...
    (h_ne : dwordAt claimedBytes 2 ≠ dwordAt (keccakBodyDigest parentBytes N rem) 2) :
    let digest := keccakBodyDigest parentBytes N rem
    let kvals := keccakEntryVals parentPtr parentLen (vals .x18) v20
    let out0 := List.replicate 32 (0 : BitVec 8)
    let saved := hvphFrameVals ret vals
    cpsTripleWithin (25 + nKeccak N rem) (H + 52) (saved .x1) fullCode
      ((.x2 ↦ᵣ spC) ** (.x1 ↦ᵣ (H + 40)) **
        (.x8 ↦ᵣ parentPtr) ** (.x9 ↦ᵣ parentLen) ** (.x18 ↦ᵣ vals .x18) **
        (.x10 ↦ᵣ old10) ** (.x11 ↦ᵣ old11) ** (.x12 ↦ᵣ old12) **
        (.x20 ↦ᵣ v20) **
      ...
      ((.x10 ↦ᵣ (2 : Word)) ** (.x1 ↦ᵣ saved .x1) ** (.x2 ↦ᵣ sp0) **
        ...
        frameSlotsSaved hvphFrame spC saved **
        (.x5 ↦ᵣ Claimed) ** (.x6 ↦ᵣ Computed) **
        (.x7 ↦ᵣ dwordAt claimedBytes 2) ** (.x28 ↦ᵣ dwordAt digest 2) **
        claimedOwn claimedBytes ** bytesRegion Computed digest **
        (frameSlotsSaved keccakFrame (spC + signExtend12 (-32 : BitVec 12)) kvals **
          (.x20 ↦ᵣ v20) **
          bytesRegion (BitVec.ofNat 64 GuestAddrs.zk3_state)
  ...
  have hG : Gmm.pcFree := by
    ...
    refine pcFree_sepConj ?_ (pcFree_sepConj ?_ (pcFree_sepConj (bytesRegion_pcFree _ _)
      (pcFree_sepConj ?_ (pcFree_sepConj (pcFree_regOwns _)
        (pcFree_sepConj (keccakCallerFreeA_pcFree _ _ _ _ (by pcf)) hF)))))
      <;> pcf
  have hmm0 := hvphFromCompareSetupMismatch2 sp0 spC ret (H + 72) parentPtr parentLen vals
    claimedBytes digest (0 : Word) Gmm hG hspC hret hclen hcdlen h0 h1 h_ne
  have hmm := cpsTripleWithin_extend_code hvph_mono hmm0
  have hall := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by
    unfold keccakCallerPost at hp
    have hregs : regsAt keccakFrame kvals =
    ...
    unfold Gmm
    xperm_hyp hp) hcall hmm
  have hn : (5 + nKeccak N rem) + 20 = 25 + nKeccak N rem := by omega
  rw [← hn]
  exact cpsTripleWithin_weaken (fun _ hp => by
    simp only [saved, out0] at hp ⊢
    xperm_hyp hp) (fun _ hq => by
...
set_option maxRecDepth 8000 in
/-- Concrete-scratch helper: `H+40` beq-ok ;; keccak ;; dword-2-mismatch. Cost `23+nK`. -/
theorem hvphFromHeadersMismatch2_vals
    (sp0 spC ret parentPtr parentLen : Word) (vals : Reg → Word)
    (old11 old12 v20 v28 v29 : Word)
    (parentBytes claimedBytes : List (BitVec 8)) (N rem : Nat)
    (os : List (BitVec 8))
    ...
    let Amb := hvphSuccKeccakAmb spC v20 os out0
      (bytesRegion thisPtr thisBytes ** F)
    cpsTripleWithin (26 + nKeccak N rem) (H + 40) (saved .x1) fullCode
      ((.x1 ↦ᵣ (H + 40)) ** (.x10 ↦ᵣ (0 : Word)) ** (.x0 ↦ᵣ (0 : Word)) **
        claimedOwn claimedBytes **
        headersCallFrameSuccCore spC ret parentPtr parentLen vals parentBytes **
        (.x28 ↦ᵣ v28) ** (.x29 ↦ᵣ v29) **
  ...
  have hbeqF := cpsTripleWithin_frameR
    ...
    (by
      ...
      · pcf
      · pcf) hbeq
  have hkm := hvphKeccakThenMismatch2 sp0 spC ret parentPtr parentLen vals
    (0 : Word) old11 old12 v20 v28 v29 parentBytes claimedBytes N rem os
    (bytesRegion thisPtr thisBytes ** F)
    (by refine pcFree_sepConj (bytesRegion_pcFree _ _) hF)
    hspC hret hplen hlen hrem_le hos
  ...
  have hall := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by
    ...
    simp only [out0, regOwns, keccakBodyFreeTemps, sepConj_emp_right'] at hp ⊢
    xperm_hyp hp) hbeqF hkm
  have hn : 1 + (25 + nKeccak N rem) = 26 + nKeccak N rem := by omega
  rw [← hn]
  exact cpsTripleWithin_weaken (fun _ hp => by
    simp only [Amb] at hp ⊢
    xperm_hyp hp) (fun _ hq => by
...
set_option maxRecDepth 8000 in
/-- From `H+40` with status 0: beq-ok ;; keccak ;; dword-2-mismatch. Cost `23+nK`. -/
theorem hvphFromHeadersMismatch2
    (sp0 spC ret parentPtr parentLen : Word) (vals : Reg → Word)
    (v20 : Word)
    (parentBytes claimedBytes : List (BitVec 8)) (N rem : Nat)
    (os : List (BitVec 8))
  ...
  refine cpsTripleWithin_weaken
    ...
    (fun _ hp => by xperm_hyp hp) (fun _ hq => hq) ?_
  simpa [digest, kvals, out0, saved, Amb] using
    hvphFromHeadersMismatch2_vals sp0 spC ret parentPtr parentLen vals
      old11 old12 v20 v28 v29 parentBytes claimedBytes N rem os
      thisPtr thisBytes F hF hspC hret hplen hlen hrem_le hos
      halign_zk hover hNbound hrem64 hb8i
      hovers hoveri hvalids hvalidi hvalidRem hvalid135 hvalidMem
...
set_option maxRecDepth 8000 in
/-- Concrete-scratch helper for `hvphFromCompareSetupMismatch3`. -/
theorem hvphFromCompareSetupMismatch3_vals
    (sp0 spC ret link parentPtr parentLen : Word) (vals : Reg → Word)
    (claimedBytes computedBytes : List (BitVec 8))
    (old5 old6 v7 v28 o10 : Word)
    (G : Assertion) (hG : G.pcFree)
    ...
    let saved := hvphFrameVals ret vals
    cpsTripleWithin 23 (H + 72) (saved .x1) hvphCode
      ((.x2 ↦ᵣ spC) ** (.x1 ↦ᵣ link) **
        (.x8 ↦ᵣ parentPtr) ** (.x9 ↦ᵣ parentLen) ** (.x18 ↦ᵣ vals .x18) **
        (.x5 ↦ᵣ old5) ** (.x6 ↦ᵣ old6) **
        (.x7 ↦ᵣ v7) ** (.x28 ↦ᵣ v28) ** (.x10 ↦ᵣ o10) **
  ...
  have hsetup := cpsTripleWithin_frameR
    ...
    (by refine pcFree_sepConj ?_ (pcFree_sepConj ?_ (pcFree_sepConj ?_
          (pcFree_sepConj ?_ (pcFree_sepConj ?_ hG))))
        <;> first | exact bytesRegion_pcFree _ _ | pcf) hsetup0
  have hexit := hvphCompareMismatch3Exit sp0 spC ret saved cur
    claimedBytes computedBytes v7 v28 o10 G hG hspC
    (by simpa [saved, hvphFrameVals] using hret) hclen hcdlen h0 h1 h2 h_ne
  have hregs : regsAt hvphFrame cur =
      ((.x1 ↦ᵣ link) ** (.x8 ↦ᵣ parentPtr) ** (.x9 ↦ᵣ parentLen) **
        (.x18 ↦ᵣ vals .x18)) := by
    simp [cur, hvphFrame, regsAt, sepConj_emp_right']
...
/-- From `H+72`: `la` ptrs ;; eq0 ;; ne1 mismatch ;; status-2. Cost `17`.
    Scratch `x5/x6/x7/x28` are `regOwn`. -/
theorem hvphFromCompareSetupMismatch3
    (sp0 spC ret link parentPtr parentLen : Word) (vals : Reg → Word)
    (claimedBytes computedBytes : List (BitVec 8))
    (o10 : Word)
    (G : Assertion) (hG : G.pcFree)
  ...
  refine cpsTripleWithin_weaken
    ...
    (fun _ hp => by xperm_hyp hp) (fun _ hq => hq) ?_
  simpa [saved] using
    hvphFromCompareSetupMismatch3_vals sp0 spC ret link parentPtr parentLen vals
      claimedBytes computedBytes old5 old6 v7 v28 o10 G hG hspC hret
      hclen hcdlen h0 h1 h2 h_ne

set_option maxRecDepth 8000 in
/-- Keccak setup+call (`H+52`) ;; dword-3 mismatch exit. Cost `22+nK`. -/
theorem hvphKeccakThenMismatch3
    (sp0 spC ret parentPtr parentLen : Word) (vals : Reg → Word)
    (old10 old11 old12 v20 v28 v29 : Word)
    (parentBytes claimedBytes : List (BitVec 8)) (N rem : Nat)
    (os : List (BitVec 8))
    ...
    (h_ne : dwordAt claimedBytes 3 ≠ dwordAt (keccakBodyDigest parentBytes N rem) 3) :
    let digest := keccakBodyDigest parentBytes N rem
    let kvals := keccakEntryVals parentPtr parentLen (vals .x18) v20
    let out0 := List.replicate 32 (0 : BitVec 8)
    let saved := hvphFrameVals ret vals
    cpsTripleWithin (28 + nKeccak N rem) (H + 52) (saved .x1) fullCode
      ((.x2 ↦ᵣ spC) ** (.x1 ↦ᵣ (H + 40)) **
        (.x8 ↦ᵣ parentPtr) ** (.x9 ↦ᵣ parentLen) ** (.x18 ↦ᵣ vals .x18) **
        (.x10 ↦ᵣ old10) ** (.x11 ↦ᵣ old11) ** (.x12 ↦ᵣ old12) **
        (.x20 ↦ᵣ v20) **
  ...
  have hG : Gmm.pcFree := by
    ...
    refine pcFree_sepConj ?_ (pcFree_sepConj ?_ (pcFree_sepConj (bytesRegion_pcFree _ _)
      (pcFree_sepConj ?_ (pcFree_sepConj (pcFree_regOwns _)
        (pcFree_sepConj (keccakCallerFreeA_pcFree _ _ _ _ (by pcf)) hF)))))
      <;> pcf
  have hmm0 := hvphFromCompareSetupMismatch3 sp0 spC ret (H + 72) parentPtr parentLen vals
    claimedBytes digest (0 : Word) Gmm hG hspC hret hclen hcdlen h0 h1 h2 h_ne
  have hmm := cpsTripleWithin_extend_code hvph_mono hmm0
  have hall := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by
    unfold keccakCallerPost at hp
    have hregs : regsAt keccakFrame kvals =
    ...
    unfold Gmm
    xperm_hyp hp) hcall hmm
  have hn : (5 + nKeccak N rem) + 23 = 28 + nKeccak N rem := by omega
  rw [← hn]
  exact cpsTripleWithin_weaken (fun _ hp => by
    simp only [saved, out0] at hp ⊢
    xperm_hyp hp) (fun _ hq => by
...
set_option maxRecDepth 8000 in
/-- Concrete-scratch helper: `H+40` beq-ok ;; keccak ;; dword-3-mismatch. Cost `23+nK`. -/
theorem hvphFromHeadersMismatch3_vals
    (sp0 spC ret parentPtr parentLen : Word) (vals : Reg → Word)
    (old11 old12 v20 v28 v29 : Word)
    (parentBytes claimedBytes : List (BitVec 8)) (N rem : Nat)
    (os : List (BitVec 8))
    ...
    let Amb := hvphSuccKeccakAmb spC v20 os out0
      (bytesRegion thisPtr thisBytes ** F)
    cpsTripleWithin (29 + nKeccak N rem) (H + 40) (saved .x1) fullCode
      ((.x1 ↦ᵣ (H + 40)) ** (.x10 ↦ᵣ (0 : Word)) ** (.x0 ↦ᵣ (0 : Word)) **
        claimedOwn claimedBytes **
        headersCallFrameSuccCore spC ret parentPtr parentLen vals parentBytes **
        (.x28 ↦ᵣ v28) ** (.x29 ↦ᵣ v29) **
  ...
  have hbeqF := cpsTripleWithin_frameR
    ...
    (by
      ...
      · pcf
      · pcf) hbeq
  have hkm := hvphKeccakThenMismatch3 sp0 spC ret parentPtr parentLen vals
    (0 : Word) old11 old12 v20 v28 v29 parentBytes claimedBytes N rem os
    (bytesRegion thisPtr thisBytes ** F)
    (by refine pcFree_sepConj (bytesRegion_pcFree _ _) hF)
    hspC hret hplen hlen hrem_le hos
  ...
  have hall := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by
    ...
    simp only [out0, regOwns, keccakBodyFreeTemps, sepConj_emp_right'] at hp ⊢
    xperm_hyp hp) hbeqF hkm
  have hn : 1 + (28 + nKeccak N rem) = 29 + nKeccak N rem := by omega
  rw [← hn]
  exact cpsTripleWithin_weaken (fun _ hp => by
    simp only [Amb] at hp ⊢
    xperm_hyp hp) (fun _ hq => by
...
set_option maxRecDepth 8000 in
/-- From `H+40` with status 0: beq-ok ;; keccak ;; dword-3-mismatch. Cost `23+nK`. -/
theorem hvphFromHeadersMismatch3
    (sp0 spC ret parentPtr parentLen : Word) (vals : Reg → Word)
    (v20 : Word)
    (parentBytes claimedBytes : List (BitVec 8)) (N rem : Nat)
    (os : List (BitVec 8))
  ...
  refine cpsTripleWithin_weaken
    ...
    (fun _ hp => by xperm_hyp hp) (fun _ hq => hq) ?_
  simpa [digest, kvals, out0, saved, Amb] using
    hvphFromHeadersMismatch3_vals sp0 spC ret parentPtr parentLen vals
      old11 old12 v20 v28 v29 parentBytes claimedBytes N rem os
      thisPtr thisBytes F hF hspC hret hplen hlen hrem_le hos
      halign_zk hover hNbound hrem64 hb8i
      hovers hoveri hvalids hvalidi hvalidRem hvalid135 hvalidMem
