/-
  Ambient dual: Front short AfterSave → legacy 20B copy bare region of_decode (typeW=0).
-/
...
theorem extractFrontAfterSaveCopy_then_epi_of_decode_short_legacy_region_ambient
  ...
  refine cpsTripleWithin_weaken (fun h hp => ?_) (fun _ hq => by
      ...
      xperm_hyp hq) hMid
  have hty := frontAfterSavePostShortAmbient_to_midJoinPre spC s regionBase loadPtr
    lenW toBuf isCreationPtr bs off len h hp
  simp only [htype0] at hty
  exact hty

#print axioms extractFrontAfterSaveCopy_then_epi_of_decode_short_legacy_region_ambient
