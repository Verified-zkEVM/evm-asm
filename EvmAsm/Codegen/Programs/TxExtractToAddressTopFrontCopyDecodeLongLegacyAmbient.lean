/-
  Ambient dual: Front long AfterSave → legacy 20B copy of_decode (typeW=0).
-/
...
theorem extractFrontAfterSaveCopy_then_epi_of_decode_long_legacy_region_ambient
  ...
  have hMid := extractLegacyAfterSaveCopy_then_epi_of_decode_region_ambient sp0 spC s
  ...
  refine cpsTripleWithin_weaken (fun h hp => ?_) (fun _ hq => by
      ...
      xperm_hyp hq) hMid
  have hty := frontAfterSavePostLongAmbient_to_midJoinPre spC s regionBase loadPtr
    lenW toBuf isCreationPtr bs off len hwi_off h hp
  simp only [htype0] at hty
  exact hty


#print axioms extractFrontAfterSaveCopy_then_epi_of_decode_long_legacy_region_ambient
