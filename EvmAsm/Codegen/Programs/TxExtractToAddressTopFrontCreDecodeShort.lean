/-
  Front short AfterSave → creation of_decode with concrete endPtr (no ∀endPtr).
-/
...
theorem extractFrontAfterSaveCreation_then_epi_of_decode_short
  ...
  exact cpsTripleWithin_weaken
    (fun h hp => frontAfterSavePostShort_to_midJoinPre spC s txBase lenW
      toBuf isCreationPtr txBytes h hp)
    (fun _ hq => hq) hMid

#print axioms extractFrontAfterSaveCreation_then_epi_of_decode_short
