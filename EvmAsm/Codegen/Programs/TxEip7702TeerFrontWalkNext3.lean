/-
  Teer front walk_next cycle 3 under applied prest.
  E → AfterWalkNext3Save via FrontWalkNext2 + Cycle3.
-/
...
private abbrev nFrontToWalkNext2Save : Nat :=
  ((34 + (6 + (1 + nTypeSteps) + 1)) + (5 + 7)) + (1 + 15 + 1 + 2) + nWalkNextCycle * 3

private abbrev nFrontToWalkNext3Save : Nat := nFrontToWalkNext2Save + nWalkNextCycle

set_option maxRecDepth 8000 in
/-- Cycle3 CycleOk with regOwn temps x5–7,x28–31. -/
theorem teerWalkNext3CycleOk_ownTemps
    ...
    (hend : v25 = endPtr) :
    cpsTripleWithin nWalkNextCycle AfterWalkNext2Save AfterWalkNext3Save
      teerLinkedEarly
      (teerWalkNext0BodyCore listBase bs old1 v24 v25 a0Old a1Old a2Old **
        regOwn .x5 ** regOwn .x6 ** regOwn .x7 **
        regOwn .x28 ** regOwn .x29 ** regOwn .x30 ** regOwn .x31)
      (fun h => ∃ next len : Word,
        (teerWalkNextSkipBodyPost listBase endPtr next len bs srcOff LinkWalkNext3) h) := by
  have hcore (t0 t1 t2 t3 t4 t5 t6 : Word) :
      cpsTripleWithin nWalkNextCycle AfterWalkNext2Save AfterWalkNext3Save
        ...
        (fun h => ∃ next len : Word,
        (teerWalkNextSkipBodyPost listBase endPtr next len bs srcOff LinkWalkNext3) h) := by
    have h0 := teerWalkNext3CycleOk listBase endPtr a2Old t0 t1 t2 t3 t4 t5 t6
...
theorem teerWalkNext3_applied_nested
    ...
    cpsTripleWithin nFrontToWalkNext3Save
      E AfterWalkNext3Save teerLinkedEarly
      ((.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
      ...
      (teerWalkNext3PostNested spC loadPtr lenW balPtr balLenW chainIdW
        s7 s11 spVal regionBase endW bs balBytes s innerVal srcOff3) := by
  intro s innerVal endW
  have hwnP := teerWalkNext2_applied ret spVal spC loadPtr lenW
  ...
  let Flat (p : Word × Word) : Assertion :=
    ...
    ⌜rlpItemDecode bs srcOff2 (regionBase + BitVec.ofNat 64 srcOff2)
      endW p.1 p.2⌝
  have hwnPE :
      cpsTripleWithin nFrontToWalkNext2Save E AfterWalkNext2Save teerLinkedEarly
      ((.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
        stackFree spVal nTeerStackDwords **
        (.x8 ↦ᵣ s0) ** (.x9 ↦ᵣ s1) **
        (.x18 ↦ᵣ s2) ** (.x19 ↦ᵣ s3) ** (.x20 ↦ᵣ s4) **
    ...
    refine ⟨(next0, len0), ?_⟩
    dsimp only [Flat, teerWalkNext2FlatRest]
    xperm_hyp hq'
  have hstep (p : Word × Word) :
    ...
    have hswap :
      ...
      have hcurK : p.1 = regionBase + BitVec.ofNat 64 srcOff3 :=
        hbridge2 p.1 p.2 (by simpa [endW, innerVal] using hpure)
      have hcy := teerWalkNext3CycleOk_ownTemps regionBase endW bs srcOff3
        LinkWalkNext2 p.1 endW p.1 (0 : Word) p.2
        (by simpa using halign) hoff3 hover3 hvalid3 hss3 hls3 hll3
        hdec3 hinb3 hcurK rfl
      ...
      have hcyF' :
        ...
        refine cpsTripleWithin_weaken
          (fun _ hp => by
            ...
            xperm_hyp hp)
          (fun h hq => by
            unfold teerWalkNext3PostNested
            obtain ⟨h1, h2, hd, hu, hEx, hA⟩ := hq
            obtain ⟨next, len1, hB⟩ := hEx
            exact ⟨next, len1, h1, h2, hd, hu, hB, hA⟩)
          hcyF
      exact cpsTripleWithin_weaken
        (teerWalkNext2FlatRest_to_wn3Pre spC loadPtr lenW balPtr balLenW chainIdW
          s7 s11 spVal regionBase endW bs balBytes s innerVal p.1 p.2)
        (fun _ hq => hq) hcyF'
    exact cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp)
      (fun _ hq => hq) hswap
  ...
  exact cpsTripleWithin_mono_nSteps
    (by decide : nFrontToWalkNext2Save + nWalkNextCycle ≤ nFrontToWalkNext3Save)
    hseq

set_option maxRecDepth 8000 in
theorem teerWalkNext3_applied
    (ret spVal spC loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
    (regionBase : Word) (bs balBytes : List (BitVec 8)) (off len : Nat)
    (hspC : spC = spVal + signExtend12 (-160 : BitVec 12))
    ...
    cpsTripleWithin nFrontToWalkNext3Save
      ...
      (fun h => ∃ next len3 : Word,
        ((.x2 ↦ᵣ spC) **
          ...
          (.x21 ↦ᵣ loadPtr + innerVal) ** (.x22 ↦ᵣ lenW - innerVal) **
          (.x23 ↦ᵣ s7) **
          (.x10 ↦ᵣ next) ** (.x11 ↦ᵣ (0 : Word)) ** (.x12 ↦ᵣ len3) **
          ...
          ⌜rlpItemDecode bs srcOff3 (regionBase + BitVec.ofNat 64 srcOff3)
            endW next len3⌝) h) := by
  intro s innerVal endW
  have h0 := teerWalkNext3_applied_nested ret spVal spC loadPtr lenW
    balPtr balLenW chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
    regionBase bs balBytes off len hspC hnez hptr hlenW hsuccess htype4
    halign hbound hover hvalid0 listOff ha0 hoffL hoverL hvalidL hlenL
    h_ge h_hi h_exact srcOff0 hcur0 hoff0 hover0 hvalid0I hss0 hls0 hll0 hdec0 hinb0
  ...
  exact cpsTripleWithin_weaken (fun _ hp => hp)
    (fun h hq => by
      unfold teerWalkNext3PostNested teerWalkNextSkipBodyPost
        teerWalkNextSkipAmbient teerWalkNext0Ambient at hq
      obtain ⟨next, len3, hq'⟩ := hq
      refine ⟨next, len3, ?_⟩
      xperm_hyp hq' ) h0

#print axioms teerWalkNext3CycleOk_ownTemps
#print axioms teerWalkNext2FlatRest_to_wn3Pre
#print axioms teerWalkNext3_applied_nested
#print axioms teerWalkNext3_applied




def teerWalkNext2FlatRestIs
    (spC loadPtr lenW balPtr balLenW chainIdW
      s7 s11 spVal regionBase endW : Word)
    (bs balBytes : List (BitVec 8)) (s : TeerSaved)
    (innerVal next0 len0 : Word) : Assertion :=
...
theorem teerWalkNext2FlatRestIs_to_wn3Pre
    (spC loadPtr lenW balPtr balLenW chainIdW
      s7 s11 spVal regionBase endW : Word)
    (bs balBytes : List (BitVec 8)) (s : TeerSaved)
    (innerVal next0 len0 : Word) :
    ∀ h,
      (teerWalkNext2FlatRestIs spC loadPtr lenW balPtr balLenW chainIdW
          s7 s11 spVal regionBase endW bs balBytes s innerVal next0 len0) h →
      (teerWalkNextSkipFocus regionBase bs
          LinkWalkNext2 next0 endW next0 (0 : Word) len0 **
        teerWalkNextSkipAmbientIs spC loadPtr lenW balPtr balLenW chainIdW
          s7 s11 spVal balBytes s innerVal) h := by
  intro h hp
  unfold teerWalkNext2FlatRestIs at hp
  unfold teerWalkNextSkipFocus teerWalkNext0Focus teerWalkNext0BodyCore
    teerWalkNextSkipAmbientIs teerWalkNext0AmbientIs
  xperm_hyp hp

def teerWalkNext3PostNestedIs
    (spC loadPtr lenW balPtr balLenW chainIdW
      s7 s11 spVal regionBase endW : Word)
    (bs balBytes : List (BitVec 8)) (s : TeerSaved)
    (innerVal : Word) (srcOff3 : Nat) : Assertion :=
...
set_option maxRecDepth 8000 in
theorem teerWalkNext3_applied_nested_is
    (ret spVal spC loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
    (regionBase : Word) (bs balBytes : List (BitVec 8)) (off len : Nat)
    (hspC : spC = spVal + signExtend12 (-160 : BitVec 12))
    ...
    cpsTripleWithin nFrontToWalkNext3Save
      ...
      ((.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
        ...
        regOwn .x16 ** regOwn .x28 ** regOwn .x29 ** regOwn .x30 **
        regOwn .x31 ** (.x0 ↦ᵣ (0 : Word)))
      (teerWalkNext3PostNestedIs spC loadPtr lenW balPtr balLenW chainIdW
        s7 s11 spVal regionBase endW bs balBytes s innerVal srcOff3) := by
  intro s innerVal endW
  have hwnP := teerWalkNext2_applied_is ret spVal spC loadPtr lenW
    balPtr balLenW chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
    regionBase bs balBytes off len hspC hnez hptr hlenW hsuccess htype4
    halign hbound hover hvalid0 listOff ha0 hoffL hoverL hvalidL hlenL
    h_ge h_hi h_exact srcOff0 hcur0 hoff0 hover0 hvalid0I hss0 hls0 hll0 hdec0 hinb0
  ...
  have hwnPE :
    ...
    obtain ⟨next0, len0, hq'⟩ := hq
    refine ⟨(next0, len0), ?_⟩
    dsimp only [Flat, teerWalkNext2FlatRestIs]
    xperm_hyp hq'
  have hstep (p : Word × Word) :
      cpsTripleWithin nWalkNextCycle AfterWalkNext2Save AfterWalkNext3Save
        teerLinkedEarly (Flat p)
      ...
      have hcyF' :
        ...
        refine cpsTripleWithin_weaken
          (fun _ hp => by
            ...
            xperm_hyp hp)
          (fun h hq => by
            unfold teerWalkNext3PostNestedIs
            obtain ⟨h1, h2, hd, hu, hEx, hA⟩ := hq
            obtain ⟨next, len1, hB⟩ := hEx
            exact ⟨next, len1, h1, h2, hd, hu, hB, hA⟩)
          hcyF
      exact cpsTripleWithin_weaken
        (teerWalkNext2FlatRestIs_to_wn3Pre spC loadPtr lenW balPtr balLenW chainIdW
          s7 s11 spVal regionBase endW bs balBytes s innerVal p.1 p.2)
        (fun _ hq => hq) hcyF'
    exact cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp)
      (fun _ hq => hq) hswap
...
set_option maxRecDepth 8000 in
theorem teerWalkNext3_applied_is
    (ret spVal spC loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
    (regionBase : Word) (bs balBytes : List (BitVec 8)) (off len : Nat)
    (hspC : spC = spVal + signExtend12 (-160 : BitVec 12))
      ...
      (fun h => ∃ next len3 : Word,
        ((.x2 ↦ᵣ spC) **
          ...
          ⌜rlpItemDecode bs srcOff3 (regionBase + BitVec.ofNat 64 srcOff3)
            endW next len3⌝) h) := by
  intro s innerVal endW
  have h0 := teerWalkNext3_applied_nested_is ret spVal spC loadPtr lenW
    balPtr balLenW chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
    regionBase bs balBytes off len hspC hnez hptr hlenW hsuccess htype4
    halign hbound hover hvalid0 listOff ha0 hoffL hoverL hvalidL hlenL
    h_ge h_hi h_exact srcOff0 hcur0 hoff0 hover0 hvalid0I hss0 hls0 hll0 hdec0 hinb0
  ...
  exact cpsTripleWithin_weaken (fun _ hp => hp)
    (fun h hq => by
      unfold teerWalkNext3PostNestedIs teerWalkNextSkipBodyPost
...
#print axioms teerWalkNext3CycleOk_ownTemps
#print axioms teerWalkNext2FlatRestIs_to_wn3Pre
#print axioms teerWalkNext3_applied_nested_is
#print axioms teerWalkNext3_applied_is
