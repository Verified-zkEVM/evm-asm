...
def bncIsInf64Fn (ptr : Word) (bs : List (BitVec 8)) : Fn where
  name := "bncIsInf64"
  region := ⟨ptr, bs⟩
  -- ⚠️ Ambient PINNED, as in `bnfIsZero32Fn` / `secfIsZero32Fn` (#12244): both flat
  -- adapters require the post to DETERMINE the ambient.
  pre := fun rf _ A =>
    rf.get .x10 = ptr ∧ bs.length = 64 ∧ ptr.toNat + 64 < 2 ^ 64 ∧ A = empAssertion
  post := fun rf _ A =>
    (rf.get .x10 = if nlz bs 64 = 64 then (1 : Word) else (0 : Word)) ∧
    64 ≤ bs.length ∧ ptr.toNat + 64 < 2 ^ 64 ∧ A = empAssertion
...
theorem bncIsInf64Fn_spec (ptr : Word) (bs : List (BitVec 8))
  ...
  case bncIsInf64.scan.inv_init =>
    ...
    refine ⟨?_, ?_, Nat.zero_le _, (by omega : 64 ≤ bs.length), hpl, hA⟩
  ...
  case bncIsInf64.scan.inv_step =>
    ...
    have hz : bs.getD i 0 = 0 := by
      ...
      bv_omega
    refine ⟨?_, ?_, nlz_continue bs 64 i hilt hlen hz hle, hlen, hpl, hA⟩
    · rw [hrf']
