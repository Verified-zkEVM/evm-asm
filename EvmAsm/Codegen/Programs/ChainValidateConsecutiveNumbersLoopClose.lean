/-
  ...
  The header-0 block [18-30] decodes header 0's field 8, saves it as the initial
  `prev` (`x21 = number[0]`), and sets `x6 = base of header 1`, `x7 = 1`, entering the
  loop guard with `LoopInv 1` (whose `⌜hdrNumOk 0 number[0]⌝` binds the threaded prev to
  the genuine decoded number[0]).  `cvcnLoop` runs the guard/iteration for the
...
theorem cvcnHdr0Call (spC hdrBase lenBase validPtr firstBadPtr x21val : Word) (L0 : Nat)
    (nN oldOut oldOff oldLen old14 oldX1 old5 o10 o11 o12 o13 : Word)
    (bytes : List (BitVec 8)) (csaved : Saved)
    (hsalign : hdrBase.toNat % 8 = 0)
    (hbytes : L0 ≤ bytes.length)
    (hnowrap : hdrBase.toNat + L0 + 9 < 2 ^ 64)
    ...
    cpsTripleWithin (5 + 1 + nCall bytes.length) (D + 72) (D + 96) fullCode
      ...
      ((.x1 ↦ᵣ LinkRA0) **
        EvmAsm.Codegen.RlpFieldToU64StrictSAsm.flatPost spC (spC + signExtend12 (-32 : BitVec 12)) hdrBase
          ...
          (⟨EvmAsm.Codegen.RlpFieldToU64StrictSAsm.B + 48, hdrBase, Num, hdrBase, validPtr, firstBadPtr, x21val⟩ : Saved)
          bytes L0 8 **
  ...
  have hsetup := cpsTripleWithin_extend_code cvcn_mono
    (cvcnHdr0Setup hdrBase lenBase L0 o10 o11 o12 o13)
  ...
  have hjalF := cpsTripleWithin_frameR
    ((.x2 ↦ᵣ spC) ** (.x8 ↦ᵣ nN) ** (.x9 ↦ᵣ lenBase) ** (.x18 ↦ᵣ hdrBase) **
      ...
      (.x10 ↦ᵣ hdrBase) ** (.x11 ↦ᵣ BitVec.ofNat 64 L0) ** (.x12 ↦ᵣ (8 : Word)) **
      (.x13 ↦ᵣ Num) ** (.x14 ↦ᵣ old14) ** regOwn .x6 ** regOwn .x7 ** regOwn .x28 **
  ...
  have hcallee0 := EvmAsm.Codegen.RlpFieldToU64StrictSAsm.rlpFieldToU64_flat_spec_within
    spC calleeNewSp hdrBase (BitVec.ofNat 64 L0) (8 : Word) Num oldOut oldOff oldLen old14
    ...
    hsalign hbytes hnowrap hover hvalid hnz (by show LinkRA0 &&& ~~~(1 : Word) = LinkRA0; decide)
  ...
  have hcallee : cpsTripleWithin (nCall bytes.length) EvmAsm.Codegen.RlpFieldToU64StrictSAsm.B LinkRA0 fullCode
      (regOwn .x5 **
        ((.x1 ↦ᵣ LinkRA0) ** (.x2 ↦ᵣ spC) ** (.x8 ↦ᵣ nN) ** (.x9 ↦ᵣ lenBase) **
          ...
          (.x10 ↦ᵣ hdrBase) ** (.x11 ↦ᵣ BitVec.ofNat 64 L0) ** (.x12 ↦ᵣ (8 : Word)) **
          (.x13 ↦ᵣ Num) ** (.x14 ↦ᵣ old14) ** regOwn .x6 ** regOwn .x7 **
...
/-! ## Header-0 finish (instructions 25--30): save initial prev, set base_1, i:=1
    ...
    `x21 := *cvcn_num` (the decoded `number[0]`, the initial `prev`), `x5 := *lenBase`
    (`= lengths[0]`), `x6 := hdrBase + lengths[0]` (base of header 1), `x7 := 1`. -/

set_option maxRecDepth 8000 in
theorem cvcnHdr0Finish (hdrBase lenBase ts0 : Word) (L0 : Nat) (old5 o6 o7 o21 : Word) :
    cpsTripleWithin 6 (D + 100) (D + 124) cvcnCode
      ((.x5 ↦ᵣ old5) ** (.x21 ↦ᵣ o21) ** (.x6 ↦ᵣ o6) ** (.x7 ↦ᵣ o7) **
        (.x9 ↦ᵣ lenBase) ** (.x18 ↦ᵣ hdrBase) ** (Num ↦ₘ ts0) **
        (lenBase ↦ₘ BitVec.ofNat 64 L0))
      ((.x5 ↦ᵣ BitVec.ofNat 64 L0) ** (.x21 ↦ᵣ ts0) **
        (.x6 ↦ᵣ (hdrBase + BitVec.ofNat 64 L0)) ** (.x7 ↦ᵣ (1 : Word)) **
        (.x9 ↦ᵣ lenBase) ** (.x18 ↦ᵣ hdrBase) ** (Num ↦ₘ ts0) **
        (lenBase ↦ₘ BitVec.ofNat 64 L0)) := by
  have hla25 := la_materialize_within .x5 old5 (D + 100) Num (by decide) (by decide)
    (CodeReq.ofProg_mem_at D (D + 100) cvcnProg 25 (.AUIPC .x5 (EvmAsm.Rv64.laHi (D + 100) Num)) (by bv_omega) (by rw [cvcn_length]; decide) (by decide) (by rw [cvcn_length]; decide))
    (CodeReq.ofProg_mem_at D (D + 104) cvcnProg 26 (.ADDI .x5 .x5 (EvmAsm.Rv64.laLo (D + 100) Num)) (by bv_omega) (by rw [cvcn_length]; decide) (by decide) (by rw [cvcn_length]; decide))
  have s27 := ld_spec_gen_within .x21 .x5 Num o21 ts0 (0 : BitVec 12) (D + 108) (by decide)
  rw [show signExtend12 (0 : BitVec 12) = (0 : Word) from by decide,
    show Num + (0 : Word) = Num from by bv_omega] at s27
  have s27' := cpsTripleWithin_extend_code
    (CodeReq.ofProg_mem_at D (D + 108) cvcnProg 27 (.LD .x21 .x5 (0 : BitVec 12))
      (by bv_omega) (by rw [cvcn_length]; decide) rfl (by rw [cvcn_length]; decide)) s27
  have s28 := ld_spec_gen_within .x5 .x9 lenBase Num (BitVec.ofNat 64 L0) (0 : BitVec 12) (D + 112) (by decide)
  rw [show signExtend12 (0 : BitVec 12) = (0 : Word) from by decide,
    show lenBase + (0 : Word) = lenBase from by bv_omega] at s28
  have s28' := cpsTripleWithin_extend_code
    (CodeReq.ofProg_mem_at D (D + 112) cvcnProg 28 (.LD .x5 .x9 (0 : BitVec 12))
      (by bv_omega) (by rw [cvcn_length]; decide) rfl (by rw [cvcn_length]; decide)) s28
  have s29 := add_spec_gen_within .x6 .x18 .x5 hdrBase (BitVec.ofNat 64 L0) o6 (D + 116) (by decide)
  have s29' := cpsTripleWithin_extend_code
    (CodeReq.ofProg_mem_at D (D + 116) cvcnProg 29 (.ADD .x6 .x18 .x5)
      (by bv_omega) (by rw [cvcn_length]; decide) rfl (by rw [cvcn_length]; decide)) s29
  have s30 := li_spec_gen_within .x7 o7 (1 : Word) (D + 120) (by decide)
  have s30' := cpsTripleWithin_extend_code
    (CodeReq.ofProg_mem_at D (D + 120) cvcnProg 30 (.LI .x7 (1 : Word))
...
/-! ## Loop induction (`D+124 → raIn`, entering iteration `i`, `1 ≤ i ≤ N`)
    ...
    pair strictly increasing" witness).  Otherwise one `cvcnIter` runs and the
...
theorem cvcnLoop (sp0 spC hdrBase lenBase validPtr firstBadPtr raIn : Word)
  ...
  have base :
    ...
    refine cpsTripleWithin_weaken (fun h hp => by unfold payload scratchRegs at hp; xperm_hyp hp)
      ...
      (show cpsTripleWithin (cvcnLoopSteps bigBytes.length 0) (D + 124) raIn fullCode
        ((((.x2 ↦ᵣ spC) ** (.x8 ↦ᵣ BitVec.ofNat 64 lengths.length) ** (.x9 ↦ᵣ lenBase) **
            ...
            memOwn Num ** memOwn RfuOff ** memOwn RfuLen ** memOwn IterChild ** memOwn IterI **
    ...
    have hbeq := beq_spec_gen_within .x7 .x8 (208 : BitVec 13)
    ...
    rw [show (D + 124) + signExtend13 (208 : BitVec 13) = D + 332 from by
      rw [show signExtend13 (208 : BitVec 13) = (208 : Word) from by decide]; bv_omega] at htaken
    ...
    have hallv := cpsTripleWithin_extend_code cvcn_mono
  ...
  | succ f ih =>
    ...
    · rw [show lengths.length - i = (lengths.length - (i + 1)) + 1 from by omega,
      ...
      have hiter := cvcnIter sp0 spC hdrBase lenBase validPtr firstBadPtr raIn csaved bigBytes lengths i
        ...
        (hAllOver i hi) (hAllValid i hi) (hAllNz i hi)
...
theorem flatPost_normalize0 (spC hbi hdrBase validPtr firstBadPtr nN lenBase prevVal
    ...
    (EvmAsm.Codegen.RlpFieldToU64StrictSAsm.flatPost spC (spC + signExtend12 (-32 : BitVec 12)) hbi
      oldOff oldLen (⟨LinkRA0, nN, lenBase⟩ : EvmAsm.Codegen.RlpFieldToU64StrictSAsm.Saved)
      (⟨EvmAsm.Codegen.RlpFieldToU64StrictSAsm.B + 48, hbi, Num, hdrBase, validPtr, firstBadPtr, prevVal⟩ : Saved)
      bytes Li 8) h →
    (∃ status value,
      (dispNorm0 spC (spC + signExtend12 (-32 : BitVec 12)) hbi hdrBase validPtr firstBadPtr nN
          lenBase prevVal value status bytes **
        ⌜EvmAsm.Codegen.RlpFieldToU64StrictSAsm.Result bytes hbi Li 8 status value⌝) h) := by
  intro h hp
...
/-! ## Header-0 status dispatch (instruction 24 onward): tie header-0's `Result`
    ...
    zero-initialized `cvcn_iter_i`); on success (`status = 0`) `cvcnHdr0Finish`
    [25-30] saves the decoded `number[0]` as the initial `prev`, sets `x6 = base of
    header 1`, `x7 = 1`, reaching `LoopInv 1` (whose `⌜hdrNumOk 0 number[0]⌝` binds
    the threaded prev to the genuine decoded `number[0]`) and the loop tail. -/

set_option maxRecDepth 8000 in
theorem cvcnHdr0Dispatch
    ...
    cpsTripleWithin (22 + nTail) (D + 96) raIn fullCode
      ((.x1 ↦ᵣ LinkRA0) **
        EvmAsm.Codegen.RlpFieldToU64StrictSAsm.flatPost spC (spC + signExtend12 (-32 : BitVec 12))
          ...
          (⟨EvmAsm.Codegen.RlpFieldToU64StrictSAsm.B + 48, hdrBase, Num, hdrBase, validPtr,
        ...
        (show cpsTripleWithin (22 + nTail) (D + 96) raIn fullCode
          ((.x1 ↦ᵣ LinkRA0) **
            (dispNorm0 spC (spC + signExtend12 (-32 : BitVec 12)) hdrBase hdrBase validPtr
              ...
              ⌜EvmAsm.Codegen.RlpFieldToU64StrictSAsm.Result bigBytes hdrBase lengths[0]! 8 status value⌝) **
  ...
  case core =>
    refine cpsTripleWithin_weaken (fun h hp => ?hpull) (fun _ hq => hq)
      (cpsTripleWithin_pure_pre
        (P := EvmAsm.Codegen.RlpFieldToU64StrictSAsm.Result bigBytes hdrBase lengths[0]! 8 status value)
    ...
    case body =>
      ...
      · -- SUCCESS: `bne` not taken → header-0 finish → LoopInv 1 → loop tail.
        subst hstatus
        have hbne := bne_spec_gen_within .x10 .x0 (216 : BitVec 13) (0 : Word) (0 : Word) (D + 96)
        have hbneC := cpsBranchWithin_extend_code cvcn_mono
          (cpsBranchWithin_extend_code (cr' := cvcnCode)
            (CodeReq.ofProg_mem_at D (D + 96) cvcnProg 24 (.BNE .x10 .x0 (216 : BitVec 13))
              (by bv_omega) (by rw [cvcn_length]; decide) rfl (by rw [cvcn_length]; decide)) hbne)
        ...
        set Rframe : Assertion :=
          ((.x1 ↦ᵣ LinkRA0) ** (.x2 ↦ᵣ spC) ** (.x8 ↦ᵣ BitVec.ofNat 64 lengths.length) **
            (.x9 ↦ᵣ lenBase) ** (.x18 ↦ᵣ hdrBase) ** (.x19 ↦ᵣ validPtr) **
            (.x20 ↦ᵣ firstBadPtr) ** (.x21 ↦ᵣ x21val) ** (Num ↦ₘ value) **
            regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** regOwn .x11 ** regOwn .x12 **
            regOwn .x13 ** regOwn .x14 ** regOwn .x28 ** regOwn .x29 ** regOwn .x30 **
            regOwn .x31 ** memOwn RfuOff ** memOwn RfuLen **
            stackFree (spC + signExtend12 (-32 : BitVec 12)) 8 **
        ...
        refine cpsTripleWithin_weaken (fun h hp => by xperm_hyp hp) (fun _ hq => hq)
          (show cpsTripleWithin (6 + nTail) (D + 100) raIn fullCode
            (((.x1 ↦ᵣ LinkRA0) ** (.x2 ↦ᵣ spC) ** (.x8 ↦ᵣ BitVec.ofNat 64 lengths.length) **
              ...
              (.x0 ↦ᵣ (0 : Word)) ** (Num ↦ₘ value) ** regOwn .x28 ** regOwn .x29 **
        ...
        have hfin := cpsTripleWithin_extend_code cvcn_mono
          (cvcnHdr0Finish hdrBase lenBase value lengths[0]! v5 v6 v7 x21val)
      ...
      · -- PARSE-FAIL: `bne` taken → retParseFail reporting index 0.
        refine cpsTripleWithin_weaken (fun h hp => by xperm_hyp hp) (fun _ hq => hq)
          (show cpsTripleWithin (22 + nTail) (D + 96) raIn fullCode
            (((.x10 ↦ᵣ status) ** (.x0 ↦ᵣ (0 : Word)) ** (.x1 ↦ᵣ LinkRA0) ** (.x2 ↦ᵣ spC) **
              ...
              (.x19 ↦ᵣ validPtr) ** (.x20 ↦ᵣ firstBadPtr) ** (.x21 ↦ᵣ x21val) ** (Num ↦ₘ value) **
        ...
        have hbne := bne_spec_gen_within .x10 .x0 (216 : BitVec 13) status (0 : Word) (D + 96)
        have hbneC := cpsBranchWithin_extend_code cvcn_mono
          (cpsBranchWithin_extend_code (cr' := cvcnCode)
            (CodeReq.ofProg_mem_at D (D + 96) cvcnProg 24 (.BNE .x10 .x0 (216 : BitVec 13))
              (by bv_omega) (by rw [cvcn_length]; decide) rfl (by rw [cvcn_length]; decide)) hbne)
        ...
        rw [show (D + 96) + signExtend13 (216 : BitVec 13) = D + 312 from by
          rw [show signExtend13 (216 : BitVec 13) = (216 : Word) from by decide]; bv_omega] at htaken
        have htakenF := cpsTripleWithin_frameR
          ((.x1 ↦ᵣ LinkRA0) ** (.x2 ↦ᵣ spC) ** (.x8 ↦ᵣ BitVec.ofNat 64 lengths.length) **
            (.x9 ↦ᵣ lenBase) ** (.x18 ↦ᵣ hdrBase) ** (.x19 ↦ᵣ validPtr) **
            (.x20 ↦ᵣ firstBadPtr) ** (.x21 ↦ᵣ x21val) **
            (Num ↦ₘ value) ** (.x5 ↦ᵣ v5) ** (.x6 ↦ᵣ v6) ** regOwn .x7 ** regOwn .x11 **
        ...
        have hp1 : ((.x5 ↦ᵣ IterI) ** (.x6 ↦ᵣ (0 : Word)) ** (Num ↦ₘ value) **
            (IterI ↦ₘ (0 : Word)) **
            EvmAsm.Codegen.RlpFieldToU64StrictSAsm.savedFrame (spC + signExtend12 (-32 : BitVec 12))
              ⟨LinkRA0, BitVec.ofNat 64 lengths.length, lenBase⟩ **
...
/-! ## Header-0 block (instructions 18--30): call ;; status dispatch
    ...
    From the `N ≥ 2` fall-through (`D+72`) to the caller's post: `cvcnHdr0Call`
    decodes header 0's field 8 and `cvcnHdr0Dispatch` splits on the status,
    reaching `LoopInv 1` (success) or `retParseFail` at index 0 (parse-fail). -/

set_option maxRecDepth 8000 in
theorem cvcnHdr0Block
    (sp0 spC hdrBase lenBase validPtr firstBadPtr raIn x21val : Word) (L0 : Nat)
    ...
    cpsTripleWithin ((5 + 1 + nCall bigBytes.length) + (22 + nTail)) (D + 72) raIn fullCode
      ...
      (cvcnPost sp0 spC (spC + signExtend12 (-32 : BitVec 12)) hdrBase lenBase validPtr
        firstBadPtr csaved bigBytes lengths) := by
  have hcall := cvcnHdr0Call spC hdrBase lenBase validPtr firstBadPtr x21val L0
    (BitVec.ofNat 64 lengths.length) oldOut oldOff oldLen old14 oldX1 old5 o10 o11 o12 o13
    bigBytes csaved hsalign hbytes hnowrap hover hvalid hnz
  have hcallF := cpsTripleWithin_frameR
    (wordArrayFrom lenBase 1 (lengths.drop 1) ** (validPtr ↦ₘ (1 : Word)) **
      (firstBadPtr ↦ₘ (0 : Word)) ** (IterI ↦ₘ (0 : Word)) **
      memOwn IterChild ** memOwn IterPrev)
    (by pcfx) hcall
  have hdisp := cvcnHdr0Dispatch sp0 spC hdrBase lenBase validPtr firstBadPtr raIn x21val L0
...
theorem chain_validate_consecutive_numbers_spec_within
    ...
    cpsTripleWithin
      ...
      (((.x2 ↦ᵣ sp0) ** (.x1 ↦ᵣ raIn) ** (.x8 ↦ᵣ cs0) ** (.x9 ↦ᵣ cs1) **
        ...
        wordArray lenBase lengths ** bytesRegion hdrBase bigBytes **
        memOwn Num ** memOwn RfuOff ** memOwn RfuLen ** memOwn IterChild **
  ...
  have hpro := cpsTripleWithin_extend_code cvcn_mono
    (cvcnPrologue sp0 spC (BitVec.ofNat 64 lengths.length) lenBase hdrBase validPtr firstBadPtr
  ...
  have htail :
    ...
    · -- `N < 2`: BLTU taken → all-valid exit (vacuously increasing).
      have hbltu := bltu_spec_gen_within .x8 .x5 (264 : BitVec 13)
        (BitVec.ofNat 64 lengths.length) (2 : Word) (D + 68)
      have hbltuC := cpsBranchWithin_extend_code cvcn_mono
        (cpsBranchWithin_extend_code (cr' := cvcnCode)
          (CodeReq.ofProg_mem_at D (D + 68) cvcnProg 17 (.BLTU .x8 .x5 (264 : BitVec 13))
            (by bv_omega) (by rw [cvcn_length]; decide) rfl (by rw [cvcn_length]; decide)) hbltu)
      have htaken := cpsBranchWithin_takenStripPure2 hbltuC (fun hp hq => by
        obtain ⟨_, _, _, _, _, hrest⟩ := hq
        exact absurd (hult.2 hlt) ((sepConj_pure_right _).1 hrest).2)
      rw [show (D + 68) + signExtend13 (264 : BitVec 13) = D + 332 from by
        rw [show signExtend13 (264 : BitVec 13) = (264 : Word) from by decide]; bv_omega] at htaken
      have htakenF := cpsTripleWithin_frameR
      ...
      have hp1 : ((.x5 ↦ᵣ (2 : Word)) ** (.x11 ↦ᵣ lenBase) ** (.x12 ↦ᵣ hdrBase) **
          ...
          ((.x10 ↦ᵣ (0 : Word)) ** (validPtr ↦ₘ (1 : Word)) ** (firstBadPtr ↦ₘ (0 : Word)) **
            ...
            stackFree (spC + signExtend12 (-32 : BitVec 12)) 8 **
            wordArray lenBase lengths ** bytesRegion hdrBase bigBytes **
            memOwn Num ** memOwn RfuOff ** memOwn RfuLen ** memOwn IterChild ** memOwn IterPrev)) h := by
        xperm_hyp hq
    ...
    · -- `N ≥ 2`: BLTU not taken → header-0 block then the loop from `i = 1`.
      ...
      have hcont :
        ...
        have hblock := cvcnHdr0Block sp0 spC hdrBase lenBase validPtr firstBadPtr raIn cs5
          ...
          (by have := hAllBytes 0 (by omega); rwa [hdrop0] at this)
          (by have := hAllNowrap 0 (by omega); rwa [hHB0] at this)
          ...
          (by have := hAllNz 0 (by omega); rwa [hdrop0] at this)
          (cvcnLoop sp0 spC hdrBase lenBase validPtr firstBadPtr raIn
      ...
      have hntakenF := cpsTripleWithin_frameR
        ((.x2 ↦ᵣ spC) ** (.x9 ↦ᵣ lenBase) ** (.x18 ↦ᵣ hdrBase) ** (.x19 ↦ᵣ validPtr) **
          ...
          (validPtr ↦ₘ (1 : Word)) ** (firstBadPtr ↦ₘ (0 : Word)) ** (IterI ↦ₘ (0 : Word)) **
          memOwn IterChild ** memOwn IterPrev **
          memOwn Num ** memOwn RfuOff ** memOwn RfuLen) (by pcfx) hntaken
      refine cpsTripleWithin_weaken (fun h hp => by xperm_hyp hp) (fun _ hq => hq)
        (cpsTripleWithin_seq_perm_same_cr (fun h hp => by
...
/-! ## Anti-vacuity cover (#12471)
    ...
    *set* of `chain_validate_consecutive_numbers_spec_within` is jointly inhabited
