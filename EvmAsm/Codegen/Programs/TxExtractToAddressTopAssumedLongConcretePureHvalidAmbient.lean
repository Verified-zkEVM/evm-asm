/-
  Ambient long creation: discharge hdec*/hvalid*/hlover/hlvalid via slice pure +
  bridges + validByteRange on regionBase. Residual path flags + hitem0..5.
-/
...
/-- Ambient long concrete Assumed creation; hdec+hvalid+hlover/hlvalid from pure+validByteRange.
    Residual path: creation/type234/long/hge7 + hitem0..5. -/
theorem extractAssumed_creation_longConcrete_pureHvalid_ambient
  ...
  simpa [absOff0, absOff1, absOff2, absOff3, absOff4, absOff5, absListOff,
    ...
    extractAssumed_creation_longConcrete_pure_ambient
...
set_option maxRecDepth 8000 in
theorem extractAssumed_creation_longConcrete_pureHvalid_ambient_fullCode
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (items : List EL.RLP.RLPItem)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_creation_longConcrete_pureHvalid_ambient
      sp0 spC s regionBase loadPtr lenW toBuf isCreationPtr bs off len items
      hspC hret htalign htover htvalid hlen hptr hbound hsuccess hcreFlag hge hdecL hlong
      hsalign hover hvalidBuf
      hitem0 hitem1 hitem2 hitem3 hitem4 hitem5 hge7)

/-- Path refinements for ambient long type234 creation arm. -/
def extractCreationType234LongPathAmbient
...
set_option maxRecDepth 8000 in
/-- Ambient Assumed footprint under long type234 creation path (statics + path + hitem).
    classical-3. Residual: other extractSuccess arms (copy/legacy/t1). -/
theorem extractAssumed_success_flat_creation_type234_long_ambient
  ...
  simpa only [hsra, hs0, hs1, hs2, hs3, hs4, hs5, hs6, hs7] using
    extractAssumed_creation_longConcrete_pureHvalid_ambient_fullCode
      spVal spC s regionBase loadPtr lenW toBuf isCreationPtr bs off len items
      hspC hret htalign htover htvalid hlen hptr hbound hsuccess hcreFlag hge hdecL hlong
      hsalign hover hvalidBuf
      hitem0 hitem1 hitem2 hitem3 hitem4 hitem5 hge7

#print axioms extractAssumed_creation_longConcrete_pureHvalid_ambient
#print axioms extractAssumed_creation_longConcrete_pureHvalid_ambient_fullCode
#print axioms extractAssumed_success_flat_creation_type234_long_ambient
