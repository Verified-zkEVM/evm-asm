/-
  Front E → AfterSave → type234 creation → ret (E2E under honesty residuals).
-/
...
/-- E → ret type234 creation. Honesty residuals: hdrop/hok/hnext/hcre. -/
theorem extractFrontCreation_then_epi
    ...
    cpsTripleWithin nFrontCreationSteps
      ...
      (fun h => ∃ next5 : Word,
        ((.x1 ↦ᵣ s.ra) ** (.x2 ↦ᵣ sp0) **
          ...
          (.x31 ↦ᵣ (next5 - (0 : Word))) **
          creExtraTemps) h) := by
  have hF := extractFrontToAfterSave sp0 spC s txBase lenW toBuf isCreationPtr
  ...
  have hC := extractFrontAfterSaveCreation_then_epi sp0 spC s
...
#print axioms extractFrontCreation_then_epi
