/-
  Strict K34 `rlp_field_to_u64_strict` caller proof.
...
namespace EvmAsm.Codegen.RlpFieldToU64StrictSAsm
...
theorem wrapper_length : rlpFieldToU64StrictWrapper_prog.length = 37 := by decide
theorem program_length : rlpFieldToU64Strict_prog.length = 37 := by
  simp [rlpFieldToU64Strict_prog, wrapper_length]

theorem reemit_byte_tie :
    rlpFieldToU64Strict_prog = rlpFieldToU64StrictWrapper_prog := by
...
abbrev C64B : Word := (GuestAddrs.rlp_content_to_u64_strict : Word)
abbrev offsetCell : Word := (GuestAddrs.rfu_offset : Word)
abbrev lengthCell : Word := (GuestAddrs.rfu_length : Word)

def wrapperCode : CodeReq := CodeReq.ofProg B rlpFieldToU64Strict_prog
def contentCode : CodeReq := rlp_content_to_u64_strict_code C64B
...
theorem wrapper_content_disjoint : wrapperCode.Disjoint contentCode := by
  unfold wrapperCode contentCode rlp_content_to_u64_strict_code B C64B
  apply CodeReq.Disjoint.ofProg_ranges
  · rw [program_length]; decide
  · rw [rlp_content_to_u64_strict_prog_length]; decide
  · rw [program_length, rlp_content_to_u64_strict_prog_length]; decide
...
theorem list_content_disjoint :
  ...
  unfold EvmAsm.Codegen.RlpListNthItemSAsm.code contentCode
    EvmAsm.Codegen.RlpListNthItemSAsm.B rlp_content_to_u64_strict_code C64B
  apply CodeReq.Disjoint.ofProg_ranges
...
theorem branchSelected
  ...
  have hb1 := cpsBranchWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 48) rlpFieldToU64Strict_prog 12
...
theorem listCalleeCallContract
  ...
  have hflat := EvmAsm.Codegen.RlpListNthItemSAsm.rlpListNthItem_flat_spec_within
    ...
    listLen index hlistLenW hindexW hindex hsalign hbytes hnowrap hover hvalid hnz hret
...
theorem callListNth
  ...
  have hcallee := listCalleeCallContract sp0 listBase listLenW indexW offsetPtr
    ...
    hsalign hbytes hnowrap hover hvalid hnz hret
  have htarget : (B + 44) + signExtend21
      (jalOff GuestAddrs.rlp_list_nth_item
        (GuestAddrs.rlp_field_to_u64_strict + 44)) = K20B := by
    unfold B K20B
    decide
  have hmem : ∀ a i, CodeReq.singleton (B + 44)
      (.JAL .x1 (jalOff GuestAddrs.rlp_list_nth_item
        (GuestAddrs.rlp_field_to_u64_strict + 44))) a = some i → code a = some i := by
    intro a i hi
    unfold code
    apply CodeReq.union_mono_left
    exact CodeReq.ofProg_mem_at B (B + 44) rlpFieldToU64Strict_prog 11
      (.JAL .x1 (jalOff GuestAddrs.rlp_list_nth_item
        (GuestAddrs.rlp_field_to_u64_strict + 44))) (by bv_omega) (by decide) rfl
      (by decide) a i hi
  have hcall := callWithin_spec (B + 44) K20B vOld
    (jalOff GuestAddrs.rlp_list_nth_item (GuestAddrs.rlp_field_to_u64_strict + 44))
    ((12 + ((85 + 93 * (index + 2)) + 6)) + 9)
...
theorem setupPrologue
  ...
  have ha := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B B rlpFieldToU64Strict_prog 0
  ...
  have hstoreMono : ∀ a i,
    ...
    intro a i hi
    exact CodeReq.ofProg_mono_sub B (B + 4) rlpFieldToU64Strict_prog
...
theorem setupMovesZero
  ...
  have h0' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 16) rlpFieldToU64Strict_prog 4 (.MV .x8 .x10)
  ...
  have h1' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 20) rlpFieldToU64Strict_prog 5 (.MV .x9 .x13)
      (by bv_omega) (by rw [program_length]; decide) rfl
  ...
  have h2' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 24) rlpFieldToU64Strict_prog 6
...
theorem setupGlobals (old13 old14 : Word) (F : Assertion) (hF : F.pcFree) :
  ...
  have hau0 := CodeReq.ofProg_mem_at B (B + 28) rlpFieldToU64Strict_prog 7
    (.AUIPC .x13 (laHi GuestAddrs.rfu_offset
      (GuestAddrs.rlp_field_to_u64_strict + 28))) (by bv_omega)
    (by rw [program_length]; decide) rfl (by rw [program_length]; decide)
  have had0 := CodeReq.ofProg_mem_at B (B + 32) rlpFieldToU64Strict_prog 8
  ...
  have hau1 := CodeReq.ofProg_mem_at B (B + 36) rlpFieldToU64Strict_prog 9
    (.AUIPC .x14 (laHi GuestAddrs.rfu_length
      (GuestAddrs.rlp_field_to_u64_strict + 36))) (by bv_omega)
    (by rw [program_length]; decide) rfl (by rw [program_length]; decide)
  have had1 := CodeReq.ofProg_mem_at B (B + 40) rlpFieldToU64Strict_prog 10
...
theorem restoreTail (sp0 newSp : Word) (saved : Saved)
  ...
  have hlMono : ∀ a i,
      CodeReq.ofProg (B + 128) (loadProg frame) a = some i →
        wrapperCode a = some i := by
    intro a i hi
    exact CodeReq.ofProg_mono_sub B (B + 128) rlpFieldToU64Strict_prog
  ...
  have hd := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 140) rlpFieldToU64Strict_prog 35
  ...
  have hr := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 144) rlpFieldToU64Strict_prog 36
...
theorem failureJoin
  ...
  have hli := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 116) rlpFieldToU64Strict_prog 29
  ...
  have hj := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 120) rlpFieldToU64Strict_prog 30
...
theorem selectedSetupExact
  ...
  have hau0 := CodeReq.ofProg_mem_at B (B + 52) rlpFieldToU64Strict_prog 13
      (.AUIPC .x5 (laHi GuestAddrs.rfu_offset
        (GuestAddrs.rlp_field_to_u64_strict + 52))) (by bv_omega)
      (by rw [program_length]; decide) rfl (by rw [program_length]; decide)
  have had0 := CodeReq.ofProg_mem_at B (B + 56) rlpFieldToU64Strict_prog 14
  ...
  have h1' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 60) rlpFieldToU64Strict_prog 15
  ...
  have h2 := add_spec_gen_rd_eq_rs2_within .x10 .x8 listBase offset
    (B + 64) (by decide)
  have h2' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 64) rlpFieldToU64Strict_prog 16
      (.ADD .x10 .x8 .x10) (by bv_omega) (by rw [program_length]; decide)
      rfl (by rw [program_length]; decide)) h2
  have hau1 := CodeReq.ofProg_mem_at B (B + 68) rlpFieldToU64Strict_prog 17
      (.AUIPC .x5 (laHi GuestAddrs.rfu_length
        (GuestAddrs.rlp_field_to_u64_strict + 68))) (by bv_omega)
      (by rw [program_length]; decide) rfl (by rw [program_length]; decide)
  have had1 := CodeReq.ofProg_mem_at B (B + 72) rlpFieldToU64Strict_prog 18
  ...
  have h4' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 76) rlpFieldToU64Strict_prog 19
...
theorem success_content_bounds
  ...
  obtain ⟨lastOff, hlast, hitem⟩ :=
    strictNthItem_last_decode hnth hcursor hnowrap
...
theorem callContentExact
  ...
  have hcallee0 := rlp_content_to_u64_strict_spec_within C64B srcBase (B + 84)
  ...
  have hmem : ∀ a i, CodeReq.singleton (B + 80)
      (.JAL .x1 (jalOff GuestAddrs.rlp_content_to_u64_strict
        (GuestAddrs.rlp_field_to_u64_strict + 80))) a = some i → code a = some i := by
    intro a i hi
    unfold code
    apply CodeReq.union_mono_left
    exact CodeReq.ofProg_mem_at B (B + 80) rlpFieldToU64Strict_prog 20
      (.JAL .x1 (jalOff GuestAddrs.rlp_content_to_u64_strict
        (GuestAddrs.rlp_field_to_u64_strict + 80))) (by bv_omega)
      (by rw [program_length]; decide) rfl (by rw [program_length]; decide)
      a i hi
  have hcall := callWithin_spec (B + 80) C64B vOld
    (jalOff GuestAddrs.rlp_content_to_u64_strict (GuestAddrs.rlp_field_to_u64_strict + 80))
...
end EvmAsm.Codegen.RlpFieldToU64StrictSAsm
