/-
  Ambient Assumed packaging for legacy short 20B copy (region partition).
  Mono legacy E2E nSteps ≤ type234 ambient of_front budget.
-/
...
theorem nFrontCopyStepsShortLegacyRegionAmbient_le :
    nFrontCopyStepsShortLegacyRegionAmbient ≤ nFrontCopyStepsShortRegionAmbient := by
  simp only [nFrontCopyStepsShortLegacyRegionAmbient, nFrontCopyStepsShortRegionAmbient,
    nTypeSteps]
  omega

set_option maxRecDepth 8000 in
theorem extractAssumed_copy_under_honesty_of_decode_short_concrete_legacy_region_ambient
  ...
  have hShort := extractFrontCopy_then_epi_of_decode_short_concrete_legacy_region_ambient
  ...
  have hMono := cpsTripleWithin_mono_nSteps
    nFrontCopyStepsShortLegacyRegionAmbient_le hShort
  refine cpsTripleWithin_weaken (fun _ hp => hp) (fun _ hq => by
    dsimp only [copyE2EPostAmbient, contentPtr, w2] at hq ⊢
    simp only [htype0] at hq ⊢
    xperm_hyp hq) hMono

set_option maxRecDepth 8000 in
theorem extractAssumed_copy_fullCode_of_decode_short_concrete_legacy_region_ambient
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (absOff0 absOff1 absOff2 absOff3 q : Nat)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_copy_under_honesty_of_decode_short_concrete_legacy_region_ambient
      sp0 spC s regionBase loadPtr lenW toBuf isCreationPtr bs off len
      absOff0 absOff1 absOff2 absOff3 q
      hspC hret hcur htype0 hsalign
      hoff0 hover0 hvalid0 hss0 hls0 hll0 hdec0 hinb0
...
#print axioms extractAssumed_copy_under_honesty_of_decode_short_concrete_legacy_region_ambient
#print axioms extractAssumed_copy_fullCode_of_decode_short_concrete_legacy_region_ambient
