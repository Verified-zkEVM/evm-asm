...
theorem csrsStep_spec (img : List (BitVec 8))
    ...
    cpsTripleWithin 1 ((GuestAddrs.bnf_add_mod_p + 60) : Word) ((GuestAddrs.bnf_add_mod_p + 64) : Word) addCr
      (((.x5 : Reg) ↦ᵣ (0xbd6e3998 : Word)) ** regOwns csrsRest
  ...
  refine cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp) (fun _ hq => hq)
    (cpsTripleWithin_peel_regOwns csrsRest (by decide)
      (P := ((.x5 : Reg) ↦ᵣ (0xbd6e3998 : Word)) ** bytesRegion arenaB img)
      (fun vf => ?_))
  have hcs := csrs_arith256Mod_spec_within ((GuestAddrs.bnf_add_mod_p + 60) : Word) .x5 (by decide)
    arenaB 272 img
    (fun r => if r = .x5 then (0xbd6e3998 : Word) else vf r)
    ...
    (by
      ...
      show (if (Reg.x5 : Reg) = .x5 then (0xbd6e3998 : Word) else vf .x5) = _
      rw [if_pos rfl]
      decide)
  ...
  rw [regFileIs_eq_regAtoms, regAtoms_eq_regAtomsOf _ _ (by decide),
    exposedRegs_split5,
    show (if (Reg.x5 : Reg) = .x5 then (0xbd6e3998 : Word) else vf .x5)
      = (0xbd6e3998 : Word) from if_pos rfl,
    regAtomsOf_congr
      (fun r => if r = .x5 then (0xbd6e3998 : Word) else vf r) vf csrsRest
      (fun r hr => by
        show (if r = .x5 then (0xbd6e3998 : Word) else vf r) = vf r
...
theorem stageC_spec (aPtr bPtr outPtr : Word)
  ...
  have hla5 := la_materialize_within .x5 (vf .x5) ((GuestAddrs.bnf_add_mod_p + 52) : Word)
    (0xbd6e3998 : Word) (cr := addCr) (by decide) (by decide)
  ...
  have hrest : cpsTripleWithin
    ...
    have hla10 := la_materialize_within .x10 (vg .x10) ((GuestAddrs.bnf_add_mod_p + 64) : Word)
      (0xbd6e38f0 : Word) (cr := addCr) (by decide) (by decide)
      (by code_mem) (by code_mem)
    ...
    have hflat := bnfLeToBeFlat_spec ((GuestAddrs.bnf_add_mod_p + 80) : Word) (0xbd6e38f0 : Word)
      outPtr (leBytes32 r) outOld (by rw [length_leBytes32]) holen
      (by
        refine ⟨?_, ?_, ?_⟩
        · show ((0xbd6e38f0 : Word)).toNat % 8 = 0
          decide
        · show ((0xbd6e38f0 : Word)).toNat + (leBytes32 r).length < 2 ^ 64
          rw [length_leBytes32]
          decide
        · intro k hk
          rw [length_leBytes32] at hk
          rw [show (0xbd6e38f0 : Word) + BitVec.ofNat 64 k
            ...
            rw [BitVec.toNat_add, BitVec.toNat_add, BitVec.toNat_ofNat,
              BitVec.toNat_ofNat,
              show ((0xbd6e38f0 : Word)).toNat = 0xbd6e38f0 from by decide,
      ...
      (by decide) (by omega)
      (by
        have hsrc : ((0xbd6e38f0 : Word)).toNat = 0xbd6e38f0 := by decide
        rcases hdO with h | h
        · left
          rw [hsrc]
          omega
    ...
    have hcallee : cpsTripleWithin ((bnfLeToBeFn (0xbd6e38f0 : Word) outPtr
    ...
    have hcall := callWithin_spec ((GuestAddrs.bnf_add_mod_p + 76) : Word) (GuestAddrs.bnf_le_to_be : Word)
      ...
      ((bnfLeToBeFn (0xbd6e38f0 : Word) outPtr (leBytes32 r) outOld).body.steps + 1)
    ...
    rw [show (bnfLeToBeFn (0xbd6e38f0 : Word) outPtr
    ...
    have hla10F := cpsTripleWithin_frameR
      (((.x5 : Reg) ↦ᵣ (0xbd6e3998 : Word)) ** ((.x1 : Reg) ↦ᵣ ((GuestAddrs.bnf_add_mod_p + 52) : Word))
    ...
    have hmvF := cpsTripleWithin_frameR
      (((.x5 : Reg) ↦ᵣ (0xbd6e3998 : Word)) ** ((.x1 : Reg) ↦ᵣ ((GuestAddrs.bnf_add_mod_p + 52) : Word))
        ** (.x8 ↦ᵣ bPtr) ** (.x10 ↦ᵣ (0xbd6e38f0 : Word))
        ** (.x6 ↦ᵣ vg .x6) ** (.x7 ↦ᵣ vg .x7) ** (.x28 ↦ᵣ vg .x28)
        ** (.x29 ↦ᵣ vg .x29) ** (.x30 ↦ᵣ vg .x30) ** (.x31 ↦ᵣ vg .x31)
        ** (.x12 ↦ᵣ vg .x12) ** (.x13 ↦ᵣ vg .x13) ** (.x14 ↦ᵣ vg .x14)
        ** (.x15 ↦ᵣ vg .x15) ** (.x16 ↦ᵣ vg .x16) ** (.x17 ↦ᵣ vg .x17)
    ...
    have hchain2 := cpsTripleWithin_seq_perm_same_cr
      (fun h hp => by
        ...
        rw [bytesRegion_window_focus arenaB img₃ 0x40 32 (by omega)
            ...
            show arenaB + BitVec.ofNat 64 0x40 = (0xbd6e38f0 : Word) from by decide]
          at hp
        ...
        have hp1 : ((.x6 ↦ᵣ vg .x6) ** (.x7 ↦ᵣ vg .x7) ** (.x28 ↦ᵣ vg .x28)
            ...
            ** (.x12 ↦ᵣ vg .x12) ** (.x13 ↦ᵣ vg .x13) ** (.x14 ↦ᵣ vg .x14)
            ** (.x15 ↦ᵣ vg .x15) ** (.x16 ↦ᵣ vg .x16) ** (.x17 ↦ᵣ vg .x17)
            ** (((.x5 : Reg)) ↦ᵣ (0xbd6e3998 : Word))
            ** (((.x1 : Reg) ↦ᵣ ((GuestAddrs.bnf_add_mod_p + 52) : Word)) ** (.x10 ↦ᵣ (0xbd6e38f0 : Word))
    ...
    have hchain2' := cpsTripleWithin_weaken (fun _ hp => hp)
      (fun h hq => by
        have hq1 : (((fun hp => ∃ ws',
            ((⌜beBytesToNat ws' = wsNat256 (leBytes32 r) 0 ∧ ws'.length = 32⌝
              ** regOwns exposedRegs ** bytesRegion outPtr ws'
              ** bytesRegion (0xbd6e38f0 : Word) (leBytes32 r))) hp) : Assertion)
            ** (((.x1 : Reg) ↦ᵣ ((GuestAddrs.bnf_add_mod_p + 80) : Word)) ** (.x8 ↦ᵣ bPtr)
    ...
    have hcont : ∀ ws' : List (BitVec 8),
      ...
      · refine ⟨ws', ?_⟩
        ...
        have hq1 : ((⌜beBytesToNat ws' = r ∧ ws'.length = 32⌝ : Assertion)
          ...
          rw [hdwin, show arenaB + BitVec.ofNat 64 0x40 = (0xbd6e38f0 : Word)
            from by decide]
          xperm_hyp hq
        rw [← bytesRegion_window_focus arenaB img₃ 0x40 32 (by omega)
                          ...
                          (sepConj_mono (regIs_to_regOwn .x14 _)
                            (sepConj_mono (regIs_to_regOwn .x15 _)
                              (sepConj_mono (regIs_to_regOwn .x16 _)
                                (sepConj_mono (regIs_to_regOwn .x17 _)
                                  (fun _ hh => hh)))))))))))))) h hp1
      have hp3 : ((((.x5 : Reg)) ↦ᵣ (0xbd6e3998 : Word)) ** regOwns csrsRest
          ** bytesRegion arenaB img₂
          ** (((.x1 : Reg) ↦ᵣ ((GuestAddrs.bnf_add_mod_p + 52) : Word)) ** (.x8 ↦ᵣ bPtr)
            ** (.x9 ↦ᵣ outPtr)
            ** bytesRegion aPtr aBE ** bytesRegion bPtr bBE
