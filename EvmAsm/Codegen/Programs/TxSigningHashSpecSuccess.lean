/-
  ...
  Composes already-proved body slices toward the nonempty list path
  (nth → prefix → kss). The prologue phases (nonempty fallthrough, the total
  outer-header parse, nth ABI setup) live in TxSigningHashSpecSetup.
  Empty-len fail lives in BodyLate / Spec.
-/
...
/-- Saved s-regs at the nth jal site after the header parse
    (`s5 = hdrLen`, the total outer-list header length). -/
def tshNthSaved (ra inPtr lenW nFields typePrefix outPtr hdrLen : Word) : Saved where
  ra := ra
  s0 := inPtr
  s1 := lenW
  s2 := nFields
...
abbrev tshNthJalMid
    (a0 a1 a2 a3 a4 v5 v6 v22 wordOld hdrLen : Word)
    (sp0 vOld v7 v28 v29 v30 v31 oldOff oldLen : Word)
  ...
  ((.x10 ↦ᵣ a0) ** (.x11 ↦ᵣ a1) ** (.x12 ↦ᵣ tshNthIndexW a2) **
    ...
    (.x20 ↦ᵣ a4) ** (.x21 ↦ᵣ hdrLen) ** (.x22 ↦ᵣ v22) **
    (.x0 ↦ᵣ (0 : Word)) **
...
abbrev tshNthJalCallPre
    (a0 a1 a2 a3 a4 vOld sp0 oldOff oldLen wordOld hdrLen : Word)
    (input : List (BitVec 8)) : Assertion :=
  ((.x1 ↦ᵣ vOld) **
    callEntryRest sp0 a0 a1 (tshNthIndexW a2) tshNthOffPtr tshNthLenPtr
      oldOff oldLen (tshNthSaved (tshNthJalPC + 4) a0 a1 a2 a3 a4 hdrLen) input) **
  tshNthCallFrame (0 : Word) wordOld a3

/-- Reshape setup-mid (regIs) into nth callWithin pre (regOwn). -/
theorem tshNthJalMid_to_callPre
    (a0 a1 a2 a3 a4 wordOld hdrLen : Word)
    (sp0 vOld v7 v28 v29 v30 v31 oldOff oldLen : Word)
    (input : List (BitVec 8))
    (h : PartialState)
    (hp : tshNthJalMid a0 a1 a2 a3 a4 (tshNthIndexW a2) (248 : Word) (0 : Word)
      wordOld hdrLen sp0 vOld v7 v28 v29 v30 v31 oldOff oldLen input h) :
    tshNthJalCallPre a0 a1 a2 a3 a4 vOld sp0 oldOff oldLen wordOld hdrLen
      input h := by
...
theorem tshSetupThroughNthCall_spec
    ...
    let saved := tshNthSaved (tshNthJalPC + 4) a0 a1 a2 a3 a4 (tshHdrLen input h0)
  ...
  have hcall := tsh_nth_callWithin vOld sp0 a0 a1 (tshNthIndexW a2) oldOff oldLen
    (tshNthSaved vOld a0 a1 a2 a3 a4 (tshHdrLen input h0)) input listLen index
    (tshNthCallFrame (0 : Word) wordOld a3)
  ...
  have c := cpsTripleWithin_seq_perm_same_cr (fun h hp =>
    tshNthJalMid_to_callPre a0 a1 a2 a3 a4 wordOld (tshHdrLen input h0)
      sp0 vOld v7 v28 v29 v30 v31
