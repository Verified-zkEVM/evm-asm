/-
  Front long AfterSave → t1 creation of_decode with concrete longWalkCursor/End.
-/
...
theorem extractFrontAfterSaveCreation_then_epi_of_decode_long_t1
  ...
  let listLen := lenW - (teerTxTypeDispatch txBytes).2.2
  let cursor := longWalkCursor txBase txBytes listOff hwi_off
  let endPtr := longWalkEnd txBase listLen listOff
  have hMid := extractT1AfterSaveCreation_then_epi_of_decode sp0 spC s
    txBase lenW
    (teerTxTypeDispatch txBytes).2.2
    cursor endPtr toBuf isCreationPtr s.s7 txBytes
    srcOff0 srcOff1 srcOff2 srcOff3 srcOff4
    hspC hret hcur hsalign
    ...
    hnext4 hcre
  refine cpsTripleWithin_weaken (fun h hp => ?_) (fun _ hq => hq) hMid
  have hty := frontAfterSavePostLong_to_midJoinPre spC s txBase lenW
    toBuf isCreationPtr txBytes hwi_off h hp
  simp only [htype1] at hty
  exact hty

#print axioms extractFrontAfterSaveCreation_then_epi_of_decode_long_t1
