/-
  EvmAsm.Codegen.Programs.Bls12PtCopySAsm

  Verified SAsm port of `blq_pt_copy`: copy the 1728-byte BLS12-381 FQ12 projective point from
  `a0` to the writable destination at `a1`.  The emitted routine is a
  bottom-test dword loop over 216 limbs.
-/
...
namespace Bls12PtCopySAsm

def frameOk1728 (src dst : Word) : Prop :=
  src.toNat + 1728 < 2 ^ 64 ∧ dst.toNat + 1728 < 2 ^ 64 ∧
    (src.toNat + 1728 ≤ dst.toNat ∨ dst.toNat + 1728 ≤ src.toNat)

def copyWin1728 (srcBytes orig : List (BitVec 8)) (i : Nat) : List (BitVec 8) :=
  srcBytes.take (8 * i) ++ orig.drop (8 * i)

theorem copyWin1728_zero (srcBytes orig : List (BitVec 8)) :
    copyWin1728 srcBytes orig 0 = orig := by
  simp [copyWin1728]

theorem copyWin1728_216_eq (srcBytes orig : List (BitVec 8))
    (hs : srcBytes.length = 1728) (ho : orig.length = 1728) :
    copyWin1728 srcBytes orig 216 = srcBytes := by
  simp only [copyWin1728, Nat.reduceMul]
  rw [List.take_of_length_le (by omega), List.drop_eq_nil_of_le (by omega), List.append_nil]

theorem length_copyWin1728 (srcBytes orig : List (BitVec 8)) (i : Nat)
    (hs : srcBytes.length = 1728) (ho : orig.length = 1728) (hi : i ≤ 216) :
    (copyWin1728 srcBytes orig i).length = 1728 := by
  simp only [copyWin1728, List.length_append, List.length_take, List.length_drop, hs, ho]
  omega

theorem copyWin1728_step (srcBytes orig : List (BitVec 8)) (i : Nat)
    (hs : srcBytes.length = 1728) (ho : orig.length = 1728) (hi : i < 216) :
    setBytes (copyWin1728 srcBytes orig i) (8 * i)
      (dwordBytes (packBytes ((srcBytes.drop (8 * i)).take 8))) =
    copyWin1728 srcBytes orig (i + 1) := by
...
private theorem src_miss8 (src dst : Word) (ws : List (BitVec 8)) (k : Nat)
    (hk : k < 216) (hws : ws.length = 1728) (hfr : frameOk1728 src dst) :
...
private theorem copy_engine (src dst : Word) (srcBytes : List (BitVec 8))
    (rf : RegFile) (ws : List (BitVec 8)) (i : Nat) (hi : i < 216)
    (hx10 : rf.get .x10 = src + BitVec.ofNat 64 (8 * i))
    (hx11 : rf.get .x11 = dst + BitVec.ofNat 64 (8 * i))
    (hws : ws.length = 1728) (hfr : frameOk1728 src dst) :
...
private theorem copy_blockVCs (src dst : Word) (srcBytes : List (BitVec 8))
    ...
    (hx10 : rf.get .x10 = src + BitVec.ofNat 64 (8 * i))
    (hx11 : rf.get .x11 = dst + BitVec.ofNat 64 (8 * i))
    (hws : ws.length = 1728) (hs : srcBytes.length = 1728) (hfr : frameOk1728 src dst) :
...
def blqPtCopyBody (src dst : Word) (srcBytes orig : List (BitVec 8)) : Stmt :=
  ...
  .doWhile "loop" (.bne .x7 .x0) 215 (copyInv src dst srcBytes orig)
    (.block "copy" copyStepBlock)

def blqPtCopyFn (src dst : Word) (srcBytes orig : List (BitVec 8)) : Fn where
  name := "blqPtCopy"
  region := ⟨src, srcBytes⟩
  rw := ⟨dst, 1728⟩
  pre := fun rf ws _ =>
...
def blqPtCopy_verified : Program :=
  (blqPtCopyBody 0 0 [] []).flatten 0

#guard (blqPtCopy_verified : List Instr).length = 7
#guard (blqPtCopyBody 0 0 [] []).flatten 0 = (blqPtCopyBody 0 0 [] []).flatten 0x80000000
#guard (blqPtCopyBody 0 0 [] []).flatten 0 ++ [Instr.JALR .x0 .x1 0] = blqPtCopy_prog

theorem blqPtCopyFn_spec (src dst : Word) (srcBytes orig : List (BitVec 8))
    (hwf : (Region.mk src srcBytes).wf) (hrww : RwRegion.wf ⟨dst, 1728⟩) (base : Word) :
    (blqPtCopyFn src dst srcBytes orig).Spec base := by
  have hbase : (blqPtCopyFn src dst srcBytes orig).rw.base = dst := rfl
  vcgen
  case region => exact ⟨hwf, hrww⟩
  case blqPtCopy.loop.inv_init =>
    rintro rf' ws' A' h
