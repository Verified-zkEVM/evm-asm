...
def blockVerdictMtxValidationTail : String :=
  ...
  -- skip. The single-tx i3djw_skip_list is {recipient, sender, coinbase} plus
  -- BAI-0 (2935+4788; #10684); multi-tx has up to 2N+1 gas/value-coupled accounts
  -- plus the same BAI-0 pair. We are
  ...
  --   skip[2N+1..2N+2] = EIP-2935 + EIP-4788 only (#10684 dropped 7002/7251/6110/SYSTEM)
  -- count = 2N+3. 32-byte-strided, address in the first 20 bytes. BEHAVIOR-NEUTRAL:
  ...
  "  addi t6, t6, 32\n" ++                                             -- t6 = &skip[2N+1]
  -- #10684: copy ONLY BAI-0 predeploys (2935, 4788) — contiguous at bbcv_sys_2935.
  -- Four deferred (7002/7251/6110/SYSTEM) dropped: no spec counterpart for skip;
  -- poison sentinel FR0 on stride-1045+r200; keep = FA surface (un-checked).
  "  la t1, bbcv_sys_2935; li t4, " ++ toString bvMtxSystemSkipEntries ++ "\n" ++
  ".Lbv_skl_sys_o:\n  li t3, 0\n" ++
  ...
  "  la t2, bv_tx_count; ld t2, 0(t2); slli t3, t2, 1; addi t3, t3, " ++ toString (1 + bvMtxSystemSkipEntries) ++ "; la t0, bv_mtx_skip_count; sd t3, 0(t0)\n" ++  -- count = 2N+1+sys
