/-
  ExtractAssumed packaging under short concrete t1 creation of_decode.
-/
...
theorem nFrontCreationStepsShortT1_le_nFront :
    nFrontCreationStepsShortT1 ≤ nFrontCreationSteps := by
  simp only [nFrontCreationStepsShortT1, nFrontCreationSteps, nTypeSteps]
  omega

set_option maxRecDepth 8000 in
theorem extractAssumed_creation_under_honesty_of_decode_short_concrete_t1
    ...
    (hcre : ∀ (next4 len4 : Word),
      rlpItemDecode txBytes srcOff4 (txBase + BitVec.ofNat 64 srcOff4)
        (shortWalkEnd txBase (lenW - (teerTxTypeDispatch txBytes).2.2)
          (teerTxTypeDispatch txBytes).2.2.toNat) next4 len4 → len4 = (0 : Word))
  ...
  have hShort := extractFrontCreation_then_epi_of_decode_short_concrete_t1
  ...
  have h := cpsTripleWithin_mono_nSteps
    nFrontCreationStepsShortT1_le_nFront hShort
  refine cpsTripleWithin_weaken (fun _ hp => hp) (fun st hq => ?_) h
  obtain ⟨next4, hq'⟩ := hq
  refine ⟨next4, ?_⟩
  simp only [creationE2EPost, creExtraTemps, htype1]
  exact hq'

set_option maxRecDepth 8000 in
theorem extractAssumed_creation_fullCode_of_decode_short_concrete_t1
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr : Word)
    (txBytes : List (BitVec 8))
    (srcOff0 srcOff1 srcOff2 srcOff3 srcOff4 : Nat)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_creation_under_honesty_of_decode_short_concrete_t1
      sp0 spC s txBase lenW toBuf isCreationPtr txBytes
      srcOff0 srcOff1 srcOff2 srcOff3 srcOff4
      hspC hret hcur htype1 hsalign
      hoff0 hover0 hvalid0 hss0 hls0 hll0 hdec0 hinb0
...
#print axioms extractAssumed_creation_under_honesty_of_decode_short_concrete_t1
#print axioms extractAssumed_creation_fullCode_of_decode_short_concrete_t1
