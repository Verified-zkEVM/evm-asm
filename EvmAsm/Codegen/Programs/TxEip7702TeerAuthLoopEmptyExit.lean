...
/-! ## Empty-short identity BridgePre (oldCount free via pick)

`hident` pins content=regionBase and listLenW=1; oldCount is ∃-picked from
BridgePre heap (list_count overwrites AuthCount). Thinner than full hspecialize. -/

/-- Exists BridgePre → ∃ oldCount fixed empty-short BridgePre under identity. -/
theorem teerAuthContentBridgePre_exists_to_empty_short_ident
    (spVal spC loadPtr lenW balPtr balLenW chainIdW : Word)
    (s7Old cursorV endW s11 : Word) (s : TeerSaved) (innerVal : Word)
    (regionBase : Word) (bs balBytes : List (BitVec 8))
    (hident : ∀ (content listLenW oldCount : Word) h,
      teerAuthContentBridgePre spVal spC LinkAuthWalkNext9 loadPtr lenW
          balPtr balLenW chainIdW content listLenW s7Old cursorV endW s11 s
          innerVal oldCount regionBase bs balBytes h →
        content = regionBase ∧ listLenW = BitVec.ofNat 64 1) :
    ∀ h,
      (∃ (content listLenW oldCount : Word),
        teerAuthContentBridgePre spVal spC LinkAuthWalkNext9 loadPtr lenW
          balPtr balLenW chainIdW content listLenW s7Old cursorV endW s11 s
  ...
  intro h ⟨content, listLenW, oldCount, hBr⟩
  obtain ⟨hc, hl⟩ := hident content listLenW oldCount h hBr
  subst hc hl
  exact ⟨oldCount, hBr⟩

/-- free26 → ∃ oldCount BridgePre regionBase (ofNat 1) under hrun + hident. -/
theorem teerEmptyAuth_free26_to_bridgePre_empty_short_ident
    {n : Nat}
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
    (s : TeerSaved)
      ...
      (fun h =>
        ∃ (oldCount : Word),
          teerAuthContentBridgePre spVal spC LinkAuthWalkNext9 loadPtr lenW
            ...
            endW s11 s innerVal oldCount regionBase bs balBytes h) := by
  have hex :=
    teerEmptyAuth_free26_to_bridgePre_pick ret spVal spC regionBase loadPtr lenW
      balPtr balLenW chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
      s bs balBytes hspC innerVal endL endW cursorV srcOffA9 hrun
  exact cpsTripleWithin_weaken (fun _ hp => hp)
    (teerAuthContentBridgePre_exists_to_empty_short_ident spVal spC loadPtr lenW
      balPtr balLenW chainIdW s7 cursorV endW s11 s innerVal regionBase bs balBytes
      hident)
    hex

#print axioms teerAuthContentBridgePre_exists_to_empty_short_ident
#print axioms teerEmptyAuth_free26_to_bridgePre_empty_short_ident
...
/-! ## Free26EmptyShort → applied_flat packaging -/

/-- Mono: Free26EmptyShort front+exit fits `nTeerSteps`. -/
theorem teerEmptyAuth_free26EmptyShort_front_then_exit_mono
    (front : TeerFrontToAuthLoopAssumedFree26EmptyShort)
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s : TeerSaved)
    (bs balBytes : List (BitVec 8)) (off len : Nat)
      ...
      (fun hp =>
        ∃ (refund _t0Old _t1Old baiW' : Word),
          ((((.x10 ↦ᵣ (0 : Word)) ** (.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
            ...
            stackFree spC 6) hp) :=
  cpsTripleWithin_mono_nSteps front.hn
    (teerEmptyAuth_free26EmptyShort_front_then_exit front ret spVal spC regionBase
      loadPtr lenW balPtr balLenW chainIdW baiW s bs balBytes off len old1 s7Old
      cursorV endW s11 innerVal hoff0 hcontent hlistLenW hold1 hret hbal hptr
      hbound hspC hra hs0w hs1w hs2w hs3w hs4w hs9w hs11w
      hsalign hslack hover hvalid h0)

/-- Free26EmptyShort → applied_flat free20 ** nested free leftover.
    Residual: inhabit EmptyShort (front+rz). -/
theorem teerEmptyAuth_free26EmptyShort_front_then_exit_applied_flat
    (teer : TeerApplied)
    (front : TeerFrontToAuthLoopAssumedFree26EmptyShort)
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s : TeerSaved)
      ...
      (fun hp =>
        ∃ (_refund _baiW' : Word),
          (((.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
            ...
            stackFree spC 6) hp) := by
  have hrun :=
    teerEmptyAuth_free26EmptyShort_front_then_exit_mono front ret spVal spC
      regionBase loadPtr lenW balPtr balLenW chainIdW baiW s bs balBytes off len
      old1 s7Old cursorV endW s11 innerVal hoff0 hcontent hlistLenW hold1 hret
      hbal hptr hbound hspC hra hs0w hs1w hs2w hs3w hs4w hs9w hs11w
      hsalign hslack hover hvalid h0
  refine cpsTripleWithin_weaken (fun _ hp => hp) ?_ hrun
  intro h hq
  obtain ⟨refund, _t0, _t1, baiW', hp0⟩ := hq
...
/-- Free26EmptyShort → applied_flat free20 ** nested under const-zero teer.
    Residual: inhabit EmptyShort (FrontToBridge + RolledZero). -/
theorem teerEmptyAuth_free26EmptyShort_front_then_exit_applied_flat_zero
    (front : TeerFrontToAuthLoopAssumedFree26EmptyShort)
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s : TeerSaved)
    (bs balBytes : List (BitVec 8)) (off len chainId bai : Nat)
      ...
      (fun hp =>
        ∃ (_refund _baiW' : Word),
          (((.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
            ...
            stackFree spC 6) hp) := by
  have h :=
    teerEmptyAuth_free26EmptyShort_front_then_exit_applied_flat teerApplied_zero
      front ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW s
      bs balBytes off len chainId bai old1 s7Old cursorV endW s11 innerVal hoff0
      hcontent hlistLenW hold1 hret hbal hptr hbound hspC hra
      hs0w hs1w hs2w hs3w hs4w hs9w hs11w
      hsalign hslack hover hvalid h0
      (teerApplied_zero_hteer0_all bs balBytes off len chainId bai hbound)
...
#print axioms teerEmptyAuth_free26EmptyShort_front_then_exit_mono
#print axioms teerEmptyAuth_free26EmptyShort_front_then_exit_applied_flat
#print axioms teerEmptyAuth_free26EmptyShort_front_then_exit_applied_flat_zero

/-! ## free26 of_hrun empty-short (Success/guards/hspe free; no Front structure)

Thin dual of `free26_to_exitPack_of_hrun_fixed` for 0xc0 listLen=1 identity.
Residual: `hrun` (applied_as_postEx), pure specialize, `hrolled0`/RolledZero,
domain fixture, ABI wire. -/

/-- free26 → ExitPack empty-short under hrun. Frees Success/guards/hspe/hlen/Link align. -/
theorem teerEmptyAuth_free26_to_exitPack_of_hrun_empty_short
    {n : Nat}
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
    ...
    (hspecialize : ∀ (next lenK oc : Word) h,
      teerAuthContentBridgePre spVal spC LinkAuthWalkNext9 loadPtr lenW
          balPtr balLenW chainIdW (next - lenK) lenK s7 cursorV endW s11 s
          innerVal oc regionBase bs balBytes h →
        next - lenK = regionBase ∧ lenK = BitVec.ofNat 64 1 ∧ oc = oldCount) :
    cpsTripleWithin (n + nListCountAuthLoopStart 1) E AfterAuthLoopLi
      teerLinkedField0
      (stackFree spVal nTeerStackWithListCount **
        teerAuthContentAppliedEntryRest ret spVal loadPtr lenW balPtr balLenW
          chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
      ...
      (fun hp =>
        ∃ (refund t0Old t1Old baiW' : Word),
          teerAuthLoopEmptyExitPack spVal spC s
            ...
            t0Old t1Old baiW'
            regionBase bs balBytes balPtr hp) := by
  have hguards := teerWalkInit_guards_empty_short bs regionBase (BitVec.ofNat 64 1)
    hoff h0 rfl
  obtain ⟨h_ge, h_hi, h_exact⟩ := hguards
  have hsuccess := teerSuccess_empty_short_list bs regionBase hoff h0
  have hspeHover : regionBase.toNat + 1 + 9 < 2 ^ 64 := by omega
  have hspe := teerListCountResultSpecialize_empty_short bs regionBase hspeHover
  have hlen : BitVec.ofNat 64 1 ≠ (0 : Word) := teer_listLenW_one_ne_zero rfl
  have hslack' : (1 : Nat) + 9 ≤ bs.length := hslack
  exact teerEmptyAuth_free26_to_exitPack_of_hrun_fixed asm ret spVal spC regionBase
    loadPtr lenW balPtr balLenW chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
    s bs balBytes hspC innerVal endL endW cursorV srcOffA9
    regionBase (BitVec.ofNat 64 1) oldCount
    (spC + signExtend12 (-48 : BitVec 12)) regionBase 1 bs
    s0' s1' s2' s3' v24 hoff rfl rfl rfl
    hs0 hs1 hs2 hs3 hv24 hs0s hs1s hs2s hs3s hs4 hs9 hs11 rfl
    hsalign hslack' hover hvalid rfl teerLinkListCount_aligned hsuccess hspe hlen
    (by omega) (by simpa using hvalid 0 hoff)
    h_ge h_hi h_exact hrolled0 hrun hspecialize

/-- free26 → ret empty-short under hrun. Residual: hrun + specialize + hrolled0 + domain. -/
theorem teerEmptyAuth_free26_toRet_of_hrun_empty_short
    {n : Nat}
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
    ...
    (hspecialize : ∀ (next lenK oc : Word) h,
      teerAuthContentBridgePre spVal spC LinkAuthWalkNext9 loadPtr lenW
          ...
          innerVal oc regionBase bs balBytes h →
        next - lenK = regionBase ∧ lenK = BitVec.ofNat 64 1 ∧ oc = oldCount) :
    cpsTripleWithin (n + nListCountAuthLoopStart 1 + 30) E ret
      teerLinkedField0
      (stackFree spVal nTeerStackWithListCount **
        teerAuthContentAppliedEntryRest ret spVal loadPtr lenW balPtr balLenW
          chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
  ...
  have hspe := teerListCountResultSpecialize_empty_short bs regionBase hspeHover
  have hlen : BitVec.ofNat 64 1 ≠ (0 : Word) := teer_listLenW_one_ne_zero rfl
  exact teerEmptyAuth_free26_toRet_of_hrun_fixed asm ret spVal spC regionBase
    loadPtr lenW balPtr balLenW chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
    s bs balBytes hspC innerVal endL endW cursorV srcOffA9
    regionBase (BitVec.ofNat 64 1) oldCount
    (spC + signExtend12 (-48 : BitVec 12)) regionBase 1 bs
    ...
    hsalign hslack hover hvalid rfl teerLinkListCount_aligned hsuccess hspe hlen
    (by omega) (by simpa using hvalid 0 hoff)
    h_ge h_hi h_exact hrolled0 hret hra hrun hspecialize

/-- free26 empty-short → applied_flat free20**nested under hrun + `teerApplied_zero`.
    Residual: hrun + specialize + hrolled0 + domain + ABI wire. -/
theorem teerEmptyAuth_free26_to_applied_flat_of_hrun_empty_short_zero
    {n : Nat}
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
      ...
      (fun hp =>
        ∃ (_refund baiW' : Word),
          (((.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
            ...
            stackFree spC 6) hp) := by
  have hret' :=
    teerEmptyAuth_free26_toRet_of_hrun_empty_short asm ret spVal spC regionBase
      loadPtr lenW balPtr balLenW chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
      s bs balBytes hspC innerVal endL endW cursorV srcOffA9 oldCount
      s0' s1' s2' s3' v24 hoff hs0 hs1 hs2 hs3 hv24 hs0s hs1s hs2s hs3s hs4 hs9 hs11
      hsalign hslack hover hvalid h0 hrolled0 hret hra hrun hspecialize
  refine cpsTripleWithin_weaken (fun _ hp => hp) ?_ hret'
  intro h hq
  obtain ⟨refund, _t0, _t1, baiW', hp0⟩ := hq
  have hbody :
      ∀ h1,
    ...
    have hpB' :
      ...
      xperm_hyp hpB
    have himp :=
      teerEmptyExitPost_imp_applied_flat_post teerApplied_zero ret spVal spC regionBase
        balPtr baiW' refund s.s0 s.s1 s.s2 s.s3 s.s4 s.s5 s.s6 s.s7 s.s8 s.s9
        s.s10 s.s11 bs balBytes off len chainId bai s
        rfl rfl rfl rfl rfl rfl rfl rfl rfl rfl rfl rfl hspC
        (teerApplied_zero_hteer0_all bs balBytes off len chainId bai hbound) h1 hpB'
    simpa [teerApplied_zero_eq] using himp
  refine ⟨refund, baiW', ?_⟩
  exact sepConj_mono hbody (fun _ hh => hh) h hp0

#print axioms teerEmptyAuth_free26_to_exitPack_of_hrun_empty_short
#print axioms teerEmptyAuth_free26_toRet_of_hrun_empty_short
#print axioms teerEmptyAuth_free26_to_applied_flat_of_hrun_empty_short_zero

/-- free26 → ExitPack empty-short under hrun + hident (no oldCount / full specialize).
    Residual: hrun, hident (content=regionBase ∧ listLenW=1), hrolled0, domain, ABI wire. -/
theorem teerEmptyAuth_free26_to_exitPack_of_hrun_empty_short_ident
    {n : Nat}
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
  ...
  have hlen : BitVec.ofNat 64 1 ≠ (0 : Word) := teer_listLenW_one_ne_zero rfl
  have hBr :=
    teerEmptyAuth_free26_to_bridgePre_empty_short_ident ret spVal spC regionBase
      loadPtr lenW balPtr balLenW chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
      s bs balBytes hspC innerVal endL endW cursorV srcOffA9 hrun hident
  have hmid : ∀ (oldCount : Word),
      cpsTripleWithin (nListCountAuthLoopStart 1) AtListCount AfterAuthLoopLi
        teerLinkedField0
        (teerAuthContentBridgePre spVal spC LinkAuthWalkNext9 loadPtr lenW
          balPtr balLenW chainIdW regionBase (BitVec.ofNat 64 1) s7 cursorV
          endW s11 s innerVal oldCount regionBase bs balBytes)
        (fun hp =>
          ∃ (refund t0Old t1Old baiW' : Word),
            teerAuthLoopEmptyExitPack spVal spC s
              (teerAuthLoopEmptyWalkCur regionBase)
              ...
              regionBase bs balBytes balPtr hp) := by
    intro oldCount
    exact teerBridgePre_to_exitPack_empty asm spVal spC
      (spC + signExtend12 (-48 : BitVec 12)) regionBase (BitVec.ofNat 64 1) oldCount
      s0' s1' s2' s3' bs 1 LinkAuthWalkNext9 s7 v24 balPtr chainIdW baiW s
      balBytes innerVal cursorV endW s11 loadPtr lenW balLenW regionBase bs
      regionBase hoff rfl rfl rfl
      hs0 hs1 hs2 hs3 hv24 hs0s hs1s hs2s hs3s hs4 hs9 hs11 rfl
      hsalign hslack hover hvalid rfl teerLinkListCount_aligned hsuccess hspe hlen
      (by omega) (by simpa using hvalid 0 hoff)
      h_ge h_hi h_exact hrolled0
  exact cpsTripleWithin_seq_exists_same_cr hBr hmid

/-- free26 → ret empty-short under hrun + hident (oldCount free). -/
theorem teerEmptyAuth_free26_toRet_of_hrun_empty_short_ident
    {n : Nat}
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
      ...
      (fun hp =>
        ∃ (refund _t0Old _t1Old baiW' : Word),
          ((((.x10 ↦ᵣ (0 : Word)) ** (.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
            ...
            stackFree spC 6) hp) := by
  have hpack :=
    teerEmptyAuth_free26_to_exitPack_of_hrun_empty_short_ident asm ret spVal spC
      regionBase loadPtr lenW balPtr balLenW chainIdW baiW
      s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s bs balBytes hspC
      innerVal endL endW cursorV srcOffA9 s0' s1' s2' s3' v24 hoff
      hs0 hs1 hs2 hs3 hv24 hs0s hs1s hs2s hs3s hs4 hs9 hs11
      hsalign hslack hover hvalid h0 hrolled0 hrun hident
  have hexit :
      cpsTripleWithin 30 AfterAuthLoopLi ret teerLinkedField0
        (fun hp =>
          ∃ (refund t0Old t1Old baiW' : Word),
...
/-- free26 → ExitPack empty-short under hrun (srcOffA9=0) + 0xc0; hident FREE via decode.
    Residual: hrun, hrolled0, domain, ABI wire. -/
theorem teerEmptyAuth_free26_to_exitPack_of_hrun_empty_short_decode
    {n : Nat}
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
  ...
  have hlen : BitVec.ofNat 64 1 ≠ (0 : Word) := teer_listLenW_one_ne_zero rfl
  have hBr :=
    teerEmptyAuth_free26_to_bridgePre_empty_short_decode ret spVal spC regionBase
      loadPtr lenW balPtr balLenW chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
      s bs balBytes hspC innerVal endL endW cursorV hoff h0 hrun
  have hmid : ∀ (oldCount : Word),
      cpsTripleWithin (nListCountAuthLoopStart 1) AtListCount AfterAuthLoopLi
        teerLinkedField0
        (teerAuthContentBridgePre spVal spC LinkAuthWalkNext9 loadPtr lenW
...
/-- free26 → ret empty-short under hrun (srcOffA9=0) + 0xc0; hident FREE via decode. -/
theorem teerEmptyAuth_free26_toRet_of_hrun_empty_short_decode
    {n : Nat}
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
      ...
      (fun hp =>
        ∃ (refund _t0Old _t1Old baiW' : Word),
          ((((.x10 ↦ᵣ (0 : Word)) ** (.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
            ...
            stackFree spC 6) hp) := by
  have hpack :=
    teerEmptyAuth_free26_to_exitPack_of_hrun_empty_short_decode asm ret spVal spC
      regionBase loadPtr lenW balPtr balLenW chainIdW baiW
      s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s bs balBytes hspC
      innerVal endL endW cursorV s0' s1' s2' s3' v24 hoff
      hs0 hs1 hs2 hs3 hv24 hs0s hs1s hs2s hs3s hs4 hs9 hs11
      hsalign hslack hover hvalid h0 hrolled0 hrun
  have hexit :
      cpsTripleWithin 30 AfterAuthLoopLi ret teerLinkedField0
        (fun hp =>
          ∃ (refund t0Old t1Old baiW' : Word),
...
/-- free26 empty-short → applied_flat free20**nested under hrun (srcOffA9=0) + 0xc0 + zero teer.
    Residual: hrun + hrolled0 + domain + ABI wire (`hident` FREE). -/
theorem teerEmptyAuth_free26_to_applied_flat_of_hrun_empty_short_decode_zero
    {n : Nat}
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
      ...
      (fun hp =>
        ∃ (_refund _baiW' : Word),
          (((.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
            ...
            stackFree spC 6) hp) := by
  have hret' :=
    teerEmptyAuth_free26_toRet_of_hrun_empty_short_decode asm ret spVal spC regionBase
      loadPtr lenW balPtr balLenW chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
      s bs balBytes hspC innerVal endL endW cursorV
      s0' s1' s2' s3' v24 hoff hs0 hs1 hs2 hs3 hv24 hs0s hs1s hs2s hs3s hs4 hs9 hs11
      hsalign hslack hover hvalid h0 hrolled0 hret hra hrun
  refine cpsTripleWithin_weaken (fun _ hp => hp) ?_ hret'
  intro h hq
  obtain ⟨refund, _t0, _t1, baiW', hp0⟩ := hq
  have hbody :
...
#print axioms teerEmptyAuth_free26_to_exitPack_of_hrun_empty_short_decode
#print axioms teerEmptyAuth_free26_toRet_of_hrun_empty_short_decode
#print axioms teerEmptyAuth_free26_to_applied_flat_of_hrun_empty_short_decode_zero

/-! ## Decode path under `TeerRolledZeroAssumed` (packages open hrolled0) -/

/-- free26 → ExitPack empty-short decode under `TeerRolledZeroAssumed`.
    Residual: hrun + rz + domain fixture + ABI wire (`hident`/`hrolled0` packaged). -/
theorem teerEmptyAuth_free26_to_exitPack_of_hrun_empty_short_decode_rz
    {n : Nat}
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (rz : TeerRolledZeroAssumed)
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
  ...
  have hrolled0 :
      ∀ (refund rolledVal : Word) h,
        ...
        rolledVal = (0 : Word) := by
    intro refund rolledVal h hp
    exact rz.holds spVal spC regionBase (BitVec.ofNat 64 1) s0' s1' s2' s3'
      bs balPtr chainIdW baiW s balBytes innerVal cursorV endW s11
      refund rolledVal h hp
  exact teerEmptyAuth_free26_to_exitPack_of_hrun_empty_short_decode asm ret spVal spC
    regionBase loadPtr lenW balPtr balLenW chainIdW baiW
    s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s bs balBytes hspC
    innerVal endL endW cursorV s0' s1' s2' s3' v24 hoff
    hs0 hs1 hs2 hs3 hv24 hs0s hs1s hs2s hs3s hs4 hs9 hs11
...
/-- free26 → ret empty-short decode under `TeerRolledZeroAssumed`. -/
theorem teerEmptyAuth_free26_toRet_of_hrun_empty_short_decode_rz
    {n : Nat}
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (rz : TeerRolledZeroAssumed)
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
  ...
  have hrolled0 :
    ...
    exact rz.holds spVal spC regionBase (BitVec.ofNat 64 1) s0' s1' s2' s3'
      bs balPtr chainIdW baiW s balBytes innerVal cursorV endW s11
      refund rolledVal h hp
  exact teerEmptyAuth_free26_toRet_of_hrun_empty_short_decode asm ret spVal spC
    regionBase loadPtr lenW balPtr balLenW chainIdW baiW
    s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s bs balBytes hspC
    innerVal endL endW cursorV s0' s1' s2' s3' v24 hoff
    hs0 hs1 hs2 hs3 hv24 hs0s hs1s hs2s hs3s hs4 hs9 hs11
...
/-- free26 → applied_flat free20**nested empty-short decode under rz + zero teer.
    Residual: hrun + rz + domain + ABI wire. -/
theorem teerEmptyAuth_free26_to_applied_flat_of_hrun_empty_short_decode_rz_zero
    {n : Nat}
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (rz : TeerRolledZeroAssumed)
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
  ...
  have hrolled0 :
    ...
    exact rz.holds spVal spC regionBase (BitVec.ofNat 64 1) s0' s1' s2' s3'
      bs balPtr chainIdW baiW s balBytes innerVal cursorV endW s11
      refund rolledVal h hp
  exact teerEmptyAuth_free26_to_applied_flat_of_hrun_empty_short_decode_zero asm
    ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW
    s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s bs balBytes off len chainId bai hspC
    innerVal endL endW cursorV s0' s1' s2' s3' v24 hoff
    hs0 hs1 hs2 hs3 hv24 hs0s hs1s hs2s hs3s hs4 hs9 hs11
    hsalign hslack hover hvalid h0 hbound hrolled0 hret hra hrun

#print axioms teerEmptyAuth_free26_to_exitPack_of_hrun_empty_short_decode_rz
#print axioms teerEmptyAuth_free26_toRet_of_hrun_empty_short_decode_rz
#print axioms teerEmptyAuth_free26_to_applied_flat_of_hrun_empty_short_decode_rz_zero

/-! ## Residual hrun packaging (empty-short PostEx at srcOffA9=0)

Wire `teerAuthContent_applied_as_postEx` (or any free20→PostEx@0) as a named
hyp. Decode_rz path then residual inhabit is only hrunA + rz + domain. -/

/-- free20 applied prest → PostEx with `srcOffA9 = 0` (empty-short identity fixture).
    Residual inhabit: supply `teerAuthContent_applied_as_postEx` with srcOffA9=0. -/
structure TeerEmptyAuthHrunEmptyShort where
  nSteps : Nat
  run :
    ∀ (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
      (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
      ...
      (bs balBytes : List (BitVec 8))
      (innerVal endL endW cursorV : Word),
      cpsTripleWithin nSteps E AtListCount teerLinkedCount
        ((.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
          stackFree spVal nTeerStackDwords **
          (.x8 ↦ᵣ s0) ** (.x9 ↦ᵣ s1) **
          (.x18 ↦ᵣ s2) ** (.x19 ↦ᵣ s3) ** (.x20 ↦ᵣ s4) **
...
/-- Fill hrunA from a concrete free20→PostEx@0 triple family (e.g. applied_as_postEx). -/
def teerEmptyAuthHrunEmptyShort_of
    (nSteps : Nat)
    (run :
      ∀ (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
        (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
        ...
        cpsTripleWithin nSteps E AtListCount teerLinkedCount
          ...
          (teerAuthContentAppliedPostEx spVal spC loadPtr lenW balPtr balLenW chainIdW
            s7 cursorV endW s11 s innerVal endL regionBase bs balBytes 0)) :
    TeerEmptyAuthHrunEmptyShort where
  nSteps := nSteps
  run := run

/-- free26 → ExitPack empty-short under residual hrunA + rz (hident FREE, hrolled0 packaged). -/
theorem teerEmptyAuth_free26_to_exitPack_of_hrunA_empty_short_decode_rz
    ...
    (hvalid : ∀ k, k < bs.length →
      isValidByteAccess (regionBase + BitVec.ofNat 64 k) = true)
    (h0 : bs[0]'hoff = (0xc0 : BitVec 8)) :
    cpsTripleWithin (hrunA.nSteps + nListCountAuthLoopStart 1) E AfterAuthLoopLi
      teerLinkedField0
      (stackFree spVal nTeerStackWithListCount **
        teerAuthContentAppliedEntryRest ret spVal loadPtr lenW balPtr balLenW
          chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
      ...
      (fun hp =>
        ∃ (refund t0Old t1Old baiW' : Word),
          teerAuthLoopEmptyExitPack spVal spC s
            ...
            t0Old t1Old baiW'
            regionBase bs balBytes balPtr hp) :=
  teerEmptyAuth_free26_to_exitPack_of_hrun_empty_short_decode_rz asm rz
    ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW
    s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s bs balBytes hspC
    innerVal endL endW cursorV s0' s1' s2' s3' v24 hoff
    hs0 hs1 hs2 hs3 hv24 hs0s hs1s hs2s hs3s hs4 hs9 hs11
    hsalign hslack hover hvalid h0
    (hrunA.run ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW
      s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s bs balBytes
      innerVal endL endW cursorV)

/-- free26 → ret empty-short under residual hrunA + rz. -/
theorem teerEmptyAuth_free26_toRet_of_hrunA_empty_short_decode_rz
    (hrunA : TeerEmptyAuthHrunEmptyShort)
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (rz : TeerRolledZeroAssumed)
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    ...
    (hret : (ret &&& ~~~(1 : Word)) = ret)
    (hra : s.ra = ret) :
    cpsTripleWithin (hrunA.nSteps + nListCountAuthLoopStart 1 + 30) E ret
      teerLinkedField0
      (stackFree spVal nTeerStackWithListCount **
        teerAuthContentAppliedEntryRest ret spVal loadPtr lenW balPtr balLenW
          chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
      ...
      (fun hp =>
        ∃ (refund _t0Old _t1Old baiW' : Word),
          ((((.x10 ↦ᵣ (0 : Word)) ** (.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
              ...
              teerEmptyAuthExitFrame baiW' spVal spC regionBase bs balBytes balPtr) **
            stackFree spC 6) hp) :=
  teerEmptyAuth_free26_toRet_of_hrun_empty_short_decode_rz asm rz
    ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW
    s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s bs balBytes hspC
    innerVal endL endW cursorV s0' s1' s2' s3' v24 hoff
    hs0 hs1 hs2 hs3 hv24 hs0s hs1s hs2s hs3s hs4 hs9 hs11
    hsalign hslack hover hvalid h0 hret hra
    (hrunA.run ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW
      s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s bs balBytes
      innerVal endL endW cursorV)

/-- free26 → applied_flat free20**nested under residual hrunA + rz + zero teer. -/
theorem teerEmptyAuth_free26_to_applied_flat_of_hrunA_empty_short_decode_rz_zero
    (hrunA : TeerEmptyAuthHrunEmptyShort)
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (rz : TeerRolledZeroAssumed)
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
      ...
      (fun hp =>
        ∃ (_refund _baiW' : Word),
          (((.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
              ...
              regOwn .x31 ** (.x0 ↦ᵣ (0 : Word))) **
            stackFree spC 6) hp) :=
  teerEmptyAuth_free26_to_applied_flat_of_hrun_empty_short_decode_rz_zero asm rz
    ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW
    s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s bs balBytes off len chainId bai hspC
    innerVal endL endW cursorV s0' s1' s2' s3' v24 hoff
    hs0 hs1 hs2 hs3 hv24 hs0s hs1s hs2s hs3s hs4 hs9 hs11
    hsalign hslack hover hvalid h0 hbound hret hra
    (hrunA.run ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW
      s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s bs balBytes
      innerVal endL endW cursorV)

#print axioms teerEmptyAuth_free26_to_exitPack_of_hrunA_empty_short_decode_rz
...
/-- AmbientPostIs → ret. No hrolled0 (ZeroIs pins rolled). -/
theorem teerAuthLoopEmptyAmbientPostIs_toRet
    (spVal spC listBase listLenW s0 s1 s2 s3 : Word)
    (bytes : List (BitVec 8))
    (balPtr chainIdW baiW : Word)
    (s : TeerSaved) (balBytes : List (BitVec 8))
  ...
  obtain ⟨h0, hcompat, h1, h2, hd, hu, hAmb, hR2⟩ := hPR
  have hSrc :=
    teerAuthLoopEmptyAmbientPostIs_to_source spVal spC listBase listLenW
      s0 s1 s2 s3 bytes balPtr chainIdW baiW s balBytes innerVal cursorV
      endW s11 h1 hAmb
  have hrun :=
    teerAuthLoopEmptySource_toRet spVal spC listBase listLenW s0 s1 s2 s3 bytes
      balPtr chainIdW s balBytes innerVal endW s11 (0 : Word) regionBase bs
      hbase hbytes hs0 hs1 hs2 hs3 hs4 hs9 hs11 hspC hret
  have hPR' :
      ((teerAuthLoopEmptySource spVal spC listBase listLenW s0 s1 s2 s3 bytes
          balPtr chainIdW s balBytes innerVal endW s11 (0 : Word) ** R)).holdsFor st :=
    ⟨h0, hcompat, h1, h2, hd, hu, hSrc, hR2⟩
  obtain ⟨k, hk, st', hexec, hpc', hQ⟩ :=
    hrun R hR st hcr hPR' hpc
  refine ⟨k, hk, st', hexec, hpc', ?_⟩
  obtain ⟨h0', hcompat', h1', h2', hd', hu', ⟨t0, t1, bai', hRet⟩, hR'⟩ := hQ
  exact ⟨h0', hcompat', h1', h2', hd', hu', ⟨(0 : Word), t0, t1, bai', hRet⟩, hR'⟩

#print axioms teerAuthLoopEmptyAmbientPostIs_toRet



/-- AmbientPostIs → ExitPack. No hrolled0. -/
theorem teerAuthLoopEmptyAmbientPostIs_to_exitPack
    (spVal spC listBase listLenW s0 s1 s2 s3 : Word)
    (bytes : List (BitVec 8))
    (balPtr chainIdW baiW : Word)
    (s : TeerSaved) (balBytes : List (BitVec 8))
  ...
  intro h hp
  have hSrc :=
    teerAuthLoopEmptyAmbientPostIs_to_source spVal spC listBase listLenW s0 s1 s2 s3
      bytes balPtr chainIdW baiW s balBytes innerVal cursorV endW s11 h hp
  obtain ⟨t0Old, t1Old, baiW', hPack⟩ :=
    teerAuthLoopEmpty_to_exitPack spVal spC listBase listLenW s0 s1 s2 s3 bytes
      balPtr chainIdW s balBytes innerVal endW s11 (0 : Word) regionBase bs
      hbase hbytes hs0 hs1 hs2 hs3 hs4 hs9 hs11 h hSrc
  exact ⟨(0 : Word), t0Old, t1Old, baiW', hPack⟩

/-- BridgePreIs → ExitPack empty-count. No hrolled0 / TeerRolledZeroAssumed. -/
theorem teerBridgePre_to_exitPack_empty_is
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (spVal spC newSp listBase listLenW oldCount s0 s1 s2 s3 : Word)
    (bytes : List (BitVec 8)) (listLen : Nat)
    (old1 s7Old v24 : Word)
      ...
      (fun hp =>
        ∃ (refund t0Old t1Old baiW' : Word),
          teerAuthLoopEmptyExitPack spVal spC s
            ...
            regionBase bs balBytes balPtr hp) := by
  have hmid :=
    teerBridgePre_to_AfterAuthLoopLi_empty_is asm spVal spC newSp listBase listLenW
      oldCount s0 s1 s2 s3 bytes listLen old1 s7Old v24 balPtr chainIdW baiW s
      balBytes innerVal cursorV endW s11 loadPtr lenW balLenW regionBase bs content
      hoff hbase hbytes hcontent hs0 hs1 hs2 hs3 hv24 hlistLenW hsalign hslack
      hover hvalid hnewSp hret hsuccess hspe hlen hoverOff hvalidOff h_ge h_hi h_exact
  have hmidF :
      cpsTripleWithin (nListCountAuthLoopStart listLen) AtListCount AfterAuthLoopLi
        ...
        (teerAuthLoopEmptyAmbientPostIs spVal spC listBase listLenW s0 s1 s2 s3 bytes
          balPtr chainIdW baiW s balBytes innerVal cursorV endW s11) := by
    have h := cpsTripleWithin_extend_code teerField0_mono_count hmid
    exact cpsTripleWithin_weaken (fun _ hp => hp)
      (fun _ hq => by
        dsimp only [teerAuthLoopEmptyAmbientPostIs]
        exact hq) h
  exact cpsTripleWithin_weaken (fun _ hp => hp)
    (fun h hq =>
      teerAuthLoopEmptyAmbientPostIs_to_exitPack spVal spC listBase listLenW
        s0 s1 s2 s3 bytes balPtr chainIdW baiW s balBytes innerVal cursorV
        endW s11 regionBase bs hbase hbytes hs0s hs1s hs2s hs3s hs4 hs9 hs11
        h hq) hmidF

#print axioms teerAuthLoopEmptyAmbientPostIs_to_exitPack
#print axioms teerBridgePre_to_exitPack_empty_is

/-! ## free26 empty-short Is path (no TeerRolledZeroAssumed / hrolled0) -/

/-- AppliedPostEx with ZeroIs ** RestWithoutVnz (matches AuthContent applied_is). -/
def teerAuthContentAppliedPostExIs
    (spVal spC loadPtr lenW balPtr balLenW chainIdW : Word)
    (s7Old cursorV endW s11 : Word)
    (s : TeerSaved) (innerVal endL : Word)
    (regionBase : Word) (bs balBytes : List (BitVec 8))
  ...
  fun h =>
    ∃ next lenK : Word,
      (((.x2 ↦ᵣ spC) **
          ...
          memOwn RecipientPtrAddr ** memOwn RecipientLenAddr **
          memOwn ValueNonzeroAddr **
          teerScratchZeroIs ** teerAuthContentRestWithoutVnzOwn) **
        ⌜rlpItemDecode bs srcOffA9 (regionBase + BitVec.ofNat 64 srcOffA9)
          endL next lenK⌝) h

/-- Entry rest matching applied_is prest (Own scratch; ZeroIs appears in post only). -/
def teerAuthContentAppliedEntryRestIs
...
/-- Nested free ** PostExIs (srcOff=0, 0xc0) → ∃ oldCount BridgePreIs regionBase 1. -/
theorem teerAuthContentNestedPostExIs_to_bridgePreIs_empty_short
    (spVal spC loadPtr lenW balPtr balLenW chainIdW : Word)
    (s7Old cursorV endW s11 : Word)
    (s : TeerSaved) (innerVal endL : Word)
    (regionBase : Word) (bs balBytes : List (BitVec 8))
    ...
    ∀ h,
      (stackFree spC 6 **
        teerAuthContentAppliedPostExIs spVal spC loadPtr lenW balPtr balLenW chainIdW
          s7Old cursorV endW s11 s innerVal endL regionBase bs balBytes 0) h →
      ∃ (oldCount : Word),
        teerAuthContentBridgePreIs spVal spC LinkAuthWalkNext9 loadPtr lenW
          balPtr balLenW chainIdW regionBase (BitVec.ofNat 64 1) s7Old cursorV
          endW s11 s innerVal oldCount regionBase bs balBytes h := by
  intro h hp
  unfold teerAuthContentAppliedPostExIs at hp
  have hpEx :=
    teerNested_sepConj_exists_pair
      (A := fun next lenK =>
        (((.x2 ↦ᵣ spC) **
  ...
  have hNestAtoms :
      (stackFree spC 6 **
        teerAuthContentAppliedFlatVnzIs spVal spC LinkAuthWalkNext9 loadPtr lenW
          balPtr balLenW chainIdW (next - lenK) lenK s7Old cursorV endW s11 s
          innerVal regionBase bs balBytes) h := by
    dsimp only [teerAuthContentAppliedFlatVnzIs]
    refine ⟨h1, h2, hd, hu, hnest, ?_⟩
    simpa [RecipientPtrAddr, RecipientLenAddr, ValueNonzeroAddr,
      TypeAddr, InnerOffAddr] using hAtoms2
  have hNest' :
      (stackFree spC 6 **
        teerAuthContentAppliedFlatVnzIs spVal spC LinkAuthWalkNext9 loadPtr lenW
          balPtr balLenW chainIdW regionBase (BitVec.ofNat 64 1) s7Old cursorV
          endW s11 s innerVal regionBase bs balBytes) h := by
    have h1' :
        (stackFree spC 6 **
          teerAuthContentAppliedFlatVnzIs spVal spC LinkAuthWalkNext9 loadPtr lenW
            balPtr balLenW chainIdW regionBase lenK s7Old cursorV endW s11 s
            innerVal regionBase bs balBytes) h := by
      dsimp only [teerAuthContentAppliedFlatVnzIs] at hNestAtoms ⊢
      simpa [hc] using hNestAtoms
    dsimp only [teerAuthContentAppliedFlatVnzIs] at h1' ⊢
    simpa [hl] using h1'
  exact teerAuthContentAppliedFlatVnzIs_nested_to_bridgePreIs
    spVal spC LinkAuthWalkNext9 loadPtr lenW balPtr balLenW chainIdW
    regionBase (BitVec.ofNat 64 1) s7Old cursorV endW s11 s innerVal
    regionBase bs balBytes h hNest'

/-- BridgePreIs mid → ExitPack empty-short Is (no hrolled0 / RolledZero). -/
theorem teerEmptyAuth_bridgePreIs_to_exitPack_empty_short
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s : TeerSaved)
    (bs balBytes : List (BitVec 8))
    (innerVal endW cursorV s7 s11 : Word)
    (s0' s1' s2' s3' v24 : Word)
    (hoff : (0 : Nat) < bs.length)
    (hs0 : s0' = loadPtr) (hs1 : s1' = lenW) (hs2 : s2' = balPtr) (hs3 : s3' = balLenW)
    (hv24 : v24 = cursorV)
  ...
  have hspe := teerListCountResultSpecialize_empty_short bs regionBase hspeHover
  have hlen : BitVec.ofNat 64 1 ≠ (0 : Word) := teer_listLenW_one_ne_zero rfl
  exact teerBridgePre_to_exitPack_empty_is asm spVal spC
    (spC + signExtend12 (-48 : BitVec 12)) regionBase (BitVec.ofNat 64 1) oldCount
    s0' s1' s2' s3' bs 1 LinkAuthWalkNext9 s7 v24 balPtr chainIdW baiW s
    balBytes innerVal cursorV endW s11 loadPtr lenW balLenW regionBase bs
    regionBase hoff rfl rfl rfl
...
#print axioms teerAuthContentNestedPostExIs_to_bridgePreIs_empty_short
#print axioms teerEmptyAuth_bridgePreIs_to_exitPack_empty_short

/-- free20 ** EntryRestIs ↔ applied Is prest (xperm). -/
theorem teerAuthContentAppliedEntry_split_is
    (ret spVal loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
    (regionBase : Word) (bs balBytes : List (BitVec 8)) :
    ∀ h,
  ...
  constructor <;> intro hp <;>
    (dsimp only [teerAuthContentAppliedEntryRestIs] at *; xperm_hyp hp)

/-- Entry free26 ** EntryRestIs → nested ** applied Is prest. -/
theorem teerAuthContentEntry_free26_to_nested_applied_is
  ...
  have hApp :=
    (teerAuthContentAppliedEntry_split_is ret spVal loadPtr lenW balPtr balLenW
      chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
      regionBase bs balBytes h2).2 hmid
  exact ⟨h1, h2, hd, hu, hnest, hApp⟩

/-- free26 entry + Is-path hrun → nested post. -/
theorem teerAuthContent_applied_nestedFree_of_run_is
    {n : Nat} {cr : CodeReq}
    (ret spVal spC loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
    (regionBase : Word) (bs balBytes : List (BitVec 8))
  ...
  exact cpsTripleWithin_weaken
    (teerAuthContentEntry_free26_to_nested_applied_is ret spVal spC loadPtr lenW
      balPtr balLenW chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
      regionBase bs balBytes hspC)
    (fun _ hq => hq) hF

/-- free26 + Is-path hrun (srcOffA9=0) + 0xc0 → ∃ oldCount BridgePreIs regionBase 1.
    Residual: hrun only (`hident` FREE via decode; `hrolled0` FREE via ZeroIs). -/
theorem teerEmptyAuth_free26_to_bridgePreIs_empty_short_decode
    {n : Nat}
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
    (s : TeerSaved)
      ...
      (fun h =>
        ∃ (oldCount : Word),
          teerAuthContentBridgePreIs spVal spC LinkAuthWalkNext9 loadPtr lenW
            ...
            endW s11 s innerVal oldCount regionBase bs balBytes h) := by
  have hNest :=
    teerAuthContent_applied_nestedFree_of_run_is ret spVal spC loadPtr lenW
      balPtr balLenW chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
      regionBase bs balBytes hspC
      (teerAuthContentAppliedPostExIs spVal spC loadPtr lenW balPtr balLenW chainIdW
        s7 cursorV endW s11 s innerVal endL regionBase bs balBytes 0)
  ...
  have hBr :=
    cpsTripleWithin_weaken (fun _ hp => hp)
      (teerAuthContentNestedPostExIs_to_bridgePreIs_empty_short spVal spC loadPtr lenW
        balPtr balLenW chainIdW s7 cursorV endW s11 s innerVal endL regionBase
        bs balBytes hoff h0)
      hNest
  exact cpsTripleWithin_extend_code teerField0_mono_count hBr

/-- free26 → ExitPack empty-short Is-path under hrun; no hrolled0/RolledZero. -/
theorem teerEmptyAuth_free26_to_exitPack_of_hrun_empty_short_decode_is
    {n : Nat}
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
    (s : TeerSaved)
    ...
    (h0 : bs[0]'hoff = (0xc0 : BitVec 8))
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (hs0 : s0 = loadPtr) (hs1 : s1 = lenW)
    (hs2 : s2 = balPtr) (hs3 : s3 = balLenW)
    (hs0s : s0 = s.s0) (hs1s : s1 = s.s1)
    (hs2s : s2 = s.s2) (hs3s : s3 = s.s3)
    (hs4 : chainIdW = s.s4) (hs9 : endW = s.s9) (hs11 : s11 = s.s11)
    (hv24 : s8 = cursorV)
    (hsalign : regionBase.toNat % 8 = 0)
    (hslack : 1 + 9 ≤ bs.length)
    ...
    (hrun : cpsTripleWithin n E AtListCount teerLinkedCount
      ...
      (teerAuthContentAppliedPostExIs spVal spC loadPtr lenW balPtr balLenW chainIdW
        s7 cursorV endW s11 s innerVal endL regionBase bs balBytes 0)) :
    cpsTripleWithin (n + nListCountAuthLoopStart 1)
      ...
      (fun h =>
        ∃ (refund t0Old t1Old baiW' : Word),
          teerAuthLoopEmptyExitPack spVal spC s
            ...
            regionBase bs balBytes balPtr h) := by
  have hBr :=
    teerEmptyAuth_free26_to_bridgePreIs_empty_short_decode ret spVal spC
      regionBase loadPtr lenW balPtr balLenW chainIdW baiW
      s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s bs balBytes hspC
      innerVal endL endW cursorV hoff h0 hrun
  have hMid :=
    teerEmptyAuth_bridgePreIs_to_exitPack_empty_short asm spVal spC
      regionBase loadPtr lenW balPtr balLenW chainIdW baiW s
      bs balBytes innerVal endW cursorV s7 s11
      s0 s1 s2 s3 s8
      hoff hs0 hs1 hs2 hs3 hv24 hs0s hs1s hs2s hs3s hs4 hs9 hs11
      hsalign hslack hover hvalid h0
  exact cpsTripleWithin_seq_exists_same_cr hBr hMid

#print axioms teerAuthContentAppliedEntry_split_is
#print axioms teerAuthContentEntry_free26_to_nested_applied_is
#print axioms teerAuthContent_applied_nestedFree_of_run_is
#print axioms teerEmptyAuth_free26_to_bridgePreIs_empty_short_decode
#print axioms teerEmptyAuth_free26_to_exitPack_of_hrun_empty_short_decode_is

/-- free20 Own prest → PostExIs @ srcOffA9=0 (Is-path empty-short).
    Residual inhabit: `teerAuthContent_applied_as_postEx_is` with srcOffA9=0. -/
structure TeerEmptyAuthHrunEmptyShortIs where
  nSteps : Nat
  run :
    ∀ (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
      (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
...
def teerEmptyAuthHrunEmptyShortIs_of
    (nSteps : Nat)
    (run :
      ∀ (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
        (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
        ...
        cpsTripleWithin nSteps E AtListCount teerLinkedCount
          ...
          (teerAuthContentAppliedPostExIs spVal spC loadPtr lenW balPtr balLenW chainIdW
            s7 cursorV endW s11 s innerVal endL regionBase bs balBytes 0)) :
    TeerEmptyAuthHrunEmptyShortIs where
  nSteps := nSteps
  run := run

/-- free26 → ExitPack empty-short Is under residual hrunA (hident+hrolled0 FREE). -/
theorem teerEmptyAuth_free26_to_exitPack_of_hrunA_empty_short_decode_is
      ...
      (fun h =>
        ∃ (refund t0Old t1Old baiW' : Word),
          teerAuthLoopEmptyExitPack spVal spC s
            ...
            t0Old t1Old baiW'
            regionBase bs balBytes balPtr h) :=
  teerEmptyAuth_free26_to_exitPack_of_hrun_empty_short_decode_is
    ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW
    s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s bs balBytes hspC
    innerVal endL endW cursorV hoff h0 asm
    hs0 hs1 hs2 hs3 hs0s hs1s hs2s hs3s hs4 hs9 hs11 hv24
    hsalign hslack hover hvalid
    (hrunA.run ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW
      s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s bs balBytes
      innerVal endL endW cursorV)

#print axioms teerEmptyAuth_free26_to_exitPack_of_hrunA_empty_short_decode_is

/-- free26 → ret empty-short Is-path under hrun; no hrolled0. -/
theorem teerEmptyAuth_free26_toRet_of_hrun_empty_short_decode_is
    {n : Nat}
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
    (s : TeerSaved)
      ...
      (fun hp =>
        ∃ (refund _t0Old _t1Old baiW' : Word),
          ((((.x10 ↦ᵣ (0 : Word)) ** (.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
            ...
            stackFree spC 6) hp) := by
  have hpack :=
    teerEmptyAuth_free26_to_exitPack_of_hrun_empty_short_decode_is ret spVal spC
      regionBase loadPtr lenW balPtr balLenW chainIdW baiW
      s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s bs balBytes hspC
      innerVal endL endW cursorV hoff h0 asm
      hs0 hs1 hs2 hs3 hs0s hs1s hs2s hs3s hs4 hs9 hs11 hv24
      hsalign hslack hover hvalid hrun
  have hexit :
      cpsTripleWithin 30 AfterAuthLoopLi ret teerLinkedField0
        (fun hp =>
          ∃ (refund t0Old t1Old baiW' : Word),
...
/-- free26 → applied_flat free20**nested empty-short Is under hrun + zero teer; no hrolled0. -/
theorem teerEmptyAuth_free26_to_applied_flat_of_hrun_empty_short_decode_is_zero
    {n : Nat}
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
    (s : TeerSaved)
      ...
      (fun hp =>
        ∃ (_refund _baiW' : Word),
          (((.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
            ...
            stackFree spC 6) hp) := by
  have hret' :=
    teerEmptyAuth_free26_toRet_of_hrun_empty_short_decode_is ret spVal spC regionBase
      loadPtr lenW balPtr balLenW chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
      s bs balBytes hspC innerVal endL endW cursorV hoff h0 asm
      hs0 hs1 hs2 hs3 hs0s hs1s hs2s hs3s hs4 hs9 hs11 hv24
      hsalign hslack hover hvalid hret hra hrun
  refine cpsTripleWithin_weaken (fun _ hp => hp) ?_ hret'
  intro h hq
  obtain ⟨refund, _t0, _t1, baiW', hp0⟩ := hq
  have hbody :
...
/-- free26 → ret under residual hrunA Is (hrolled0 FREE). -/
theorem teerEmptyAuth_free26_toRet_of_hrunA_empty_short_decode_is
    (hrunA : TeerEmptyAuthHrunEmptyShortIs)
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
      ...
      (fun hp =>
        ∃ (refund _t0Old _t1Old baiW' : Word),
          ((((.x10 ↦ᵣ (0 : Word)) ** (.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
              ...
              teerEmptyAuthExitFrame baiW' spVal spC regionBase bs balBytes balPtr) **
            stackFree spC 6) hp) :=
  teerEmptyAuth_free26_toRet_of_hrun_empty_short_decode_is ret spVal spC
    regionBase loadPtr lenW balPtr balLenW chainIdW baiW
    s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s bs balBytes hspC
    innerVal endL endW cursorV hoff h0 asm
    hs0 hs1 hs2 hs3 hs0s hs1s hs2s hs3s hs4 hs9 hs11 hv24
    hsalign hslack hover hvalid hret hra
    (hrunA.run ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW
      s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s bs balBytes
      innerVal endL endW cursorV)

/-- free26 → applied_flat under residual hrunA Is + zero teer (hrolled0 FREE). -/
theorem teerEmptyAuth_free26_to_applied_flat_of_hrunA_empty_short_decode_is_zero
    (hrunA : TeerEmptyAuthHrunEmptyShortIs)
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
      ...
      (fun hp =>
        ∃ (_refund _baiW' : Word),
          (((.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
              ...
              regOwn .x31 ** (.x0 ↦ᵣ (0 : Word))) **
            stackFree spC 6) hp) :=
  teerEmptyAuth_free26_to_applied_flat_of_hrun_empty_short_decode_is_zero ret spVal spC
    regionBase loadPtr lenW balPtr balLenW chainIdW baiW
    s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s bs balBytes off len chainId bai hspC
    innerVal endL endW cursorV hoff h0 asm
    hs0 hs1 hs2 hs3 hs0s hs1s hs2s hs3s hs4 hs9 hs11 hv24
    hsalign hslack hover hvalid hbound hret hra
    (hrunA.run ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW
      s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s bs balBytes
      innerVal endL endW cursorV)

/-! ## Free26EmptyShort from hrunA Is (no RolledZero, no FrontToBridge)

`content` ghost = `regionBase` at each `run` (identity empty-short).
Residual inhabit: `TeerEmptyAuthHrunEmptyShortIs` (applied_as_postEx_is @ srcOffA9=0).
Domain fixture stays on `run`. -/

/-- Fill Free26EmptyShort from hrunA Is via decode_is ExitPack (hident+hrolled0 FREE). -/
def teerFrontToAuthLoopAssumedFree26EmptyShort_of_hrunA_is
    (hrunA : TeerEmptyAuthHrunEmptyShortIs)
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (content : Word)
    (hn : hrunA.nSteps + nListCountAuthLoopStart 1 + 30 ≤ nTeerSteps) :
  ...
  run := fun ret spVal spC regionBase loadPtr lenW balPtr balLenW chainIdW baiW
      ...
      hsalign hslack hover hvalid h0 => by
    have hpack :=
      teerEmptyAuth_free26_to_exitPack_of_hrunA_empty_short_decode_is
        hrunA asm ret spVal spC regionBase loadPtr lenW balPtr balLenW
        chainIdW baiW
        s.s0 s.s1 s.s2 s.s3 s.s4 s.s5 s.s6 s.s7 s.s8 s.s9 s.s10 s.s11
        s bs balBytes hspC
        innerVal endW endW s.s8 hoff0 h0
        hs0w.symm hs1w.symm hs2w.symm hs3w.symm
        rfl rfl rfl rfl
        hs4w hs9w rfl rfl
        hsalign hslack hover hvalid
    refine cpsTripleWithin_weaken ?_ (fun _ hq => by simpa [hc] using hq) hpack
    ...
    have hp' :
        (stackFree spVal nTeerStackWithListCount **
          teerAuthContentAppliedEntryRestIs ret spVal loadPtr lenW balPtr balLenW
            chainIdW baiW s.s0 s.s1 s.s2 s.s3 s.s4 s.s5 s.s6 s.s7 s.s8 s.s9
            s.s10 s.s11 regionBase bs balBytes) h := by
      unfold teerAuthContentAppliedEntryRestIs
      xperm_hyp hp
    exact hp'

/-- `hn` free when hrunA uses applied AuthContent step count. -/
def teerFrontToAuthLoopAssumedFree26EmptyShort_of_hrunA_is_applied
    (hrunA : TeerEmptyAuthHrunEmptyShortIs)
    (asm : TeerListCountAuthLoopAssumed teerLinkedCount)
    (content : Word)
    (hnSteps : hrunA.nSteps = nFrontToAtListCount) :
    TeerFrontToAuthLoopAssumedFree26EmptyShort :=
  teerFrontToAuthLoopAssumedFree26EmptyShort_of_hrunA_is hrunA asm content (by
    simpa [hnSteps] using teer_nFree26Empty_applied_budget)

#print axioms teerEmptyAuth_free26_toRet_of_hrun_empty_short_decode_is
#print axioms teerEmptyAuth_free26_to_applied_flat_of_hrun_empty_short_decode_is_zero
#print axioms teerEmptyAuth_free26_toRet_of_hrunA_empty_short_decode_is
#print axioms teerEmptyAuth_free26_to_applied_flat_of_hrunA_empty_short_decode_is_zero
