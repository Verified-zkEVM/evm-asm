/-
  Ambient t1 MidJoin: AfterSave → creation under pure decode (split loadPtr/regionBase, 5 walks).
-/
...
theorem t14Exists_to_creation_ambient
  ...
  have h := extractT1HaveFieldCreation_then_epi_ambient sp0 spC s loadPtr
    regionBase lenW innerW endPtr next4 toBuf isCreationPtr s7 bs absOff4 hspC hret
  refine cpsTripleWithin_weaken (fun hst hp => by
    obtain ⟨h1, h2, hd, hu, hOkC, hM⟩ := hp
    obtain ⟨hRegs, hdec⟩ := (sepConj_pure_right h1).mp (by
      simpa only [t14OkConcreteAmbient] using hOkC)
    have hlen : len4 = (0 : Word) := hcre next4 len4 hdec
    have hOk0 : t14OkConcreteAmbient loadPtr regionBase lenW innerW endPtr next4 (0 : Word)
        bs absOff4 h1 := by
      simp only [t14OkConcreteAmbient, hlen] at hRegs hdec ⊢
...
theorem extractT1AfterSaveCreation_then_epi_of_decode_ambient
    (sp0 spC : Word) (s : ExtractSaved)
  ...
  have hC := t14Exists_to_creation_ambient sp0 spC s loadPtr regionBase lenW innerW
    endPtr toBuf isCreationPtr s7 bs absOff4 hspC hret hcre
  exact cpsTripleWithin_seq_same_cr hW hC

#print axioms t14Exists_to_creation_ambient
#print axioms extractT1AfterSaveCreation_then_epi_of_decode_ambient
