/-
  Ambient dual: Front long AfterSave → 20B copy bare region of_decode
  (split loadPtr/regionBase; content via region partition).
-/
...
theorem extractFrontAfterSaveCopy_then_epi_of_decode_long_region_ambient
    ...
    (hwi_off : ambientAbsOff off (teerTxTypeDispatch (txSlice bs off len)).2.2.toNat <
      bs.length)
    (hspC : spC = sp0 + signExtend12 (-80 : BitVec 12))
    (hret : s.ra &&& ~~~(1 : Word) = s.ra)
    (hcur : longWalkCursorAmbient regionBase bs
        (ambientAbsOff off (teerTxTypeDispatch (txSlice bs off len)).2.2.toNat) hwi_off =
    ...
    (hlen20 : ∀ (next5 len5 : Word),
      rlpItemDecode bs absOff5 (regionBase + BitVec.ofNat 64 absOff5)
        (longWalkEndAmbient regionBase (lenW - (teerTxTypeDispatch (txSlice bs off len)).2.2)
          (ambientAbsOff off (teerTxTypeDispatch (txSlice bs off len)).2.2.toNat)) next5 len5 → len5 = (20 : Word))
    ...
    cpsTripleWithin
      ...
      (frontAfterSavePostLongAmbient spC s regionBase loadPtr lenW toBuf isCreationPtr bs off len hwi_off)
      ((.x1 ↦ᵣ s.ra) ** (.x2 ↦ᵣ sp0) **
  ...
  intro contentPtr w2
  let listOff := ambientAbsOff off (teerTxTypeDispatch (txSlice bs off len)).2.2.toNat
  let listLen := lenW - (teerTxTypeDispatch (txSlice bs off len)).2.2
  let cursor := longWalkCursorAmbient regionBase bs listOff hwi_off
  let endPtr := longWalkEndAmbient regionBase listLen listOff
  have hMid := extractType234AfterSaveCopy_then_epi_of_decode_region_ambient sp0 spC s
    loadPtr regionBase lenW
    (teerTxTypeDispatch (txSlice bs off len)).2.1
    (teerTxTypeDispatch (txSlice bs off len)).2.2
    cursor endPtr toBuf isCreationPtr s.s7 bs
  ...
  exact cpsTripleWithin_weaken
    (fun h hp =>
      frontAfterSavePostLongAmbient_to_midJoinPre spC s regionBase loadPtr lenW
        toBuf isCreationPtr bs off len hwi_off h hp)
    (fun _ hq => by
      dsimp only [contentPtr, w2] at hq ⊢
      xperm_hyp hq) hMid

#print axioms extractFrontAfterSaveCopy_then_epi_of_decode_long_region_ambient
