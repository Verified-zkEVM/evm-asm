/-
  Ambient dual: long legacy creation pure honesty (typeW=0, fields 0..3).
...
theorem extractAssumed_creation_longConcrete_pure_legacy_ambient
  ...
  have hfields02 :=
    extractSuccess_creation_legacy_hnext_fields02_long slice hsuccess hcreFlag htype0
      items hdecL hlong
  have hoverEnd :
      regionBase.toNat +
  ...
  have hhnext :=
    extractSuccess_creation_legacy_hnext_hcre_srcOff_long slice loadPtr hsuccess
      hcreFlag htype0 items hdecL hlong
  ...
  have hcre : ∀ (next3 len3 : Word),
    ...
    simpa [absOff3, srcOff3, listOff, endW, absListOff] using
      hcre_ambient_of_slice_long regionBase loadPtr bs off len listOff items 3 endW
  ...
  have hss3 :=
    hss_ambient_of_long_list_field regionBase loadPtr bs off len listOff items 3
      hptr hbound hencInner hlong hn3 hitem3 hoff3_sl hover (Or.inl (by omega : 4 < items.length))
      hvalid1_3 hoff3
  have hls0 :
  ...
  exact extractAssumed_creation_under_honesty_of_decode_long_concrete_legacy_ambient
...
set_option maxRecDepth 8000 in
theorem extractAssumed_creation_longConcrete_pure_legacy_ambient_fullCode
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (items : List EL.RLP.RLPItem)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_creation_longConcrete_pure_legacy_ambient sp0 spC s
...
#print axioms extractAssumed_creation_longConcrete_pure_legacy_ambient
#print axioms extractAssumed_creation_longConcrete_pure_legacy_ambient_fullCode
