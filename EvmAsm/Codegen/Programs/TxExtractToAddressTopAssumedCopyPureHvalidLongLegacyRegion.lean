/-
  ...
  Gates content dword-alignment: longListSrcOff 3 + 1 = 8*q.
-/
...
/-- Bare Assumed long legacy copy under pure honesty + dword-aligned content.
    `hq_align : longListSrcOff listOff items 5 + 1 = 8 * q`. classical-3. -/
theorem extractAssumed_copy_longConcrete_pureHvalid_legacy_region
    ...
    (hq_align : longListSrcOff (teerTxTypeDispatch txBytes).2.2.toNat items 3 + 1 = 8 * q)
    ...
    (hitem0 : (encode (items[0]'(by
        have := extractSuccess_copy_legacy_items_length_long txBytes hsuccess
  ...
  have hhoff :=
    extractSuccess_copy_legacy_hoff_srcOff_long txBytes hsuccess hcopyFlag htype0
      items hdecL hlong
  have hhover :=
    extractSuccess_copy_legacy_hover_srcOff_long txBytes txBase hsuccess hcopyFlag htype0
      items hdecL hlong hover
  have hlenItems :=
    extractSuccess_copy_legacy_items_length_long txBytes hsuccess hcopyFlag htype0
      items hdecL hlong
  ...
  have hfields02 :=
    extractSuccess_copy_legacy_hnext_fields02_long txBytes hsuccess hcopyFlag htype0
      items hdecL hlong
  have hendEq :
  ...
  have hcur :
    ...
    simpa [longWalkCursor, srcOff0, listOff] using
      longWalkCursor_eq_srcOff0 txBytes txBase listOff items hencInner hlong hbound
        hoff hoverC
  have hhnext :=
    extractSuccess_copy_legacy_hnext_hlen20_srcOff_long txBytes txBase hsuccess
  ...
  have hcontentPtr : contentPtr = txBase + BitVec.ofNat 64 srcOff3 + (1 : Word) := by
    have hsrc : srcOff3 + 1 = 8 * q := by
      simpa [srcOff3, listOff] using hq_align
    have hbase : txBase.toNat + (srcOff3 + 1) < 2 ^ 64 := by
      have : srcOff3 + 1 ≤ 8 * q + 16 := by omega
      omega
    have h1 : BitVec.ofNat 64 (srcOff3 + 1) = BitVec.ofNat 64 srcOff3 + (1 : Word) := by
      have hs : srcOff3 < 2 ^ 64 := by omega
      have hs1 : srcOff3 + 1 < 2 ^ 64 := by omega
      apply BitVec.eq_of_toNat_eq
      change (srcOff3 + 1) % 2 ^ 64 = (BitVec.ofNat 64 srcOff3 + (1 : Word)).toNat
    ...
    simpa [contentPtr, ← hsrc, h1] using
      (BitVec.add_assoc txBase (BitVec.ofNat 64 srcOff3) (1 : Word)).symm
  have hnext_content : ∀ (next3 len3 : Word),
      rlpItemDecode txBytes srcOff3 (txBase + BitVec.ofNat 64 srcOff3) endW next3 len3 →
      next3 = txBase + BitVec.ofNat 64 (8 * q) + (20 : Word) := by
    intro n l hd
    have hc := extractSuccess_copy_legacy_hnext_content_long txBytes txBase contentPtr
      srcOff3 hsuccess hcopyFlag htype0 items hdecL hlong rfl hcontentPtr endW n l hd
    simpa [contentPtr] using hc
  have hitem3 :
      (encode (items[3]'hn3)).length ≤ 55 := by
    have hf := extractSuccess_copy_legacy_field3_pfx94_long txBytes hsuccess hcopyFlag htype0
      items hdecL hlong
    obtain ⟨_, ⟨content, hget, hlen20⟩, _⟩ := hf
    have hval : items[3]'hn3 = .bytes content := (List.getElem?_eq_some_iff.mp hget).2
    have henc : encode (items[3]'hn3) = BitVec.ofNat 8 0x94 :: content := by
      rw [hval, encode_bytes_len20_pfx content hlen20]
    have hlenE : (encode (items[3]'hn3)).length = 21 := by
      simp [henc, hlen20]
    omega
  have hinb0 :=
  ...
  exact extractAssumed_copy_of_front_long_concrete_legacy_region
    sp0 spC s txBase lenW toBuf isCreationPtr txBytes
    srcOff0 srcOff1 srcOff2 srcOff3 q
    hspC hret hoff hcur htype0 halign
    ...
    hdec2 hinb2
    hhoff.2.2.2 hhover.2.2.2 hv.2.2.2.2.2.2.1 hss3
    (hls_vacuous_of_long_list_item txBytes listOff items 3 hencInner hlong hn3 hitem3 hhoff.2.2.2)
    (hll_vacuous_of_long_list_item txBytes listOff items 3 hencInner hlong hn3 hitem3 hhoff.2.2.2)
    hdec3 hinb3
...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_longConcrete_pureHvalid_legacy_region_fullCode
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr : Word)
    (txBytes : List (BitVec 8))
    (items : List EL.RLP.RLPItem)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_copy_longConcrete_pureHvalid_legacy_region
      sp0 spC s txBase lenW toBuf isCreationPtr txBytes items q
      hspC hret htalign htover htvalid hq_align hq hcover hcvalid
      hlen hsuccess hcopyFlag htype0 hdecL hlong
      halign hover hvalidBuf
      hitem0 hitem1 hitem2 hge5)

#print axioms extractAssumed_copy_longConcrete_pureHvalid_legacy_region
#print axioms extractAssumed_copy_longConcrete_pureHvalid_legacy_region_fullCode

/-- Path refinements for long legacy copy + dword-aligned content. -/
def extractCopyLegacyLongPathRegion
  ...
  extractSuccess txBytes ∧
    ...
    5 ≤ items.length ∧
    longListSrcOff (teerTxTypeDispatch txBytes).2.2.toNat items 3 + 1 = 8 * q ∧
    8 * q + 16 < txBytes.length

set_option maxRecDepth 8000 in
/-- Bare Assumed footprint under long legacy copy + aligned content. classical-3. -/
theorem extractAssumed_success_flat_copy_legacy_long
    ...
    (hcvalid : isValidMemAccess
      (txBase + BitVec.ofNat 64 (8 * q) + (16 : Word)) = true)
    (hpath : extractCopyLegacyLongPathRegion txBytes items q)
    (hitem0 : (encode (items[0]'(by
  ...
  obtain ⟨hsuccess, hcopyFlag, htype0, hdecL, hlong, hge5, hq_align, hq⟩ := hpath
  let s : ExtractSaved :=
  ...
  simpa only [hsra, hs0, hs1, hs2, hs3, hs4, hs5, hs6, hs7] using
    extractAssumed_copy_longConcrete_pureHvalid_legacy_region_fullCode
...
#print axioms extractAssumed_success_flat_copy_legacy_long
