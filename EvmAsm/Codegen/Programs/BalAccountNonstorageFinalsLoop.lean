...
private theorem se64 : signExtend12 (64 : BitVec 12) = (64 : Word) := by decide
private theorem se72 : signExtend12 (72 : BitVec 12) = (72 : Word) := by decide
...
private def flHeadPre (newSp cursor endW : Word) : Assertion :=
  ((.x2 : Reg) ↦ᵣ newSp) **
  ((newSp + 64) ↦ₘ cursor) ** ((newSp + 72) ↦ₘ endW) **
  regOwn .x5 ** regOwn .x6

private def flHeadPost (newSp cursor endW : Word) : Assertion :=
  ((.x2 : Reg) ↦ᵣ newSp) **
  ((newSp + 64) ↦ₘ cursor) ** ((newSp + 72) ↦ₘ endW) **
...
private def flCF (aB : Word) (acctBytes : List (BitVec 8)) : Assertion :=
  regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** regOwn .x28 ** regOwn .x29 **
  regOwn .x30 ** regOwn .x31 ** ((.x0 : Reg) ↦ᵣ (0 : Word)) **
  ((.x1 : Reg) ↦ᵣ (B + 240 + 4)) ** bytesRegion aB acctBytes

/-- Shorthand for the framed extras at the return site. -/
private def flEx (aB newSp : Word) (off endOff : Nat) (v19 v20 : Word)
    (F : Assertion) : Assertion :=
  ((.x2 : Reg) ↦ᵣ newSp) **
  ((newSp + 64) ↦ₘ (aB + BitVec.ofNat 64 off)) **
  ((newSp + 72) ↦ₘ (aB + BitVec.ofNat 64 endOff)) **
