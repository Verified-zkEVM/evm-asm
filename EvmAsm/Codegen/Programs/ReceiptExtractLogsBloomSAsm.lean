/-
  Verified SAsm port of `receipt_extract_logs_bloom` (K152).
  The wrapper selects receipt field 2 through the strict K20 list selector,
...
def wrapperCode : CodeReq :=
  CodeReq.ofProg B receiptExtractLogsBloom_prog
def code : CodeReq :=
...
/-! The semantic result of K152.  Failure and wrong-length outcomes preserve
...
theorem copyLoopNoAbiFn_code_spec (src dst : Word)
  ...
  have hmono : ∀ a i,
    ...
    let seg : List Instr := [.BEQ .x30 .x0 (28 : BitVec 13)] ++ copyStepBlock ++
      [.JAL .x0 (-24 : BitVec 21)]
    have hbase := CodeReq.ofProg_mono_sub B (B + 112) receiptExtractLogsBloom_prog
...
/-! ## K152's ABI frame and entry setup -/
...
theorem setupPrologue
  ...
  have ha := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B B receiptExtractLogsBloom_prog 0
  ...
  have hstoreMono : ∀ a i,
    ...
    intro a i hi
    exact CodeReq.ofProg_mono_sub B (B + 4) receiptExtractLogsBloom_prog
...
theorem setupMoves
  ...
  have h0' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 20) receiptExtractLogsBloom_prog 5
  ...
  have h1' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 24) receiptExtractLogsBloom_prog 6
  ...
  have h2' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 28) receiptExtractLogsBloom_prog 7
...
theorem callListNth
  ...
  have hcall := callWithin_spec (B + 60) K20B vOld
    (jalOff GuestAddrs.rlp_list_nth_item
      (GuestAddrs.receipt_extract_logs_bloom + 60))
    ((12 + ((85 + 93 * (index + 2)) + 6)) + 9)
    htarget hmem (by
...
theorem branchSelected
  ...
  have hb1 := cpsBranchWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 64) receiptExtractLogsBloom_prog 16
...
theorem setupArgs
    ...
    cpsTripleWithin 3 (B + 32) (B + 44) code
      ...
      (((regIs .x8 listBase) ** (regIs .x9 listLen) **
        (regIs .x18 outputPtr) ** (regIs .x10 listBase) **
        (regIs .x11 listLen) ** (regIs .x12 (2 : Word))) ** F) := by
  have h0 := mv_spec_gen_within .x10 .x8 listBase listBase (B + 32) (by decide)
  have h0' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 32) receiptExtractLogsBloom_prog 8
      (.MV .x10 .x8) (by bv_omega) (by rw [program_length]; decide) rfl
      (by rw [program_length]; decide)) h0
  have h1 := mv_spec_gen_within .x11 .x9 listLen listLen (B + 36) (by decide)
  have h1' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 36) receiptExtractLogsBloom_prog 9
      (.MV .x11 .x9) (by bv_omega) (by rw [program_length]; decide) rfl
      (by rw [program_length]; decide)) h1
  have h2 := li_spec_gen_within .x12 outputPtr (2 : Word) (B + 40) (by decide)
  have h2' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 40) receiptExtractLogsBloom_prog 10
      (.LI .x12 2) (by bv_omega) (by rw [program_length]; decide) rfl
      (by rw [program_length]; decide)) h2
  have h0F := cpsTripleWithin_frameR
  ...
  have hlocal := cpsTripleWithin_weaken
    ...
    (Q' := (regIs .x8 listBase) ** (regIs .x9 listLen) **
      (regIs .x18 outputPtr) ** (regIs .x10 listBase) **
      (regIs .x11 listLen) ** (regIs .x12 (2 : Word)))
...
theorem setupGlobals
  ...
  have hau0 := CodeReq.ofProg_mem_at B (B + 44) receiptExtractLogsBloom_prog 11
    (.AUIPC .x13 (laHi GuestAddrs.relb_offset
      (GuestAddrs.receipt_extract_logs_bloom + 44)))
    (by bv_omega) (by rw [program_length]; decide) rfl
    (by rw [program_length]; decide)
  have had0 := CodeReq.ofProg_mem_at B (B + 48) receiptExtractLogsBloom_prog 12
    (.ADDI .x13 .x13 (laLo GuestAddrs.relb_offset
      (GuestAddrs.receipt_extract_logs_bloom + 44)))
    (by bv_omega) (by rw [program_length]; decide) rfl
    (by rw [program_length]; decide)
  have ho := la_materialize_within .x13 old13 (B + 44) offsetCell
    (by decide) (by unfold B offsetCell; decide) hau0 had0
  have hau1 := CodeReq.ofProg_mem_at B (B + 52) receiptExtractLogsBloom_prog 13
    (.AUIPC .x14 (laHi GuestAddrs.relb_length
      (GuestAddrs.receipt_extract_logs_bloom + 52)))
    (by bv_omega) (by rw [program_length]; decide) rfl
    (by rw [program_length]; decide)
  have had1 := CodeReq.ofProg_mem_at B (B + 56) receiptExtractLogsBloom_prog 14
    (.ADDI .x14 .x14 (laLo GuestAddrs.relb_length
      (GuestAddrs.receipt_extract_logs_bloom + 52)))
    (by bv_omega) (by rw [program_length]; decide) rfl
    (by rw [program_length]; decide)
...
theorem selectedLengthExact
  ...
  have hau := CodeReq.ofProg_mem_at B (B + 68) receiptExtractLogsBloom_prog 17
    (.AUIPC .x5 (laHi GuestAddrs.relb_length
      (GuestAddrs.receipt_extract_logs_bloom + 68))) (by bv_omega)
    (by rw [program_length]; decide) rfl (by rw [program_length]; decide)
  have had := CodeReq.ofProg_mem_at B (B + 72) receiptExtractLogsBloom_prog 18
    (.ADDI .x5 .x5 (laLo GuestAddrs.relb_length
      (GuestAddrs.receipt_extract_logs_bloom + 68))) (by bv_omega)
    (by rw [program_length]; decide) rfl (by rw [program_length]; decide)
  ...
  have h1' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 76) receiptExtractLogsBloom_prog 19
  ...
  have h2' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 80) receiptExtractLogsBloom_prog 20
...
private theorem lengthBranchCase
  ...
  have hb1 := cpsBranchWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 84) receiptExtractLogsBloom_prog 21
...
theorem cursorSetupExact
  ...
  have hau := CodeReq.ofProg_mem_at B (B + 88) receiptExtractLogsBloom_prog 22
    (.AUIPC .x5 (laHi GuestAddrs.relb_offset
      (GuestAddrs.receipt_extract_logs_bloom + 88))) (by bv_omega)
    (by rw [program_length]; decide) rfl (by rw [program_length]; decide)
  have had := CodeReq.ofProg_mem_at B (B + 92) receiptExtractLogsBloom_prog 23
    (.ADDI .x5 .x5 (laLo GuestAddrs.relb_offset
      (GuestAddrs.receipt_extract_logs_bloom + 88))) (by bv_omega)
    (by rw [program_length]; decide) rfl (by rw [program_length]; decide)
  ...
  rw [show offsetCell + signExtend12 (0 : BitVec 12) = offsetCell from by
    rw [show signExtend12 (0 : BitVec 12) = (0 : Word) from by decide]; bv_omega] at h1
  have h1' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 96) receiptExtractLogsBloom_prog 24
      (.LD .x6 .x5 (0 : BitVec 12)) (by bv_omega)
      (by rw [program_length]; decide) rfl (by rw [program_length]; decide)) h1
  have h2 := add_spec_gen_within .x28 .x8 .x6 listBase offset old28
    (B + 100) (by decide)
  have h2' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 100) receiptExtractLogsBloom_prog 25
  ...
  have h3' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 104) receiptExtractLogsBloom_prog 26
  ...
  have h4' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 108) receiptExtractLogsBloom_prog 27
...
theorem successStatusTail (old10 : Word) (F : Assertion) (hF : F.pcFree) :
    cpsTripleWithin 2 (B + 140) (B + 160) code
      ((.x10 ↦ᵣ old10) ** F) ((.x10 ↦ᵣ (0 : Word)) ** F) := by
  have h0 := li_spec_gen_within .x10 old10 (0 : Word) (B + 140) (by decide)
  have h0' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 140) receiptExtractLogsBloom_prog 35
      (.LI .x10 (0 : Word)) (by bv_omega) (by rw [program_length]; decide)
      rfl (by rw [program_length]; decide)) h0
  have hj := jal_x0_spec_gen_within (16 : BitVec 21) (B + 144)
  rw [show B + 144 + signExtend21 (16 : BitVec 21) = B + 160 by decide] at hj
  have hj' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 144) receiptExtractLogsBloom_prog 36
      (.JAL .x0 (16 : BitVec 21)) (by bv_omega)
      (by rw [program_length]; decide) rfl (by rw [program_length]; decide)) hj
  have h0F := cpsTripleWithin_frameR F hF h0'
  have hjF := cpsTripleWithin_frameR ((.x10 ↦ᵣ (0 : Word)) ** F)
    (pcFree_sepConj pcFree_regIs hF) hj'
  have hjS : cpsTripleWithin 1 (B + 144) (B + 160) wrapperCode
  ...
  exact cpsTripleWithin_extend_code wrapperCode_mono
    (cpsTripleWithin_seq_same_cr h0F hjS)
theorem failureStatusTail (old10 : Word) (F : Assertion) (hF : F.pcFree) :
    cpsTripleWithin 2 (B + 148) (B + 160) code
      ((.x10 ↦ᵣ old10) ** F) ((.x10 ↦ᵣ (1 : Word)) ** F) := by
  have h0 := li_spec_gen_within .x10 old10 (1 : Word) (B + 148) (by decide)
  have h0' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 148) receiptExtractLogsBloom_prog 37
      (.LI .x10 (1 : Word)) (by bv_omega) (by rw [program_length]; decide)
      rfl (by rw [program_length]; decide)) h0
  have hj := jal_x0_spec_gen_within (8 : BitVec 21) (B + 152)
  rw [show B + 152 + signExtend21 (8 : BitVec 21) = B + 160 by decide] at hj
  have hj' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 152) receiptExtractLogsBloom_prog 38
      (.JAL .x0 (8 : BitVec 21)) (by bv_omega)
      (by rw [program_length]; decide) rfl (by rw [program_length]; decide)) hj
  have h0F := cpsTripleWithin_frameR F hF h0'
  have hjF := cpsTripleWithin_frameR ((.x10 ↦ᵣ (1 : Word)) ** F)
    (pcFree_sepConj pcFree_regIs hF) hj'
  have hjS : cpsTripleWithin 1 (B + 152) (B + 160) wrapperCode
      ((.x10 ↦ᵣ (1 : Word)) ** F) ((.x10 ↦ᵣ (1 : Word)) ** F) :=
    cpsTripleWithin_weaken
      (fun h hp => by simpa only [sepConj_emp_left'] using hp)
      (fun h hp => by simpa only [sepConj_emp_left'] using hp) hjF
  exact cpsTripleWithin_extend_code wrapperCode_mono
...
theorem tooLongStatusTail (old10 : Word) (F : Assertion) (hF : F.pcFree) :
    cpsTripleWithin 1 (B + 156) (B + 160) code
      ((.x10 ↦ᵣ old10) ** F) ((.x10 ↦ᵣ (2 : Word)) ** F) := by
  have h0 := li_spec_gen_within .x10 old10 (2 : Word) (B + 156) (by decide)
  have h0' := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 156) receiptExtractLogsBloom_prog 39
      (.LI .x10 (2 : Word)) (by bv_omega) (by rw [program_length]; decide)
      rfl (by rw [program_length]; decide)) h0
  exact cpsTripleWithin_frameR F hF
    (cpsTripleWithin_extend_code wrapperCode_mono h0')
theorem restoreTail
    (sp0 newSp : Word) (saved : Saved) (F : Assertion) (hF : F.pcFree)
    (hnewSp : newSp = sp0 + signExtend12 (-32 : BitVec 12))
    (hret : saved.ra &&& ~~~(1 : Word) = saved.ra) :
    cpsTripleWithin 6 (B + 160) saved.ra code
      (((.x2 ↦ᵣ newSp) ** regsOwnAt frame ** savedFrame newSp saved) ** F)
      (((.x2 ↦ᵣ sp0) ** regsAt frame (savedVals saved) **
        savedFrame newSp saved) ** F) := by
  have hl0 := loadSeq_spec_own frame newSp (savedVals saved)
    (B + 160) (by decide) (by decide)
  have hlMono : ∀ a i,
      CodeReq.ofProg (B + 160) (loadProg frame) a = some i →
        wrapperCode a = some i := by
    intro a i hi
    exact CodeReq.ofProg_mono_sub B (B + 160) receiptExtractLogsBloom_prog
      (loadProg frame) 40 (by bv_omega) (by rfl)
      (by rw [program_length]; simp [frame])
      (by rw [program_length]; decide) a i hi
  have hl := cpsTripleWithin_extend_code hlMono hl0
  rw [show B + 160 + BitVec.ofNat 64 (4 * frame.length) = B + 176 by
    simp [frame]; bv_omega] at hl
  rw [frameSlotsSaved_frame] at hl
  have hlF := cpsTripleWithin_frameR F hF hl
  have hd0 := addi_spec_gen_same_within .x2 newSp (32 : BitVec 12)
    (B + 176) (by decide)
  rw [show newSp + signExtend12 (32 : BitVec 12) = sp0 by
    rw [hnewSp]
    exact sext_frameRestore sp0 (-32 : BitVec 12) (32 : BitVec 12) (by decide)] at hd0
  have hd := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 176) receiptExtractLogsBloom_prog 44
  ...
  have hdF := cpsTripleWithin_frameR
    ...
    (by
      ...
      · apply pcFree_sepConj
        ...
        · exact hF) hd
  have hr0 := EvmAsm.Evm64.ret_spec_within' (B + 180) saved.ra
  rw [hret] at hr0
  have hr := cpsTripleWithin_extend_code (cr' := wrapperCode)
    (CodeReq.ofProg_mem_at B (B + 180) receiptExtractLogsBloom_prog 45
  ...
  have hrF := cpsTripleWithin_frameR
    (((.x2 ↦ᵣ sp0) ** (.x8 ↦ᵣ saved.s0) ** (.x9 ↦ᵣ saved.s1) **
      (.x18 ↦ᵣ saved.s2) ** savedFrame newSp saved) ** F) (by
        apply pcFree_sepConj
  ...
  exact cpsTripleWithin_extend_code wrapperCode_mono hlocal
