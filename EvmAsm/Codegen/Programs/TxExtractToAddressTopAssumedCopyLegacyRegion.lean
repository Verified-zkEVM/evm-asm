/-
  Assumed bare packaging for legacy short 20B copy region path.
-/
...
theorem nFrontCopyStepsShortLegacyRegion_le_nExtract :
    nFrontCopyStepsShortLegacyRegion ≤ nExtractSteps := by
  simp only [nFrontCopyStepsShortLegacyRegion, nExtractSteps, nTypeSteps]
  omega

set_option maxRecDepth 8000 in
theorem extractAssumed_copy_of_front_short_concrete_legacy_region
  ...
  have hE2E : ∀ (old5 old6 old7 old14 old15 old16 : Word),
      cpsTripleWithin nFrontCopyStepsShortLegacyRegion E s.ra extractLinkedCode
    ...
    have h := extractFrontCopy_then_epi_of_decode_short_concrete_legacy_region
  ...
  · intro old5 old6 old7 old14 old15 old16
    ...
    have hle : nFrontCopyStepsShortLegacyRegion ≤ nFrontCopyStepsShortRegion := by
      simp only [nFrontCopyStepsShortLegacyRegion, nFrontCopyStepsShortRegion, nTypeSteps]
      omega
    have h1 := cpsTripleWithin_mono_nSteps hle h0
    exact h1

set_option maxRecDepth 8000 in
theorem extractAssumed_copy_fullCode_of_decode_short_concrete_legacy_region
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr : Word)
    (txBytes : List (BitVec 8))
    (srcOff0 srcOff1 srcOff2 srcOff3 q : Nat)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_copy_of_front_short_concrete_legacy_region sp0 spC s
...
#print axioms extractAssumed_copy_of_front_short_concrete_legacy_region
#print axioms extractAssumed_copy_fullCode_of_decode_short_concrete_legacy_region
