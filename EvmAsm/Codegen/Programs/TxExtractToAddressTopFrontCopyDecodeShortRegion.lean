/-
  Front short AfterSave → 20B copy bare region (no contentDwords).
-/
...
theorem extractFrontAfterSaveCopy_then_epi_of_decode_short_region
  ...
  exact cpsTripleWithin_weaken
    (fun h hp =>
      frontAfterSavePostShort_to_midJoinPre spC s txBase lenW
        toBuf isCreationPtr txBytes h hp)
    (fun _ hq => by
      dsimp only [contentPtr, w2] at hq ⊢
      xperm_hyp hq) hMid

#print axioms extractFrontAfterSaveCopy_then_epi_of_decode_short_region
