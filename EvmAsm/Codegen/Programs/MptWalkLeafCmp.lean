/-
  Leaf path-nibble compare loop (#11799): pc258→pc266.

  Same shape as extension compare (MptWalkExtCmp) at different PCs:
    BEQ rem,0 → pc266 match exit
    LBU buf; LBU path; BNE mismatch → pc297
    ADDI buf+1; ADDI path+1; ADDI rem-1; JAL → pc258

  Domain: HP segment nibbles equal remaining path (exact match).
-/
...
private theorem beq_leaf_cmp_exit_off :
    pc 258 + signExtend13 (32 : BitVec 13) = pc 266 := by
  unfold pc walkB signExtend13; decide

/-- Mismatch BNE lands at pc297 (status-1), not pc300. -/
private theorem bne_leaf_cmp_mis_off :
    pc 261 + signExtend13 (144 : BitVec 13) = pc 297 := by
  unfold pc walkB signExtend13; decide

private theorem jal_leaf_cmp_back_off :
    pc 265 + signExtend21 (-28 : BitVec 21) = pc 258 := by
  unfold pc walkB
...
def leafCmpInv (bufBase pathCurBase : Word) (k done : Nat)
...
/-- Match exit at pc170. -/
def leafCmpMatch (bufBase pathCurBase : Word) (count : Nat)
    (bufBytes pathSeg : List (BitVec 8)) (F : Assertion) : Assertion :=
...
theorem leaf_cmp_exit_zero
    (bufBase pathCurBase : Word) (count : Nat)
    (bufBytes pathSeg : List (BitVec 8))
    (F : Assertion) (hF : F.pcFree) :
    cpsTripleWithin 1 (pc 258) (pc 266) fullCode
      (leafCmpInv bufBase pathCurBase 0 count bufBytes pathSeg F)
      (leafCmpMatch bufBase pathCurBase count bufBytes pathSeg F) := by
  have hbr0 := beq_spec_gen_within .x29 .x0 (32 : BitVec 13)
    (0 : Word) (0 : Word) (pc 258)
  rw [beq_leaf_cmp_exit_off, show pc 258 + 4 = pc 259 from pc_succ 258] at hbr0
  have hbr := cpsBranchWithin_extend_code
    (walkMem (pc 258) 258 (.BEQ .x29 .x0 (32 : BitVec 13))
  ...
  exact cpsTripleWithin_weaken
    (fun _ hp => by
      simp only [leafCmpInv, ofNat_zero, G] at hp ⊢
      xperm_chunked hp)
    (fun _ hq => by
      simp only [leafCmpMatch, G] at hq ⊢
      xperm_chunked hq)
    htF
...
theorem leaf_cmp_step
    ...
    (F : Assertion) (hF : F.pcFree) :
    cpsTripleWithin 8 (pc 258) (pc 258) fullCode
      (leafCmpInv bufBase pathCurBase (k + 1) done bufBytes pathSeg F)
      (leafCmpInv bufBase pathCurBase k (done + 1) bufBytes pathSeg F) := by
  ...
  -- BEQ ntaken
  have hbr0 := beq_spec_gen_within .x29 .x0 (32 : BitVec 13)
    (BitVec.ofNat 64 (k + 1)) (0 : Word) (pc 258)
  rw [beq_leaf_cmp_exit_off, show pc 258 + 4 = pc 259 from pc_succ 258] at hbr0
  have hbr := cpsBranchWithin_extend_code
    (walkMem (pc 258) 258 (.BEQ .x29 .x0 (32 : BitVec 13))
      (by decide) (by unfold pc walkB; decide) rfl) hbr0
  ...
  have hlbuB : ∀ v30,
      cpsTripleWithin 1 (pc 259) (pc 260) fullCode
        (((.x7 ↦ᵣ (bufBase + BitVec.ofNat 64 done)) **
          bytesRegion bufBase bufBytes **
          (.x28 ↦ᵣ (pathCurBase + BitVec.ofNat 64 done)) **
  ...
  -- LBU path: peel x31
  have hlbuP : ∀ v31,
      cpsTripleWithin 1 (pc 260) (pc 261) fullCode
        (((.x28 ↦ᵣ (pathCurBase + BitVec.ofNat 64 done)) **
          bytesRegion pathCurBase pathSeg **
          (.x7 ↦ᵣ (bufBase + BitVec.ofNat 64 done)) **
          (.x30 ↦ᵣ ((bufBytes[done]'hbuf).zeroExtend 64)) **
    ...
    intro v31
    have hlbu := bytesRegion_lbu_within .x31 .x28 pathCurBase v31 (pc 260)
      pathSeg done (by decide) hpathAlign hpath hpathOver hvalidP
    have hlbuE := cpsTripleWithin_extend_code
      (walkMem (pc 260) 260 (.LBU .x31 .x28 (0 : BitVec 12))
        (by decide) (by unfold pc walkB; decide) rfl) hlbu
    rw [pc_succ 260] at hlbuE
    have hFr := cpsTripleWithin_frameR
      ((.x7 ↦ᵣ (bufBase + BitVec.ofNat 64 done)) **
       (.x30 ↦ᵣ ((bufBytes[done]'hbuf).zeroExtend 64)) **
       (.x29 ↦ᵣ BitVec.ofNat 64 (k + 1)) ** (.x0 ↦ᵣ (0 : Word)) **
  ...
  have heqW :
    ...
    simp only [hmatch]
  have hbrm0 := bne_spec_gen_within .x30 .x31 (144 : BitVec 13)
    ((bufBytes[done]'hbuf).zeroExtend 64)
    ((pathSeg[done]'hpath).zeroExtend 64) (pc 261)
  rw [bne_leaf_cmp_mis_off, show pc 261 + 4 = pc 262 from pc_succ 261] at hbrm0
  have hbrm := cpsBranchWithin_extend_code
    (walkMem (pc 261) 261 (.BNE .x30 .x31 (144 : BitVec 13))
  ...
  have hadd70 := addi_spec_gen_same_within .x7 (bufBase + BitVec.ofNat 64 done)
    (1 : BitVec 12) (pc 262) (by decide)
  have hadd7 := cpsTripleWithin_extend_code
    (walkMem (pc 262) 262 (.ADDI .x7 .x7 (1 : BitVec 12))
      (by decide) (by unfold pc walkB; decide) rfl) hadd70
  rw [pc_succ 262, signExtend12_1c] at hadd7
  ...
  have hadd280 := addi_spec_gen_same_within .x28 (pathCurBase + BitVec.ofNat 64 done)
    (1 : BitVec 12) (pc 263) (by decide)
  have hadd28 := cpsTripleWithin_extend_code
    (walkMem (pc 263) 263 (.ADDI .x28 .x28 (1 : BitVec 12))
      (by decide) (by unfold pc walkB; decide) rfl) hadd280
  rw [pc_succ 263, signExtend12_1c] at hadd28
  ...
  have hadd290 := addi_spec_gen_same_within .x29 (BitVec.ofNat 64 (k + 1))
    (-1 : BitVec 12) (pc 264) (by decide)
  have hadd29 := cpsTripleWithin_extend_code
    (walkMem (pc 264) 264 (.ADDI .x29 .x29 (-1 : BitVec 12))
      (by decide) (by unfold pc walkB; decide) rfl) hadd290
  rw [pc_succ 264, signExtend12_m1c] at hadd29
  ...
  -- JAL back
  have hjal0 := jal_x0_spec_gen_within (-28 : BitVec 21) (pc 265)
  have hjal := cpsTripleWithin_extend_code
    (walkMem (pc 265) 265 (.JAL .x0 (-28 : BitVec 21))
      (by decide) (by unfold pc walkB; decide) rfl) hjal0
  rw [jal_leaf_cmp_back_off] at hjal
  ...
  have c' : cpsTripleWithin 8 (pc 258) (pc 258) fullCode
      (leafCmpInv bufBase pathCurBase (k + 1) done bufBytes pathSeg F)
    ...
    · intro h hp; simp only [leafCmpInv] at hp ⊢; xperm_chunked hp
    · intro h hq
  ...
  have hq2 :=
    sepConj_mono_right
      ...
      h hq1
  simp only [leafCmpInv] at hq2 ⊢
  xperm_chunked hq2

/-- Exact match over `count` nibbles. -/
def LeafPathMatch (bufBytes pathSeg : List (BitVec 8)) (count : Nat) : Prop :=
...
theorem leaf_cmp_loop_match
    (bufBase pathCurBase : Word) (count : Nat)
    (bufBytes pathSeg : List (BitVec 8))
    (hmatch : LeafPathMatch bufBytes pathSeg count)
    (hbufAlign : bufBase.toNat % 8 = 0)
    (hpathAlign : pathCurBase.toNat % 8 = 0)
    ...
    (F : Assertion) (hF : F.pcFree) :
    cpsTripleWithin (count * 8 + 1) (pc 258) (pc 266) fullCode
      (leafCmpInv bufBase pathCurBase count 0 bufBytes pathSeg F)
      (leafCmpMatch bufBase pathCurBase count bufBytes pathSeg F) := by
  suffices h : ∀ n, n ≤ count →
      cpsTripleWithin (n * 8 + 1) (pc 258) (pc 266) fullCode
        (leafCmpInv bufBase pathCurBase n (count - n) bufBytes pathSeg F)
        (leafCmpMatch bufBase pathCurBase count bufBytes pathSeg F) by
  ...
  | zero =>
    ...
    simpa [Nat.sub_zero] using
      leaf_cmp_exit_zero bufBase pathCurBase count bufBytes pathSeg F hF
  | succ k ih =>
    intro hk
    have hklt : k < count := Nat.lt_of_succ_le hk
    obtain ⟨hbLen, hpLen, hbytes⟩ := hmatch
    have hstep := leaf_cmp_step bufBase pathCurBase k (count - (k + 1))
      bufBytes pathSeg
      (by omega) (by omega)
    ...
    have hstep' : cpsTripleWithin 8 (pc 258) (pc 258) fullCode
        (leafCmpInv bufBase pathCurBase (k + 1) (count - (k + 1)) bufBytes pathSeg F)
        (leafCmpInv bufBase pathCurBase k (count - k) bufBytes pathSeg F) := by
      convert hstep using 1; simp only [leafCmpInv, hdoneEq]
    have c := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_chunked hp)
      hstep' hih
    convert c using 1; omega

/-- From setup regs through match exit. -/
theorem leaf_cmp_setup_to_match
    (bufBase pathCurBase countW : Word)
    (bufBytes pathSeg : List (BitVec 8))
    (hmatch : LeafPathMatch bufBytes pathSeg countW.toNat)
    (hbufAlign : bufBase.toNat % 8 = 0)
    (hpathAlign : pathCurBase.toNat % 8 = 0)
    ...
    (hvalidP : ∀ d, d < countW.toNat →
      isValidByteAccess (pathCurBase + BitVec.ofNat 64 d) = true)
    (F : Assertion) (hF : F.pcFree) :
    cpsTripleWithin (countW.toNat * 8 + 1) (pc 258) (pc 266) fullCode
      ((.x7 ↦ᵣ bufBase) ** (.x28 ↦ᵣ pathCurBase) **
       (.x29 ↦ᵣ countW) ** (.x0 ↦ᵣ (0 : Word)) **
       bytesRegion bufBase bufBytes ** bytesRegion pathCurBase pathSeg **
       (regOwn .x30 ** regOwn .x31) ** F)
      (leafCmpMatch bufBase pathCurBase countW.toNat bufBytes pathSeg F) := by
  have hloop := leaf_cmp_loop_match bufBase pathCurBase countW.toNat
  ...
  have hgoal :
    ...
    xperm_chunked hp
  simpa only [leafCmpInv] using hgoal
