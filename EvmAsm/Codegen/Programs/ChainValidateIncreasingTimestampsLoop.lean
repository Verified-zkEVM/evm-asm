...
set_option maxRecDepth 8000 in
theorem cvitIterEntry (spC hdrBase lenBase validPtr firstBadPtr prevVal : Word)
  ...
  have hbeq := beq_spec_gen_within .x7 .x8 (204 : BitVec 13) (BitVec.ofNat 64 i)
...
theorem cvitIterDispatch
    ...
    (hprevOk : hdrTsOk hdrBase bigBytes lengths (i - 1) prevVal)
    (hprefix : ∀ j, 1 ≤ j → j < i → tsIncreasing hdrBase bigBytes lengths j)
    (htail : (∀ j, 1 ≤ j → j < i + 1 → tsIncreasing hdrBase bigBytes lengths j) →
      cpsTripleWithin nTail (D + 124) raIn fullCode
        ...
        (cvitPost sp0 spC calleeNewSp hdrBase lenBase validPtr firstBadPtr csaved
          bigBytes lengths)) :
  ...
  case core =>
    ...
    case body =>
      ...
      · -- SUCCESS arm: `bne` not taken → reload → order compare.
        ...
        set RframeOk : Assertion :=
          ((.x1 ↦ᵣ LinkRA) ** (.x2 ↦ᵣ spC) ** (.x8 ↦ᵣ BitVec.ofNat 64 lengths.length) **
            (.x9 ↦ᵣ lenBase) ** (.x18 ↦ᵣ hdrBase) ** (.x19 ↦ᵣ validPtr) **
            (.x20 ↦ᵣ firstBadPtr) ** (.x21 ↦ᵣ prevVal) ** (Ts ↦ₘ value) **
            regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** regOwn .x11 ** regOwn .x12 **
            regOwn .x13 ** regOwn .x14 ** regOwn .x28 ** regOwn .x29 ** regOwn .x30 **
            regOwn .x31 ** memOwn RfuOff ** memOwn RfuLen **
            stackFree (spC + signExtend12 (-32 : BitVec 12)) 8 **
        ...
        have hbne := bne_spec_gen_within .x10 .x0 (112 : BitVec 13) (0 : Word) (0 : Word)
          (D + 196)
        have hbneC := cpsBranchWithin_extend_code cvit_mono
          (cpsBranchWithin_extend_code (cr' := cvitCode)
            (CodeReq.ofProg_mem_at D (D + 196) cvitProg 49 (.BNE .x10 .x0 (112 : BitVec 13))
              (by bv_omega) (by rw [cvit_length]; decide) rfl
              (by rw [cvit_length]; decide)) hbne)
        ...
        refine cpsTripleWithin_weaken (fun h hp => by xperm_hyp hp) (fun _ hq => hq)
          (show cpsTripleWithin (23 + nTail) (D + 200) raIn fullCode
            (((.x10 ↦ᵣ (0 : Word)) ** (.x0 ↦ᵣ (0 : Word)) ** (.x1 ↦ᵣ LinkRA) ** (.x2 ↦ᵣ spC) **
              ...
              (.x19 ↦ᵣ validPtr) ** (.x20 ↦ᵣ firstBadPtr) ** (.x21 ↦ᵣ prevVal) ** (Ts ↦ₘ value) **
        ...
        set Rstate2 : Assertion :=
          ((.x5 ↦ᵣ IterPrev) ** (Ts ↦ₘ value) ** (IterPrev ↦ₘ prevVal)) ** Rreload
          with hRstate2
        -- reload [50-55]
        have hreload := cpsTripleWithin_extend_code cvit_mono
          (cvitReload value prevVal v5 v28 v29)
        ...
        rw [show (D + 224) + signExtend13 (56 : BitVec 13) = D + 280 from by
          rw [show signExtend13 (56 : BitVec 13) = (56 : Word) from by decide]; bv_omega] at hbgeu
        have hbgeuC := cpsBranchWithin_extend_code cvit_mono
          (cpsBranchWithin_extend_code (cr' := cvitCode)
            (CodeReq.ofProg_mem_at D (D + 224) cvitProg 56 (.BGEU .x29 .x28 (56 : BitVec 13))
              (by bv_omega) (by rw [cvit_length]; decide) rfl
              (by rw [cvit_length]; decide)) hbgeu)
        ...
        have h_t : cpsTripleWithin (16 + nTail) (D + 280) raIn fullCode
          ...
          refine cpsTripleWithin_weaken (fun h hp => by xperm_hyp hp) (fun _ hq => hq)
            (cpsTripleWithin_pure_pre (P := ¬ BitVec.ult prevVal value)
              ...
              (fun hnult => ?_))
          have hviol := cpsTripleWithin_extend_code cvit_mono
            (retViolation sp0 spC raIn (BitVec.ofNat 64 i) validPtr firstBadPtr csaved
              ((.x29 ↦ᵣ prevVal) ** (.x28 ↦ᵣ value) ** (Ts ↦ₘ value) ** (IterPrev ↦ₘ prevVal) **
                regOwn .x11 ** regOwn .x12 ** regOwn .x13 ** regOwn .x14 **
        ...
        have h_f : cpsTripleWithin (16 + nTail) (D + 228) raIn fullCode
            (((.x29 ↦ᵣ prevVal) ** (.x28 ↦ᵣ value) ** ⌜BitVec.ult prevVal value⌝) ** Rstate2)
            (cvitPost sp0 spC (spC + signExtend12 (-32 : BitVec 12)) hdrBase lenBase
              validPtr firstBadPtr csaved bigBytes lengths) := by
          rw [hRstate2, hRreload]
          refine cpsTripleWithin_weaken (fun h hp => by xperm_hyp hp) (fun _ hq => hq)
            (cpsTripleWithin_pure_pre (P := BitVec.ult prevVal value)
              (H := (.x29 ↦ᵣ prevVal) ** (.x28 ↦ᵣ value) ** (.x5 ↦ᵣ IterPrev) ** (Ts ↦ₘ value) **
                ...
                (.x6 ↦ᵣ v6) ** (.x7 ↦ᵣ v7) ** (.x30 ↦ᵣ v30) ** (.x31 ↦ᵣ v31))
              (fun hult => ?_))
          have hprefix' : ∀ j, 1 ≤ j → j < i + 1 → tsIncreasing hdrBase bigBytes lengths j := by
            intro j hj1 hj
            rcases (by omega : j < i ∨ j = i) with hlt | heq
            · exact hprefix j hj1 hlt
            · subst heq; exact ⟨prevVal, value, hprevOk, hResult, hult⟩
          have hadv := cpsTripleWithin_extend_code cvit_mono
            (cvitAdvance (hdrBaseAt hdrBase lengths i) lenBase (BitVec.ofNat 64 i) value IterPrev
              v6 v7 prevVal v30 v31 lengths[i]!)
          rw [shiftLeft3_ofNat i] at hadv
          have hadvF := cpsTripleWithin_frameR
            ((.x29 ↦ᵣ prevVal) ** (Ts ↦ₘ value) ** (IterPrev ↦ₘ prevVal) ** (.x10 ↦ᵣ (0 : Word)) **
      ...
      · -- PARSE-FAIL arm: `bne` taken → status ≠ 0 exit.
        ...
        have hbne := bne_spec_gen_within .x10 .x0 (112 : BitVec 13) status (0 : Word) (D + 196)
        have hbneC := cpsBranchWithin_extend_code cvit_mono
          (cpsBranchWithin_extend_code (cr' := cvitCode)
            (CodeReq.ofProg_mem_at D (D + 196) cvitProg 49 (.BNE .x10 .x0 (112 : BitVec 13))
              (by bv_omega) (by rw [cvit_length]; decide) rfl
        ...
        rw [show (D + 196) + signExtend13 (112 : BitVec 13) = D + 308 from by
          rw [show signExtend13 (112 : BitVec 13) = (112 : Word) from by decide]; bv_omega] at htaken
        ...
        have hpfC := cpsTripleWithin_extend_code cvit_mono
          (retParseFail sp0 spC raIn (BitVec.ofNat 64 i) firstBadPtr csaved
            ((.x0 ↦ᵣ (0 : Word)) ** (Ts ↦ₘ value) ** regOwn .x7 ** regOwn .x11 ** regOwn .x12 **
              regOwn .x13 ** regOwn .x14 ** regOwn .x28 ** regOwn .x29 ** regOwn .x30 **
              regOwn .x31 ** memOwn RfuOff ** memOwn RfuLen **
              stackFree (spC + signExtend12 (-32 : BitVec 12)) 8 **
              bytesRegion (hdrBaseAt hdrBase lengths i) (bigBytes.drop (hdrOff lengths i)) **
...
/-! ## One full loop iteration: guard → call → dispatch (`D+124 → raIn`, `1 ≤ i < N`)
    ...
    Strips `LoopInv i`'s `∃ prevVal` and its `⌜hdrTsOk (i-1) prevVal ∧ …⌝`
...
set_option maxRecDepth 8000 in
theorem cvitIter (sp0 spC hdrBase lenBase validPtr firstBadPtr raIn : Word)
  ...
  refine cpsTripleWithin_weaken (fun h hp => by rw [hEBody] at hp; xperm_hyp hp)
    ...
    (cpsTripleWithin_seq_same_cr
      (cvitIterEntry spC hdrBase lenBase validPtr firstBadPtr prevVal csaved bigBytes lengths i
        oldOut oldOff oldLen hi hN hsalign hslack hover hvalid)
      (cvitIterDispatch sp0 spC (spC + signExtend12 (-32 : BitVec 12)) hdrBase lenBase validPtr
