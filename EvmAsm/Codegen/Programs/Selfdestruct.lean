...
def selfdestructNewAccountSurchargeAsm : String :=
  ...
  "  li a2, 7; jal ra, account_writes_latest_balance\n" ++
  "  mv t6, a0\n" ++
  "  ld x10, 0(sp)\n  ld x12, 8(sp)\n  ld x13, 16(sp)\n  addi sp, sp, 24\n" ++
  ...
  "  addi sp, sp, -16\n  sd x10, 0(sp)\n  sd x12, 8(sp)\n" ++
  "  la a0, evm_selfdestruct_beneficiary\n  la a1, evm_selfdestruct_balance_scratch\n" ++
  "  li a2, 8; jal ra, account_writes_latest_balance\n" ++
  "  mv t6, a0\n" ++
  "  ld x10, 0(sp)\n  ld x12, 8(sp)\n  addi sp, sp, 16\n" ++
  ...
  "  sub t2, t2, t3\n  sd t2, 568(x20)\n" ++
  -- Keep the gas-left portion visible to the frame-pool derivation.  The
  -- legacy accumulator records the complete charge, while a reservoir
  -- shortage spills the remainder into env+568; without this cell update the
  -- derived path would see only the reservoir debit.
...
def selfdestructBalanceTransferRuntimeAsm : String :=
  ...
  -- non-created path then prefers account_writes_latest_balance (still pre-debit).
...
def selfdestructEip7708LogRuntimeAsm : String :=
  ...
  "  mv a0, sp\n" ++
  "  la a1, evm_selfdestruct_balance_scratch\n" ++
  "  li a2, 9; jal ra, account_writes_latest_balance\n" ++
  ...
  "  li a2, 10; jal ra, account_writes_latest_balance\n" ++
...
def selfdestructBeneficiaryNonstorageAsm : String :=
  ...
  "  li a2, 12; jal ra, account_writes_latest_balance\n" ++
  ...
  "  li a2, 13; jal ra, account_writes_latest_balance\n" ++
  ...
  "  li a2, 14; jal ra, account_writes_latest_balance\n" ++
  ...
  "  la a0, sdai_origin_address; addi a1, sp, 40; li a2, 15; jal ra, account_writes_latest_nonce_tx\n" ++
  "  ld a3, 0(sp); ld a4, 40(sp)\n" ++
  "  j .L_sdbn_origin_debit\n" ++
  ".L_sdbn_origin_nonce_as:\n" ++
  -- status 3: no pre RLP. Nonce is whatever the account-write map holds (CREATE deposited 1);
  -- EIP-6780 preserves it, so pre_nonce = post_nonce = AS latest.
  "  sd zero, 0(sp); la a0, sdai_origin_address; addi a1, sp, 0; li a2, 16; jal ra, account_writes_latest_nonce_tx\n" ++
  "  ld a3, 0(sp); mv a4, a3\n" ++
  ".L_sdbn_origin_debit:\n" ++
