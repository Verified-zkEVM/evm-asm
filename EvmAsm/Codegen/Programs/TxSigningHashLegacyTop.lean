/-
  ...
  **K146 `tx_signing_hash_legacy_eip155` — whole-routine wrap (#12038).**
  ...
  Two whole-routine `cpsTripleWithin`s at the `GuestAddrs` anchor:

  * `tx_signing_hash_legacy_eip155_spec_within` — the **registered** one.
    Non-empty input length and an outer-RLP LIST header (both widths); the
    post is the guest's real outcome disjunction, keccak success or the
    status-1 reject.
  * `tx_signing_hash_legacy_eip155_spec_within_empty_len` — the `a1 = 0`
    reject slice, deliberately not a second registry row (same convention as
    K145's `_empty_len`).

  Between them sits the glue the twelve body modules did not join: the
  H+36→H+124 entry leg (setup, header parse, `rlp_list_nth_item` call), the
  H+124 branch peel joining both Nth outcomes, and the ABI-frame wrap.

  This file holds the second half — the two post-Nth body arms, their join,
  the whole-body triple and the ABI-frame wrap.  The frame scaffolding, the
  `_empty_len` slice, the entry leg and the payload slice are in
  `TxSigningHashLegacyEntry` (same namespace; the split is only for the
  file-size guardrail).
-/

import EvmAsm.Codegen.Programs.TxSigningHashLegacyEntry
...
theorem legacyPayloadSupply
  ...
  refine ⟨legacyPayloadSourceSpec_len input (legacyHdrLen input h0) offVal
    lenVal hfit, ?_, ?_⟩
  · intro bs
