/-
  EvmAsm.Codegen.Programs.Bls12G1IsZeroNSAsm

  Verified SAsm drop-in for `blsg_is_zero_n`: dynamic-length byte-zero scan used by BLS callers.
-/
...
def blsgIsZeroN_prog : Program :=
  (blsgIsZeroNBody 0 [] 0).flatten 0 ++ [Instr.JALR .x0 .x1 (0 : BitVec 12)]

def blsgIsZeroNFunction : String :=
  "blsg_is_zero_n:\n" ++ emitProgram blsgIsZeroN_prog

theorem blsgIsZeroNFunction_eq_prog :
...
theorem blsgIsZeroNFn_spec (ptr : Word) (bs : List (BitVec 8)) (len : Nat)
    (hwf : (Region.mk ptr bs).wf) (base : Word) :
    (blsgIsZeroNFn ptr bs len).Spec base := by
  ...
  case blsgIsZeroN.scan.inv_init =>
    ...
    · rw [RegFile.get_set_self _ _ _ (by decide),
      ...
      simp
  case blsgIsZeroN.scan.inv_step =>
    ...
    have hbyte : (blsgIsZeroNFn ptr bs len).region.byteAt (rfb.get .x5 + signExtend12 0)
      ...
      rw [show (blsgIsZeroNFn ptr bs len).region.bytes = bs from rfl,
          show (blsgIsZeroNFn ptr bs len).region.base = ptr from rfl, hx5, hse0]
      congr 1
