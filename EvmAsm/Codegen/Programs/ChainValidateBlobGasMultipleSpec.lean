/-
  Whole-program caller contract for the 68-instruction
  `chain_validate_blob_gas_used_multiple` accessor.

  `chainValidateBlobGasUsedMultiple_prog` iterates over an array of `N` block
  headers and validates that EVERY header's `blob_gas_used` (RLP field 17)
  decodes to a `u64` that is a MULTIPLE of `GAS_PER_BLOB = 131072 = 2^17`.
  EIP-4844 defines `blob_gas_used = len(blob_versioned_hashes) * GAS_PER_BLOB`,
  so any valid Cancun+ header must have `blob_gas_used` divisible by
  `GAS_PER_BLOB`.
  ...
  Calling convention (see `ChainValidateBlob.lean`):
    ...
    ra (input)  : return
    a0 (output) : 0 = every header a multiple; 1 = some header failed RLP parse
  ...
  The real validity verdict lives in the two output memory cells:
    *is_valid       : 1 iff every header's field-17 u64 is a multiple of
                      GAS_PER_BLOB (`value &&& Mask = 0`), else 0.
  ...
  Per iteration `i` (`i < N`) the program:
    ...
    * else reloads the decoded value, computes `x30 := value &&& Mask` and
      branches on `bne x30, x0`:
        - `value &&& Mask ≠ 0` → `*is_valid = 0`, `*first_bad = i`, `a0 = 0`;
        - `value &&& Mask = 0`  → advance `x18 += header_lengths[i]`, `i += 1`.
...
abbrev D : Word := (ChainValidateOfflineAddrs.chain_validate_blob_gas_used_multiple : Word)

/-- The chain accessor's own program. -/
abbrev cvbgmProg : Program := EvmAsm.Codegen.chainValidateBlobGasUsedMultiple_prog

theorem cvbgm_length : cvbgmProg.length = 68 := by decide
...
abbrev IterPtr : Word := (GuestAddrs.cvbgm_iter_ptr : Word)
abbrev IterI : Word := (GuestAddrs.cvbgm_iter_i : Word)
abbrev Field : Word := (GuestAddrs.cvbgm_field : Word)
...
/-- The `GAS_PER_BLOB - 1` low-bit mask: `GAS_PER_BLOB = 131072 = 2^17`, so
    `Mask = 131071 = 0x1ffff`, materialized by `lui x7, 32` (`32 <<< 12 = 2^17`)
    then `addiw x7, x7, -1`.  `blob_gas_used` is a multiple of `GAS_PER_BLOB`
    iff `(blob_gas_used &&& Mask) = 0`. -/
abbrev Mask : Word := (131071 : Word)

/-- Header `i` decodes field 17 to a u64 `value` that is a multiple of
    `GAS_PER_BLOB` (`value &&& Mask = 0`). -/
...
/-- Header `i` decodes field 17 to a u64 `value` that is NOT a multiple of
    `GAS_PER_BLOB` (`value &&& Mask ≠ 0`). -/
...
def postAllValid (sp0 spC calleeNewSp hdrBase lenBase validPtr firstBadPtr : Word)
  ...
  ⌜∀ j, j < lengths.length → hdrMultiple hdrBase bigBytes lengths j⌝ **
...
def postViolation (sp0 spC calleeNewSp hdrBase lenBase validPtr firstBadPtr : Word)
    (csaved : Saved) (bigBytes : List (BitVec 8)) (lengths : List Nat) : Assertion :=
  fun h => ∃ k,
    (⌜k < lengths.length ∧ (∀ j, j < k → hdrMultiple hdrBase bigBytes lengths j) ∧
        hdrNotMultiple hdrBase bigBytes lengths k⌝ **
...
def cvbgmPost (sp0 spC calleeNewSp hdrBase lenBase validPtr firstBadPtr : Word)
    (csaved : Saved) (bigBytes : List (BitVec 8)) (lengths : List Nat) : Assertion :=
  fun h =>
...
theorem cvbgmPrologue
    ...
    cpsTripleWithin 17 D (D + 68) cvbgmCode
...
theorem cvbgmEpilogue
  ...
  have hblock : cpsTripleWithin 8 (D + 236) (D + 268) cvbgmCode
  ...
  have hjalr := EvmAsm.Evm64.ret_spec_within' (D + 268) raIn
  rw [hret] at hjalr
  have hjalrC := cpsTripleWithin_extend_code
    (CodeReq.ofProg_mem_at D (D + 268) cvbgmProg 67 (.JALR .x0 .x1 (0 : BitVec 12))
      (by bv_omega) (by rw [cvbgm_length]; decide) rfl (by rw [cvbgm_length]; decide))
    hjalr
...
theorem retAllValid
    ...
    (hret : raIn &&& ~~~(1 : Word) = raIn) :
    cpsTripleWithin 10 (D + 232) raIn cvbgmCode
  ...
  subst hraSaved
  have h56 := li_spec_gen_within .x10 o10 (0 : Word) (D + 232) (by decide)
  have h56C := cpsTripleWithin_extend_code
    (CodeReq.ofProg_mem_at D (D + 232) cvbgmProg 58 (.LI .x10 (0 : Word))
      (by bv_omega) (by rw [cvbgm_length]; decide) rfl (by rw [cvbgm_length]; decide))
    h56
  ...
  have hepi := cvbgmEpilogue sp0 spC csaved.ra csaved.s0 csaved.s1 csaved.s2 csaved.s3
...
theorem retViolation
    ...
    (hraSaved : csaved.ra = raIn)
    (hret : raIn &&& ~~~(1 : Word) = raIn) :
    cpsTripleWithin 13 (D + 208) raIn cvbgmCode
      ...
      ((.x10 ↦ᵣ (0 : Word)) ** (validPtr ↦ₘ (0 : Word)) ** (firstBadPtr ↦ₘ iWord) **
        ...
        ((spC + 40) ↦ₘ csaved.s4) ** ((spC + 48) ↦ₘ csaved.s5) ** G) := by
  subst hraSaved
  have s50 := sd_spec_gen_own_within .x19 .x0 validPtr (0 : Word) (0 : BitVec 12) (D + 208)
  rw [show signExtend12 (0 : BitVec 12) = (0 : Word) from by decide,
    show validPtr + (0 : Word) = validPtr from by bv_omega] at s50
  have s51 := sd_spec_gen_own_within .x20 .x21 firstBadPtr iWord (0 : BitVec 12) (D + 212)
  rw [show signExtend12 (0 : BitVec 12) = (0 : Word) from by decide,
    show firstBadPtr + (0 : Word) = firstBadPtr from by bv_omega] at s51
  have s52 := li_spec_gen_within .x10 o10 (0 : Word) (D + 216) (by decide)
  have s53 := jal_x0_spec_gen_within (16 : BitVec 21) (D + 220)
  rw [show (D + 220) + signExtend21 (16 : BitVec 21) = D + 236 from by
    rw [show signExtend21 (16 : BitVec 21) = (16 : Word) from by decide]; bv_omega] at s53
  have hblock : cpsTripleWithin 4 (D + 208) (D + 236) cvbgmCode
...
theorem retParseFail
    ...
    (hraSaved : csaved.ra = raIn)
    (hret : raIn &&& ~~~(1 : Word) = raIn) :
    cpsTripleWithin 11 (D + 224) raIn cvbgmCode
      ...
      ((.x10 ↦ᵣ o10) ** (firstBadPtr ↦ₘ iWord) ** (.x1 ↦ᵣ raIn) **
        ...
        ((spC + 40) ↦ₘ csaved.s4) ** ((spC + 48) ↦ₘ csaved.s5) ** G) := by
  subst hraSaved
  have s54 := sd_spec_gen_own_within .x20 .x21 firstBadPtr iWord (0 : BitVec 12) (D + 224)
  rw [show signExtend12 (0 : BitVec 12) = (0 : Word) from by decide,
    show firstBadPtr + (0 : Word) = firstBadPtr from by bv_omega] at s54
  have s55 := jal_x0_spec_gen_within (8 : BitVec 21) (D + 228)
  rw [show (D + 228) + signExtend21 (8 : BitVec 21) = D + 236 from by
    rw [show signExtend21 (8 : BitVec 21) = (8 : Word) from by decide]; bv_omega] at s55
  have hblock : cpsTripleWithin 2 (D + 224) (D + 236) cvbgmCode
