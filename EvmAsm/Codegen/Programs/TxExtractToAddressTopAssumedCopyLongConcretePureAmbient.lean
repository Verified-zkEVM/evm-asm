/-
  Wire pure honesty into ambient long concrete Assumed creation.
...
/-- Ambient long concrete Assumed copy with pure-discharged hcur/hnext/hlen20/hnext_content/hinb/
    hoff/hover/hls/hll and long walk guards. Residual: hvalid*/hvalid1_*/hdec*/hlover/hlvalid. -/
theorem extractAssumed_copy_longConcrete_pure_ambient
  ...
  have hfields04 :=
    extractSuccess_copy_type234_hnext_fields04_long slice hsuccess hcopyFlag hge
      items hdecL hlong
  have hoverEnd :
      regionBase.toNat +
  ...
  have hhnext :=
    extractSuccess_copy_type234_hnext_hlen20_srcOff_long slice loadPtr hsuccess
  ...
  have hspan4a : regionBase.toNat + (off + srcOff4) < 2 ^ 64 := by
    have hs := hhover.2.2.2.2.1; omega
  have hspan5a : regionBase.toNat + (off + srcOff5) < 2 ^ 64 := by
    have hs := hhover.2.2.2.2.2; omega
  ...
  have hroom5 : srcOff5 + 1 < slice.length :=
    longListSrcOff_succ_room slice listOff items 5 hencInner hlong (by omega)
  ...
  have hnext5 : ∀ (next4 len4 : Word),
    ...
    simpa [absOff4, absOff5, srcOff4, srcOff5, listOff, endW, absListOff] using
      packaging_hnext_ambient_field_long regionBase loadPtr bs off len listOff items 4 5
        endW hptr hbound hencInner hlong hn4 hitem4 hoff4_sl hspan4a hspan5a hroom4
  ...
  have hlen20 : ∀ (next5 len5 : Word),
    ...
    simpa [absOff5, srcOff5, listOff, endW, absListOff] using
      hlen20_ambient_of_slice_long regionBase loadPtr bs off len listOff items 5 endW
        hptr hbound hencInner hlong hn5 hitem5 hoff5_sl hspan5a hroom5
  ...
  have hnext_sl_content : ∀ (next5 len5 : Word),
    ...
    intro n l hd
    let contentPtrSl := loadPtr + BitVec.ofNat 64 srcOff5 + (1 : Word)
    have hcontentPtr : contentPtrSl =
        loadPtr + BitVec.ofNat 64 srcOff5 + (1 : Word) := rfl
    have hsrc : srcOff5 = longListSrcOff listOff items 5 := rfl
    have hc := extractSuccess_copy_type234_hnext_content_long slice loadPtr contentPtrSl
      srcOff5 hsuccess hcopyFlag hge items hdecL hlong hsrc hcontentPtr endW n l hd
    simpa [contentPtrSl, BitVec.add_assoc] using hc
  ...
  have hnext_content : ∀ (next5 len5 : Word),
    ...
    simpa [absOff5, srcOff5, listOff, endW, absListOff] using
      hnext_content_ambient_of_slice_long regionBase loadPtr bs off len listOff items 5 q endW
        hptr hbound hencInner hlong hn5 hitem5 hoff5_sl hspan5a hroom5 hq_abs hcover20
        hnext_sl_content
  have hinb0 :=
  ...
  have hinb5 :=
    hinb_ambient_long_list_end regionBase off listOff items 5 hn5 hoverEnd endW hendEq
  ...
  have hss0 :=
    hss_ambient_of_long_list_field regionBase loadPtr bs off len listOff items 0
      hptr hbound hencInner hlong hn0 hitem0 hoff0_sl hover (Or.inl hfields04.1)
      hvalid1_0 hoff0
  have hss1 :=
    hss_ambient_of_long_list_field regionBase loadPtr bs off len listOff items 1
      hptr hbound hencInner hlong hn1 hitem1 hoff1_sl hover (Or.inl hfields04.2.1)
      hvalid1_1 hoff1
  have hss2 :=
    hss_ambient_of_long_list_field regionBase loadPtr bs off len listOff items 2
      hptr hbound hencInner hlong hn2 hitem2 hoff2_sl hover (Or.inl hfields04.2.2.1)
      hvalid1_2 hoff2
  have hss3 :=
    hss_ambient_of_long_list_field regionBase loadPtr bs off len listOff items 3
      hptr hbound hencInner hlong hn3 hitem3 hoff3_sl hover (Or.inl hfields04.2.2.2.1)
      hvalid1_3 hoff3
  have hss4 :=
    hss_ambient_of_long_list_field regionBase loadPtr bs off len listOff items 4
      hptr hbound hencInner hlong hn4 hitem4 hoff4_sl hover (Or.inl hfields04.2.2.2.2)
      hvalid1_4 hoff4
  have hss5 :=
    hss_ambient_of_long_list_field regionBase loadPtr bs off len listOff items 5
      hptr hbound hencInner hlong hn5 hitem5 hoff5_sl hover (Or.inl (by omega : 6 < items.length))
      hvalid1_5 hoff5
  have hls0 :
  ...
  have hls5 :
      ¬ BitVec.ult ((bs[absOff5]'hoff5).zeroExtend 64) (0xb8 : Word) = true →
        ...
        ∀ j, j < ((bs[absOff5]'hoff5).zeroExtend 64 - (0xb7 : Word)).toNat →
          isValidByteAccess (regionBase + BitVec.ofNat 64 (absOff5 + 1 + j)) = true := by
    intro hlo hhi
    ...
    have hlo' := hlo; rw [heq] at hlo'
    have hhi' := hhi; rw [heq] at hhi'
    exact hls_vacuous_of_long_list_item slice listOff items 5
      hencInner hlong hn5 hitem5 hoff5_sl hlo' hhi'
  have hll5 :
      ¬ BitVec.ult ((bs[absOff5]'hoff5).zeroExtend 64) (0xf8 : Word) = true →
        ...
        ∀ j, j < ((bs[absOff5]'hoff5).zeroExtend 64 - (0xf7 : Word)).toNat →
          isValidByteAccess (regionBase + BitVec.ofNat 64 (absOff5 + 1 + j)) = true := by
    intro hgeF8
    have heq : bs[absOff5]'hoff5 = slice[srcOff5]'hoff5_sl := by
      simp only [absOff5, ambientAbsOff, srcOff5]
      ...
      exact (txSlice_getElem_eq bs off len srcOff5 hbound hrel hoff5_sl hoff5).symm
    have h' := hgeF8; rw [heq] at h'
    exact hll_vacuous_of_long_list_item slice listOff items 5
      hencInner hlong hn5 hitem5 hoff5_sl h'
  have hoff := hoff_list
  have hspan : regionBase.toNat + (off + listOff) < 2 ^ 64 := hspan_list
  exact extractAssumed_copy_of_front_long_concrete_region_ambient
    sp0 spC s regionBase loadPtr lenW toBuf isCreationPtr bs off len
    absOff0 absOff1 absOff2 absOff3 absOff4 absOff5 q
    hspC hret hoff hcur hne0 hne1 hsalign
...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_longConcrete_pure_ambient_fullCode
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (items : List EL.RLP.RLPItem)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_copy_longConcrete_pure_ambient sp0 spC s
...
#print axioms extractAssumed_copy_longConcrete_pure_ambient
#print axioms extractAssumed_copy_longConcrete_pure_ambient_fullCode
