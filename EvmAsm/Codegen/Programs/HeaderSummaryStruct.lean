...
/-! ## header_compute_summary_struct -- PR-K214
    ...
    decodes four scalar fields with `rlp_content_to_u64_strict` (avoiding
    four full `rlp_field_to_u64` rescans). Field-to-call mapping is taken
    from the emitted body below (four `jal ra, rlp_content_to_u64_strict`
    sites; zero lenient `rlp_content_to_u64` sites):
    ...
    All four strict is correct per execution-specs amsterdam `Header`
    (`blocks.py`): number/gas_used/base_fee_per_gas are `Uint` and
    timestamp is `U256`; both RLP deserialize paths reject non-canonical
    scalar encodings via `_deserialize_to_uint`. Canonicality-only —
    guest width/representability assumptions for these fields are
    unchanged.
