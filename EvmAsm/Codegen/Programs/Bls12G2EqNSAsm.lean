...
#guard GuestAddrs.blsg2_eq_n = 0x80033d58
...
theorem blsg2EqN_spec (ptr1 ptr2 ret : Word) (bs1 bs2 : List (BitVec 8))
    ...
    cpsTripleWithin (n * 8 + 7) (0x80033d58 : Word) ret
      (CodeReq.ofProg (0x80033d58 : Word) blsg2EqN_prog)
  ...
  set CR := CodeReq.ofProg (0x80033d58 : Word) blsg2EqN_prog with hCR
  ...
  have hmv6 := liftCode (cr' := CR)
    (mv_spec_gen_within .x6 .x10 ptr1 v6 (0x80033d58 : Word) (by decide))
    (by rw [hCR]; code_mem)
  rw [show (0x80033d58 : Word) + 4 = (0x80033d5c : Word) from by decide] at hmv6
  have hmv7 := liftCode (cr' := CR)
    (mv_spec_gen_within .x7 .x11 ptr2 v7 (0x80033d5c : Word) (by decide))
    (by rw [hCR]; code_mem)
  rw [show (0x80033d5c : Word) + 4 = (0x80033d60 : Word) from by decide] at hmv7
  have hmv5 := liftCode (cr' := CR)
    (mv_spec_gen_within .x5 .x12 (BitVec.ofNat 64 n) v5 (0x80033d60 : Word)
      (by decide))
    (by rw [hCR]; code_mem)
  rw [show (0x80033d60 : Word) + 4 = (0x80033d64 : Word) from by decide] at hmv5
  -- ---- the dynamic-length byte dual-read scan (lifted into CR) ----
  have hscan := cpsTripleWithin_extend_code (cr' := CR)
    (hmono := by
      rw [hCR]
      exact CodeReq.ofProg_mono_sub (0x80033d58 : Word) (0x80033d64 : Word)
        blsg2EqN_prog (DualReadByteScan.byteScanProg .x5 .x28 .x29 .x6 .x7) 3
        (by decide) (by decide) (by decide) (by decide))
    (h := DualReadByteScan.scan_spec .x5 .x28 .x29 .x6 .x7
      (0x80033d64 : Word) ret ptr1 ptr2 bs1 bs2 n
