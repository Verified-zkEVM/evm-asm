...
theorem cvcn_length : cvcnProg.length = 93 := by decide
...
theorem cvcn_mono : ∀ a i, cvcnCode a = some i → fullCode a = some i := by
  intro a i hi
  unfold fullCode
...
abbrev Num : Word := (GuestAddrs.cvcn_num : Word)
abbrev IterChild : Word := (GuestAddrs.cvcn_iter_child : Word)
abbrev IterI : Word := (GuestAddrs.cvcn_iter_i : Word)
abbrev IterPrev : Word := (GuestAddrs.cvcn_iter_prev : Word)
...
/-- Header `i` decodes RLP field 8 (`number`) to a u64 `value`, with status
...
/-- Adjacent pair `(i-1, i)` is consecutive: both headers decode field 8 to a
    u64 (status 0) and `num[i] = num[i-1] + 1` in `BitVec 64` (wrapping add) —
    exactly the relation the guest checks (`ADDI x29 x29 1` then `BNE x29 x28`
    not-taken).  This matches the execution-spec's `header.number =
    parent.number + 1` on all reachable block numbers (the only `BitVec`-vs-`Nat`
    divergence would be the unreachable wraparound at `num[i-1] = 2^64 - 1`, for
    which the successor `2^64` cannot be a u64-decoded field). -/
def numConsecutive (hdrBase : Word) (bigBytes : List (BitVec 8)) (lengths : List Nat)
    (i : Nat) : Prop :=
  ∃ prev cur, hdrNumOk hdrBase bigBytes lengths (i - 1) prev ∧
    hdrNumOk hdrBase bigBytes lengths i cur ∧ cur = prev + 1

/-- Adjacent pair `(k-1, k)` violates consecutiveness: both headers decode field
    8 to a u64 (status 0) but `num[k] ≠ num[k-1] + 1` (`BitVec 64`) — exactly the
    guest's `BNE x29 x28` taken condition after `x29 := prev + 1`. -/
def numViolation (hdrBase : Word) (bigBytes : List (BitVec 8)) (lengths : List Nat)
    (k : Nat) : Prop :=
  ∃ prev cur, hdrNumOk hdrBase bigBytes lengths (k - 1) prev ∧
    hdrNumOk hdrBase bigBytes lengths k cur ∧ cur ≠ prev + 1

/-- Header `k` fails the field-8 u64 decode (status ≠ 0). -/
...
/-- Static memory/register footprint carried unchanged through the whole loop:
    ...
    (owned).  `cvcn_iter_i` (`IterI`) starts zeroed so the header-0 parse-fail
    reports index 0. -/
...
def LoopInv (_sp0 spC calleeNewSp hdrBase lenBase validPtr firstBadPtr : Word)
    ...
    (i : Nat) : Assertion :=
  fun h => ∃ prevVal,
    (⌜hdrNumOk hdrBase bigBytes lengths (i - 1) prevVal ∧
        (∀ j, 1 ≤ j → j < i → numConsecutive hdrBase bigBytes lengths j)⌝ **
...
def postAllValid (sp0 spC calleeNewSp hdrBase lenBase validPtr firstBadPtr : Word)
  ...
  ⌜lengths.length < 2 ∨
    (∀ i, 1 ≤ i → i < lengths.length → numConsecutive hdrBase bigBytes lengths i)⌝ **
...
def postViolation (sp0 spC calleeNewSp hdrBase lenBase validPtr firstBadPtr : Word)
  ...
  fun h => ∃ k,
    (⌜1 ≤ k ∧ k < lengths.length ∧
        (∀ j, 1 ≤ j → j < k → numConsecutive hdrBase bigBytes lengths j) ∧
        numViolation hdrBase bigBytes lengths k⌝ **
...
def cvcnPost (sp0 spC calleeNewSp hdrBase lenBase validPtr firstBadPtr : Word)
    (csaved : Saved) (bigBytes : List (BitVec 8)) (lengths : List Nat) : Assertion :=
  fun h =>
...
theorem cvcnPrologue
    ...
    cpsTripleWithin 17 D (D + 68) cvcnCode
...
theorem cvcnEpilogue
  ...
  have hblock : cpsTripleWithin 8 (D + 336) (D + 368) cvcnCode
  ...
  have hjalr := EvmAsm.Evm64.ret_spec_within' (D + 368) raIn
  rw [hret] at hjalr
  have hjalrC := cpsTripleWithin_extend_code
    (CodeReq.ofProg_mem_at D (D + 368) cvcnProg 92 (.JALR .x0 .x1 (0 : BitVec 12))
      (by bv_omega) (by rw [cvcn_length]; decide) rfl (by rw [cvcn_length]; decide))
    hjalr
...
theorem retAllValid
    ...
    (hret : raIn &&& ~~~(1 : Word) = raIn) :
    cpsTripleWithin 10 (D + 332) raIn cvcnCode
  ...
  subst hraSaved
  have h82 := li_spec_gen_within .x10 o10 (0 : Word) (D + 332) (by decide)
  have h82C := cpsTripleWithin_extend_code
    (CodeReq.ofProg_mem_at D (D + 332) cvcnProg 83 (.LI .x10 (0 : Word))
      (by bv_omega) (by rw [cvcn_length]; decide) rfl (by rw [cvcn_length]; decide))
    h82
  ...
  have hepi := cvcnEpilogue sp0 spC csaved.ra csaved.s0 csaved.s1 csaved.s2 csaved.s3
...
/-! ## Violation exit (instructions 70--76 → epilogue)
    ...
    `*validPtr := 0`, then reload `x6 := *cvcn_iter_i` (the failing index),
...
theorem retViolation
    ...
    (hraSaved : csaved.ra = raIn)
    (hret : raIn &&& ~~~(1 : Word) = raIn) :
    cpsTripleWithin 16 (D + 284) raIn cvcnCode
      ...
      ((.x10 ↦ᵣ (0 : Word)) ** (validPtr ↦ₘ (0 : Word)) ** (firstBadPtr ↦ₘ iVal) **
        ...
        ((spC + 40) ↦ₘ csaved.s4) ** ((spC + 48) ↦ₘ csaved.s5) ** G) := by
  subst hraSaved
  have s70 := sd_spec_gen_own_within .x19 .x0 validPtr (0 : Word) (0 : BitVec 12) (D + 284)
  rw [show signExtend12 (0 : BitVec 12) = (0 : Word) from by decide,
    show validPtr + (0 : Word) = validPtr from by bv_omega] at s70
  have s70' := cpsTripleWithin_extend_code
    (CodeReq.ofProg_mem_at D (D + 284) cvcnProg 71 (.SD .x19 .x0 (0 : BitVec 12))
      (by bv_omega) (by rw [cvcn_length]; decide) rfl (by rw [cvcn_length]; decide)) s70
  have hla71 := la_materialize_within .x5 old5 (D + 288) IterI (by decide) (by decide)
    (CodeReq.ofProg_mem_at D (D + 288) cvcnProg 72 (.AUIPC .x5 (EvmAsm.Rv64.laHi (D + 288) IterI)) (by bv_omega) (by rw [cvcn_length]; decide) (by decide) (by rw [cvcn_length]; decide))
    (CodeReq.ofProg_mem_at D (D + 292) cvcnProg 73 (.ADDI .x5 .x5 (EvmAsm.Rv64.laLo (D + 288) IterI)) (by bv_omega) (by rw [cvcn_length]; decide) (by decide) (by rw [cvcn_length]; decide))
  have s73 := ld_spec_gen_within .x6 .x5 IterI o6 iVal (0 : BitVec 12) (D + 296) (by decide)
  rw [show signExtend12 (0 : BitVec 12) = (0 : Word) from by decide,
    show IterI + (0 : Word) = IterI from by bv_omega] at s73
  have s73' := cpsTripleWithin_extend_code
    (CodeReq.ofProg_mem_at D (D + 296) cvcnProg 74 (.LD .x6 .x5 (0 : BitVec 12))
      (by bv_omega) (by rw [cvcn_length]; decide) rfl (by rw [cvcn_length]; decide)) s73
  have s74 := sd_spec_gen_own_within .x20 .x6 firstBadPtr iVal (0 : BitVec 12) (D + 300)
  rw [show signExtend12 (0 : BitVec 12) = (0 : Word) from by decide,
    show firstBadPtr + (0 : Word) = firstBadPtr from by bv_omega] at s74
  have s74' := cpsTripleWithin_extend_code
    (CodeReq.ofProg_mem_at D (D + 300) cvcnProg 75 (.SD .x20 .x6 (0 : BitVec 12))
      (by bv_omega) (by rw [cvcn_length]; decide) rfl (by rw [cvcn_length]; decide)) s74
  have s75 := li_spec_gen_within .x10 o10 (0 : Word) (D + 304) (by decide)
  have s75' := cpsTripleWithin_extend_code
    (CodeReq.ofProg_mem_at D (D + 304) cvcnProg 76 (.LI .x10 (0 : Word))
      (by bv_omega) (by rw [cvcn_length]; decide) rfl (by rw [cvcn_length]; decide)) s75
  have s76 := jal_x0_spec_gen_within (28 : BitVec 21) (D + 308)
  rw [show (D + 308) + signExtend21 (28 : BitVec 21) = D + 336 from by
    rw [show signExtend21 (28 : BitVec 21) = (28 : Word) from by decide]; bv_omega] at s76
  have s76' := cpsTripleWithin_extend_code
    (CodeReq.ofProg_mem_at D (D + 308) cvcnProg 77 (.JAL .x0 (28 : BitVec 21))
      (by bv_omega) (by rw [cvcn_length]; decide) rfl (by rw [cvcn_length]; decide)) s76
  have hblock : cpsTripleWithin 7 (D + 284) (D + 336) cvcnCode
...
/-! ## Parse-fail exit (instructions 77--81 → epilogue)
    ...
    Reload `x6 := *cvcn_iter_i`, `*firstBadPtr := x6`, `jal +8` to the epilogue.
...
theorem retParseFail
    ...
    (hraSaved : csaved.ra = raIn)
    (hret : raIn &&& ~~~(1 : Word) = raIn) :
    cpsTripleWithin 14 (D + 312) raIn cvcnCode
      ...
      ((.x10 ↦ᵣ o10) ** (firstBadPtr ↦ₘ iVal) ** (IterI ↦ₘ iVal) **
        ...
        ((spC + 40) ↦ₘ csaved.s4) ** ((spC + 48) ↦ₘ csaved.s5) ** G) := by
  subst hraSaved
  have hla77 := la_materialize_within .x5 old5 (D + 312) IterI (by decide) (by decide)
    (CodeReq.ofProg_mem_at D (D + 312) cvcnProg 78 (.AUIPC .x5 (EvmAsm.Rv64.laHi (D + 312) IterI)) (by bv_omega) (by rw [cvcn_length]; decide) (by decide) (by rw [cvcn_length]; decide))
    (CodeReq.ofProg_mem_at D (D + 316) cvcnProg 79 (.ADDI .x5 .x5 (EvmAsm.Rv64.laLo (D + 312) IterI)) (by bv_omega) (by rw [cvcn_length]; decide) (by decide) (by rw [cvcn_length]; decide))
  have s79 := ld_spec_gen_within .x6 .x5 IterI o6 iVal (0 : BitVec 12) (D + 320) (by decide)
  rw [show signExtend12 (0 : BitVec 12) = (0 : Word) from by decide,
    show IterI + (0 : Word) = IterI from by bv_omega] at s79
  have s79' := cpsTripleWithin_extend_code
    (CodeReq.ofProg_mem_at D (D + 320) cvcnProg 80 (.LD .x6 .x5 (0 : BitVec 12))
      (by bv_omega) (by rw [cvcn_length]; decide) rfl (by rw [cvcn_length]; decide)) s79
  have s80 := sd_spec_gen_own_within .x20 .x6 firstBadPtr iVal (0 : BitVec 12) (D + 324)
  rw [show signExtend12 (0 : BitVec 12) = (0 : Word) from by decide,
    show firstBadPtr + (0 : Word) = firstBadPtr from by bv_omega] at s80
  have s80' := cpsTripleWithin_extend_code
    (CodeReq.ofProg_mem_at D (D + 324) cvcnProg 81 (.SD .x20 .x6 (0 : BitVec 12))
      (by bv_omega) (by rw [cvcn_length]; decide) rfl (by rw [cvcn_length]; decide)) s80
  have s81 := jal_x0_spec_gen_within (8 : BitVec 21) (D + 328)
  rw [show (D + 328) + signExtend21 (8 : BitVec 21) = D + 336 from by
    rw [show signExtend21 (8 : BitVec 21) = (8 : Word) from by decide]; bv_omega] at s81
  have s81' := cpsTripleWithin_extend_code
    (CodeReq.ofProg_mem_at D (D + 328) cvcnProg 82 (.JAL .x0 (8 : BitVec 21))
      (by bv_omega) (by rw [cvcn_length]; decide) rfl (by rw [cvcn_length]; decide)) s81
  have hblock : cpsTripleWithin 5 (D + 312) (D + 336) cvcnCode
