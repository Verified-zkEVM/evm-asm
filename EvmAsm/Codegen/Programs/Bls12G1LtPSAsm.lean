/-
  EvmAsm.Codegen.Programs.Bls12G1LtPSAsm

  `blsg_lt_p` — the EIP-2537 BLS12-381 coordinate range check — via the
  ...
  **Genuine post**: `a0 = if beBytesToNat in < beBytesToNat bls12PBytes
  then 1 else 0` — the REAL numeric strict comparison against the BN254
  base-field prime (pinned by `#guard` to
  `0x1a0111ea…aaab = p_{BLS12-381}`), the input and the `globalConst` prime
  region untouched.

  Byte-transparent: the spec is stated at the `#guard`-tied symbolic
  `GuestAddrs.blsg_lt_p` over the emitted `blsgLtP_prog` directly — no
...
#guard GuestAddrs.blsg_lt_p = 0x8002f784
#guard GuestAddrs.blsg_p_be = 0xbb55df08
...
#guard bls12PBytes.length = 48

-- The constant bytes ARE the BLS12-381 base-field prime.
#guard beBytesToNat bls12PBytes =
...
theorem ltP_laHi_agree :
    Codegen.laHi GuestAddrs.blsg_p_be (GuestAddrs.blsg_lt_p + 0)
      = EvmAsm.Rv64.laHi ltPBase pConstAddr := by decide

theorem ltP_laLo_agree :
    Codegen.laLo GuestAddrs.blsg_p_be (GuestAddrs.blsg_lt_p + 0)
      = EvmAsm.Rv64.laLo ltPBase pConstAddr := by decide

/-
  Emitted layout relative to `GuestAddrs.blsg_lt_p`:
    +0   auipc x5, %pcrel_hi(blsg_p_be)
...
private theorem counter_dec (i : Nat) (hi : i < 48) :
    BitVec.ofNat 64 (48 - i) + signExtend12 (-1 : BitVec 12)
      = BitVec.ofNat 64 (48 - (i + 1)) := by
...
private theorem ctr_ne_zero (i : Nat) (hi : i < 48) :
    ¬ (BitVec.ofNat 64 (48 - i) = (0 : Word)) := by
  intro h
...
private theorem pByte_valid (k : Nat) (hk : k < 48) :
...
/-- Loop invariant at the header after `i` matched bytes: the counter holds
    `48 - i`, both cursors sit at byte `i` (the const-side cursor walks the
...
private def ltInv (i : Nat) : Assertion :=
  ⌜∀ j, j < i → xs.getD j 0 = bls12PBytes.getD j 0⌝ **
  ((.x6 : Reg) ↦ᵣ BitVec.ofNat 64 (48 - i)) **
  ((.x7 : Reg) ↦ᵣ (inPtr + BitVec.ofNat 64 i)) **
  ((.x5 : Reg) ↦ᵣ (pConstAddr + BitVec.ofNat 64 i)) **
  ((.x10 : Reg) ↦ᵣ inPtr) **
  ((.x1 : Reg) ↦ᵣ ret) ** ((.x0 : Reg) ↦ᵣ (0 : Word)) **
  regOwn .x28 ** regOwn .x29 **
  bytesRegion inPtr xs ** bytesRegion pConstAddr bls12PBytes

/-- The genuine post: `a0` is the REAL numeric strict comparison of the
    48-byte big-endian input against the prime; the input and the
    constant region untouched. -/
private def ltPost : Assertion :=
  ((.x10 : Reg) ↦ᵣ (if beBytesToNat xs < beBytesToNat bls12PBytes
...
/-- One iteration at the header with `i < 48` bytes known equal: either a
...
private theorem ltIter_spec
    ...
    cpsBranchWithin 9 (ltPBase + 16)
      (CodeReq.ofProg ltPBase blsgLtP_prog)
      (ltInv inPtr ret xs i)
      ret (ltPost inPtr ret xs)
      (ltPBase + 16) (ltInv inPtr ret xs (i + 1)) := by
  set CR := CodeReq.ofProg ltPBase blsgLtP_prog with hCR
  have hplen : bls12PBytes.length = 48 := by decide
  have hix : i < xs.length := by omega
  have hip : i < bls12PBytes.length := by omega
  set xByte := (xs[i]'hix).zeroExtend 64 with hxByte
  set pByte := (bls12PBytes[i]'hip).zeroExtend 64 with hpByte
  have hxBN : xByte.toNat = (xs[i]'hix).toNat := by
    ...
    have := (xs[i]'hix).isLt
    omega
  have hpBN : pByte.toNat = (bls12PBytes[i]'hip).toNat := by
    rw [hpByte]
    show (BitVec.setWidth 64 _).toNat = _
    rw [BitVec.toNat_setWidth]
    have := (bls12PBytes[i]'hip).isLt
    omega
  have hgdX : xs.getD i 0 = xs[i]'hix := by
    rw [List.getD_eq_getElem?_getD, List.getElem?_eq_getElem hix]
    rfl
  have hgdP : bls12PBytes.getD i 0 = bls12PBytes[i]'hip := by
  ...
  refine cpsBranchWithin_weaken (fun h hp => by xperm_hyp hp)
    ...
    (cpsBranchWithin_of_forall_regIs_to_regOwn (r := .x28)
      (P := (((.x6 : Reg) ↦ᵣ BitVec.ofNat 64 (48 - i)) **
        ((.x7 : Reg) ↦ᵣ (inPtr + BitVec.ofNat 64 i)) **
        ((.x5 : Reg) ↦ᵣ (pConstAddr + BitVec.ofNat 64 i)) **
        ((.x10 : Reg) ↦ᵣ inPtr) **
        ((.x1 : Reg) ↦ᵣ ret) ** ((.x0 : Reg) ↦ᵣ (0 : Word)) **
  ...
  have hlbuP := liftCode (cr' := CR)
    (bytesRegion_lbu_within .x29 .x5 pConstAddr v29 (ltPBase + 24)
      ...
      (by have h : pConstAddr.toNat = 0xbb55df08 := by decide
          omega)
      (pByte_valid i hi))
    (by rw [hCR]; code_mem)
  ...
  have hlbuXF := cpsTripleWithin_frameR
    (((.x6 : Reg) ↦ᵣ BitVec.ofNat 64 (48 - i)) **
      ...
      ((.x1 : Reg) ↦ᵣ ret) ** ((.x0 : Reg) ↦ᵣ (0 : Word)) **
      ((.x29 : Reg) ↦ᵣ v29) **
      bytesRegion pConstAddr bls12PBytes)
    (by pcf) hlbuX
  ...
  -- ---- the header BEQ station (+16; never taken at i < 48) ----
  have hbrHdr := cpsBranchWithin_frameR
    ...
    (cpsBranchWithin_extend_code (cr' := CR)
      (h := beq_spec_gen_within .x6 .x0 (44 : BitVec 13)
        (BitVec.ofNat 64 (48 - i)) (0 : Word) (ltPBase + 16))
  ...
  set WSL : Assertion :=
    ...
    ((.x10 : Reg) ↦ᵣ inPtr) **
    ((.x1 : Reg) ↦ᵣ ret) ** ((.x0 : Reg) ↦ᵣ (0 : Word)) **
    bytesRegion inPtr xs ** bytesRegion pConstAddr bls12PBytes with hWSL
  ...
  have htailOne : BitVec.ult xByte pByte →
    ...
    intro hc
    have hltN : (xs[i]'hix).toNat < (bls12PBytes[i]'hip).toNat := by
      have hc' : xByte.toNat < pByte.toNat := by
        simpa [BitVec.ult, decide_eq_true_eq] using hc
      omega
    have hlt : beBytesToNat xs < beBytesToNat bls12PBytes :=
      beBytesToNat_lt_of_prefix_lt xs bls12PBytes (by omega) i hix hpref
            ...
            (((.x7 : Reg) ↦ᵣ (inPtr + BitVec.ofNat 64 i)) **
              (((.x28 : Reg) ↦ᵣ xByte) ** (((.x29 : Reg) ↦ᵣ pByte) **
                (((.x10 : Reg) ↦ᵣ (1 : Word)) **
                 ((.x1 : Reg) ↦ᵣ ret) ** ((.x0 : Reg) ↦ᵣ (0 : Word)) **
                 bytesRegion inPtr xs **
                 bytesRegion pConstAddr bls12PBytes)))))) h := by
        xperm_hyp hq
  ...
  have htailZeroGt : ¬ BitVec.ult xByte pByte → BitVec.ult pByte xByte →
      cpsTripleWithin 4 (ltPBase + 60) ret CR WSL
        (ltPost inPtr ret xs) := by
    intro _ hc
    have hgtN : (bls12PBytes[i]'hip).toNat < (xs[i]'hix).toNat := by
      have hc' : pByte.toNat < xByte.toNat := by
        simpa [BitVec.ult, decide_eq_true_eq] using hc
      omega
    have hgt : beBytesToNat bls12PBytes < beBytesToNat xs :=
      beBytesToNat_lt_of_prefix_lt bls12PBytes xs (by omega) i hip
        (fun j hj => (hpref j hj).symm)
        (by rw [hgdX, hgdP]; omega)
    have hnlt : ¬ (beBytesToNat xs < beBytesToNat bls12PBytes) := by
      omega
    have h := cpsTripleWithin_frameR
      (((.x28 : Reg) ↦ᵣ xByte) ** ((.x29 : Reg) ↦ᵣ pByte) **
        ((.x6 : Reg) ↦ᵣ BitVec.ofNat 64 (48 - i)) **
  ...
  have hcont : ¬ BitVec.ult xByte pByte → ¬ BitVec.ult pByte xByte →
    ...
    intro hnXP hnPX
    have hEqByte : xs[i]'hix = bls12PBytes[i]'hip := by
      apply BitVec.eq_of_toNat_eq
      ...
      omega
    have hpref' : ∀ j, j < i + 1 → xs.getD j 0 = bls12PBytes.getD j 0 := by
      intro j hj
      by_cases hji : j < i
      · exact hpref j hji
      · have : j = i := by omega
    ...
    have haddi6 := liftCode (cr' := CR)
      (addi_spec_gen_same_within .x6 (BitVec.ofNat 64 (48 - i)) (-1 : BitVec 12)
    ...
    have hjalF := cpsTripleWithin_frameR
      (((.x6 : Reg) ↦ᵣ BitVec.ofNat 64 (48 - (i + 1))) **
        ((.x7 : Reg) ↦ᵣ (inPtr + BitVec.ofNat 64 (i + 1))) **
        ((.x5 : Reg) ↦ᵣ (pConstAddr + BitVec.ofNat 64 (i + 1))) **
        ((.x28 : Reg) ↦ᵣ xByte) ** ((.x29 : Reg) ↦ᵣ pByte) **
        ((.x10 : Reg) ↦ᵣ inPtr) **
    ...
    have hq1 : (((.x28 : Reg) ↦ᵣ xByte) ** (((.x29 : Reg) ↦ᵣ pByte) **
        (((.x6 : Reg) ↦ᵣ BitVec.ofNat 64 (48 - (i + 1))) **
         ...
         ((.x10 : Reg) ↦ᵣ inPtr) **
         ((.x1 : Reg) ↦ᵣ ret) ** ((.x0 : Reg) ↦ᵣ (0 : Word)) **
         bytesRegion inPtr xs ** bytesRegion pConstAddr bls12PBytes))) h := by
      xperm_hyp hq
  ...
  refine cpsBranchWithin_weaken (fun h hp => by xperm_hyp hp)
    (fun _ hq => hq) (fun _ hq => hq)
    (breakStation_spec (cond := (BitVec.ofNat 64 (48 - i) = (0 : Word)))
      (PT := ((.x6 : Reg) ↦ᵣ BitVec.ofNat 64 (48 - i)) **
        ((.x7 : Reg) ↦ᵣ (inPtr + BitVec.ofNat 64 i)) **
        ((.x5 : Reg) ↦ᵣ (pConstAddr + BitVec.ofNat 64 i)) **
        ((.x10 : Reg) ↦ᵣ inPtr) **
        ((.x1 : Reg) ↦ᵣ ret) ** ((.x0 : Reg) ↦ᵣ (0 : Word)) **
...
-- ============================================================================
-- §4  Loop exhaustion: all 48 bytes equal → zero tail returns 0
-- ============================================================================
...
private theorem ltExh_spec
    ...
    cpsTripleWithin 5 (ltPBase + 16) ret
      (CodeReq.ofProg ltPBase blsgLtP_prog)
      (ltInv inPtr ret xs 48)
      (ltPost inPtr ret xs) := by
  set CR := CodeReq.ofProg ltPBase blsgLtP_prog with hCR
  have hplen : bls12PBytes.length = 48 := by decide
  unfold ltInv
  refine cpsTripleWithin_pure_pre (fun hpref => ?_)
  have hEq : xs = bls12PBytes := bytes_eq_of_prefix_all xs bls12PBytes
    (by omega) (fun j hj => hpref j (by omega))
  have heqN : beBytesToNat xs = beBytesToNat bls12PBytes := by rw [hEq]
  have hnlt : ¬ (beBytesToNat xs < beBytesToNat bls12PBytes) := by omega
  -- header BEQ, taken (counter = 0)
  have hbrHdr := cpsBranchWithin_frameR
    (((.x7 : Reg) ↦ᵣ (inPtr + BitVec.ofNat 64 48)) **
      ((.x5 : Reg) ↦ᵣ (pConstAddr + BitVec.ofNat 64 48)) **
      ((.x10 : Reg) ↦ᵣ inPtr) **
      ((.x1 : Reg) ↦ᵣ ret) **
      regOwn .x28 ** regOwn .x29 **
      bytesRegion inPtr xs ** bytesRegion pConstAddr bls12PBytes)
    ...
    (cpsBranchWithin_extend_code (cr' := CR)
      (h := beq_spec_gen_within .x6 .x0 (44 : BitVec 13)
        (BitVec.ofNat 64 (48 - 48)) (0 : Word) (ltPBase + 16))
      (hmono := by rw [hCR]; code_mem))
  rw [show (ltPBase + 16 : Word) + signExtend13 (44 : BitVec 13)
        = (ltPBase + 60 : Word) from by decide,
      show (ltPBase + 16 : Word) + 4 = (ltPBase + 20 : Word) from by decide]
  ...
  have htail : cpsTripleWithin 4 (ltPBase + 60) ret CR
      (((.x6 : Reg) ↦ᵣ BitVec.ofNat 64 (48 - 48)) **
       ((.x7 : Reg) ↦ᵣ (inPtr + BitVec.ofNat 64 48)) **
       ((.x5 : Reg) ↦ᵣ (pConstAddr + BitVec.ofNat 64 48)) **
       ((.x10 : Reg) ↦ᵣ inPtr) **
       ((.x1 : Reg) ↦ᵣ ret) ** ((.x0 : Reg) ↦ᵣ (0 : Word)) **
    ...
    · unfold ltPost
      rw [if_neg hnlt]
      have hq1 : (((.x5 : Reg) ↦ᵣ (pConstAddr + BitVec.ofNat 64 48)) **
          (((.x6 : Reg) ↦ᵣ BitVec.ofNat 64 (48 - 48)) **
            (((.x7 : Reg) ↦ᵣ (inPtr + BitVec.ofNat 64 48)) **
              (((.x10 : Reg) ↦ᵣ (0 : Word)) **
               ((.x1 : Reg) ↦ᵣ ret) ** ((.x0 : Reg) ↦ᵣ (0 : Word)) **
  ...
  refine cpsTripleWithin_weaken (fun h hp => by xperm_hyp hp)
    (fun _ hq => hq)
    (retJoinStation_spec
      (cond := (BitVec.ofNat 64 (48 - 48) = (0 : Word)))
      (PT := (((.x6 : Reg) ↦ᵣ BitVec.ofNat 64 (48 - 48)) **
        ((.x7 : Reg) ↦ᵣ (inPtr + BitVec.ofNat 64 48)) **
        ((.x5 : Reg) ↦ᵣ (pConstAddr + BitVec.ofNat 64 48)) **
        ((.x10 : Reg) ↦ᵣ inPtr) **
        ((.x1 : Reg) ↦ᵣ ret) ** ((.x0 : Reg) ↦ᵣ (0 : Word)) **
        regOwn .x28 ** regOwn .x29 **
        bytesRegion inPtr xs ** bytesRegion pConstAddr bls12PBytes))
      (PF := (((.x6 : Reg) ↦ᵣ BitVec.ofNat 64 (48 - 48)) **
        ((.x7 : Reg) ↦ᵣ (inPtr + BitVec.ofNat 64 48)) **
        ((.x5 : Reg) ↦ᵣ (pConstAddr + BitVec.ofNat 64 48)) **
        ((.x10 : Reg) ↦ᵣ inPtr) **
        ((.x1 : Reg) ↦ᵣ ret) ** ((.x0 : Reg) ↦ᵣ (0 : Word)) **
      ...
      (fun hc => absurd (by decide :
        (BitVec.ofNat 64 (48 - 48) : Word) = (0 : Word)) hc))
...
/-- **`blsg_lt_p` at its linked address** (genuine post): `a0` is the
    REAL numeric strict comparison of the 48-byte big-endian input
    against the BLS12-381 base-field prime — `1` for `< p`, `0` otherwise
    (`beBytesToNat bls12PBytes` IS `p`, pinned by the `#guard` above);
    the input and the read-only `globalConst` prime region untouched.
    The `la` materialization of `blsg_p_be` is PROVEN
    (`la_materialize_within`), not assumed. -/
theorem blsgLtP_spec (inPtr ret : Word) (xs : List (BitVec 8))
    (hlenX : xs.length = 48)
    (halignIn : inPtr.toNat % 8 = 0)
    (hovIn : inPtr.toNat + 48 < 2 ^ 64)
    (hvalidIn : ∀ k, k < 48 → isValidByteAccess (inPtr + BitVec.ofNat 64 k) = true)
    (halignRet : (ret &&& ~~~(1 : Word)) = ret) :
    cpsTripleWithin 441 ltPBase ret
      (CodeReq.ofProg ltPBase blsgLtP_prog)
      (((.x10 : Reg) ↦ᵣ inPtr) **
       ((.x1 : Reg) ↦ᵣ ret) ** ((.x0 : Reg) ↦ᵣ (0 : Word)) **
       regOwn .x5 ** regOwn .x6 ** regOwn .x7 **
       regOwn .x28 ** regOwn .x29 **
       bytesRegion inPtr xs **
       globalConst pConstAddr bls12PBytes)
      (((.x10 : Reg) ↦ᵣ (if beBytesToNat xs < beBytesToNat bls12PBytes
         then (1 : Word) else (0 : Word))) **
       ((.x1 : Reg) ↦ᵣ ret) ** ((.x0 : Reg) ↦ᵣ (0 : Word)) **
       regOwn .x5 ** regOwn .x6 ** regOwn .x7 **
       regOwn .x28 ** regOwn .x29 **
       bytesRegion inPtr xs **
       globalConst pConstAddr bls12PBytes) := by
  unfold globalConst
  set CR := CodeReq.ofProg ltPBase blsgLtP_prog with hCR
  ...
  have hloop := twoBreakRetLoop_spec (hdr := (ltPBase + 16 : Word)) (ret := ret)
    (cr := CR) (Q := ltPost inPtr ret xs) 48 9 5
    (ltInv inPtr ret xs)
  ...
  have hmv7F := cpsTripleWithin_frameR
    (((.x5 : Reg) ↦ᵣ pConstAddr) ** ((.x6 : Reg) ↦ᵣ (48 : Word)) **
      ((.x1 : Reg) ↦ᵣ ret) ** ((.x0 : Reg) ↦ᵣ (0 : Word)) **
      regOwn .x28 ** regOwn .x29 **
      bytesRegion inPtr xs ** bytesRegion pConstAddr bls12PBytes)
    (by pcf) hmv7
  ...
  have hc3 := cpsTripleWithin_seq_perm_same_cr
    (fun h hp => by
      ...
      rw [show (BitVec.ofNat 64 (48 - 0) : Word) = (48 : Word) from by decide,
