/-
  EvmAsm.Codegen.Programs.RlpEncodeListPrefixLong8Spec

  **`rlp_encode_list_prefix`, 8-length-byte long form** (GH #10780 item 3, width 8).

  This is the last width. `RlpSpliceHelperSpec.lean` proves the short form (`len < 56`)
  and the 1-length-byte long form; `RlpEncodeListPrefixLong2Spec.lean` …
  `RlpEncodeListPrefixLong7Spec.lean` prove widths 2–7. This module adds
  `72057594037927936 ≤ len`: header byte `0xFF` followed by the 8 big-endian length
  bytes, header length 9. With this arm the ladder is covered at every width the
  routine can produce.

  ## What is different from the long7 arm — exactly three things

  1. **No branch is taken at all.** Every other long arm leaves the ladder by a
     *taken* `BLTU` to the shared header writer at idx30 (`base+120`). Here the last
     one, `BLTU x10, 72057594037927936` at idx28 (`base+112`), is **not** taken, so
     control falls into idx29 (`base+116`, `x28 := 8`) and then *falls through* into
     idx30. So this arm costs **one** extra dispatch step over long7, not three:
     24 ladder steps rather than 23.
  2. **The domain has no upper bound.** Long2–long7 each carry `h_len_hi`. At width 8
     the upper bound is `len.toNat < 2 ^ 64`, which `len.isLt` already gives, so the
     arm is stated with a lower bound only — `72057594037927936 ≤ len.toNat` is the
     whole input condition. This is what makes the family exhaustive rather than
     merely long: no `len : Word` escapes widths 1–8.
  3. **The loop is cited at `m := 8`,** its maximum (`lpLolLoop` requires `m ≤ 8`), so
     it contributes `7 * 8 + 1 = 57` steps and writes 8 length bytes at `out[1]`…`out[8]`.

  Everything else is long7 byte for byte: the header writer idx30–34, the epilogue
  idx42–44, the closing `JALR`, the frame/clobber set, and the shape of the
  postcondition.

  ## Step count 90, derived from the program

  `24` to reach idx30 (idx 0, 1, 8–29)
  `+ 5` for idx30–34 `+ 57` for the loop (`7*8+1`) `+ 3` for idx42–44 `+ 1`
  for the `JALR` `= 90`. Long7's 82 is the same accounting with 23 ladder steps and
  a 50-step loop.

  ## ⭐ The overflow side condition, derived rather than assumed

  `lpLolLoop` requires `outPtr.toNat + (di + m) ≤ 2 ^ 64`, which at `di := 1, m := 8`
  is `outPtr.toNat + 9 ≤ 2 ^ 64`. Long2–long7 discharge their `+ 2` … `+ 8` from the
  alignment hypothesis alone: `outPtr.toNat % 8 = 0` leaves eight bytes of slack, so
  `omega` closes it. `long7`'s module note observed that this headroom runs out here —
  `+ 9` is one more than alignment supplies — and predicted this arm would need an
  **explicit extra bound**.

  It does not. The bound is already implied by a hypothesis the arm carries anyway:
  `h_out_valid 0` says `isValidByteAccess outPtr`, and *every* valid region ends at
  or below `RAM_MEM_END = 0xc0000000` (`Rv64/Word.lean:100`). So a valid `outPtr` is
  below `2 ^ 64 - 9` by a margin of about `2 ^ 63`, and `out_room_of_validByte` below
  turns that into the loop's precondition in two lines.

  That is strictly better than a new hypothesis: this arm's interface stays
  **identical** to long2–long7's (`h_len_lo`, alignment, length, validity), so a
  caller dispatching over widths does not special-case the last one. The same
  derivation would have worked at every other width; alignment simply got there first.

  ## Byte order, read off the loop rather than assumed

  `x29` starts at `lenlen - 1 = 7` and counts **down**, while `x30` starts at
  `outPtr + 1` and counts **up**. So the iterations store `len >>> 56`, …,
  `len` at `out[1]` … `out[8]` — big-endian, most significant first, matching RLP. As in
  ...
  `u64ByteLen`'s own bounds, and `long8_first_length_byte_ne_zero` below is its
  `lenlen = 8` instance.
...
namespace RlpEncodeListPrefixLong8Spec
...
/-- Code-membership for a `∀ base` `ofProg` slice: instruction `k` of the program,
    ...
    name in the long1/long2/long3/long4/long7/loop modules (each is `local`, so not
    importable). -/
...
/-! ## ⭐ The 9-byte header cannot wrap the address space

    The one genuinely new obligation at this width. See the module docstring: at
    `lenlen = 8` the loop needs `outPtr.toNat + 9 ≤ 2 ^ 64`, which is one byte more
    than the alignment hypothesis can supply. It comes from validity instead. -/

/-- A byte address that passes `isValidByteAccess` lies in one of three regions, and
    all three end at or below `RAM_MEM_END = 0xc0000000`. -/
private theorem toNat_le_of_validByte {a : Word} (h : isValidByteAccess a = true) :
    a.toNat ≤ 0xc0000000 := by
  -- ⚠️ The region bounds must be spelled `EvmAsm.Rv64.*` in FULL. `Layout.lean:60-76`
  -- declares a second, identically-named and identically-valued set in
  -- `EvmAsm.Codegen` — the namespace this file is in — so the bare names resolve to
  -- *those*, and the rewrite silently addresses a constant the goal does not contain.
  -- The failure mode is quiet: as `simp only` arguments the bare names are merely
  -- reported unused, and `omega` then carries `Rv64.MEM_END` as an unconstrained atom
  -- and reports a counterexample above `RAM_MEM_END` that cannot exist.
  unfold isValidByteAccess isValidMemAddr at h
  unfold EvmAsm.Rv64.MEM_START EvmAsm.Rv64.MEM_END EvmAsm.Rv64.INPUT_MEM_START
    EvmAsm.Rv64.INPUT_MEM_END EvmAsm.Rv64.RAM_MEM_START EvmAsm.Rv64.RAM_MEM_END at h
  simp only [Bool.or_eq_true, Bool.and_eq_true, decide_eq_true_eq] at h
  rcases h with (⟨_, h⟩ | ⟨_, h⟩) | ⟨_, h⟩ <;> omega

/-- `lpLolLoop`'s no-wrap precondition at `di = 1, m = 8`, from validity of the
    header's first byte. The margin is about `2 ^ 63`, so this is not tight. -/
private theorem out_room_of_validByte {outPtr : Word}
    (h : isValidByteAccess outPtr = true) : outPtr.toNat + 9 ≤ 2 ^ 64 := by
  have := toNat_le_of_validByte h
  omega

/-! ## The loop's output, in the long4 postcondition's shape

    `lpLolLoop` hands back `writeShift outBytes 1 len.toNat 8`. The theorem below states
    the region as 8 nested `List.set`s instead, exactly as long4 does with four, so the
...
/-- ⭐ **8 length bytes at index 1, spelled as `List.set`s.** Each index is
    tied to its own shift: `out[1]` to `len >>> 56`, … `out[8]` to `len`.
...
private theorem writeShift_eight (dst : List Byte) (len : Word) :
    writeShift dst 1 len.toNat 8
      =
        ((((((((dst.set 1 (BitVec.ofNat 8 (len >>> (56 : Nat)).toNat)).set 2
  ...
  have hstep : writeShift dst 1 len.toNat 8
      =
        ((((((((dst.set 1 (BitVec.ofNat 8 (len.toNat / 256 ^ 7 % 256))).set 2
            (BitVec.ofNat 8 (len.toNat / 256 ^ 6 % 256))).set 3
            (BitVec.ofNat 8 (len.toNat / 256 ^ 5 % 256))).set 4
            (BitVec.ofNat 8 (len.toNat / 256 ^ 4 % 256))).set 5
            (BitVec.ofNat 8 (len.toNat / 256 ^ 3 % 256))).set 6
            (BitVec.ofNat 8 (len.toNat / 256 ^ 2 % 256))).set 7
            (BitVec.ofNat 8 (len.toNat / 256 ^ 1 % 256))).set 8
            (BitVec.ofNat 8 (len.toNat / 256 ^ 0 % 256))) := rfl
  have h7 := byte_div_eq_shift len 7
  ...
  rw [hstep, h7, h6, h5, h4, h3, h2, h1, h0]
...
/-- **The first length byte is nonzero**, at this arm's width.
    ...
    contains, and this is its `lenlen = 8` instance — the same relationship
    `long7_first_length_byte_ne_zero` has to the long7 arm. Combined with the triple
    below, `out[1] ≠ 0`, which is what makes the emitted header canonical RLP rather
    than a header that merely parses (#10780 item 1). -/
theorem long8_first_length_byte_ne_zero {len : Word}
    (h_lo : 72057594037927936 ≤ len.toNat) :
    BitVec.ofNat 8 (len >>> (56 : Nat)).toNat ≠ 0 := by
  have hwidth : u64ByteLen len = 8 := by
    unfold u64ByteLen
    split_ifs <;> omega
  have h := RlpEncodeListPrefixCanonical.first_length_byte_ne_zero (len := len) (by omega)
  rwa [hwidth, show 8 * (8 - 1) = 56 from by norm_num] at h

/-! ## The triple -/

/-- **`rlp_encode_list_prefix`, 8-length-byte long form** (`72057594037927936 ≤ len`):
    writes the header bytes `[0xFF, len >>> 56, len >>> 48, len >>> 40, len >>> 32,
    len >>> 24, len >>> 16, len >>> 8, len]` and header length 9, returns `a0 = 0`;
    scratch registers pinned.

    Clobbers `t0`/`t3`–`t6` (`x5`, `x28`–`x31`) as the long1–long7 arms do; `x6`/`x7`
    are untouched on this path and so do not appear.

    ⚠️ **No upper bound on `len`** — unlike every other long arm. `len : Word` already
    gives `len.toNat < 2 ^ 64`, so this arm's domain is the whole top of the range and
    the eight arms together are exhaustive.

    The 8 length bytes are pinned to their own shifts because the loop counts `x29`
    down while counting `x30` up, and a symmetric statement would not detect the cursors
    being swapped. Together with `long8_first_length_byte_ne_zero`, `out[1]` is
    ...
    `m := 8`, `di := 1` — 57 of the 90 steps — rather than unrolled per
    iteration. -/
theorem rlp_encode_list_prefix_long8_pinned_spec_within
    ...
    cpsTripleWithin 90 base (raVal &&& ~~~1)
  ...
  -- ⭐ the width-8 no-wrap bound, from validity rather than alignment
  have h_room : outPtr.toNat + 9 ≤ 2 ^ 64 := by
    have hv := h_out_valid 0 h_out_len0
    rw [show outPtr + BitVec.ofNat 64 0 = outPtr from by bv_omega] at hv
    exact out_room_of_validByte hv
  ...
  have hnult25 : ¬ BitVec.ult len (281474976710656 : Word) :=
    not_ult_of_toNat_ge
      (by rw [show ((281474976710656 : Word)).toNat = 281474976710656 from by decide]
          omega)
  ...
  -- ⭐ idx28 (base+112): BLTU x10, x29, +8 — NOT taken (len ≥ 2^56) → base+116.
  -- This is the only long arm that leaves the ladder by falling through.
  ...
  have hnult28 : ¬ BitVec.ult len (72057594037927936 : Word) :=
    not_ult_of_toNat_ge
      (by rw [show ((72057594037927936 : Word)).toNat = 72057594037927936 from by
              decide]
          omega)
  have hnt28 := cpsBranchWithin_ntakenStripPure2 hbr28 (fun hp hQt => by
    obtain ⟨_, _, _, _, _, hQ⟩ := hQt
    exact hnult28 ((sepConj_pure_right _).1 hQ).2)
  -- idx29 (base+116): LI x28, 8 — the extra step this width costs
  have hli28_8 := liftCode (cr' := CR)
    (li_spec_gen_within .x28 (7 : Word) (8 : Word) (base + 116) (by decide))
    (by rw [hCR]; cmem 29)
  rw [show (base + 116 : Word) + 4 = base + 120 from by bv_omega] at hli28_8
  -- idx30 (base+120): ADDI x29, x28, 247 — x29 := 0xFF
  have ha30 := liftCode (cr' := CR)
    (addi_spec_gen_within .x29 .x28 (72057594037927936 : Word) (8 : Word)
      (247 : BitVec 12) (base + 120) (by decide))
    (by rw [hCR]; cmem 30)
  rw [show (base + 120 : Word) + 4 = base + 124 from by bv_omega,
      show (8 : Word) + signExtend12 (247 : BitVec 12) = (255 : Word) from by
        decide] at ha30
  -- idx31 (base+124): SB x11, x29 — out[0] := 0xFF
  have hsb31 := liftCode (cr' := CR)
    (bytesRegion_sb_within .x11 .x29 outPtr (255 : Word) (base + 124) outBytes 0
  ...
  rw [show outPtr + BitVec.ofNat 64 0 = outPtr from by bv_omega,
      show ((255 : Word)).truncate 8 = (0xFF : BitVec 8) from by decide,
  ...
  -- idx34 (base+136): ADDI x29, x28, -1 — x29 := 7 = lenlen - 1
  have ha34 := liftCode (cr' := CR)
    (addi_spec_gen_within .x29 .x28 (255 : Word) (8 : Word)
      (-1 : BitVec 12) (base + 136) (by decide))
    (by rw [hCR]; cmem 34)
  rw [show (base + 136 : Word) + 4 = base + 140 from by bv_omega,
      show (8 : Word) + signExtend12 (-1 : BitVec 12) = (7 : Word) from by
        decide] at ha34
  -- ══ idx35–41 (base+140 → base+168): the length-byte loop, cited whole at m := 8 ══
  have hloop := lpLolLoop base outPtr v31 (56 : Word) len
    (outBytes.set 0 (0xFF : BitVec 8)) 1 8 (by omega) h_out_align
    (by rw [List.length_set]; omega)
    (by omega)
    (fun k hk => h_out_valid (1 + k) (by omega))
  rw [show 7 * 8 + 1 = 57 from by norm_num,
      show BitVec.ofNat 64 8 - 1 = (7 : Word) from by decide,
      show (1 : Nat) + 8 = 9 from by norm_num,
      writeShift_eight (outBytes.set 0 (0xFF : BitVec 8)) len] at hloop
  -- idx42 (base+168): ADDI x30, x28, 1 — x30 := 9 (header length)
  have ha42 := liftCode (cr' := CR)
    (addi_spec_gen_within .x30 .x28 (outPtr + BitVec.ofNat 64 9) (8 : Word)
      (1 : BitVec 12) (base + 168) (by decide))
    (by rw [hCR]; cmem 42)
  rw [show (base + 168 : Word) + 4 = base + 172 from by bv_omega,
      show (8 : Word) + signExtend12 (1 : BitVec 12) = (9 : Word) from by
        decide] at ha42
  -- idx43 (base+172): SD x12, x30 — *cell := 9
  have hsd := liftCode (cr' := CR)
    (sd_spec_within .x12 .x30 cellPtr (9 : Word) cellOld (0 : BitVec 12)
      (base + 172))
    (by rw [hCR]; cmem 43)
  ...
  have hsll_7F := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ raVal) ** ((.x10 : Reg) ↦ᵣ len) **
     ...
     bytesRegion outPtr outBytes ** (cellPtr ↦ₘ cellOld))
    (by pcf) hsll_7
  have hnt28F := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ raVal) ** ((.x11 : Reg) ↦ᵣ outPtr) ** ((.x12 : Reg) ↦ᵣ cellPtr) **
     ((.x5 : Reg) ↦ᵣ (56 : Word)) ** ((.x28 : Reg) ↦ᵣ (7 : Word)) **
     ((.x30 : Reg) ↦ᵣ v30) ** ((.x31 : Reg) ↦ᵣ v31) **
     ((.x0 : Reg) ↦ᵣ (0 : Word)) **
     bytesRegion outPtr outBytes ** (cellPtr ↦ₘ cellOld))
    (by pcf) hnt28
  have hli28_8F := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ raVal) ** ((.x10 : Reg) ↦ᵣ len) **
     ((.x11 : Reg) ↦ᵣ outPtr) ** ((.x12 : Reg) ↦ᵣ cellPtr) **
     ((.x5 : Reg) ↦ᵣ (56 : Word)) **
     ((.x29 : Reg) ↦ᵣ (72057594037927936 : Word)) **
     ((.x30 : Reg) ↦ᵣ v30) ** ((.x31 : Reg) ↦ᵣ v31) **
     ((.x0 : Reg) ↦ᵣ (0 : Word)) **
     bytesRegion outPtr outBytes ** (cellPtr ↦ₘ cellOld))
    (by pcf) hli28_8
  ...
  have hsb31F := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ raVal) ** ((.x10 : Reg) ↦ᵣ len) ** ((.x12 : Reg) ↦ᵣ cellPtr) **
     ((.x5 : Reg) ↦ᵣ (56 : Word)) ** ((.x28 : Reg) ↦ᵣ (8 : Word)) **
     ((.x30 : Reg) ↦ᵣ v30) ** ((.x31 : Reg) ↦ᵣ v31) **
  ...
  have hmvF := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ raVal) ** ((.x10 : Reg) ↦ᵣ len) ** ((.x12 : Reg) ↦ᵣ cellPtr) **
     ((.x5 : Reg) ↦ᵣ (56 : Word)) ** ((.x28 : Reg) ↦ᵣ (8 : Word)) **
     ((.x29 : Reg) ↦ᵣ (255 : Word)) ** ((.x31 : Reg) ↦ᵣ v31) **
     ((.x0 : Reg) ↦ᵣ (0 : Word)) **
     bytesRegion outPtr (outBytes.set 0 (0xFF : BitVec 8)) ** (cellPtr ↦ₘ cellOld))
    (by pcf) hmv
  ...
  have hloopF := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ raVal) ** ((.x11 : Reg) ↦ᵣ outPtr) ** ((.x12 : Reg) ↦ᵣ cellPtr) **
     ((.x28 : Reg) ↦ᵣ (8 : Word)) ** (cellPtr ↦ₘ cellOld))
    (by pcf) hloop
  ...
  have hli10F := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ raVal) ** ((.x11 : Reg) ↦ᵣ outPtr) ** ((.x12 : Reg) ↦ᵣ cellPtr) **
     regOwn .x5 ** ((.x28 : Reg) ↦ᵣ (8 : Word)) **
     ((.x29 : Reg) ↦ᵣ (-1 : Word)) ** ((.x30 : Reg) ↦ᵣ (9 : Word)) **
     regOwn .x31 **
     ((.x0 : Reg) ↦ᵣ (0 : Word)) **
     bytesRegion outPtr
       (((((((((outBytes.set 0 (0xFF : BitVec 8)).set 1
  ...
  have hretF := cpsTripleWithin_frameR
    (((.x10 : Reg) ↦ᵣ (0 : Word)) ** ((.x11 : Reg) ↦ᵣ outPtr) **
     ...
     (cellPtr ↦ₘ (9 : Word)))
    (by pcf) hret
  -- ══ compose: 24 ladder + 5 header + 57 loop + 3 epilogue + 1 ret = 90 ══
  ...
  have hc22 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc21 hnt28F
  have hc23 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc22 hli28_8F
  have hc24 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc23 ha30F
  have hc25 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc24 hsb31F
  have hc26 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc25 hmvF
  have hc27 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc26 ha33F
  have hc28 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc27 ha34F
  have hc29 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc28 hloopF
  have hc30 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc29 ha42F
  have hc31 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc30 hsdF
  have hc32 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc31 hli10F
  have hc33 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc32 hretF
  refine cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp) (fun h hq => ?_) hc33
  have hq1 : (((.x28 : Reg) ↦ᵣ (8 : Word)) **
      (((.x29 : Reg) ↦ᵣ (-1 : Word)) **
       (((.x30 : Reg) ↦ᵣ (9 : Word)) **
        (((.x1 : Reg) ↦ᵣ raVal) ** ((.x10 : Reg) ↦ᵣ (0 : Word)) **
         ((.x11 : Reg) ↦ᵣ outPtr) ** ((.x12 : Reg) ↦ᵣ cellPtr) **
...
end RlpEncodeListPrefixLong8Spec
