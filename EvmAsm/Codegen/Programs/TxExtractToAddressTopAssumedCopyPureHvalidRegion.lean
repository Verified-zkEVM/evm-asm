/-
  Wire pure honesty into short concrete bare Assumed copy path (region).
  Gates content dword-alignment: shortListSrcOff 5 + 1 = 8*q.
-/
...
/-- Bare Assumed short type234 copy under pure honesty + dword-aligned content.
    `hq_align : shortListSrcOff listOff items 5 + 1 = 8 * q`. classical-3. -/
theorem extractAssumed_copy_shortConcrete_pureHvalid_region
    ...
    (hq_align : shortListSrcOff (teerTxTypeDispatch txBytes).2.2.toNat items 5 + 1 = 8 * q)
  ...
  have hhoff :=
    extractSuccess_copy_type234_hoff_srcOff txBytes hsuccess hcopyFlag hge
      items hdecL hshort
  have hhover :=
    extractSuccess_copy_type234_hover_srcOff txBytes txBase hsuccess hcopyFlag hge
  ...
  have hfields04 :=
    extractSuccess_copy_type234_hnext_fields04 txBytes hsuccess hcopyFlag hge
      items hdecL hshort
  have hencInner :
  ...
  have hhnext :=
    extractSuccess_copy_type234_hnext_hlen20_srcOff txBytes txBase hsuccess
  ...
  have hcontentPtr : contentPtr = txBase + BitVec.ofNat 64 srcOff5 + (1 : Word) := by
    ...
    -- contentPtr = txBase + ofNat (8*q) = txBase + ofNat (srcOff5+1)
    -- need = txBase + ofNat srcOff5 + 1
    have hbase : txBase.toNat + (srcOff5 + 1) < 2 ^ 64 := by
      have : srcOff5 + 1 ≤ 8 * q + 16 := by omega
      omega
    -- Use BitVec add ofNat lemmas
  ...
  have hnext_content : ∀ (next5 len5 : Word),
    ...
    intro n l hd
    have hc := extractSuccess_copy_type234_hnext_content txBytes txBase contentPtr srcOff5
      hsuccess hcopyFlag hge items hdecL hshort rfl hcontentPtr endW n l hd
    -- contentPtr + 20 = txBase + 8*q + 20
  ...
  have hoff := hoffInner
  exact extractAssumed_copy_of_front_short_concrete_region
...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_shortConcrete_pureHvalid_region_fullCode
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr : Word)
    (txBytes : List (BitVec 8))
    (items : List EL.RLP.RLPItem)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_copy_shortConcrete_pureHvalid_region
...
#print axioms extractAssumed_copy_shortConcrete_pureHvalid_region
#print axioms extractAssumed_copy_shortConcrete_pureHvalid_region_fullCode
