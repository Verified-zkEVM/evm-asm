...
def secfBeToLeBody (src dst : Word) (inBytes : List (BitVec 8)) : Stmt :=
...
def secfBeToLe_verified : Program := (secfBeToLeBody 0 0 []).flatten 0

#guard (secfBeToLe_verified : List Instr).length = 19
#guard (secfBeToLeBody 0 0 []).flatten 0 = (secfBeToLeBody 0 0 []).flatten 0x80000000
-- Byte-identity to the emitted routine: the nested bottom-test loop plus the
-- calling-convention `ret` epilogue reproduce `secfBeToLe_prog` exactly.
    ...
    (hsp : Stmt.sp ⟨src, inBytes⟩ ⟨dst, 32⟩ (Stmt.block "setup" setupInstrs)
      (fun rf ws A =>
        Stmt.sp ⟨src, inBytes⟩ ⟨dst, 32⟩ (Stmt.block "init" [.LI .x5 (0 : Word)])
            (secfBeToLeFn src dst inBytes orig).pre rf ws A
...
theorem secfBeToLeFn_spec (src dst : Word) (inBytes orig : List (BitVec 8))
    (hwf : (Region.mk src inBytes).wf) (hrww : RwRegion.wf ⟨dst, 32⟩)
    (hilen : inBytes.length = 32) (base : Word) :
    (secfBeToLeFn src dst inBytes orig).Spec base := by
  vcgen
  ...
  case secfBeToLe.outer.body.inner.exhausted =>
    ...
    decide
  case secfBeToLe.outer.exhausted =>
    rintro rf ws A ⟨hx5, hx6, -, -, -, -, -, -⟩
    intro hc
    apply hc
    rw [hx5, hx6]
    decide
  case secfBeToLe.outer.body.inner.body.body.mem =>
    ...
    · obtain ⟨k, hk, _, hs6, _, _, _, _, _, _, _⟩ :=
      ...
      exact inner_blockVCs src dst inBytes ws rf (24 - 8 * k + (i + 1)) (by omega) hilen hx6
        hlen hpfr
  case secfBeToLe.outer.body.storeLimb.mem =>
  ...
  case secfBeToLe.outer.body.inner.inv_init =>
    ...
    · rw [e10, hs10]
    · rw [e11, hs11]
  case secfBeToLe.outer.body.inner.inv_step =>
    ...
    exact inner_step_engine src dst inBytes rf₀ ws₀ A₀ rfp ws' A' i k hk hi hilen hwsp hs6
      hkeq hfr hInv
  case secfBeToLe.outer.inv_init =>
    rintro rf' ws' A'
  ...
  case secfBeToLe.outer.inv_step =>
    rintro i hi rf' ws' A'
      ⟨rf2, ws2, hws2len, ⟨rf₀, ws₀, A₀, hsetup, ⟨j, hj, hInv⟩, hng⟩, rfl, rfl⟩
    have hj7 : j = 7 := by
      have hp29 : rf2.get .x29 = BitVec.ofNat 64 (7 - j) := hInv.1
