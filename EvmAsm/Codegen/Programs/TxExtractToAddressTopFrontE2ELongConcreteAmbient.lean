/-
  Ambient dual: E → ret type234 long creation of_decode.
-/
...
theorem extractFrontCreation_then_epi_of_decode_long_concrete_ambient
    ...
    cpsTripleWithin
      ...
      (fun h => ∃ next5 : Word,
        ((.x1 ↦ᵣ s.ra) ** (.x2 ↦ᵣ sp0) **
          ...
          (.x31 ↦ᵣ (next5 - (0 : Word))) **
          creExtraTemps) h) := by
  have hF := extractFrontToAfterSave_long_concrete_ambient sp0 spC s regionBase loadPtr
    ...
    hoff hinover hinvalid hspan hlistLen_ne h_ge h_ge_f8 hllen hlover hlvalid hwi_off1
    h_fits h_llz h_min h_match
  have hC := extractFrontAfterSaveCreation_then_epi_of_decode_long_ambient sp0 spC s
...
#print axioms extractFrontCreation_then_epi_of_decode_long_concrete_ambient
