/-
  Front E → AfterSave long concrete → legacy 20B copy → ret bare region.
-/
...
/-- Long-path E2E legacy copy steps: walk_init 15; 4 walks. -/
...
theorem extractFrontCopy_then_epi_of_decode_long_concrete_legacy_region
    ...
    (hlen20 : ∀ (next3 len3 : Word),
      rlpItemDecode txBytes srcOff3 (txBase + BitVec.ofNat 64 srcOff3)
        (longWalkEnd txBase (lenW - (teerTxTypeDispatch txBytes).2.2)
          (teerTxTypeDispatch txBytes).2.2.toNat) next3 len3 → len3 = (20 : Word))
    ...
    cpsTripleWithin (nFrontCopyStepsLongLegacyRegion
        (((txBytes[(teerTxTypeDispatch txBytes).2.2.toNat]'hoff).zeroExtend 64 -
          (0xf7 : Word)).toNat))
  ...
  have hC := extractFrontAfterSaveCopy_then_epi_of_decode_long_legacy_region sp0 spC s
...
#print axioms extractFrontCopy_then_epi_of_decode_long_concrete_legacy_region
