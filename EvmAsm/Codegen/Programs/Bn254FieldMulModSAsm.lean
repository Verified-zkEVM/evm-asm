...
private theorem winRw_wf : winRw.wf := by
  ...
  have : (BitVec.ofNat 64 3142660848 + BitVec.ofNat 64 k).toNat = 3142660848 + k := by
    ...
    have h1 : (3142660848 : Nat) < 2 ^ 64 := by decide
    have h2 : k < 2 ^ 64 := by omega
    have h3 : (3142660848 + k : Nat) < 2 ^ 64 := by omega
    rw [Nat.mod_eq_of_lt h2, Nat.mod_eq_of_lt h1, Nat.mod_eq_of_lt h3]
  show isValidMemAddr (BitVec.ofNat 64 3142660848 + BitVec.ofNat 64 k) = true
  unfold isValidMemAddr
  have : (BitVec.ofNat 64 3142660848 + BitVec.ofNat 64 k).toNat = 3142660848 + k := this
  rw [this]
  have h1 : decide (Rv64.RAM_MEM_START ≤ 3142660848 + k) = true := by
    show decide (2684354560 ≤ 3142660848 + k) = true
