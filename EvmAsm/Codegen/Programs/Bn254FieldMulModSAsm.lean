...
private theorem winRw_wf : winRw.wf := by
  ...
  rw [show GuestAddrs.bnf_le_a = 3170777136 from rfl]
  have : (BitVec.ofNat 64 3170777136 + BitVec.ofNat 64 k).toNat = 3170777136 + k := by
    rw [BitVec.toNat_add, BitVec.toNat_ofNat, BitVec.toNat_ofNat]
    have h1 : (3170777136 : Nat) < 2 ^ 64 := by decide
    have h2 : k < 2 ^ 64 := by omega
    have h3 : (3170777136 + k : Nat) < 2 ^ 64 := by omega
    rw [Nat.mod_eq_of_lt h2, Nat.mod_eq_of_lt h1, Nat.mod_eq_of_lt h3]
  show isValidMemAddr (BitVec.ofNat 64 3170777136 + BitVec.ofNat 64 k) = true
  unfold isValidMemAddr
  have : (BitVec.ofNat 64 3170777136 + BitVec.ofNat 64 k).toNat = 3170777136 + k := this
  rw [this]
  have h1 : decide (Rv64.RAM_MEM_START ≤ 3170777136 + k) = true := by
    show decide (2684354560 ≤ 3170777136 + k) = true
    exact decide_eq_true_iff.mpr (by omega)
  have h2 : decide (3170777136 + k ≤ Rv64.RAM_MEM_END) = true := by
    show decide (3170777136 + k ≤ 3221225472) = true
