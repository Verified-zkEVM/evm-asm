/-
  Teer front walk_next cycle 5 under applied prest.
  E → AfterWalkNext5Bne via FrontWalkNext4 + Cycle5.
-/
...
private abbrev nFrontToWalkNext4Save : Nat :=
  ((34 + (6 + (1 + nTypeSteps) + 1)) + (5 + 7)) + (1 + 15 + 1 + 2) + nWalkNextCycle * 5

private abbrev nFrontToWalkNext5Bne : Nat := nFrontToWalkNext4Save + nWalkNext5CycleOnly


/-- Cycle5 body post (no SaveS8; x24 stays cursor). -/
def teerWalkNext5BodyPost
...
/-- Cycle5 CycleOk with regOwn temps x5–7,x28–31. -/
theorem teerWalkNext5CycleOk_ownTemps
    ...
    (hend : v25 = endPtr) :
    cpsTripleWithin nWalkNext5CycleOnly AfterWalkNext4Save AfterWalkNext5Bne
      teerLinkedEarly
      (teerWalkNext0BodyCore listBase bs old1 v24 v25 a0Old a1Old a2Old **
        regOwn .x5 ** regOwn .x6 ** regOwn .x7 **
        regOwn .x28 ** regOwn .x29 ** regOwn .x30 ** regOwn .x31)
      (fun h => ∃ next len : Word,
        (teerWalkNext5BodyPost listBase endPtr (listBase + BitVec.ofNat 64 srcOff)
  ...
  have hcore (t0 t1 t2 t3 t4 t5 t6 : Word) :
      cpsTripleWithin nWalkNext5CycleOnly AfterWalkNext4Save AfterWalkNext5Bne
        ...
        (teerWalkNext5BodyPost listBase endPtr (listBase + BitVec.ofNat 64 srcOff)
          next len bs srcOff) h) := by
    have h0 := teerWalkNext5CycleOk listBase endPtr a2Old t0 t1 t2 t3 t4 t5 t6
...
theorem teerWalkNext5_applied_nested
    ...
    cpsTripleWithin nFrontToWalkNext5Bne
      E AfterWalkNext5Bne teerLinkedEarly
      ((.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
      ...
      (teerWalkNext5PostNested spC loadPtr lenW balPtr balLenW chainIdW
        s7 s11 spVal regionBase endW bs balBytes s innerVal srcOff5) := by
  intro s innerVal endW
  have hwnP := teerWalkNext4_applied ret spVal spC loadPtr lenW
  ...
  let Flat (p : Word × Word) : Assertion :=
    ...
    ⌜rlpItemDecode bs srcOff4 (regionBase + BitVec.ofNat 64 srcOff4)
      endW p.1 p.2⌝
  have hwnPE :
      cpsTripleWithin nFrontToWalkNext4Save E AfterWalkNext4Save teerLinkedEarly
      ((.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
        stackFree spVal nTeerStackDwords **
        (.x8 ↦ᵣ s0) ** (.x9 ↦ᵣ s1) **
        (.x18 ↦ᵣ s2) ** (.x19 ↦ᵣ s3) ** (.x20 ↦ᵣ s4) **
    ...
    refine ⟨(next0, len0), ?_⟩
    dsimp only [Flat, teerWalkNext4FlatRest]
    xperm_hyp hq'
  have hstep (p : Word × Word) :
    ...
    have hswap :
      ...
      have hcurK : p.1 = regionBase + BitVec.ofNat 64 srcOff5 :=
        hbridge4 p.1 p.2 (by simpa [endW, innerVal] using hpure)
      have hcy := teerWalkNext5CycleOk_ownTemps regionBase endW bs srcOff5
        LinkWalkNext4 p.1 endW p.1 (0 : Word) p.2
        (by simpa using halign) hoff5 hover5 hvalid5 hss5 hls5 hll5
        hdec5 hinb5 hcurK rfl
      ...
      have hcyF' :
        ...
        refine cpsTripleWithin_weaken
          (fun _ hp => by
            ...
            xperm_hyp hp)
          (fun h hq => by
            unfold teerWalkNext5PostNested
            obtain ⟨h1, h2, hd, hu, hEx, hA⟩ := hq
            obtain ⟨next, len1, hB⟩ := hEx
            exact ⟨next, len1, h1, h2, hd, hu, hB, hA⟩)
          hcyF
      exact cpsTripleWithin_weaken
        (teerWalkNext4FlatRest_to_wn5Pre spC loadPtr lenW balPtr balLenW chainIdW
          s7 s11 spVal regionBase endW bs balBytes s innerVal p.1 p.2)
        (fun _ hq => hq) hcyF'
    exact cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp)
      (fun _ hq => hq) hswap
  ...
  exact cpsTripleWithin_mono_nSteps
    (by decide : nFrontToWalkNext4Save + nWalkNext5CycleOnly ≤ nFrontToWalkNext5Bne)
    hseq

set_option maxRecDepth 8000 in
theorem teerWalkNext5_applied
    (ret spVal spC loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
    (regionBase : Word) (bs balBytes : List (BitVec 8)) (off len : Nat)
    (hspC : spC = spVal + signExtend12 (-160 : BitVec 12))
    ...
    cpsTripleWithin nFrontToWalkNext5Bne
      ...
      (fun h => ∃ next len5 : Word,
        ((.x2 ↦ᵣ spC) **
          ...
          (.x24 ↦ᵣ (regionBase + BitVec.ofNat 64 srcOff5)) ** (.x25 ↦ᵣ endW) **
          (.x26 ↦ᵣ (0 : Word)) **
          (.x27 ↦ᵣ s11) **
          frameSlotsSaved teerFrame spC (teerSavedVals s) **
          (.x0 ↦ᵣ (0 : Word)) **
          ...
          ⌜rlpItemDecode bs srcOff5 (regionBase + BitVec.ofNat 64 srcOff5)
            endW next len5⌝) h) := by
  intro s innerVal endW
  have h0 := teerWalkNext5_applied_nested ret spVal spC loadPtr lenW
    balPtr balLenW chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
    regionBase bs balBytes off len hspC hnez hptr hlenW hsuccess htype4
    halign hbound hover hvalid0 listOff ha0 hoffL hoverL hvalidL hlenL
    h_ge h_hi h_exact srcOff0 hcur0 hoff0 hover0 hvalid0I hss0 hls0 hll0 hdec0 hinb0
  ...
  exact cpsTripleWithin_weaken (fun _ hp => hp)
    (fun h hq => by
      unfold teerWalkNext5PostNested teerWalkNext5BodyPost
        teerWalkNextSkipAmbient teerWalkNext0Ambient at hq
      obtain ⟨next, len5, hq'⟩ := hq
      refine ⟨next, len5, ?_⟩
      xperm_hyp hq' ) h0

#print axioms teerWalkNext5CycleOk_ownTemps
#print axioms teerWalkNext4FlatRest_to_wn5Pre
#print axioms teerWalkNext5_applied_nested
#print axioms teerWalkNext5_applied




def teerWalkNext4FlatRestIs
    (spC loadPtr lenW balPtr balLenW chainIdW
      s7 s11 spVal regionBase endW : Word)
    (bs balBytes : List (BitVec 8)) (s : TeerSaved)
    (innerVal next0 len0 : Word) : Assertion :=
...
theorem teerWalkNext4FlatRestIs_to_wn5Pre
    (spC loadPtr lenW balPtr balLenW chainIdW
      s7 s11 spVal regionBase endW : Word)
    (bs balBytes : List (BitVec 8)) (s : TeerSaved)
    (innerVal next0 len0 : Word) :
    ∀ h,
      (teerWalkNext4FlatRestIs spC loadPtr lenW balPtr balLenW chainIdW
          s7 s11 spVal regionBase endW bs balBytes s innerVal next0 len0) h →
      (teerWalkNextSkipFocus regionBase bs
          LinkWalkNext4 next0 endW next0 (0 : Word) len0 **
        teerWalkNextSkipAmbientIs spC loadPtr lenW balPtr balLenW chainIdW
          s7 s11 spVal balBytes s innerVal) h := by
  intro h hp
  unfold teerWalkNext4FlatRestIs at hp
  unfold teerWalkNextSkipFocus teerWalkNext0Focus teerWalkNext0BodyCore
    teerWalkNextSkipAmbientIs teerWalkNext0AmbientIs
  xperm_hyp hp

def teerWalkNext5PostNestedIs
    (spC loadPtr lenW balPtr balLenW chainIdW
      s7 s11 spVal regionBase endW : Word)
    (bs balBytes : List (BitVec 8)) (s : TeerSaved)
    (innerVal : Word) (srcOff5 : Nat) : Assertion :=
...
set_option maxRecDepth 8000 in
theorem teerWalkNext5_applied_nested_is
    (ret spVal spC loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
    (regionBase : Word) (bs balBytes : List (BitVec 8)) (off len : Nat)
    (hspC : spC = spVal + signExtend12 (-160 : BitVec 12))
    ...
    cpsTripleWithin nFrontToWalkNext5Bne
      ...
      ((.x1 ↦ᵣ ret) ** (.x2 ↦ᵣ spVal) **
        ...
        regOwn .x16 ** regOwn .x28 ** regOwn .x29 ** regOwn .x30 **
        regOwn .x31 ** (.x0 ↦ᵣ (0 : Word)))
      (teerWalkNext5PostNestedIs spC loadPtr lenW balPtr balLenW chainIdW
        s7 s11 spVal regionBase endW bs balBytes s innerVal srcOff5) := by
  intro s innerVal endW
  have hwnP := teerWalkNext4_applied_is ret spVal spC loadPtr lenW
    balPtr balLenW chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
    regionBase bs balBytes off len hspC hnez hptr hlenW hsuccess htype4
    halign hbound hover hvalid0 listOff ha0 hoffL hoverL hvalidL hlenL
    h_ge h_hi h_exact srcOff0 hcur0 hoff0 hover0 hvalid0I hss0 hls0 hll0 hdec0 hinb0
  ...
  have hwnPE :
    ...
    obtain ⟨next0, len0, hq'⟩ := hq
    refine ⟨(next0, len0), ?_⟩
    dsimp only [Flat, teerWalkNext4FlatRestIs]
    xperm_hyp hq'
  have hstep (p : Word × Word) :
      cpsTripleWithin nWalkNext5CycleOnly AfterWalkNext4Save AfterWalkNext5Bne
        teerLinkedEarly (Flat p)
      ...
      have hcyF' :
        ...
        refine cpsTripleWithin_weaken
          (fun _ hp => by
            ...
            xperm_hyp hp)
          (fun h hq => by
            unfold teerWalkNext5PostNestedIs
            obtain ⟨h1, h2, hd, hu, hEx, hA⟩ := hq
            obtain ⟨next, len1, hB⟩ := hEx
            exact ⟨next, len1, h1, h2, hd, hu, hB, hA⟩)
          hcyF
      exact cpsTripleWithin_weaken
        (teerWalkNext4FlatRestIs_to_wn5Pre spC loadPtr lenW balPtr balLenW chainIdW
          s7 s11 spVal regionBase endW bs balBytes s innerVal p.1 p.2)
        (fun _ hq => hq) hcyF'
    exact cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp)
      (fun _ hq => hq) hswap
...
set_option maxRecDepth 8000 in
theorem teerWalkNext5_applied_is
    (ret spVal spC loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
    (regionBase : Word) (bs balBytes : List (BitVec 8)) (off len : Nat)
    (hspC : spC = spVal + signExtend12 (-160 : BitVec 12))
      ...
      (fun h => ∃ next len5 : Word,
        ((.x2 ↦ᵣ spC) **
          ...
          ⌜rlpItemDecode bs srcOff5 (regionBase + BitVec.ofNat 64 srcOff5)
            endW next len5⌝) h) := by
  intro s innerVal endW
  have h0 := teerWalkNext5_applied_nested_is ret spVal spC loadPtr lenW
    balPtr balLenW chainIdW baiW s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11
    regionBase bs balBytes off len hspC hnez hptr hlenW hsuccess htype4
    halign hbound hover hvalid0 listOff ha0 hoffL hoverL hvalidL hlenL
    h_ge h_hi h_exact srcOff0 hcur0 hoff0 hover0 hvalid0I hss0 hls0 hll0 hdec0 hinb0
  ...
  exact cpsTripleWithin_weaken (fun _ hp => hp)
    (fun h hq => by
      unfold teerWalkNext5PostNestedIs teerWalkNext5BodyPost
...
#print axioms teerWalkNext5CycleOk_ownTemps
#print axioms teerWalkNext4FlatRestIs_to_wn5Pre
#print axioms teerWalkNext5_applied_nested_is
#print axioms teerWalkNext5_applied_is
