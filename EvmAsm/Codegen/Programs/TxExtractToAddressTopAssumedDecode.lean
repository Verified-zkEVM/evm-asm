/-
  ExtractAssumed packaging under of_decode E2E (no universal hok).
-/
...
/-- Assumed creation packaging under type234 E2E of_decode residuals
    (hdrop/hdec*/hinb*/hnext*/hcre + walk statics; no universal hok). -/
theorem extractAssumed_creation_under_honesty_of_decode
  ...
  have h := extractFrontCreation_then_epi_of_decode sp0 spC s txBase lenW toBuf
...
theorem extractAssumed_creation_fullCode_of_decode
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr : Word)
    (txBytes : List (BitVec 8))
    (srcOff0 srcOff1 srcOff2 srcOff3 srcOff4 srcOff5 : Nat)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_creation_under_honesty_of_decode sp0 spC s txBase lenW toBuf
...
#print axioms extractAssumed_creation_under_honesty_of_decode
#print axioms extractAssumed_creation_fullCode_of_decode
