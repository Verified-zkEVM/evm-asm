/-
  Caller contract for the 8-instruction `header_extract_gas_limit` wrapper.

  `headerExtractGasLimit_prog` allocates a 16-byte frame, saves `ra`, shuffles
  ...
  big-endian decode of the real field-9 content (via K34's `Result`).
-/
...
namespace EvmAsm.Codegen.HeaderExtractGasLimitSpec
...
/-! ## Base addresses and linked code
    ...
    Header field 9 (`gas_limit`) is `Uint` in the reference model, so this
    wrapper uses the strict K34 path for canonicality.  The machine-side width
    assumption remains explicit in the bridge below. -/

/- The gas-limit wrapper is not linked into `GuestAddrs` yet.  Keep this proof
   on a concrete, disjoint placeholder base; the later registry row can repin
   `H` when the emitted symbol receives its linked address.  The placeholder
   matches the program's current symbolic-JAL origin (`0x80000010`). -/
abbrev HNat : Nat := 0x80000000
abbrev H : Word := (0x80000000 : Word)

theorem hdr_length : headerExtractGasLimit_prog.length = 8 := by decide

/-- The wrapper's own re-emitted instructions at `header_extract_gas_limit`. -/
def hdrCode : CodeReq := CodeReq.ofProg H headerExtractGasLimit_prog
...
/-- Success return: `a0 = 0` (or the empty join encoded in K34's `Result`),
    ...
    the field-9 content (pinned inside `successPayload`'s
    `Result`). -/
def hdrSuccess
    ...
    ((.x1 ↦ᵣ raIn) ** (spH ↦ₘ raIn) ** (.x2 ↦ᵣ sp0) **
      ((.x8 ↦ᵣ outer.s0) ** (.x9 ↦ᵣ outer.s1) ** savedFrame newSp outer **
       successPayload newSp listBase offset len v12 x5 scalarStatus
         wrapperStatus outputValue saved bytes listLen 9)) h
...
theorem hdrPrologue
    ...
    cpsTripleWithin 4 H (H + 16) fullCode
      ...
      ((.x1 ↦ᵣ raIn) ** (spH ↦ₘ raIn) **
       flatPre spH newSp listBase listLenW (9 : Word) outputPtr oldOut
  ...
  have h0' := cpsTripleWithin_extend_code (cr' := hdrCode)
    (CodeReq.ofProg_mem_at H H headerExtractGasLimit_prog 0
  ...
  have h1' := cpsTripleWithin_extend_code (cr' := hdrCode)
    (CodeReq.ofProg_mem_at H (H + 4) headerExtractGasLimit_prog 1
  ...
  have h2' := cpsTripleWithin_extend_code (cr' := hdrCode)
    (CodeReq.ofProg_mem_at H (H + 8) headerExtractGasLimit_prog 2 (.MV .x13 .x12)
      (by bv_omega) (by rw [hdr_length]; decide) rfl
      (by rw [hdr_length]; decide)) h2
  -- [3] LI x12 8 : x12 := 8
  have h3 := li_spec_gen_within .x12 outputPtr (9 : Word) (H + 12) (by decide)
  have h3' := cpsTripleWithin_extend_code (cr' := hdrCode)
    (CodeReq.ofProg_mem_at H (H + 12) headerExtractGasLimit_prog 3
      (.LI .x12 (9 : Word)) (by bv_omega) (by rw [hdr_length]; decide) rfl
...
theorem epiCore (sp0 spH raIn : Word) (G : Assertion) (hG : G.pcFree)
  ...
  have h5' := cpsTripleWithin_extend_code hdr_mono
    (cpsTripleWithin_extend_code (cr' := hdrCode)
      (CodeReq.ofProg_mem_at H (H + 20) headerExtractGasLimit_prog 5
  ...
  have h6' := cpsTripleWithin_extend_code hdr_mono
    (cpsTripleWithin_extend_code (cr' := hdrCode)
      (CodeReq.ofProg_mem_at H (H + 24) headerExtractGasLimit_prog 6
  ...
  have h7' := cpsTripleWithin_extend_code hdr_mono
    (cpsTripleWithin_extend_code (cr' := hdrCode)
      (CodeReq.ofProg_mem_at H (H + 28) headerExtractGasLimit_prog 7
...
theorem hdrEpilogue
    ...
    cpsTripleWithin 3 (H + 20) raIn fullCode
      ((.x1 ↦ᵣ (H + 20)) ** (spH ↦ₘ raIn) **
       flatPost spH newSp listBase oldOffset oldLen outer saved bytes listLen 9)
  ...
  have tripleS : cpsTripleWithin 3 (H + 20) raIn fullCode
      ((.x1 ↦ᵣ (H + 20)) ** (spH ↦ₘ raIn) **
       flatSuccessReturned spH newSp listBase outer saved bytes listLen 9)
      (hdrPost sp0 spH newSp raIn listBase oldOffset oldLen outer saved bytes
        listLen) :=
    ...
    (fun h' (hh : ((spH ↦ₘ raIn) **
        (fun h'' => flatSuccessReturned spH newSp listBase outer saved bytes
          ...
          flatFailureReturned spH newSp listBase oldOffset oldLen outer saved
            bytes listLen 9 h'')) h') => sepConj_or_split h' hh) h hp
...
set_option maxRecDepth 8000 in
/-- **`header_extract_gas_limit` caller contract.**  The 8-instruction wrapper =
    ...
    of the real field-9 content (pinned inside `successPayload`'s `Result`);
    `a0 = 1` is an RLP parse failure and `a0 = 2` a field wider than 8 bytes,
    both with a zeroed output cell. -/
theorem header_extract_gas_limit_spec_within
    (sp0 raIn oldRaSlot spH newSp listBase listLenW outputPtr old13 old14
      oldOut oldOffset oldLen s0In s1In s2 s3 s4 s5 : Word)
    (bytes : List (BitVec 8)) (listLen : Nat)
    (hspH : spH = sp0 + signExtend12 (-16 : BitVec 12))
    ...
    let saved : EvmAsm.Codegen.RlpListNthItemSAsm.Saved :=
      { ra := B + 48, s0 := listBase, s1 := outputPtr, s2 := s2, s3 := s3,
        s4 := s4, s5 := s5 }
    let callSteps := 1 + ((12 + ((85 + 93 * (9 + 2)) + 6)) + 9)
  ...
  have hflat := rlpFieldToU64_flat_spec_within spH newSp listBase listLenW
    (9 : Word) outputPtr oldOut oldOffset oldLen old14 outer s2 s3 s4 s5 bytes
    listLen 9 hnewSp hlistLenW (by decide) (by decide) hsalign hslack hover
  ...
  have hcall := callWithin_spec (H + 16) B raIn
    (jalOff GuestAddrs.rlp_field_to_u64_strict (HNat + 16))
    n34 (by
      change BitVec.ofNat 64 HNat + BitVec.ofNat 64 16 +
    ...
    (fun a i hi => hdr_mono a i
      (CodeReq.ofProg_mem_at H (H + 16) headerExtractGasLimit_prog 4
  ...
  have hcall' : cpsTripleWithin (1 + n34) (H + 16) (H + 20) fullCode
      ((.x1 ↦ᵣ raIn) ** (spH ↦ₘ raIn) **
        flatPre spH newSp listBase listLenW (9 : Word) outputPtr oldOut oldOffset
...
end EvmAsm.Codegen.HeaderExtractGasLimitSpec
