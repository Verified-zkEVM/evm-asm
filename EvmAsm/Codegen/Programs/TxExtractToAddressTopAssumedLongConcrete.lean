/-
  ExtractAssumed packaging under long concrete of_decode E2E.
-/
...
private theorem nFrontCreationStepsLong_le_nFront
    (lol : Nat) (hlol : lol ≤ 8) :
...
/-- Assumed creation long concrete of_decode. -/
theorem extractAssumed_creation_under_honesty_of_decode_long_concrete
    ...
    (hcur : longWalkCursor txBase txBytes (teerTxTypeDispatch txBytes).2.2.toNat hoff =
  ...
  have hLong := extractFrontCreation_then_epi_of_decode_long_concrete sp0 spC s
  ...
  have hle := nFrontCreationStepsLong_le_nFront
    (((txBytes[(teerTxTypeDispatch txBytes).2.2.toNat]'hoff).zeroExtend 64 -
        (0xf7 : Word)).toNat) hlol
...
theorem extractAssumed_creation_fullCode_of_decode_long_concrete
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr : Word)
    (txBytes : List (BitVec 8))
    (srcOff0 srcOff1 srcOff2 srcOff3 srcOff4 srcOff5 : Nat)
    ...
    (hlol :
      (((txBytes[(teerTxTypeDispatch txBytes).2.2.toNat]'hoff).zeroExtend 64 -
          (0xf7 : Word)).toNat) ≤ 8)
    :
    cpsTripleWithin nExtractSteps
      (BitVec.ofNat 64 GuestAddrs.tx_extract_to_address) s.ra fullCode
      (extractAssumedPre s.ra sp0 txBase lenW toBuf isCreationPtr
        s.s0 s.s1 s.s2 s.s3 s.s4 s.s5 s.s6 s.s7 txBytes)
      (extractAssumedPost s.ra sp0 txBase toBuf isCreationPtr
        s.s0 s.s1 s.s2 s.s3 s.s4 s.s5 s.s6 s.s7 txBytes) :=
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_creation_under_honesty_of_decode_long_concrete
...
#print axioms extractAssumed_creation_under_honesty_of_decode_long_concrete
#print axioms extractAssumed_creation_fullCode_of_decode_long_concrete
