/-
  ExtractAssumed packaging under short concrete legacy creation of_decode.
-/
...
theorem nFrontCreationStepsShortLegacy_le_nFront :
    nFrontCreationStepsShortLegacy ≤ nFrontCreationSteps := by
  simp only [nFrontCreationStepsShortLegacy, nFrontCreationSteps, nTypeSteps]
  omega

set_option maxRecDepth 8000 in
/-- Assumed creation short concrete legacy of_decode. -/
theorem extractAssumed_creation_under_honesty_of_decode_short_concrete_legacy
  ...
  have hShort := extractFrontCreation_then_epi_of_decode_short_concrete_legacy
  ...
  have h := cpsTripleWithin_mono_nSteps
    nFrontCreationStepsShortLegacy_le_nFront hShort
  refine cpsTripleWithin_weaken (fun _ hp => hp) (fun st hq => ?_) h
  obtain ⟨next3, hq'⟩ := hq
  refine ⟨next3, ?_⟩
  -- legacy post has TeaTypeAddr ↦ 0; creationE2EPost uses teer type (=0)
  simp only [creationE2EPost, creExtraTemps, htype0]
  exact hq'

set_option maxRecDepth 8000 in
theorem extractAssumed_creation_fullCode_of_decode_short_concrete_legacy
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr : Word)
    (txBytes : List (BitVec 8))
    (srcOff0 srcOff1 srcOff2 srcOff3 : Nat)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_creation_under_honesty_of_decode_short_concrete_legacy
      sp0 spC s txBase lenW toBuf isCreationPtr txBytes
      srcOff0 srcOff1 srcOff2 srcOff3
      hspC hret hcur htype0 hsalign
      hoff0 hover0 hvalid0 hss0 hls0 hll0 hdec0 hinb0
...
#print axioms extractAssumed_creation_under_honesty_of_decode_short_concrete_legacy
#print axioms extractAssumed_creation_fullCode_of_decode_short_concrete_legacy
