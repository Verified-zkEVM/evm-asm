...
theorem extractAssumed_creation_longConcrete_pure_ambient
    ...
    (hitem0 : (encode (items[0]'(by
        have := extractSuccess_creation_type234_items_length_long (txSlice bs off len) hsuccess
  ...
  have hne1 : (teerTxTypeDispatch slice).2.1 ≠ 1 := by
    ...
    omega
  have hhoff :=
    extractSuccess_creation_type234_hoff_srcOff_long slice hsuccess hcreFlag hge
      items hdecL hlong
  have hhover :=
    extractSuccess_creation_type234_hover_srcOff_long slice loadPtr hsuccess hcreFlag hge
      items hdecL hlong hover_sl
  have hlenItems :=
    extractSuccess_creation_type234_items_length_long slice hsuccess hcreFlag hge
      items hdecL hlong
  ...
  have hfields04 :=
    extractSuccess_creation_type234_hnext_fields04_long slice hsuccess hcreFlag hge
      items hdecL hlong
  have hoverEnd :
      regionBase.toNat +
  ...
  have hhnext :=
    extractSuccess_creation_type234_hnext_hcre_srcOff_long slice loadPtr hsuccess
  ...
  have hcre : ∀ (next5 len5 : Word),
    ...
    simpa [absOff5, srcOff5, listOff, endW, absListOff] using
      hcre_ambient_of_slice_long regionBase loadPtr bs off len listOff items 5 endW
...
set_option maxRecDepth 8000 in
theorem extractAssumed_creation_longConcrete_pure_ambient_fullCode
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (items : List EL.RLP.RLPItem)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_creation_longConcrete_pure_ambient sp0 spC s
      regionBase loadPtr lenW toBuf isCreationPtr bs off len items
      hspC hret htalign htover htvalid hlen hptr hbound hsuccess hcreFlag hge hdecL hlong
...
#print axioms extractAssumed_creation_longConcrete_pure_ambient
#print axioms extractAssumed_creation_longConcrete_pure_ambient_fullCode
