/-
  Ambient dual: Front long AfterSave → creation of_decode (split loadPtr/regionBase).
-/
...
theorem extractFrontAfterSaveCreation_then_epi_of_decode_long_ambient
    ...
    (hcre : ∀ (next5 len5 : Word),
      rlpItemDecode bs absOff5 (regionBase + BitVec.ofNat 64 absOff5)
        (longWalkEndAmbient regionBase (lenW - (teerTxTypeDispatch (txSlice bs off len)).2.2)
          (ambientAbsOff off (teerTxTypeDispatch (txSlice bs off len)).2.2.toNat)) next5 len5 → len5 = (0 : Word)) :
  ...
  have hMid := extractType234AfterSaveCreation_then_epi_of_decode_ambient sp0 spC s
  ...
  exact cpsTripleWithin_weaken
    (fun h hp => frontAfterSavePostLongAmbient_to_midJoinPre spC s regionBase loadPtr
      lenW toBuf isCreationPtr bs off len hwi_off h hp)
    (fun _ hq => hq) hMid


#print axioms extractFrontAfterSaveCreation_then_epi_of_decode_long_ambient
