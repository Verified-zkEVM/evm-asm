/-
  ExtractAssumed packaging under short walk_init + of_decode E2E (no hdrop/hok).
-/
...
/-- Assumed creation under short walk_init + of_decode (no universal hdrop/hok). -/
theorem extractAssumed_creation_under_honesty_of_decode_short
  ...
  have hShort := extractFrontCreation_then_epi_of_decode_short sp0 spC s txBase lenW toBuf
...
theorem extractAssumed_creation_fullCode_of_decode_short
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr : Word)
    (txBytes : List (BitVec 8))
    (srcOff0 srcOff1 srcOff2 srcOff3 srcOff4 srcOff5 : Nat)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_creation_under_honesty_of_decode_short sp0 spC s txBase lenW toBuf
...
#print axioms extractAssumed_creation_under_honesty_of_decode_short
#print axioms extractAssumed_creation_fullCode_of_decode_short
