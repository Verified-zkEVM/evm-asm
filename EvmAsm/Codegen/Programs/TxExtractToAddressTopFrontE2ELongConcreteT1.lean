/-
  Front E → ret long concrete t1 creation of_decode (no ∀endPtr).
-/
...
/-- Front long walk_init + t1 AfterSave→creation steps. -/
def nFrontCreationStepsLongT1 (lol : Nat) : Nat :=
...
theorem extractFrontCreation_then_epi_of_decode_long_concrete_t1
    ...
    cpsTripleWithin (nFrontCreationStepsLongT1
        (((txBytes[(teerTxTypeDispatch txBytes).2.2.toNat]'hoff).zeroExtend 64 -
          (0xf7 : Word)).toNat))
  ...
  have hC := extractFrontAfterSaveCreation_then_epi_of_decode_long_t1 sp0 spC s
...
#print axioms extractFrontCreation_then_epi_of_decode_long_concrete_t1
