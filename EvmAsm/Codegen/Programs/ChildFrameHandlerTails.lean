/-
  ...
  `FileSizeGuard` line cap. Holds the two large `.tail` builders used by
  `childFrameHandlers` (`createUnsupportedTail` for the CREATE family and
  `basicPrecompileCallTail` for the CALL/STATICCALL precompile path). They
  are pure assembly-string builders that close over neither the dispatch
  fall-through params nor each other, so lifting them to top-level defs in
  this sibling module is behaviour-preserving.
-/
