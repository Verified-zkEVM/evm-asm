/-
  Ambient dual: long legacy creation of_decode packaging (typeW=0, fields 0..3).
-/
...
private theorem nFrontCreationStepsLongLegacy_le_nFront
    (lol : Nat) (hlol : lol ≤ 8) :
    nFrontCreationStepsLongLegacy lol ≤ nFrontCreationSteps := by
  simp only [nFrontCreationStepsLongLegacy, nFrontCreationSteps, nTypeSteps]
  omega

set_option maxRecDepth 8000 in
theorem extractAssumed_creation_under_honesty_of_decode_long_concrete_legacy_ambient
  ...
  have hLong := extractFrontCreation_then_epi_of_decode_long_concrete_legacy_ambient
    sp0 spC s regionBase loadPtr lenW toBuf isCreationPtr bs off len
    absOff0 absOff1 absOff2 absOff3
    hspC hret htype0 hsalign
  ...
  have hle := nFrontCreationStepsLongLegacy_le_nFront
    (((bs[ambientAbsOff off (teerTxTypeDispatch (txSlice bs off len)).2.2.toNat]'hoff
...
theorem extractAssumed_creation_fullCode_of_decode_long_concrete_legacy_ambient
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (absOff0 absOff1 absOff2 absOff3 : Nat)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_creation_under_honesty_of_decode_long_concrete_legacy_ambient
      sp0 spC s regionBase loadPtr lenW toBuf isCreationPtr bs off len
      absOff0 absOff1 absOff2 absOff3
      hspC hret htype0 hsalign
    hoff0 hover0 hvalid0 hss0 hls0 hll0 hdec0 hinb0
...
#print axioms extractAssumed_creation_under_honesty_of_decode_long_concrete_legacy_ambient
#print axioms extractAssumed_creation_fullCode_of_decode_long_concrete_legacy_ambient
