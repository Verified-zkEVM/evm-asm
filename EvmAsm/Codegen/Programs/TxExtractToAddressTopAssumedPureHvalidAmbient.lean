/-
  Ambient short creation: discharge hdec*/hvalid* via slice pure + bridges +
  validByteRange on regionBase. Residual path flags only (creation/type234/short/hge7).
-/
...
/-- Ambient short concrete Assumed creation; hdec+hvalid from pure+validByteRange.
    Residual path: creation/type234/short/hge7/hdecL. -/
theorem extractAssumed_creation_shortConcrete_pureHvalid_ambient
  ...
  have hhoff :=
    extractSuccess_creation_type234_hoff_srcOff slice hsuccess hcreFlag hge
      items hdecL hshort
  have hhover :=
    extractSuccess_creation_type234_hover_srcOff slice loadPtr hsuccess hcreFlag hge
      items hdecL hshort hover_sl
  have hlenItems :=
    extractSuccess_creation_type234_items_length slice hsuccess hcreFlag hge
      items hdecL hshort
  ...
  have hdec4 :=
    hdec_ambient_field' regionBase loadPtr bs off len listOff items 4 endW
      hptr hbound hencInner hshort hn4 hhoff.2.2.2.2.1 hspan4 hoverEnd_sl hoverEnd_abs hendEq
  have hdec5 :=
    hdec_ambient_field' regionBase loadPtr bs off len listOff items 5 endW
      hptr hbound hencInner hshort hn5 hhoff.2.2.2.2.2 hspan5 hoverEnd_sl hoverEnd_abs hendEq
  ...
  simpa [absOff0, absOff1, absOff2, absOff3, absOff4, absOff5, absListOff,
    ...
    extractAssumed_creation_shortConcrete_pure_ambient
      sp0 spC s regionBase loadPtr lenW toBuf isCreationPtr bs off len items
      hspC hret htalign htover htvalid hlen hptr hbound hsuccess hcreFlag hge hdecL hshort
...
set_option maxRecDepth 8000 in
theorem extractAssumed_creation_shortConcrete_pureHvalid_ambient_fullCode
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (items : List EL.RLP.RLPItem)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_creation_shortConcrete_pureHvalid_ambient
      sp0 spC s regionBase loadPtr lenW toBuf isCreationPtr bs off len items
      hspC hret htalign htover htvalid hlen hptr hbound hsuccess hcreFlag hge hdecL hshort
      hsalign hover hvalidBuf hge7)

/-- Path refinements for ambient short type234 creation arm. -/
def extractCreationType234ShortPathAmbient
...
set_option maxRecDepth 8000 in
/-- Ambient Assumed footprint under short type234 creation path (statics + path).
    classical-3. Residual: other extractSuccess arms (copy/legacy/t1/long). -/
theorem extractAssumed_success_flat_creation_type234_short_ambient
  ...
  simpa only [hsra, hs0, hs1, hs2, hs3, hs4, hs5, hs6, hs7] using
    extractAssumed_creation_shortConcrete_pureHvalid_ambient_fullCode
      spVal spC s regionBase loadPtr lenW toBuf isCreationPtr bs off len items
      hspC hret htalign htover htvalid hlen hptr hbound hsuccess hcreFlag hge hdecL hshort
      hsalign hover hvalidBuf hge7

#print axioms extractAssumed_creation_shortConcrete_pureHvalid_ambient
#print axioms extractAssumed_creation_shortConcrete_pureHvalid_ambient_fullCode
#print axioms extractAssumed_success_flat_creation_type234_short_ambient
