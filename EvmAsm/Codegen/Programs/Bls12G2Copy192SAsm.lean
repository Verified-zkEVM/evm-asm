...
#guard GuestAddrs.blsf_copy_quads = 0x8002ec58
#guard GuestAddrs.blsg2_copy192 = 0x8003333c
...
def copy192Cr : CodeReq :=
  (CodeReq.ofProg (0x8003333c : Word) blsg2Copy192_prog).union
    (CodeReq.ofProg (0x8002ec58 : Word) blsfCopyQuads_prog)

private theorem callerSub :
    ∀ a i, CodeReq.ofProg (0x8003333c : Word) blsg2Copy192_prog a = some i →
      copy192Cr a = some i := by
  intro a i h
  simp only [copy192Cr, CodeReq.union, h]

private theorem calleeSub :
    ∀ a i, CodeReq.ofProg (0x8002ec58 : Word) blsfCopyQuads_prog a = some i →
      copy192Cr a = some i := by
  intro a i h
  obtain ⟨k, hk, rfl⟩ := ofProg_some_range h
  have hnone : CodeReq.ofProg (0x8003333c : Word) blsg2Copy192_prog
      ((0x8002ec58 : Word) + BitVec.ofNat 64 (4 * k)) = none := by
...
private theorem blsfCopyLoopBody_spec (src dst : Word) (srcWords dstWords : List Word)
    ...
    (n : Nat) (hn : n < 24) :
    cpsTripleWithin 6 (0x8002ec5c : Word) (0x8002ec58 : Word) copy192Cr
  ...
  have hld := ld_spec_gen_within .x5 .x10 (src + BitVec.ofNat 64 (8 * p))
    (lastLoaded srcWords init5 p) (srcWords.getD p 0) (0 : BitVec 12) (0x8002ec5c : Word)
    (by decide)
  rw [add_sext0] at hld
  rw [show (0x8002ec5c : Word) + 4 = (0x8002ec60 : Word) from by decide] at hld
  have hldC := liftCode (cr' := copy192Cr) hld (by code_mem)
  have hsd := sd_spec_gen_within .x11 .x5 (dst + BitVec.ofNat 64 (8 * p))
    (srcWords.getD p 0) (L.getD p 0) (0 : BitVec 12) (0x8002ec60 : Word)
  rw [add_sext0] at hsd
  rw [show (0x8002ec60 : Word) + 4 = (0x8002ec64 : Word) from by decide] at hsd
  have hsdC := liftCode (cr' := copy192Cr) hsd (by code_mem)
  have ha0 := addi_spec_gen_same_within .x10 (src + BitVec.ofNat 64 (8 * p))
    (8 : BitVec 12) (0x8002ec64 : Word) (by decide)
  rw [addr_step8] at ha0
  rw [show (0x8002ec64 : Word) + 4 = (0x8002ec68 : Word) from by decide] at ha0
  have ha0C := liftCode (cr' := copy192Cr) ha0 (by code_mem)
  have ha1 := addi_spec_gen_same_within .x11 (dst + BitVec.ofNat 64 (8 * p))
    (8 : BitVec 12) (0x8002ec68 : Word) (by decide)
  rw [addr_step8] at ha1
  rw [show (0x8002ec68 : Word) + 4 = (0x8002ec6c : Word) from by decide] at ha1
  have ha1C := liftCode (cr' := copy192Cr) ha1 (by code_mem)
  have hcnt := addi_spec_gen_same_within .x12 (BitVec.ofNat 64 (n + 1))
    (-1 : BitVec 12) (0x8002ec6c : Word) (by decide)
  rw [cnt_step_down] at hcnt
  rw [show (0x8002ec6c : Word) + 4 = (0x8002ec70 : Word) from by decide] at hcnt
  have hcntC := liftCode (cr' := copy192Cr) hcnt (by code_mem)
  have hjal := jal_x0_spec_gen_within (-24 : BitVec 21) (0x8002ec70 : Word)
  rw [show (0x8002ec70 : Word) + signExtend21 (-24 : BitVec 21) = (0x8002ec58 : Word) from by decide] at hjal
...
theorem blsfCopyQuads24Flat_spec (ret src dst init5 : Word)
    ...
    cpsTripleWithin (24 * (6 + 1) + 1 + 1) (0x8002ec58 : Word) ret copy192Cr
  ...
  have hloop : cpsTripleWithin (24 * (6 + 1) + 1) (0x8002ec58 : Word) (0x8002ec74 : Word)
      copy192Cr
      ((.x12 ↦ᵣ BitVec.ofNat 64 24) ** (Reg.x0 ↦ᵣ (0 : Word))
        ** copyInvF src dst srcWords dstWords init5 24)
      ((.x12 ↦ᵣ BitVec.ofNat 64 0) ** (Reg.x0 ↦ᵣ (0 : Word))
        ** copyInvF src dst srcWords dstWords init5 0) := by
    have h := countdownLoop_spec copy192Cr (0x8002ec58 : Word) (0x8002ec74 : Word)
  ...
  have hret := Fn.jalr_ret_spec (0x8002ec74 : Word) ret halign
...
def copy192Vals' : Reg → Word :=
  fun r => match r with | .x1 => (0x8003334c : Word) | _ => 0
...
theorem blsg2Copy192Frame_spec (sp0 ret src dst init5 v12 : Word)
    ...
    cpsTripleWithin (1 + copy192Frame.length + (1 + (1 + (24 * (6 + 1) + 1 + 1)))
      ...
      ((.x2 ↦ᵣ sp0) ** regsAt copy192Frame (copy192Vals ret)
        ...
        ** ((.x5 ↦ᵣ lastLoaded srcWords init5 24)
          ...
          ** (.x12 ↦ᵣ BitVec.ofNat 64 0) ** (Reg.x0 ↦ᵣ (0 : Word))
          ** dwordsIs src srcWords ** dwordsIs dst srcWords)) := by
  have hli := li_spec_gen_within .x12 v12 (24 : Word) (0x80033344 : Word) (by decide)
  rw [show (24 : Word) = BitVec.ofNat 64 24 from rfl] at hli
  rw [show (0x80033344 : Word) + 4 = (0x80033348 : Word) from by decide] at hli
  have hliC := liftCode (cr' := copy192Cr) hli (by code_mem)
  have hliF := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ ret) ** (.x5 ↦ᵣ init5) ** (.x10 ↦ᵣ src) ** (.x11 ↦ᵣ dst)
      ** (Reg.x0 ↦ᵣ (0 : Word)) ** dwordsIs src srcWords ** dwordsIs dst dstWords)
    (by pcf) hliC
  have hcallee := blsfCopyQuads24Flat_spec ((0x80033348 : Word) + 4) src dst init5
    srcWords dstWords hs hd (by decide)
  have hcall := callWithin_spec (0x80033348 : Word) (0x8002ec58 : Word) ret
    (jalOff GuestAddrs.blsf_copy_quads (GuestAddrs.blsg2_copy192 + 12))
    (24 * (6 + 1) + 1 + 1)
    (by decide) (by code_mem) (by pcf) hcallee
  rw [show (0x80033348 : Word) + 4 = (0x8003334c : Word) from by decide] at hcall
  ...
  have hbody : cpsTripleWithin (1 + (1 + (24 * (6 + 1) + 1 + 1)))
      ((0x8003333c : Word) + BitVec.ofNat 64 (4 * (1 + copy192Frame.length)))
      ((0x8003333c : Word) + BitVec.ofNat 64 (4 * (1 + copy192Frame.length + copy192Body.length)))
      copy192Cr
      ...
      ((.x2 ↦ᵣ (sp0 + signExtend12 (-16 : BitVec 12)))
        ...
        ** ((.x5 ↦ᵣ lastLoaded srcWords init5 24)
          ...
          ** (.x12 ↦ᵣ BitVec.ofNat 64 0) ** (Reg.x0 ↦ᵣ (0 : Word))
          ** dwordsIs src srcWords ** dwordsIs dst srcWords)) := by
    have hentry : (0x8003333c : Word) + BitVec.ofNat 64 (4 * (1 + copy192Frame.length))
        = (0x80033344 : Word) := by decide
    have hexit : (0x8003333c : Word)
          + BitVec.ofNat 64 (4 * (1 + copy192Frame.length + copy192Body.length))
        = (0x8003334c : Word) := by decide
    rw [hentry, hexit]
