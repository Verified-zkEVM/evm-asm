/-
  EvmAsm.Codegen.Programs.HeaderValidateParentHashMatch

  Match-path residual + adapter for `header_validate_parent_hash`
  (status = 0). Same namespace as `HeaderValidateParentHashSpec`.
-/
