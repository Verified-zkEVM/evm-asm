...
def k67Ambient (sp0 base omConst lenW v18 v19 v20 v21 : Word) (svals : Reg → Word) : Assertion :=
...
theorem k67Ambient_pcFree (sp0 base omConst lenW v18 v19 v20 v21 : Word) (svals : Reg → Word) :
    (k67Ambient sp0 base omConst lenW v18 v19 v20 v21 svals).pcFree := by
  unfold k67Ambient
  repeat' first
    | exact pcFree_regIs
    | exact pcFree_memIs
...
theorem k67InitStep
    (sp0 base omConst oldRa v12 v5 v6 v7 v28 v29 v30 v31 v18 v19 v20 v21 : Word) (svals : Reg → Word)
    ...
    cpsTripleWithin (1 + 81) (K + 40) (K + 44) fullCode
      ((.x1 ↦ᵣ oldRa) **
        ((.x10 ↦ᵣ base) ** (.x11 ↦ᵣ (BitVec.ofNat 64 lenN)) ** (.x12 ↦ᵣ v12) **
          ...
          k67Ambient sp0 base omConst (BitVec.ofNat 64 lenN) v18 v19 v20 v21 svals))
      (((k67InitCommon base bytes ** (.x0 ↦ᵣ (0 : Word))) **
          EvmAsm.Codegen.RlpListNthItemSAsm.initOutcome base bytes lenN hoff) **
        k67Ambient sp0 base omConst (BitVec.ofNat 64 lenN) v18 v19 v20 v21 svals) := by
  ...
  have hwiF := cpsTripleWithin_frameR
    (k67Ambient sp0 base omConst (BitVec.ofNat 64 lenN) v18 v19 v20 v21 svals)
    (k67Ambient_pcFree sp0 base omConst _ v18 v19 v20 v21 svals) hwi
...
theorem k67PrologueSetup (sp0 spC base omConst ret v8 v9 v18 v19 v20 v21 v12 v5 v6
    ...
    : cpsTripleWithin (10 + (1 + 81)) K (K + 44) fullCode
      ...
      (k67InitCommon base bytes ** (.x0 ↦ᵣ (0 : Word)) **
        EvmAsm.Codegen.RlpListNthItemSAsm.initOutcome base bytes lenN hoff **
        k67Ambient sp0 base omConst (BitVec.ofNat 64 lenN) v18 v19 (0 : Word) v21 (k67PrologueVals ret v8 v9 v18 v19 v20) **
  ...
  have hins := k67InitStep sp0 base omConst ret v12 v5 v6 v7 v28 v29 v30 v31
    v18 v19 (0 : Word) v21 (k67PrologueVals ret v8 v9 v18 v19 v20) bytes lenN
...
/-- Instructions 12–13: on the init-success path (`x12 = 0`, cursor and end
    pointer live in `x10`/`x11`), save them into the durable loop registers
    `x18`/`x19`.  The conclusion is the concrete loop-head state: `x18` and
    `x10` both hold the cursor, `x19` and `x11` both hold the end pointer,
    `x20 = 0`, `x12 = 0`; everything else framed through unchanged. -/
theorem k67PrologueFall
    (sp0 base omConst cursor endPtr lenW v18 v19 v21 : Word) (svals : Reg → Word)
    (bytes : List (BitVec 8)) :
    cpsTripleWithin 2 (K + 48) (K + 56) fullCode
      ((.x1 ↦ᵣ (K + 44)) ** (.x2 ↦ᵣ (sp0 + signExtend12 (-48 : BitVec 12))) **
        (.x8 ↦ᵣ base) ** (.x9 ↦ᵣ lenW) ** (.x10 ↦ᵣ cursor) **
        (.x11 ↦ᵣ endPtr) ** (.x12 ↦ᵣ (0 : Word)) ** (.x18 ↦ᵣ v18) **
      ...
      ((.x1 ↦ᵣ (K + 44)) ** (.x2 ↦ᵣ (sp0 + signExtend12 (-48 : BitVec 12))) **
        (.x8 ↦ᵣ base) ** (.x9 ↦ᵣ lenW) ** (.x10 ↦ᵣ cursor) **
        (.x11 ↦ᵣ endPtr) ** (.x12 ↦ᵣ (0 : Word)) ** (.x18 ↦ᵣ cursor) **
        (.x19 ↦ᵣ endPtr) ** (.x20 ↦ᵣ (0 : Word)) ** (.x21 ↦ᵣ v21) **
        regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** regOwn .x13 ** regOwn .x14 **
        regOwn .x28 ** regOwn .x29 ** regOwn .x30 ** regOwn .x31 **
        (.x0 ↦ᵣ (0 : Word)) **
        frameSlotsSaved k67Frame (sp0 + signExtend12 (-48 : BitVec 12)) svals **
  ...
  have h12 : cpsTripleWithin 1 (K + 48) (K + 48 + 4)
      (CodeReq.singleton (K + 48) (.MV .x18 .x10))
      ((.x10 ↦ᵣ cursor) ** (.x18 ↦ᵣ v18))
      ((.x10 ↦ᵣ cursor) ** (.x18 ↦ᵣ cursor)) :=
    mv_spec_gen_within .x18 .x10 cursor v18 (K + 48) (by decide)
  have h13 : cpsTripleWithin 1 (K + 52) (K + 52 + 4)
      (CodeReq.singleton (K + 52) (.MV .x19 .x11))
      ((.x11 ↦ᵣ endPtr) ** (.x19 ↦ᵣ v19))
      ((.x11 ↦ᵣ endPtr) ** (.x19 ↦ᵣ endPtr)) :=
    mv_spec_gen_within .x19 .x11 endPtr v19 (K + 52) (by decide)
  have h12C := cpsTripleWithin_extend_code
    (CodeReq.ofProg_mem_at K (K + 48) k67Prog 12 (.MV .x18 .x10)
      (by unfold K; bv_omega) (by rw [k67_length]; decide) rfl
      (by rw [k67_length]; decide)) h12
  have h13C := cpsTripleWithin_extend_code
    (CodeReq.ofProg_mem_at K (K + 52) k67Prog 13 (.MV .x19 .x11)
      (by unfold K; bv_omega) (by rw [k67_length]; decide) rfl
      (by rw [k67_length]; decide)) h13
  have hG12 : ((.x1 ↦ᵣ (K + 44)) ** (.x2 ↦ᵣ (sp0 + signExtend12 (-48 : BitVec 12))) **
        (.x8 ↦ᵣ base) ** (.x9 ↦ᵣ lenW) ** (.x11 ↦ᵣ endPtr) **
        (.x12 ↦ᵣ (0 : Word)) ** (.x19 ↦ᵣ v19) **
        (.x20 ↦ᵣ (0 : Word)) ** (.x21 ↦ᵣ v21) **
        regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** regOwn .x13 ** regOwn .x14 ** regOwn .x28 ** regOwn .x29 ** regOwn .x30 ** regOwn .x31 **
        (.x0 ↦ᵣ (0 : Word)) **
        frameSlotsSaved k67Frame (sp0 + signExtend12 (-48 : BitVec 12)) svals **
        bytesRegion base bytes **
    ...
    repeat' first
      ...
      | exact pcFree_emp
  have h12F :=
    cpsTripleWithin_frameR ((.x1 ↦ᵣ (K + 44)) ** (.x2 ↦ᵣ (sp0 + signExtend12 (-48 : BitVec 12))) **
        (.x8 ↦ᵣ base) ** (.x9 ↦ᵣ lenW) ** (.x11 ↦ᵣ endPtr) **
        (.x12 ↦ᵣ (0 : Word)) ** (.x19 ↦ᵣ v19) **
        (.x20 ↦ᵣ (0 : Word)) ** (.x21 ↦ᵣ v21) **
        regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** regOwn .x13 ** regOwn .x14 ** regOwn .x28 ** regOwn .x29 ** regOwn .x30 ** regOwn .x31 **
        ...
        frameSlotsSaved k67Frame (sp0 + signExtend12 (-48 : BitVec 12)) svals **
        bytesRegion base bytes **
        bytesRegion omConst (k67OmBytes)) hG12 h12C
  have hG13 : ((.x1 ↦ᵣ (K + 44)) ** (.x2 ↦ᵣ (sp0 + signExtend12 (-48 : BitVec 12))) **
        (.x8 ↦ᵣ base) ** (.x9 ↦ᵣ lenW) ** (.x10 ↦ᵣ cursor) **
        (.x18 ↦ᵣ cursor) ** (.x12 ↦ᵣ (0 : Word)) **
        (.x20 ↦ᵣ (0 : Word)) ** (.x21 ↦ᵣ v21) **
        regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** regOwn .x13 ** regOwn .x14 ** regOwn .x28 ** regOwn .x29 ** regOwn .x30 ** regOwn .x31 **
        (.x0 ↦ᵣ (0 : Word)) **
        frameSlotsSaved k67Frame (sp0 + signExtend12 (-48 : BitVec 12)) svals **
  ...
  have h13F :=
    cpsTripleWithin_frameR ((.x1 ↦ᵣ (K + 44)) ** (.x2 ↦ᵣ (sp0 + signExtend12 (-48 : BitVec 12))) **
        ...
        frameSlotsSaved k67Frame (sp0 + signExtend12 (-48 : BitVec 12)) svals **
        bytesRegion base bytes **
        bytesRegion omConst (k67OmBytes)) hG13 h13C
  have hfall :=
    cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) h12F h13F
  exact cpsTripleWithin_extend_code k67_mono
    (cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp)
      (fun _ hq => by xperm_hyp hq) hfall)

/-- Loop-body load (instructions 14-15): refresh the walker ABI from the
    durable cursor/end registers (`x18`/`x19`) into `x10`/`x11` ahead of the
    `rlp_walk_next` call at [16].  The old `x10`/`x11` values are dead
    (callee-clobbered outputs of the previous walk). -/
theorem k67LoopLoad
    (sp0 base omConst raVal v8 v9 v12 cursor endPtr iW x10Old x11Old v21 : Word) (svals : Reg → Word)
    (bytes : List (BitVec 8)) :
    cpsTripleWithin 2 (K + 56) (K + 64) fullCode
      ((.x1 ↦ᵣ raVal) ** (.x2 ↦ᵣ (sp0 + signExtend12 (-48 : BitVec 12))) **
        (.x8 ↦ᵣ v8) ** (.x9 ↦ᵣ v9) ** (.x10 ↦ᵣ x10Old) **
        (.x11 ↦ᵣ x11Old) ** (.x12 ↦ᵣ v12) ** (.x18 ↦ᵣ cursor) **
        (.x19 ↦ᵣ endPtr) ** (.x20 ↦ᵣ iW) ** (.x21 ↦ᵣ v21) **
        regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** regOwn .x13 ** regOwn .x14 **
        regOwn .x28 ** regOwn .x29 ** regOwn .x30 ** regOwn .x31 **
        (.x0 ↦ᵣ (0 : Word)) **
        frameSlotsSaved k67Frame (sp0 + signExtend12 (-48 : BitVec 12)) svals **
        ...
        bytesRegion omConst (k67OmBytes))
      ((.x1 ↦ᵣ raVal) ** (.x2 ↦ᵣ (sp0 + signExtend12 (-48 : BitVec 12))) **
        (.x8 ↦ᵣ v8) ** (.x9 ↦ᵣ v9) ** (.x10 ↦ᵣ cursor) **
        (.x11 ↦ᵣ endPtr) ** (.x12 ↦ᵣ v12) ** (.x18 ↦ᵣ cursor) **
        (.x19 ↦ᵣ endPtr) ** (.x20 ↦ᵣ iW) ** (.x21 ↦ᵣ v21) **
        regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** regOwn .x13 ** regOwn .x14 **
        regOwn .x28 ** regOwn .x29 ** regOwn .x30 ** regOwn .x31 **
        (.x0 ↦ᵣ (0 : Word)) **
  ...
  have h14 : cpsTripleWithin 1 (K + 56) (K + 56 + 4)
      ...
      ((.x18 ↦ᵣ cursor) ** (.x10 ↦ᵣ x10Old))
      ((.x18 ↦ᵣ cursor) ** (.x10 ↦ᵣ cursor)) :=
    mv_spec_gen_within .x10 .x18 cursor x10Old (K + 56) (by decide)
  ...
  have hG14 : ((.x1 ↦ᵣ raVal) ** (.x2 ↦ᵣ (sp0 + signExtend12 (-48 : BitVec 12))) **
        (.x8 ↦ᵣ v8) ** (.x9 ↦ᵣ v9) ** (.x11 ↦ᵣ x11Old) ** (.x12 ↦ᵣ v12) **
        (.x19 ↦ᵣ endPtr) ** (.x20 ↦ᵣ iW) ** (.x21 ↦ᵣ v21) **
        regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** regOwn .x13 ** regOwn .x14 **
        regOwn .x28 ** regOwn .x29 ** regOwn .x30 ** regOwn .x31 **
        (.x0 ↦ᵣ (0 : Word)) **
    ...
    repeat' first
      ...
      | exact pcFree_emp
  have h14F :=
    cpsTripleWithin_frameR ((.x1 ↦ᵣ raVal) ** (.x2 ↦ᵣ (sp0 + signExtend12 (-48 : BitVec 12))) **
        (.x8 ↦ᵣ v8) ** (.x9 ↦ᵣ v9) ** (.x11 ↦ᵣ x11Old) ** (.x12 ↦ᵣ v12) **
        (.x19 ↦ᵣ endPtr) ** (.x20 ↦ᵣ iW) ** (.x21 ↦ᵣ v21) **
        regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** regOwn .x13 ** regOwn .x14 **
        regOwn .x28 ** regOwn .x29 ** regOwn .x30 ** regOwn .x31 **
        ...
        frameSlotsSaved k67Frame (sp0 + signExtend12 (-48 : BitVec 12)) svals **
        bytesRegion base bytes **
        bytesRegion omConst (k67OmBytes)) hG14 h14C
  have hG15 : ((.x1 ↦ᵣ raVal) ** (.x2 ↦ᵣ (sp0 + signExtend12 (-48 : BitVec 12))) **
        (.x8 ↦ᵣ v8) ** (.x9 ↦ᵣ v9) ** (.x10 ↦ᵣ cursor) ** (.x12 ↦ᵣ v12) **
        (.x18 ↦ᵣ cursor) ** (.x20 ↦ᵣ iW) ** (.x21 ↦ᵣ v21) **
        regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** regOwn .x13 ** regOwn .x14 **
        regOwn .x28 ** regOwn .x29 ** regOwn .x30 ** regOwn .x31 **
        (.x0 ↦ᵣ (0 : Word)) **
        frameSlotsSaved k67Frame (sp0 + signExtend12 (-48 : BitVec 12)) svals **
  ...
  have h15F :=
    cpsTripleWithin_frameR ((.x1 ↦ᵣ raVal) ** (.x2 ↦ᵣ (sp0 + signExtend12 (-48 : BitVec 12))) **
        ...
        frameSlotsSaved k67Frame (sp0 + signExtend12 (-48 : BitVec 12)) svals **
        bytesRegion base bytes **
        bytesRegion omConst (k67OmBytes)) hG15 h15C
  have hload :=
    cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) h14F h15F
  exact cpsTripleWithin_extend_code k67_mono
    (cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp)
...
theorem k67LoopCall
    ...
    cpsTripleWithin (2 + (1 + 87)) (K + 56) (K + 68) fullCode
      ((.x1 ↦ᵣ raVal) ** (.x12 ↦ᵣ v12) **
        ...
        regOwn .x13 ** regOwn .x14 **
        bytesRegion base bytes ** (.x2 ↦ᵣ (sp0 + signExtend12 (-48 : BitVec 12))) ** (.x8 ↦ᵣ base) ** (.x9 ↦ᵣ v9) ** (.x18 ↦ᵣ (base + BitVec.ofNat 64 off)) ** (.x19 ↦ᵣ endPtr) ** (.x20 ↦ᵣ iW) ** (.x21 ↦ᵣ v21) ** frameSlotsSaved k67Frame (sp0 + signExtend12 (-48 : BitVec 12)) svals ** bytesRegion omConst (k67OmBytes))
      ((.x1 ↦ᵣ (K + 68)) **
        regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** regOwn .x28 ** regOwn .x29 **
        regOwn .x30 ** regOwn .x31 ** (.x0 ↦ᵣ (0 : Word)) **
        regOwn .x13 ** regOwn .x14 **
  ...
  have h15C := cpsTripleWithin_extend_code
    (CodeReq.ofProg_mem_at K (K + 60) k67Prog 15 (.MV .x11 .x19)
      (by unfold K; bv_omega) (by rw [k67_length]; decide) rfl
      (by rw [k67_length]; decide)) h15
  have hG14 : ((.x1 ↦ᵣ raVal) ** (.x12 ↦ᵣ v12) ** (.x5 ↦ᵣ v5) ** (.x6 ↦ᵣ v6) ** (.x7 ↦ᵣ v7) ** (.x28 ↦ᵣ v28) ** (.x29 ↦ᵣ v29) ** (.x30 ↦ᵣ v30) ** (.x31 ↦ᵣ v31) ** (.x0 ↦ᵣ (0 : Word)) ** (.x8 ↦ᵣ base) ** regOwn .x13 ** regOwn .x14 ** bytesRegion base bytes ** (.x2 ↦ᵣ (sp0 + signExtend12 (-48 : BitVec 12))) ** (.x9 ↦ᵣ v9) ** (.x20 ↦ᵣ iW) ** (.x21 ↦ᵣ v21) ** frameSlotsSaved k67Frame (sp0 + signExtend12 (-48 : BitVec 12)) svals ** bytesRegion omConst (k67OmBytes) ** (.x19 ↦ᵣ endPtr) ** (.x11 ↦ᵣ x11Old)).pcFree := by
    repeat' first
      | exact pcFree_regIs | exact pcFree_memIs | exact pcFree_memOwn
      | exact pcFree_regOwn | apply pcFree_sepConj
      | exact pcFree_frameSlotsSaved _ _ _ | exact bytesRegion_pcFree _ _
      | exact pcFree_emp
  have hG15 : ((.x1 ↦ᵣ raVal) ** (.x12 ↦ᵣ v12) ** (.x5 ↦ᵣ v5) ** (.x6 ↦ᵣ v6) ** (.x7 ↦ᵣ v7) ** (.x28 ↦ᵣ v28) ** (.x29 ↦ᵣ v29) ** (.x30 ↦ᵣ v30) ** (.x31 ↦ᵣ v31) ** (.x0 ↦ᵣ (0 : Word)) ** (.x8 ↦ᵣ base) ** regOwn .x13 ** regOwn .x14 ** bytesRegion base bytes ** (.x2 ↦ᵣ (sp0 + signExtend12 (-48 : BitVec 12))) ** (.x9 ↦ᵣ v9) ** (.x20 ↦ᵣ iW) ** (.x21 ↦ᵣ v21) ** frameSlotsSaved k67Frame (sp0 + signExtend12 (-48 : BitVec 12)) svals ** bytesRegion omConst (k67OmBytes) ** (.x18 ↦ᵣ (base + BitVec.ofNat 64 off)) ** (.x10 ↦ᵣ (base + BitVec.ofNat 64 off))).pcFree := by
    repeat' first
      | exact pcFree_regIs | exact pcFree_memIs | exact pcFree_memOwn
      | exact pcFree_regOwn | apply pcFree_sepConj
      | exact pcFree_frameSlotsSaved _ _ _ | exact bytesRegion_pcFree _ _
  ...
  have hwalk := k67WalkNextStep
    (F := (.x2 ↦ᵣ (sp0 + signExtend12 (-48 : BitVec 12))) **
        ...
        regOwn .x13 ** regOwn .x14 **
        frameSlotsSaved k67Frame (sp0 + signExtend12 (-48 : BitVec 12)) svals **
        bytesRegion omConst (k67OmBytes)) hF
