...
open EvmAsm.Codegen.TxExtractToAddressModel
  (extractSuccess teerExtractToAddress decodeListItems decodeListItems_some_ne_nil
    ...
    extractSuccess_creation extractSuccess_to_field)
...
/-! ### Unified short-creation ambient packaging

Residual only type-dependent `hgeN` (`toFieldIndex ty + 2 ≤ items.length`):
legacy 5, t1 6, type234 7 — empty-`to` hss room past to-field.
-/

/-- Package short-creation arm from hshort (any success type). Residual unified hgeN. -/
def extractAssumedAmbientArm_creShort_of_hshort
    (regionBase : Word) (bs : List (BitVec 8)) (off len : Nat)
    (items : List RLPItem)
    (hsuccess : extractSuccess (txSlice bs off len))
    (hcre : (teerExtractToAddress (txSlice bs off len)).2.2 = (1 : Word))
  ...
  · have htype0 : (teerTxTypeDispatch slice).2.1 = (0 : Word) := by
    ...
    have hge5 : 5 ≤ items.length := by
      simpa [slice, ty, h0, toFieldIndex_legacy] using hgeN
    exact extractAssumedAmbientArm_creShortLegacy_of_hshort
      regionBase bs off len items hsuccess
      (by simpa [slice] using hcre) htype0 (by simpa [slice] using hdec)
      hshort hge5
  · by_cases h1 : ty = 1
    · have htype1 : (teerTxTypeDispatch slice).2.1 = (1 : Word) := by
      ...
      have hge6 : 6 ≤ items.length := by
        simpa [slice, ty, h0, h1, toFieldIndex_t1] using hgeN
      exact extractAssumedAmbientArm_creShortT1_of_hshort
        regionBase bs off len items hsuccess
        (by simpa [slice] using hcre) htype1 (by simpa [slice] using hdec)
        hshort hge6
    · have hge : 2 ≤ ty := by omega
      ...
      exact extractAssumedAmbientArm_creShortType234_of_hshort
        regionBase bs off len items hsuccess
        (by simpa [slice] using hcre) (by simpa [slice, ty] using hge)
        (by simpa [slice] using hdec) hshort hge7

set_option maxRecDepth 8000 in
/-- Short-creation ambient unified: residual only `hgeN` (`toFieldIndex+2 ≤ length`) + `hshort`.
    classical-3. -/
theorem extractAssumed_success_flat_ambient_creShort_of_hshort
    (ret spVal regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 : Word)
    (bs : List (BitVec 8)) (off len : Nat) (items : List RLPItem)
    (hret : (ret &&& ~~~(1 : Word)) = ret)
  ...
  extractAssumed_success_flat_ambient_of_arm
    ...
    s0 s1 s2 s3 s4 s5 s6 s7 bs off len
    hret hptr hlenW hsalign hbound hover hvalidBuf htalign htover htvalid
    (extractAssumedAmbientArm_creShort_of_hshort regionBase bs off len items
      hsuccess hcre hdec hshort hgeN)

set_option maxRecDepth 8000 in
/-- Obtain `hdec`/`hcre` from extractSuccess + empty to. Residual `hshort` + `hgeN`. classical-3. -/
theorem extractAssumed_success_flat_ambient_creShort_of_success_hshort
    (ret spVal regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (hret : (ret &&& ~~~(1 : Word)) = ret)
    ...
    (htvalid : isValidMemAccess (toBuf + (16 : Word)) = true)
    (hsuccess : extractSuccess (txSlice bs off len))
    (hempty : (teerExtractToAddress (txSlice bs off len)).2.1 = [])
    (hshort : ∀ items,
      decodeListItems
          ((txSlice bs off len).drop
            (teerTxTypeDispatch (txSlice bs off len)).2.2.toNat) = some items →
    ...
    cpsTripleWithin nExtractSteps E ret fullCode
      ...
      (extractAssumedPostAmbient ret spVal
        ...
        regionBase toBuf isCreationPtr bs) := by
  set slice := txSlice bs off len
  have hcre := extractSuccess_creation slice hsuccess hempty
  obtain ⟨items, hdec⟩ := extractSuccess_decode slice hsuccess
  exact extractAssumed_success_flat_ambient_creShort_of_hshort
    ret spVal regionBase loadPtr lenW toBuf isCreationPtr
    s0 s1 s2 s3 s4 s5 s6 s7 bs off len items
    hret hptr hlenW hsalign hbound hover hvalidBuf htalign htover htvalid
    hsuccess hcre hdec (hshort items hdec) (hgeN items hdec)

set_option maxRecDepth 8000 in
/-- Short-creation ambient from extractSuccess + empty to + short-list head.
    Residual only unified `hgeN`. classical-3. -/
theorem extractAssumed_success_flat_ambient_creShort_of_success_head
    (ret spVal regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (hret : (ret &&& ~~~(1 : Word)) = ret)
  ...
  exact extractAssumed_success_flat_ambient_creShort_of_hshort
    ...
    s0 s1 s2 s3 s4 s5 s6 s7 bs off len items
    hret hptr hlenW hsalign hbound hover hvalidBuf htalign htover htvalid
    hsuccess hcre hdec hshort (hgeN items hdec)

#print axioms extractAssumedAmbientArm_creShort_of_hshort
#print axioms extractAssumed_success_flat_ambient_creShort_of_hshort
#print axioms extractAssumed_success_flat_ambient_creShort_of_success_hshort
#print axioms extractAssumed_success_flat_ambient_creShort_of_success_head
...
/-! ### Long-creation ambient unified type case-split

Dual of creShort_of_hshort. Residual:
* `hgeN` (`toFieldIndex+2 ≤ length`)
* `hitems` — every non-`to` field encodes ≤ 55
* long-list head (empty-to hitem free via encode_le55)
-/

/-- Non-`to` fields encode as short items (≤ 55 bytes). -/
def creLongNonToHitems
    (ty : Nat) (items : List RLPItem)
    (hgeN : toFieldIndex ty + 2 ≤ items.length) : Prop :=
...
def extractAssumedAmbientArm_creLong_of_hlong
  ...
  · have htype0 : (teerTxTypeDispatch slice).2.1 = (0 : Word) := by
    ...
    have hitem3 : (encode (items[3]'(by omega))).length ≤ 55 := by
      have hidx' : toFieldIndex (teerTxTypeDispatch slice).2.1.toNat = 3 := by
        simp [slice, ty, hz, toFieldIndex_legacy]
      have hn : toFieldIndex (teerTxTypeDispatch slice).2.1.toNat < items.length := by
        simpa [hidx'] using (by omega : 3 < items.length)
      have hle :=
        TxExtractToAddressHonesty.extractSuccess_creation_to_field_encode_le55_of
          slice hsuccess (by simpa [slice] using hcre) items
          (by simpa [slice] using hdec) hn
      simpa [hidx'] using hle
    exact extractAssumedAmbientArm_creLongLegacy
      regionBase bs off len items hbound hsuccess
      (by simpa [slice] using hcre) htype0 (by simpa [slice] using hdec)
      (by simpa [slice] using h0) (by simpa [slice] using hge_f8)
      hge5 hitem0 hitem1 hitem2 hitem3
  · by_cases h1 : ty = 1
    · have htype1 : (teerTxTypeDispatch slice).2.1 = (1 : Word) := by
        apply BitVec.eq_of_toNat_eq
        have : ty = 1 := h1
      ...
      have hitem4 : (encode (items[4]'(by omega))).length ≤ 55 := by
        have hidx' : toFieldIndex (teerTxTypeDispatch slice).2.1.toNat = 4 := by
          simp [slice, ty, h1, toFieldIndex_t1]
        have hn : toFieldIndex (teerTxTypeDispatch slice).2.1.toNat < items.length := by
          simpa [hidx'] using (by omega : 4 < items.length)
        have hle :=
          TxExtractToAddressHonesty.extractSuccess_creation_to_field_encode_le55_of
            slice hsuccess (by simpa [slice] using hcre) items
            (by simpa [slice] using hdec) hn
        simpa [hidx'] using hle
      exact extractAssumedAmbientArm_creLongT1
        regionBase bs off len items hbound hsuccess
        (by simpa [slice] using hcre) htype1 (by simpa [slice] using hdec)
        (by simpa [slice] using h0) (by simpa [slice] using hge_f8)
        hge6 hitem0 hitem1 hitem2 hitem3 hitem4
    · have hge : 2 ≤ ty := by omega
      have hidx : toFieldIndex ty = 5 := toFieldIndex_type234 ty hge hle
      have hge7 : 7 ≤ items.length := by
        simpa [slice, ty, hidx] using hgeN
      ...
      have hitem5 : (encode (items[5]'(by omega))).length ≤ 55 := by
        have hidx' : toFieldIndex (teerTxTypeDispatch slice).2.1.toNat = 5 := by
          simp [slice, ty, hidx]
        have hn : toFieldIndex (teerTxTypeDispatch slice).2.1.toNat < items.length := by
          simpa [hidx'] using (by omega : 5 < items.length)
        have hle :=
          TxExtractToAddressHonesty.extractSuccess_creation_to_field_encode_le55_of
            slice hsuccess (by simpa [slice] using hcre) items
            (by simpa [slice] using hdec) hn
        simpa [hidx'] using hle
      exact extractAssumedAmbientArm_creLongType234
        regionBase bs off len items hbound hsuccess
        (by simpa [slice] using hcre) (by simpa [slice, ty] using hge)
        (by simpa [slice] using hdec)
        (by simpa [slice] using h0) (by simpa [slice] using hge_f8)
        hge7 hitem0 hitem1 hitem2 hitem3 hitem4 hitem5

set_option maxRecDepth 8000 in
/-- Long-creation ambient unified: residual `hgeN` + non-to `hitems` + long head. classical-3. -/
theorem extractAssumed_success_flat_ambient_creLong_of_hlong
    (ret spVal regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 : Word)
    (bs : List (BitVec 8)) (off len : Nat) (items : List RLPItem)
    (hret : (ret &&& ~~~(1 : Word)) = ret)
  ...
  extractAssumed_success_flat_ambient_of_arm
    ...
    s0 s1 s2 s3 s4 s5 s6 s7 bs off len
    hret hptr hlenW hsalign hbound hover hvalidBuf htalign htover htvalid
    (extractAssumedAmbientArm_creLong_of_hlong regionBase bs off len items
      hbound hsuccess hcre hdec h0 hge_f8 hgeN hitems)

set_option maxRecDepth 8000 in
/-- Obtain `hdec`/`hcre` from extractSuccess + empty to.
    Residual long head + `hgeN` + non-to `hitems`. classical-3. -/
theorem extractAssumed_success_flat_ambient_creLong_of_success_hlong
    (ret spVal regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (hret : (ret &&& ~~~(1 : Word)) = ret)
  ...
  have hpos : 0 <
      (slice.drop (teerTxTypeDispatch slice).2.2.toNat).length :=
    decodeListItems_drop_pos slice _ items (by simpa [slice] using hdec)
  exact extractAssumed_success_flat_ambient_creLong_of_hlong
    ret spVal regionBase loadPtr lenW toBuf isCreationPtr
    s0 s1 s2 s3 s4 s5 s6 s7 bs off len items
    hret hptr hlenW hsalign hbound hover hvalidBuf htalign htover htvalid
    hsuccess hcre hdec
...
set_option maxRecDepth 8000 in
/-- Long-creation ambient from extractSuccess + empty to + long-list head.
    Residual unified `hgeN` + non-to `hitems`. classical-3. -/
theorem extractAssumed_success_flat_ambient_creLong_of_success_head
    (ret spVal regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (hret : (ret &&& ~~~(1 : Word)) = ret)
    ...
    cpsTripleWithin nExtractSteps E ret fullCode
      ...
      (extractAssumedPostAmbient ret spVal
        s0 s1 s2 s3 s4 s5 s6 s7
        regionBase toBuf isCreationPtr bs) :=
  extractAssumed_success_flat_ambient_creLong_of_success_hlong
    ret spVal regionBase loadPtr lenW toBuf isCreationPtr
    s0 s1 s2 s3 s4 s5 s6 s7 bs off len
    hret hptr hlenW hsalign hbound hover hvalidBuf htalign htover htvalid
    hsuccess hempty hge_f8 hgeN hitems
...
* `hitems` — non-content encode ≤ 55 (`copyLongNonContentHitems`)
  (hgeN/hgeC DROPPED: content field exists ⇒ toField+1 ≤ length)
* content-start `% 8 = 0` (`halign`); q/hq derived in per-type flats
* long-list head
-/

/-- Content field exists under copy+decode ⇒ `toFieldIndex + 1 ≤ items.length`. -/
theorem hgeC_of_copy_content_field
    ...
    (hdec : decodeListItems (txBytes.drop (teerTxTypeDispatch txBytes).2.2.toNat) =
      some items) :
    toFieldIndex (teerTxTypeDispatch txBytes).2.1.toNat + 1 ≤ items.length := by
  obtain ⟨items', content, hdec', hitem, _hlen20, _henc⟩ :=
    extractSuccess_copy_encode_addr20 txBytes h hcopy
  have hitems : items = items' := by
    exact Option.some.inj (hdec.symm.trans hdec')
  subst hitems
  have hn : toFieldIndex (teerTxTypeDispatch txBytes).2.1.toNat < items.length :=
    (List.getElem?_eq_some_iff.1 hitem).1
  exact Nat.succ_le_of_lt hn

#print axioms hgeC_of_copy_content_field

set_option maxRecDepth 8000 in
/-- Long-copy ambient unified: residual non-content `hitems` + `halign` + long head.
    `hgeC` free from content field. -/
theorem extractAssumed_success_flat_ambient_copyLong_of_hlong
    ...
    (hitems : ∀ hgeC : toFieldIndex (teerTxTypeDispatch (txSlice bs off len)).2.1.toNat + 1 ≤
        items.length,
  ...
  have hle : ty ≤ 4 := by
    simpa [slice, ty] using extractSuccess_type_le4 slice hsuccess
  have hgeC : toFieldIndex (teerTxTypeDispatch slice).2.1.toNat + 1 ≤ items.length :=
    hgeC_of_copy_content_field slice hsuccess (by simpa [slice] using hcopyFlag) items
      (by simpa [slice] using hdec)
  have hitems' := hitems hgeC
...
/-- Obtain `hdec`/`hcopy` from extractSuccess + 20B content.
    Residual long head + non-content `hitems` + `halign` (hgeN/hgeC DROPPED). classical-3. -/
theorem extractAssumed_success_flat_ambient_copyLong_of_success_hlong
  ...
  exact extractAssumed_success_flat_ambient_copyLong_of_hlong
    ...
    (by simpa [slice] using hpos)
    (hge_f8 (by simpa [slice] using hpos))
    (fun hgeC => hitems items hdec hgeC)
    (halign items hdec)

set_option maxRecDepth 8000 in
/-- Long-copy ambient from extractSuccess + 20B content + long-list head + align.
    Residual non-content `hitems` + `halign` (hgeC free from content field). classical-3. -/
theorem extractAssumed_success_flat_ambient_copyLong_of_success_head_aligned
  ...
  extractAssumed_success_flat_ambient_copyLong_of_success_hlong
    ...
    s0 s1 s2 s3 s4 s5 s6 s7 bs off len
    hret hptr hlenW hsalign hbound hover hvalidBuf htalign htover htvalid
    hsuccess hcontent hge_f8 hitems halign
