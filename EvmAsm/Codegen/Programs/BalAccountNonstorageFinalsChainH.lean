/-
  EvmAsm.Codegen.Programs.BalAccountNonstorageFinalsChainH

  Outer nonce-field assembly for bal_account_nonstorage_finals.
-/
...
theorem bansf_nonceFieldInit97_spec (aB : Word) (aLen fOff : Nat) (fSpanW : Word)
    ...
    cpsBranchWithin 84 (B + 388) bansfCR
      ...
      (B + 396) (fieldInitPost aB fOff fSpanW.toNat acctBytes (B + 388 + 4) F) := by
  have hoffb : fOff < acctBytes.length := by omega
  have hovOff : aB.toNat + fOff < 2 ^ 64 := by omega
  -- the callee triple at its entry with ra = B + 388 + 4
  have hwi := rlp_walk_init_spec_within WI aB (B + 388 + 4) fSpanW
  ...
  have hcall := bansf_callSite97_walk_init (n := 81) vRa (by pcf) hwi'
  rw [show (B + 388) + 4 = B + 392 from by bv_omega] at hcall
  ...
  have hsucc : cpsBranchWithin 2 (B + 392) bansfCR
      (fun h => ∃ cOff : Nat,
  ...
  have hfailc : cpsBranchWithin 2 (B + 392) bansfCR
    ...
    have hf := bansf_nonceFieldInitFailure98_spec aB cur endW k acctBytes F hF hk
    exact cpsTripleWithin_as_cpsBranchWithin_left (B + 396)
      (fieldInitPost aB fOff fSpanW.toNat acctBytes (B + 388 + 4) F) hf
  ...
  have rebuild : ∀ (arm : Assertion), arm h4 →
      ((((regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** regOwn .x28 ** regOwn .x29 **
          ...
          ((.x1 : Reg) ↦ᵣ (B + 388 + 4)) ** bytesRegion aB acctBytes) ** arm) ** F)) h :=
...
theorem bansf_item4_spec (aB newSp : Word) (aLen off : Nat)
    ...
    (hvalid : ∀ k, k < acctBytes.length →
      isValidByteAccess (aB + BitVec.ofNat 64 k) = true)
    (hoffle : off ≤ aLen) :
    cpsBranchWithin 93 (B + 352) bansfCR
      ...
      (B + 372) (itemOk aB newSp aLen off acctBytes F) := by
  ...
  have hld1 := ld_spec_gen_within .x10 .x2 newSp v10 (aB + BitVec.ofNat 64 off)
    (48 : BitVec 12) (B + 352) (by decide)
  rw [show signExtend12 (48 : BitVec 12) = (48 : Word) from by decide, show (B + 352) + 4 = B + 356 from by bv_omega] at hld1
  have hld1L := liftCode (cr' := bansfCR) hld1
    bansf_item4_code.1
  have hld2 := ld_spec_gen_within .x11 .x2 newSp v11 (aB + BitVec.ofNat 64 aLen)
    (56 : BitVec 12) (B + 356) (by decide)
  rw [show signExtend12 (56 : BitVec 12) = (56 : Word) from by decide, show (B + 356) + 4 = B + 360 from by bv_omega] at hld2
  have hld2L := liftCode (cr' := bansfCR) hld2
    bansf_item4_code.2.1
  ...
  -- the callee triple with ra = B + 360 + 4
  have hwn := rlp_walk_next_spec_within WN aB (aB + BitVec.ofNat 64 aLen)
    (B + 360 + 4) v12 v5 v6 v7 v28 v29 v30 v31 acctBytes off hsalign hoffb (by omega)
    (hvalid off hoffb)
  ...
  have hwn' := cpsTripleWithin_weaken
    ...
    (P' := ((.x1 : Reg) ↦ᵣ (B + 360 + 4)) **
      (((.x10 : Reg) ↦ᵣ (aB + BitVec.ofNat 64 off)) **
       ...
       ((.x30 : Reg) ↦ᵣ v30) ** ((.x31 : Reg) ↦ᵣ v31) **
       ((.x0 : Reg) ↦ᵣ (0 : Word)) ** bytesRegion aB acctBytes))
  have hcall := bansf_callSite90_walk_next (n := 87) vRa (by pcf) hwn'
  rw [show (B + 360) + 4 = B + 364 from by bv_omega] at hcall
  ...
  have hokc : cpsBranchWithin 2 (B + 364) bansfCR
      (fun h => ∃ next len : Word,
        ((((.x10 : Reg) ↦ᵣ next) ** ((.x11 : Reg) ↦ᵣ (0 : Word)) **
          ...
          regOwn .x5 ** regOwn .x6 ** regOwn .x7 **
          regOwn .x28 ** regOwn .x29 ** regOwn .x30 ** regOwn .x31 **
          ((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x1 : Reg) ↦ᵣ (B + 360 + 4)) **
          bytesRegion aB acctBytes ** F) **
    ...
    have hbne := bne_spec_gen_within .x11 .x0 (368 : BitVec 13) (0 : Word) (0 : Word) (B + 364)
    rw [show (B + 364) + 4 = B + 368 from by bv_omega] at hbne
    have hbneL := cpsBranchWithin_extend_code (cr' := bansfCR)
      bansf_item4_code.2.2.1
      hbne
    ...
    have hsd := sd_spec_gen_within .x2 .x10 newSp next (aB + BitVec.ofNat 64 off)
      (48 : BitVec 12) (B + 368)
    rw [show signExtend12 (48 : BitVec 12) = (48 : Word) from by decide, show (B + 368) + 4 = B + 372 from by bv_omega] at hsd
    have hsdL := liftCode (cr' := bansfCR) hsd
      bansf_item4_code.2.2.2.1
    ...
    have hout : cpsTripleWithin 2 (B + 364) (B + 372) bansfCR
        (((.x10 : Reg) ↦ᵣ next) ** ((.x11 : Reg) ↦ᵣ (0 : Word)) **
         ((.x12 : Reg) ↦ᵣ len) **
         ((.x2 : Reg) ↦ᵣ newSp) **
         ((newSp + 48) ↦ₘ (aB + BitVec.ofNat 64 off)) **
  ...
  -- ===== fail continuation =====
  have hfailc : cpsBranchWithin 2 (B + 364) bansfCR
      (fun h => ∃ cur k : Word,
    ...
    refine cpsBranchWithin_exists_pre (fun k => ?_)
    refine cpsBranchWithin_pure_pre_right (fun hk => ?_)
    have hbne := bne_spec_gen_within .x11 .x0 (368 : BitVec 13) k (0 : Word) (B + 364)
    rw [show (B + 364) + signExtend13 (368 : BitVec 13) = B + 732 from by
          rw [show signExtend13 (368 : BitVec 13) = (368 : Word) from by decide]
          bv_omega,
        show (B + 364) + 4 = B + 368 from by bv_omega] at hbne
    have hbneL := cpsBranchWithin_extend_code (cr' := bansfCR)
      bansf_item4_code.2.2.1
      hbne
    ...
    have hchain := cpsTripleWithin_seq_perm_same_cr
      (fun h hp => by
        ...
        xperm_hyp hp2)
      htaken hrejF
    have hout : cpsTripleWithin 2 (B + 364) (B + 736) bansfCR
        (((.x10 : Reg) ↦ᵣ cur) ** ((.x11 : Reg) ↦ᵣ k) **
         ((.x12 : Reg) ↦ᵣ (0 : Word)) **
         ((.x2 : Reg) ↦ᵣ newSp) **
         ((newSp + 48) ↦ₘ (aB + BitVec.ofNat 64 off)) **
  ...
  have rebuild : ∀ (arm : Assertion), arm h4 →
      ((((regOwn .x5 ** regOwn .x6 ** regOwn .x7 ** regOwn .x28 ** regOwn .x29 **
          regOwn .x30 ** regOwn .x31 ** ((.x0 : Reg) ↦ᵣ (0 : Word)) **
          ((.x1 : Reg) ↦ᵣ (B + 360 + 4)) ** bytesRegion aB acctBytes) ** arm) **
        (((.x2 : Reg) ↦ᵣ newSp) **
         ((newSp + 48) ↦ₘ (aB + BitVec.ofNat 64 off)) **
...
def nonceStationOuterPost (aB newSp oB : Word) (aLen off : Nat)
    (acctBytes : List (BitVec 8)) (G F : Assertion) : Assertion :=
  fun h => ∃ n4 l4 : Word,
    (nonceStationPost aB newSp oB aLen ((n4 - l4 - aB).toNat)
        l4.toNat n4 acctBytes G F **
      ⌜rlpItemDecode acctBytes off (aB + BitVec.ofNat 64 off)
        (aB + BitVec.ofNat 64 aLen) n4 l4⌝) h

/-- Stable state entering the outer nonce item, excluding caller-saved
    register ownership peeled by the station theorem. -/
def nonceStationOuterBase (aB newSp oB : Word) (aLen off : Nat)
    (acctBytes : List (BitVec 8)) (G F : Assertion) : Assertion :=
  ((newSp + 48) ↦ₘ (aB + BitVec.ofNat 64 off)) **
  ((newSp + 56) ↦ₘ (aB + BitVec.ofNat 64 aLen)) **
  ((.x2 : Reg) ↦ᵣ newSp) ** memOwn (newSp + 64) ** memOwn (newSp + 72) **
...
/-- **The nonce station** (`B + 352 → B + 736 | B + 540`): decode outer
    account item 4, parse its last nonce tuple, and materialize the nonce
    result while retaining the full outer/inner derivation. -/
theorem bansf_nonceStation_spec (aB newSp oB : Word) (aLen off : Nat)
    (acctBytes : List (BitVec 8)) (G F : Assertion)
    (hG : G.pcFree) (hF : F.pcFree)
    (hsalign : aB.toNat % 8 = 0)
    (hslack : aLen + 9 ≤ acctBytes.length)
    ...
    cpsBranchWithin (98 * (aLen + 1) + (7 * acctBytes.length + 800))
      ...
      (((nonceStationOuterBase aB newSp oB aLen off acctBytes G F **
       ...
       regOwn .x29 ** regOwn .x30 ** regOwn .x31 ** regOwn .x1))
      (B + 736) (nonceStationRej aB newSp oB aLen acctBytes G F)
      (B + 540) (nonceStationOuterPost aB newSp oB aLen off acctBytes G F) := by
  refine cpsBranchWithin_of_forall_regIs_to_regOwn8
    (fun v5 v6 v7 v28 v29 v30 v31 vRa => ?_)
  let V8 : Assertion :=
    ((.x5 : Reg) ↦ᵣ v5) ** ((.x6 : Reg) ↦ᵣ v6) **
  ...
  let H : Assertion :=
    ...
    ((.x9 : Reg) ↦ᵣ BitVec.ofNat 64 aLen) ** ((.x18 : Reg) ↦ᵣ oB) **
    ((.x19 : Reg) ↦ᵣ v19) ** ((.x20 : Reg) ↦ᵣ v20)
  have hi := bansf_item4_spec aB newSp aLen off acctBytes
  ...
  have hiW := cpsBranchWithin_weaken
    ...
    (fun h hq => by
      ...
      exact item4Reject_to_nonceStationRej aB newSp oB aLen
        acctBytes G F h (by xperm_hyp hqOwn))
    (fun _ x => x) hiF
  let SpanPre : Word → Word → Assertion := fun n4 l4 =>
    (((.x10 : Reg) ↦ᵣ n4) ** ((.x11 : Reg) ↦ᵣ (0 : Word)) **
     ((.x12 : Reg) ↦ᵣ l4) ** ((.x19 : Reg) ↦ᵣ v19) **
     ((.x20 : Reg) ↦ᵣ v20) ** ((.x2 : Reg) ↦ᵣ newSp) **
     ((newSp + 48) ↦ₘ n4) **
     ...
     ((oB + 56) ↦ₘ (0 : Word)) ** ((oB + 64) ↦ₘ (0 : Word)) **
     ((oB + 72) ↦ₘ (0 : Word)) ** bytesRegion aB acctBytes ** F)
  let ItemSuccess : Assertion := fun h => ∃ n4 l4 : Word,
    (SpanPre n4 l4 **
     ⌜rlpItemDecode acctBytes off (aB + BitVec.ofNat 64 off)
       (aB + BitVec.ofNat 64 aLen) n4 l4⌝) h
  have hcont : cpsBranchWithin
      (98 * (aLen + 1) + (7 * acctBytes.length + 700))
      (B + 372) bansfCR ItemSuccess
      (B + 736) (nonceStationRej aB newSp oB aLen acctBytes G F)
      (B + 540) (nonceStationOuterPost aB newSp oB aLen off acctBytes G F) := by
    refine cpsBranchWithin_exists_pre (fun n4 => ?_)
    refine cpsBranchWithin_exists_pre (fun l4 => ?_)
    refine cpsBranchWithin_pure_pre_right (fun hdec => ?_)
    have hc := bansf_nonceStationCont372_spec aB newSp oB aLen off
      n4 l4 v19 v20 acctBytes G F hG hF hsalign hslack hover hvalid hoff hdec
    refine cpsBranchWithin_weaken
      (fun h hp => by xperm_hyp hp)
      (fun _ x => x) (fun h hq => ?_) hc
    unfold nonceStationOuterPost
    exact ⟨n4, l4, (sepConj_pure_right h).2 ⟨hq, hdec⟩⟩
  have hcont' := cpsBranchWithin_weaken
    (fun h hp => by
      have hp' := item4Ok_to_nonceSpanPre aB newSp oB aLen off v19 v20
        acctBytes G F h hp
  ...
  exact cpsBranchWithin_weaken
    (fun h hp => by
      unfold nonceStationOuterBase at hp
      dsimp only [H, V8]
      xperm_hyp hp)
    (fun _ x => x) (fun _ x => x)
    (cpsBranchWithin_mono_nSteps (by omega) hfull)
