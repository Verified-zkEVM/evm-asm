...
"
  ...
  (if !valueBearing then "" else
    ...
    "  addi sp, sp, -32\n  sd x10, 0(sp)\n  sd x12, 8(sp)\n  sd x13, 16(sp)\n" ++
    "  la a0, nse_callee_be\n  la a1, nse_acct\n  addi a1, a1, 8\n" ++
    "  li a2, 5; jal ra, account_writes_latest_balance\n  mv t6, a0\n" ++
    "  ld x10, 0(sp)\n  ld x12, 8(sp)\n  ld x13, 16(sp)\n  addi sp, sp, 32\n" ++
  ...
  (if valueBearing then
     ...
     "  beqz t2, .Lcd_ib_no_delegate_" ++ tag ++ "\n" ++
     "  li t3, 1; beq t2, t3, .Lcd_ib_no_delegate_" ++ tag ++ "\n" ++
     "  li t3, 5; bne t2, t3, .Lcd_ib_cahsr_bad_" ++ tag ++ "\n" ++
     "  la t3, cd_empty_code_hash\n" ++
     "  la t4, cahsr_acct_struct\n" ++
     "  ld t5, 0(t3); ld t6, 72(t4); bne t5, t6, .Lcd_ib_cahsr_bad_" ++ tag ++ "\n" ++
     "  ld t5, 8(t3); ld t6, 80(t4); bne t5, t6, .Lcd_ib_cahsr_bad_" ++ tag ++ "\n" ++
     "  ld t5, 16(t3); ld t6, 88(t4); bne t5, t6, .Lcd_ib_cahsr_bad_" ++ tag ++ "\n" ++
     "  ld t5, 24(t3); ld t6, 96(t4); bne t5, t6, .Lcd_ib_cahsr_bad_" ++ tag ++ "\n" ++
     "  j .Lcd_ib_no_delegate_" ++ tag ++ "\n" ++
     ".Lcd_ib_cahsr_bad_" ++ tag ++ ":\n" ++
     "  la t0, ib_deleg_cahsr_unresolved_flag; li t1, 1; sd t1, 0(t0)\n" ++
  ...
  -- Every successfully descended CALL-family kind reaches the common
  -- `process_message` body entry after `call_frame_descend` has installed the
  -- child frame's code, stack, memory, environment, and rollback state.
  -- Root-only transaction preparation and the root global stack/memory
  -- bootstrap remain before this seam. CALLCODE (mode 2) retains its separate
  -- pre-descent value-bearing balance gate.
  (if mode == 0 || mode == 1 || mode == 2 || mode == 3 then
    s!"  j {runtimeMessageEntryLabel}"
   else
    dispatchContinueRet)


-- [PR #26] ChildFrameHandlerTailsGuards folded into ChildFrameHandlers (single importer).
