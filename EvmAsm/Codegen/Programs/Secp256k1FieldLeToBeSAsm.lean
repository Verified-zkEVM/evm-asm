...
private theorem inner_step_engine (src dst : Word) (inBytes : List (BitVec 8))
  ...
  obtain ⟨hp29, hp28, hp6, hp5, hp10, hp11, hpws, hpfr, hpbytes, hpframe⟩ := hInv
...
private theorem outer_step_engine (src dst : Word) (inBytes : List (BitVec 8))
  ...
  obtain ⟨-, -, -, hp5, hp10, hp11, hpws, hpfr, hpbytes, hpframe⟩ := hInv
  ...
  dsimp only [outerInv]
  refine ⟨hb5, hb6, hb10.trans hp10, hb11.trans hp11, ?_, hpfr, ?_⟩
  · rw [hb2]; exact hws2
  · intro m hm
    rw [hb2]
...
private theorem inner_init_engine (src dst : Word) (inBytes : List (BitVec 8))
    ...
    (hwslen : wsp.length = 32) (hfr : frameOk src dst) :
...
theorem secfLeToBeFn_spec (src dst : Word) (inBytes orig : List (BitVec 8))
  ...
  case secfLeToBe.post =>
    rintro rf ws A ⟨⟨i, hile, hx5, hx6, -, -, hwslen, -, hlimbs⟩, hng⟩
    have hi3 : i = 3 := by
  ...
  case secfLeToBe.outer.inv_init =>
    ...
    obtain ⟨hx10, hx11, hwseq, holen, -, hnws, hnwd, hdisj⟩ := hpre
    ...
    exact outer_step_engine src dst inBytes rf₀ ws₀ A₀ rf2 ws' A' 0 (by omega)
      hs5 hs28 hws0len (fun m hm => by omega) hInv
  case secfLeToBe.outer.inv_step =>
    ...
    obtain ⟨ho5, -, ho10, ho11, howslen, hofr, holimbs⟩ := houter
    obtain ⟨he5, he28, -, -, -, -, he2⟩ :=
