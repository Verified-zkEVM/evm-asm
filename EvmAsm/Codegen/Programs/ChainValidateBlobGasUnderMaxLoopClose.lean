/-
  ...
  `cvbgumLoop` runs the guard/iteration from `D+68` for `N − i` remaining
  headers (induction on the fuel `N − i`, tying each iteration's K34 `Result`
  into the accumulating `hprefix`), and
  `chain_validate_blob_gas_used_under_max_spec_within` glues the prologue in
  front.
-/
...
/-- The guard/loop from `D+68` entering iteration `i` (`i ≤ N`), with all
    ...
    all-valid exit; otherwise one `cvbgumIter` runs and the induction hypothesis
    handles the rest (with `hprefix` extended by the freshly-decoded header). -/
theorem cvbgumLoop (sp0 spC hdrBase lenBase validPtr firstBadPtr raIn : Word)
  ...
  have base : (∀ j, j < lengths.length → hdrUnderMax hdrBase bigBytes lengths j) →
    ...
    have hbeq := beq_spec_gen_within .x21 .x8 (156 : BitVec 13)
    ...
    rw [show (D + 68) + signExtend13 (156 : BitVec 13) = D + 224 from by
    ...
    have hallv := cpsTripleWithin_extend_code cvbgum_mono
...
/-- **`chain_validate_blob_gas_used_under_max` caller contract.**  The
    66-instruction accessor iterates over `N = lengths.length` block headers,
    validating that every header's RLP field 17 (`blob_gas_used`) decodes to a
    `u64` at most `MAX_BLOB_GAS_PER_BLOCK = 2752512`.  Its three-way post pins
    the result: all-valid (`a0 = 0`, `*validPtr = 1`, every header under max),
    first-violation (`a0 = 0`, `*validPtr = 0`, `*firstBad = k`, header `k` over
    max and all earlier under max), or first parse-failure (`a0 = status`,
    `*firstBad = k`, header `k` fails the strict field-17 decode and all earlier
    under max) — each genuinely tied to K34's per-header `Result`. -/
theorem chain_validate_blob_gas_used_under_max_spec_within
  ...
  have hpro := cpsTripleWithin_frameR
    ...
    (cpsTripleWithin_extend_code cvbgum_mono
      (cvbgumPrologue sp0 spC nWord lenBase hdrBase validPtr firstBadPtr raIn
