...
/-! ## One full iteration: guard → call → dispatch (`C+68 → raIn`, `i < N`)
    ...
    Shapes `LoopInv i` into `cvedlIterEntry`'s split precondition (peeling the
    two output cells to arbitrary incumbents and splitting the arrays), runs the
    entry half to K20's `returnResult`, then the dispatch. -/

set_option maxRecDepth 8000 in
theorem cvedlIter (sp0 spC hdrBase lenBase validPtr firstBadPtr raIn : Word)
    ...
    cpsTripleWithin ((1 + (15 + 1 + nCall)) + (25 + nTail)) (C + 68) raIn fullCode
      (LoopInv sp0 spC (spC + signExtend12 (-64 : BitVec 12)) hdrBase lenBase validPtr
  ...
  have hLi : lengths[i]! = lengths[i] := getElem!_pos lengths i hi
  have hHB : hdrBaseAt hdrBase lengths i = hdrBase + BitVec.ofNat 64 (hdrOff lengths i) := rfl
  -- The entry-half footprint minus the two output cells.
  set EBody : Assertion :=
    ((.x2 ↦ᵣ spC) ** (.x8 ↦ᵣ BitVec.ofNat 64 lengths.length) ** (.x9 ↦ᵣ lenBase) **
      ...
      regOwn .x29 ** regOwn .x30 ** regOwn .x31 ** (.x0 ↦ᵣ (0 : Word)) **
      frameSlotsOwn listNthFrame (spC + signExtend12 (-64 : BitVec 12))) with hEBody
  refine cpsTripleWithin_weaken (fun h hp => by
    ...
    (show cpsTripleWithin ((1 + (15 + 1 + nCall)) + (25 + nTail)) (C + 68) raIn fullCode
      ((EBody ** memOwn COff) ** memOwn CLen)
      (cvedlPost sp0 spC (spC + signExtend12 (-64 : BitVec 12)) hdrBase lenBase validPtr
        firstBadPtr csaved bigBytes lengths) from ?_)
  refine cpsTripleWithin_of_forall_memIs_to_memOwn (fun oldLen => ?_)
  refine cpsTripleWithin_weaken (fun h hp => by xperm_hyp hp) (fun _ hq => hq)
    (show cpsTripleWithin ((1 + (15 + 1 + nCall)) + (25 + nTail)) (C + 68) raIn fullCode
      ((EBody ** (CLen ↦ₘ oldLen)) ** memOwn COff)
      (cvedlPost sp0 spC (spC + signExtend12 (-64 : BitVec 12)) hdrBase lenBase validPtr
        firstBadPtr csaved bigBytes lengths) from ?_)
