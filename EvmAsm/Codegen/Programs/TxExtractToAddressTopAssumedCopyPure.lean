/-
  Wire pure honesty into short concrete Assumed**content copy path.
-/
...
/-- Short concrete Assumed**content copy with pure-discharged walk hyps.
    Residual: hvalid*/hvalid1_*/hdec* + content statics. -/
theorem extractAssumed_copy_shortConcrete_pure
  ...
  have hnext_content : ∀ (next5 len5 : Word),
      rlpItemDecode txBytes srcOff5 (txBase + BitVec.ofNat 64 srcOff5) endW next5 len5 →
      next5 = contentPtr + (20 : Word) := by
    intro n l hd
    have hc : contentPtr = txBase + BitVec.ofNat 64 srcOff5 + (1 : Word) := by
      simpa [srcOff5, listOff] using hcontent
    exact extractSuccess_copy_type234_hnext_content txBytes txBase contentPtr srcOff5
      hsuccess hcopyFlag hge items hdecL hshort rfl hc endW n l hd
  have hinb0 :=
  ...
  have hoff := hoffInner
  exact extractAssumed_copy_of_front_short_concrete
    sp0 spC s txBase lenW toBuf isCreationPtr contentPtr w0 w1 w2 txBytes
    ...
    hcalign hcover hcvalid htalign htover htvalid hlen hty0 hover
    hvalidTx0 hoff hinover hinvalid hlistLen_ne h_ge h_hi h_exact

set_option maxRecDepth 8000 in
theorem extractAssumed_copy_shortConcrete_pure_fullCode
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr contentPtr w0 w1 w2 : Word)
    (txBytes : List (BitVec 8))
    (items : List EL.RLP.RLPItem)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_copy_shortConcrete_pure sp0 spC s
      txBase lenW toBuf isCreationPtr contentPtr w0 w1 w2 txBytes items
...
#print axioms extractAssumed_copy_shortConcrete_pure
#print axioms extractAssumed_copy_shortConcrete_pure_fullCode
