/-
  Ambient dual: E → AfterSave long → legacy 20B copy of_decode (typeW=0).
-/
...
theorem extractFrontCopy_then_epi_of_decode_long_concrete_legacy_region_ambient
    ...
    (hlen20 : ∀ (next3 len3 : Word),
      rlpItemDecode bs absOff3 (regionBase + BitVec.ofNat 64 absOff3)
        (longWalkEndAmbient regionBase (lenW - (teerTxTypeDispatch (txSlice bs off len)).2.2)
          (ambientAbsOff off (teerTxTypeDispatch (txSlice bs off len)).2.2.toNat)) next3 len3 → len3 = (20 : Word))
    ...
    (hcur : longWalkCursorAmbient regionBase bs (ambientAbsOff off (teerTxTypeDispatch (txSlice bs off len)).2.2.toNat) hoff =
    ...
    cpsTripleWithin
      (nFrontCopyStepsLongLegacyRegion
        ((bs[ambientAbsOff off (teerTxTypeDispatch (txSlice bs off len)).2.2.toNat]'hoff).zeroExtend 64 -
          (0xf7 : Word)).toNat)
  ...
  have hC := extractFrontAfterSaveCopy_then_epi_of_decode_long_legacy_region_ambient sp0 spC s
...
#print axioms extractFrontCopy_then_epi_of_decode_long_concrete_legacy_region_ambient
