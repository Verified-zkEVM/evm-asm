/-
  EvmAsm.Codegen.Programs.TxSigningHashLegacyEntry

  **K146 `tx_signing_hash_legacy_eip155` — ABI frame + entry leg (#12038).**

  First half of the K146 capstone (`TxSigningHashLegacyTop` is the second):

  * the `abiFrame_spec_own` scaffolding for `legacyFrame`
    (`legacyFrame_cons`/`_ne_zero`/`_restore`, `legacy_ofProg_sub_fullCode`,
    the flattened `regsAt`/`regsOwnAt` forms);
  * the whole-routine **empty-input-length reject slice**
    `tx_signing_hash_legacy_eip155_spec_within_empty_len`, with its gate's
    negative control.  Deliberately not a registry row of its own — same
    convention as K145's `_empty_len`;
  * the entry leg H+36 → H+124 (setup moves, header parse, the
    `rlp_list_nth_item` call), generalised over the four callee-saved
    registers the deployed segment lemma pinned at 0;
  * the K146 payload slice `legacyPayloadOf` and its
    `KssInputSourceSpec`, opened up from the existential
    `legacyNthSuccess_payloadSlice` so the two static-view layout bridges
    can be applied to it.

  Same namespace as `TxSigningHashLegacyTop`; split only for the file-size
  guardrail.
-/
...
theorem legacyPayloadSourceSpec_len
    (input : List (BitVec 8)) (hdrLen offset len : Word)
    (hfit : hdrLen.toNat + ((offset + len) - hdrLen).toNat ≤ input.length) :
