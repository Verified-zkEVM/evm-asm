/-
  ExtractAssumed bare packaging for type234 short 20B copy path.
...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_concrete_region
  ...
  have h1 := cpsTripleWithin_mono_nSteps nFrontCopyStepsShortRegion_le_nExtract hE2E
...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_temps_region
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr contentPtr w2 : Word)
    (txBytes : List (BitVec 8))
    (hspC : spC = sp0 + signExtend12 (-80 : BitVec 12))
  ...
  have htemps : cpsTripleWithin nExtractSteps E s.ra extractLinkedCode
    ...
    have hc := extractAssumed_copy_concrete_region sp0 spC s txBase lenW
...
set_option maxRecDepth 8000 in
/-- Wire Assumed bare under short concrete copy E2E of_decode region. -/
theorem extractAssumed_copy_of_front_short_concrete_region
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr : Word)
    (txBytes : List (BitVec 8))
  ...
  refine extractAssumed_copy_temps_region sp0 spC s txBase lenW toBuf
    isCreationPtr contentPtr w2 txBytes hspC (fun old5 old6 old7 old14 old15 old16 => ?_)
  have hE := extractFrontCopy_then_epi_of_decode_short_concrete_region
  ...
  -- hE post is defeq copyE2EPostRegion after unfolding contentPtr/w2 lets
  refine cpsTripleWithin_weaken (fun _ hp => by
...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_fullCode_of_decode_short_concrete_region
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr : Word)
    (txBytes : List (BitVec 8))
    (srcOff0 srcOff1 srcOff2 srcOff3 srcOff4 srcOff5 q : Nat)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_copy_of_front_short_concrete_region sp0 spC s
...
#print axioms extractAssumed_copy_of_front_short_concrete_region
#print axioms extractAssumed_copy_fullCode_of_decode_short_concrete_region
