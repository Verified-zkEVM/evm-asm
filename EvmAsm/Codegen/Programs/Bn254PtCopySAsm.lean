...
def copyInv (src dst : Word) (srcBytes orig : List (BitVec 8)) :
  ...
  fun i rf ws A =>
    ...
    ws = copyWin1152 srcBytes orig (i + 1) ∧ A = empAssertion
...
def bnqPtCopyFn (src dst : Word) (srcBytes orig : List (BitVec 8)) : Fn where
  ...
  pre := fun rf ws A =>
    ...
    srcBytes.length = 1152 ∧ orig.length = 1152 ∧ frameOk1152 src dst ∧ A = empAssertion
...
theorem bnqPtCopyFn_spec (src dst : Word) (srcBytes orig : List (BitVec 8))
  ...
  case bnqPtCopy.post =>
    ...
    exact ⟨by rw [hws, copyWin1152_144_eq srcBytes orig hs ho], hA⟩
...
def bnqPtCopyCr : CodeReq :=
  CodeReq.ofProg (GuestAddrs.bnq_pt_copy : Word) bnqPtCopy_prog

/-- The exposed registers other than `a0`/`a1`. -/
def bnqPtCopyScratch : List Reg :=
  [.x5, .x6, .x7, .x28, .x29, .x30, .x31,
   .x12, .x13, .x14, .x15, .x16, .x17]

private theorem exposedRegs_split_bnqPtCopy (vf : Reg → Word) :
    regAtomsOf vf exposedRegs
      = ((.x10 ↦ᵣ vf .x10) ** (.x11 ↦ᵣ vf .x11) ** regAtomsOf vf bnqPtCopyScratch) := by
  show regAtomsOf vf
      [.x5, .x6, .x7, .x28, .x29, .x30, .x31,
       .x10, .x11, .x12, .x13, .x14, .x15, .x16, .x17] = _
  simp only [bnqPtCopyScratch, regAtomsOf_cons, regAtomsOf_nil]
  xperm

private theorem x10_notin_bnqPtCopy_scratch : (.x10 : Reg) ∉ bnqPtCopyScratch := by decide
private theorem x11_notin_bnqPtCopy_scratch : (.x11 : Reg) ∉ bnqPtCopyScratch := by decide

/-- **`bnq_pt_copy`, whole-routine flat triple at the guest entry.**

    Copies the 1152 bytes at `a0` to `a1`. Anchored over
    `bnqPtCopyCr = CodeReq.ofProg (GuestAddrs.bnq_pt_copy) bnqPtCopy_prog`, the `GuestImageEntries`
    ...
    ⚠️ NOT total over its argument types: `frameOk1152 src dst` unfolds to both bases
    non-overflowing AND the two windows DISJOINT, so the overlapping case is outside
    the domain rather than handled. -/
theorem bnqPtCopyFlat_spec (ret src dst : Word) (srcBytes orig : List (BitVec 8))
    (hwfR : Region.wf ⟨src, srcBytes⟩) (hrww : RwRegion.wf ⟨dst, 1152⟩)
    (hs : srcBytes.length = 1152) (ho : orig.length = 1152)
    (hfr : frameOk1152 src dst)
    (halign : (ret &&& ~~~(1 : Word)) = ret) :
    cpsTripleWithin ((bnqPtCopyFn src dst srcBytes orig).body.steps + 1)
      (GuestAddrs.bnq_pt_copy : Word) ret bnqPtCopyCr
      (((.x1 : Reg) ↦ᵣ ret) ** (.x10 ↦ᵣ src) ** (.x11 ↦ᵣ dst) **
        regOwns bnqPtCopyScratch ** bytesRegion dst orig ** bytesRegion src srcBytes)
      (((.x1 : Reg) ↦ᵣ ret) ** regOwns exposedRegs **
        bytesRegion dst srcBytes ** bytesRegion src srcBytes) := by
  refine cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp) (fun _ hq => hq)
    (cpsTripleWithin_peel_regOwns bnqPtCopyScratch (by decide)
      (P := ((.x1 : Reg) ↦ᵣ ret) ** (.x10 ↦ᵣ src) ** (.x11 ↦ᵣ dst) **
        bytesRegion dst orig ** bytesRegion src srcBytes)
      (fun vf => ?_))
  have hpre : (bnqPtCopyFn src dst srcBytes orig).pre
    ...
    · show RegFile.get _ .x11 = dst
      ...
      exact if_pos rfl
  have had := Fn.retSpecFlat (bnqPtCopyFn src dst srcBytes orig)
    (GuestAddrs.bnq_pt_copy : Word)
    (bnqPtCopyFn_spec src dst srcBytes orig hwfR hrww (GuestAddrs.bnq_pt_copy : Word))
  ...
  rw [show (bnqPtCopyFn src dst srcBytes orig).programRet (GuestAddrs.bnq_pt_copy : Word)
      = bnqPtCopy_prog from rfl] at had
  rw [show (bnqPtCopyFn src dst srcBytes orig).region = (⟨src, srcBytes⟩ : Region) from rfl,
      show (bnqPtCopyFn src dst srcBytes orig).rw.base = dst from rfl] at had
  rw [regFileIs_eq_regAtoms, regAtoms_eq_regAtomsOf _ _ (by decide),
    exposedRegs_split_bnqPtCopy,
    ...
    regAtomsOf_congr
      ...
      (fun r hr => by
        ...
        rw [if_neg (fun (hc : r = .x10) => x10_notin_bnqPtCopy_scratch (hc ▸ hr)),
            if_neg (fun (hc : r = .x11) => x11_notin_bnqPtCopy_scratch (hc ▸ hr))])] at had
