/-
  Ambient long t1 creation: discharge hdec*/hvalid*/hlover/hlvalid via slice pure +
  bridges + validByteRange. Path Prop + flat success (hitem0..4, hge6).
-/
...
/-- Ambient long concrete Assumed creation; hdec+hvalid+hlover/hlvalid from pure+validByteRange.
    Residual path: creation/t1/long/hge6 + hitem0..4. -/
theorem extractAssumed_creation_longConcrete_pureHvalid_t1_ambient
  ...
  simpa [absOff0, absOff1, absOff2, absOff3, absOff4, absListOff,
    ...
    extractAssumed_creation_longConcrete_pure_t1_ambient
...
set_option maxRecDepth 8000 in
theorem extractAssumed_creation_longConcrete_pureHvalid_t1_ambient_fullCode
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (items : List EL.RLP.RLPItem)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_creation_longConcrete_pureHvalid_t1_ambient
      sp0 spC s regionBase loadPtr lenW toBuf isCreationPtr bs off len items
      hspC hret htalign htover htvalid hlen hptr hbound hsuccess hcreFlag htype1 hdecL hlong
      hsalign hover hvalidBuf
      hitem0 hitem1 hitem2 hitem3 hitem4 hge6)

/-- Path refinements for ambient long t1 creation arm. -/
def extractCreationT1LongPathAmbient
...
set_option maxRecDepth 8000 in
/-- Ambient Assumed footprint under long t1 creation path (statics + path + hitem).
    classical-3. Residual: long copy arms. -/
theorem extractAssumed_success_flat_creation_t1_long_ambient
  ...
  simpa only [hsra, hs0, hs1, hs2, hs3, hs4, hs5, hs6, hs7] using
    extractAssumed_creation_longConcrete_pureHvalid_t1_ambient_fullCode
      spVal spC s regionBase loadPtr lenW toBuf isCreationPtr bs off len items
      hspC hret htalign htover htvalid hlen hptr hbound hsuccess hcreFlag htype1 hdecL hlong
      hsalign hover hvalidBuf
      hitem0 hitem1 hitem2 hitem3 hitem4 hge6

#print axioms extractAssumed_creation_longConcrete_pureHvalid_t1_ambient
#print axioms extractAssumed_creation_longConcrete_pureHvalid_t1_ambient_fullCode
#print axioms extractAssumed_success_flat_creation_t1_long_ambient
