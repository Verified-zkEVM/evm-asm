...
-- non-membership facts used to live here, needed by the conv adapter proofs
-- alone. Those proofs are now ⊆-monotonicity corollaries (#12244), so the
-- split moved to `Bn254FieldConvFlatEntry` with the theorem that needs it.
...
theorem bnfBeToLeFlat_spec (ret srci dsti : Word) (inb ob : List (BitVec 8))
  ...
  -- `liftCode (cr' := mulCr)` on it, so this loses nothing but the duplicate.
  liftCode (Bn254FieldConvSAsm.bnfBeToLeFlatEntry_spec ret srci dsti inb ob
...
theorem bnfLeToBeFlat_spec (ret srci dsti : Word) (inb ob : List (BitVec 8))
  ...
  liftCode (Bn254FieldConvSAsm.bnfLeToBeFlatEntry_spec ret srci dsti inb ob
    hilen holen hwfR hrww hso hdo hdisj halign) (by code_mem)
...
theorem stageC_spec (aPtr bPtr outPtr : Word)
    ...
    (hdO : (0xa0b00ee0 : Nat) ≤ outPtr.toNat ∨ outPtr.toNat + 32 ≤ (0xa0b00ec0 : Nat))
    (hpa₂ : wsDword img₂ 0xC0 = arenaB + BitVec.ofNat 64 0)
    (hpb₂ : wsDword img₂ 0xC8 = arenaB + BitVec.ofNat 64 0x20)
    (hpc₂ : wsDword img₂ 0xD0 = arenaB + BitVec.ofNat 64 0x60)
    (hpm₂ : wsDword img₂ 0xD8 = arenaB + BitVec.ofNat 64 0xA0)
        ...
        · intro k hk
          ...
          rw [show (GuestAddrs.bnf_le_d : Word) + BitVec.ofNat 64 k
            ...
            rw [BitVec.toNat_add, BitVec.toNat_add, BitVec.toNat_ofNat,
              BitVec.toNat_ofNat,
              show ((GuestAddrs.bnf_le_d : Word)).toNat = 0xa0b00ec0 from by decide,
              show (arenaB).toNat = 0xa0b00e80 from by decide]
            omega]
          exact harval (0x40 + k) (by omega))
      ⟨hoal, by omega, hovalid⟩
      (by decide) (by omega)
      (by
        have hsrc : ((GuestAddrs.bnf_le_d : Word)).toNat = 0xa0b00ec0 := by decide
        rcases hdO with h | h
        · left
          rw [hsrc]
          omega
