/-
  Wire pure honesty into long concrete Assumed creation for legacy (type=0):
...
/-- Long concrete Assumed creation legacy; pure-discharged honesty.
    Residual: hvalid*/hvalid1_*/hdec*/hlover/hlvalid RAM. -/
theorem extractAssumed_creation_longConcrete_pure_t1
    ...
    (hitem0 : (encode (items[0]'(by
        have := extractSuccess_creation_t1_items_length_long txBytes hsuccess
  ...
  have hhoff :=
    extractSuccess_creation_t1_hoff_srcOff_long txBytes hsuccess hcreFlag htype1
      items hdecL hlong
  have hhover :=
    extractSuccess_creation_t1_hover_srcOff_long txBytes txBase hsuccess hcreFlag htype1
      items hdecL hlong hover
  have hlenItems :=
    extractSuccess_creation_t1_items_length_long txBytes hsuccess hcreFlag htype1
      items hdecL hlong
  ...
  have hfields03 :=
    extractSuccess_creation_t1_hnext_fields03_long txBytes hsuccess hcreFlag htype1
      items hdecL hlong
  have hendEq :
  ...
  have hcur :
    ...
    simpa [longWalkCursor, srcOff0, listOff] using
      longWalkCursor_eq_srcOff0 txBytes txBase listOff items hencInner hlong hbound
        hoff hoverC
  have hhnext :=
    extractSuccess_creation_t1_hnext_hcre_srcOff_long txBytes txBase hsuccess
  ...
  exact extractAssumed_creation_under_honesty_of_decode_long_concrete_t1
    ...
    (hls_vacuous_of_long_list_item txBytes listOff items 4 hencInner hlong hn4 hitem4 hhoff.2.2.2.2)
    (hll_vacuous_of_long_list_item txBytes listOff items 4 hencInner hlong hn4 hitem4 hhoff.2.2.2.2)
    hdec4 hinb4
...
set_option maxRecDepth 8000 in
theorem extractAssumed_creation_longConcrete_pure_t1_fullCode
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr : Word)
    (txBytes : List (BitVec 8))
    (items : List EL.RLP.RLPItem)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_creation_longConcrete_pure_t1 sp0 spC s
      txBase lenW toBuf isCreationPtr txBytes items
      hspC hret htalign htover htvalid hlen hsuccess hcreFlag htype1 hdecL hlong
...
#print axioms extractAssumed_creation_longConcrete_pure_t1
#print axioms extractAssumed_creation_longConcrete_pure_t1_fullCode
