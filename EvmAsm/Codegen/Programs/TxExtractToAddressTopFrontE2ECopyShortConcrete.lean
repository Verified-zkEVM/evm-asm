/-
  Front E → AfterSave short concrete → 20B copy → ret of_decode (no ∀endPtr).
-/
...
/-- E → ret type234 20B copy short+of_decode with concrete endPtr (no ∀endPtr). -/
theorem extractFrontCopy_then_epi_of_decode_short_concrete
  ...
  have hF0 := extractFrontToAfterSave_short_concrete sp0 spC s txBase lenW toBuf isCreationPtr
  ...
  have hF := cpsTripleWithin_frameR
    (contentDwords contentPtr w0 w1 w2)
    (contentDwords_pcFree contentPtr w0 w1 w2) hF0
  have hC := extractFrontAfterSaveCopy_then_epi_of_decode_short sp0 spC s
    ...
    hcalign hcover hcvalid htalign htover htvalid
  exact cpsTripleWithin_seq_same_cr hF hC

#print axioms extractFrontCopy_then_epi_of_decode_short_concrete
