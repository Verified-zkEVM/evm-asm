/-
  EvmAsm.Codegen.Programs.Bls12G1LeToBeSAsm

  Verified SAsm port of the secp256k1 LE→BE field-element converter
  (bead evm-asm-4ch8f.38.2 wave-2, the inverse of `blsgBeToLe`):

  - `blsgLeToBe` (`blsgLeToBe_prog`, Bls12G1.lean:205): read six
    LITTLE-ENDIAN u64 limbs (LSB-first) at `a0` and write the 48-byte
    BIG-ENDIAN buffer at `a1`.

  Same nested bottom-test shape as `blsgBeToLe` (outer `doWhile` over the 6
...
def innerInv (src dst : Word) :
  ...
  fun rf₀ ws₀ _ j rf ws A =>
    ...
    ∧ (∀ t, t ≤ j → getByteAt ws ((47 - 8 * k) - t) = leByte limb t)
    ∧ (∀ idx, 47 - 8 * k < idx → getByteAt ws idx = getByteAt ws₀ idx)
    ∧ A = empAssertion
...
def outerInv (src dst : Word) (inBytes : List (BitVec 8)) :
  ...
  fun i rf ws A =>
    ...
    ∧ ws.length = 48 ∧ frameOk src dst
    ∧ (∀ m, m ≤ i →
        (ws.drop (40 - 8 * m)).take 8 = beBytesOfLimb (wsDword inBytes (8 * m)))
    ∧ A = empAssertion

-- ============================================================================
-- The routine body (byte-identical to  blsgLeToBe_prog / blsgLeToBe_prog)
-- ============================================================================

def blsgLeToBeBody (src dst : Word) (inBytes : List (BitVec 8)) : Stmt :=
...
def blsgLeToBe_verified : Program := (blsgLeToBeBody 0 0 []).flatten 0

#guard (blsgLeToBe_verified : List Instr).length = 18
#guard (blsgLeToBeBody 0 0 []).flatten 0 = (blsgLeToBeBody 0 0 []).flatten 0x80000000
-- Byte-identity to the emitted routine: guest bytes do not move.
#guard (blsgLeToBeBody 0 0 []).flatten 0 ++ [Instr.JALR .x0 .x1 (0 : BitVec 12)]
  = blsgLeToBe_prog
...
def blsgLeToBeFn (src dst : Word) (inBytes orig : List (BitVec 8)) : Fn where
  name := "blsgLeToBe"
  region := ⟨src, inBytes⟩
  rw := ⟨dst, 48⟩
  pre := fun rf ws A =>
  ...
  post := fun _ ws A =>
    ...
    ∧ ws.length = 48 ∧ A = empAssertion
  body := blsgLeToBeBody src dst inBytes
...
/-- Byte-list bridge (inverse of `leLimbs_chunks_eq_beBytesToNat`): a 48-byte
    big-endian buffer whose six 8-byte big-endian blocks encode the limbs
    `[v5, v4, v3, v2, v1, v0]` decodes to `leLimbsToNat [v0, v1, v2, v3, v4, v5]`. -/
theorem beBytesToNat_beBlocks (v0 v1 v2 v3 v4 v5 : Word) :
    beBytesToNat (beBytesOfLimb v5 ++ beBytesOfLimb v4 ++ beBytesOfLimb v3
        ++ beBytesOfLimb v2 ++ beBytesOfLimb v1 ++ beBytesOfLimb v0)
      = Accel.leLimbsToNat [v0, v1, v2, v3, v4, v5] := by
  rw [beBytesToNat_append, beBytesToNat_append, beBytesToNat_append,
    beBytesToNat_append, beBytesToNat_append,
...
private def setupInstrs : List Instr :=
  [.SLLI .x6 .x5 (3 : BitVec 6), .ADD .x7 .x10 .x6, .LD .x28 .x7 (0 : BitVec 12),
   .LI .x6 (47 : Word), .SLLI .x7 .x5 (3 : BitVec 6), .SUB .x6 .x6 .x7,
...
/-- A read-only dword at `src + 8k` misses the disjoint 48-byte writable
    window. -/
private theorem src_dword_miss (src dst : Word) (ws : List (BitVec 8)) (k : Nat)
    (hk : k < 6) (hws : ws.length = 48) (hfr : frameOk src dst) :
...
private theorem setup_exec (src dst : Word) (inBytes wsp : List (BitVec 8))
    (rfp : RegFile) (k : Nat) (hk : k < 6)
    (hx5 : rfp.get .x5 = BitVec.ofNat 64 k) (hx10 : rfp.get .x10 = src)
    (hx11 : rfp.get .x11 = dst) (hws : wsp.length = 48) (hfr : frameOk src dst) :
  ...
  rw [show setupInstrs = [.SLLI .x6 .x5 (3 : BitVec 6), .ADD .x7 .x10 .x6,
      .LD .x28 .x7 (0 : BitVec 12), .LI .x6 (47 : Word), .SLLI .x7 .x5 (3 : BitVec 6),
...
private theorem inner_body_exec (src dst : Word) (inBytes ws : List (BitVec 8))
    (rf : RegFile) (off : Nat) (hoff : off < 48)
...
/-- Address side conditions of the inner body: its single `SB` writes byte
    `off < 48` of the writable window (in range; byte stores are 1-aligned). -/
private theorem inner_blockVCs (src dst : Word) (inBytes ws : List (BitVec 8))
    (rf : RegFile) (off : Nat) (hoff : off < 48)
    (hx6 : rf.get .x6 = dst + BitVec.ofNat 64 off) (hws : ws.length = 48) :
...
/-- The inner-loop snapshot after `setup`: `x5 = k < 6`, `x28` holds limb `k`,
...
private theorem snap_facts (src dst : Word) (inBytes orig : List (BitVec 8))
    ...
    (hsp : Stmt.sp ⟨src, inBytes⟩ ⟨dst, 48⟩ (Stmt.block "setup" setupInstrs)
      (fun rf ws A =>
        Stmt.sp ⟨src, inBytes⟩ ⟨dst, 48⟩ (Stmt.block "init" [.LI .x5 (0 : Word)])
            (blsgLeToBeFn src dst inBytes orig).pre rf ws A
    ...
    ∃ k, k < 6 ∧ rf₀.get .x5 = BitVec.ofNat 64 k
      ...
      ∧ rf₀.get .x6 = dst + BitVec.ofNat 64 (47 - 8 * k)
      ∧ rf₀.get .x29 = (8 : Word)
      ∧ rf₀.get .x10 = src ∧ rf₀.get .x11 = dst
      ∧ ws₀.length = 48 ∧ frameOk src dst
      ∧ (∀ m, m < k →
  ...
  obtain ⟨k, hk, hpx5, hpx10, hpx11, hpwslen, hpfr, hplimbs, hA⟩ :
    ...
    · obtain ⟨rfi, wsi, hwsi, hpre, rfl, rfl⟩ := hinit
      ...
      refine ⟨0, by omega, ?_, ?_, ?_, ?_, ⟨hnws, hnwd, hdisj⟩,
        by intro m hm; omega, hA⟩
    ...
    · obtain ⟨hx5, hx6, hx10, hx11, hwslen, hfr, hlimbs, hA⟩ := houter
      exact ⟨i + 1, by omega, hx5, hx10, hx11, hwslen, hfr,
        fun m hm => hlimbs m (by omega), hA⟩
...
private theorem inner_step_engine (src dst : Word) (inBytes : List (BitVec 8))
    ...
    (hk : k < 6) (hi : i < 7)
    (hs6 : rf₀.get .x6 = dst + BitVec.ofNat 64 (47 - 8 * k))
  ...
  have hle : i + 1 ≤ 47 - 8 * k := by omega
  have hoff : (47 - 8 * k) - (i + 1) < 48 := by omega
  have hse1 : signExtend12 (-1 : BitVec 12) = (-1 : Word) := by decide
  -- current pointer = dst + off, off = (31-8k)-(i+1)
  have hx6 : rf.get .x6 = dst + BitVec.ofNat 64 ((47 - 8 * k) - (i + 1)) := by
    rw [hp6, hs6]
    apply BitVec.eq_of_toNat_eq
    have hd := dst.isLt
  ...
  obtain ⟨e2, e28, e6, e29, e5, e10, e11⟩ :=
    inner_body_exec src dst inBytes ws rf ((47 - 8 * k) - (i + 1)) hoff hx6
  ...
  · -- byte fact: t ≤ i+1
    ...
    · subst hte
      rw [if_pos (by simp only [List.length_singleton]; omega),
        show (47 - 8 * k) - (i + 1) - ((47 - 8 * k) - (i + 1)) = 0 from by omega]
...
private theorem bump_exec (reg : Region) (rwb : Word) (rf : RegFile)
    ...
    ∧ (execBlock reg rwb rf ws bumpInstrs).1.get .x6 = (6 : Word)
...
private theorem slice_eq_beBytesOfLimb (ws : List (BitVec 8)) (limb : Word) (k : Nat)
    (hk : k < 6) (hws : ws.length = 48)
    (hb : ∀ t, t ≤ 7 → getByteAt ws ((47 - 8 * k) - t) = leByte limb t) :
    (ws.drop (40 - 8 * k)).take 8 = beBytesOfLimb limb := by
  ...
  · intro s h1 _
    ...
    have hidx : (40 - 8 * k) + s = (47 - 8 * k) - (7 - s) := by omega
    have hlt : (40 - 8 * k) + s < ws.length := by rw [hws]; omega
    rw [List.getElem_take, List.getElem_drop]
    rw [show (ws[(40 - 8 * k) + s]) = getByteAt ws ((40 - 8 * k) + s) from by
...
private theorem setup_blockVCs (src dst : Word) (inBytes ws : List (BitVec 8))
    (rf : RegFile) (k : Nat) (hk : k < 6) (hilen : inBytes.length = 48)
    (hx5 : rf.get .x5 = BitVec.ofNat 64 k) (hx10 : rf.get .x10 = src)
    (hws : ws.length = 48) (hfr : frameOk src dst) :
...
private theorem outer_step_engine (src dst : Word) (inBytes : List (BitVec 8))
    ...
    (hk : k < 6) (hs5 : rf₀.get .x5 = BitVec.ofNat 64 k)
    (hs28 : rf₀.get .x28 = wsDword inBytes (8 * k)) (hws0len : ws₀.length = 48)
    (hlimbs : ∀ m, m < k →
      (ws₀.drop (40 - 8 * m)).take 8 = beBytesOfLimb (wsDword inBytes (8 * m)))
  ...
  have hws2 : ws2.length = 48 := hpws
  obtain ⟨hb5, hb6, hb10, hb11, hb2⟩ := bump_exec ⟨src, inBytes⟩ dst rf2 ws2 k hx5
  dsimp only [outerInv]
  refine ⟨hb5, hb6, hb10.trans hp10, hb11.trans hp11, ?_, hpfr, ?_, hpA⟩
  · rw [hb2]; exact hws2
  · intro m hm
    rw [hb2]
    ...
    · -- earlier limb m < k: its slice (offsets 24-8m..31-8m, all > 31-8k) is
      ...
      have hslice : (ws2.drop (40 - 8 * m)).take 8 = (ws₀.drop (40 - 8 * m)).take 8 := by
        apply List.ext_getElem
        ...
        · intro s h1 _
          ...
          have hlt2 : (40 - 8 * m) + s < ws2.length := by rw [hws2]; omega
          have hlt0 : (40 - 8 * m) + s < ws₀.length := by rw [hws0len]; omega
          have hfr := hpframe ((40 - 8 * m) + s) (by omega)
...
private theorem inner_init_engine (src dst : Word) (inBytes : List (BitVec 8))
    (rfp : RegFile) (wsp : List (BitVec 8)) (A₀ A' : Assertion) (k : Nat) (hk : k < 6)
    (hs5 : rfp.get .x5 = BitVec.ofNat 64 k)
    (hs6 : rfp.get .x6 = dst + BitVec.ofNat 64 (47 - 8 * k))
    (hs29 : rfp.get .x29 = (8 : Word))
    (hs10 : rfp.get .x10 = src) (hs11 : rfp.get .x11 = dst)
    (hwslen : wsp.length = 48) (hfr : frameOk src dst)
    (hA' : A' = empAssertion) :
  ...
  obtain ⟨e2, e28, e6, e29, e5, e10, e11⟩ :=
    inner_body_exec src dst inBytes wsp rfp (47 - 8 * k) (by omega) hs6
  dsimp only [innerInv]
  rw [hkeq]
  refine ⟨?_, ?_, ?_, ?_, ?_, ?_, ?_, hfr, ?_, ?_, hA'⟩
  ...
  · intro t ht
    ...
    rw [e2, getByteAt_setBytes _ _ _ _ (by rw [hwslen]; simp only [List.length_singleton]; omega),
      show (47 - 8 * k) - 0 = 47 - 8 * k from by omega,
      if_pos (by simp only [List.length_singleton]; omega),
      show (47 - 8 * k) - (47 - 8 * k) = 0 from by omega]
...
theorem blsgLeToBeFn_spec (src dst : Word) (inBytes orig : List (BitVec 8))
  ...
  case blsgLeToBe.outer.body.inner.body.body.mem =>
    rintro rf ws A hlen hreach
    rcases hreach with hsetup | ⟨rf₀, ws₀, A₀, hsnap, i, hi, hInv, hg⟩
    · obtain ⟨k, hk, -, -, hs6, -, -, -, -, -, -, -⟩ :=
        snap_facts src dst inBytes orig rf ws A hsetup
      exact inner_blockVCs src dst inBytes ws rf (47 - 8 * k) (by omega) hs6 hlen
    · obtain ⟨k, hk, -, -, hs6, -, -, -, -, -, -, -⟩ :=
      ...
      have hx6 : rf.get .x6 = dst + BitVec.ofNat 64 ((47 - 8 * k) - (i + 1)) := by
        ...
        simp only [BitVec.toNat_add, BitVec.toNat_sub, BitVec.toNat_ofNat]
        omega
      exact inner_blockVCs src dst inBytes ws rf ((47 - 8 * k) - (i + 1)) (by omega) hx6 hlen
  case blsgLeToBe.post =>
    ...
    simp only [show (40 - 8 * 0 : Nat) = 40 from rfl, show (8 * 0 : Nat) = 0 from rfl,
      show (40 - 8 * 1 : Nat) = 32 from rfl, show (8 * 1 : Nat) = 8 from rfl,
      show (40 - 8 * 2 : Nat) = 24 from rfl, show (8 * 2 : Nat) = 16 from rfl,
      show (40 - 8 * 3 : Nat) = 16 from rfl, show (8 * 3 : Nat) = 24 from rfl,
      show (40 - 8 * 4 : Nat) = 8 from rfl, show (8 * 4 : Nat) = 32 from rfl,
      show (40 - 8 * 5 : Nat) = 0 from rfl, show (8 * 5 : Nat) = 40 from rfl,
      List.drop_zero] at l0 l1 l2 l3 l4 l5
    have hd40 : (ws.drop 40).take 8 = ws.drop 40 :=
      List.take_of_length_le (by rw [List.length_drop, hwslen])
    have l0' : ws.drop 40 = beBytesOfLimb (wsDword inBytes 0) := by rw [← hd40]; exact l0
    have hsplit : ws = ws.take 8 ++ ((ws.drop 8).take 8 ++ ((ws.drop 16).take 8
        ++ ((ws.drop 24).take 8 ++ ((ws.drop 32).take 8 ++ ws.drop 40)))) := by
      ...
      rw [List.drop_drop]
      congr 1
      conv_lhs => rw [← List.take_append_drop 8 (ws.drop 24)]
      rw [List.drop_drop]
      congr 1
  ...
  case blsgLeToBe.outer.body.inner.inv_init =>
    ...
    subst hrp hwp
    obtain ⟨k, hk, hs5, -, hs6, hs29, hs10, hs11, hws0len, hfr, -, hA0⟩ :=
    ...
    exact inner_init_engine src dst inBytes rfp wsp A₀ A' k hk hs5 hs6 hs29 hs10 hs11
      hws0len hfr (hAeq ▸ hA0)
  case blsgLeToBe.outer.body.inner.inv_step =>
  ...
  case blsgLeToBe.outer.inv_init =>
    rintro rf' ws' A'
    ...
    rw [show (blsgLeToBeFn src dst inBytes orig).region = (⟨src, inBytes⟩ : Region) from rfl,
      show (blsgLeToBeFn src dst inBytes orig).rw.base = dst from rfl] at hrf0 hws0 ⊢
    have hs5 : rf₀.get .x5 = BitVec.ofNat 64 0 := by rw [hrf0]; exact he5
    have hs28 : rf₀.get .x28 = wsDword inBytes (8 * 0) := by rw [hrf0]; exact he28
    have hws0len : ws₀.length = 48 := by rw [hws0]; exact he2 ▸ hprelen
    exact outer_step_engine src dst inBytes rf₀ ws₀ A₀ rf2 ws' A' 0 (by omega)
      hs5 hs28 hws0len (fun m hm => by omega) hInv
  case blsgLeToBe.outer.inv_step =>
    rintro i hi rf' ws' A'
      ⟨rf2, ws2, hws2len, ⟨rf₀, ws₀, A₀, hsetup, ⟨j, hj, hInv⟩, hng⟩, rfl, rfl⟩
    have hj7 : j = 7 := by
      have hp29 : rf2.get .x29 = BitVec.ofNat 64 (7 - j) := hInv.1
    ...
    have hws0len : ws₀.length = 48 := by rw [hws0]; exact he2 ▸ howslen
    have hlimbs : ∀ m, m < i + 1 →
        (ws₀.drop (40 - 8 * m)).take 8 = beBytesOfLimb (wsDword inBytes (8 * m)) := by
