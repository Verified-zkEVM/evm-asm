...
def teerEmptyAuthResidualLedger : Prop :=
  ...
  --    list_count asm FREE via teerLinked; walk hover/hvalid FREE from dom+hoff on abi_dom path
  --    (teer_hover_of_dom / teer_hvalid_of_dom); residual: domain fixture inhabit + walk
  --    hoff/hdec/hss/… guards + hvalid0(tx off) + hret
  True ∧
