/-
  ...
  SSZ `ExecutionRequests` section from the three EXECUTION-DERIVED request bodies
  ...
  SSZ `ExecutionRequests` is an SSZ container of three variable-length byte fields,
  so its serialization is `[u32 off0][u32 off1][u32 off2][deposits][withdrawals]
  [consolidations]` with `off0 = 12`, `off1 = 12 + len(deposits)`, `off2 = off1 +
  len(withdrawals)` (little-endian offsets, relative to the section start). The bodies
  are the RAW request payloads with NO type prefix — `execution_requests_hash` adds the
  per-position type byte (0x00/0x01/0x02) when it hashes (see RequestsHash.lean).

  Standalone + probe-verifiable: the execution-gated piece is GETTING the three derived
...
/-! ## assemble_execution_requests
    ...
    a6 = output SSZ section ptr (>= 12 + a1 + a3 + a5 bytes, 8-aligned)
    a0 (output) = total SSZ section length (12 + a1 + a3 + a5). -/
def assembleExecutionRequestsFunction : String :=
  "assemble_execution_requests:\n" ++
  -- 3 u32 little-endian offsets: off0=12, off1=12+dlen, off2=12+dlen+wlen.
  "  li t0, 12; sw t0, 0(a6)\n" ++
  "  add t0, t0, a1; sw t0, 4(a6)\n" ++
  "  add t0, t0, a3; sw t0, 8(a6)\n" ++
  "  addi t1, a6, 12              # dst = section + 12 (after the offsets)\n" ++
  -- copy deposit body
  "  mv t2, a0; mv t3, a1\n" ++
  ".Laer_dcopy:\n" ++
  "  beqz t3, .Laer_dd\n" ++
...
/-- `zisk_assemble_execution_requests`: focused probe.
    Input (after the ziskemu length wrapper at 0x40000000):
      ...
      bytes 32..    : deposit body, withdrawal body, consolidation body (concatenated)
    Output:
...
def ziskAssembleExecutionRequestsPrologue : String :=
  ...
  "  addi a0, a7, 32             # deposit body ptr\n" ++
