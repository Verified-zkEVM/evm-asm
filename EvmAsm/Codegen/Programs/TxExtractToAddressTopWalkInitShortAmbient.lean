/-
  Ambient dual of short walk_init fromTypeLoad packaging.
-/
...
theorem extractWalkInitCall_short_fromTypeLoad_ambient
    ...
    cpsTripleWithin (1 + 15) WalkInitJalPc LinkWalkInit extractLinkedCode
      ...
      (walkInitAmbient loadPtr lenW
          (teerTxTypeDispatch (txSlice bs off len)).2.1 (teerTxTypeDispatch (txSlice bs off len)).2.2 **
        extractWalkInitCommon regionBase bs **
        extractWalkInitShortOkRegs regionBase (lenW - (teerTxTypeDispatch (txSlice bs off len)).2.2)
          (ambientAbsOff off (teerTxTypeDispatch (txSlice bs off len)).2.2.toNat)) := by
  ...
  have h_hi' : BitVec.ult ((bs[listOff]'hoff).zeroExtend 64) (0xf8 : Word) = true := by
    simpa only [listOff, h_listOff, h_inner] using h_hi
  have h_exact' : (regionBase + BitVec.ofNat 64 listOff) +
      (((bs[listOff]'hoff).zeroExtend 64 - (0xc0 : Word)) +
        signExtend12 (1 : BitVec 12)) =
    (regionBase + BitVec.ofNat 64 listOff) + listLen := by
    simpa only [listOff, listLen, h_listOff, h_listLen, h_inner] using h_exact
  have hcall := extractWalkInitCall_short_ambient regionBase listLen a2Old TeaInnerAddr t1Old t2Old
  ...
  have hcallW : cpsTripleWithin (1 + 15) WalkInitJalPc LinkWalkInit extractLinkedCode
      ...
      (walkInitAmbient loadPtr lenW typeW inner **
        extractWalkInitShortPost regionBase listLen bs listOff inner t6Old) := by
  ...
  refine cpsTripleWithin_weaken (fun _ hp => by
    ...
    have hq' :
        (walkInitAmbient loadPtr lenW typeW inner **
          extractWalkInitShortPost regionBase listLen bs listOff inner t6Old) h := by
      xperm_hyp hq
    obtain ⟨hA, hP, hd, hu, hamb, hpost⟩ := hq'
    have hconc := extractWalkInitShortPost_to_okConcrete regionBase listLen bs
    ...
    have hmid :
        (extractWalkInitCommon regionBase bs **
          extractWalkInitShortOkRegs regionBase listLen listOff) hP :=
      ⟨hL, hR, hd2, hu2, htemps', hRegs⟩
    -- goal wants ShortOkRegs with teer inner / lenW-inner
    have hmid' :
        (extractWalkInitCommon regionBase bs **
          extractWalkInitShortOkRegs regionBase (lenW - inner) (ambientAbsOff off inner.toNat)) hP := by
      simpa only [listLen, listOff, h_listLen, h_listOff, h_inner] using hmid
    exact ⟨hA, hP, hd, hu, hamb, hmid'⟩) hcallW

set_option maxRecDepth 8000 in
theorem extractWalkInitCall_short_ok_framed_s5s6_ambient
    (regionBase loadPtr lenW : Word) (bs : List (BitVec 8)) (off len : Nat)
    (old1 : Word)
    (hptr : loadPtr = regionBase + BitVec.ofNat 64 off)
    (hsalign : regionBase.toNat % 8 = 0)
  ...
  have h0 := extractWalkInitCall_short_fromTypeLoad_ambient
...
#print axioms extractWalkInitCall_short_fromTypeLoad_ambient
#print axioms extractWalkInitCall_short_ok_framed_s5s6_ambient
