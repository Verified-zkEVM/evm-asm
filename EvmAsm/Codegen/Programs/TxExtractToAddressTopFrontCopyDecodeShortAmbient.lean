/-
  Ambient dual: Front short AfterSave → 20B copy bare region of_decode
...
theorem extractFrontAfterSaveCopy_then_epi_of_decode_short_region_ambient
    ...
    cpsTripleWithin
      ...
      (frontAfterSavePostShortAmbient spC s regionBase loadPtr lenW toBuf isCreationPtr bs off len)
      ((.x1 ↦ᵣ s.ra) ** (.x2 ↦ᵣ sp0) **
  ...
  let cursor := shortWalkCursor regionBase listOff
  let endPtr := shortWalkEnd regionBase listLen listOff
  ...
  exact cpsTripleWithin_weaken
    (fun h hp => frontAfterSavePostShortAmbient_to_midJoinPre spC s regionBase loadPtr
      lenW toBuf isCreationPtr bs off len h hp)
    (fun _ hq => by
      dsimp only [contentPtr, w2] at hq ⊢
      xperm_hyp hq) hMid

#print axioms extractFrontAfterSaveCopy_then_epi_of_decode_short_region_ambient
