/-
  Front short AfterSave → legacy creation of_decode with concrete endPtr (no ∀endPtr).
-/
...
theorem afterSaveFrameTy_zero_eq_afterSaveFrame
    (txBase lenW innerW cursor endPtr : Word) (txBytes : List (BitVec 8)) :
    afterSaveFrameTy txBase lenW (0 : Word) innerW cursor endPtr txBytes =
      afterSaveFrame txBase lenW innerW cursor endPtr txBytes := by
  simp only [afterSaveFrameTy, afterSaveFrame]

set_option maxRecDepth 8000 in
theorem extractFrontAfterSaveCreation_then_epi_of_decode_short_legacy
  ...
  let cursor := shortWalkCursor txBase listOff
  let endPtr := shortWalkEnd txBase listLen listOff
  ...
  have hty := frontAfterSavePostShort_to_midJoinPre spC s txBase lenW
    toBuf isCreationPtr txBytes h hp
  simp only [htype0, afterSaveFrameTy_zero_eq_afterSaveFrame] at hty
  exact hty

#print axioms extractFrontAfterSaveCreation_then_epi_of_decode_short_legacy
