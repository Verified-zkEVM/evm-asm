/-
  Front long AfterSave → creation of_decode with concrete longWalkCursor/End.
-/
...
theorem extractFrontAfterSaveCreation_then_epi_of_decode_long
  ...
  have hMid := extractType234AfterSaveCreation_then_epi_of_decode sp0 spC s
    txBase lenW
  ...
  exact cpsTripleWithin_weaken
    (fun h hp => frontAfterSavePostLong_to_midJoinPre spC s txBase lenW
      toBuf isCreationPtr txBytes hwi_off h hp)
    (fun _ hq => hq) hMid

#print axioms extractFrontAfterSaveCreation_then_epi_of_decode_long
