...
def balAccountCodeAtOrBefore_prog : Program :=
  [ .ADDI .x2 .x2 (-160 : BitVec 12),
    ...
    .BLTU .x19 .x5 (76 : BitVec 13),
    .BLTU .x5 .x23 (72 : BitVec 13),
...
/-- Reloc side-table for `balAccountCodeAtOrBefore_prog`: the `la`/cross-`jal` instruction indices
    ...
    above carries concrete immediates for verification. This helper is a probe-only,
    unlinked entry, so its five external-call immediates are zero placeholders in the
    concrete Program; the emitted probe string keeps those calls symbolic. -/
