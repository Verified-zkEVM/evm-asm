/-
  ...
  `tsh_prefix_any_callWithin` (`payloadLen < 2^56`) into the success path
  with zero-init 8-byte BSS and `segs` = bare `rlpListPrefix`.
...
/-- nth-success continue through contiguous `rlp_encode_list_prefix` (`len < 2^56`).

    Requires the full BSS slot (`7 < |outBytes|`). Clobbers short+long temps. -/
theorem tshPostNthThenPrefixCall_any_spec
    ...
    (h_len : ((offVal + lenVal) - hdrLen).toNat < 72057594037927936)
...
/-- Peeled nth-success through any-form prefix and typed multi-rate finish.
    ...
    Specializes the BSS slot to zero-init 8 bytes so Apply and bare header
    agree under `bytesRegion`; `segs` use `rlpListPrefix` (header only). -/
theorem tshNthOkThroughTypedSuccess_any_spec
      ...
      (((.x1 ↦ᵣ (tshKssJalPC + 4)) **
        (tshKssCallPost sp0 newSp (tshKssJalPC + 4) tshSegsBase outPtr segs
          ...
          ((.x29 ↦ᵣ cellVal) ** (.x30 ↦ᵣ tshSegsBase) **
            ...
            ((tshNthOffPtr ↦ₘ offVal) ** (tshNthLenPtr ↦ₘ lenVal) **
              regOwn .x28 ** F))))) := by
  intro outBytes payloadLen prefixBs cellVal segs newSp prefixFuel gatherFuel kssFuel
  let Amb : Assertion :=
    (.x8 ↦ᵣ inPtr) ** (.x9 ↦ᵣ v9) ** (.x18 ↦ᵣ v18) **
      (.x19 ↦ᵣ typePrefix) ** (.x20 ↦ᵣ outPtr) **
      (.x2 ↦ᵣ sp0) **
  ...
  have hAmb : Amb.pcFree := by
    ...
    repeat first
      ...
      | exact hF
      | exact (by pcf)
  have hout_len : 7 < outBytes.length := by
  ...
  have hpref := tshNthOkThenPrefixCall_any_spec v11 v12 v22 offVal lenVal hdrLen cellOld
    outBytes Amb hAmb h_len h_out_align hout_len h_out_valid'
  let Post : Assertion :=
    (.x1 ↦ᵣ (tshKssJalPC + 4)) **
      (tshKssCallPost sp0 newSp (tshKssJalPC + 4) tshSegsBase outPtr segs
        inPtr v9 v18 typePrefix outPtr hdrLen payloadLen A **
        ((.x29 ↦ᵣ cellVal) ** (.x30 ↦ᵣ tshSegsBase) **
          ...
          ((tshNthOffPtr ↦ₘ offVal) ** (tshNthLenPtr ↦ₘ lenVal) **
            regOwn .x28 ** F)))
  -- Peel x29..x31 from post-prefix regOwn into PrefixReturn's regIs pre.
  have htail_of : ∀ v29 v30 v31,
    ...
    let F28 : Assertion := regOwn .x28 ** F
    have hF28 : F28.pcFree := pcFree_sepConj pcFree_regOwn hF
    ...
    exact cpsTripleWithin_weaken (fun _ hp => by
        ...
        xperm_hyp hp)
      (fun _ hq => by
        simp only [Post, payloadLen, prefixBs, cellVal, segs, F28, newSp] at hq ⊢
        exact hq) h
  have htail31 : ∀ v29 v30,
      cpsTripleWithin (gatherFuel + kssFuel) (H + 220) tshBodyExit fullCode
        (((.x1 ↦ᵣ (tshPrefixJalPC + 4)) **
          (.x10 ↦ᵣ (0 : Word)) ** (.x0 ↦ᵣ (0 : Word)) **
          regOwn .x5 ** regOwn .x6 ** regOwn .x7 **
    ...
    refine cpsTripleWithin_of_forall_regIs_to_regOwn (fun v31 => ?_)
    exact cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp)
      (fun _ hq => hq) (htail_of v29 v30 v31)
  have htail30 : ∀ v29,
      cpsTripleWithin (gatherFuel + kssFuel) (H + 220) tshBodyExit fullCode
        (((.x1 ↦ᵣ (tshPrefixJalPC + 4)) **
          (.x10 ↦ᵣ (0 : Word)) ** (.x0 ↦ᵣ (0 : Word)) **
          ...
          bytesRegion KssZk3 os **
          bytesRegion outPtr (List.replicate 32 (0 : BitVec 8)) **
          regOwns kssFreeTemps ** A ** F ** regOwn .x31) **
          regOwn .x30)
        Post := by
    intro v29
    refine cpsTripleWithin_of_forall_regIs_to_regOwn (fun v30 => ?_)
    exact cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp)
      (fun _ hq => hq) (htail31 v29 v30)
  have htail29 :
      cpsTripleWithin (gatherFuel + kssFuel) (H + 220) tshBodyExit fullCode
        (((.x1 ↦ᵣ (tshPrefixJalPC + 4)) **
          (.x10 ↦ᵣ (0 : Word)) ** (.x0 ↦ᵣ (0 : Word)) **
          ...
          bytesRegion KssZk3 os **
          bytesRegion outPtr (List.replicate 32 (0 : BitVec 8)) **
          regOwns kssFreeTemps ** A ** F ** regOwn .x31 ** regOwn .x30) **
          regOwn .x29)
        Post := by
    refine cpsTripleWithin_of_forall_regIs_to_regOwn (fun v29 => ?_)
    exact cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp)
      (fun _ hq => hq) (htail30 v29)
  have hbr :
      bytesRegion tshPrefixOutPtr (tshPrefixApply outBytes payloadLen.toNat) =
        bytesRegion tshPrefixOutPtr prefixBs := by
    simp only [outBytes, prefixBs]
    exact tshPrefix_bytesRegion_apply_eq_hdr tshPrefixOutPtr payloadLen.toNat h_len
      (tshPrefixNH_pos _)
  have c := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by
      simp only [Amb, payloadLen, prefixBs, cellVal, outBytes, hbr] at hp ⊢
      xperm_hyp hp) hpref htail29
  exact cpsTripleWithin_weaken (fun _ hp => by
      simp only [Amb, outBytes] at hp ⊢
      xperm_hyp hp)
    (fun _ hq => by
      simp only [Post, payloadLen, prefixBs, cellVal, segs] at hq ⊢
      exact hq) c
