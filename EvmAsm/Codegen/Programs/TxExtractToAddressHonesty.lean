...
/-- Decode + short-form list head (`pfx < 0xf8`) ⇒ outer payload encode ≤ 55.
    `hlen` is free from ambient hover (`bs.length < 2^64 = 256^8`).
    Unblocks ambient short-arm `hshort` from `extractSuccess` + head guard. -/
theorem encodeItems_le_55_of_decode_short_list_head
    (bs : List Byte) (items : List RLPItem)
    (h : decodeListItems bs = some items)
    (h0 : 0 < bs.length)
    (hlen : bs.length < 256 ^ 8)
    (hhi : BitVec.ult ((bs[0]'h0).zeroExtend 64) (0xf8 : Word) = true) :
  ...
  · have hgt : 55 < (encode.encodeItems items).length := by omega
    have hlong := encode_list_long items hgt
    set n := (encode.encodeItems items).length with hn
    set lol := Nat.toBytesBE n with hlol
    have hlol_pos : 0 < lol.length := by
      obtain ⟨_, _, hcons, _⟩ := Nat.toBytesBE_eq_cons_of_pos n (by omega)
      simp [hlol, hcons]
    have hn_lt : n < 256 ^ 8 := by
      have hbs_len :
          bs.length =
            (BitVec.ofNat 8 (0xF7 + lol.length) ::
              (lol ++ encode.encodeItems items)).length := by
        rw [henc, hlong, hlol]
      have hsum :
          (BitVec.ofNat 8 (0xF7 + lol.length) ::
              (lol ++ encode.encodeItems items)).length =
            1 + lol.length + n := by
        simp [List.length_cons, List.length_append, hn]
        omega
      omega
    have hlol_le8 : lol.length ≤ 8 := Nat.toBytesBE_length_le n 8 hn_lt
    have hpfx_lt : 0xF7 + lol.length < 256 := by omega
    have hhead :
        bs[0]'h0 = BitVec.ofNat 8 (0xF7 + lol.length) := by
      have henc0 :
          bs = BitVec.ofNat 8 (0xF7 + lol.length) :: (lol ++ encode.encodeItems items) := by
        rw [henc, hlong, hlol]
      simp [henc0]
    have hnot : ¬ BitVec.ult ((bs[0]'h0).zeroExtend 64) (0xf8 : Word) = true := by
      intro hult
      have hlt := (BitVec.ult_iff_lt).mp hult
      rw [hhead, BitVec.lt_def] at hlt
      have hb : (0xf8 : Word).toNat = 248 := by decide
      have hsum :
          (BitVec.ofNat 8 (0xF7 + lol.length)).toNat = 0xF7 + lol.length := by
        rw [BitVec.toNat_ofNat, Nat.mod_eq_of_lt hpfx_lt]
      have hze :
          ((BitVec.ofNat 8 (0xF7 + lol.length)).zeroExtend 64).toNat =
            0xF7 + lol.length := by
        rw [toNat_zeroExtend_byte, hsum]
      rw [hze, hb] at hlt
      omega
...
/-- Same as `extractSuccess_creation_to_field_encode_le55` with explicit index bound. -/
theorem extractSuccess_creation_to_field_encode_le55_of
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcre : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (items : List RLPItem)
    (hdec : decodeListItems (txBytes.drop (teerTxTypeDispatch txBytes).2.2.toNat) =
      some items)
    (hn : toFieldIndex (teerTxTypeDispatch txBytes).2.1.toNat < items.length) :
    (encode (items[toFieldIndex (teerTxTypeDispatch txBytes).2.1.toNat]'hn)).length ≤ 55 := by
  obtain ⟨hn', hle⟩ := extractSuccess_creation_to_field_encode_le55 txBytes h hcre items hdec
  exact hle
...
#print axioms extractSuccess_creation_to_field_encode_le55_of
...
theorem shortListSrcOff_add_one_le_succ
    (listOff : Nat) (items : List RLPItem) (n : Nat)
    (hn : n < items.length) :
    shortListSrcOff listOff items n + 1 ≤ shortListSrcOff listOff items (n + 1) := by
  have hpos : 0 < (encode (items[n]'hn)).length := encode_item_length_pos _
  have hsucc := shortListSrcOff_succ listOff items n hn
  omega

/-- `srcOff_n + 1 < length` when a following field exists (encode nonempty). -/
theorem shortListSrcOff_succ_room
    (bs : List Byte) (listOff : Nat) (items : List RLPItem) (n : Nat)
    (henc : bs.drop listOff = encode (.list items))
    (hshort : (encode.encodeItems items).length ≤ 55)
    (hnxt : n + 1 < items.length) :
    shortListSrcOff listOff items n + 1 < bs.length := by
  have hn : n < items.length := Nat.lt_of_succ_lt hnxt
  have hle := shortListSrcOff_add_one_le_succ listOff items n hn
  have hnext := shortListSrcOff_lt_length bs listOff items (n + 1) henc hshort hnxt
  omega

/-- Pack creation type234 short hvalid/hvalid1 at all six field offsets from
    one buffer-wide `validByteRange`. Needs `7 ≤ items.length` so field5 has a
    following item (srcOff5+1 in-bounds). -/
theorem extractSuccess_creation_type234_hvalid_srcOff
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (hge : 2 ≤ (teerTxTypeDispatch txBytes).2.1.toNat)
  ...
  have hlen :=
    extractSuccess_creation_type234_items_length txBytes h hcreFlag hge
      items hdecL hshort
  have hn0 : (0 : Nat) < items.length := by omega
  have hn1 : (1 : Nat) < items.length := by omega
  have hn2 : (2 : Nat) < items.length := by omega
...
/-- `hvalidTx0` + inner-off validity from buffer range under extractSuccess. -/
theorem extractSuccess_hvalid_tx0_inner
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hvalidBuf : validByteRange txBase txBytes.length) :
    isValidByteAccess (txBase + BitVec.ofNat 64 0) = true ∧
      isValidByteAccess
        (txBase + BitVec.ofNat 64 (teerTxTypeDispatch txBytes).2.2.toNat) = true := by
  have hinner := extractSuccess_inner_lt txBytes h
  have hpos : 0 < txBytes.length := Nat.zero_lt_of_lt hinner
  exact ⟨
...
/-- Copy type234 short: field-5 (20B address) encodes as ≥2 bytes (`0x94`::content). -/
theorem extractSuccess_copy_type234_field5_encode_ge_two
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (hge : 2 ≤ (teerTxTypeDispatch txBytes).2.1.toNat)
...
#print axioms extractSuccess_copy_type234_field5_encode_ge_two
...
theorem extractSuccess_copy_type234_hvalid_srcOff
  ...
  have h4s := shortListSrcOff_succ_room txBytes listOff items 4 hencInner hshort (by omega)
  have hge2_5 :=
    extractSuccess_copy_type234_field5_encode_ge_two txBytes h hcopyFlag hge
      items hdecL hshort hn5
  have h5s :=
    hss_room_of_encode_ge_two txBytes listOff items 5 hencInner hshort hn5 hge2_5
  exact ⟨
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf h0,
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf h0s,
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf h1,
...
/-! ### T1 (type=1) creation short-list packaging pure -/

/-- Creation + t1 + short list ⇒ field index 4 is empty bytes at `0x80`. -/
theorem extractSuccess_creation_t1_field4_pfx80
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    ...
    (hshort : (encode.encodeItems items).length ≤ 55) :
    let listOff := (teerTxTypeDispatch txBytes).2.2.toNat
    let off := listOff + 1 + encodeItemsPrefixLen items 4
    (4 : Nat) < items.length ∧
      items[4]? = some (.bytes []) ∧
      ∃ hoff : off < txBytes.length, txBytes[off]'hoff = (0x80 : BitVec 8) := by
  intro listOff off
  have hidx : toFieldIndex (teerTxTypeDispatch txBytes).2.1.toNat = 4 := by
    simp only [htype1]
    exact toFieldIndex_t1
  have htf := extractSuccess_creation_encode_empty txBytes h hcreFlag
  obtain ⟨items', hdec', hitem, _henc80⟩ := htf
  have hitems : items = items' := by
    have : some items = some items' := hdec.symm.trans hdec'
    exact Option.some.inj this
  subst hitems
  have hget : items[4]? = some (.bytes []) := by simpa [hidx] using hitem
  have hn : (4 : Nat) < items.length := (List.getElem?_eq_some_iff.mp hget).1
  have hval : items[4]'hn = .bytes [] := (List.getElem?_eq_some_iff.mp hget).2
  have hencFull := decodeListItems_eq_encode _ _ hdec
  have hpfx := short_list_empty_bytes_pfx txBytes listOff items 4 hencFull hshort hn hval
  exact ⟨hn, hget, hpfx⟩

theorem extractSuccess_creation_t1_items_length
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    (hshort : (encode.encodeItems items).length ≤ 55) :
    5 ≤ items.length := by
  have hf := extractSuccess_creation_t1_field4_pfx80 txBytes h hcreFlag htype1
    items hdecL hshort
  omega

theorem extractSuccess_creation_t1_hcre
    (txBytes : List (BitVec 8)) (txBase : Word) (srcOff4 : Nat)
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    (items : List RLPItem)
    ...
    (hshort : (encode.encodeItems items).length ≤ 55)
    (hsrc : srcOff4 =
      (teerTxTypeDispatch txBytes).2.2.toNat + 1 + encodeItemsPrefixLen items 4) :
    ∀ (endPtr next4 len4 : Word),
      rlpItemDecode txBytes srcOff4 (txBase + BitVec.ofNat 64 srcOff4)
        endPtr next4 len4 → len4 = (0 : Word) := by
  have hf := extractSuccess_creation_t1_field4_pfx80 txBytes h hcreFlag htype1
    items hdecL hshort
  obtain ⟨_hn, _hget, hpfx⟩ := hf
  obtain ⟨hoff, hb⟩ := hpfx
  have hsrc' : srcOff4 =
      (teerTxTypeDispatch txBytes).2.2.toNat + 1 + encodeItemsPrefixLen items 4 := hsrc
  intro endPtr next4 len4 hdec
  have hoff' : srcOff4 < txBytes.length := by simpa [hsrc'] using hoff
  have hb' : txBytes[srcOff4]'hoff' = (0x80 : BitVec 8) := by simpa [hsrc'] using hb
  exact hcre_decode_of_pfx80 txBytes srcOff4
    (txBase + BitVec.ofNat 64 srcOff4) endPtr hoff' hb' next4 len4 hdec

theorem extractSuccess_creation_t1_hcre_srcOff
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    ∀ (endPtr next4 len4 : Word),
      rlpItemDecode txBytes srcOff4 (txBase + BitVec.ofNat 64 srcOff4)
        endPtr next4 len4 → len4 = (0 : Word) := by
  intro listOff srcOff4
  exact extractSuccess_creation_t1_hcre txBytes txBase srcOff4 h hcreFlag htype1
    items hdecL hshort rfl

theorem extractSuccess_creation_t1_hnext_fields03
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    (0 : Nat) + 1 < items.length ∧
    (1 : Nat) + 1 < items.length ∧
    (2 : Nat) + 1 < items.length ∧
    (3 : Nat) + 1 < items.length := by
  have _ := extractSuccess_creation_t1_items_length txBytes h hcreFlag htype1
    items hdecL hshort
  omega

theorem extractSuccess_creation_t1_hoff_srcOff
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    shortListSrcOff listOff items 2 < txBytes.length ∧
    shortListSrcOff listOff items 3 < txBytes.length ∧
    shortListSrcOff listOff items 4 < txBytes.length := by
  intro listOff
  have hlen := extractSuccess_creation_t1_items_length txBytes h hcreFlag htype1
    items hdecL hshort
  have henc := decodeListItems_eq_encode _ _ hdecL
  have hn0 : (0 : Nat) < items.length := by omega
  have hn1 : (1 : Nat) < items.length := by omega
  ...
  exact ⟨
    ...
    shortListSrcOff_lt_length txBytes listOff items 2 henc hshort hn2,
    shortListSrcOff_lt_length txBytes listOff items 3 henc hshort hn3,
    shortListSrcOff_lt_length txBytes listOff items 4 henc hshort hn4⟩

theorem extractSuccess_creation_t1_hover_srcOff
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    txBase.toNat + shortListSrcOff listOff items 2 < 2 ^ 64 ∧
    txBase.toNat + shortListSrcOff listOff items 3 < 2 ^ 64 ∧
    txBase.toNat + shortListSrcOff listOff items 4 < 2 ^ 64 := by
  intro listOff
  have hoffs := extractSuccess_creation_t1_hoff_srcOff txBytes h hcreFlag htype1
    items hdecL hshort
  obtain ⟨h0, h1, h2, h3, h4⟩ := hoffs
  exact ⟨hover_of_buffer_span txBase _ _ hover h0,
    hover_of_buffer_span txBase _ _ hover h1,
    hover_of_buffer_span txBase _ _ hover h2,
    hover_of_buffer_span txBase _ _ hover h3,
...
theorem extractSuccess_creation_t1_hnext_hcre_srcOff
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    (∀ (endPtr next4 len4 : Word),
      rlpItemDecode txBytes srcOff4 (txBase + BitVec.ofNat 64 srcOff4)
        endPtr next4 len4 → len4 = (0 : Word)) := by
  intro listOff srcOff0 srcOff1 srcOff2 srcOff3 srcOff4
  have hlen := extractSuccess_creation_t1_items_length txBytes h hcreFlag htype1
    items hdecL hshort
  have hn0 : (0 : Nat) < items.length := by omega
  have hn1 : (1 : Nat) < items.length := by omega
  ...
  · intro endPtr next len hdec
    exact packaging_hnext_shortListSrcOff txBytes txBase listOff items 3 hn3
      hencFull hshort hover3 hover4 endPtr next len hdec
  · intro endPtr next4 len4 hdec
    exact extractSuccess_creation_t1_hcre_srcOff txBytes txBase h hcreFlag htype1
      items hdecL hshort endPtr next4 len4 hdec

theorem extractSuccess_creation_t1_hvalid_srcOff
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    isValidByteAccess (txBase + BitVec.ofNat 64 (shortListSrcOff listOff items 3 + 1)) = true ∧
    isValidByteAccess (txBase + BitVec.ofNat 64 (shortListSrcOff listOff items 4)) = true ∧
    isValidByteAccess (txBase + BitVec.ofNat 64 (shortListSrcOff listOff items 4 + 1)) = true := by
  let listOff := (teerTxTypeDispatch txBytes).2.2.toNat
  have hencInner :
      txBytes.drop listOff = encode (.list items) :=
    decodeListItems_eq_encode _ _ hdecL
  have hlen :=
    extractSuccess_creation_t1_items_length txBytes h hcreFlag htype1
      items hdecL hshort
  have hn0 : (0 : Nat) < items.length := by omega
  have hn1 : (1 : Nat) < items.length := by omega
  have hn2 : (2 : Nat) < items.length := by omega
  ...
  have h2s := shortListSrcOff_succ_room txBytes listOff items 2 hencInner hshort (by omega)
  have h3s := shortListSrcOff_succ_room txBytes listOff items 3 hencInner hshort (by omega)
  have h4s := shortListSrcOff_succ_room txBytes listOff items 4 hencInner hshort hn5
  exact ⟨
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf h0,
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf h0s,
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf h1,
...
#print axioms extractSuccess_creation_t1_field4_pfx80
#print axioms extractSuccess_creation_t1_items_length
#print axioms extractSuccess_creation_t1_hnext_hcre_srcOff
#print axioms extractSuccess_creation_t1_hvalid_srcOff




/-! ### Legacy (type=0) copy short-list packaging pure -/

/-- Copy + legacy + short list ⇒ field index 3 is 20B address at `0x94`. -/
theorem extractSuccess_copy_legacy_field3_pfx94
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
    ...
    let off := listOff + 1 + encodeItemsPrefixLen items 3
    (3 : Nat) < items.length ∧
      (∃ content, items[3]? = some (.bytes content) ∧ content.length = 20) ∧
      ∃ hoff : off < txBytes.length, txBytes[off]'hoff = (0x94 : BitVec 8) := by
  intro listOff off
  have hidx : toFieldIndex (teerTxTypeDispatch txBytes).2.1.toNat = 3 := by
    simp only [htype0]
  ...
  have hitems : items = items' := by
    ...
    exact Option.some.inj this
  subst hitems
  have hget : items[3]? = some (.bytes content) := by simpa [hidx] using hitem
  have hn : (3 : Nat) < items.length := (List.getElem?_eq_some_iff.mp hget).1
  have hval : items[3]'hn = .bytes content := (List.getElem?_eq_some_iff.mp hget).2
  have hencFull := decodeListItems_eq_encode _ _ hdec
  have hpfx := short_list_bytes20_pfx txBytes listOff items 3 hencFull hshort hn
    content hval hlen20
  exact ⟨hn, ⟨content, hget, hlen20⟩, hpfx⟩

theorem extractSuccess_copy_legacy_items_length
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
    ...
    (hshort : (encode.encodeItems items).length ≤ 55) :
    4 ≤ items.length := by
  have hf := extractSuccess_copy_legacy_field3_pfx94 txBytes h hcopyFlag htype0
    items hdecL hshort
  omega

theorem extractSuccess_copy_legacy_hlen20
    (txBytes : List (BitVec 8)) (txBase : Word) (srcOff3 : Nat)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
    ...
    ∀ (endPtr next3 len3 : Word),
      rlpItemDecode txBytes srcOff3 (txBase + BitVec.ofNat 64 srcOff3)
        endPtr next3 len3 → len3 = (20 : Word) := by
  have hf := extractSuccess_copy_legacy_field3_pfx94 txBytes h hcopyFlag htype0
    items hdecL hshort
  obtain ⟨_hn, _hget, hpfx⟩ := hf
  obtain ⟨hoff, hb⟩ := hpfx
  ...
  intro endPtr next3 len3 hdec
  have hoff' : srcOff3 < txBytes.length := by simpa [hsrc'] using hoff
  have hb' : txBytes[srcOff3]'hoff' = (0x94 : BitVec 8) := by simpa [hsrc'] using hb
  exact hlen20_decode_of_pfx94 txBytes srcOff3
    (txBase + BitVec.ofNat 64 srcOff3) endPtr hoff' hb' next3 len3 hdec

theorem extractSuccess_copy_legacy_hnext_content
    (txBytes : List (BitVec 8)) (txBase contentPtr : Word) (srcOff3 : Nat)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
    (items : List RLPItem)
    ...
    (hsrc : srcOff3 =
      (teerTxTypeDispatch txBytes).2.2.toNat + 1 + encodeItemsPrefixLen items 3)
    (hcontent : contentPtr =
      txBase + BitVec.ofNat 64 srcOff3 + (1 : Word)) :
    ∀ (endPtr next3 len3 : Word),
      rlpItemDecode txBytes srcOff3 (txBase + BitVec.ofNat 64 srcOff3)
        endPtr next3 len3 → next3 = contentPtr + (20 : Word) := by
  have hf := extractSuccess_copy_legacy_field3_pfx94 txBytes h hcopyFlag htype0
    items hdecL hshort
  obtain ⟨_hn, _hget, hpfx⟩ := hf
  obtain ⟨hoff, hb⟩ := hpfx
  ...
  have hoff' : srcOff3 < txBytes.length := by simpa [hsrc'] using hoff
  have hb' : txBytes[srcOff3]'hoff' = (0x94 : BitVec 8) := by simpa [hsrc'] using hb
  exact hnext_content_decode_of_pfx94 txBytes srcOff3
    (txBase + BitVec.ofNat 64 srcOff3) endPtr contentPtr
    hoff' hb' hcontent next3 len3 hdec

theorem extractSuccess_copy_legacy_hoff_srcOff
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
    ...
    shortListSrcOff listOff items 3 < txBytes.length := by
  intro listOff
  have hlen := extractSuccess_copy_legacy_items_length txBytes h hcopyFlag htype0
    items hdecL hshort
  have henc := decodeListItems_eq_encode _ _ hdecL
  have hn0 : (0 : Nat) < items.length := by omega
  have hn1 : (1 : Nat) < items.length := by omega
...
theorem extractSuccess_copy_legacy_hover_srcOff
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
    ...
    txBase.toNat + shortListSrcOff listOff items 3 < 2 ^ 64 := by
  intro listOff
  have hoffs := extractSuccess_copy_legacy_hoff_srcOff txBytes h hcopyFlag htype0
    items hdecL hshort
  obtain ⟨h0, h1, h2, h3⟩ := hoffs
  exact ⟨hover_of_buffer_span txBase _ _ hover h0,
    hover_of_buffer_span txBase _ _ hover h1,
...
theorem extractSuccess_copy_legacy_hnext_fields02
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
    ...
    (1 : Nat) + 1 < items.length ∧
    (2 : Nat) + 1 < items.length := by
  have _ := extractSuccess_copy_legacy_items_length txBytes h hcopyFlag htype0
    items hdecL hshort
  omega

theorem extractSuccess_copy_legacy_hnext_hlen20_srcOff
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
  ...
  · intro endPtr next len hdec
    exact packaging_hnext_shortListSrcOff txBytes txBase listOff items 2 hn2
      hencFull hshort hover2 hover3 endPtr next len hdec
  · intro endPtr next3 len3 hdec
    exact extractSuccess_copy_legacy_hlen20 txBytes txBase srcOff3 h hcopyFlag htype0
      items hdecL hshort rfl endPtr next3 len3 hdec

theorem extractSuccess_copy_legacy_field3_encode_ge_two
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
...
theorem extractSuccess_copy_legacy_hvalid_srcOff
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
  ...
  have hencInner :
    ...
    decodeListItems_eq_encode _ _ hdecL
  have hlen :=
    extractSuccess_copy_legacy_items_length txBytes h hcopyFlag htype0
      items hdecL hshort
  have hn0 : (0 : Nat) < items.length := by omega
  have hn1 : (1 : Nat) < items.length := by omega
  have hn2 : (2 : Nat) < items.length := by omega
  ...
  have h2s := shortListSrcOff_succ_room txBytes listOff items 2 hencInner hshort (by omega)
  have hge2_3 :=
    extractSuccess_copy_legacy_field3_encode_ge_two txBytes h hcopyFlag htype0
      items hdecL hshort hn3
  have h3s :=
    hss_room_of_encode_ge_two txBytes listOff items 3 hencInner hshort hn3 hge2_3
  exact ⟨
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf h0,
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf h0s,
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf h1,
...
#print axioms extractSuccess_copy_legacy_field3_pfx94
#print axioms extractSuccess_copy_legacy_field3_encode_ge_two
#print axioms extractSuccess_copy_legacy_hnext_hlen20_srcOff
#print axioms extractSuccess_copy_legacy_hvalid_srcOff

/-! ### T1 (type=1) copy short-list packaging pure -/

theorem extractSuccess_copy_t1_field4_pfx94
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    let off := listOff + 1 + encodeItemsPrefixLen items 4
    (4 : Nat) < items.length ∧
      (∃ content, items[4]? = some (.bytes content) ∧ content.length = 20) ∧
      ∃ hoff : off < txBytes.length, txBytes[off]'hoff = (0x94 : BitVec 8) := by
  intro listOff off
  have hidx : toFieldIndex (teerTxTypeDispatch txBytes).2.1.toNat = 4 := by
    simp only [htype1]
  ...
  have hitems : items = items' := by
    ...
    exact Option.some.inj this
  subst hitems
  have hget : items[4]? = some (.bytes content) := by simpa [hidx] using hitem
  have hn : (4 : Nat) < items.length := (List.getElem?_eq_some_iff.mp hget).1
  have hval : items[4]'hn = .bytes content := (List.getElem?_eq_some_iff.mp hget).2
  have hencFull := decodeListItems_eq_encode _ _ hdec
  have hpfx := short_list_bytes20_pfx txBytes listOff items 4 hencFull hshort hn
    content hval hlen20
  exact ⟨hn, ⟨content, hget, hlen20⟩, hpfx⟩

theorem extractSuccess_copy_t1_items_length
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    (hshort : (encode.encodeItems items).length ≤ 55) :
    5 ≤ items.length := by
  have hf := extractSuccess_copy_t1_field4_pfx94 txBytes h hcopyFlag htype1
    items hdecL hshort
  omega

theorem extractSuccess_copy_t1_hlen20
    (txBytes : List (BitVec 8)) (txBase : Word) (srcOff4 : Nat)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    ∀ (endPtr next4 len4 : Word),
      rlpItemDecode txBytes srcOff4 (txBase + BitVec.ofNat 64 srcOff4)
        endPtr next4 len4 → len4 = (20 : Word) := by
  have hf := extractSuccess_copy_t1_field4_pfx94 txBytes h hcopyFlag htype1
    items hdecL hshort
  obtain ⟨_hn, _hget, hpfx⟩ := hf
  obtain ⟨hoff, hb⟩ := hpfx
  ...
  intro endPtr next4 len4 hdec
  have hoff' : srcOff4 < txBytes.length := by simpa [hsrc'] using hoff
  have hb' : txBytes[srcOff4]'hoff' = (0x94 : BitVec 8) := by simpa [hsrc'] using hb
  exact hlen20_decode_of_pfx94 txBytes srcOff4
    (txBase + BitVec.ofNat 64 srcOff4) endPtr hoff' hb' next4 len4 hdec

theorem extractSuccess_copy_t1_hnext_content
    (txBytes : List (BitVec 8)) (txBase contentPtr : Word) (srcOff4 : Nat)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    (items : List RLPItem)
    ...
    (hsrc : srcOff4 =
      (teerTxTypeDispatch txBytes).2.2.toNat + 1 + encodeItemsPrefixLen items 4)
    (hcontent : contentPtr =
      txBase + BitVec.ofNat 64 srcOff4 + (1 : Word)) :
    ∀ (endPtr next4 len4 : Word),
      rlpItemDecode txBytes srcOff4 (txBase + BitVec.ofNat 64 srcOff4)
        endPtr next4 len4 → next4 = contentPtr + (20 : Word) := by
  have hf := extractSuccess_copy_t1_field4_pfx94 txBytes h hcopyFlag htype1
    items hdecL hshort
  obtain ⟨_hn, _hget, hpfx⟩ := hf
  obtain ⟨hoff, hb⟩ := hpfx
  ...
  have hoff' : srcOff4 < txBytes.length := by simpa [hsrc'] using hoff
  have hb' : txBytes[srcOff4]'hoff' = (0x94 : BitVec 8) := by simpa [hsrc'] using hb
  exact hnext_content_decode_of_pfx94 txBytes srcOff4
    (txBase + BitVec.ofNat 64 srcOff4) endPtr contentPtr
    hoff' hb' hcontent next4 len4 hdec

theorem extractSuccess_copy_t1_hoff_srcOff
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    shortListSrcOff listOff items 4 < txBytes.length := by
  intro listOff
  have hlen := extractSuccess_copy_t1_items_length txBytes h hcopyFlag htype1
    items hdecL hshort
  have henc := decodeListItems_eq_encode _ _ hdecL
  have hn0 : (0 : Nat) < items.length := by omega
  have hn1 : (1 : Nat) < items.length := by omega
...
theorem extractSuccess_copy_t1_hover_srcOff
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    txBase.toNat + shortListSrcOff listOff items 4 < 2 ^ 64 := by
  intro listOff
  have hoffs := extractSuccess_copy_t1_hoff_srcOff txBytes h hcopyFlag htype1
    items hdecL hshort
  obtain ⟨h0, h1, h2, h3, h4⟩ := hoffs
  exact ⟨hover_of_buffer_span txBase _ _ hover h0,
    hover_of_buffer_span txBase _ _ hover h1,
...
theorem extractSuccess_copy_t1_hnext_fields03
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    (2 : Nat) + 1 < items.length ∧
    (3 : Nat) + 1 < items.length := by
  have _ := extractSuccess_copy_t1_items_length txBytes h hcopyFlag htype1
    items hdecL hshort
  omega

theorem extractSuccess_copy_t1_hnext_hlen20_srcOff
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
  ...
  · intro endPtr next len hdec
    exact packaging_hnext_shortListSrcOff txBytes txBase listOff items 3 hn3
      hencFull hshort hover3 hover4 endPtr next len hdec
  · intro endPtr next4 len4 hdec
    exact extractSuccess_copy_t1_hlen20 txBytes txBase srcOff4 h hcopyFlag htype1
      items hdecL hshort rfl endPtr next4 len4 hdec

theorem extractSuccess_copy_t1_field4_encode_ge_two
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
...
theorem extractSuccess_copy_t1_hvalid_srcOff
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
  ...
  have hencInner :
    ...
    decodeListItems_eq_encode _ _ hdecL
  have hlen :=
    extractSuccess_copy_t1_items_length txBytes h hcopyFlag htype1
      items hdecL hshort
  have hn0 : (0 : Nat) < items.length := by omega
  have hn1 : (1 : Nat) < items.length := by omega
  have hn2 : (2 : Nat) < items.length := by omega
  ...
  have h3s := shortListSrcOff_succ_room txBytes listOff items 3 hencInner hshort (by omega)
  have hge2_4 :=
    extractSuccess_copy_t1_field4_encode_ge_two txBytes h hcopyFlag htype1
      items hdecL hshort hn4
  have h4s :=
    hss_room_of_encode_ge_two txBytes listOff items 4 hencInner hshort hn4 hge2_4
  exact ⟨
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf h0,
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf h0s,
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf h1,
...
#print axioms extractSuccess_copy_t1_field4_pfx94
#print axioms extractSuccess_copy_t1_field4_encode_ge_two
...
/-! ## Long-list leaf hyps (h_llz / h_min / hllen / hoff1 / h_fits / h_match) -/

private theorem signExtend12_one : signExtend12 (1 : BitVec 12) = (1 : Word) := by decide

private theorem word_one_toNat : (1 : Word).toNat = 1 := by decide

/-- Full long-list encode shape at absolute drop. -/
theorem long_list_encode_cons
    (txBytes : List (BitVec 8)) (listOff : Nat) (items : List RLPItem)
    (henc : txBytes.drop listOff = encode (.list items))
    (hlong : 55 < (encode.encodeItems items).length) :
    txBytes.drop listOff =
      BitVec.ofNat 8 (0xF7 + longListLol items) ::
        (Nat.toBytesBE (encode.encodeItems items).length ++ encode.encodeItems items) := by
  have hstr := encode_list_long items hlong
  unfold longListLol
...
/-- Take lol after prefix = toBytesBE payload length. -/
theorem long_list_tx_take_lol
    (txBytes : List (BitVec 8)) (listOff : Nat) (items : List RLPItem)
    (henc : txBytes.drop listOff = encode (.list items))
    (hlong : 55 < (encode.encodeItems items).length) :
    (txBytes.drop (listOff + 1)).take (longListLol items) =
      Nat.toBytesBE (encode.encodeItems items).length := by
  have hdrop : txBytes.drop (listOff + 1) = (txBytes.drop listOff).drop 1 := by
    rw [← List.drop_drop]
  have hcons := long_list_encode_cons txBytes listOff items henc hlong
  rw [hdrop, hcons]
  -- (pfx :: (lenBytes ++ payload)).drop 1 = lenBytes ++ payload
  change ((Nat.toBytesBE (encode.encodeItems items).length ++ encode.encodeItems items).take
      (longListLol items)) =
    Nat.toBytesBE (encode.encodeItems items).length
  unfold longListLol
  exact List.take_left

/-- Prefix byte at absolute offset. -/
theorem long_list_pfx_at
    (txBytes : List (BitVec 8)) (listOff : Nat) (items : List RLPItem)
    (henc : txBytes.drop listOff = encode (.list items))
    (hlong : 55 < (encode.encodeItems items).length)
    (hoff : listOff < txBytes.length) :
    txBytes[listOff]'hoff = BitVec.ofNat 8 (0xF7 + longListLol items) := by
  have hcons := long_list_encode_cons txBytes listOff items henc hlong
  have hpos : 0 < (txBytes.drop listOff).length := by
    rw [hcons]; exact Nat.zero_lt_succ _
  have heq := List.getElem_drop (xs := txBytes) (i := listOff) (j := 0) (h := hpos)
  simp only [Nat.add_zero] at heq
  have hhead : (txBytes.drop listOff)[0]'hpos =
      BitVec.ofNat 8 (0xF7 + longListLol items) := by
    have hlen' : 0 <
        (BitVec.ofNat 8 (0xF7 + longListLol items) ::
          (Nat.toBytesBE (encode.encodeItems items).length ++
      ...
      Nat.zero_lt_succ _
    have heqL :
        (txBytes.drop listOff)[0]'hpos =
          (BitVec.ofNat 8 (0xF7 + longListLol items) ::
            (Nat.toBytesBE (encode.encodeItems items).length ++
              encode.encodeItems items))[0]'hlen' :=
      List.getElem_of_eq hcons hpos
    rw [heqL, List.getElem_cons_zero]
  exact heq.symm.trans hhead

/-- `hoff1` from long-list structure. -/
theorem hoff1_of_long_list
    (txBytes : List (BitVec 8)) (listOff : Nat) (items : List RLPItem)
    (henc : txBytes.drop listOff = encode (.list items))
    (hlong : 55 < (encode.encodeItems items).length) :
    listOff + 1 < txBytes.length := by
  have h1 := longListLol_pos items hlong
  have hdrop : (txBytes.drop listOff).length =
      1 + longListLol items + longListPayloadLen items := by
    rw [henc, encode_list_long_length items hlong]
  have : 1 < (txBytes.drop listOff).length := by omega
  simp only [List.length_drop] at this
  omega

/-- `hllen` from long-list structure. -/
theorem hllen_of_long_list
    (txBytes : List (BitVec 8)) (listOff : Nat) (items : List RLPItem)
    (henc : txBytes.drop listOff = encode (.list items))
    (hlong : 55 < (encode.encodeItems items).length)
    (hoff : listOff < txBytes.length)
  ...
  rw [hpfx, pfx_sub_F7_eq_lol items hlong hbound]
  have hdrop : (txBytes.drop listOff).length =
      1 + longListLol items + longListPayloadLen items := by
    rw [henc, encode_list_long_length items hlong]
...
/-- Leading length byte at listOff+1. -/
theorem long_list_len_byte_at
    (txBytes : List (BitVec 8)) (listOff : Nat) (items : List RLPItem)
    (henc : txBytes.drop listOff = encode (.list items))
    (hlong : 55 < (encode.encodeItems items).length)
    (hoff1 : listOff + 1 < txBytes.length) :
    ∃ b tl, Nat.toBytesBE (encode.encodeItems items).length = b :: tl ∧
      b ≠ (0 : Byte) ∧ txBytes[listOff + 1]'hoff1 = b := by
  obtain ⟨b, tl, heq, hbne⟩ := long_list_len_head_ne_zero items hlong
  refine ⟨b, tl, heq, hbne, ?_⟩
  have hcons := long_list_encode_cons txBytes listOff items henc hlong
  have hfull : txBytes.drop listOff =
      BitVec.ofNat 8 (0xF7 + longListLol items) :: b :: (tl ++ encode.encodeItems items) := by
    rw [hcons, heq]
    rfl
  have hpos1 : 1 < (txBytes.drop listOff).length := by
    rw [hfull]; simp
  have hidx : (txBytes.drop listOff)[1]'hpos1 = b := by
    have hlen' : 1 <
        (BitVec.ofNat 8 (0xF7 + longListLol items) :: b ::
          (tl ++ encode.encodeItems items)).length := by simp
    have heqL :
        (txBytes.drop listOff)[1]'hpos1 =
          (BitVec.ofNat 8 (0xF7 + longListLol items) :: b ::
            (tl ++ encode.encodeItems items))[1]'hlen' :=
      List.getElem_of_eq hfull hpos1
    rw [heqL, List.getElem_cons_succ, List.getElem_cons_zero]
  have heq' := List.getElem_drop (xs := txBytes) (i := listOff) (j := 1) (h := hpos1)
  exact heq'.symm.trans hidx

/-- `h_llz`: leading length byte zeroExtend ≠ 0. -/
theorem h_llz_of_long_list
    (txBytes : List (BitVec 8)) (listOff : Nat) (items : List RLPItem)
    (henc : txBytes.drop listOff = encode (.list items))
    (hlong : 55 < (encode.encodeItems items).length)
    (hoff1 : listOff + 1 < txBytes.length) :
    (txBytes[listOff + 1]'hoff1).zeroExtend 64 ≠ (0 : Word) := by
  obtain ⟨b, _, _, hbne, hbyte⟩ := long_list_len_byte_at txBytes listOff items henc hlong hoff1
  intro hz
  apply hbne
  apply BitVec.eq_of_toNat_eq
  have hz' : b.zeroExtend 64 = (0 : Word) := by rw [← hbyte]; exact hz
  have ht := congrArg BitVec.toNat hz'
  rw [toNat_zeroExtend_byte] at ht
  change b.toNat = (0 : Word).toNat at ht
  simpa using ht

/-- `h_min`: decoded payload Word ≥ 56. -/
theorem h_min_of_long_list
    (txBytes : List (BitVec 8)) (listOff : Nat) (items : List RLPItem)
    (henc : txBytes.drop listOff = encode (.list items))
    (hlong : 55 < (encode.encodeItems items).length)
    (hoff : listOff < txBytes.length)
    (hbound : (encode.encodeItems items).length < 2 ^ 64) :
    ¬ BitVec.ult
        (BitVec.ofNat 64 (Nat.fromBytesBE
          ((txBytes.drop (listOff + 1)).take
            ((txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word)).toNat)))
        (56 : Word) = true := by
  have hbound8 : (encode.encodeItems items).length < 256 ^ 8 := by
    rw [pow256_8_eq_pow2_64]; exact hbound
  have hpfx := long_list_pfx_at txBytes listOff items henc hlong hoff
  have hlolEq : ((txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word)).toNat =
      longListLol items := by
    rw [hpfx]; exact pfx_sub_F7_eq_lol items hlong hbound8
  have htake := long_list_tx_take_lol txBytes listOff items henc hlong
  have hdec : Nat.fromBytesBE ((txBytes.drop (listOff + 1)).take
      ((txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word)).toNat) =
      (encode.encodeItems items).length := by
    rw [hlolEq, htake, long_list_fromBytesBE_payload]
  rw [hdec]
  simp only [BitVec.ult, decide_eq_true_eq, not_lt, BitVec.toNat_ofNat]
  have h56 : (56 : Word).toNat = 56 := by decide
  rw [h56, Nat.mod_eq_of_lt hbound]
  exact Nat.succ_le_of_lt hlong

private theorem toNat_add_word_of_lt (a b : Word)
    (h : a.toNat + b.toNat < 2 ^ 64) :
    (a + b).toNat = a.toNat + b.toNat := by
  rw [BitVec.toNat_add, Nat.mod_eq_of_lt h]

private theorem lol_plus_one_toNat (items : List RLPItem)
    (hlong : 55 < (encode.encodeItems items).length)
    (hbound : (encode.encodeItems items).length < 256 ^ 8)
    (pfx : Word)
    (hlolW : pfx = BitVec.ofNat 64 (longListLol items)) :
    (pfx + (1 : Word)).toNat = longListLol items + 1 := by
  have h1 := longListLol_pos items hlong
  have h8 := longListLol_le_8 items hbound
  rw [hlolW]
  have hsum : (BitVec.ofNat 64 (longListLol items)).toNat + (1 : Word).toNat < 2 ^ 64 := by
    rw [BitVec.toNat_ofNat, word_one_toNat, Nat.mod_eq_of_lt (by omega)]
    omega
  rw [toNat_add_word_of_lt _ _ hsum, BitVec.toNat_ofNat, word_one_toNat,
    Nat.mod_eq_of_lt (by omega)]

/-- `h_fits` under non-wrapping ptr/end bounds. -/
theorem h_fits_of_long_list
    (txBase listLen : Word) (txBytes : List (BitVec 8)) (listOff : Nat)
    (items : List RLPItem)
    (henc : txBytes.drop listOff = encode (.list items))
    (hlong : 55 < (encode.encodeItems items).length)
    (hoff : listOff < txBytes.length)
    (hbound : (encode.encodeItems items).length < 256 ^ 8)
    (hlenNat : listLen.toNat = (txBytes.drop listOff).length)
    (hptr : (txBase + BitVec.ofNat 64 listOff).toNat = txBase.toNat + listOff)
    (hend : (txBase + BitVec.ofNat 64 listOff).toNat + listLen.toNat < 2 ^ 64) :
  ...
  have hlolEq : ((txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word)).toNat =
      longListLol items := by
    rw [hpfx]; exact pfx_sub_F7_eq_lol items hlong hbound
  have hpay : listLen.toNat = 1 + longListLol items + longListPayloadLen items := by
    rw [hlenNat, henc, encode_list_long_length items hlong]
  have h1 := longListLol_pos items hlong
  have h8 := longListLol_le_8 items hbound
  have hse := signExtend12_one
  have hlolW : (txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word) =
      BitVec.ofNat 64 (longListLol items) := by
    apply BitVec.eq_of_toNat_eq
    rw [hlolEq, BitVec.toNat_ofNat, Nat.mod_eq_of_lt (by omega)]
  set ptr := txBase + BitVec.ofNat 64 listOff
  have hendN : (ptr + listLen).toNat = ptr.toNat + listLen.toNat :=
    toNat_add_word_of_lt ptr listLen hend
  have hcurOff :
      (((txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word)) +
          signExtend12 (1 : BitVec 12)).toNat =
        longListLol items + 1 := by
    rw [hlolW, hse]
    exact lol_plus_one_toNat items hlong hbound _ rfl
  have hcurLt : ptr.toNat + longListLol items + 1 < 2 ^ 64 := by
    have := hend; rw [hpay] at this; omega
  have hcurN :
      (ptr + (((txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word)) +
          signExtend12 (1 : BitVec 12))).toNat =
        ptr.toNat + longListLol items + 1 := by
    have hsum : ptr.toNat +
        (((txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word)) +
          signExtend12 (1 : BitVec 12)).toNat < 2 ^ 64 := by
      rw [hcurOff]; exact hcurLt
    rw [toNat_add_word_of_lt ptr _ hsum, hcurOff]
    omega
  simp only [BitVec.ult, decide_eq_true_eq, not_lt]
  rw [hendN, hcurN, hpay]
  omega

/-- `h_match` under same bounds as h_fits. -/
theorem h_match_of_long_list
    (txBase listLen : Word) (txBytes : List (BitVec 8)) (listOff : Nat)
    (items : List RLPItem)
    (henc : txBytes.drop listOff = encode (.list items))
    (hlong : 55 < (encode.encodeItems items).length)
  ...
  have hlolW : (txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word) =
    ...
    rw [hlolEq, BitVec.toNat_ofNat, Nat.mod_eq_of_lt (by omega)]
  have htake := long_list_tx_take_lol txBytes listOff items henc hlong
  have hdecN : Nat.fromBytesBE ((txBytes.drop (listOff + 1)).take
      ((txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word)).toNat) =
      longListPayloadLen items := by
    rw [hlolEq, htake, longListPayloadLen, long_list_fromBytesBE_payload]
  set ptr := txBase + BitVec.ofNat 64 listOff
  apply BitVec.eq_of_toNat_eq
  have hL : (ptr + listLen).toNat = ptr.toNat + listLen.toNat :=
    toNat_add_word_of_lt ptr listLen hend
  have hcurOff :
      (((txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word)) +
          signExtend12 (1 : BitVec 12)).toNat =
  ...
  have hcur :
    ...
    rw [toNat_add_word_of_lt ptr _ hsum, hcurOff]
    omega
  have hpayLt : longListPayloadLen items < 2 ^ 64 := by
    simp only [longListPayloadLen]
    have : (encode.encodeItems items).length < 256 ^ 8 := hbound
    rw [pow256_8_eq_pow2_64] at this; exact this
  have hdecW :
      (BitVec.ofNat 64 (Nat.fromBytesBE ((txBytes.drop (listOff + 1)).take
          ((txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word)).toNat))).toNat =
        longListPayloadLen items := by
    rw [hdecN, BitVec.toNat_ofNat, Nat.mod_eq_of_lt hpayLt]
  have hR :
      ((ptr + (((txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word)) +
          signExtend12 (1 : BitVec 12))) +
        BitVec.ofNat 64 (Nat.fromBytesBE ((txBytes.drop (listOff + 1)).take
          ((txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word)).toNat))).toNat =
        ptr.toNat + longListLol items + 1 + longListPayloadLen items := by
    have hsum :
        (ptr + (((txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word)) +
            signExtend12 (1 : BitVec 12))).toNat +
          (BitVec.ofNat 64 (Nat.fromBytesBE ((txBytes.drop (listOff + 1)).take
      ...
      rw [hcur, hdecW]
      have := hend; rw [hpay] at this; omega
    rw [toNat_add_word_of_lt _ _ hsum, hcur, hdecW]
  rw [hR, hL, hpay]
  omega
...
#print axioms h_llz_of_long_list
...
/-- Single-byte field under long outer list. -/
theorem hnext_single_long_matches_srcOff_succ
    (txBytes : List (BitVec 8)) (txBase : Word) (listOff : Nat)
    (items : List RLPItem) (n : Nat) (hn : n < items.length)
    (b : Byte) (hb : b.toNat < 0x80)
    (hitem : items[n]'hn = .bytes [b])
  ...
  have hdrop' : txBytes.drop srcOff = b :: encode.encodeItems (items.drop (n + 1)) := by
    simpa [srcOff, longListSrcOff, hitem, henc1] using hdrop
  obtain ⟨hoff, hbp⟩ := getElem_of_drop_cons txBytes srcOff b _ hdrop'
  have hover' : txBase.toNat + srcOff < 2 ^ 64 := by simpa [srcOff] using hover
  have hsucc : srcOff' = srcOff + 1 := by
    have h := longListSrcOff_succ listOff items n hn
    simpa [srcOff, srcOff', hitem, hlen1] using h
  have hspan1 : txBase.toNat + srcOff + 1 < 2 ^ 64 := by
    have hsp : txBase.toNat + srcOff' < 2 ^ 64 := by simpa [srcOff'] using hspan
    rwa [hsucc] at hsp
...
/-- Short-string `.bytes` field under long outer list. -/
theorem hnext_short_string_long_matches_srcOff_succ
    (txBytes : List (BitVec 8)) (txBase : Word) (listOff : Nat)
    (items : List RLPItem) (n : Nat) (hn : n < items.length)
    (data : List Byte)
    (hitem : items[n]'hn = .bytes data)
  ...
  have hdrop' :
    ...
    have : encode (items[n]'hn) = [BitVec.ofNat 8 (0x80 + data.length)] ++ data := by
      rw [hitem, hencI]
    simpa [srcOff, longListSrcOff, this, List.cons_append] using hdrop
  obtain ⟨hoff, hb⟩ :=
    getElem_of_drop_cons txBytes srcOff (BitVec.ofNat 8 (0x80 + data.length)) _ hdrop'
  have hover' : txBase.toNat + srcOff < 2 ^ 64 := by simpa [srcOff] using hover
  have hp : (BitVec.ofNat 8 (0x80 + data.length)).toNat = 0x80 + data.length := by
  ...
  have hlenc : (encode (items[n]'hn)).length = 1 + data.length := by
    rw [hitem, encode_bytes_short_string_length data hlen hnotSingle]
  have hsrc' : srcOff' = srcOff + (1 + data.length) := by
    have hsucc := longListSrcOff_succ listOff items n hn
    simpa [srcOff, srcOff', hlenc] using hsucc
  have hspan1 : txBase.toNat + srcOff + 1 + len.toNat < 2 ^ 64 := by
    have hsp : txBase.toNat + srcOff' < 2 ^ 64 := by simpa [srcOff'] using hspan
    rw [hsrc'] at hsp
...
/-- Unified `.bytes` packaging hnext under long outer list.
    Requires the field itself short-encodes (≤55); outer list is long. -/
theorem hnext_bytes_long_matches_srcOff_succ
    (txBytes : List (BitVec 8)) (txBase : Word) (listOff : Nat)
    (items : List RLPItem) (n : Nat) (hn : n < items.length)
    (data : List Byte)
    (hitem : items[n]'hn = .bytes data)
    (henc : txBytes.drop listOff = encode (.list items))
    (hlong : 55 < (encode.encodeItems items).length)
    (hitemLe : (encode (.bytes data)).length ≤ 55)
    (hover : txBase.toNat + longListSrcOff listOff items n < 2 ^ 64)
    (hspan : txBase.toNat + longListSrcOff listOff items (n + 1) < 2 ^ 64) :
    let srcOff := longListSrcOff listOff items n
    let srcOff' := longListSrcOff listOff items (n + 1)
  ...
  · obtain ⟨b, hdata, hb⟩ := hsingle
    subst hdata
    exact hnext_single_long_matches_srcOff_succ txBytes txBase listOff items n hn b hb
      hitem henc hlong hover hspan endPtr next len hdec
  · have hlenD := bytes_data_length_le_55_of_encode_le data hitemLe hsingle
    exact hnext_short_string_long_matches_srcOff_succ txBytes txBase listOff items n hn
      data hitem hlenD hsingle henc hlong hover hspan endPtr next len hdec

/-- Nested short-list field under long outer list (item encode ≤55). -/
theorem hnext_short_list_long_matches_srcOff_succ
    (txBytes : List (BitVec 8)) (txBase : Word) (listOff : Nat)
    (items : List RLPItem) (n : Nat) (hn : n < items.length)
    (sub : List RLPItem)
    (hitem : items[n]'hn = .list sub)
    (henc : txBytes.drop listOff = encode (.list items))
    (hlong : 55 < (encode.encodeItems items).length)
    (hitemLe : (encode (.list sub)).length ≤ 55)
    (hover : txBase.toNat + longListSrcOff listOff items n < 2 ^ 64)
    (hspan : txBase.toNat + longListSrcOff listOff items (n + 1) < 2 ^ 64) :
    let srcOff := longListSrcOff listOff items n
    let srcOff' := longListSrcOff listOff items (n + 1)
    ∀ (endPtr next len : Word),
      ...
      next = txBase + BitVec.ofNat 64 srcOff' := by
  intro srcOff srcOff' endPtr next len hdec
  obtain ⟨hpayLe, hencForm⟩ := encode_list_of_encode_le_55 sub hitemLe
  set pay := (encode.encodeItems sub).length with hpay_def
  have hpayLe' : pay ≤ 55 := by simpa [hpay_def] using hpayLe
  have hdrop := long_list_item_drop txBytes listOff items n henc hlong hn
  have hdrop' :
...
/-- Unified packaging hnext under long outer list; each item short-encodes (≤55). -/
theorem hnext_item_long_matches_srcOff_succ
    (txBytes : List (BitVec 8)) (txBase : Word) (listOff : Nat)
    (items : List RLPItem) (n : Nat) (hn : n < items.length)
    (henc : txBytes.drop listOff = encode (.list items))
    (hlong : 55 < (encode.encodeItems items).length)
  ...
  match hitem : items[n]'hn with
  | .bytes data =>
    have hitemLe' : (encode (.bytes data)).length ≤ 55 := by simpa [hitem] using hitemLe
    exact hnext_bytes_long_matches_srcOff_succ txBytes txBase listOff items n hn data
      hitem henc hlong hitemLe' hover hspan endPtr next len hdec
  | .list sub =>
    have hitemLe' : (encode (.list sub)).length ≤ 55 := by simpa [hitem] using hitemLe
    exact hnext_short_list_long_matches_srcOff_succ txBytes txBase listOff items n hn sub
      hitem henc hlong hitemLe' hover hspan endPtr next len hdec

/-- Packaging form: decode-gated hnext at `longListSrcOff k` → `longListSrcOff (k+1)`. -/
theorem packaging_hnext_longListSrcOff
    (txBytes : List (BitVec 8)) (txBase : Word) (listOff : Nat)
    (items : List RLPItem) (k : Nat) (hk : k < items.length)
    (henc : txBytes.drop listOff = encode (.list items))
    (hlong : 55 < (encode.encodeItems items).length)
    (hitemLe : (encode (items[k]'hk)).length ≤ 55)
    (hover : txBase.toNat + longListSrcOff listOff items k < 2 ^ 64)
    (hspan : txBase.toNat + longListSrcOff listOff items (k + 1) < 2 ^ 64) :
    ∀ (endPtr next len : Word),
      rlpItemDecode txBytes (longListSrcOff listOff items k)
        (txBase + BitVec.ofNat 64 (longListSrcOff listOff items k))
        endPtr next len →
      next = txBase + BitVec.ofNat 64 (longListSrcOff listOff items (k + 1)) := by
  intro endPtr next len hdec
  exact hnext_item_long_matches_srcOff_succ txBytes txBase listOff items k hk
    henc hlong hitemLe hover hspan endPtr next len hdec

/-- Creation type234 long: items has length ≥ 6. -/
theorem extractSuccess_creation_type234_items_length_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (hge : 2 ≤ (teerTxTypeDispatch txBytes).2.1.toNat)
...
/-- Creation type234 long: packaging hnext1..5 + hcre with longListSrcOff.
    Requires fields 0..4 short-encode (≤55); field5 empty is length 1. -/
theorem extractSuccess_creation_type234_hnext_hcre_srcOff_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (hge : 2 ≤ (teerTxTypeDispatch txBytes).2.1.toNat)
    ...
    (hitem0 : (encode (items[0]'(by
        have := extractSuccess_creation_type234_items_length_long txBytes h hcreFlag hge
          items hdecL hlong; omega))).length ≤ 55)
    ...
    (hitem4 : (encode (items[4]'(by
        have := extractSuccess_creation_type234_items_length_long txBytes h hcreFlag hge
          items hdecL hlong; omega))).length ≤ 55)
    (hover0 : txBase.toNat +
        longListSrcOff (teerTxTypeDispatch txBytes).2.2.toNat items 0 < 2 ^ 64)
    (hover1 : txBase.toNat +
        longListSrcOff (teerTxTypeDispatch txBytes).2.2.toNat items 1 < 2 ^ 64)
    (hover2 : txBase.toNat +
        longListSrcOff (teerTxTypeDispatch txBytes).2.2.toNat items 2 < 2 ^ 64)
    (hover3 : txBase.toNat +
        longListSrcOff (teerTxTypeDispatch txBytes).2.2.toNat items 3 < 2 ^ 64)
    (hover4 : txBase.toNat +
        longListSrcOff (teerTxTypeDispatch txBytes).2.2.toNat items 4 < 2 ^ 64)
    (hspan1 : txBase.toNat +
        longListSrcOff (teerTxTypeDispatch txBytes).2.2.toNat items 1 < 2 ^ 64)
    (hspan2 : txBase.toNat +
        longListSrcOff (teerTxTypeDispatch txBytes).2.2.toNat items 2 < 2 ^ 64)
    ...
    (hspan4 : txBase.toNat +
        longListSrcOff (teerTxTypeDispatch txBytes).2.2.toNat items 4 < 2 ^ 64)
    (hspan5 : txBase.toNat +
        longListSrcOff (teerTxTypeDispatch txBytes).2.2.toNat items 5 < 2 ^ 64) :
    let listOff := (teerTxTypeDispatch txBytes).2.2.toNat
    (∀ (endPtr next len : Word),
      rlpItemDecode txBytes (longListSrcOff listOff items 0)
        (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 0))
        endPtr next len →
      next = txBase + BitVec.ofNat 64 (longListSrcOff listOff items 1)) ∧
    (∀ (endPtr next len : Word),
      rlpItemDecode txBytes (longListSrcOff listOff items 1)
        (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 1))
        endPtr next len →
      next = txBase + BitVec.ofNat 64 (longListSrcOff listOff items 2)) ∧
    (∀ (endPtr next len : Word),
      rlpItemDecode txBytes (longListSrcOff listOff items 2)
        (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 2))
        endPtr next len →
      next = txBase + BitVec.ofNat 64 (longListSrcOff listOff items 3)) ∧
    (∀ (endPtr next len : Word),
      rlpItemDecode txBytes (longListSrcOff listOff items 3)
        (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 3))
        endPtr next len →
      next = txBase + BitVec.ofNat 64 (longListSrcOff listOff items 4)) ∧
    (∀ (endPtr next len : Word),
      rlpItemDecode txBytes (longListSrcOff listOff items 4)
        (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 4))
        endPtr next len →
      next = txBase + BitVec.ofNat 64 (longListSrcOff listOff items 5)) ∧
    (∀ (endPtr next5 len5 : Word),
      rlpItemDecode txBytes (longListSrcOff listOff items 5)
        (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 5))
        endPtr next5 len5 → len5 = (0 : Word)) := by
  intro listOff
  have hencFull := decodeListItems_eq_encode _ _ hdecL
  ...
  have hn4 : (4 : Nat) < items.length := by omega
  refine ⟨?_, ?_, ?_, ?_, ?_, ?_⟩
  · exact packaging_hnext_longListSrcOff txBytes txBase listOff items 0 hn0
      hencFull hlong hitem0 hover0 hspan1
  · exact packaging_hnext_longListSrcOff txBytes txBase listOff items 1 hn1
      hencFull hlong hitem1 hover1 hspan2
  · exact packaging_hnext_longListSrcOff txBytes txBase listOff items 2 hn2
      hencFull hlong hitem2 hover2 hspan3
  · exact packaging_hnext_longListSrcOff txBytes txBase listOff items 3 hn3
      hencFull hlong hitem3 hover3 hspan4
  · exact packaging_hnext_longListSrcOff txBytes txBase listOff items 4 hn4
      hencFull hlong hitem4 hover4 hspan5
  · exact extractSuccess_creation_type234_hcre_long txBytes txBase h hcreFlag hge
      items hdecL hlong

#print axioms hnext_single_long_matches_srcOff_succ
#print axioms hnext_short_string_long_matches_srcOff_succ
#print axioms hnext_bytes_long_matches_srcOff_succ
#print axioms hnext_short_list_long_matches_srcOff_succ
#print axioms hnext_item_long_matches_srcOff_succ
...
/-! ## Long-list hdec + hvalid packaging helpers

    Mirror short-list remaining/gap/hdec/succ_room/hvalid with `longListSrcOff`
    and per-item short-encode path hyp (`hitemLe`).
-/

/-- Remaining bytes at long-list item start. -/
theorem long_list_remaining_at
    (txBytes : List (BitVec 8)) (listOff : Nat) (items : List RLPItem) (k : Nat)
    (henc : txBytes.drop listOff = encode (.list items))
    (hlong : 55 < (encode.encodeItems items).length)
    (hn : k < items.length) :
    (txBytes.drop (longListSrcOff listOff items k)).length =
      (encode (items[k]'hn)).length +
        (encode.encodeItems (items.drop (k + 1))).length := by
  have hdrop := long_list_item_drop txBytes listOff items k henc hlong hn
  have hlen := congrArg List.length hdrop
  simpa [longListSrcOff, List.length_append] using hlen

/-- Cursor→end gap at long-list end equals remaining payload from item start. -/
theorem long_list_end_gap
    (txBase : Word) (listOff : Nat) (items : List RLPItem) (k : Nat)
    (hn : k < items.length)
    (hover : txBase.toNat +
      (listOff + 1 + longListLol items + (encode.encodeItems items).length) < 2 ^ 64)
    (endPtr : Word)
    (hend : endPtr = longListEndPtr txBase listOff items) :
    let cursor := txBase + BitVec.ofNat 64 (longListSrcOff listOff items k)
    (endPtr - cursor).toNat =
      (encode.encodeItems items).length - encodeItemsPrefixLen items k := by
  intro cursor
  have hsrc : longListSrcOff listOff items k =
  ...
  have hover' : txBase.toNat + longListSrcOff listOff items k < 2 ^ 64 := by
    simp only [hsrc]; omega
  have hcurN : cursor.toNat = txBase.toNat + longListSrcOff listOff items k :=
    toNat_add_ofNat_lt txBase _ hover'
  have hendN : endPtr.toNat =
      txBase.toNat +
        (listOff + 1 + longListLol items + (encode.encodeItems items).length) := by
    rw [hend, longListEndPtr, toNat_add_ofNat_lt txBase _ hover]
  have hle : cursor.toNat ≤ endPtr.toNat := by
    rw [hcurN, hendN, hsrc]; omega
  rw [BitVec.toNat_sub_of_le hle, hcurN, hendN, hsrc]
  omega

/-- Unified: any long-list item with short encode admits `rlpItemDecode` at longListSrcOff. -/
theorem hdec_long_list_item
    (txBytes : List (BitVec 8)) (txBase : Word) (listOff : Nat)
    (items : List RLPItem) (k : Nat)
    (henc : txBytes.drop listOff = encode (.list items))
    (hlong : 55 < (encode.encodeItems items).length)
    ...
    (hoff : longListSrcOff listOff items k < txBytes.length)
    (hover : txBase.toNat +
      (listOff + 1 + longListLol items + (encode.encodeItems items).length) < 2 ^ 64)
    (endPtr : Word)
    (hend : endPtr = longListEndPtr txBase listOff items) :
    ∃ n l : Word,
      rlpItemDecode txBytes (longListSrcOff listOff items k)
        (txBase + BitVec.ofNat 64 (longListSrcOff listOff items k)) endPtr n l := by
  set srcOff := longListSrcOff listOff items k with hsrc
  set cursor := txBase + BitVec.ofNat 64 srcOff with hcur
  have hitemLen : (encode (items[k]'hn)).length ≤ 55 := hitemLe
  have hdrop := long_list_item_drop txBytes listOff items k henc hlong hn
  have hrem := long_list_remaining_at txBytes listOff items k henc hlong hn
  have hgap := long_list_end_gap txBase listOff items k hn hover endPtr hend
  have hgap' : (endPtr - cursor).toNat =
      (encode (items[k]'hn)).length +
        (encode.encodeItems (items.drop (k + 1))).length := by
    have hpref := encodeItems_drop_at items k hn
    have hlen := congrArg List.length hpref
    simp only [List.length_drop, List.length_append] at hlen
    have hsrc' : srcOff = listOff + 1 + longListLol items + encodeItemsPrefixLen items k := by
      simp only [srcOff, longListSrcOff]
    have : (encode.encodeItems items).length - encodeItemsPrefixLen items k =
        (encode (items[k]'hn)).length +
          (encode.encodeItems (items.drop (k + 1))).length := by
      have hge : encodeItemsPrefixLen items k ≤ (encode.encodeItems items).length := by
  ...
  | bytes data =>
    ...
    · obtain ⟨b, hbdata, hblt⟩ := hsingle
      ...
      have hdrop' : txBytes.drop srcOff = b :: encode.encodeItems (items.drop (k + 1)) := by
        have : encode (items[k]'hn) = [b] := hencI
        simpa [srcOff, longListSrcOff, this, List.cons_append, List.nil_append] using hdrop
      obtain ⟨hoff', hb⟩ := getElem_of_drop_cons txBytes srcOff b _ hdrop'
      have hinb := hinb_long_list_end txBase listOff items k hn hover endPtr hend
      refine ⟨_, _, rlpItemDecode_single_byte txBytes srcOff cursor endPtr hoff' ?_ ?_⟩
      · rw [hb]; exact hblt
      · simpa [cursor, srcOff] using hinb
    · have hdataLe : data.length ≤ 55 :=
...
/-- Packaging: `hdec` at longWalkEnd for field `k` under long list. -/
theorem hdec_long_list_end
    (txBytes : List (BitVec 8)) (txBase : Word) (listOff : Nat)
    (items : List RLPItem) (k : Nat)
    (henc : txBytes.drop listOff = encode (.list items))
    (hlong : 55 < (encode.encodeItems items).length)
    ...
    ∃ n l : Word,
      rlpItemDecode txBytes (longListSrcOff listOff items k)
        (txBase + BitVec.ofNat 64 (longListSrcOff listOff items k)) endPtr n l :=
  hdec_long_list_item txBytes txBase listOff items k henc hlong hn hitemLe hoff hover endPtr hend

/-- Content offset of field `n` is ≤ start of field `n+1` by ≥1. -/
theorem longListSrcOff_add_one_le_succ
    (listOff : Nat) (items : List RLPItem) (n : Nat)
    (hn : n < items.length) :
    longListSrcOff listOff items n + 1 ≤ longListSrcOff listOff items (n + 1) := by
  have hpos : 0 < (encode (items[n]'hn)).length := encode_item_length_pos _
  have hsucc := longListSrcOff_succ listOff items n hn
  omega

/-- `srcOff_n + 1 < length` when a following field exists. -/
theorem longListSrcOff_succ_room
    (bs : List Byte) (listOff : Nat) (items : List RLPItem) (n : Nat)
    (henc : bs.drop listOff = encode (.list items))
    (hlong : 55 < (encode.encodeItems items).length)
    (hnxt : n + 1 < items.length) :
    longListSrcOff listOff items n + 1 < bs.length := by
  have hn : n < items.length := Nat.lt_of_succ_lt hnxt
  have hle := longListSrcOff_add_one_le_succ listOff items n hn
...
theorem extractSuccess_creation_type234_hvalid_srcOff_long
    ...
    (hvalidBuf : validByteRange txBase txBytes.length) :
    let listOff := (teerTxTypeDispatch txBytes).2.2.toNat
    isValidByteAccess (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 0)) = true ∧
    isValidByteAccess (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 0 + 1)) = true ∧
    isValidByteAccess (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 1)) = true ∧
    isValidByteAccess (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 1 + 1)) = true ∧
    isValidByteAccess (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 2)) = true ∧
    isValidByteAccess (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 2 + 1)) = true ∧
    isValidByteAccess (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 3)) = true ∧
    isValidByteAccess (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 3 + 1)) = true ∧
    isValidByteAccess (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 4)) = true ∧
    isValidByteAccess (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 4 + 1)) = true ∧
    isValidByteAccess (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 5)) = true ∧
    isValidByteAccess (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 5 + 1)) = true := by
  let listOff := (teerTxTypeDispatch txBytes).2.2.toNat
  have hencInner :
      txBytes.drop listOff = encode (.list items) :=
    decodeListItems_eq_encode _ _ hdecL
  have hlen :=
    extractSuccess_creation_type234_items_length_long txBytes h hcreFlag hge
      items hdecL hlong
  have hn0 : (0 : Nat) < items.length := by omega
  have hn1 : (1 : Nat) < items.length := by omega
  have hn2 : (2 : Nat) < items.length := by omega
  ...
  have hn5 : (5 : Nat) < items.length := by omega
  have hn6 : (6 : Nat) < items.length := by omega
  have h0 := longListSrcOff_lt_length txBytes listOff items 0 hencInner hlong hn0
  have h1 := longListSrcOff_lt_length txBytes listOff items 1 hencInner hlong hn1
  have h2 := longListSrcOff_lt_length txBytes listOff items 2 hencInner hlong hn2
  have h3 := longListSrcOff_lt_length txBytes listOff items 3 hencInner hlong hn3
  have h4 := longListSrcOff_lt_length txBytes listOff items 4 hencInner hlong hn4
  have h5 := longListSrcOff_lt_length txBytes listOff items 5 hencInner hlong hn5
  have h0s := longListSrcOff_succ_room txBytes listOff items 0 hencInner hlong (by omega)
  have h1s := longListSrcOff_succ_room txBytes listOff items 1 hencInner hlong (by omega)
  have h2s := longListSrcOff_succ_room txBytes listOff items 2 hencInner hlong (by omega)
  have h3s := longListSrcOff_succ_room txBytes listOff items 3 hencInner hlong (by omega)
  have h4s := longListSrcOff_succ_room txBytes listOff items 4 hencInner hlong (by omega)
  have h5s := longListSrcOff_succ_room txBytes listOff items 5 hencInner hlong hn6
  exact ⟨
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf h0,
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf h0s,
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf h1,
...
/-- `hlover` + `hlvalid` for long walk_init length-field scan from buffer validity. -/
theorem extractSuccess_long_hlover_hlvalid
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (items : List RLPItem)
    (hdecL : decodeListItems (txBytes.drop (teerTxTypeDispatch txBytes).2.2.toNat) =
    ...
    (hvalidBuf : validByteRange txBase txBytes.length) :
    let listOff := (teerTxTypeDispatch txBytes).2.2.toNat
    txBase.toNat + (listOff + 1 + longListLol items) ≤ 2 ^ 64 ∧
      (∀ k, k < longListLol items →
        isValidByteAccess (txBase + BitVec.ofNat 64 (listOff + 1 + k)) = true) := by
  intro listOff
  have hencInner :
      txBytes.drop listOff = encode (.list items) :=
    decodeListItems_eq_encode _ _ hdecL
  have hoffInner : listOff < txBytes.length := extractSuccess_inner_lt txBytes h
  have hencLen := encode_list_long_length items hlong
  have hdropLen : (txBytes.drop listOff).length = (encode (.list items)).length := by
    rw [hencInner]
  have hdropNat : (txBytes.drop listOff).length = txBytes.length - listOff := by
    simp only [List.length_drop]
  have hsum : listOff + (encode (.list items)).length = txBytes.length := by
    have hle : listOff ≤ txBytes.length := Nat.le_of_lt hoffInner
    omega
  -- encode long = 1 + lol + payload
  have hstruct : (encode (.list items)).length =
      1 + longListLol items + (encode.encodeItems items).length := hencLen
  have hroom : listOff + 1 + longListLol items ≤ txBytes.length := by
    omega
  refine ⟨?_, ?_⟩
  · omega
  · intro k hk
    have hkLt : listOff + 1 + k < txBytes.length := by omega
    exact isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf hkLt

#print axioms long_list_remaining_at
#print axioms long_list_end_gap
#print axioms hdec_long_list_item
#print axioms hdec_long_list_end
#print axioms longListSrcOff_succ_room
#print axioms extractSuccess_creation_type234_hvalid_srcOff_long
#print axioms extractSuccess_long_hlover_hlvalid

/-- 20-byte item at long-list placement has prefix `0x94`. -/
theorem long_list_bytes20_pfx
    (bs : List Byte) (listOff : Nat) (items : List RLPItem) (n : Nat)
    (henc : bs.drop listOff = encode (.list items))
    (hlong : 55 < (encode.encodeItems items).length)
    (hn : n < items.length)
...
/-- Copy + type234 + long list ⇒ field index 5 is 20B address at `0x94` offset. -/
theorem extractSuccess_copy_type234_field5_pfx94_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (hge : 2 ≤ (teerTxTypeDispatch txBytes).2.1.toNat)
  ...
  have hval : items[5]'hn = .bytes content := (List.getElem?_eq_some_iff.mp hget).2
  have hencFull := decodeListItems_eq_encode _ _ hdec
  have hpfx := long_list_bytes20_pfx txBytes listOff items 5 hencFull hlong hn
    content hval hlen20
  exact ⟨hn, ⟨content, hget, hlen20⟩, hpfx⟩

theorem extractSuccess_copy_type234_items_length_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (hge : 2 ≤ (teerTxTypeDispatch txBytes).2.1.toNat)
...
/-- Field5 encode length is 21 under copy type234 long (0x94 :: 20B). -/
theorem extractSuccess_copy_type234_field5_encode_le55_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (hge : 2 ≤ (teerTxTypeDispatch txBytes).2.1.toNat)
    ...
    (hdecL : decodeListItems (txBytes.drop (teerTxTypeDispatch txBytes).2.2.toNat) =
      some items)
    (hlong : 55 < (encode.encodeItems items).length) :
    let hn : (5 : Nat) < items.length := by
      have := extractSuccess_copy_type234_items_length_long txBytes h hcopyFlag hge
        items hdecL hlong; omega
    (encode (items[5]'hn)).length ≤ 55 := by
  intro hn
  have hf := extractSuccess_copy_type234_field5_pfx94_long txBytes h hcopyFlag hge
    items hdecL hlong
  obtain ⟨_hn', ⟨content, hget, hlen20⟩, _⟩ := hf
  have hval : items[5]'hn = .bytes content := by
    have : items[5]? = some (.bytes content) := hget
    exact (List.getElem?_eq_some_iff.mp this).2
  rw [hval, encode_bytes_len20_pfx content hlen20]
  simp only [List.length_cons, hlen20]
  omega

theorem extractSuccess_copy_type234_hlen20_long
    (txBytes : List (BitVec 8)) (txBase : Word) (srcOff5 : Nat)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (hge : 2 ≤ (teerTxTypeDispatch txBytes).2.1.toNat)
    ...
    (hdecL : decodeListItems (txBytes.drop (teerTxTypeDispatch txBytes).2.2.toNat) =
      some items)
    (hlong : 55 < (encode.encodeItems items).length)
    (hsrc : srcOff5 = longListSrcOff (teerTxTypeDispatch txBytes).2.2.toNat items 5) :
    ∀ (endPtr next5 len5 : Word),
      rlpItemDecode txBytes srcOff5 (txBase + BitVec.ofNat 64 srcOff5)
        endPtr next5 len5 → len5 = (20 : Word) := by
  have hf := extractSuccess_copy_type234_field5_pfx94_long txBytes h hcopyFlag hge
  ...
  obtain ⟨hoff, hb⟩ := hpfx
  intro endPtr next5 len5 hdec
  have hoff' : srcOff5 < txBytes.length := by simpa [hsrc] using hoff
  have hb' : txBytes[srcOff5]'hoff' = (0x94 : BitVec 8) := by simpa [hsrc] using hb
  exact hlen20_decode_of_pfx94 txBytes srcOff5
    (txBase + BitVec.ofNat 64 srcOff5) endPtr hoff' hb' next5 len5 hdec

theorem extractSuccess_copy_type234_hnext_content_long
    (txBytes : List (BitVec 8)) (txBase contentPtr : Word) (srcOff5 : Nat)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (hge : 2 ≤ (teerTxTypeDispatch txBytes).2.1.toNat)
...
/-- Copy type234 long: packaging hnext1..5 + hlen20 with longListSrcOff.
    Fields 0..4 need short-encode ≤55; field5 is 21B (0x94::20). -/
theorem extractSuccess_copy_type234_hnext_hlen20_srcOff_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (hge : 2 ≤ (teerTxTypeDispatch txBytes).2.1.toNat)
  ...
  intro listOff
  have hencFull := decodeListItems_eq_encode _ _ hdecL
  have hlen := extractSuccess_copy_type234_items_length_long txBytes h hcopyFlag hge
    items hdecL hlong
  have hn0 : (0 : Nat) < items.length := by omega
  have hn1 : (1 : Nat) < items.length := by omega
  have hn2 : (2 : Nat) < items.length := by omega
  ...
  · exact packaging_hnext_longListSrcOff txBytes txBase listOff items 4 hn4
      hencFull hlong hitem4 hover4 hspan5
  · exact extractSuccess_copy_type234_hlen20_long txBytes txBase
      (longListSrcOff listOff items 5) h hcopyFlag hge items hdecL hlong rfl

theorem extractSuccess_copy_type234_hoff_srcOff_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (hge : 2 ≤ (teerTxTypeDispatch txBytes).2.1.toNat)
...
theorem extractSuccess_copy_type234_hover_srcOff_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (hge : 2 ≤ (teerTxTypeDispatch txBytes).2.1.toNat)
    ...
    txBase.toNat + longListSrcOff listOff items 5 < 2 ^ 64 := by
  intro listOff
  have hoffs := extractSuccess_copy_type234_hoff_srcOff_long txBytes h hcopyFlag hge
    items hdecL hlong
  obtain ⟨h0, h1, h2, h3, h4, h5⟩ := hoffs
  exact ⟨hover_of_buffer_span txBase _ _ hover h0,
    hover_of_buffer_span txBase _ _ hover h1,
...
theorem extractSuccess_copy_type234_hnext_fields04_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (hge : 2 ≤ (teerTxTypeDispatch txBytes).2.1.toNat)
...
/-- Copy type234 long: all srcOff/srcOff+1 hvalid from `validByteRange`. Needs `7 ≤ items`. -/
theorem extractSuccess_copy_type234_hvalid_srcOff_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (hge : 2 ≤ (teerTxTypeDispatch txBytes).2.1.toNat)
  ...
  have hencInner :
    ...
    decodeListItems_eq_encode _ _ hdecL
  have hlen :=
    extractSuccess_copy_type234_items_length_long txBytes h hcopyFlag hge
      items hdecL hlong
  have hn0 : (0 : Nat) < items.length := by omega
  have hn1 : (1 : Nat) < items.length := by omega
  have hn2 : (2 : Nat) < items.length := by omega
...
#print axioms extractSuccess_copy_type234_hoff_srcOff_long
#print axioms extractSuccess_copy_type234_hover_srcOff_long
#print axioms extractSuccess_copy_type234_hnext_fields04_long
#print axioms extractSuccess_copy_type234_hvalid_srcOff_long



theorem extractSuccess_creation_legacy_field3_pfx80_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
    ...
    (hlong : 55 < (encode.encodeItems items).length) :
    let listOff := (teerTxTypeDispatch txBytes).2.2.toNat
    let off := longListSrcOff listOff items 3
    (3 : Nat) < items.length ∧
      items[3]? = some (.bytes []) ∧
      ∃ hoff : off < txBytes.length, txBytes[off]'hoff = (0x80 : BitVec 8) := by
  intro listOff off
  ...
  have hval : items[3]'hn = .bytes [] := (List.getElem?_eq_some_iff.mp hget).2
  have hencFull := decodeListItems_eq_encode _ _ hdec
  have hpfx := long_list_empty_bytes_pfx txBytes listOff items 3 hencFull hlong hn hval
  exact ⟨hn, hget, hpfx⟩

theorem extractSuccess_creation_legacy_items_length_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
    ...
    (hlong : 55 < (encode.encodeItems items).length) :
    4 ≤ items.length := by
  have hf := extractSuccess_creation_legacy_field3_pfx80_long txBytes h hcreFlag htype0
    items hdecL hlong
  omega

theorem extractSuccess_creation_legacy_hcre_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
    ...
    ∀ (endPtr next3 len3 : Word),
      rlpItemDecode txBytes srcOff3 (txBase + BitVec.ofNat 64 srcOff3)
        endPtr next3 len3 → len3 = (0 : Word) := by
  intro listOff srcOff3 endPtr next3 len3 hdec
  have hf := extractSuccess_creation_legacy_field3_pfx80_long txBytes h hcreFlag htype0
    items hdecL hlong
  obtain ⟨_hn, _hget, hpfx⟩ := hf
  obtain ⟨hoff, hb⟩ := hpfx
  have hoff' : srcOff3 < txBytes.length := by simpa [srcOff3, listOff] using hoff
  have hb' : txBytes[srcOff3]'hoff' = (0x80 : BitVec 8) := by
    simpa [srcOff3, listOff] using hb
  exact hcre_decode_of_pfx80 txBytes srcOff3
    (txBase + BitVec.ofNat 64 srcOff3) endPtr hoff' hb' next3 len3 hdec

theorem extractSuccess_creation_legacy_hcre_srcOff_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
    ...
    ∀ (endPtr next3 len3 : Word),
      rlpItemDecode txBytes (longListSrcOff listOff items 3)
        ...
        endPtr next3 len3 → len3 = (0 : Word) := by
  intro listOff
  exact extractSuccess_creation_legacy_hcre_long txBytes txBase h hcreFlag htype0
    items hdecL hlong

theorem extractSuccess_creation_legacy_hnext_hcre_srcOff_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
    ...
    (hlong : 55 < (encode.encodeItems items).length)
    (hitem0 : (encode (items[0]'(by
        have := extractSuccess_creation_legacy_items_length_long txBytes h hcreFlag htype0
          items hdecL hlong; omega))).length ≤ 55)
    (hitem1 : (encode (items[1]'(by
        have := extractSuccess_creation_legacy_items_length_long txBytes h hcreFlag htype0
          items hdecL hlong; omega))).length ≤ 55)
  ...
  intro listOff
  have hencFull := decodeListItems_eq_encode _ _ hdecL
  have hlen := extractSuccess_creation_legacy_items_length_long txBytes h hcreFlag htype0
    items hdecL hlong
  have hn0 : (0 : Nat) < items.length := by omega
  have hn1 : (1 : Nat) < items.length := by omega
  have hn2 : (2 : Nat) < items.length := by omega
...
theorem extractSuccess_creation_legacy_hoff_srcOff_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
    ...
    longListSrcOff listOff items 1 < txBytes.length ∧
    longListSrcOff listOff items 2 < txBytes.length ∧
    longListSrcOff listOff items 3 < txBytes.length := by
  intro listOff
  have hlen := extractSuccess_creation_legacy_items_length_long txBytes h hcreFlag htype0
    items hdecL hlong
  have henc := decodeListItems_eq_encode _ _ hdecL
  ...
  have hn3 : (3 : Nat) < items.length := by omega
  exact ⟨longListSrcOff_lt_length txBytes listOff items 0 henc hlong hn0,
    longListSrcOff_lt_length txBytes listOff items 1 henc hlong hn1,
    longListSrcOff_lt_length txBytes listOff items 2 henc hlong hn2,
    longListSrcOff_lt_length txBytes listOff items 3 henc hlong hn3⟩

theorem extractSuccess_creation_legacy_hover_srcOff_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
    ...
    txBase.toNat + longListSrcOff listOff items 1 < 2 ^ 64 ∧
    txBase.toNat + longListSrcOff listOff items 2 < 2 ^ 64 ∧
    txBase.toNat + longListSrcOff listOff items 3 < 2 ^ 64 := by
  intro listOff
  have hoffs := extractSuccess_creation_legacy_hoff_srcOff_long txBytes h hcreFlag htype0
    items hdecL hlong
  obtain ⟨h0, h1, h2, h3⟩ := hoffs
  exact ⟨hover_of_buffer_span txBase _ _ hover h0,
    hover_of_buffer_span txBase _ _ hover h1,
...
theorem extractSuccess_creation_legacy_hnext_fields02_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
    ...
    (hdecL : decodeListItems (txBytes.drop (teerTxTypeDispatch txBytes).2.2.toNat) =
      some items)
    (hlong : 55 < (encode.encodeItems items).length) :
    (0 + 1 < items.length) ∧ (1 + 1 < items.length) ∧ (2 + 1 < items.length) := by
  have hlen := extractSuccess_creation_legacy_items_length_long txBytes h hcreFlag htype0
    items hdecL hlong
  omega

/-- Pack hvalid/hvalid1 at longListSrcOff 0..3. Needs `5 ≤ items.length` for field3+1. -/
theorem extractSuccess_creation_legacy_hvalid_srcOff_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
    ...
    isValidByteAccess (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 2 + 1)) = true ∧
    isValidByteAccess (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 3)) = true ∧
    isValidByteAccess (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 3 + 1)) = true := by
  intro listOff
  have henc := decodeListItems_eq_encode _ _ hdecL
  have hlen := extractSuccess_creation_legacy_items_length_long txBytes h hcreFlag htype0
    items hdecL hlong
  ...
  have hn2 : (2 : Nat) < items.length := by omega
  have hn3 : (3 : Nat) < items.length := by omega
  have hlt0 := longListSrcOff_lt_length txBytes listOff items 0 henc hlong hn0
  have hlt1 := longListSrcOff_lt_length txBytes listOff items 1 henc hlong hn1
  have hlt2 := longListSrcOff_lt_length txBytes listOff items 2 henc hlong hn2
  have hlt3 := longListSrcOff_lt_length txBytes listOff items 3 henc hlong hn3
  have hr0 := longListSrcOff_succ_room txBytes listOff items 0 henc hlong (by omega)
  have hr1 := longListSrcOff_succ_room txBytes listOff items 1 henc hlong (by omega)
  have hr2 := longListSrcOff_succ_room txBytes listOff items 2 henc hlong (by omega)
  have hr3 := longListSrcOff_succ_room txBytes listOff items 3 henc hlong (by omega)
  exact ⟨isValidByteAccess_of_validByteRange txBase txBytes.length
      (longListSrcOff listOff items 0) hvalidBuf hlt0,
    isValidByteAccess_of_validByteRange txBase txBytes.length
      (longListSrcOff listOff items 0 + 1) hvalidBuf hr0,
    isValidByteAccess_of_validByteRange txBase txBytes.length
      (longListSrcOff listOff items 1) hvalidBuf hlt1,
    isValidByteAccess_of_validByteRange txBase txBytes.length
      (longListSrcOff listOff items 1 + 1) hvalidBuf hr1,
    isValidByteAccess_of_validByteRange txBase txBytes.length
      (longListSrcOff listOff items 2) hvalidBuf hlt2,
    isValidByteAccess_of_validByteRange txBase txBytes.length
      (longListSrcOff listOff items 2 + 1) hvalidBuf hr2,
    isValidByteAccess_of_validByteRange txBase txBytes.length
      (longListSrcOff listOff items 3) hvalidBuf hlt3,
    isValidByteAccess_of_validByteRange txBase txBytes.length
      (longListSrcOff listOff items 3 + 1) hvalidBuf hr3⟩

#print axioms extractSuccess_creation_legacy_field3_pfx80_long
#print axioms extractSuccess_creation_legacy_hnext_hcre_srcOff_long
#print axioms extractSuccess_creation_legacy_hvalid_srcOff_long




/-- Long-list copy legacy: field3 is 20B address with prefix 0x94. -/
theorem extractSuccess_copy_legacy_field3_pfx94_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
  ...
  have hval : items[3]'hn = .bytes content := (List.getElem?_eq_some_iff.mp hget).2
  have hencFull := decodeListItems_eq_encode _ _ hdec
  have hpfx := long_list_bytes20_pfx txBytes listOff items 3 hencFull hlong hn
    content hval hlen20
  exact ⟨hn, ⟨content, hget, hlen20⟩, hpfx⟩

theorem extractSuccess_copy_legacy_items_length_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
  ...
  have hf := extractSuccess_copy_legacy_field3_pfx94_long txBytes h hcopyFlag htype0
    items hdecL hlong
  exact Nat.succ_le_of_lt hf.1

theorem extractSuccess_copy_legacy_hlen20_long
    (txBytes : List (BitVec 8)) (txBase : Word) (srcOff3 : Nat)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
    ...
    (hdecL : decodeListItems (txBytes.drop (teerTxTypeDispatch txBytes).2.2.toNat) =
      some items)
    (hlong : 55 < (encode.encodeItems items).length)
    (hsrc : srcOff3 = longListSrcOff (teerTxTypeDispatch txBytes).2.2.toNat items 3) :
    ∀ (endPtr next3 len3 : Word),
      rlpItemDecode txBytes srcOff3 (txBase + BitVec.ofNat 64 srcOff3)
        endPtr next3 len3 → len3 = (20 : Word) := by
  have hf := extractSuccess_copy_legacy_field3_pfx94_long txBytes h hcopyFlag htype0
...
theorem extractSuccess_copy_legacy_hnext_content_long
    (txBytes : List (BitVec 8)) (txBase contentPtr : Word) (srcOff3 : Nat)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
...
theorem extractSuccess_copy_legacy_hnext_hlen20_srcOff_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
    ...
    (hlong : 55 < (encode.encodeItems items).length)
    (hitem0 : (encode (items[0]'(by
        have := extractSuccess_copy_legacy_items_length_long txBytes h hcopyFlag htype0
          items hdecL hlong; omega))).length ≤ 55)
    (hitem1 : (encode (items[1]'(by
        have := extractSuccess_copy_legacy_items_length_long txBytes h hcopyFlag htype0
          items hdecL hlong; omega))).length ≤ 55)
  ...
  intro listOff
  have hencFull := decodeListItems_eq_encode _ _ hdecL
  have hlen := extractSuccess_copy_legacy_items_length_long txBytes h hcopyFlag htype0
    items hdecL hlong
  have hn0 : (0 : Nat) < items.length := by omega
  have hn1 : (1 : Nat) < items.length := by omega
  have hn2 : (2 : Nat) < items.length := by omega
  ...
  · exact packaging_hnext_longListSrcOff txBytes txBase listOff items 2 hn2
      hencFull hlong hitem2 hover2 hspan3
  · exact extractSuccess_copy_legacy_hlen20_long txBytes txBase
      (longListSrcOff listOff items 3) h hcopyFlag htype0 items hdecL hlong rfl

theorem extractSuccess_copy_legacy_hoff_srcOff_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
  ...
  have hn3 : (3 : Nat) < items.length := by omega
  exact ⟨
    longListSrcOff_lt_length txBytes listOff items 0 henc hlong hn0,
    longListSrcOff_lt_length txBytes listOff items 1 henc hlong hn1,
    longListSrcOff_lt_length txBytes listOff items 2 henc hlong hn2,
    longListSrcOff_lt_length txBytes listOff items 3 henc hlong hn3⟩

theorem extractSuccess_copy_legacy_hover_srcOff_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
    ...
    txBase.toNat + longListSrcOff listOff items 3 < 2 ^ 64 := by
  intro listOff
  have hoffs := extractSuccess_copy_legacy_hoff_srcOff_long txBytes h hcopyFlag htype0
    items hdecL hlong
  obtain ⟨h0, h1, h2, h3⟩ := hoffs
  exact ⟨hover_of_buffer_span txBase _ _ hover h0,
    hover_of_buffer_span txBase _ _ hover h1,
...
theorem extractSuccess_copy_legacy_hnext_fields02_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
...
theorem extractSuccess_copy_legacy_hvalid_srcOff_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype0 : (teerTxTypeDispatch txBytes).2.1 = (0 : Word))
  ...
  have hr3 := longListSrcOff_succ_room txBytes listOff items 3 henc hlong (by omega)
  exact ⟨
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf hlt0,
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf hr0,
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf hlt1,
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf hr1,
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf hlt2,
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf hr2,
...
/-- Long-list copy t1: field4 is 20B address with prefix 0x94. -/
theorem extractSuccess_copy_t1_field4_pfx94_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    (hlong : 55 < (encode.encodeItems items).length) :
    let listOff := (teerTxTypeDispatch txBytes).2.2.toNat
    let off := longListSrcOff listOff items 4
    (4 : Nat) < items.length ∧
      (∃ content, items[4]? = some (.bytes content) ∧ content.length = 20) ∧
      ∃ hoff : off < txBytes.length, txBytes[off]'hoff = (0x94 : BitVec 8) := by
  intro listOff off
  ...
  have hval : items[4]'hn = .bytes content := (List.getElem?_eq_some_iff.mp hget).2
  have hencFull := decodeListItems_eq_encode _ _ hdec
  have hpfx := long_list_bytes20_pfx txBytes listOff items 4 hencFull hlong hn
    content hval hlen20
  exact ⟨hn, ⟨content, hget, hlen20⟩, hpfx⟩

theorem extractSuccess_copy_t1_items_length_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
...
theorem extractSuccess_copy_t1_hlen20_long
    (txBytes : List (BitVec 8)) (txBase : Word) (srcOff4 : Nat)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    (hdecL : decodeListItems (txBytes.drop (teerTxTypeDispatch txBytes).2.2.toNat) =
      some items)
    (hlong : 55 < (encode.encodeItems items).length)
    (hsrc : srcOff4 = longListSrcOff (teerTxTypeDispatch txBytes).2.2.toNat items 4) :
    ∀ (endPtr next4 len4 : Word),
      rlpItemDecode txBytes srcOff4 (txBase + BitVec.ofNat 64 srcOff4)
        endPtr next4 len4 → len4 = (20 : Word) := by
  have hf := extractSuccess_copy_t1_field4_pfx94_long txBytes h hcopyFlag htype1
...
theorem extractSuccess_copy_t1_hnext_content_long
    (txBytes : List (BitVec 8)) (txBase contentPtr : Word) (srcOff4 : Nat)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
...
theorem extractSuccess_copy_t1_hnext_hlen20_srcOff_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    (hlong : 55 < (encode.encodeItems items).length)
    (hitem0 : (encode (items[0]'(by
        have := extractSuccess_copy_t1_items_length_long txBytes h hcopyFlag htype1
          items hdecL hlong; omega))).length ≤ 55)
    (hitem1 : (encode (items[1]'(by
        have := extractSuccess_copy_t1_items_length_long txBytes h hcopyFlag htype1
          items hdecL hlong; omega))).length ≤ 55)
  ...
  intro listOff
  have hencFull := decodeListItems_eq_encode _ _ hdecL
  have hlen := extractSuccess_copy_t1_items_length_long txBytes h hcopyFlag htype1
    items hdecL hlong
  have hn0 : (0 : Nat) < items.length := by omega
  have hn1 : (1 : Nat) < items.length := by omega
  have hn2 : (2 : Nat) < items.length := by omega
  ...
  · exact packaging_hnext_longListSrcOff txBytes txBase listOff items 3 hn3
      hencFull hlong hitem3 hover3 hspan4
  · exact extractSuccess_copy_t1_hlen20_long txBytes txBase
      (longListSrcOff listOff items 4) h hcopyFlag htype1 items hdecL hlong rfl

theorem extractSuccess_copy_t1_hoff_srcOff_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    longListSrcOff listOff items 2 < txBytes.length ∧
    longListSrcOff listOff items 3 < txBytes.length ∧
    longListSrcOff listOff items 4 < txBytes.length := by
  intro listOff
  have henc := decodeListItems_eq_encode _ _ hdecL
  have hlen := extractSuccess_copy_t1_items_length_long txBytes h hcopyFlag htype1
    items hdecL hlong
  ...
  exact ⟨
    ...
    longListSrcOff_lt_length txBytes listOff items 2 henc hlong hn2,
    longListSrcOff_lt_length txBytes listOff items 3 henc hlong hn3,
    longListSrcOff_lt_length txBytes listOff items 4 henc hlong hn4⟩

theorem extractSuccess_copy_t1_hover_srcOff_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    txBase.toNat + longListSrcOff listOff items 2 < 2 ^ 64 ∧
    txBase.toNat + longListSrcOff listOff items 3 < 2 ^ 64 ∧
    txBase.toNat + longListSrcOff listOff items 4 < 2 ^ 64 := by
  intro listOff
  have hoffs := extractSuccess_copy_t1_hoff_srcOff_long txBytes h hcopyFlag htype1
    items hdecL hlong
  obtain ⟨h0, h1, h2, h3, h4⟩ := hoffs
  exact ⟨hover_of_buffer_span txBase _ _ hover h0,
    hover_of_buffer_span txBase _ _ hover h1,
...
theorem extractSuccess_copy_t1_hnext_fields03_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    (hdecL : decodeListItems (txBytes.drop (teerTxTypeDispatch txBytes).2.2.toNat) =
      some items)
    (hlong : 55 < (encode.encodeItems items).length) :
    (0 + 1 < items.length) ∧ (1 + 1 < items.length) ∧ (2 + 1 < items.length) ∧ (3 + 1 < items.length) := by
  have hlen := extractSuccess_copy_t1_items_length_long txBytes h hcopyFlag htype1
    items hdecL hlong
  exact ⟨by omega, by omega, by omega, by omega⟩

theorem extractSuccess_copy_t1_hvalid_srcOff_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcopyFlag : (teerExtractToAddress txBytes).2.2 = (0 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    isValidByteAccess (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 3 + 1)) = true ∧
    isValidByteAccess (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 4)) = true ∧
    isValidByteAccess (txBase + BitVec.ofNat 64 (longListSrcOff listOff items 4 + 1)) = true := by
  intro listOff
  have henc := decodeListItems_eq_encode _ _ hdecL
  have hlen := extractSuccess_copy_t1_items_length_long txBytes h hcopyFlag htype1
    items hdecL hlong
  ...
  have hlt2 := longListSrcOff_lt_length txBytes listOff items 2 henc hlong hn2
  have hlt3 := longListSrcOff_lt_length txBytes listOff items 3 henc hlong hn3
  have hlt4 := longListSrcOff_lt_length txBytes listOff items 4 henc hlong hn4
  have hr0 := longListSrcOff_succ_room txBytes listOff items 0 henc hlong (by omega)
  have hr1 := longListSrcOff_succ_room txBytes listOff items 1 henc hlong (by omega)
  have hr2 := longListSrcOff_succ_room txBytes listOff items 2 henc hlong (by omega)
  have hr3 := longListSrcOff_succ_room txBytes listOff items 3 henc hlong (by omega)
  have hr4 := longListSrcOff_succ_room txBytes listOff items 4 henc hlong (by omega)
  exact ⟨
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf hlt0,
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf hr0,
    isValidByteAccess_of_validByteRange txBase _ _ hvalidBuf hlt1,
...
/-- Long-list creation t1: field4 empty 0x80 at longListSrcOff. -/
theorem extractSuccess_creation_t1_field4_pfx80_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
  ...
  have hval : items[4]'hn = .bytes [] := (List.getElem?_eq_some_iff.mp hget).2
  have hencFull := decodeListItems_eq_encode _ _ hdec
  have hpfx := long_list_empty_bytes_pfx txBytes listOff items 4 hencFull hlong hn hval
  exact ⟨hn, hget, hpfx⟩

theorem extractSuccess_creation_t1_items_length_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    (hlong : 55 < (encode.encodeItems items).length) :
    5 ≤ items.length := by
  have hf := extractSuccess_creation_t1_field4_pfx80_long txBytes h hcreFlag htype1
    items hdecL hlong
  exact Nat.succ_le_of_lt hf.1

theorem extractSuccess_creation_t1_hcre_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    ∀ (endPtr next4 len4 : Word),
      rlpItemDecode txBytes srcOff4 (txBase + BitVec.ofNat 64 srcOff4)
        endPtr next4 len4 → len4 = (0 : Word) := by
  intro listOff srcOff4 endPtr next4 len4 hdec
  have hf := extractSuccess_creation_t1_field4_pfx80_long txBytes h hcreFlag htype1
    items hdecL hlong
  obtain ⟨hn, _, hoff, hpfx⟩ := hf
  have hoff' : srcOff4 < txBytes.length := by simpa [srcOff4, listOff] using hoff
  have hbyte : txBytes[srcOff4]'hoff' = (0x80 : BitVec 8) := by
    simpa [srcOff4, listOff] using hpfx
  exact hcre_decode_of_pfx80 txBytes srcOff4 (txBase + BitVec.ofNat 64 srcOff4)
    endPtr hoff' hbyte next4 len4 hdec

theorem extractSuccess_creation_t1_hcre_srcOff_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    ∀ (endPtr next4 len4 : Word),
      rlpItemDecode txBytes (longListSrcOff listOff items 4)
        ...
        endPtr next4 len4 → len4 = (0 : Word) := by
  intro listOff
  exact extractSuccess_creation_t1_hcre_long txBytes txBase h hcreFlag htype1
    items hdecL hlong

theorem extractSuccess_creation_t1_hnext_hcre_srcOff_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    (hlong : 55 < (encode.encodeItems items).length)
    (hitem0 : (encode (items[0]'(by
        have := extractSuccess_creation_t1_items_length_long txBytes h hcreFlag htype1
          items hdecL hlong; omega))).length ≤ 55)
    (hitem1 : (encode (items[1]'(by
        have := extractSuccess_creation_t1_items_length_long txBytes h hcreFlag htype1
          items hdecL hlong; omega))).length ≤ 55)
  ...
  intro listOff
  have hencFull := decodeListItems_eq_encode _ _ hdecL
  have hlen := extractSuccess_creation_t1_items_length_long txBytes h hcreFlag htype1
    items hdecL hlong
  have hn0 : (0 : Nat) < items.length := by omega
  have hn1 : (1 : Nat) < items.length := by omega
  have hn2 : (2 : Nat) < items.length := by omega
...
theorem extractSuccess_creation_t1_hoff_srcOff_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
  ...
  have hn4 : (4 : Nat) < items.length := by omega
  exact ⟨
    longListSrcOff_lt_length txBytes listOff items 0 hencFull hlong hn0,
    longListSrcOff_lt_length txBytes listOff items 1 hencFull hlong hn1,
    longListSrcOff_lt_length txBytes listOff items 2 hencFull hlong hn2,
    longListSrcOff_lt_length txBytes listOff items 3 hencFull hlong hn3,
    longListSrcOff_lt_length txBytes listOff items 4 hencFull hlong hn4⟩

theorem extractSuccess_creation_t1_hover_srcOff_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    txBase.toNat + longListSrcOff listOff items 4 < 2 ^ 64 := by
  intro listOff
  have hoffs := extractSuccess_creation_t1_hoff_srcOff_long txBytes h hcreFlag htype1
    items hdecL hlong
  obtain ⟨h0, h1, h2, h3, h4⟩ := hoffs
  exact ⟨hover_of_buffer_span txBase _ _ hover h0,
    hover_of_buffer_span txBase _ _ hover h1,
...
theorem extractSuccess_creation_t1_hnext_fields03_long
    (txBytes : List (BitVec 8))
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
    ...
    (hdecL : decodeListItems (txBytes.drop (teerTxTypeDispatch txBytes).2.2.toNat) =
      some items)
    (hlong : 55 < (encode.encodeItems items).length) :
    (0 + 1 < items.length) ∧ (1 + 1 < items.length) ∧
    (2 + 1 < items.length) ∧ (3 + 1 < items.length) := by
  have hlen := extractSuccess_creation_t1_items_length_long txBytes h hcreFlag htype1
    items hdecL hlong
  exact ⟨by omega, by omega, by omega, by omega⟩

/-- Pack hvalid/hvalid1 at longListSrcOff 0..4. Needs `6 ≤ items.length` for field4+1. -/
theorem extractSuccess_creation_t1_hvalid_srcOff_long
    (txBytes : List (BitVec 8)) (txBase : Word)
    (h : extractSuccess txBytes)
    (hcreFlag : (teerExtractToAddress txBytes).2.2 = (1 : Word))
    (htype1 : (teerTxTypeDispatch txBytes).2.1 = (1 : Word))
...
#print axioms extractSuccess_creation_t1_field4_pfx80_long
#print axioms extractSuccess_creation_t1_items_length_long
#print axioms extractSuccess_creation_t1_hnext_hcre_srcOff_long
#print axioms extractSuccess_creation_t1_hvalid_srcOff_long
