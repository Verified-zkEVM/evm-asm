/-
  Ambient dual: long t1 creation of_decode packaging (typeW=1, fields 0..4).
-/
...
private theorem nFrontCreationStepsLongT1_le_nFront
    (lol : Nat) (hlol : lol ≤ 8) :
    nFrontCreationStepsLongT1 lol ≤ nFrontCreationSteps := by
  simp only [nFrontCreationStepsLongT1, nFrontCreationSteps, nTypeSteps]
  omega

set_option maxRecDepth 8000 in
theorem extractAssumed_creation_under_honesty_of_decode_long_concrete_t1_ambient
    ...
    (hcre : ∀ (next4 len4 : Word),
      rlpItemDecode bs absOff4 (regionBase + BitVec.ofNat 64 absOff4)
        (longWalkEndAmbient regionBase (lenW - (teerTxTypeDispatch (txSlice bs off len)).2.2)
          (ambientAbsOff off (teerTxTypeDispatch (txSlice bs off len)).2.2.toNat)) next4 len4 → len4 = (0 : Word))
    ...
    (hcur : longWalkCursorAmbient regionBase bs
    ...
    (hwi_off1 : ambientAbsOff off (teerTxTypeDispatch (txSlice bs off len)).2.2.toNat + 1 <
      bs.length)
    (h_fits : ¬ BitVec.ult
        ((regionBase + BitVec.ofNat 64
    ...
    (h_llz : (bs[ambientAbsOff off (teerTxTypeDispatch (txSlice bs off len)).2.2.toNat + 1]'hwi_off1
    ...
    (hlol :
      (((bs[ambientAbsOff off (teerTxTypeDispatch (txSlice bs off len)).2.2.toNat]'hoff
          ).zeroExtend 64 - (0xf7 : Word)).toNat) ≤ 8) :
  ...
  refine extractAssumed_creation_of_front_long_ambient sp0 spC s regionBase loadPtr
    lenW toBuf isCreationPtr bs off len hspC ?_
  intro old5 old6 old7 old14 old15 old16
  have hLong := extractFrontCreation_then_epi_of_decode_long_concrete_t1_ambient
    ...
    hlvalid hwi_off1 h_fits h_llz h_min h_match
    old5 old6 old7 old14 old15 old16
  have hle := nFrontCreationStepsLongT1_le_nFront
    (((bs[ambientAbsOff off (teerTxTypeDispatch (txSlice bs off len)).2.2.toNat]'hoff
        ).zeroExtend 64 - (0xf7 : Word)).toNat) hlol
  have h := cpsTripleWithin_mono_nSteps hle hLong
  refine cpsTripleWithin_weaken (fun _ hp => hp) (fun st hq => ?_) h
  obtain ⟨next4, hq'⟩ := hq
  refine ⟨next4, ?_⟩
  simp only [creationE2EPostAmbient, creExtraTemps, htype1]
  exact hq'

theorem extractAssumed_creation_fullCode_of_decode_long_concrete_t1_ambient
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (absOff0 absOff1 absOff2 absOff3 absOff4 : Nat)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_creation_under_honesty_of_decode_long_concrete_t1_ambient
      sp0 spC s regionBase loadPtr lenW toBuf isCreationPtr bs off len
      absOff0 absOff1 absOff2 absOff3 absOff4
      hspC hret htype1 hsalign
    hoff0 hover0 hvalid0 hss0 hls0 hll0 hdec0 hinb0
    ...
    htalign htover htvalid hlen hsuccess hover hvalidTx0
    hoff hcur hinover hinvalid hspan hptr hbound hlistLen_ne h_ge h_ge_f8 hllen hlover
    hlvalid hwi_off1 h_fits h_llz h_min h_match hlol)

#print axioms extractAssumed_creation_under_honesty_of_decode_long_concrete_t1_ambient
#print axioms extractAssumed_creation_fullCode_of_decode_long_concrete_t1_ambient
