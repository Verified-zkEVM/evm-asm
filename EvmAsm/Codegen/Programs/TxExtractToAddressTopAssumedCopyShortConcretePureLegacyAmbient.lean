...
theorem extractAssumed_copy_shortConcrete_pure_legacy_ambient
  ...
  have hhnext :=
    extractSuccess_copy_legacy_hnext_hlen20_srcOff slice loadPtr hsuccess
  ...
  have hlen20 : ∀ (next3 len3 : Word),
    ...
    simpa [absOff3, srcOff3, listOff, endW, absListOff] using
      hlen20_ambient_of_slice regionBase loadPtr bs off len listOff items 3 endW
  ...
  have hnext_sl_content : ∀ (next3 len3 : Word),
    ...
    intro n l hd
    have hsrc : srcOff3 =
        (teerTxTypeDispatch slice).2.2.toNat + 1 + encodeItemsPrefixLen items 3 := by
      simp only [srcOff3, shortListSrcOff, listOff]
    have hc := extractSuccess_copy_legacy_hnext_content slice loadPtr
      (loadPtr + BitVec.ofNat 64 srcOff3 + (1 : Word)) srcOff3
      hsuccess hcopyFlag htype0 items hdecL hshort hsrc rfl
  ...
  have hnext_content : ∀ (next3 len3 : Word),
    ...
    simpa [absOff3, srcOff3, listOff, endW, absListOff] using
      hnext_content_ambient_of_slice regionBase loadPtr bs off len listOff items 3 q endW
        hptr hbound hencInner hshort hn3 hoff3_sl hspan3a hroom3 hq_abs hcover20
        hnext_sl_content
  have hinb0 :=
  ...
  exact extractAssumed_copy_under_honesty_of_decode_short_concrete_legacy_region_ambient
...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_shortConcrete_pure_legacy_ambient_fullCode
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (items : List EL.RLP.RLPItem)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_copy_shortConcrete_pure_legacy_ambient sp0 spC s
