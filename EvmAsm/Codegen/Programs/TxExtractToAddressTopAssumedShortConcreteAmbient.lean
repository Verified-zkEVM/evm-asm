/-
  Ambient Assumed packaging under short concrete of_decode E2E.
-/
...
theorem extractAssumed_creation_under_honesty_of_decode_short_concrete_ambient
  ...
  have hShort := extractFrontCreation_then_epi_of_decode_short_concrete_ambient
  ...
  refine cpsTripleWithin_weaken (fun _ hp => hp) (fun st hq => ?_) hShort
  obtain ⟨next5, hq'⟩ := hq
  exact ⟨next5, by
    simp only [creationE2EPostAmbient, creExtraTemps]
    exact hq'⟩

set_option maxRecDepth 8000 in
theorem extractAssumed_creation_fullCode_of_decode_short_concrete_ambient
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (absOff0 absOff1 absOff2 absOff3 absOff4 absOff5 : Nat)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_creation_under_honesty_of_decode_short_concrete_ambient
      sp0 spC s regionBase loadPtr lenW toBuf isCreationPtr bs off len
      absOff0 absOff1 absOff2 absOff3 absOff4 absOff5
      hspC hret hcur hne0 hne1 hsalign
    hoff0 hover0 hvalid0 hss0 hls0 hll0 hdec0 hinb0
...
#print axioms extractAssumed_creation_under_honesty_of_decode_short_concrete_ambient
#print axioms extractAssumed_creation_fullCode_of_decode_short_concrete_ambient
