...
#guard GuestAddrs.bnq_zero = 0x80030b00
#guard GuestAddrs.bnq_set_one = 0x80030b18
#guard GuestAddrs.bnq_zero + 4 * bnqZero_prog.length = 0x80030b18
...
/-- The verification `CodeReq`: both adjacent routines, at their guest
    addresses. -/
def bnqCr : CodeReq :=
  CodeReq.ofProg (0x80030fac : Word) (bnqZero_prog ++ bnqSetOne_prog)
...
theorem bnqZeroFlat_spec (ret dst : Word) (vs : List Word)
    ...
    cpsTripleWithin (1 + 48 * (3 + 1) + 1) (0x80030fac : Word) ret bnqCr
  ...
  rw [show (Bn254Fq12ZeroSAsm.bnqZeroFn dst (vs.flatMap dwordBytes)).programRet (0x80030fac : Word)
...
def setOneVals' (dst : Word) : Reg → Word :=
  fun r => match r with | .x1 => (0x80030fd8 : Word) | .x8 => dst | _ => 0
...
theorem bnqSetOneFrame_spec (sp0 ret dst arb8 v5 v7 : Word) (vs : List Word)
  ...
  -- mv s0, a0
  have hmv := mv_spec_gen_within .x8 .x10 dst arb8 (0x80030fd0 : Word) (by decide)
  rw [show (0x80030fd0 : Word) + 4 = (0x80030fd4 : Word) from by decide] at hmv
  ...
  have hcallee := bnqZeroFlat_spec ((0x80030fd4 : Word) + 4) dst vs hlen hwf (by decide)
  have hcall := callWithin_spec (0x80030fd4 : Word) (0x80030fac : Word) ret
    (jalOff GuestAddrs.bnq_zero (GuestAddrs.bnq_set_one + 16)) (1 + 48 * (3 + 1) + 1)
    (by decide) (by code_mem) (by pcf) hcallee
  rw [show (0x80030fd4 : Word) + 4 = (0x80030fd8 : Word) from by decide] at hcall
  ...
  have hli := li_spec_gen_own_within .x5 (1 : Word) (0x80030fd8 : Word) (by decide)
  rw [show (0x80030fd8 : Word) + 4 = (0x80030fdc : Word) from by decide] at hli
  have hliC := liftCode (cr' := bnqCr) hli (by code_mem)
  have hliF := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ (0x80030fd8 : Word)) ** (.x8 ↦ᵣ dst)
  ...
  have hsd := sd_spec_gen_within .x8 .x5 dst (1 : Word)
    ((List.replicate 48 (0 : Word)).getD 0 0) (0 : BitVec 12) (0x80030fdc : Word)
  rw [add_sext0] at hsd
  rw [show (0x80030fdc : Word) + 4 = (0x80030fe0 : Word) from by decide] at hsd
  have hsdC := liftCode (cr' := bnqCr) hsd (by code_mem)
  have hsdF := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ (0x80030fd8 : Word))
      ** (.x10 ↦ᵣ (dst + BitVec.ofNat 64 384))
      ** regOwn .x7 ** regOwns bnqRiders
  ...
  have hbody : cpsTripleWithin (1 + (1 + (1 + 48 * (3 + 1) + 1)) + 1 + 1)
      ((0x80030fc4 : Word) + BitVec.ofNat 64 (4 * (1 + setOneFrame.length)))
      ((0x80030fc4 : Word)
      ...
      ((.x2 ↦ᵣ (sp0 + signExtend12 (-16 : BitVec 12)))
        ...
        ** ((.x10 ↦ᵣ (dst + BitVec.ofNat 64 384)) ** regOwn .x5 ** regOwn .x7
          ...
          ** (Reg.x0 ↦ᵣ (0 : Word))
          ** dwordsIs dst ((1 : Word) :: List.replicate 47 (0 : Word)))) := by
    have hentry : (0x80030fc4 : Word) + BitVec.ofNat 64 (4 * (1 + setOneFrame.length))
        = (0x80030fd0 : Word) := by decide
    have hexit : (0x80030fc4 : Word)
          + BitVec.ofNat 64 (4 * (1 + setOneFrame.length + setOneBody.length))
