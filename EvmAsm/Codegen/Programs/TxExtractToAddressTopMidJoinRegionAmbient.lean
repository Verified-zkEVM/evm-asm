/-
  Ambient MidJoin bare copy region: AfterSave → copy → ret (split loadPtr/regionBase).
-/
...
theorem wn5Exists_to_copy_region_ambient
  ...
  have hpure :
      cpsTripleWithin
        ...
        (⌜rlpItemDecode bs absOff5 (regionBase + BitVec.ofNat 64 absOff5)
            endPtr next5 len5⌝ **
          (wn5OkRegsAmbient loadPtr regionBase lenW typeW innerW endPtr next5 len5
            bs absOff5 **
    ...
    have hleaf := extractType234HaveFieldCopy_then_epi_region_ambient sp0 spC s loadPtr
      regionBase lenW typeW innerW endPtr next5 toBuf isCreationPtr s7 bs absOff5 q
      hspC hret (by simpa only [contentPtr] using hnext) hq halign hcover
      hcvalid htalign htover htvalid
    refine cpsTripleWithin_weaken (fun _ hp => by
      simp only [hlen, wn5OkConcreteAmbient] at hp ⊢
...
theorem extractType234AfterSaveCopy_then_epi_of_decode_region_ambient
    (sp0 spC : Word) (s : ExtractSaved)
    ...
    cpsTripleWithin
      ...
      (afterSaveFrameTyAmbient loadPtr regionBase lenW typeW innerW cursor endPtr bs **
  ...
  have hC := wn5Exists_to_copy_region_ambient sp0 spC s loadPtr regionBase lenW typeW innerW
    endPtr toBuf isCreationPtr s7 bs absOff5 q
    hspC hret hlen20 hnext_content hq hsalign hcover hcvalid htalign htover htvalid
  exact cpsTripleWithin_seq_same_cr hW hC


#print axioms wn5Exists_to_copy_region_ambient
#print axioms extractType234AfterSaveCopy_then_epi_of_decode_region_ambient
