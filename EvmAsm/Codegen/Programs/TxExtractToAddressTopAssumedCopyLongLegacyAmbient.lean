/-
  Ambient Assumed packaging for legacy long 20B copy (regionBase/loadPtr).
...
theorem extractAssumed_copy_concrete_long_legacy_region_ambient
...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_temps_long_legacy_region_ambient
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr contentPtr w2 : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (lol : Nat)
  ...
  have htemps : cpsTripleWithin nExtractSteps E s.ra extractLinkedCode
    ...
    have hc := extractAssumed_copy_concrete_long_legacy_region_ambient sp0 spC s regionBase loadPtr lenW
...
theorem extractAssumed_copy_of_front_long_concrete_legacy_region_ambient
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (bs : List (BitVec 8)) (off len : Nat)
  ...
  let lol := ((bs[ambientAbsOff off (teerTxTypeDispatch (txSlice bs off len)).2.2.toNat]'hoff).zeroExtend 64 -
      (0xf7 : Word)).toNat
  refine extractAssumed_copy_temps_long_legacy_region_ambient sp0 spC s regionBase loadPtr lenW toBuf
    isCreationPtr contentPtr w2 bs off len lol hspC hlol
    (fun old5 old6 old7 old14 old15 old16 => ?_)
  have hE := extractFrontCopy_then_epi_of_decode_long_concrete_legacy_region_ambient
...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_fullCode_of_decode_long_concrete_legacy_region_ambient
    (sp0 spC : Word) (s : ExtractSaved)
    (regionBase loadPtr lenW toBuf isCreationPtr : Word)
    (bs : List (BitVec 8)) (off len : Nat)
    (absOff0 absOff1 absOff2 absOff3 q : Nat)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_copy_of_front_long_concrete_legacy_region_ambient sp0 spC s
...
#print axioms extractAssumed_copy_of_front_long_concrete_legacy_region_ambient
#print axioms extractAssumed_copy_fullCode_of_decode_long_concrete_legacy_region_ambient
