...
def mulVals' (bPtr outPtr : Word) : Reg → Word :=
  fun r => match r with
  | .x1 => (0x800301d8 : Word) | .x8 => bPtr | .x9 => outPtr | _ => 0
...
theorem bnfMulModP_spec (sp0 aPtr bPtr outPtr ret v8 v9 : Word)
  ...
  have hbody : cpsTripleWithin
      (17 + ((bnfBeToLeFn 0 0 [] []).body.steps + 1) * 2
        + ((bnfLeToBeFn 0 0 [] []).body.steps + 1))
      ((0x80030188 : Word) + BitVec.ofNat 64 (4 * (1 + mulFrame.length)))
      ((0x80030188 : Word)
        ...
        ** (fun hp => ∃ ws₁ ws₂ out',
            ((⌜wsNat256 ws₁ 0 = beBytesToNat aBE ∧ ws₁.length = 32
              ...
              ** bytesRegion arenaB
                (setBytes (setBytes (setBytes ws 0 ws₁) 0x20 ws₂) 0x40
                  (leBytes32 (mulResult aBE bBE ws))))) hp)) := by
    have hentry : (0x80030188 : Word)
          + BitVec.ofNat 64 (4 * (1 + mulFrame.length))
        = (0x80030198 : Word) := by decide
    have hexit : (0x80030188 : Word)
          + BitVec.ofNat 64 (4 * (1 + mulFrame.length + mulBody.length))
        = (0x800301dc : Word) := by decide
    rw [hentry, hexit]
    ...
    have hB : ∀ ws₁ : List (BitVec 8),
        cpsTripleWithin
          ...
          (0x800301ac : Word) (0x800301dc : Word) mulCr
          (⌜wsNat256 ws₁ 0 = beBytesToNat aBE ∧ ws₁.length = 32⌝
            ** ((.x1 : Reg) ↦ᵣ (0x800301ac : Word)) ** regOwns exposedRegs
      ...
      have hC : ∀ ws₂ : List (BitVec 8),
          cpsTripleWithin (7 + ((bnfLeToBeFn 0 0 [] []).body.steps + 1) + 1)
            ...
            (⌜wsNat256 ws₂ 0 = beBytesToNat bBE ∧ ws₂.length = 32⌝
              ** ((.x1 : Reg) ↦ᵣ (0x800301bc : Word)) ** regOwns exposedRegs
        ...
        · -- reassemble the arena around the written `_b` window
          have hp1 : ((((.x1 : Reg) ↦ᵣ (0x800301bc : Word)) ** (.x8 ↦ᵣ bPtr)
        ...
        · -- rebuild the whole-routine ∃-post from the stage-C witness
          ...
          have hout : ((((.x1 : Reg) ↦ᵣ (0x800301d8 : Word)) ** (.x8 ↦ᵣ bPtr)
              ** (.x9 ↦ᵣ outPtr))
              ** (fun hp => ∃ ws₁' ws₂' out'',
                ((⌜wsNat256 ws₁' 0 = beBytesToNat aBE ∧ ws₁'.length = 32
                  ∧ wsNat256 ws₂' 0 = beBytesToNat bBE ∧ ws₂'.length = 32
      ...
      refine cpsTripleWithin_weaken (fun h hp => ?_) (fun _ hq => hq)
        (cpsTripleWithin_peel_regOwns [.x10, .x11] (by decide)
          (P := ((.x1 : Reg) ↦ᵣ (0x800301ac : Word)) ** regOwns convScratch
      ...
      · rw [ownsExposedSplit] at hp
        have hp1 : ((((.x1 : Reg) ↦ᵣ (0x800301ac : Word)) ** regOwns [.x10, .x11]
      ...
      · -- a0 := s0
        have hmv10 := liftCode (cr' := mulCr)
          (mv_spec_gen_within .x10 .x8 bPtr (vf .x10) (0x800301ac : Word)
            (by decide))
          (by code_mem)
        rw [show (0x800301ac : Word) + 4 = (0x800301b0 : Word) from by decide]
          at hmv10
        -- la a1, bnf_le_b
        have hla11 := la_materialize_within .x11 (vf .x11) (0x800301b0 : Word)
          (0xbb55d9d0 : Word) (cr := mulCr) (by decide) (by decide)
          (by code_mem) (by code_mem)
        rw [show (0x800301b0 : Word) + 8 = (0x800301b8 : Word) from by decide]
          at hla11
        -- the second conversion call over the FOCUSED `_b` window
        have hflat2 := bnfBeToLeFlat_spec (0x800301bc : Word) bPtr
        ...
        have hcallee2 : cpsTripleWithin
            ...
            (0x8003003c : Word) (0x800301bc : Word) mulCr
            (((.x1 : Reg) ↦ᵣ (0x800301bc : Word))
          ...
          xperm_hyp hin
        have hcall2 := callWithin_spec (0x800301b8 : Word) (0x8003003c : Word)
          (0x800301ac : Word)
          (jalOff GuestAddrs.bnf_be_to_le (GuestAddrs.bnf_mul_mod_p + 48))
          ((bnfBeToLeFn bPtr (0xbb55d9d0 : Word) bBE
            (((setBytes ws 0 ws₁).drop 0x20).take 32)).body.steps + 1)
          (by decide) (by code_mem) (by pcf) hcallee2
        rw [show (0x800301b8 : Word) + 4 = (0x800301bc : Word) from by decide]
          at hcall2
        ...
        have hd2' : cpsTripleWithin
            (4 + ((bnfBeToLeFn 0 0 [] []).body.steps + 1))
            (0x800301ac : Word) (0x800301bc : Word) mulCr
    ...
    have hm1 := liftCode (cr' := mulCr)
      (mv_spec_gen_within .x8 .x11 bPtr v8 (0x80030198 : Word) (by decide))
      (by code_mem)
    rw [show (0x80030198 : Word) + 4 = (0x8003019c : Word) from by decide] at hm1
    have hm2 := liftCode (cr' := mulCr)
      (mv_spec_gen_within .x9 .x12 outPtr v9 (0x8003019c : Word) (by decide))
      (by code_mem)
    rw [show (0x8003019c : Word) + 4 = (0x800301a0 : Word) from by decide] at hm2
    have hla := la_materialize_within .x11 bPtr (0x800301a0 : Word)
      (0xbb55d9b0 : Word) (cr := mulCr) (by decide) (by decide)
      (by code_mem) (by code_mem)
    rw [show (0x800301a0 : Word) + 8 = (0x800301a8 : Word) from by decide] at hla
    have hflat1 := bnfBeToLeFlat_spec (0x800301ac : Word) aPtr
    ...
    have hcallee1 : cpsTripleWithin
        ...
        (0x8003003c : Word) (0x800301ac : Word) mulCr
        (((.x1 : Reg) ↦ᵣ (0x800301ac : Word))
      ...
      refine exists_pull h ⟨ws', ?_⟩
      xperm_hyp hin
    have hcall1 := callWithin_spec (0x800301a8 : Word) (0x8003003c : Word) ret
      (jalOff GuestAddrs.bnf_be_to_le (GuestAddrs.bnf_mul_mod_p + 32))
      ((bnfBeToLeFn aPtr (0xbb55d9b0 : Word) aBE
        ((ws.drop 0).take 32)).body.steps + 1)
      (by decide) (by code_mem) (by pcf) hcallee1
    rw [show (0x800301a8 : Word) + 4 = (0x800301ac : Word) from by decide]
      at hcall1
      ...
      (fun h hp => by
        ...
        have hp1 : ((.x12 ↦ᵣ outPtr)
            ** (((.x1 : Reg) ↦ᵣ ret) ** (.x10 ↦ᵣ aPtr)
              ** (.x11 ↦ᵣ (0xbb55d9b0 : Word)) ** regOwns mulRest
              ** bytesRegion (0xbb55d9b0 : Word) ((ws.drop 0).take 32)
              ** bytesRegion aPtr aBE ** (.x8 ↦ᵣ bPtr) ** (.x9 ↦ᵣ outPtr)
              ** bytesRegion bPtr bBE ** bytesRegion outPtr outOld
    ...
    have ha3' : cpsTripleWithin (5 + ((bnfBeToLeFn 0 0 [] []).body.steps + 1))
        (0x80030198 : Word) (0x800301ac : Word) mulCr
