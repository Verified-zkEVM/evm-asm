...
/-! Closed status-0 arm witness for #12849.  The concrete arm is the
    under-target branch (`this = parent.excess = parent.blob_gas_used = 0`),
    so it is deliberately independent of the still-open Amsterdam price
    loop contract. -/
...
/-! The closed arm below is the status-0 under-target arm.  Its branch
    predicate is stated independently of the still-open price arm: the
    machine has already formed `parentTotal` in `x20`, takes the under-target
    branch when that value is below the Amsterdam target, and returns status 0
    exactly when `thisExcess` is zero.  The two small witnesses make the gate
    non-vacuous and keep its boundary visible to the registry audit. -/
def status0ArmGate (thisExcess parentTotal : Word) : Prop :=
  BitVec.ult parentTotal (((448 : BitVec 20).zeroExtend 32 <<< 12).signExtend 64) = true ∧
    thisExcess = 0

theorem status0ArmGate_admits : status0ArmGate 0 0 := by
  simp [status0ArmGate, BitVec.ult]

theorem status0ArmGate_refutable : ¬ status0ArmGate 1 0 := by
  simp [status0ArmGate, BitVec.ult]
...
set_option maxRecDepth 8000 in
theorem header_validate_excess_blob_gas_status0_arm_spec_within :
