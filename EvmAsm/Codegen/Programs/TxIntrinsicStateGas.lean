...
def eip7702AuthorityAsof_prog : Program :=
  [ .ADDI .x2 .x2 (-64 : BitVec 12),
    ...
    -- The no-current-overlay path must use the same transaction-start source
    -- as the current-overlay path: committed block-map state first, then the
    -- authenticated pre-block header.  Keep this five-instruction seam
    -- layout-neutral so the existing address-pinned tail remains stable.
    .MV .x10 .x8,
    .ADDI .x11 .x2 (56 : BitVec 12),
    .ADDI .x12 .x2 (48 : BitVec 12),
    .JAL .x1 (jalOff GuestAddrs.account_writes_auth_block (GuestAddrs.eip7702_authority_asof + 668)),
    .JAL .x0 (jalOff (GuestAddrs.eip7702_authority_asof + 104) (GuestAddrs.eip7702_authority_asof + 672)),
    .LD .x11 .x5 (0 : BitVec 12),
    .MV .x12 .x8,
...
def eip7702AuthorityAsof_relocs : RelocTable :=
  [ (13, .jal .x1 "account_writes_auth_current"),
    ...
    (167, .jal .x1 "account_writes_auth_block"),
