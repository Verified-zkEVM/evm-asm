/-
  ExtractAssumed packaging for type234 short 20B copy path.
  Pre/post = Assumed footprint ** contentDwords (content ambient).
-/
...
theorem nFrontCopyStepsShort_le_nExtract :
...
/-- Flat E2E copy post (matches FrontE2ECopyShortConcrete). -/
def copyE2EPost (sp0 : Word) (s : ExtractSaved)
    (txBase toBuf isCreationPtr contentPtr w0 w1 w2 : Word)
...
private def copyKeep (sp0 : Word) (s : ExtractSaved)
    (txBase toBuf contentPtr w0 w1 w2 : Word)
    (txBytes : List (BitVec 8)) : Assertion :=
  (.x1 ↦ᵣ s.ra) ** (.x2 ↦ᵣ sp0) **
    stackFree sp0 nExtractStackDwords **
    (.x10 ↦ᵣ (0 : Word)) **
...
private def copyConvIs (isCreationPtr contentPtr w2 : Word)
    (txBytes : List (BitVec 8)) : Assertion :=
  (isCreationPtr ↦ₘ (0 : Word)) **
    (TeaTypeAddr ↦ₘ (teerTxTypeDispatch txBytes).2.1) **
    (TeaInnerAddr ↦ₘ (teerTxTypeDispatch txBytes).2.2) **
...
private def copyConvOwn (isCreationPtr : Word) : Assertion :=
...
private theorem copyConvIs_to_own (isCreationPtr contentPtr w2 : Word)
    (txBytes : List (BitVec 8)) :
    ∀ (st : PartialState), copyConvIs isCreationPtr contentPtr w2 txBytes st →
      copyConvOwn isCreationPtr st := by
  intro st hp
  simp only [copyConvIs, copyConvOwn] at hp ⊢
...
/-- Copy E2E post → Assumed post ** contentDwords (KEEP s0–s7 + content). -/
theorem copyPost_to_assumed
    (sp0 : Word) (s : ExtractSaved)
    (txBase toBuf isCreationPtr contentPtr w0 w1 w2 : Word)
    (txBytes : List (BitVec 8)) :
    ∀ h, copyE2EPost sp0 s txBase toBuf isCreationPtr contentPtr w0 w1 w2
        txBytes h →
      (extractAssumedPost s.ra sp0 txBase toBuf isCreationPtr
        s.s0 s.s1 s.s2 s.s3 s.s4 s.s5 s.s6 s.s7 txBytes **
        contentDwords contentPtr w0 w1 w2) h := by
  intro h hp
  simp only [copyE2EPost] at hp
  have hp1 : (copyKeep sp0 s txBase toBuf contentPtr w0 w1 w2 txBytes **
      copyConvIs isCreationPtr contentPtr w2 txBytes) h := by
    simp only [copyKeep, copyConvIs]
    xperm_hyp hp
  obtain ⟨hk, hc, hd, hu, hKeep, hConv⟩ := hp1
  have hConv' := copyConvIs_to_own isCreationPtr contentPtr w2 txBytes hc hConv
  have hJoined : (copyKeep sp0 s txBase toBuf contentPtr w0 w1 w2 txBytes **
      copyConvOwn isCreationPtr) h :=
    ⟨hk, hc, hd, hu, hKeep, hConv'⟩
  simp only [copyKeep, copyConvOwn, extractAssumedPost, teaScratchOwn,
    contentDwords] at hJoined ⊢
  xperm_hyp hJoined

/-- E2E copy pre = creationE2EPre ** contentDwords. -/
def copyE2EPre (sp0 spC : Word) (s : ExtractSaved)
...
set_option maxRecDepth 8000 in
theorem assumedPreConcrete_to_copy_e2e
    ...
    ∀ h, (assumedPreConcrete s.ra sp0 s txBase lenW toBuf isCreationPtr
      ...
      copyE2EPre sp0 spC s txBase lenW toBuf isCreationPtr contentPtr w0 w1 w2
        old5 old6 old7 old14 old15 old16 txBytes h := by
  intro h hp
  simp only [copyE2EPre] at hp ⊢
  obtain ⟨h1, h2, hd, hu, hA, hC⟩ := hp
  have hA' := assumedPreConcrete_to_e2e sp0 spC s txBase lenW toBuf
    isCreationPtr old5 old6 old7 old14 old15 old16 txBytes hspC h1 hA
  exact ⟨h1, h2, hd, hu, hA', hC⟩

set_option maxRecDepth 8000 in
theorem extractAssumed_copy_concrete
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr contentPtr w0 w1 w2 : Word)
    (old5 old6 old7 old14 old15 old16 : Word)
    (txBytes : List (BitVec 8))
    ...
    (hE2E : cpsTripleWithin nFrontCopyStepsShort E s.ra extractLinkedCode
      (copyE2EPre sp0 spC s txBase lenW toBuf isCreationPtr contentPtr w0 w1 w2
        old5 old6 old7 old14 old15 old16 txBytes)
      (copyE2EPost sp0 s txBase toBuf isCreationPtr contentPtr w0 w1 w2
        txBytes)) :
  ...
  have h1 := cpsTripleWithin_mono_nSteps nFrontCopyStepsShort_le_nExtract hE2E
  refine cpsTripleWithin_weaken
    (fun st hp => assumedPreConcrete_to_copy_e2e sp0 spC s txBase lenW toBuf
      isCreationPtr contentPtr w0 w1 w2 old5 old6 old7 old14 old15 old16
      txBytes hspC st hp)
    (fun st hq => copyPost_to_assumed sp0 s txBase toBuf isCreationPtr
      contentPtr w0 w1 w2 txBytes st hq) h1
...
private def assumedCoreCopy (sp0 : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr contentPtr w0 w1 w2 : Word)
    (txBytes : List (BitVec 8)) : Assertion :=
  (.x1 ↦ᵣ s.ra) ** (.x2 ↦ᵣ sp0) **
    stackFree sp0 nExtractStackDwords **
...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_temps
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr contentPtr w0 w1 w2 : Word)
    (txBytes : List (BitVec 8))
    (hspC : spC = sp0 + signExtend12 (-80 : BitVec 12))
    (hE2E : ∀ (old5 old6 old7 old14 old15 old16 : Word),
      cpsTripleWithin nFrontCopyStepsShort E s.ra extractLinkedCode
        (copyE2EPre sp0 spC s txBase lenW toBuf isCreationPtr contentPtr
          w0 w1 w2 old5 old6 old7 old14 old15 old16 txBytes)
        (copyE2EPost sp0 s txBase toBuf isCreationPtr contentPtr w0 w1 w2
          txBytes)) :
    cpsTripleWithin nExtractSteps E s.ra extractLinkedCode
  ...
  let Q := extractAssumedPost s.ra sp0 txBase toBuf isCreationPtr
    s.s0 s.s1 s.s2 s.s3 s.s4 s.s5 s.s6 s.s7 txBytes **
    contentDwords contentPtr w0 w1 w2
  let Core := assumedCoreCopy sp0 s txBase lenW toBuf isCreationPtr
    contentPtr w0 w1 w2 txBytes
  have htemps : cpsTripleWithin nExtractSteps E s.ra extractLinkedCode
    ...
    have hc := extractAssumed_copy_concrete sp0 spC s txBase lenW
      toBuf isCreationPtr contentPtr w0 w1 w2 old5 old6 old7 old14 old15 old16
      txBytes hspC (hE2E old5 old6 old7 old14 old15 old16)
    refine cpsTripleWithin_weaken (fun _ hp => by
      dsimp [Core, assumedCoreCopy, assumedPreConcrete, contentDwords] at hp ⊢
      xperm_hyp hp) (fun _ hq => by
      dsimp [Q] at hq ⊢; exact hq) hc
  refine cpsTripleWithin_weaken (fun _ hp => by
    simp only [extractAssumedPre] at hp ⊢
    dsimp [Core, assumedCoreCopy] at hp ⊢
    xperm_hyp hp) (fun _ hq => by
    dsimp [Q] at hq ⊢; exact hq) htemps

set_option maxRecDepth 8000 in
/-- Wire Assumed**content under short concrete copy E2E of_decode. -/
theorem extractAssumed_copy_of_front_short_concrete
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr contentPtr w0 w1 w2 : Word)
    (txBytes : List (BitVec 8))
    ...
    cpsTripleWithin nExtractSteps E s.ra extractLinkedCode
      ...
      (extractAssumedPost s.ra sp0 txBase toBuf isCreationPtr
        s.s0 s.s1 s.s2 s.s3 s.s4 s.s5 s.s6 s.s7 txBytes **
        contentDwords contentPtr w0 w1 w2) :=
  extractAssumed_copy_temps sp0 spC s txBase lenW toBuf isCreationPtr
    contentPtr w0 w1 w2 txBytes hspC (fun old5 old6 old7 old14 old15 old16 => by
      have h := extractFrontCopy_then_epi_of_decode_short_concrete sp0 spC s
        ...
        hcalign hcover hcvalid htalign htover htvalid hlen hsuccess hover
        hvalidTx0 hoff hinover hinvalid hlistLen_ne h_ge h_hi h_exact
        old5 old6 old7 old14 old15 old16
      simpa only [copyE2EPre, creationE2EPre, copyE2EPost] using h)

set_option maxRecDepth 8000 in
theorem extractAssumed_copy_fullCode_of_decode_short_concrete
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr contentPtr w0 w1 w2 : Word)
    (txBytes : List (BitVec 8))
    (srcOff0 srcOff1 srcOff2 srcOff3 srcOff4 srcOff5 : Nat)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_copy_of_front_short_concrete sp0 spC s
      txBase lenW toBuf isCreationPtr contentPtr w0 w1 w2 txBytes
      srcOff0 srcOff1 srcOff2 srcOff3 srcOff4 srcOff5
      hspC hret hcur hne0 hne1 hsalign
      hoff0 hover0 hvalid0 hss0 hls0 hll0 hdec0 hinb0
      hoff1 hover1 hvalid1 hss1 hls1 hll1 hdec1 hinb1
...
#print axioms copyPost_to_assumed
#print axioms extractAssumed_copy_of_front_short_concrete
#print axioms extractAssumed_copy_fullCode_of_decode_short_concrete
