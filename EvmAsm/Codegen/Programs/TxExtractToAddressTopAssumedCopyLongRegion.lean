...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_temps_long_region
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr contentPtr w2 : Word)
    (txBytes : List (BitVec 8))
    (lol : Nat)
  ...
  have htemps : cpsTripleWithin nExtractSteps E s.ra extractLinkedCode
    ...
    have hc := extractAssumed_copy_concrete_long_region sp0 spC s txBase lenW
...
theorem extractAssumed_copy_of_front_long_concrete_region
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr : Word)
    (txBytes : List (BitVec 8))
  ...
  let lol := ((txBytes[(teerTxTypeDispatch txBytes).2.2.toNat]'hoff).zeroExtend 64 -
      (0xf7 : Word)).toNat
  refine extractAssumed_copy_temps_long_region sp0 spC s txBase lenW toBuf
    isCreationPtr contentPtr w2 txBytes lol hspC hlol
    (fun old5 old6 old7 old14 old15 old16 => ?_)
  have hE := extractFrontCopy_then_epi_of_decode_long_concrete_region
    ...
    hvalidTx0 hoff hinover hinvalid hcur hlistLen_ne h_ge h_ge_f8 hllen hlover hlvalid
...
set_option maxRecDepth 8000 in
theorem extractAssumed_copy_fullCode_of_decode_long_concrete_region
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW toBuf isCreationPtr : Word)
    (txBytes : List (BitVec 8))
    (srcOff0 srcOff1 srcOff2 srcOff3 srcOff4 srcOff5 q : Nat)
  ...
  cpsTripleWithin_extend_code extractLinked_mono
    (extractAssumed_copy_of_front_long_concrete_region sp0 spC s
