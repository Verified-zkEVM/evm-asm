/-
  ...
  entries into the chunked committed map under their real `(addrHash, slotKey)`
  keys, so a later read can recover the latest already-committed value for any
  account.
-/
...
/-! ## bv_mtx_committed_chunked_snapshot_upsert
    ...
    The chunked helper keeps the 128-byte committed-map entry layout, and its
    ABI treats a4/a5 as global counts over the contiguous
