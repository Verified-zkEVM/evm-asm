...
/-! ## Alias-safe in-place subtraction (`bPtr = outPtr`)

    K73 uses the output buffer as the subtrahend.  The loop consumes byte 31
    before storing it and then moves toward byte 0, so a later iteration never
    reads a byte already overwritten by an earlier one.  This is a distinct
    one-pointer contract rather than a weakening of `u256SubBeFn`'s general
    disjointness contract; the API makes the supported alias explicit and
    leaves partial-overlap cases unrepresentable.
-/
...
def u256SubBeInPlaceBefore (aPtr _outPtr : Word)
...
def u256SubBeInPlaceBody (aPtr outPtr : Word)
  ...
  .block "init" [.LI .x5 (31 : Word), .LI .x6 (0 : Word)] ;;;
  .«doWhileBreak» "loop" 31 (u256SubBeInPlaceInv aPtr outPtr aBytes orig)
...
theorem u256SubBeInPlace_retVal_post (aPtr outPtr : Word)
    (aBytes orig : List (BitVec 8)) :
    ∀ rf ws A,
      sp Region.empty ⟨outPtr, 32⟩ (.block "retVal" [.MV .x10 .x6])
        (u256SubBeInPlaceLoopPost aPtr outPtr aBytes orig) rf ws A →
      (u256SubBeInPlaceFn aPtr outPtr aBytes orig).post rf ws A := by
...
theorem u256SubBeInPlaceBody_flatten (L : GuestLayout) :
    (u256SubBeInPlaceBody 0 0 [] []).flatten 0 ++
      [Instr.JALR .x0 .x1 (0 : BitVec 12)] = u256SubBe_prog_of L := rfl

private theorem subBorrowState_length (a b orig : List (BitVec 8)) (k : Nat) :
    (subBorrowState a b orig k).1.length = orig.length := by
  induction k with
  | zero => rfl
  | succ k ih =>
...
private theorem subBorrowState_unprocessed_in_place (a orig : List (BitVec 8))
    (k j : Nat) (hj : j < 32 - k) :
    (subBorrowState a orig orig k).1.getD j 0 = orig.getD j 0 := by
  induction k with
  | zero => rfl
  | succ k ih =>
      rw [subBorrowState_succ]
...
private theorem idx_toNat_inPlace (i : Nat) (hi : i ≤ 31) :
    (BitVec.ofNat 64 (31 - i)).toNat = 31 - i := by
  rw [BitVec.toNat_ofNat]
  omega

private theorem add_idx_sub_self_inPlace (ptr : Word) (i : Nat) (hi : i ≤ 31) :
    (ptr + BitVec.ofNat 64 (31 - i) - ptr).toNat = 31 - i := by
  have hidx : (BitVec.ofNat 64 (31 - i)).toNat = 31 - i :=
    idx_toNat_inPlace i hi
  rw [BitVec.toNat_sub, BitVec.toNat_add, hidx]
  omega

private theorem add_idx_sub_base_inPlace (ptr base : Word) (i : Nat) (hi : i ≤ 31) :
...
private theorem readLbu_blockVCs_inPlace (ptr outPtr : Word) (rf : RegFile)
  ...
  · unfold Region.loadOk
    ...
    rw [add_idx_sub_self_inPlace ptr i hi, hroLen]
    exact ⟨one_dvd _, by omega⟩
  · unfold inRw
    rw [hws, add_idx_sub_base_inPlace ptr outPtr i hi]
    intro hcontra
    rcases hdisj with hd | hd <;> omega

private theorem execBlock_lbu_get_ne_inPlace (ro : Region) (rwBase : Word)
    (rf : RegFile) (ws : List (BitVec 8)) (rd rs : Reg) (ofs : BitVec 12)
    (r : Reg) (hne : r ≠ rd) :
...
private theorem not_inRw_disjoint32_inPlace (ptr outPtr : Word) (ws : List (BitVec 8))
    (i : Nat) (hi : i ≤ 31) (hws : ws.length = 32)
    (hptr : ptr.toNat + 32 < 2 ^ 64) (hout : outPtr.toNat + 32 < 2 ^ 64)
    (hdisj : ptr.toNat + 32 ≤ outPtr.toNat ∨ outPtr.toNat + 32 ≤ ptr.toNat) :
...
private theorem byteAt_idx_inPlace (ptr : Word) (bytes : List (BitVec 8))
    (i : Nat) (hi : i ≤ 31) :
    Region.byteAt ⟨ptr, bytes⟩ (ptr + BitVec.ofNat 64 (31 - i)) =
      bytes.getD (31 - i) 0 := by
  unfold Region.byteAt
  rw [add_idx_sub_self_inPlace ptr i hi]

private theorem execBlock_lbu_ws_inPlace (ro : Region) (rwBase : Word)
    (rf : RegFile) (ws : List (BitVec 8)) (rd rs : Reg) (ofs : BitVec 12) :
    (execBlock ro rwBase rf ws [.LBU rd rs ofs]).2 = ws := by
  rw [execBlock_cons, execBlock_nil, execInstrRF]
  dsimp only [aluSem, loadSem]

private theorem execBlock_lbu_ro_idx_inPlace (ptr outPtr : Word) (rf : RegFile)
    (ws robytes : List (BitVec 8)) (rd addrReg : Reg) (i : Nat) (hi : i ≤ 31)
    (haddr : rf.get addrReg = ptr + BitVec.ofNat 64 (31 - i))
    (hws : ws.length = 32) (hptrBound : ptr.toNat + 32 < 2 ^ 64)
    (houtBound : outPtr.toNat + 32 < 2 ^ 64)
    (hdisj : ptr.toNat + 32 ≤ outPtr.toNat ∨ outPtr.toNat + 32 ≤ ptr.toNat) :
    execBlock ⟨ptr, robytes⟩ outPtr rf ws
      [.LBU rd addrReg (0 : BitVec 12)] =
  ...
  rw [if_neg (by
    rw [haddr0]
    exact not_inRw_disjoint32_inPlace ptr outPtr ws i hi hws hptrBound houtBound hdisj)]
  rw [haddr0, byteAt_idx_inPlace ptr robytes i hi, execBlock_nil]

private theorem execBlock_lbu_rw_subInPlace (outPtr : Word) (rf : RegFile)
    (ws aBytes orig : List (BitVec 8)) (i : Nat) (hi : i ≤ 31)
    (haddr : rf.get .x28 = outPtr + BitVec.ofNat 64 (31 - i))
    (hws : ws = (subBorrowState aBytes orig orig i).1)
    (hlen : orig.length = 32) :
  ...
  rw [execBlock_cons, execInstrRF_lbu_byte _ _ _ _ _ _ _ (31 - i)
    (by
      rw [haddr, show signExtend12 (0 : BitVec 12) = (0 : Word) from by decide]
      have := add_idx_sub_self_inPlace outPtr i hi
      bv_omega)
    (by
      rw [hws, subBorrowState_length, hlen]
      omega), execBlock_nil]
  rw [hws, subBorrowState_unprocessed_in_place aBytes orig i (31 - i) (by omega)]

private theorem readLbuRwSubInPlace_blockVCs (outPtr : Word) (rf : RegFile)
  ...
  · unfold Region.loadOk
    ...
    rw [add_idx_sub_self_inPlace outPtr i hi, hws]
    exact ⟨one_dvd _, by omega⟩
  · unfold inRw
    rw [add_idx_sub_self_inPlace outPtr i hi, hws]
    omega

private theorem subStore_blockVCs_inPlace (outPtr : Word) (rf : RegFile)
    (ws : List (BitVec 8)) (i : Nat) (hi : i ≤ 31)
...
private theorem andi255_truncate8_inPlace (x : Word) :
...
private theorem subStore_effect_inPlace (outPtr : Word) (rf : RegFile)
    (ws : List (BitVec 8)) (i : Nat) (hi : i ≤ 31) (a b : BitVec 8) (borrow : Word)
    ...
    let r := execBlock Region.empty outPtr rf ws
      [.SUB .x30 .x30 .x31, .SUB .x30 .x30 .x6,
       .SLT .x6 .x30 .x0, .ANDI .x30 .x30 (255 : BitVec 12),
       .SB .x29 .x30 (0 : BitVec 12)]
    r.1.get .x10 = rf.get .x10 ∧ r.1.get .x11 = rf.get .x11 ∧
    r.1.get .x12 = rf.get .x12 ∧ r.1.get .x5 = rf.get .x5 ∧
  ...
  rw [execBlock_cons, execInstrRF_sb_byte _ _ _ _ _ _ _ (31 - i) (by
    ...
    rw [show outPtr + BitVec.ofNat 64 (31 - i) + (0 : Word) =
        outPtr + BitVec.ofNat 64 (31 - i) by bv_omega]
    exact add_idx_sub_self_inPlace outPtr i hi)]
  rw [execBlock_nil]
  ...
  rw [andi255_truncate8_inPlace]

private theorem u256SubBeInPlaceBefore_effect (aPtr outPtr : Word)
    (aBytes orig : List (BitVec 8)) (i : Nat) :
    ∀ rf' ws' A',
      sp Region.empty ⟨outPtr, 32⟩
        (u256SubBeInPlaceBefore aPtr outPtr aBytes orig)
        (u256SubBeInPlaceInv aPtr outPtr aBytes orig i) rf' ws' A' →
      rf'.get .x10 = aPtr ∧ rf'.get .x11 = outPtr ∧ rf'.get .x12 = outPtr ∧
      rf'.get .x5 = BitVec.ofNat 64 (31 - i) ∧
      rf'.get .x6 = (subBorrowState aBytes orig orig (i + 1)).2 ∧
      ws' = (subBorrowState aBytes orig orig (i + 1)).1 ∧
      i ≤ 31 ∧ aBytes.length = 32 ∧ orig.length = 32 ∧
      aPtr.toNat + 32 < 2 ^ 64 ∧ outPtr.toNat + 32 < 2 ^ 64 ∧
      (aPtr.toNat + 32 ≤ outPtr.toNat ∨ outPtr.toNat + 32 ≤ aPtr.toNat) ∧
      A' = bytesRegion aPtr aBytes := by
  intro rf' ws' A' hsp
  unfold u256SubBeInPlaceBefore at hsp
  obtain ⟨rfS, wsS, hwsS, hreachB, hrf', hws'⟩ := hsp
  obtain ⟨rfB0, wsB0, hwsB0, hreachA, hrfB, hwsB⟩ := hreachB
  ...
  dsimp only [u256SubBeInPlaceFn] at hlenARead hrf0 hws0eq hrfA hwsA hrfB hwsB hrf' hws'
  ...
  have hreadA : execBlock { base := rfA0.get .x10, bytes := robA } outPtr rfA0 wsA0
    ...
    rw [hrobA]
    apply execBlock_lbu_ro_idx_inPlace
    · exact hi
    · rw [haddrA, hptrA]
    · exact hlenARead
    · rw [hptrA]
  ...
  have hwsAeq : wsB0 = wsA0 := by
    rw [hwsA, execBlock_lbu_ws_inPlace]
  have haddrB : rfB0.get .x28 = outPtr + BitVec.ofNat 64 (31 - i) := by
    rw [hrfA, hreadA, RegFile.get_set_ne _ _ _ _ (by decide : Reg.x28 ≠ .x30), hrf0]
    simp only [execBlock_cons, execBlock_nil, execInstrRF, aluSem,
      RegFile.get_set_ne, RegFile.get_set_self, ne_eq, reduceCtorEq,
      not_false_eq_true, hx11, hx5]
  have hreadB : execBlock Region.empty outPtr rfB0 wsB0
      [.LBU .x31 .x28 (0 : BitVec 12)] =
      (rfB0.set .x31 ((orig.getD (31 - i) 0).zeroExtend 64), wsB0) := by
    apply execBlock_lbu_rw_subInPlace outPtr rfB0 wsB0 aBytes orig i hi haddrB
    rw [hwsAeq, hws0eq]
    simp only [execBlock_cons, execBlock_nil, execInstrRF, aluSem]
    exact hwsState
    exact hlenO
  have hwsSeq : wsS = (subBorrowState aBytes orig orig i).1 := by
  ...
  have hx6S : rfS.get .x6 = (subBorrowState aBytes orig orig i).2 := by
  ...
  have hsub := subStore_effect_inPlace outPtr rfS wsS i hi
    (aBytes.getD (31 - i) 0) (orig.getD (31 - i) 0)
    (subBorrowState aBytes orig orig i).2 hx30S hx31S hx6S hx29S
  dsimp only at hsub
  ...
  refine ⟨?_, ?_, ?_, ?_, ?_, ?_, hi, hlenA, hlenO, hplA, hplO,
    hdisjA, hAfinal⟩
  ...
  · rw [hrf', hsx5, hrfB, hreadB,
    ...
    simp only [execBlock_cons, execBlock_nil, execInstrRF, aluSem,
      RegFile.get_set_ne, ne_eq, reduceCtorEq, not_false_eq_true]
    exact hx5
  · rw [hrf', hsx6, subBorrowState_succ]
  · rw [hws', hsws, hwsSeq, subBorrowState_succ]
...
theorem u256SubBeInPlace_spec (aPtr outPtr : Word)
  ...
  case u256SubBeInPlace.loop.inv_init =>
    rintro rf ws A ⟨rf₀, ws₀, hws₀, hpre, hrf, hws⟩
    obtain ⟨hx10, hx11, hx12, hwsOrig, hlenA, hlenO, hplA, hplO,
      hdisjA, hA⟩ := hpre
    subst hrf
    subst hws
    unfold u256SubBeInPlaceInv
    simp only [u256SubBeInPlaceFn, execBlock_cons, execBlock_nil, execInstrRF, aluSem]
    ...
    · rw [RegFile.get_set_self _ _ _ (by decide)]
      rfl
  case u256SubBeInPlace.loop.inv_step =>
    rintro i hiLt rf' ws' A' hsp
    obtain ⟨rfa, wsa, hwsa, ⟨hspbb, _hnbreak⟩, hrf', hws'⟩ := hsp
    have hb := u256SubBeInPlaceBefore_effect aPtr outPtr aBytes orig i rfa wsa A' hspbb
  ...
  case u256SubBeInPlace.loop.exhausted =>
    rintro rf' ws' A' hspbb
    have hb := u256SubBeInPlaceBefore_effect aPtr outPtr aBytes orig 31 rf' ws' A' hspbb
    obtain ⟨_, _, _, hx5, _⟩ := hb
    simp only [Cond.holds]
    rw [hx5]
    rfl
  case u256SubBeInPlace.loop.break =>
    rintro i hi rf' ws' A' hspbb hbreak
    have hb := u256SubBeInPlaceBefore_effect aPtr outPtr aBytes orig i rf' ws' A' hspbb
    ...
    have hi31 : i = 31 := by
      ...
      rw [idx_toNat_inPlace i hik] at hto
      change 31 - i = 0 at hto
      omega
    subst hi31
    unfold u256SubBeInPlaceLoopPost u256SubBeBorrow u256SubBeBytes
    refine ⟨hx10, hx11, hx12, ?_, hx6, hwsState, hA⟩
    rw [hx5]
    rfl
  case u256SubBeInPlace.loop.before.readA.focus =>
    rintro rf ws A hreach hApc hp hhp
    obtain ⟨rf₀, ws₀, hws₀, ⟨i, hi, hinv⟩, hrf, hws⟩ := hreach
    obtain ⟨hx10, hx11, hx12, hx5, hx6, hwsState, hik, hlenA, hlenO,
      hplA, hplO, hdisjA, hA⟩ := hinv
    dsimp only [u256SubBeInPlaceFn] at hrf hws hws₀
    subst hrf
    subst hws
    simp only [execBlock_cons, execBlock_nil, execInstrRF, aluSem,
      RegFile.get_set_ne, ne_eq, reduceCtorEq, not_false_eq_true] at hhp ⊢
    refine ⟨aBytes, empAssertion, ⟨?_, rfl, rfl⟩, ?_,
      pcFree_emp, ?_⟩
    · exact hx10
    · rw [hA] at hhp
  ...
  case u256SubBeInPlace.loop.before.readA.mem =>
    ...
    exact readLbu_blockVCs_inPlace (rf₀.get .x10) outPtr _ ws robytes .x30 .x7 i hi haddr hws32
      (by rw [hrob]; exact hlenA) (by rw [hx10]; exact hplA) hplO
      (by rw [hx10]; exact hdisjA)
  case u256SubBeInPlace.loop.before.readB.mem =>
    ...
    obtain ⟨hx10, hx11, hx12, hx5, hx6, hwsState, hik, hlenA, hlenO,
      hplA, hplO, hdisjA, hA⟩ := hinv
    dsimp only [u256SubBeInPlaceFn] at hrfA hrf0 hwsA hws0 hlenARead hws₀ hws ⊢
    have hws32 : ws.length = 32 := hws
    have haddr : rf.get .x28 = rf.get .x11 + BitVec.ofNat 64 (31 - i) := by
      rw [hrfA,
        execBlock_lbu_get_ne_inPlace _ _ _ _ .x30 .x7 (0 : BitVec 12) .x28 (by decide),
        execBlock_lbu_get_ne_inPlace _ _ _ _ .x30 .x7 (0 : BitVec 12) .x11 (by decide),
        hrf0]
      simp only [execBlock_cons, execBlock_nil, execInstrRF, aluSem,
        RegFile.get_set_ne, RegFile.get_set_self, ne_eq, reduceCtorEq,
        not_false_eq_true, hx11, hx5]
    have hx11r : rf.get .x11 = outPtr := by
      rw [hrfA,
        execBlock_lbu_get_ne_inPlace _ _ _ _ .x30 .x7 (0 : BitVec 12) .x11
          (by decide), hrf0]
      simp only [execBlock_cons, execBlock_nil, execInstrRF, aluSem,
    ...
    have haddr' : rf.get .x28 = outPtr + BitVec.ofNat 64 (31 - i) := by
      rw [haddr, hx11r]
    exact readLbuRwSubInPlace_blockVCs outPtr rf ws i hi haddr' hws32
  case u256SubBeInPlace.loop.before.subStore.mem =>
    rintro rf ws A hws hreach
    obtain ⟨rfB, wsB, hwsB0, hreachA, hrfB, hwsB⟩ := hreach
    obtain ⟨rfA, wsA, AA, robA, restA, hlenARead, hreach0, _hsatA,
      hroArel, hrfA, hwsA, hAeqA⟩ := hreachA
    obtain ⟨rf₀, ws₀, hws₀, ⟨i, hi, hinv⟩, hrf0, hws0⟩ := hreach0
    obtain ⟨hx10, hx11, hx12, hx5, hx6, hwsState, hik, hlenA, hlenO,
      hplA, hplO, hdisjA, hA⟩ := hinv
    dsimp only [u256SubBeInPlaceFn] at hrf0 hws0 hlenARead hrfA hrfB hwsA hwsB hws₀ hws ⊢
    have hws32 : ws.length = 32 := hws
    have hx29 : rf.get .x29 = outPtr + BitVec.ofNat 64 (31 - i) := by
      rw [hrfB,
        execBlock_lbu_get_ne_inPlace _ _ _ _ .x31 .x28 (0 : BitVec 12) .x29 (by decide),
        hrfA, execBlock_lbu_get_ne_inPlace _ _ _ _ .x30 .x7 (0 : BitVec 12) .x29
        (by decide), hrf0]
      simp only [execBlock_cons, execBlock_nil, execInstrRF, aluSem,
        RegFile.get_set_ne, RegFile.get_set_self, ne_eq, reduceCtorEq,
        not_false_eq_true, hx12, hx5]
    exact subStore_blockVCs_inPlace outPtr rf ws i hi hx29 hws32
  case u256SubBeInPlace.post =>
    intro rf ws A h
    exact u256SubBeInPlace_retVal_post aPtr outPtr aBytes orig rf ws A h
...
/-! ## Linked exact-alias adapter

K73 passes the output pointer in both the subtrahend and destination
registers.  This adapter exposes that exact alias at the linked entry while
retaining the general two-input contract above unchanged.
-/

def u256SubBeInPlaceCr : CodeReq :=
...
private theorem exposedRegs_split_u256Sub (vf : Reg → Word) :
...
private theorem u256Sub_args_notin_scratch :
    ∀ r ∈ u256SubBeInPlaceScratch,
...
theorem u256SubBeInPlaceFlat_spec (ret aPtr outPtr : Word)
    ...
    cpsTripleWithin
      ...
      (GuestAddrs.u256_sub_be : Word) ret u256SubBeInPlaceCr
      ...
      (((.x1 : Reg) ↦ᵣ ret) **
        (.x10 ↦ᵣ u256SubBeBorrow aBytes orig orig) **
        (.x11 ↦ᵣ outPtr) ** (.x12 ↦ᵣ outPtr) **
  ...
  have had := Fn.retSpecFlatAmbient
    ...
    (u256SubBeInPlace_spec aPtr outPtr aBytes orig hrw hroA
      (GuestAddrs.u256_sub_be : Word))
    hsz ret halign
    (fun r => if r = .x10 then aPtr else
      if r = .x11 then outPtr else if r = .x12 then outPtr else vf r)
    ...
    (fun rf' ws' _hlen hpost hp hh => by
      ...
      rw [regFileIs_eq_regAtoms, regAtoms_eq_regAtomsOf _ _ (by decide),
        exposedRegs_split_u256Sub, g10, g11, g12] at hh
      rw [show (u256SubBeInPlaceFn aPtr outPtr aBytes orig).rw.base = outPtr
        from rfl] at hh
      have hh1 :
          (((((((.x10 : Reg) ↦ᵣ u256SubBeBorrow aBytes orig orig) **
            (.x11 ↦ᵣ outPtr)) ** (.x12 ↦ᵣ outPtr)) **
            bytesRegion outPtr (u256SubBeBytes aBytes orig orig)) **
            bytesRegion aPtr aBytes) **
            regAtomsOf (fun r => rf' r) u256SubBeInPlaceScratch) hp := by
        xperm_hyp hh
  ...
  rw [regFileIs_eq_regAtoms, regAtoms_eq_regAtomsOf _ _ (by decide),
    ...
    regAtomsOf_congr
      ...
      (fun r hr => by
        obtain ⟨h10, h11, h12⟩ := u256Sub_args_notin_scratch r hr
