/-
  EvmAsm.Codegen.Programs.RlpEncodeListPrefixLong6Spec

  **`rlp_encode_list_prefix`, 6-length-byte long form** (GH #10780 item 3, width 6).
  ...
  adds `1099511627776 ≤ len < 281474976710656`: header byte `0xFD` followed by the
  6 big-endian length bytes, header length 7.

  ## What is different from the long5 arm — exactly two things

  1. **The lenlen ladder falls one branch further.** In long5 the
     `BLTU x10, 1099511627776` at idx22 (`base+88`) is *taken* and
     jumps straight to the shared header writer at idx30 (`base+120`). Here it is **not**
     taken, so control falls into idx23–idx25
     (`base+92`, `base+96`, `base+100`),
     where `x28 := 6` and `x29 := 1099511627776 <<< 8` before
     `BLTU x10, 281474976710656` (idx25, offset `+20`) jumps to
     `base+120`. That is three extra dispatch steps over long5 — 20 ladder
     steps rather than 17 — and it is the only genuinely per-width part of the
     arm: the routine computes `x28` by falling through `k` branches, so the ladder is
     inherently eight cases.
  2. **The loop is cited at `m := 6`.** `RlpEncodeListPrefixLoopSpec.lpLolLoop` covers
     idx35–idx41 whole, at a symbolic trip count `m`, in `7 * m + 1` steps with
     postcondition `writeShift dst di len.toNat m`. Long5 instantiates it at
     `m := 5`; this arm instantiates it at `m := 6`, `di := 1`, so the loop
     contributes **43 steps** instead of 36. Nothing about the loop
  ...
  ## Step count 72, derived from the program

  `20` to reach idx30 (idx 0, 1, 8–25)
  `+ 5` for idx30–34 `+ 43` for the loop (`7*6+1`) `+ 3` for idx42–44 `+ 1`
  for the `JALR` `= 72`. Long4's 52 is the same accounting with 14 ladder steps and
  a 29-step loop.

  ## Byte order, read off the loop rather than assumed

  `x29` starts at `lenlen - 1 = 5` and counts **down**, while `x30` starts at
  `outPtr + 1` and counts **up**. So the iterations store `len >>> 40`, …,
  `len` at `out[1]` … `out[6]` — big-endian, most significant first, matching RLP. As in
  ...
  `u64ByteLen`'s own bounds, and `long6_first_length_byte_ne_zero` below is its
  `lenlen = 6` instance.

  ## The overflow side condition, checked rather than assumed

  `lpLolLoop` requires `outPtr.toNat + (di + m) ≤ 2 ^ 64`, which at `di := 1, m := 6` is
  `outPtr.toNat + 7 ≤ 2 ^ 64`. The alignment hypothesis `outPtr.toNat % 8 = 0`
...
namespace RlpEncodeListPrefixLong6Spec
...
/-! ## The loop's output, in the long4 postcondition's shape
    ...
    `lpLolLoop` hands back `writeShift outBytes 1 len.toNat 6`. The theorem below states
    the region as 6 nested `List.set`s instead, exactly as long4 does with four, so the
...
/-- ⭐ **6 length bytes at index 1, spelled as `List.set`s.** Each index is
    tied to its own shift: `out[1]` to `len >>> 40`, … `out[6]` to `len`.
...
private theorem writeShift_six (dst : List Byte) (len : Word) :
    writeShift dst 1 len.toNat 6
      =
        ((((((dst.set 1 (BitVec.ofNat 8 (len >>> (40 : Nat)).toNat)).set 2
            (BitVec.ofNat 8 (len >>> (32 : Nat)).toNat)).set 3
            (BitVec.ofNat 8 (len >>> (24 : Nat)).toNat)).set 4
            (BitVec.ofNat 8 (len >>> (16 : Nat)).toNat)).set 5
            (BitVec.ofNat 8 (len >>> (8 : Nat)).toNat)).set 6
            (BitVec.ofNat 8 len.toNat)) := by
  have hstep : writeShift dst 1 len.toNat 6
      =
        ((((((dst.set 1 (BitVec.ofNat 8 (len.toNat / 256 ^ 5 % 256))).set 2
            (BitVec.ofNat 8 (len.toNat / 256 ^ 4 % 256))).set 3
            (BitVec.ofNat 8 (len.toNat / 256 ^ 3 % 256))).set 4
            (BitVec.ofNat 8 (len.toNat / 256 ^ 2 % 256))).set 5
            (BitVec.ofNat 8 (len.toNat / 256 ^ 1 % 256))).set 6
...
/-- **The first length byte is nonzero**, at this arm's width.
    ...
    contains, and this is its `lenlen = 6` instance — the same relationship
...
theorem long6_first_length_byte_ne_zero {len : Word}
    (h_lo : 1099511627776 ≤ len.toNat) (h_hi : len.toNat < 281474976710656) :
    BitVec.ofNat 8 (len >>> (40 : Nat)).toNat ≠ 0 := by
  have hwidth : u64ByteLen len = 6 := by
    unfold u64ByteLen
    split_ifs <;> omega
  have h := RlpEncodeListPrefixCanonical.first_length_byte_ne_zero (len := len) (by omega)
  rwa [hwidth, show 8 * (6 - 1) = 40 from by norm_num] at h

/-! ## The triple -/

/-- **`rlp_encode_list_prefix`, 6-length-byte long form** (`1099511627776 ≤ len <
    281474976710656`): writes the header bytes `[0xFD, len >>> 40, len >>> 32, len >>> 24,
    len >>> 16, len >>> 8, len]` and header length 7, returns `a0 = 0`; scratch registers
    pinned.

    Clobbers `t0`/`t3`–`t6` (`x5`, `x28`–`x31`) as the long1–long4 arms do; `x6`/`x7`
    are untouched on this path and so do not appear.

    The 6 length bytes are pinned to their own shifts because the loop counts `x29`
    down while counting `x30` up, and a symmetric statement would not detect the cursors
    being swapped. Together with `long6_first_length_byte_ne_zero`, `out[1]` is
    ...
    `m := 6`, `di := 1` — 43 of the 72 steps — rather than unrolled per
    iteration. -/
theorem rlp_encode_list_prefix_long6_pinned_spec_within
    ...
    cpsTripleWithin 72 base (raVal &&& ~~~1)
      ...
      (((.x1 : Reg) ↦ᵣ raVal) ** ((.x10 : Reg) ↦ᵣ (0 : Word)) **
       ...
       ((.x0 : Reg) ↦ᵣ (0 : Word)) **
       bytesRegion outPtr
         (((((((outBytes.set 0 (0xFD : BitVec 8)).set 1
             (BitVec.ofNat 8 (len >>> (40 : Nat)).toNat)).set 2
             (BitVec.ofNat 8 (len >>> (32 : Nat)).toNat)).set 3
             (BitVec.ofNat 8 (len >>> (24 : Nat)).toNat)).set 4
             (BitVec.ofNat 8 (len >>> (16 : Nat)).toNat)).set 5
             (BitVec.ofNat 8 (len >>> (8 : Nat)).toNat)).set 6
  ...
  -- idx25 (base+100): BLTU x10, x29, +20 — TAKEN (len < 281474976710656) → base+120
  ...
  have hult25 : BitVec.ult len (281474976710656 : Word) :=
    ult_of_toNat_lt
      (by rw [show ((281474976710656 : Word)).toNat = 281474976710656 from by decide]; omega)
  have ht25 := cpsBranchWithin_takenStripPure2 hbr25 (fun hp hQf => by
    obtain ⟨_, _, _, _, _, hQ⟩ := hQf
    exact ((sepConj_pure_right _).1 hQ).2 hult25)
  -- idx30 (base+120): ADDI x29, x28, 247 — x29 := 0xFD
  have ha30 := liftCode (cr' := CR)
    (addi_spec_gen_within .x29 .x28 (281474976710656 : Word) (6 : Word)
      (247 : BitVec 12) (base + 120) (by decide))
    (by rw [hCR]; cmem 30)
  rw [show (base + 120 : Word) + 4 = base + 124 from by bv_omega,
      show (6 : Word) + signExtend12 (247 : BitVec 12) = (253 : Word) from by
        decide] at ha30
  -- idx31 (base+124): SB x11, x29 — out[0] := 0xFD
  have hsb31 := liftCode (cr' := CR)
    (bytesRegion_sb_within .x11 .x29 outPtr (253 : Word) (base + 124) outBytes 0
  ...
  rw [show outPtr + BitVec.ofNat 64 0 = outPtr from by bv_omega,
      show ((253 : Word)).truncate 8 = (0xFD : BitVec 8) from by decide,
  ...
  -- idx34 (base+136): ADDI x29, x28, -1 — x29 := 5 = lenlen - 1
  have ha34 := liftCode (cr' := CR)
    (addi_spec_gen_within .x29 .x28 (253 : Word) (6 : Word)
      (-1 : BitVec 12) (base + 136) (by decide))
    (by rw [hCR]; cmem 34)
  rw [show (base + 136 : Word) + 4 = base + 140 from by bv_omega,
      show (6 : Word) + signExtend12 (-1 : BitVec 12) = (5 : Word) from by
        decide] at ha34
  -- ══ idx35–41 (base+140 → base+168): the length-byte loop, cited whole at m := 6 ══
  have hloop := lpLolLoop base outPtr v31 (56 : Word) len
    (outBytes.set 0 (0xFD : BitVec 8)) 1 6 (by omega) h_out_align
    (by rw [List.length_set]; omega)
    (by have := outPtr.isLt; omega)
    (fun k hk => h_out_valid (1 + k) (by omega))
  rw [show 7 * 6 + 1 = 43 from by norm_num,
      show BitVec.ofNat 64 6 - 1 = (5 : Word) from by decide,
      show (1 : Nat) + 6 = 7 from by norm_num,
      writeShift_six (outBytes.set 0 (0xFD : BitVec 8)) len] at hloop
  -- idx42 (base+168): ADDI x30, x28, 1 — x30 := 7 (header length)
  have ha42 := liftCode (cr' := CR)
    (addi_spec_gen_within .x30 .x28 (outPtr + BitVec.ofNat 64 7) (6 : Word)
      (1 : BitVec 12) (base + 168) (by decide))
    (by rw [hCR]; cmem 42)
  rw [show (base + 168 : Word) + 4 = base + 172 from by bv_omega,
      show (6 : Word) + signExtend12 (1 : BitVec 12) = (7 : Word) from by
        decide] at ha42
  -- idx43 (base+172): SD x12, x30 — *cell := 7
  have hsd := liftCode (cr' := CR)
    (sd_spec_within .x12 .x30 cellPtr (7 : Word) cellOld (0 : BitVec 12)
      (base + 172))
    (by rw [hCR]; cmem 43)
  ...
  have hsll_6F := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ raVal) ** ((.x10 : Reg) ↦ᵣ len) **
     ...
     bytesRegion outPtr outBytes ** (cellPtr ↦ₘ cellOld))
    (by pcf) hsll_6
  have ht25F := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ raVal) ** ((.x11 : Reg) ↦ᵣ outPtr) ** ((.x12 : Reg) ↦ᵣ cellPtr) **
     ((.x5 : Reg) ↦ᵣ (56 : Word)) ** ((.x28 : Reg) ↦ᵣ (6 : Word)) **
     ((.x30 : Reg) ↦ᵣ v30) ** ((.x31 : Reg) ↦ᵣ v31) **
     ((.x0 : Reg) ↦ᵣ (0 : Word)) **
  ...
  have hmvF := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ raVal) ** ((.x10 : Reg) ↦ᵣ len) ** ((.x12 : Reg) ↦ᵣ cellPtr) **
     ((.x5 : Reg) ↦ᵣ (56 : Word)) ** ((.x28 : Reg) ↦ᵣ (6 : Word)) **
     ((.x29 : Reg) ↦ᵣ (253 : Word)) ** ((.x31 : Reg) ↦ᵣ v31) **
     ((.x0 : Reg) ↦ᵣ (0 : Word)) **
     bytesRegion outPtr (outBytes.set 0 (0xFD : BitVec 8)) ** (cellPtr ↦ₘ cellOld))
    (by pcf) hmv
  ...
  have hloopF := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ raVal) ** ((.x11 : Reg) ↦ᵣ outPtr) ** ((.x12 : Reg) ↦ᵣ cellPtr) **
     ((.x28 : Reg) ↦ᵣ (6 : Word)) ** (cellPtr ↦ₘ cellOld))
    (by pcf) hloop
  ...
  have hli10F := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ raVal) ** ((.x11 : Reg) ↦ᵣ outPtr) ** ((.x12 : Reg) ↦ᵣ cellPtr) **
     regOwn .x5 ** ((.x28 : Reg) ↦ᵣ (6 : Word)) **
     ((.x29 : Reg) ↦ᵣ (-1 : Word)) ** ((.x30 : Reg) ↦ᵣ (7 : Word)) **
     regOwn .x31 **
     ((.x0 : Reg) ↦ᵣ (0 : Word)) **
     bytesRegion outPtr
       (((((((outBytes.set 0 (0xFD : BitVec 8)).set 1
  ...
  have hretF := cpsTripleWithin_frameR
    (((.x10 : Reg) ↦ᵣ (0 : Word)) ** ((.x11 : Reg) ↦ᵣ outPtr) **
     ...
     (cellPtr ↦ₘ (7 : Word)))
    (by pcf) hret
  -- ══ compose: 20 ladder + 5 header + 43 loop + 3 epilogue + 1 ret = 72 ══
  ...
  have hc19 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc18 ht25F
  have hc20 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc19 ha30F
  have hc21 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc20 hsb31F
  have hc22 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc21 hmvF
  have hc23 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc22 ha33F
  have hc24 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc23 ha34F
  have hc25 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc24 hloopF
  have hc26 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc25 ha42F
  have hc27 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc26 hsdF
  have hc28 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc27 hli10F
  have hc29 := cpsTripleWithin_seq_perm_same_cr (fun _ hp => by xperm_hyp hp) hc28 hretF
  refine cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp) (fun h hq => ?_) hc29
  have hq1 : (((.x28 : Reg) ↦ᵣ (6 : Word)) **
      (((.x29 : Reg) ↦ᵣ (-1 : Word)) **
       (((.x30 : Reg) ↦ᵣ (7 : Word)) **
        (((.x1 : Reg) ↦ᵣ raVal) ** ((.x10 : Reg) ↦ᵣ (0 : Word)) **
         ((.x11 : Reg) ↦ᵣ outPtr) ** ((.x12 : Reg) ↦ᵣ cellPtr) **
...
end RlpEncodeListPrefixLong6Spec
