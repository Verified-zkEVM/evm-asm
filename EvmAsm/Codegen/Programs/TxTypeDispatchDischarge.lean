/-
  ...
  Teer residual 11→10: pin entry = GuestAddrs.tx_type_dispatch, nSteps = 256,
...
/-- prover1-shaped assumed contract (augmented: regOwn a1–a3). Local until
    `TeerBodyAssumptions` lands on main; field shapes match. -/
...
def txTypeDispatchAssumed_of_spec (cr : CodeReq)
  ...
  flat := fun ret txBase txLen typePtr innerPtr t0Old t1Old typeOld innerOld txBytes
      hret hlen halign hover hvalid0 => by
    have h0 :=
      txTypeDispatch_flat_under_typeCode ret txBase txLen typePtr innerPtr
        t0Old t1Old typeOld innerOld txBytes hret hlen halign hover hvalid0
