/-
  Wire teerAuthContent_applied as free20→PostEx hrun for free26 packaging.
-/
...
/-- `teerAuthContent_applied` post is `teerAuthContentAppliedPostEx` (s7Old:=s7). -/
theorem teerAuthContent_applied_as_postEx
...
(hsuccess : (teerTxTypeDispatch (txSlice bs off len)).1 = (0 : Word))
    ...
    cpsTripleWithin nFrontToAtListCount
      ...
      (teerAuthContentAppliedPostEx spVal spC loadPtr lenW balPtr balLenW chainIdW
        s7 cursorV endW s11 s innerVal endL regionBase bs balBytes srcOffA9) := by
  intro s innerVal listLen endL endW cursorV
  have h :=
    teerAuthContent_applied
  ...
  exact cpsTripleWithin_weaken (fun _ hp => hp) (fun _ hq => hq) h

#print axioms teerAuthContent_applied_as_postEx

theorem teerAuthContent_applied_as_postEx_is
    (ret spVal spC loadPtr lenW balPtr balLenW chainIdW baiW : Word)
    (s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 : Word)
    (regionBase : Word) (bs balBytes : List (BitVec 8)) (off len : Nat)
    (hspC : spC = spVal + signExtend12 (-160 : BitVec 12))
...
#print axioms teerAuthContent_applied_as_postEx_is
