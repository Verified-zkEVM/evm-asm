...
theorem cnt_zero : cnt 64 = (0 : Word) := by decide
...
theorem cnt_ne_zero : ∀ j, j < 64 → cnt j ≠ (0 : Word) := by
  intro j h h0
...
theorem lcnt_zero : lcnt 6 = (0 : Word) := by decide
...
theorem lcnt_ne_zero : ∀ j < 6, lcnt j ≠ (0 : Word) := by
  intro j hj h0
