/-
  ...
  fact through:
    ...
    entry            (premise-free)                  K73 .. K73 + 88
    mul call/status  (deployed mul callee)           K73 + 88 .. K73 + 96
    div pair         (premise-free, htargetPos)      K73 + 96 .. K73 + 128
    is_zero call     (strengthened: result valued)   K73 + 128 .. K73 + 140
    zero branch      (facts: raw = 0 / raw <> 0)     K73 + 140 .. K73 + 176
    add chain + tails                                K73 + 176 .. raIn
...
private theorem k73_incr_first_routeB
  ...
  · -- failure disjunct (x10 = 1)
    ...
    have hEq2 : (((.x2 : Reg) ↦ᵣ wspH) ** (((.x10 : Reg) ↦ᵣ (1 : Word)) ** ((regOwn .x11) ** ((regOwn .x12) ** ((bytesRegion Expected (U256AddBeSAsm.u256AddBeBytes parentBytes q2 q2)) ** (((.x1 : Reg) ↦ᵣ (H + 40)) ** (((.x8 : Reg) ↦ᵣ headerPtr) ** (((.x9 : Reg) ↦ᵣ v9) ** (((.x18 : Reg) ↦ᵣ old18) ** (((.x19 : Reg) ↦ᵣ v19) ** (((.x20 : Reg) ↦ᵣ v20) ** ((frameSlotsSaved k73Frame wspK (k73Saved (H + 40) headerPtr v9 old18 v19 v20)) ** (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((regOwns EvmAsm.Codegen.U256AddBeBInPlaceSAsm.u256AddBeBInPlaceScratch) ** ((bytesRegion parentPtr parentBytes) ** ((U256MulU64Be.frameSlots (wspK + signExtend12 (-48 : BitVec 12)) (K73 + 92) parentPtr Expected target (gasUsed - target) (1 : Word)) ** ((bytesRegion U256MulU64Be.accBase A) ** (k73_incr_piggyback wspH old8 headerPtr headerBytes Frest))))))))))))))))))
...
private theorem k73_incr_second_routeB
  ...
  rcases hp with h1 | h0
  · -- failure disjunct (x10 = 1)
    have hEq1 : (((.x2 : Reg) ↦ᵣ wspH) ** ((regsAt k73Frame (k73Saved (H + 40) headerPtr v9 old18 v19 v20)) ** ((frameSlotsSaved k73Frame wspK (k73Saved (H + 40) headerPtr v9 old18 v19 v20)) ** (((.x10 : Reg) ↦ᵣ (1 : Word)) ** (((.x0 : Reg) ↦ᵣ (0 : Word)) ** (((.x11 : Reg) ↦ᵣ Expected) ** (((.x12 : Reg) ↦ᵣ Expected) ** ((regOwns EvmAsm.Codegen.U256AddBeBInPlaceSAsm.u256AddBeBInPlaceScratch) ** ((bytesRegion Expected (U256AddBeSAsm.u256AddBeBytes parentBytes orig orig)) ** ((bytesRegion parentPtr parentBytes) ** ((U256MulU64Be.frameSlots (wspK + signExtend12 (-48 : BitVec 12)) (K73 + 92) parentPtr Expected target (gasUsed - target) (1 : Word)) ** ((bytesRegion U256MulU64Be.accBase A) ** (k73_incr_piggyback wspH old8 headerPtr headerBytes Frest)))))))))))))
...
private theorem k73_incr_carry_routeB_fail
  ...
  intro s hp
  have hEq1 : (k73IncreaseCarryFinalPost wspH wspK (H + 40) gasUsed parentPtr Expected target headerPtr v9 old18 v19 v20 parentBytes A outBytes (k73_incr_piggyback wspH old8 headerPtr headerBytes Frest)) = ((.x2 ↦ᵣ wspH) ** ((.x1 ↦ᵣ (H + 40)) ** ((.x8 ↦ᵣ headerPtr) ** ((.x9 ↦ᵣ v9) ** ((.x18 ↦ᵣ old18) ** ((.x19 ↦ᵣ v19) ** ((.x20 ↦ᵣ v20) ** (frameSlotsSaved k73Frame wspK (k73Saved (H + 40) headerPtr v9 old18 v19 v20) ** ((.x10 ↦ᵣ (1 : Word)) ** ((.x0 ↦ᵣ (0 : Word)) ** (EvmAsm.Codegen.U256MulU64Be.mulTailExtra parentPtr (gasUsed - target) Expected parentBytes ** ((fun u => ∃ k, (U256MulU64Be.frameSlots (wspK + signExtend12 (-48 : BitVec 12)) (K73 + 92) parentPtr Expected target (gasUsed - target) (1 : Word) ** bytesRegion Expected outBytes ** k73MulOverflowCoreNoStatus A k) u) ** (regOwn .x14 ** (regOwn .x15 ** (regOwn .x16 ** (regOwn .x17 ** (k73_incr_piggyback wspH old8 headerPtr headerBytes Frest))))))))))))))))) := by
  ...
  have hCoreEq : (((.x2 ↦ᵣ wspH) ** ((.x10 ↦ᵣ (1 : Word)) ** ((.x0 ↦ᵣ (0 : Word)) ** ((regOwn .x7 ** (regOwn .x11 ** (regOwn .x12 ** (U256MulU64Be.frameSlots (wspK + signExtend12 (-48 : BitVec 12)) (K73 + 92) parentPtr Expected target (gasUsed - target) (1 : Word) ** (bytesRegion Expected outBytes ** (((.x5 ↦ᵣ (EvmAsm.Codegen.U256MulU64Be.accBase + BitVec.ofNat 64 (32 + k))) ** ((.x6 ↦ᵣ BitVec.ofNat 64 (8 - k)) ** (regOwn .x28 ** EvmAsm.Rv64.bytesRegion EvmAsm.Codegen.U256MulU64Be.accBase A))) ** ((.x1 ↦ᵣ (H + 40)) ** ((.x8 ↦ᵣ headerPtr) ** ((.x9 ↦ᵣ v9) ** ((.x18 ↦ᵣ old18) ** ((.x19 ↦ᵣ v19) ** ((.x20 ↦ᵣ v20) ** (frameSlotsSaved k73Frame wspK (k73Saved (H + 40) headerPtr v9 old18 v19 v20) ** (regOwn .x13 ** (regOwn .x29 ** (regOwn .x30 ** (regOwn .x31 ** (bytesRegion parentPtr parentBytes ** (regOwn .x14 ** (regOwn .x15 ** (regOwn .x16 ** (regOwn .x17 ** (k73_incr_piggyback wspH old8 headerPtr headerBytes Frest)))))))))))))))))))))))))))) = (((.x2 ↦ᵣ wspH) ** ((.x10 ↦ᵣ (1 : Word)) ** ((.x0 ↦ᵣ (0 : Word)) ** ((regOwn .x7 ** (regOwn .x11 ** (regOwn .x12 ** (U256MulU64Be.frameSlots (wspK + signExtend12 (-48 : BitVec 12)) (K73 + 92) parentPtr Expected target (gasUsed - target) (1 : Word) ** (bytesRegion Expected outBytes ** ((.x5 ↦ᵣ (EvmAsm.Codegen.U256MulU64Be.accBase + BitVec.ofNat 64 (32 + k))) ** (((.x6 ↦ᵣ BitVec.ofNat 64 (8 - k)) ** (regOwn .x28 ** EvmAsm.Rv64.bytesRegion EvmAsm.Codegen.U256MulU64Be.accBase A)) ** ((.x1 ↦ᵣ (H + 40)) ** ((.x8 ↦ᵣ headerPtr) ** ((.x9 ↦ᵣ v9) ** ((.x18 ↦ᵣ old18) ** ((.x19 ↦ᵣ v19) ** ((.x20 ↦ᵣ v20) ** (frameSlotsSaved k73Frame wspK (k73Saved (H + 40) headerPtr v9 old18 v19 v20) ** (regOwn .x13 ** (regOwn .x29 ** (regOwn .x30 ** (regOwn .x31 ** (bytesRegion parentPtr parentBytes ** (regOwn .x14 ** (regOwn .x15 ** (regOwn .x16 ** (regOwn .x17 ** (k73_incr_piggyback wspH old8 headerPtr headerBytes Frest))))))))))))))))))))))))))))) := by
    xperm_cert_eq
  have hc12u : (((.x2 ↦ᵣ wspH) ** ((.x10 ↦ᵣ (1 : Word)) ** ((.x0 ↦ᵣ (0 : Word)) ** ((regOwn .x7 ** (regOwn .x11 ** (regOwn .x12 ** (U256MulU64Be.frameSlots (wspK + signExtend12 (-48 : BitVec 12)) (K73 + 92) parentPtr Expected target (gasUsed - target) (1 : Word) ** (bytesRegion Expected outBytes ** (((.x5 ↦ᵣ (EvmAsm.Codegen.U256MulU64Be.accBase + BitVec.ofNat 64 (32 + k))) ** ((.x6 ↦ᵣ BitVec.ofNat 64 (8 - k)) ** (regOwn .x28 ** EvmAsm.Rv64.bytesRegion EvmAsm.Codegen.U256MulU64Be.accBase A))) ** ((.x1 ↦ᵣ (H + 40)) ** ((.x8 ↦ᵣ headerPtr) ** ((.x9 ↦ᵣ v9) ** ((.x18 ↦ᵣ old18) ** ((.x19 ↦ᵣ v19) ** ((.x20 ↦ᵣ v20) ** (frameSlotsSaved k73Frame wspK (k73Saved (H + 40) headerPtr v9 old18 v19 v20) ** (regOwn .x13 ** (regOwn .x29 ** (regOwn .x30 ** (regOwn .x31 ** (bytesRegion parentPtr parentBytes ** (regOwn .x14 ** (regOwn .x15 ** (regOwn .x16 ** (regOwn .x17 ** (k73_incr_piggyback wspH old8 headerPtr headerBytes Frest)))))))))))))))))))))))))))) s := hc12
  have hc12b : (((.x2 ↦ᵣ wspH) ** ((.x10 ↦ᵣ (1 : Word)) ** ((.x0 ↦ᵣ (0 : Word)) ** ((regOwn .x7 ** (regOwn .x11 ** (regOwn .x12 ** (U256MulU64Be.frameSlots (wspK + signExtend12 (-48 : BitVec 12)) (K73 + 92) parentPtr Expected target (gasUsed - target) (1 : Word) ** (bytesRegion Expected outBytes ** ((.x5 ↦ᵣ (EvmAsm.Codegen.U256MulU64Be.accBase + BitVec.ofNat 64 (32 + k))) ** (((.x6 ↦ᵣ BitVec.ofNat 64 (8 - k)) ** (regOwn .x28 ** EvmAsm.Rv64.bytesRegion EvmAsm.Codegen.U256MulU64Be.accBase A)) ** ((.x1 ↦ᵣ (H + 40)) ** ((.x8 ↦ᵣ headerPtr) ** ((.x9 ↦ᵣ v9) ** ((.x18 ↦ᵣ old18) ** ((.x19 ↦ᵣ v19) ** ((.x20 ↦ᵣ v20) ** (frameSlotsSaved k73Frame wspK (k73Saved (H + 40) headerPtr v9 old18 v19 v20) ** (regOwn .x13 ** (regOwn .x29 ** (regOwn .x30 ** (regOwn .x31 ** (bytesRegion parentPtr parentBytes ** (regOwn .x14 ** (regOwn .x15 ** (regOwn .x16 ** (regOwn .x17 ** (k73_incr_piggyback wspH old8 headerPtr headerBytes Frest))))))))))))))))))))))))))))) s := by
    have h := hc12u
    rw [hCoreEq] at h
    exact h
  have hc5 := (EvmAsm.Codegen.HeaderValidateBaseFeeCompositionDecreaseRoute.decr_under_id (B := (.x2 ↦ᵣ wspH)) (EvmAsm.Codegen.HeaderValidateBaseFeeCompositionDecreaseRoute.decr_under_id (B := (.x10 ↦ᵣ (1 : Word))) (EvmAsm.Codegen.HeaderValidateBaseFeeCompositionDecreaseRoute.decr_under_id (B := (.x0 ↦ᵣ (0 : Word))) (EvmAsm.Codegen.HeaderValidateBaseFeeCompositionDecreaseRoute.decr_under_id (B := regOwn .x7) (EvmAsm.Codegen.HeaderValidateBaseFeeCompositionDecreaseRoute.decr_under_id (B := regOwn .x11) (EvmAsm.Codegen.HeaderValidateBaseFeeCompositionDecreaseRoute.decr_under_id (B := regOwn .x12) (EvmAsm.Codegen.HeaderValidateBaseFeeCompositionDecreaseRoute.decr_under_id (B := U256MulU64Be.frameSlots (wspK + signExtend12 (-48 : BitVec 12)) (K73 + 92) parentPtr Expected target (gasUsed - target) (1 : Word)) (EvmAsm.Codegen.HeaderValidateBaseFeeCompositionDecreaseRoute.decr_under_id (B := bytesRegion Expected outBytes) (EvmAsm.Codegen.HeaderValidateBaseFeeCompositionDecreaseRoute.decr_sep_pin_lift (r := Reg.x5) (v := EvmAsm.Codegen.U256MulU64Be.accBase + BitVec.ofNat 64 (32 + k)))))))))) s hc12b)
  have hCoreEq6 : (((.x2 ↦ᵣ wspH) ** ((.x10 ↦ᵣ (1 : Word)) ** ((.x0 ↦ᵣ (0 : Word)) ** ((regOwn .x7 ** (regOwn .x11 ** (regOwn .x12 ** (U256MulU64Be.frameSlots (wspK + signExtend12 (-48 : BitVec 12)) (K73 + 92) parentPtr Expected target (gasUsed - target) (1 : Word) ** (bytesRegion Expected outBytes ** (regOwn .x5 ** (((.x6 ↦ᵣ BitVec.ofNat 64 (8 - k)) ** (regOwn .x28 ** EvmAsm.Rv64.bytesRegion EvmAsm.Codegen.U256MulU64Be.accBase A)) ** ((.x1 ↦ᵣ (H + 40)) ** ((.x8 ↦ᵣ headerPtr) ** ((.x9 ↦ᵣ v9) ** ((.x18 ↦ᵣ old18) ** ((.x19 ↦ᵣ v19) ** ((.x20 ↦ᵣ v20) ** (frameSlotsSaved k73Frame wspK (k73Saved (H + 40) headerPtr v9 old18 v19 v20) ** (regOwn .x13 ** (regOwn .x29 ** (regOwn .x30 ** (regOwn .x31 ** (bytesRegion parentPtr parentBytes ** (regOwn .x14 ** (regOwn .x15 ** (regOwn .x16 ** (regOwn .x17 ** (k73_incr_piggyback wspH old8 headerPtr headerBytes Frest))))))))))))))))))))))))))))) = (((.x2 ↦ᵣ wspH) ** ((.x10 ↦ᵣ (1 : Word)) ** ((.x0 ↦ᵣ (0 : Word)) ** ((regOwn .x7 ** (regOwn .x11 ** (regOwn .x12 ** (U256MulU64Be.frameSlots (wspK + signExtend12 (-48 : BitVec 12)) (K73 + 92) parentPtr Expected target (gasUsed - target) (1 : Word) ** (bytesRegion Expected outBytes ** (regOwn .x5 ** (.x6 ↦ᵣ BitVec.ofNat 64 (8 - k)) ** (regOwn .x28 ** EvmAsm.Rv64.bytesRegion EvmAsm.Codegen.U256MulU64Be.accBase A) ** ((.x1 ↦ᵣ (H + 40)) ** ((.x8 ↦ᵣ headerPtr) ** ((.x9 ↦ᵣ v9) ** ((.x18 ↦ᵣ old18) ** ((.x19 ↦ᵣ v19) ** ((.x20 ↦ᵣ v20) ** (frameSlotsSaved k73Frame wspK (k73Saved (H + 40) headerPtr v9 old18 v19 v20) ** (regOwn .x13 ** (regOwn .x29 ** (regOwn .x30 ** (regOwn .x31 ** (bytesRegion parentPtr parentBytes ** (regOwn .x14 ** (regOwn .x15 ** (regOwn .x16 ** (regOwn .x17 ** (k73_incr_piggyback wspH old8 headerPtr headerBytes Frest)))))))))))))))))))))))))))) := by
    xperm_cert_eq
  have hc6b : (((.x2 ↦ᵣ wspH) ** ((.x10 ↦ᵣ (1 : Word)) ** ((.x0 ↦ᵣ (0 : Word)) ** ((regOwn .x7 ** (regOwn .x11 ** (regOwn .x12 ** (U256MulU64Be.frameSlots (wspK + signExtend12 (-48 : BitVec 12)) (K73 + 92) parentPtr Expected target (gasUsed - target) (1 : Word) ** (bytesRegion Expected outBytes ** (regOwn .x5 ** (.x6 ↦ᵣ BitVec.ofNat 64 (8 - k)) ** (regOwn .x28 ** EvmAsm.Rv64.bytesRegion EvmAsm.Codegen.U256MulU64Be.accBase A) ** ((.x1 ↦ᵣ (H + 40)) ** ((.x8 ↦ᵣ headerPtr) ** ((.x9 ↦ᵣ v9) ** ((.x18 ↦ᵣ old18) ** ((.x19 ↦ᵣ v19) ** ((.x20 ↦ᵣ v20) ** (frameSlotsSaved k73Frame wspK (k73Saved (H + 40) headerPtr v9 old18 v19 v20) ** (regOwn .x13 ** (regOwn .x29 ** (regOwn .x30 ** (regOwn .x31 ** (bytesRegion parentPtr parentBytes ** (regOwn .x14 ** (regOwn .x15 ** (regOwn .x16 ** (regOwn .x17 ** (k73_incr_piggyback wspH old8 headerPtr headerBytes Frest)))))))))))))))))))))))))))) s := by
    have h := hc5
    rw [hCoreEq6] at h
    exact h
  ...
  have hEq2 : (((.x2 ↦ᵣ wspH) ** ((.x10 ↦ᵣ (1 : Word)) ** ((.x0 ↦ᵣ (0 : Word)) ** ((regOwn .x7 ** (regOwn .x11 ** (regOwn .x12 ** (U256MulU64Be.frameSlots (wspK + signExtend12 (-48 : BitVec 12)) (K73 + 92) parentPtr Expected target (gasUsed - target) (1 : Word) ** (bytesRegion Expected outBytes ** (regOwn .x5 ** regOwn .x6 ** (regOwn .x28 ** EvmAsm.Rv64.bytesRegion EvmAsm.Codegen.U256MulU64Be.accBase A) ** ((.x1 ↦ᵣ (H + 40)) ** ((.x8 ↦ᵣ headerPtr) ** ((.x9 ↦ᵣ v9) ** ((.x18 ↦ᵣ old18) ** ((.x19 ↦ᵣ v19) ** ((.x20 ↦ᵣ v20) ** (frameSlotsSaved k73Frame wspK (k73Saved (H + 40) headerPtr v9 old18 v19 v20) ** (regOwn .x13 ** (regOwn .x29 ** (regOwn .x30 ** (regOwn .x31 ** (bytesRegion parentPtr parentBytes ** (regOwn .x14 ** (regOwn .x15 ** (regOwn .x16 ** (regOwn .x17 ** (k73_incr_piggyback wspH old8 headerPtr headerBytes Frest)))))))))))))))))))))))))))) =
...
/-- Token form of the multiply product (plain inline spellings whnf-diverge
    when elaborated against the increase route binders). -/
private def k73_incr_accT (parentBytes : List (BitVec 8)) (delta : Word) :
    List (BitVec 8) :=
  EvmAsm.Codegen.U256MulU64Be.mulState parentBytes delta 32

/-- Pointwise value of the `copyState` write chain: position `j` holds
    `M.getD (31 - j) 0` as soon as the chain has passed it. -/
private theorem k73_incr_cs_get (M X : List (BitVec 8)) (hlenX : X.length = 32) :
    ∀ k ≤ 32, ∀ j < 32, 31 - j < k →
      (EvmAsm.Codegen.U256MulU64Be.copyState M X k)[j]? = some (M.getD (31 - j) 0) := by
  intro k
  induction k with
  | zero => intro _ j _ h31; omega
  | succ k ih =>
    intro hk j hj h31
    rw [EvmAsm.Codegen.U256MulU64Be.copyState_succ]
    by_cases hjk : j = 31 - k
    · subst hjk
      rw [List.getElem?_set_self
        (by rw [EvmAsm.Codegen.U256MulU64Be.copyState_len M X k hlenX]; omega)]
      have he : 31 - (31 - k) = k := by omega
      rw [he]
    · rw [List.getElem?_set_ne (by omega)]
      exact ih (by omega) j hj (by omega)

/-- `copyState` idempotency: the second round of 32 writes lands on a window
    that already carries those exact bytes at those exact positions. -/
private theorem k73_incr_copyState_idem (M W : List (BitVec 8)) (hlen : W.length = 32) :
    EvmAsm.Codegen.U256MulU64Be.copyState M (EvmAsm.Codegen.U256MulU64Be.copyState M W 32) 32
      = EvmAsm.Codegen.U256MulU64Be.copyState M W 32 := by
  have hl1 : (EvmAsm.Codegen.U256MulU64Be.copyState M W 32).length = 32 :=
    EvmAsm.Codegen.U256MulU64Be.copyState_len M W 32 hlen
  have hl2 : (EvmAsm.Codegen.U256MulU64Be.copyState M
      (EvmAsm.Codegen.U256MulU64Be.copyState M W 32) 32).length = 32 :=
    EvmAsm.Codegen.U256MulU64Be.copyState_len M _ 32 hl1
  refine List.ext_getElem? fun j => ?_
  by_cases hj : j < 32
  · rw [k73_incr_cs_get M _ hl1 32 (by omega) j hj (by omega),
      k73_incr_cs_get M W hlen 32 (by omega) j hj (by omega)]
...
private theorem k73_incr_second_routeB_shim
    ...
    (hs : k73IncreaseSecondFinalPost spH spK (H + 40) gasUsed parentPtr Expected
        (gasLimit >>> 1) headerPtr v9 old18 v19 v20 parentBytes (k73_incr_accT parentBytes (gasUsed - (gasLimit >>> 1)))
        orig
    ...
    (((.x1 ↦ᵣ (H + 40)) ** k73RouteBCallPost spH spK (H + 40) old8 headerPtr
        ...
        (k73_incr_outj spK parentPtr gasUsed (gasLimit >>> 1) parentBytes (k73_incr_accT parentBytes (gasUsed - (gasLimit >>> 1))) F)) s) :=
  k73_incr_second_routeB (gasLimit := gasLimit) spH spK old8 headerPtr parentPtr
    v9 old18 v19 v20 gasUsed (gasLimit >>> 1) parentBytes (k73_incr_accT parentBytes (gasUsed - (gasLimit >>> 1)))
...
private theorem k73_incr_funnel_close
    ...
    (hbranch : cpsTripleWithin (13 + Nstatus + Ntail) K73 (H + 40) wholeCode
      (k73HeadPre spH spK (H + 40) gasLimit gasUsed parentPtr Expected
        ...
        (U256MulU64Be.frameSlots (spK + signExtend12 (-48 : BitVec 12)) f0 f1 f2 f3 f4 f5 **
          bytesRegion U256MulU64Be.accBase (k73_incr_accT parentBytes (gasUsed - (gasLimit >>> 1))) **
      ...
      (k73IncreaseStatusFinalPost spH spK (H + 40) gasUsed
        ...
        parentBytes (k73_incr_accT parentBytes (gasUsed - (gasLimit >>> 1)))
        (k73_incr_outT parentBytes (gasUsed - (gasLimit >>> 1)) outWin)
    ...
    cpsTripleWithin (13 + Nstatus + Ntail) K73 (H + 40) wholeCode
      ...
      ((.x1 ↦ᵣ (H + 40)) ** k73RouteBCallPost spH spK (H + 40) old8 headerPtr
        ...
        (k73_incr_outj spK parentPtr gasUsed (gasLimit >>> 1) parentBytes
          (k73_incr_accT parentBytes (gasUsed - (gasLimit >>> 1))) F)) := by
  rw [hexp]
  have hpreEq := k73_incr_pre_eq spH spK headerPtr old8 gasLimit gasUsed
    parentPtr v9 old18 v19 v20 parentBytes
    (k73_incr_outT parentBytes (gasUsed - (gasLimit >>> 1)) outWin)
    headerBytes (k73_incr_accT parentBytes (gasUsed - (gasLimit >>> 1)))
    f0 f1 f2 f3 f4 f5 F
  ...
  have hpreCast : ∀ s,
      ((.x1 ↦ᵣ (H + 40)) ** k73PreRest spH spK headerPtr v9 old18 v19 v20
        ...
        (k73_incr_env spK f0 f1 f2 f3 f4 f5 (k73_incr_accT parentBytes (gasUsed - (gasLimit >>> 1))) F)) s →
      (k73HeadPre spH spK (H + 40) gasLimit gasUsed parentPtr Expected
        headerPtr v9 old18 v19 v20 parentBytes
        (k73_incr_outT parentBytes (gasUsed - (gasLimit >>> 1)) outWin)
        (U256MulU64Be.frameSlots (spK + signExtend12 (-48 : BitVec 12)) f0 f1 f2 f3 f4 f5 **
  ...
  · -- zero case: (Carry ∨ Second(1-bytes)) ** ⌜beBytesToNat q2-token = 0⌝
    ...
    · exact k73_incr_second_routeB_shim spH spK old8 headerPtr parentPtr gasLimit gasUsed
        v9 old18 v19 v20 parentBytes headerBytes F
...
private theorem k73_incr_adapter_bridge
    ...
    (hcallee : cpsTripleWithin 3850
      ...
      (k73IncreaseMulCalleePre spK parentPtr Expected (gasLimit >>> 1) gasUsed
        f0 f1 f2 f3 f4 f5 parentBytes (k73_incr_accT parentBytes (gasUsed - (gasLimit >>> 1))) ((k73_incr_outT parentBytes (gasUsed - (gasLimit >>> 1)) outWin))
      ...
      (k73IncreaseMulCalleePost spK parentPtr Expected (gasLimit >>> 1) gasUsed
        parentBytes (k73_incr_accT parentBytes (gasUsed - (gasLimit >>> 1))) ((k73_incr_outT parentBytes (gasUsed - (gasLimit >>> 1)) outWin))
        (regOwns [.x14, .x15, .x16, .x17] ** k73_incr_piggyback spH old8 headerPtr headerBytes F)))
  ...
  have hlen2 : (u256DivU64BeQuotBytes (k73_incr_q1 (k73_incr_outT parentBytes (gasUsed - (gasLimit >>> 1)) outWin) (gasLimit >>> 1)) (k73_incr_q1 (k73_incr_outT parentBytes (gasUsed - (gasLimit >>> 1)) outWin) (gasLimit >>> 1)) 8).length = 32 := by
    ...
    rw [hq]
    simp
  have hretm : ((K73 + 92 : Word) &&& ~~~(1 : Word)) = K73 + 92 :=
    EvmAsm.Rv64.BitAux.word_add_even_andn_one (by decide) (by decide)
  have hret1 : ((K73 + 108 : Word) + 4) &&& ~~~(1 : Word) = (K73 + 108) + 4 := by
    decide
  have hret2 : ((K73 + 124 : Word) + 4) &&& ~~~(1 : Word) = (K73 + 124) + 4 := by
    decide
  have hcallRet : ((K73 + 192 : Word) + 4) &&& ~~~(1 : Word) = K73 + 192 + 4 := by
    decide
  ...
  exact k73_incr_funnel_close spH spK old8 headerPtr parentPtr gasLimit gasUsed
    v9 old18 v19 v20 f0 f1 f2 f3 f4 f5 parentBytes expectedBytes headerBytes outWin F Nstatus Ntail
    (k73_incr_hw_triple spH spK (H + 40) gasLimit gasUsed (gasLimit >>> 1) parentPtr Expected
      headerPtr v9 old18 v19 v20 f0 f1 f2 f3 f4 f5 parentBytes (k73_incr_accT parentBytes (gasUsed - (gasLimit >>> 1)))
      (k73_incr_outT parentBytes (gasUsed - (gasLimit >>> 1)) outWin)
...
/-- The multiply-callee obligation for the K73+88 call site, discharged from
    the `.proven` whole-routine contract `mulWhole_spec` (the call site has the
    disjoint source/output geometry: source `parentPtr`, output `Expected`,
    disjoint by `hdisj`).  Own declaration: the heavy `mulWhole_spec`
    instantiation lives in this budget, callers term-pass. -/
private theorem k73_incr_callee_discharged
    (spH spK old8 headerPtr parentPtr gasLimit gasUsed : Word)
    (f0 f1 f2 f3 f4 f5 : Word)
    (parentBytes outWin : List (BitVec 8)) (F : Assertion)
    (hF : F.pcFree)
    (hlenP : parentBytes.length = 32)
    (hlenOutWin : outWin.length = 32)
    ...
    cpsTripleWithin 3850
      ...
      (k73IncreaseMulCalleePost spK parentPtr Expected (gasLimit >>> 1) gasUsed
        parentBytes (k73_incr_accT parentBytes (gasUsed - (gasLimit >>> 1)))
          (k73_incr_outT parentBytes (gasUsed - (gasLimit >>> 1)) outWin)
        (regOwns [.x14, .x15, .x16, .x17] ** k73_incr_piggyback spH old8 headerPtr headerBytes F)) := by
  have hlenAccT : (k73_incr_accT parentBytes (gasUsed - (gasLimit >>> 1))).length = 40 :=
    EvmAsm.Codegen.U256MulU64Be.mulState_len parentBytes (gasUsed - (gasLimit >>> 1)) 32
  have hspec := EvmAsm.Codegen.U256MulU64Be.mulWhole_spec
    (regOwns [.x14, .x15, .x16, .x17] **
      k73_incr_piggyback spH old8 headerPtr headerBytes F)
    (by dsimp; pcf; exact hF)
    parentBytes (k73_incr_accT parentBytes (gasUsed - (gasLimit >>> 1))) (k73_incr_outT parentBytes (gasUsed - (gasLimit >>> 1)) outWin)
    hlenP hlenAccT (EvmAsm.Codegen.U256MulU64Be.copyState_len _ _ 32 hlenOutWin)
    spK (K73 + 92) parentPtr Expected (gasLimit >>> 1)
      (gasUsed - (gasLimit >>> 1)) (1 : Word)
    parentPtr (gasUsed - (gasLimit >>> 1)) Expected Expected
    f0 f1 f2 f3 f4 f5
    halignA hoverA hvalidA halignOut hoverOut hvalidOut (by decide)
  unfold k73IncreaseMulCalleePre k73IncreaseMulCalleePost
  refine cpsTripleWithin_extend_code (fun _ _ h => h) ?_
  refine cpsTripleWithin_weaken (fun s hp => hp) (fun s hq => ?_) hspec
  dsimp only [k73_incr_outT, k73_incr_accT] at hq ⊢
  rw [k73_incr_copyState_idem
    (EvmAsm.Codegen.U256MulU64Be.mulState parentBytes (gasUsed - (gasLimit >>> 1)) 32)
    outWin hlenOutWin] at hq
  exact hq
...
/-- Wrapper-vocabulary Route-B adapter for the increase arm: the whole
    ...
    threaded as the computed multiply image.  The multiply-callee obligation
    is NOT assumed: the K73+88 call site has the disjoint source/output
    geometry (`aPtr = parentPtr`, `outPtr = Expected`, disjoint by `hdisj`),
    so `hcallee` is discharged in the body from the `.proven` whole-routine
    contract `mulWhole_spec` through `k73_increase_mul_callee_of_mulWhole`,
    leaving only ordinary ABI facts (40-byte accumulator window, alignment,
    extent, byte-access validity) where the callee premise used to sit.
    `htargetPos` is the honest static entry condition for
...
theorem k73_incr_route_adapter {cr : CodeReq}
  ...
  := fun R hR s hcr hPR hpc =>
    k73_incr_adapter_bridge spH spK old8 headerPtr gasLimit gasUsed parentPtr
      v9 old18 v19 v20
      parentBytes expectedBytes headerBytes outWin f0 f1 f2 f3 f4 f5 F Nstatus Ntail
      hspK hne hlt hret hF htargetPos hleTarget hMulFit hlenP hlenOutWin hexp
      hoverA hoverOut hdisj hrw hroBase
        (k73_incr_callee_discharged spH spK old8 headerPtr parentPtr gasLimit gasUsed
          f0 f1 f2 f3 f4 f5 parentBytes outWin F
          hF hlenP hlenOutWin halignA hoverA hvalidA halignOut hoverOut hvalidOut)
        hsz1 hsz2
...
theorem k73_incr_route_adapter_inhabited :
  ...
  k73_incr_route_adapter (cr := wholeCode)
    ...
    (List.replicate 32 0) (k73_incr_outT (List.replicate 32 0) ((2500 : Word)) (List.replicate 32 0)) (List.replicate 32 0) (List.replicate 32 0)
