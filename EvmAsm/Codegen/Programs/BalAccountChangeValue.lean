/-
  ...
  Map-authoritative account change for state-root replay (#11384).
  Derive the world-state trie path from the address in a2 (often the synthetic
  empty AccountChanges shell `bsr_map_item`) and rewrite the account RLP with
  post-values from the execution maps when `bsr_account_from_map` is set —
  not from BAL field payloads.
-/
...
/-! ## map_account_change_value -- pre account + address shell -> path + post value
...
def mapAccountChangeValue_prog : Program :=
  [ .ADDI .x2 .x2 (-80 : BitVec 12),
    ...
    .MV .x21 .x15,
    .MV .x22 .x16,
    .AUIPC .x5 (laHi GuestAddrs.bacv_fail_code (GuestAddrs.map_account_change_value + 64)),
    .ADDI .x5 .x5 (laLo GuestAddrs.bacv_fail_code (GuestAddrs.map_account_change_value + 64)),
    .SD .x5 .x0 (0 : BitVec 12),
    .MV .x10 .x18,
    .MV .x11 .x19,
    .MV .x12 .x20,
    .JAL .x1 (jalOff GuestAddrs.bal_account_path (GuestAddrs.map_account_change_value + 88)),
    ...
    .MV .x14 .x21,
    .MV .x15 .x22,
    .JAL .x1 (jalOff GuestAddrs.map_account_apply_post_fields (GuestAddrs.map_account_change_value + 120)),
    .BNE .x10 .x0 (32 : BitVec 13),
    .JAL .x0 (48 : BitVec 21),
    .LI .x5 (401 : Word),
    .AUIPC .x6 (laHi GuestAddrs.bacv_fail_code (GuestAddrs.map_account_change_value + 136)),
    .ADDI .x6 .x6 (laLo GuestAddrs.bacv_fail_code (GuestAddrs.map_account_change_value + 136)),
    .SD .x6 .x5 (0 : BitVec 12),
    .LI .x10 (1 : Word),
    .JAL .x0 (24 : BitVec 21),
    .LI .x5 (402 : Word),
    .AUIPC .x6 (laHi GuestAddrs.bacv_fail_code (GuestAddrs.map_account_change_value + 160)),
    .ADDI .x6 .x6 (laLo GuestAddrs.bacv_fail_code (GuestAddrs.map_account_change_value + 160)),
    .SD .x6 .x5 (0 : BitVec 12),
    .LI .x10 (1 : Word),
...
/-- Reloc side-table for `mapAccountChangeValue_prog`: the `la`/cross-`jal` instruction indices
    kept SYMBOLIC in the emitted image text (`emitProgramR`), while the Program
    above carries the concrete guest-linked immediates for verification. -/
def mapAccountChangeValue_relocs : RelocTable :=
  [ (16, .la .x5 "bacv_fail_code"),
    (22, .jal .x1 "bal_account_path"),
    (30, .jal .x1 "map_account_apply_post_fields"),
    (34, .la .x6 "bacv_fail_code"),
    (40, .la .x6 "bacv_fail_code") ]

def mapAccountChangeValueFunction : String :=
  "map_account_change_value:\n" ++ emitProgramR mapAccountChangeValue_prog mapAccountChangeValue_relocs

/-- Kernel-checked drift guard: the emitted (image-agnostic, symbolic) Codegen
    string is exactly `mapAccountChangeValue_prog` rendered under its label with the `la`/`jal`
    relocs kept symbolic (bead evm-asm-4ch8f.9.3, mechanical conversion by
    `scripts/asm_to_program.py`). Guest binary byte-identity + guest-linked
    consistency of the concrete Program verified offline by assemble/link+cmp. -/
theorem mapAccountChangeValueFunction_eq_prog :
...
def ziskMapAccountChangeValuePrologue : String :=
  "  li sp, 0xa0050000\n" ++
  "  li t0, 0x40000000\n" ++
...
def ziskMapAccountChangeValueDataSection : String :=
  ziskMapAccountApplyPostFieldsDataSection ++ "\n" ++
  ".balign 8\n" ++
  "bacp_hash:\n  .zero 32\n" ++
  ".balign 8\n" ++
  "bacv_fail_code:\n  .zero 8\n" ++
