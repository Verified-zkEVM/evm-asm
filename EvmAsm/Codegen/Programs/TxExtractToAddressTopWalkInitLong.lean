/-
  Long-list walk_init packaging: typeLoad shape → concrete LongOkRegs
  without universal walkInitOkFail_drop (uses extractWalkInitCall_long).
-/
...
/-- Concrete long OK cursor = list + lol + 1. -/
def longWalkCursor (txBase : Word) (txBytes : List (BitVec 8))
    (listOff : Nat) (hoff : listOff < txBytes.length) : Word :=
  (txBase + BitVec.ofNat 64 listOff) +
    (((txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word)) +
      signExtend12 (1 : BitVec 12))

def longWalkEnd (txBase listLen : Word) (listOff : Nat) : Word :=
  (txBase + BitVec.ofNat 64 listOff) + listLen

/-- Long nSteps = 1 + (7*lol + 25). -/
def nWalkInitLongSteps (txBytes : List (BitVec 8)) (listOff : Nat)
    (hoff : listOff < txBytes.length) : Nat :=
...
/-- WalkInitJalPc → LinkWalkInit long path; post ambient ** common ** LongOkRegs.
    Long pure hyps from extractSuccess_long_walkInit_leaf_hyps (+ RAM statics). -/
theorem extractWalkInitCall_long_fromTypeLoad
  ...
  have h_ge_f8' : ¬ BitVec.ult ((txBytes[listOff]'hoff).zeroExtend 64) (0xf8 : Word) = true := by
    simpa only [listOff, h_listOff, h_inner] using h_ge_f8
  have hllen' : listOff + 1 + ((txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word)).toNat ≤
      txBytes.length := by
  ...
  have hoff1' : listOff + 1 < txBytes.length := by
    simpa only [listOff, h_listOff, h_inner] using hoff1
  have h_fits' : ¬ BitVec.ult ((txBase + BitVec.ofNat 64 listOff) + listLen)
      ((txBase + BitVec.ofNat 64 listOff) +
        (((txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word)) +
          signExtend12 (1 : BitVec 12))) = true := by
    simpa only [listOff, listLen, h_listOff, h_listLen, h_inner, h_off] using h_fits
  have h_llz' : (txBytes[listOff + 1]'hoff1').zeroExtend 64 ≠ (0 : Word) := by
    simpa only [listOff, h_listOff, h_inner] using h_llz
  have h_min' : ¬ BitVec.ult (BitVec.ofNat 64 (Nat.fromBytesBE
      ((txBytes.drop (listOff + 1)).take
        ((txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word)).toNat)))
      (56 : Word) = true := by
    simpa only [listOff, h_listOff, h_inner] using h_min
  have h_match' : ((txBase + BitVec.ofNat 64 listOff) +
        (((txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word)) +
          signExtend12 (1 : BitVec 12))) +
      BitVec.ofNat 64 (Nat.fromBytesBE ((txBytes.drop (listOff + 1)).take
        ((txBytes[listOff]'hoff).zeroExtend 64 - (0xf7 : Word)).toNat))
    = (txBase + BitVec.ofNat 64 listOff) + listLen := by
    simpa only [listOff, listLen, h_listOff, h_listLen, h_inner, h_off] using h_match
  have hcall := extractWalkInitCall_long txBase listLen a2Old TeaInnerAddr t1Old t2Old
  ...
  refine cpsTripleWithin_weaken (fun _ hp => by
    ...
    have hq' :
        (walkInitAmbient txBase lenW typeW inner **
          extractWalkInitLongPost txBase listLen txBytes listOff hoff) h := by
      xperm_hyp hq
    obtain ⟨hA, hP, hd, hu, hamb, hpost⟩ := hq'
    have hconc := extractWalkInitLongPost_to_okConcrete txBase listLen txBytes
      listOff hoff _ hpost
    obtain ⟨hL, hR, hd2, hu2, htemps, hRegs⟩ := hconc
    have hmid :
        (extractWalkInitCommon txBase txBytes **
          extractWalkInitLongOkRegs txBase listLen txBytes listOff hoff) hP := by
    ...
    have hmid' :
        (extractWalkInitCommon txBase txBytes **
          extractWalkInitLongOkRegs txBase (lenW - inner) txBytes inner.toNat hoff) hP := by
      simpa only [listLen, listOff, h_listLen, h_listOff, h_inner] using hmid
    exact ⟨hA, hP, hd, hu, hamb, hmid'⟩) hcallW'

set_option maxRecDepth 8000 in
/-- Frame s5/s6 through long call_fromTypeLoad. -/
theorem extractWalkInitCall_long_ok_framed_s5s6
    (txBase lenW : Word) (txBytes : List (BitVec 8))
    (old1 : Word)
    (hsalign : txBase.toNat % 8 = 0)
    (hoff : (teerTxTypeDispatch txBytes).2.2.toNat < txBytes.length)
  ...
  have h0 := extractWalkInitCall_long_fromTypeLoad txBase lenW txBytes old1
    hsalign hoff hover hvalid hlen h_ge h_ge_f8 hllen hlover hlvalid hoff1
    h_fits h_llz h_min h_match
...
#print axioms extractWalkInitCall_long_fromTypeLoad
#print axioms extractWalkInitCall_long_ok_framed_s5s6
