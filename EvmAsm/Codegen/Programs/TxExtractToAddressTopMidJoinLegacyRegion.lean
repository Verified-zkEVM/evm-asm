/-
  Legacy MidJoin bare copy region: AfterSave → copy → ret without contentDwords.
-/
...
/-- AfterLegacyWalk3 ∃ → copy arm under region (no contentDwords). -/
theorem leg3Exists_to_copy_region
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW innerW endPtr toBuf isCreationPtr s7 : Word)
    (txBytes : List (BitVec 8)) (srcOff3 q : Nat)
    (hspC : spC = sp0 + signExtend12 (-80 : BitVec 12))
    (hret : s.ra &&& ~~~(1 : Word) = s.ra)
    (hlen20 : ∀ (next3 len3 : Word),
      rlpItemDecode txBytes srcOff3 (txBase + BitVec.ofNat 64 srcOff3)
        endPtr next3 len3 → len3 = (20 : Word))
  ...
  have hpure :
      cpsTripleWithin
        ...
        (⌜rlpItemDecode txBytes srcOff3 (txBase + BitVec.ofNat 64 srcOff3)
            endPtr next3 len3⌝ **
          (leg3OkRegs txBase lenW innerW endPtr next3 len3
            txBytes srcOff3 **
    ...
    have hlen : len3 = (20 : Word) := hlen20 next3 len3 hdec
    have hnext : next3 = contentPtr + (20 : Word) := by
      simpa only [contentPtr] using hnext_content next3 len3 hdec
    have hleaf := extractLegacyHaveFieldCopy_then_epi_region sp0 spC s txBase
      lenW innerW endPtr next3 toBuf isCreationPtr s7 txBytes srcOff3 q
      hspC hret (by simpa only [contentPtr] using hnext) hq halign hcover
      hcvalid htalign htover htvalid
    refine cpsTripleWithin_weaken (fun _ hp => by
      simp only [hlen, leg3OkConcrete] at hp ⊢
  ...
  refine cpsTripleWithin_weaken (fun st hp => by
    ...
    obtain ⟨hRegs, hdec⟩ := (sepConj_pure_right h1).mp (by
      simpa only [leg3OkConcrete] using hOkC)
    have hRest :
        (leg3OkRegs txBase lenW innerW endPtr next3 len3
          txBytes srcOff3 **
...
/-- Legacy AfterSave → 20B copy → ret bare region (no contentDwords). -/
theorem extractLegacyAfterSaveCopy_then_epi_of_decode_region
    (sp0 spC : Word) (s : ExtractSaved)
    ...
    cpsTripleWithin
      ...
      ((.x1 ↦ᵣ s.ra) ** (.x2 ↦ᵣ sp0) **
        ...
        regOwn .x28 ** regOwn .x29 ** regOwn .x30) := by
  intro contentPtr w2
  have hW := extractLegacyToWalk3Ok_owned_of_decode spC s txBase lenW innerW
  ...
  have hC := leg3Exists_to_copy_region sp0 spC s txBase lenW innerW
    endPtr toBuf isCreationPtr s7 txBytes srcOff3 q
    hspC hret hlen20 hnext_content hq hsalign hcover hcvalid htalign htover htvalid
  exact cpsTripleWithin_seq_same_cr hW hC

#print axioms leg3Exists_to_copy_region
#print axioms extractLegacyAfterSaveCopy_then_epi_of_decode_region
