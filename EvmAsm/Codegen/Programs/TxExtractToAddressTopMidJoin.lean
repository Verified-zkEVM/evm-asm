/-
  Mid-chain join: type234 AfterSave → creation → ret under midOwned.
-/
...
/-- type234 AfterSave → creation → ret under midOwned.
    `hcre` / `hnext*` are honesty residuals discharged by extractSuccess. -/
theorem extractType234AfterSaveCreation_then_epi
    (sp0 spC : Word) (s : ExtractSaved)
    ...
    cpsTripleWithin
      ...
      (fun h => ∃ next5 : Word,
        ((.x1 ↦ᵣ s.ra) ** (.x2 ↦ᵣ sp0) **
          ...
          (.x31 ↦ᵣ (next5 - (0 : Word))) **
          creExtraTemps) h) := by
  have hW := extractType234ToWn5Ok_owned spC s txBase lenW typeW innerW
  ...
  have hC := wn5Exists_to_creation sp0 spC s txBase lenW typeW innerW
    endPtr toBuf isCreationPtr s7 txBytes srcOff5 hspC hret hcre
  exact cpsTripleWithin_seq_same_cr hW hC

/-- Content dwords at fixed `contentPtr` (honesty: `next5 = contentPtr + 20`). -/
def contentDwords (contentPtr w0 w1 w2 : Word) : Assertion :=
  (contentPtr ↦ₘ w0) ** ((contentPtr + 8) ↦ₘ w1) ** ((contentPtr + 16) ↦ₘ w2)

private theorem contentDwords_pcFree (contentPtr w0 w1 w2 : Word) :
    (contentDwords contentPtr w0 w1 w2).pcFree := by
...
/-- type234 AfterSave → 20B copy → ret under midOwned + content dwords.
    `hlen20` / `hnext_content` / `hnext*` / `hok*` honesty residuals. -/
theorem extractType234AfterSaveCopy_then_epi
    (sp0 spC : Word) (s : ExtractSaved)
    (txBase lenW typeW innerW cursor endPtr toBuf isCreationPtr s7 : Word)
    (txBytes : List (BitVec 8))
    (srcOff0 srcOff1 srcOff2 srcOff3 srcOff4 srcOff5 : Nat)
    ...
    cpsTripleWithin
      ...
      ((.x1 ↦ᵣ s.ra) ** (.x2 ↦ᵣ sp0) **
        ...
        regOwn .x13 ** regOwn .x14 ** regOwn .x15 ** regOwn .x16 **
        regOwn .x28 ** regOwn .x29 ** regOwn .x30) := by
  have hW0 := extractType234ToWn5Ok_owned spC s txBase lenW typeW innerW
    cursor endPtr toBuf isCreationPtr s7 txBytes
    srcOff0 srcOff1 srcOff2 srcOff3 srcOff4 srcOff5
    hcur hne0 hne1 hsalign
    hoff0 hover0 hvalid0 hss0 hls0 hll0 hok0
    ...
    hoff5 hover5 hvalid5 hss5 hls5 hll5 hok5
    hnext1 hnext2 hnext3 hnext4 hnext5
  have hW := cpsTripleWithin_frameR
    (contentDwords contentPtr w0 w1 w2)
    (contentDwords_pcFree contentPtr w0 w1 w2) hW0
  have hC := wn5Exists_to_copy sp0 spC s txBase lenW typeW innerW
    endPtr toBuf isCreationPtr s7 txBytes srcOff5 contentPtr w0 w1 w2
    hspC hret hlen20 hnext_content hcalign hcover hcvalid htalign htover htvalid
  have hW' :
      cpsTripleWithin
        ((((((((1 + (1 + (1 + 1))) + (1 + 1)) + ((1 + 87) + 1)) +
              (((1 + (1 + 1)) + (1 + 87)) + 1)) +
    ...
    refine cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp)
      (fun h hq => by
        obtain ⟨h1, h2, hd, hu, hEx, hCont⟩ := hq
        obtain ⟨next5, len5, hOk⟩ := hEx
        refine ⟨next5, len5, ?_⟩
        have hflat :
            ((wn5OkConcrete txBase lenW typeW innerW endPtr next5 len5
                txBytes srcOff5 **
              midOwned spC s toBuf isCreationPtr s7) **
              contentDwords contentPtr w0 w1 w2) h :=
          ⟨h1, h2, hd, hu, hOk, hCont⟩
        xperm_hyp hflat) hW
  exact cpsTripleWithin_seq_same_cr hW' hC

#print axioms extractType234AfterSaveCreation_then_epi
#print axioms extractType234AfterSaveCopy_then_epi
