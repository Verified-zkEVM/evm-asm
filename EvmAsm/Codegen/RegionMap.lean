...
def textSizeBytes : Nat := 0x066d68
...
def bssSizeBytes : Nat := 0x1c0eefa0
...
def bssRegion : GuestRegion :=
  { name := ".bss", base := 0xa3110000, size := bssSizeBytes, mode := .nobits, zone := .ram,
    evidence := "ELF --section-start=.bss=0xa3110000; 0x1c0eefa0-byte NOBITS extent" }
...
/-! ## The authoritative EMITTED-REALITY region map.
    ...
    (`0xa3000000`); `.data` ends `0xa3005370`, `.bss` ends `0xbf1fefa0`,
