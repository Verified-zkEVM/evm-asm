  ...
  | .ok opts => do
      ...
      | .listTestCases => do
          ...
          for tc in Tests.opcodeTestCases do
            -- M21..M31/selfdestruct-staging: 20-column TSV. Columns:
            ...
            --   11 expectedHaltKind,
            --   12 expectedPersistentLogLength (M24; retired count always 0),
            ...
            --   16 gasLimit (M30 — pack-bytecode.py --gas),
            --   17 expectedReturnDataCopied (M31 — OUTPUT+248),
            --   18 expectedReturnDataLength (M31 — OUTPUT+64),
            --   19 expectedReturnDataHex (M31 — bytes at OUTPUT+72),
            --   20 expectedSelfdestructBeneficiary (OUTPUT+56).
            -- M25 expectedPostStorage retired with the persistent-slot
            -- serializer (#11556 / #11326): no guest surface, no runner.
            -- All cols beyond 3 are optional; the bash runner asserts
            -- only the ones with non-empty values.
            IO.println
              s!"{tc.name}\t{tc.expectedOutHex}\t{tc.bytecode}\t{tc.calldata}\t{tc.storage}\t{tc.blobBaseFee}\t{tc.blobHashes}\t{tc.blockNumber}\t{tc.blockHashes}\t{tc.env}\t{tc.expectedHaltKind}\t{tc.expectedPersistentLogLength}\t{tc.expectedTransientLogLength}\t{tc.expectedEventLogCount}\t{tc.expectedEventLogFirst}\t{tc.gasLimit}\t{tc.expectedReturnDataCopied}\t{tc.expectedReturnDataLength}\t{tc.expectedReturnDataHex}\t{tc.expectedSelfdestructBeneficiary}"
          return 0
      | .program => do
