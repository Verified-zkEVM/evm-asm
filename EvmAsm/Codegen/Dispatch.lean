...
/-- Maximum bytecode length (in bytes) covered by the precomputed
    ...
    Set to `0x20000 = 131,072`, the `MAX_INIT_CODE_SIZE` of the *live*
    EIP-7954 draft (https://eips.ethereum.org/EIPS/eip-7954, which
    doubles `MAX_CODE_SIZE` to `0x10000`). Fork context:
    - Spurious Dragon..Osaka: code 24,576 (EIP-170), initcode 49,152
      (EIP-3860).
    - Vendored-specs Amsterdam (this repo's conformance target) pins an
      earlier EIP-7954 revision: `MAX_CODE_SIZE = 0x8000`,
      `MAX_INIT_CODE_SIZE = 0x10000 = 65,536`
      (`execution-specs/src/ethereum/forks/amsterdam/vm/interpreter.py`).
    So this constant carries 2× headroom over the vendored target and
    ...
    memory-safe. -/
def jumpdestBitmapCodeCapacity : Nat := 131072
...
def dispatcherTxGasSettle_prog : Program :=
  [ .LUI .x5 (10 : BitVec 20),
    ...
    .AUIPC .x7 (laHi GuestAddrs.evm_state_gas_left (GuestAddrs.dispatcher_tx_gas_settle + 40)),
    .ADDI .x7 .x7 (laLo GuestAddrs.evm_state_gas_left (GuestAddrs.dispatcher_tx_gas_settle + 40)),
    .LD .x7 .x7 (0 : BitVec 12),
    .AUIPC .x28 (laHi GuestAddrs.evm_refund_acc (GuestAddrs.dispatcher_tx_gas_settle + 52)),
    .ADDI .x28 .x28 (laLo GuestAddrs.evm_refund_acc (GuestAddrs.dispatcher_tx_gas_settle + 52)),
    .LD .x11 .x28 (0 : BitVec 12),
    .LI .x12 (1 : Word),
    .BEQ .x6 .x0 (172 : BitVec 13),
    .LI .x28 (1 : Word),
    .BEQ .x6 .x28 (164 : BitVec 13),
    .LI .x28 (5 : Word),
    .BEQ .x6 .x28 (156 : BitVec 13),
    .LI .x11 (0 : Word),
    .LI .x12 (0 : Word),
    .AUIPC .x30 (laHi GuestAddrs.evm_state_gas_used (GuestAddrs.dispatcher_tx_gas_settle + 96)),
    .ADDI .x30 .x30 (laLo GuestAddrs.evm_state_gas_used (GuestAddrs.dispatcher_tx_gas_settle + 96)),
    .LD .x28 .x30 (0 : BitVec 12),
    .AUIPC .x31 (laHi GuestAddrs.evm_state_gas_spilled (GuestAddrs.dispatcher_tx_gas_settle + 108)),
    .ADDI .x31 .x31 (laLo GuestAddrs.evm_state_gas_spilled (GuestAddrs.dispatcher_tx_gas_settle + 108)),
    .LI .x29 (2 : Word),
    .BEQ .x6 .x29 (64 : BitVec 13),
    .ADD .x5 .x27 .x0,
...
def dispatcherTxGasSettle_relocs : RelocTable :=
  [ (4, .la .x5 "evm_env"),
    ...
    (13, .la .x28 "evm_refund_acc"),
    (24, .la .x30 "evm_state_gas_used"),
    (27, .la .x31 "evm_state_gas_spilled") ]
...
#guard dispatcherTxGasSettle_prog.length = 63
