...
def sszScratchBase : Nat := 0xbf600000
...
/-- **The fits proof that actually matters (200M layout):** the BAL/state-replay
    ...
    (454 MiB) — ~143 MiB of slack for the remaining `.data` objects (~80 MiB
...
/-- The usable `.data`→`.sszscratch` span is `0x1c600000` = 476,053,504 B
    = 454 MiB. Under the 200M layout the BAL-replay arenas (~83 MiB) and the
    standalone frame array (~228 MiB) leave ample room for the rest of `.data`. -/
theorem data_gap_bytes : sszScratchBase - dataBase = 0x1c600000 := by decide
