...
def routineRegistry : List RoutineEntry := [
  ...
  routine "runtime_same_block_delegation_code" .conditional
      ...
      (gate := "TWO input-domain gates, and NEITHER is invented here — both "
        ...
        ++ "`3` deleted), and with them the empty-code hit at index 33, the "
        ++ "23-byte length check at index 15, the `0xef 0x01 0x00` marker "
        ++ "comparison at indices 16..23 and BOTH store paths (24..29 and "
        ++ "33..38); none is claimed. Because every `la` in the routine lives "
