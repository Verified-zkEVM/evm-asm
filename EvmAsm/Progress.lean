...
def registry : List OpcodeEntry := [
  ...
  entry "EXP" .partly (some "evm_exp_headroom_public_stack_shape_spec_within")
      "exp_correct proven; pure executable stack transition names EvmWord.exp; headroom RV64 full-loop composition now reaches a Spec-level theorem with old register/scratch values and the internal scratch base existentially packaged, with the final live stack split at evmSp+32; consumed operand/scratch cells and leftover headroom stack cells are weakened to ownership; leftover frame/scratch registers x0, x2, x9, and x5 are also owned in the leftover frame, with remaining returned resources framed explicitly; public evm_exp_stack_spec_within wrapper still pending",
...
private noncomputable abbrev _exp_witness         := @EvmAsm.Evm64.evm_exp_headroom_public_stack_shape_spec_within
