...
def registry : List OpcodeEntry := [
  ...
  entry "EXP" .partly (some "evm_exp_headroom_owned_base_live_stack_spec_within")
      "exp_correct proven; pure executable stack transition names EvmWord.exp; headroom RV64 full-loop composition now reaches a Spec-level named-pre live-stack theorem with the final stack rooted at evmSp+32 and the consumed base word weakened to ownership; headroom/leftover frame cleanup is still pending before the public evm_exp_stack_spec_within wrapper",
...
private noncomputable abbrev _exp_witness         := @EvmAsm.Evm64.evm_exp_headroom_owned_base_live_stack_spec_within
