...
def registry : List OpcodeEntry := [
  ...
  entry "ADDMOD" .partly (some "evm_addmod_partial_domain_named_regs_owned_live_stack_spec_within")
      ("addmod_correct proven; zero-modulus stack spec done for arbitrary b; " ++
       ...
       "current partial theorem has named domain plus a final live-stack post at sp+64, with consumed operand cells and all leftover registers in the current frame weakened to ownership and scratch resources framed explicitly; zero-modulus and no-overflow branches are exposed directly as evm_addmod_n0_regs_owned_live_stack_spec_within and evm_addmod_no_overflow_regs_owned_live_stack_spec_within"),
  ...
  entry "EXP" .partly (some "evm_exp_headroom_public_stack_shape_base_aligned_spec_within")
      "exp_correct proven; pure executable stack transition names EvmWord.exp; headroom RV64 full-loop composition now reaches a Spec-level theorem with ordinary entry-base alignment, old register/scratch values and the internal scratch base existentially packaged, and the final live stack split at evmSp+32; consumed operand/scratch cells and leftover headroom stack cells are weakened to ownership; leftover frame/scratch registers x0, x2, x9, and x5 are also owned in the leftover frame, with remaining returned resources framed explicitly; public evm_exp_stack_spec_within wrapper still pending",
...
private noncomputable abbrev _addmod_witness     := @EvmAsm.Evm64.evm_addmod_partial_domain_named_regs_owned_live_stack_spec_within
private noncomputable abbrev _mulmod_witness      := @EvmAsm.Evm64.MulMod.Compose.evm_mulmod_stack_spec_within
private noncomputable abbrev _exp_witness         := @EvmAsm.Evm64.evm_exp_headroom_public_stack_shape_base_aligned_spec_within
