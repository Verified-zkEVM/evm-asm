...
def registry : List OpcodeEntry := [
  ...
  entry "ADDMOD" .partly (some "evm_addmod_zero_or_no_overflow_word_mod_body_stack3_post_spec_within")
      ("addmod_correct proven; zero-modulus stack spec done for arbitrary b; " ++
       ...
       "current partial theorem accepts evmStackIs sp [a,b,N] and returns a folded stack post"),
  ...
  entry "EXP" .partly (some "evm_exp_headroom_clean_stack_visible_stack_spec_within")
      "exp_correct proven; pure executable stack transition names EvmWord.exp; headroom RV64 full-loop composition now reaches a Spec-level clean stack-visible theorem over the ordinary two-operand stack prefix with the loop counter exposed as zero and the consumed-base/result-tail cells folded as a stack; headroom/leftover frame cleanup is still pending before the public evm_exp_stack_spec_within wrapper",
...
private noncomputable abbrev _addmod_witness     := @EvmAsm.Evm64.evm_addmod_zero_or_no_overflow_word_mod_body_stack3_post_spec_within
