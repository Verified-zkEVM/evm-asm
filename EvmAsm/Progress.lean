...
def registry : List OpcodeEntry := [
  ...
  entry "EXP" .partly (some "evm_exp_headroom_scratch_regs_owned_leftover_live_stack_spec_within")
      "exp_correct proven; pure executable stack transition names EvmWord.exp; headroom RV64 full-loop composition now reaches a Spec-level named-pre theorem that splits the final live stack at evmSp+32; consumed operand/scratch cells and leftover headroom stack cells are weakened to ownership; leftover scratch registers x9 and x5 are also owned in the leftover frame, with remaining returned resources framed explicitly; public evm_exp_stack_spec_within wrapper still pending",
...
private noncomputable abbrev _exp_witness         := @EvmAsm.Evm64.evm_exp_headroom_scratch_regs_owned_leftover_live_stack_spec_within
