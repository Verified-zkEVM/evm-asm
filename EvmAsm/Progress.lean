...
def registry : List OpcodeEntry := [
  ...
  entry "ADDMOD" .partly (some "evm_addmod_partial_domain_named_owned_live_stack_spec_within")
      ("addmod_correct proven; zero-modulus stack spec done for arbitrary b; " ++
       ...
       "current partial theorem has named domain plus a final live-stack post at sp+64, with consumed operand cells weakened to ownership and scratch resources framed explicitly"),
  ...
  entry "EXP" .partly (some "evm_exp_headroom_framed_live_stack_spec_within")
      "exp_correct proven; pure executable stack transition names EvmWord.exp; headroom RV64 full-loop composition now reaches a Spec-level named-pre theorem that splits the final live stack at evmSp+32 from an explicit owned-base/scratch frame; headroom/leftover frame cleanup is still pending before the public evm_exp_stack_spec_within wrapper",
...
private noncomputable abbrev _addmod_witness     := @EvmAsm.Evm64.evm_addmod_partial_domain_named_owned_live_stack_spec_within
