/-
  EvmAsm.Evm64.SMod.Compose.PrefixChainV5

  v5 SMOD wrapper-prefix composition ladder over `smodCodeV5` — verbatim v4→v5
  mirrors (the wrapper prefix is byte-identical between evm_smod_v4 and
  evm_smod_v5; only the appended callable differs). Bottom-up: saveRa →
  dividendSign → preserve → divisorSign → dividendAbs → divisorAbs → modCall →
  dispatchReady. Toward the SMOD `.proven` flip over evm_mod_callable_v5.
-/
