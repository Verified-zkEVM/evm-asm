/-
  EvmAsm.Evm64.EvmWord

  Pure types and conversions for 256-bit EVM words.
  Each EvmWord is stored as 4 little-endian 64-bit limbs.
  This module has no dependency on the RV64 machine model — it only
  needs BitVec/Nat/Fin, making it importable from the EL layer and
...
def getLimb (v : EvmWord) (i : Fin 4) : BitVec 64 :=
...
def fromLimbs (limbs : Fin 4 → BitVec 64) : EvmWord :=
...
theorem getLimb_fromLimbs_0 (limbs : Fin 4 → BitVec 64) :
  ...
  simp [hj, h2, h4, h5]
theorem getLimb_fromLimbs_1 (limbs : Fin 4 → BitVec 64) :
  ...
  simp [h2, h3, h4, h5, h6]
theorem getLimb_fromLimbs_2 (limbs : Fin 4 → BitVec 64) :
...
theorem getLimb_fromLimbs_3 (limbs : Fin 4 → BitVec 64) :
...
@[evmword_limb, grind =] theorem getLimb_fromLimbs {limbs : Fin 4 → BitVec 64} {i : Fin 4} :
...
theorem fromLimbs_toNat {f : Fin 4 → BitVec 64} :
...
def toLimbs (v : EvmWord) : List (BitVec 64) :=
...
def getLimbN (v : EvmWord) (k : Nat) : BitVec 64 :=
...
theorem getLimb_fromLimbs_const {w : BitVec 64} {i : Fin 4} :
...
theorem getLimbN_fromLimbs_const {w : BitVec 64} {k : Nat} :
...
/-- `k`-specialized variants of `getLimbN_fromLimbs_const` for `k = 0, 1, 2, 3`.
    ...
    indices (issue #263). -/
theorem getLimbN_fromLimbs_const_0 {w : BitVec 64} :
    (fromLimbs (fun _ => w)).getLimbN 0 = w := by
  rw [getLimbN_fromLimbs_const, if_pos (by decide)]
theorem getLimbN_fromLimbs_const_1 {w : BitVec 64} :
    (fromLimbs (fun _ => w)).getLimbN 1 = w := by
  rw [getLimbN_fromLimbs_const, if_pos (by decide)]
theorem getLimbN_fromLimbs_const_2 {w : BitVec 64} :
    (fromLimbs (fun _ => w)).getLimbN 2 = w := by
  rw [getLimbN_fromLimbs_const, if_pos (by decide)]
theorem getLimbN_fromLimbs_const_3 {w : BitVec 64} :
    (fromLimbs (fun _ => w)).getLimbN 3 = w := by
  rw [getLimbN_fromLimbs_const, if_pos (by decide)]

/-- Generic `k`-specialized `getLimbN` of `fromLimbs` for an arbitrary
    `limbs : Fin 4 → BitVec 64`. Generalizes `getLimbN_fromLimbs_const_k` to
...
@[evmword_limb, grind =] theorem getLimbN_fromLimbs_gen_0 {limbs : Fin 4 → BitVec 64} :
    (fromLimbs limbs).getLimbN 0 = limbs 0 := by
  rw [getLimbN_lt _ _ (by decide), getLimb_fromLimbs]; rfl
@[evmword_limb, grind =] theorem getLimbN_fromLimbs_gen_1 {limbs : Fin 4 → BitVec 64} :
    (fromLimbs limbs).getLimbN 1 = limbs 1 := by
  rw [getLimbN_lt _ _ (by decide), getLimb_fromLimbs]; rfl
@[evmword_limb, grind =] theorem getLimbN_fromLimbs_gen_2 {limbs : Fin 4 → BitVec 64} :
    (fromLimbs limbs).getLimbN 2 = limbs 2 := by
  rw [getLimbN_lt _ _ (by decide), getLimb_fromLimbs]; rfl
@[evmword_limb, grind =] theorem getLimbN_fromLimbs_gen_3 {limbs : Fin 4 → BitVec 64} :
