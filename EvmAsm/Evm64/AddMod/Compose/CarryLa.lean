/-
  ...
  This file lands the `save_operands ;; minus_one_args` prefix (La12); the
  call1 boundary is threaded in the companion pipeline file.
-/
...
/-- Registers untouched by both `save_operands` and `minus_one_args`, owned so
    the downstream MOD-call adapter can fix them at generic values. `x0` is kept
    concrete (`minus_one_args` reads it). -/
def addmodLaRegTail : Assertion :=
  regOwn .x1 ** regOwn .x2 ** regOwn .x6 ** regOwn .x7 **
  regOwn .x9 ** regOwn .x10 ** regOwn .x11

theorem addmodLaRegTail_pcFree : addmodLaRegTail.pcFree := by
  unfold addmodLaRegTail; pcFree

instance : Assertion.PCFree addmodLaRegTail := ⟨addmodLaRegTail_pcFree⟩
...
/-- Combined value-agnostic tail framed through both `save_operands` and
    `minus_one_args`: the constant `x0`, the owned register tail, and the
    scratch/S3 tail. -/
def addmodLaTail (F : Word)
    (m0 m1 m2 m3 q0 q1 q2 q3 u0 u1 u2 u3 u4 u5 u6 u7
     shiftMem nMem jMem retMem dMem dloMem scratch_un0 scratchMem : Word) :
    Assertion :=
  (.x0 ↦ᵣ (0 : Word)) ** addmodLaRegTail **
  addmodLaScratchTail F m0 m1 m2 m3 q0 q1 q2 q3 u0 u1 u2 u3 u4 u5 u6 u7
...
theorem addmodLaTail_pcFree (F : Word)
    (m0 m1 m2 m3 q0 q1 q2 q3 u0 u1 u2 u3 u4 u5 u6 u7
     shiftMem nMem jMem retMem dMem dloMem scratch_un0 scratchMem : Word) :
    (addmodLaTail F m0 m1 m2 m3 q0 q1 q2 q3 u0 u1 u2 u3 u4 u5 u6 u7
      shiftMem nMem jMem retMem dMem dloMem scratch_un0 scratchMem).pcFree := by
...
theorem la_save_in_C
    ...
    cpsTripleWithin 16 (bt + 160) ((bt + 160) + 64)
      ...
      (((.x12 ↦ᵣ F) ** (.x5 ↦ᵣ x5Old) **
       ...
       addmodLaTail F sm0 sm1 sm2 sm3 dq0 dq1 dq2 dq3 du0 du1 du2 du3 du4 du5 du6 du7
         shiftMem nMem jMem retMem dMem dloMem scratch_un0 scratchMem)
      (((.x12 ↦ᵣ F) ** (.x5 ↦ᵣ r3) **
        ((F + signExtend12 (32 : BitVec 12)) ↦ₘ n0) **
        ((F + signExtend12 (40 : BitVec 12)) ↦ₘ n1) **
  ...
  refine carry_block_in_C
    ...
    (cpsTripleWithin_frameR _
      (addmodLaTail_pcFree F sm0 sm1 sm2 sm3 dq0 dq1 dq2 dq3 du0 du1 du2 du3 du4 du5 du6 du7
        shiftMem nMem jMem retMem dMem dloMem scratch_un0 scratchMem)
...
theorem addmodLaMinusOneFrame_pcFree (F : Word)
    ...
    (shiftMem nMem jMem retMem dMem dloMem scratch_un0 scratchMem : Word) :
    (addmodLaMinusOneFrame F n0 n1 n2 n3 r0 r1 r2 r3
...
theorem la_minus_one_in_C
    (bt F n0 n1 n2 n3 r0 r1 r2 r3 : Word)
    (sm0 sm1 sm2 sm3 dq0 dq1 dq2 dq3 du0 du1 du2 du3 du4 du5 du6 du7 : Word)
    (shiftMem nMem jMem retMem dMem dloMem scratch_un0 scratchMem : Word)
    (mo1 mo2 mo3 moNC : BitVec 21) (calleeEntry : Word) :
  ...
  refine carry_block_in_C
    ...
    (cpsTripleWithin_frameR _
      (addmodLaMinusOneFrame_pcFree F n0 n1 n2 n3 r0 r1 r2 r3
        sm0 sm1 sm2 sm3 dq0 dq1 dq2 dq3 du0 du1 du2 du3 du4 du5 du6 du7
        shiftMem nMem jMem retMem dMem dloMem scratch_un0 scratchMem)
...
/-- La12: `save_operands ;; minus_one_args` over `C`, from carry entry (byte 160)
    to byte 244 (the first MOD-call JAL site). Parks N→S1, r→S2, and writes the
...
theorem la12_spec_within
  ...
  exact cpsTripleWithin_seq_perm_same_cr
    (by
      ...
      xperm_hyp hp)
    hs
    (la_minus_one_in_C bt F n0 n1 n2 n3 r0 r1 r2 r3
      sm0 sm1 sm2 sm3 dq0 dq1 dq2 dq3 du0 du1 du2 du3 du4 du5 du6 du7
      shiftMem nMem jMem retMem dMem dloMem scratch_un0 scratchMem mo1 mo2 mo3 moNC calleeEntry)
