...
/-- Scan a persistent-log suffix (given most-recent-first) for the first entry
    keyed by `(addrHash, slotKey)`; 0 when none matches. -/
def persistentLookupRev (addrHash slotKey : EvmWord) :
    List StorageLogEntry → EvmWord
  | [] => 0
  | e :: es =>
      if e.addrHash = addrHash ∧ e.slotKey = slotKey then e.current
      else persistentLookupRev addrHash slotKey es

/-- The EIP-1153 SLOAD value: the `current` of the LAST entry of `entries`
    matching `(addrHash, slotKey)` (the log is append-only, so the last match
    is the most-recent write), or 0 when no entry matches. -/
def persistentLookup (addrHash slotKey : EvmWord)
    (entries : List StorageLogEntry) : EvmWord :=
  persistentLookupRev addrHash slotKey entries.reverse

@[simp] theorem persistentLookup_nil {addrHash slotKey : EvmWord} :
    persistentLookup addrHash slotKey [] = 0 := rfl

/-- The snoc unfold driving the reverse-scan induction: the entry appended
    LAST is scanned FIRST. -/
theorem persistentLookup_snoc {addrHash slotKey : EvmWord}
    {es : List StorageLogEntry} {e : StorageLogEntry} :
    persistentLookup addrHash slotKey (es ++ [e]) =
      if e.addrHash = addrHash ∧ e.slotKey = slotKey then e.current
      else persistentLookup addrHash slotKey es := by
  simp [persistentLookup, List.reverse_append, persistentLookupRev]
...
/-- Base address of persistent-log entry `L`. -/
def sloadEnt (L : Nat) : Word :=
  PERSISTENT_STORAGE_LOG_BASE + BitVec.ofNat 64 (L * 128)

theorem sloadEnt_zero : sloadEnt 0 = PERSISTENT_STORAGE_LOG_BASE := by
  simp [sloadEnt]

theorem sloadEnt_succ (L : Nat) : sloadEnt (L + 1) = sloadEnt L + 128 := by
  unfold sloadEnt
...
/-! ## Compare-block lemmas (the 25-instruction slice `evm_sload_cmp`)
...
theorem evm_sload_cmp_exit0_spec_within
    (b2 ent envAddr sp x16old x17old : Word)
    (a0 a1 a2 a3 k0 k1 k2 k3 e0 e1 e2 e3 e4 e5 e6 e7 : Word)
    (hne0 : e0 ≠ a0) :
    cpsTripleWithin 4 b2 (b2 + 136) (evm_sload_cmp_code b2)
...
theorem evm_sload_cmp_exit1_spec_within
    (b2 ent envAddr sp x16old x17old : Word)
    (a0 a1 a2 a3 k0 k1 k2 k3 e0 e1 e2 e3 e4 e5 e6 e7 : Word)
    (heq0 : e0 = a0)
    (hne1 : e1 ≠ a1) :
    cpsTripleWithin 7 b2 (b2 + 136) (evm_sload_cmp_code b2)
      ((((.x20)) ↦ᵣ envAddr) ** (((.x12)) ↦ᵣ sp) ** (((.x14)) ↦ᵣ (ent + 128)) **
       (((.x16)) ↦ᵣ x16old) ** (((.x17)) ↦ᵣ x17old) **
       ((envAddr) ↦ₘ a0) ** ((envAddr + 8) ↦ₘ a1) **
       ((envAddr + 16) ↦ₘ a2) ** ((envAddr + 24) ↦ₘ a3) **
...
theorem evm_sload_cmp_exit2_spec_within
    (b2 ent envAddr sp x16old x17old : Word)
    (a0 a1 a2 a3 k0 k1 k2 k3 e0 e1 e2 e3 e4 e5 e6 e7 : Word)
    (heq0 : e0 = a0) (heq1 : e1 = a1)
    (hne2 : e2 ≠ a2) :
    cpsTripleWithin 10 b2 (b2 + 136) (evm_sload_cmp_code b2)
      ((((.x20)) ↦ᵣ envAddr) ** (((.x12)) ↦ᵣ sp) ** (((.x14)) ↦ᵣ (ent + 128)) **
       (((.x16)) ↦ᵣ x16old) ** (((.x17)) ↦ᵣ x17old) **
       ((envAddr) ↦ₘ a0) ** ((envAddr + 8) ↦ₘ a1) **
       ((envAddr + 16) ↦ₘ a2) ** ((envAddr + 24) ↦ₘ a3) **
...
theorem evm_sload_cmp_exit3_spec_within
    (b2 ent envAddr sp x16old x17old : Word)
    (a0 a1 a2 a3 k0 k1 k2 k3 e0 e1 e2 e3 e4 e5 e6 e7 : Word)
    (heq0 : e0 = a0) (heq1 : e1 = a1) (heq2 : e2 = a2)
    (hne3 : e3 ≠ a3) :
    cpsTripleWithin 13 b2 (b2 + 136) (evm_sload_cmp_code b2)
      ((((.x20)) ↦ᵣ envAddr) ** (((.x12)) ↦ᵣ sp) ** (((.x14)) ↦ᵣ (ent + 128)) **
       (((.x16)) ↦ᵣ x16old) ** (((.x17)) ↦ᵣ x17old) **
       ((envAddr) ↦ₘ a0) ** ((envAddr + 8) ↦ₘ a1) **
       ((envAddr + 16) ↦ₘ a2) ** ((envAddr + 24) ↦ₘ a3) **
...
theorem evm_sload_cmp_exit4_spec_within
    (b2 ent envAddr sp x16old x17old : Word)
    (a0 a1 a2 a3 k0 k1 k2 k3 e0 e1 e2 e3 e4 e5 e6 e7 : Word)
    (heq0 : e0 = a0) (heq1 : e1 = a1) (heq2 : e2 = a2) (heq3 : e3 = a3)
    (hne4 : e4 ≠ k0) :
    cpsTripleWithin 16 b2 (b2 + 136) (evm_sload_cmp_code b2)
      ((((.x20)) ↦ᵣ envAddr) ** (((.x12)) ↦ᵣ sp) ** (((.x14)) ↦ᵣ (ent + 128)) **
       (((.x16)) ↦ᵣ x16old) ** (((.x17)) ↦ᵣ x17old) **
       ((envAddr) ↦ₘ a0) ** ((envAddr + 8) ↦ₘ a1) **
       ((envAddr + 16) ↦ₘ a2) ** ((envAddr + 24) ↦ₘ a3) **
...
theorem evm_sload_cmp_exit5_spec_within
    (b2 ent envAddr sp x16old x17old : Word)
    (a0 a1 a2 a3 k0 k1 k2 k3 e0 e1 e2 e3 e4 e5 e6 e7 : Word)
    (heq0 : e0 = a0) (heq1 : e1 = a1) (heq2 : e2 = a2) (heq3 : e3 = a3) (heq4 : e4 = k0)
    (hne5 : e5 ≠ k1) :
    cpsTripleWithin 19 b2 (b2 + 136) (evm_sload_cmp_code b2)
      ((((.x20)) ↦ᵣ envAddr) ** (((.x12)) ↦ᵣ sp) ** (((.x14)) ↦ᵣ (ent + 128)) **
       (((.x16)) ↦ᵣ x16old) ** (((.x17)) ↦ᵣ x17old) **
       ((envAddr) ↦ₘ a0) ** ((envAddr + 8) ↦ₘ a1) **
       ((envAddr + 16) ↦ₘ a2) ** ((envAddr + 24) ↦ₘ a3) **
...
theorem evm_sload_cmp_exit6_spec_within
    (b2 ent envAddr sp x16old x17old : Word)
    (a0 a1 a2 a3 k0 k1 k2 k3 e0 e1 e2 e3 e4 e5 e6 e7 : Word)
    (heq0 : e0 = a0) (heq1 : e1 = a1) (heq2 : e2 = a2) (heq3 : e3 = a3) (heq4 : e4 = k0) (heq5 : e5 = k1)
    (hne6 : e6 ≠ k2) :
    cpsTripleWithin 22 b2 (b2 + 136) (evm_sload_cmp_code b2)
      ((((.x20)) ↦ᵣ envAddr) ** (((.x12)) ↦ᵣ sp) ** (((.x14)) ↦ᵣ (ent + 128)) **
       (((.x16)) ↦ᵣ x16old) ** (((.x17)) ↦ᵣ x17old) **
       ((envAddr) ↦ₘ a0) ** ((envAddr + 8) ↦ₘ a1) **
       ((envAddr + 16) ↦ₘ a2) ** ((envAddr + 24) ↦ₘ a3) **
...
theorem evm_sload_cmp_exit7_spec_within
    (b2 ent envAddr sp x16old x17old : Word)
    (a0 a1 a2 a3 k0 k1 k2 k3 e0 e1 e2 e3 e4 e5 e6 e7 : Word)
    (heq0 : e0 = a0) (heq1 : e1 = a1) (heq2 : e2 = a2) (heq3 : e3 = a3) (heq4 : e4 = k0) (heq5 : e5 = k1) (heq6 : e6 = k2)
    (hne7 : e7 ≠ k3) :
    cpsTripleWithin 25 b2 (b2 + 136) (evm_sload_cmp_code b2)
      ((((.x20)) ↦ᵣ envAddr) ** (((.x12)) ↦ᵣ sp) ** (((.x14)) ↦ᵣ (ent + 128)) **
       (((.x16)) ↦ᵣ x16old) ** (((.x17)) ↦ᵣ x17old) **
       ((envAddr) ↦ₘ a0) ** ((envAddr + 8) ↦ₘ a1) **
       ((envAddr + 16) ↦ₘ a2) ** ((envAddr + 24) ↦ₘ a3) **
...
theorem evm_sload_cmp_pass_spec_within
    (b2 ent envAddr sp x16old x17old : Word)
    (a0 a1 a2 a3 k0 k1 k2 k3 : Word) :
    cpsTripleWithin 25 b2 (b2 + 100) (evm_sload_cmp_code b2)
      ((((.x20)) ↦ᵣ envAddr) ** (((.x12)) ↦ᵣ sp) ** (((.x14)) ↦ᵣ (ent + 128)) **
       (((.x16)) ↦ᵣ x16old) ** (((.x17)) ↦ᵣ x17old) **
       ((envAddr) ↦ₘ a0) ** ((envAddr + 8) ↦ₘ a1) **
       ((envAddr + 16) ↦ₘ a2) ** ((envAddr + 24) ↦ₘ a3) **
...
theorem evm_sload_cmp_mismatch_spec_within
    (b2 ent envAddr sp x16old x17old : Word)
    (a0 a1 a2 a3 k0 k1 k2 k3 e0 e1 e2 e3 e4 e5 e6 e7 : Word)
  ...
  case neg =>
    exact cpsTripleWithin_mono_nSteps (by omega)
      (cpsTripleWithin_weaken (fun h hp => hp) sepConj_own2_after3
        (evm_sload_cmp_exit0_spec_within b2 ent envAddr sp x16old x17old
  ...
  case pos =>
    ...
    case neg =>
      exact cpsTripleWithin_mono_nSteps (by omega)
        (cpsTripleWithin_weaken (fun h hp => hp) sepConj_own2_after3
          (evm_sload_cmp_exit1_spec_within b2 ent envAddr sp x16old x17old
    ...
    case pos =>
      ...
      case neg =>
        exact cpsTripleWithin_mono_nSteps (by omega)
          (cpsTripleWithin_weaken (fun h hp => hp) sepConj_own2_after3
            (evm_sload_cmp_exit2_spec_within b2 ent envAddr sp x16old x17old
      ...
      case pos =>
        ...
        case neg =>
          exact cpsTripleWithin_mono_nSteps (by omega)
            (cpsTripleWithin_weaken (fun h hp => hp) sepConj_own2_after3
              (evm_sload_cmp_exit3_spec_within b2 ent envAddr sp x16old x17old
        ...
        case pos =>
          ...
          case neg =>
            exact cpsTripleWithin_mono_nSteps (by omega)
              (cpsTripleWithin_weaken (fun h hp => hp) sepConj_own2_after3
                (evm_sload_cmp_exit4_spec_within b2 ent envAddr sp x16old x17old
          ...
          case pos =>
            ...
            case neg =>
              exact cpsTripleWithin_mono_nSteps (by omega)
                (cpsTripleWithin_weaken (fun h hp => hp) sepConj_own2_after3
                  (evm_sload_cmp_exit5_spec_within b2 ent envAddr sp x16old x17old
            ...
            case pos =>
              ...
              case neg =>
                exact cpsTripleWithin_mono_nSteps (by omega)
                  (cpsTripleWithin_weaken (fun h hp => hp) sepConj_own2_after3
                    (evm_sload_cmp_exit6_spec_within b2 ent envAddr sp x16old x17old
              ...
              case pos =>
                ...
                case neg =>
                  exact cpsTripleWithin_mono_nSteps (by omega)
                    (cpsTripleWithin_weaken (fun h hp => hp) sepConj_own2_after3
                      (evm_sload_cmp_exit7_spec_within b2 ent envAddr sp x16old x17old
