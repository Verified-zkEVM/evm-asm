/-
  EvmAsm.Evm64.Storage.LoadSpec

  Stack-level `cpsTripleWithin` specification for the EVM `SLOAD` opcode
  (0x5c, EIP-1153 persistent storage; see `EvmAsm/Evm64/Storage/LoadProgram.lean`).

  SLOAD scans the persistent-storage exec-log from the END for the most-recent
  ...
  (or 0 when no entry matches). The pure model is `persistentLookup`.

  Proof layout (bottom-up):
  - `evm_sload_cmp_*`: the 25-instruction compare block, one lemma per exit —
    eight mismatch exits (merged into `evm_sload_cmp_mismatch_spec_within` by
    limb `by_cases`) and the all-limbs-equal pass-through
    (`evm_sload_cmp_pass_spec_within`). Proven over `evm_sload_cmp_code b2`
    (variable entry) and extended to the loop slice.
  - `evm_sload_copy_spec_within` / `evm_sload_tail_{continue,exit}_spec_within`:
  ...
  - `evm_sload_iter_{match,nomatch_continue,nomatch_exit}_spec_within`: one
    full loop iteration over `evm_sload_loop_code b2`.
  - `evm_sload_loop_spec_within`: snoc induction (`List.reverseRecOn`) over the
    unscanned prefix; the loop invariant at entry with `m` entries left is
    `x15 = m`, `x14 = PERSISTENT_STORAGE_LOG_BASE + 128*m`, and the final stack
    top is `persistentLookup` over those `m` entries.
  - `evm_sload_spec_within` (head + loop / empty-log path) and the public
    witness `evm_sload_stack_spec_within`.
-/
...
import EvmAsm.Evm64.Storage.LoadLoopSpec
...
open EvmAsm.Rv64
open EvmAsm.Evm64
-- `storageSlotIs_eq_flat` and `shift7_eq_mul128` are base-independent generic
-- helpers that currently live in the transient store spec; reuse them here.
open EvmAsm.Evm64.Transient (storageSlotIs_eq_flat shift7_eq_mul128)
/-- The 9-instruction match-copy arm (`evm_sload_copy`): copy the matched
...
theorem evm_sload_copy_spec_within
    (b3 ent sp x16old : Word) (c0 c1 c2 c3 k0 k1 k2 k3 : Word) :
    cpsTripleWithin 9 b3 (b3 + 60) (evm_sload_copy_code b3)
...
/-- The decrement/loop-back path of the tail (`evm_sload_tail`): one fewer
    entry left and it is nonzero, so the backward BNE returns to the loop
    entry 136 bytes above this slice. -/
theorem evm_sload_tail_continue_spec_within
    (b4 v : Word) (hv : v - 1 ≠ 0) :
    cpsTripleWithin 2 b4 (b4 - 136) (evm_sload_tail_code b4)
      ((((.x15)) ↦ᵣ v) ** (((.x0)) ↦ᵣ (0 : Word)))
...
/-- The scan-exhausted path of the tail: the decremented counter is zero, so
    ...
    top becomes 0. -/
theorem evm_sload_tail_exit_spec_within
    (b4 sp v : Word) (k0 k1 k2 k3 : Word) (hv : v - 1 = 0) :
    cpsTripleWithin 6 b4 (b4 + 24) (evm_sload_tail_code b4)
...
private theorem cmp_sub_loop (b2 : Word) :
    ∀ a i, (evm_sload_cmp_code b2) a = some i →
      (evm_sload_loop_code b2) a = some i := by
  intro a i h
  exact CodeReq.ofProg_mono_append_left b2
    (evm_sload_cmp .x20 ++ evm_sload_copy) evm_sload_tail a i
    (CodeReq.ofProg_mono_append_left b2 (evm_sload_cmp .x20) evm_sload_copy a i h)

private theorem copy_sub_loop (b2 : Word) :
    ∀ a i, (evm_sload_copy_code (b2 + 100)) a = some i →
      (evm_sload_loop_code b2) a = some i := by
  have h := CodeReq.ofProg_mono_subrange b2 (evm_sload_cmp .x20) evm_sload_copy
    evm_sload_tail (by decide)
  rw [show BitVec.ofNat 64 (4 * (evm_sload_cmp .x20).length) = (100 : Word)
        from by decide] at h
  exact h

private theorem tail_sub_loop (b2 : Word) :
    ∀ a i, (evm_sload_tail_code (b2 + 136)) a = some i →
      (evm_sload_loop_code b2) a = some i := by
  have h := CodeReq.ofProg_mono_append_right b2
    (evm_sload_cmp .x20 ++ evm_sload_copy) evm_sload_tail (by decide)
  rw [show BitVec.ofNat 64 (4 * (evm_sload_cmp .x20 ++ evm_sload_copy).length)
        = (136 : Word) from by decide] at h
  exact h

private theorem loop_sub_full (base : Word) :
    ∀ a i, (evm_sload_loop_code (base + 28)) a = some i →
      (evm_sload_code .x20 base) a = some i := by
  have h := CodeReq.ofProg_mono_append_right base
    (evm_sload_head .x20) (evm_sload_loop .x20) (by decide)
  rw [show BitVec.ofNat 64 (4 * (evm_sload_head .x20).length) = (28 : Word)
        from by decide] at h
  exact h
...
theorem evm_sload_iter_match_spec_within
    (b2 ent envAddr sp x16old x17old : Word)
    (a0 a1 a2 a3 k0 k1 k2 k3 c0 c1 c2 c3 : Word) :
    cpsTripleWithin 34 b2 (b2 + 160) (evm_sload_loop_code b2)
  ...
  have pass := cpsTripleWithin_extend_code (cmp_sub_loop b2)
    (evm_sload_cmp_pass_spec_within b2 ent envAddr sp x16old x17old
      a0 a1 a2 a3 k0 k1 k2 k3)
  have copy := cpsTripleWithin_extend_code (copy_sub_loop b2)
    (evm_sload_copy_spec_within (b2 + 100) ent sp k3 c0 c1 c2 c3 k0 k1 k2 k3)
...
theorem evm_sload_iter_nomatch_continue_spec_within
    ...
    (hm : m - 1 ≠ 0) :
    cpsTripleWithin 27 b2 b2 (evm_sload_loop_code b2)
      ((((.x20)) ↦ᵣ envAddr) ** (((.x12)) ↦ᵣ sp) ** (((.x14)) ↦ᵣ (ent + 128)) **
       (((.x15)) ↦ᵣ m) ** (((.x0)) ↦ᵣ (0 : Word)) **
       (((.x16)) ↦ᵣ x16old) ** (((.x17)) ↦ᵣ x17old) **
       ((envAddr) ↦ₘ a0) ** ((envAddr + 8) ↦ₘ a1) **
  ...
  have mis := cpsTripleWithin_extend_code (cmp_sub_loop b2)
    (evm_sload_cmp_mismatch_spec_within b2 ent envAddr sp x16old x17old
      a0 a1 a2 a3 k0 k1 k2 k3 e0 e1 e2 e3 e4 e5 e6 e7 hne)
  have tailc := cpsTripleWithin_extend_code (tail_sub_loop b2)
    (evm_sload_tail_continue_spec_within (b2 + 136) m hm)
...
/-- Iteration on a NON-matching LAST entry: some compare pair differs and the
    ...
    exits (31 steps). -/
theorem evm_sload_iter_nomatch_exit_spec_within
    (b2 ent envAddr sp m x16old x17old : Word)
    (a0 a1 a2 a3 k0 k1 k2 k3 e0 e1 e2 e3 e4 e5 e6 e7 : Word)
    (hne : ¬(e0 = a0 ∧ e1 = a1 ∧ e2 = a2 ∧ e3 = a3 ∧
             e4 = k0 ∧ e5 = k1 ∧ e6 = k2 ∧ e7 = k3))
    (hm : m - 1 = 0) :
    cpsTripleWithin 31 b2 (b2 + 160) (evm_sload_loop_code b2)
      ((((.x20)) ↦ᵣ envAddr) ** (((.x12)) ↦ᵣ sp) ** (((.x14)) ↦ᵣ (ent + 128)) **
       (((.x15)) ↦ᵣ m) ** (((.x0)) ↦ᵣ (0 : Word)) **
       (((.x16)) ↦ᵣ x16old) ** (((.x17)) ↦ᵣ x17old) **
       ((envAddr) ↦ₘ a0) ** ((envAddr + 8) ↦ₘ a1) **
  ...
  have taile := cpsTripleWithin_extend_code (tail_sub_loop b2)
    (evm_sload_tail_exit_spec_within (b2 + 136) sp m k0 k1 k2 k3 hm)
  ...
  | append_singleton es' e ih =>
    ...
    · -- The most-recent entry matches: copy its `current`, exit.
      have coreF := cpsTripleWithin_frameR
        (storageLogIs PERSISTENT_STORAGE_LOG_BASE es' **
         ...
         (((.x0)) ↦ᵣ (0 : Word)))
        (by pcFree)
        (evm_sload_iter_match_spec_within b2
          (PERSISTENT_STORAGE_LOG_BASE + BitVec.ofNat 64 (es'.length * 128))
          envAddr sp v16 v17
    ...
    · -- The most-recent entry does not match.
      ...
      · -- Last entry scanned without a match: fall into the zero arm.
        ...
        have hTL : persistentLookup addrHash slotKey es' = 0 := by
          rw [hnil]; rfl
        have coreF := cpsTripleWithin_frameR
          (storageLogIs PERSISTENT_STORAGE_LOG_BASE es' **
           (((PERSISTENT_STORAGE_LOG_BASE +
          ...
          (by pcFree)
          (evm_sload_iter_nomatch_exit_spec_within b2
            (PERSISTENT_STORAGE_LOG_BASE + BitVec.ofNat 64 (es'.length * 128))
        ...
        · rw [storageLogIs_snoc, storageSlotIs_eq_flat,
              ...
              hent] at hp
          xperm_hyp hp
        · rw [persistentLookup_snoc, if_neg hm, hTL, storageLogIs_snoc,
      ...
      · -- No match and more entries left: loop and use the IH on `es'`.
        ...
        have iterF := cpsTripleWithin_frameR
          (storageLogIs PERSISTENT_STORAGE_LOG_BASE es' **
           ...
           (((PERSISTENT_STORAGE_LOG_BASE +
              ...
              e.current.getLimbN 3))
          (by pcFree)
          (evm_sload_iter_nomatch_continue_spec_within b2
            (PERSISTENT_STORAGE_LOG_BASE + BitVec.ofNat 64 (es'.length * 128))
            envAddr sp (BitVec.ofNat 64 (es'.length + 1)) v16 v17
            (addrHash.getLimbN 0) (addrHash.getLimbN 1)
            (addrHash.getLimbN 2) (addrHash.getLimbN 3)
        ...
        · rw [storageLogIs_snoc, storageSlotIs_eq_flat,
              ...
              hent] at hp
          xperm_hyp hp
        · rw [persistentLookup_snoc, if_neg hm, storageLogIs_snoc,
              storageSlotIs_eq_flat]
          xperm_hyp hq

/-! ## Head, empty-log path, and the raw full-program spec -/

private theorem sE448 :
    signExtend12 (BitVec.ofNat 12 persistentLogLengthOff) = (448 : Word) := by
  decide

/-- The 7-instruction head on a nonempty log: loads `n`, falls through the
    BEQ, materializes the persistent-log base, and computes the one-past-end
    scan pointer. -/
theorem evm_sload_head_spec_within
    (base envAddr : Word) (x14old x15old x16old : Word) (n : Nat)
    (hn : BitVec.ofNat 64 n ≠ 0) :
    cpsTripleWithin 7 base (base + 28) (evm_sload_code .x20 base)
      ((((.x20)) ↦ᵣ envAddr) ** (((.x14)) ↦ᵣ x14old) **
       (((.x15)) ↦ᵣ x15old) ** (((.x16)) ↦ᵣ x16old) **
       (((.x0)) ↦ᵣ (0 : Word)) **
       ((envAddr + 448) ↦ₘ BitVec.ofNat 64 n))
      ((((.x20)) ↦ᵣ envAddr) **
       (((.x14)) ↦ᵣ (PERSISTENT_STORAGE_LOG_BASE +
       ...
       (((.x0)) ↦ᵣ (0 : Word)) **
       ((envAddr + 448) ↦ₘ BitVec.ofNat 64 n)) := by
  have hld := ld_spec_gen_within .x15 .x20 envAddr x15old (BitVec.ofNat 64 n)
    (BitVec.ofNat 12 persistentLogLengthOff) base (by decide)
  simp only [sE448] at hld
  ...
  have haddiw := addiw_spec_gen_same_within .x14 (40960 : Word)
    (BitVec.ofNat 12 99) (base + 12) (by decide)
  rw [show (((40960 : Word).truncate 32 +
        (signExtend12 (BitVec.ofNat 12 99)).truncate 32 : BitVec 32).signExtend 64)
        = (41059 : Word) from by decide] at haddiw
  have hslli16 := slli_spec_gen_same_within .x14 (41059 : Word) (16 : BitVec 6)
    (base + 16) (by decide)
  rw [show (41059 : Word) <<< (16 : BitVec 6).toNat = PERSISTENT_STORAGE_LOG_BASE
  ...
  have hadd := add_spec_gen_rd_eq_rs1_within .x14 .x16 PERSISTENT_STORAGE_LOG_BASE
    (BitVec.ofNat 64 (n * 128)) (base + 24) (by decide)
  runBlock hld hbeq hlui haddiw hslli16 hslli7 hadd

/-- Empty persistent log: the head BEQ jumps straight to the zero arm and the
    stack top becomes 0. -/
theorem evm_sload_empty_spec_within
    (base envAddr sp : Word) (x15old : Word) (k0 k1 k2 k3 : Word) :
    cpsTripleWithin 6 base (base + 188) (evm_sload_code .x20 base)
      ((((.x20)) ↦ᵣ envAddr) ** (((.x12)) ↦ᵣ sp) **
       (((.x15)) ↦ᵣ x15old) ** (((.x0)) ↦ᵣ (0 : Word)) **
       ((envAddr + 448) ↦ₘ BitVec.ofNat 64 0) **
       ((sp) ↦ₘ k0) ** ((sp + 8) ↦ₘ k1) **
       ((sp + 16) ↦ₘ k2) ** ((sp + 24) ↦ₘ k3))
      ((((.x20)) ↦ᵣ envAddr) ** (((.x12)) ↦ᵣ sp) **
...
/-- Raw full-program SLOAD spec: from the length cell (`env+448`), the frame's
    `env.ADDRESS`, the slot key at the stack top, and the persistent log, the
    47-instruction scan replaces the stack top in place by
    `persistentLookup addrHash slotKey entries` within `7 + 34*n` steps.
    Scratch registers `x14`–`x17` end clobbered; everything else is
    preserved. -/
theorem evm_sload_spec_within
    ...
    cpsTripleWithin (7 + 34 * n) base (base + 188) (evm_sload_code .x20 base)
      ((((.x20)) ↦ᵣ envAddr) ** (((.x12)) ↦ᵣ sp) **
       ...
       ((envAddr + 448) ↦ₘ BitVec.ofNat 64 n) **
       evmWordIs envAddr addrHash ** evmWordIs sp slotKey **
       storageLogIs PERSISTENT_STORAGE_LOG_BASE entries)
      (regOwn .x14 ** regOwn .x15 ** regOwn .x16 ** regOwn .x17 **
       (((.x20)) ↦ᵣ envAddr) ** (((.x12)) ↦ᵣ sp) **
       (((.x0)) ↦ᵣ (0 : Word)) **
       ((envAddr + 448) ↦ₘ BitVec.ofNat 64 n) **
       evmWordIs envAddr addrHash **
       evmWordIs sp (persistentLookup addrHash slotKey entries) **
       storageLogIs PERSISTENT_STORAGE_LOG_BASE entries) := by
  by_cases hn0 : n = 0
  · -- Empty log: the head BEQ takes the zero arm.
    ...
    have coreF := cpsTripleWithin_frameR
      ((((.x14)) ↦ᵣ x14old) ** (((.x16)) ↦ᵣ x16old) ** (((.x17)) ↦ᵣ x17old) **
       evmWordIs envAddr addrHash **
       storageLogIs PERSISTENT_STORAGE_LOG_BASE ([] : List StorageLogEntry))
      (by pcFree)
      (evm_sload_empty_spec_within base envAddr sp x15old
        (slotKey.getLimbN 0) (slotKey.getLimbN 1)
        (slotKey.getLimbN 2) (slotKey.getLimbN 3))
    refine cpsTripleWithin_mono_nSteps (by omega)
      (cpsTripleWithin_weaken (fun h hp => ?_) (fun h hq => ?_) coreF)
    · rw [evmWordIs_flat sp slotKey] at hp
      xperm_hyp hp
    · rw [persistentLookup_nil, evmWordIs_zero]
  ...
  · -- Nonempty log: head then loop.
    ...
    have headF := cpsTripleWithin_frameR
      ((((.x12)) ↦ᵣ sp) ** (((.x17)) ↦ᵣ x17old) **
       ...
       storageLogIs PERSISTENT_STORAGE_LOG_BASE entries)
      (by pcFree)
      (evm_sload_head_spec_within base envAddr x14old x15old x16old n
        (ofNat64_ne_zero hn0 hcap))
    have loop := evm_sload_loop_spec_within (base + 28) envAddr sp
      addrHash slotKey entries hne (by rw [hlen]; exact hcap)
    rw [hlen] at loop
    have loopF := cpsTripleWithin_frameR
      ((envAddr + 448) ↦ₘ BitVec.ofNat 64 n) (by pcFree)
...
/-- **SLOAD stack spec** (the `.proven` witness): with the persistent log of
    length `n` (`storageLogLenIs`/`storageLogIs`), the executing frame's
    ...
    `persistentLookup addrHash slotKey entries` — the `current` of the
...
theorem evm_sload_stack_spec_within
    ...
    cpsTripleWithin (7 + 34 * n) codeBase (codeBase + 188)
      (evm_sload_code .x20 codeBase)
      ((((.x20)) ↦ᵣ envAddr) ** (((.x12)) ↦ᵣ sp) **
       (((.x14)) ↦ᵣ x14old) ** (((.x15)) ↦ᵣ x15old) **
       (((.x16)) ↦ᵣ x16old) ** (((.x17)) ↦ᵣ x17old) **
       (((.x0)) ↦ᵣ (0 : Word)) **
       storageLogLenIs envAddr n **
       storageLogIs PERSISTENT_STORAGE_LOG_BASE entries **
       evmWordIs envAddr addrHash **
      ...
      (regOwn .x14 ** regOwn .x15 ** regOwn .x16 ** regOwn .x17 **
       ...
       storageLogIs PERSISTENT_STORAGE_LOG_BASE entries **
       evmWordIs envAddr addrHash **
       evmStackIs sp (persistentLookup addrHash slotKey entries :: rest)) := by
  have framed := cpsTripleWithin_frameR (evmStackIs (sp + 32) rest)
    (by pcFree)
    (evm_sload_spec_within n codeBase envAddr sp x14old x15old x16old x17old
      addrHash slotKey entries hlen hcap)
  have hoff : (BitVec.ofNat 64 EvmEnv.persistentLogLengthOff) = (448 : Word) := by
    decide
  refine cpsTripleWithin_weaken (fun h hp => ?_) (fun h hq => ?_) framed
  · rw [evmStackIs_cons] at hp
    simp only [storageLogLenIs, hoff] at hp
    xperm_hyp hp
  · rw [evmStackIs_cons]
    simp only [storageLogLenIs, hoff]
    xperm_hyp hq

/-! ## Anti-vacuity cover (the `.conditional` `coverRef`) -/

/-- **Cover lemma for the SLOAD `.conditional` registry entry**
    (`OpcodeEntry.coverRef`, R-A3 anti-near-vacuity). The gating "hit"
    antecedent of the reverse scan — the queried slot IS present in the
    committed persistent log — is satisfiable on a representative input: a
    one-entry log keyed by `(addrHash = 7, slotKey = 3)` makes
    `persistentLookup` return that entry's `current` (42), not the miss
    default 0. So the match branch witnessed by
    `evm_sload_stack_spec_within` is genuinely reachable and the spec is not
    a vacuous miss→0 statement. `decide`-checked. -/
theorem sload_precondition_reachable :
    persistentLookup (7 : EvmWord) (3 : EvmWord)
        [{ addrHash := 7, slotKey := 3, original := 0, current := 42 }]
      = (42 : EvmWord) := by
  decide
