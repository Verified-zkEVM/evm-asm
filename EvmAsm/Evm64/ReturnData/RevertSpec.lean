/-
  ...
  returndata length load, `start + size` computation, and the three
  `.exit_invalid` checks that precede gas/MSIZE and the byte-copy loop in the
  emitted handler.
-/
...
theorem evm_returndatacopy_guard_success_spec_within
    ...
    (h_cap : ¬ BitVec.ult (256 : Word) (dataOffset + size)) :
    let code := evm_returndatacopy_revert_code frameHi frameLo off1 off2 off3 base
    cpsTripleWithin 11 base (base + 44) code
      ...
      ((.x12 ↦ᵣ sp) ** (.x14 ↦ᵣ destOffset) ** (.x15 ↦ᵣ dataOffset) **
       (.x16 ↦ᵣ size) ** (.x17 ↦ᵣ frameAddr) ** (.x18 ↦ᵣ (256 : Word)) **
  ...
  have hli_cap := li_spec_gen_within .x18 returnDataLen (256 : Word)
    (base + 36) (by decide)
  have hbltu_cap_raw := bltu_spec_gen_within .x18 .x19 off3
    (256 : Word) (dataOffset + size) (base + 40)
  rw [show (base + 40 : Word) + 4 = base + 44 from by bv_omega] at hbltu_cap_raw
  have hbltu_cap := cpsBranchWithin_ntakenStripPure2 hbltu_cap_raw
    (fun hp hQt => by
      obtain ⟨_, _, _, _, _, hQ⟩ := hQt
      exact h_cap ((sepConj_pure_right _).mp hQ).2)
  subst code
  unfold evm_returndatacopy_revert_code evm_returndatacopy_revert
  change cpsTripleWithin 11 base (base + 44)
    (CodeReq.ofProg base
      [.LD .x14 .x12 0,
  ...
  rw [show (base + 32 : Word) + 4 = base + 36 by bv_omega]
  rw [show (base + 36 : Word) + 4 = base + 40 by bv_omega]
  runBlock hld_dest hld_start hld_size hauipc haddi hld_len hadd_end
    hbltu_wrap hbltu_len hli_cap hbltu_cap
...
/-- Third failure path of the RETURNDATACOPY returndata guard:
    the range is within the stored length but exceeds the fixed 256-byte
    runtime frame cap. -/
theorem evm_returndatacopy_guard_cap_invalid_spec_within
    (frameHi : BitVec 20) (frameLo : BitVec 12)
    (off1 off2 off3 : BitVec 13)
    (sp base frameAddr x14Old x15Old x16Old x17Old x18Old x19Old : Word)
    (destOffset dataOffset size returnDataLen : Word)
    ...
    (h_nowrap : ¬ BitVec.ult (dataOffset + size) dataOffset)
    (h_in_bounds : ¬ BitVec.ult returnDataLen (dataOffset + size))
    (h_cap_oob : BitVec.ult (256 : Word) (dataOffset + size)) :
    let code := evm_returndatacopy_revert_code frameHi frameLo off1 off2 off3 base
    cpsTripleWithin 11 base (base + 40 + signExtend13 off3) code
      ((.x12 ↦ᵣ sp) ** (.x14 ↦ᵣ x14Old) ** (.x15 ↦ᵣ x15Old) **
       (.x16 ↦ᵣ x16Old) ** (.x17 ↦ᵣ x17Old) ** (.x18 ↦ᵣ x18Old) **
       (.x19 ↦ᵣ x19Old) **
       (sp ↦ₘ destOffset) ** ((sp + 32) ↦ₘ dataOffset) **
  ...
  have hbltu_cap_raw := bltu_spec_gen_within .x18 .x19 off3
    (256 : Word) (dataOffset + size) (base + 40)
  have hbltu_cap := cpsBranchWithin_takenStripPure2 hbltu_cap_raw
    (fun hp hQf => by
      obtain ⟨_, _, _, _, _, hQ⟩ := hQf
      exact ((sepConj_pure_right _).mp hQ).2 h_cap_oob)
  subst code
  unfold evm_returndatacopy_revert_code evm_returndatacopy_revert
  change cpsTripleWithin 11 base (base + 40 + signExtend13 off3)
    (CodeReq.ofProg base
      [.LD .x14 .x12 0,
       .LD .x15 .x12 (BitVec.ofNat 12 32),
       .LD .x16 .x12 (BitVec.ofNat 12 64),
...
theorem evm_returndatacopy_guard_success_stack_spec_within
    ...
    cpsTripleWithin 11 base (base + 44) code
      ...
      ((.x12 ↦ᵣ sp) ** (.x14 ↦ᵣ destOffset.getLimbN 0) **
       ...
       (.x17 ↦ᵣ frameAddr) ** (.x18 ↦ᵣ (256 : Word)) **
  ...
  have hRaw := evm_returndatacopy_guard_success_spec_within
    frameHi frameLo off1 off2 off3 sp base frameAddr x14Old x15Old x16Old
    x17Old x18Old x19Old (destOffset.getLimbN 0) (dataOffset.getLimbN 0)
    (size.getLimbN 0) returnDataLen hla h_nowrap h_in_bounds h_cap
...
/-- Stack-form lift of the 256-byte frame-cap invalid guard path. -/
theorem evm_returndatacopy_guard_cap_invalid_stack_spec_within
    (frameHi : BitVec 20) (frameLo : BitVec 12)
    (off1 off2 off3 : BitVec 13)
    (sp base frameAddr x14Old x15Old x16Old x17Old x18Old x19Old : Word)
    (destOffset dataOffset size : EvmWord) (returnDataLen : Word)
  ...
  let frame : Assertion :=
    ...
    (((sp + 64) + 24) ↦ₘ size.getLimbN 3) **
    evmStackIs (sp + 96) rest
  have hRaw := evm_returndatacopy_guard_cap_invalid_spec_within
    frameHi frameLo off1 off2 off3 sp base frameAddr x14Old x15Old x16Old
    x17Old x18Old x19Old (destOffset.getLimbN 0) (dataOffset.getLimbN 0)
    (size.getLimbN 0) returnDataLen hla h_nowrap h_in_bounds h_cap_oob
  have hFramePC : frame.pcFree := by
    dsimp [frame]
    pcFree
  have hFramed := cpsTripleWithin_frameR frame hFramePC hRaw
