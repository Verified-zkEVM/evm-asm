/-
  EvmAsm.Evm64.Terminating.StopSpec

  Halt-triple spec for the `STOP` (0x00) handler tail — the FIRST
  terminating/halt opcode, establishing the shape the rest of the family
  (INVALID / RETURN / REVERT / SELFDESTRUCT) follows.

  `evm_stop_stack_spec_within` is a `cpsTripleWithin 7` over the verified
  `evm_stop` program (`StopProgram.lean`), the byte image of the emitted
  `dispatchHaltRet 1` tail. It proves the flag-set-and-return behavior:

  * `evm_halt_flag` cell goes from `f0` to `1` (STOP routing code);
  * `x5 := 1`, `x6 := flagAddr` (the halt-flag cell address);
  * `x1 := resumeAddr` (the `.Ldispatch_resume` address) — STOP, unlike the
    stack-underflow guard's halt block, *rewrites* `x1` via `la x1, resume`
    before the `ret`, so control reaches `resume &&& ~~~1` (the dispatcher's
    flag-routing resume point) rather than the caller's return address;
  * the triple exits at `resume &&& ~~~1`.
  ...
  Proof method: the 4-instruction `li; la evm_halt_flag; sd` core mirrors
  `stackGuardHalt` (with routing code `1` in place of `7`); the extra
  `la .Ldispatch_resume` (2 instrs) + `ret` are composed on the end, with the
  `ret` reaching the freshly-materialised `resume` rather than a preserved
...
/-- **The verified STOP (0x00) halt tail.** Sitting at `hbase` (the emitted
    `dispatchHaltRet 1` tail entry), the seven instructions of `evm_stop` set
    the `evm_halt_flag` cell to the STOP routing code `1`, point `x1` at
    ...
    This is the halt-triple template: INVALID / RETURN / REVERT /
    SELFDESTRUCT instantiate the same shape with a different routing code
    (2/3/4) and any pre-halt body. -/
theorem evm_stop_stack_spec_within (hi2 : BitVec 20) (lo2 : BitVec 12)
    ...
    cpsTripleWithin 7 hbase (resume &&& ~~~1)
      (CodeReq.ofProg hbase (evm_stop hi2 lo2 hi1 lo1))
      ((.x5 ↦ᵣ v5) ** (.x6 ↦ᵣ v6) ** (.x1 ↦ᵣ v1) ** (flag ↦ₘ f0))
      ((.x5 ↦ᵣ (1 : Word)) ** (.x6 ↦ᵣ flag) ** (.x1 ↦ᵣ resume) **
        (flag ↦ₘ (1 : Word))) := by
  -- Step 1: LI x5, 1 at hbase.
  have t1 := li_spec_within .x5 v5 1 hbase (by nofun)
  ...
  have t2f := cpsTripleWithin_frameR
    ((.x5 ↦ᵣ (1 : Word)) ** (.x1 ↦ᵣ v1) ** (flag ↦ₘ f0)) (by pcFree) t2
  have hd12 : (CodeReq.singleton hbase (Instr.LI .x5 1)).Disjoint
  ...
  have t3f := cpsTripleWithin_frameR
    ((.x5 ↦ᵣ (1 : Word)) ** (.x1 ↦ᵣ v1) ** (flag ↦ₘ f0)) (by pcFree) t3
  have hd123 : ((CodeReq.singleton hbase (Instr.LI .x5 1)).union
  ...
  -- Step 4: SD x5, 0(x6) at hbase+12; the flag cell becomes 1.
  have t4 := sd_spec_within .x6 .x5 flag (1 : Word) f0 0 (hbase + 12)
  ...
  have hd1234 : (((CodeReq.singleton hbase (Instr.LI .x5 1)).union
  ...
  have t5f := cpsTripleWithin_frameR
    ((.x6 ↦ᵣ flag) ** (.x5 ↦ᵣ (1 : Word)) ** (flag ↦ₘ (1 : Word))) (by pcFree) t5
  have hd12345 : ((((CodeReq.singleton hbase (Instr.LI .x5 1)).union
      (CodeReq.singleton (hbase + 4) (Instr.AUIPC .x6 hi2))).union
  ...
  have t6f := cpsTripleWithin_frameR
    ((.x6 ↦ᵣ flag) ** (.x5 ↦ᵣ (1 : Word)) ** (flag ↦ₘ (1 : Word))) (by pcFree) t6
  have hd123456 : (((((CodeReq.singleton hbase (Instr.LI .x5 1)).union
      (CodeReq.singleton (hbase + 4) (Instr.AUIPC .x6 hi2))).union
      (CodeReq.singleton (hbase + 8) (Instr.ADDI .x6 .x6 lo2))).union
  ...
  have t7f := cpsTripleWithin_frameR
    ((.x6 ↦ᵣ flag) ** (.x5 ↦ᵣ (1 : Word)) ** (flag ↦ₘ (1 : Word))) (by pcFree) t7
  have hd1234567 : ((((((CodeReq.singleton hbase (Instr.LI .x5 1)).union
      (CodeReq.singleton (hbase + 4) (Instr.AUIPC .x6 hi2))).union
      (CodeReq.singleton (hbase + 8) (Instr.ADDI .x6 .x6 lo2))).union
      (CodeReq.singleton (hbase + 12) (Instr.SD .x6 .x5 0))).union
  ...
  have hcode : CodeReq.ofProg hbase (evm_stop hi2 lo2 hi1 lo1) =
      ((((((CodeReq.singleton hbase (Instr.LI .x5 1)).union
        (CodeReq.singleton (hbase + 4) (Instr.AUIPC .x6 hi2))).union
        (CodeReq.singleton (hbase + 8) (Instr.ADDI .x6 .x6 lo2))).union
        (CodeReq.singleton (hbase + 12) (Instr.SD .x6 .x5 0))).union
        (CodeReq.singleton (hbase + 16) (Instr.AUIPC .x1 hi1))).union
    ...
    simp only [evm_stop, CodeReq.ofProg_cons, CodeReq.ofProg_nil,
