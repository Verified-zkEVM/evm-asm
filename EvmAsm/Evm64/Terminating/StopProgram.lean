/-
  ...
  for `.Ldispatch_resume`). It is the byte image of the emitted tail; the
...
/-- The verified `Program` image of `dispatchHaltRet 1` (the emitted STOP
    tail): set `evm_halt_flag := 1`, point `x1` at `.Ldispatch_resume`, and
    `ret`. `hi2`/`lo2` are the `la evm_halt_flag` immediate pair; `hi1`/`lo1`
    the `la .Ldispatch_resume` pair. -/
