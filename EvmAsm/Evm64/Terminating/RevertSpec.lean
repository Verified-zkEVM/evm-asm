/-
  ...
  * the generic instruction specs (`li`/`ld`/`auipc`/`addi`/`mv`/`add`/`beq`/
  ...
  The full emitted tail (`Codegen.Programs.NoopHalt.returnRevertTail 2 rollbackAsm false`)
  is modelled here as one `Program` image `revertTailProg`, parameterised by the
  ...
  `evm_revert_stack_spec_within` is proved over `CodeReq.ofProg hbase
  revertTailProg`, from the post-gas handler entry `hbase` through the whole
  descriptor window and rollback stores to `resume &&& ~~~1`.
...
def revertDescBase : Word := 0xa0010000

/-- **The verified `Program` image of `returnRevertTail 2 rollbackAsm false`.**
    61 instructions from the post-gas handler entry through the descriptor
    window, rollback stores, and the shared halt core, parameterised by the
    linker `la` immediate pairs for the two `evm_memory` materialisations and
    the halt core.

    The first six immediate parameters are intentionally retained for arity
    symmetry with `ReturnSpec` but are not used by the REVERT byte image: the
    standalone REVERT path has no `system_call_mode` capture block. -/
def revertTailProg
    (_hiSCM : BitVec 20) (_loSCM : BitVec 12)
    (_hiLen : BitVec 20) (_loLen : BitVec 12)
    (_hiRd : BitVec 20) (_loRd : BitVec 12)
  ...
  [ -- prologue: read offset/size off the stack scratch (x12)
    .LD .x14 .x12 0,                        -- 0
    .LD .x15 .x12 32,                       -- 1
    -- header + descriptor-body zeroing
    .LI .x16 revertDescBase,                 -- 2
    ...
    -- label8: kind store, rollback, then the shared halt core
    .LI .x17 2,                              -- 47
    .SD .x16 .x17 32,                        -- 48  kind = 2 @ +32 (dword 4)
    .LD .x17 .x20 456,                       -- 49
    .SD .x20 .x17 448,                       -- 50
    .SD .x20 .x0 464,                        -- 51
    .LD .x17 .x20 480,                       -- 52
...
@[simp] theorem revertTailProg_length
    ...
    (revertTailProg hiSCM loSCM hiLen loLen hiRd loRd hiMem loMem hiMem2 loMem2
      hi2 lo2 hi1 lo1).length = 61 := rfl
...
local notation "PROG" =>
  revertTailProg hiSCM loSCM hiLen loLen hiRd loRd hiMem loMem hiMem2 loMem2 hi2 lo2 hi1 lo1
...
theorem revert_seg_prologue (hbase p off size x14o x15o : Word) :
  ...
  have t0f := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x15 : Reg) ↦ᵣ x15o) **
  ...
  have t1f := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x14 : Reg) ↦ᵣ off) **
      ((p + signExtend12 0) ↦ₘ off))
    (by pcFree) t1e
...
def revertDescZeroed (descInit : List (BitVec 8)) : List (BitVec 8) :=
  zeroDwords (zeroDwords descInit 0 4) 9 22

/-- **Header + descriptor-body zeroing segment** (`hbase+8 → hbase+56`).
...
theorem revert_seg_header (hbase : Word) (x16o x19o x21o : Word)
    ...
    cpsTripleWithin 118 (hbase + 8) (hbase + 56)
      ...
      (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x16 : Reg) ↦ᵣ x16o) **
        ...
        bytesRegion revertDescBase descInit)
      (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x16 : Reg) ↦ᵣ revertDescBase) **
        ((.x19 : Reg) ↦ᵣ (revertDescBase + BitVec.ofNat 64 (8 * (9 + 0 + 22)))) **
        ((.x21 : Reg) ↦ᵣ (0 : Word)) **
        bytesRegion revertDescBase (revertDescZeroed descInit)) := by
  -- idx 2: li x16, descBase
  have s0 := li_spec_within .x16 x16o revertDescBase (hbase + 8) (by nofun)
  ...
  have s0f := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x19 : Reg) ↦ᵣ x19o) ** ((.x21 : Reg) ↦ᵣ x21o) **
      bytesRegion revertDescBase descInit) (by pcFreeR) s0e
  -- idx 3..6: sd x0 at 0/8/16/24 (header dwords)
  have hd0 := bytesRegion_sd_off_within .x16 .x0 revertDescBase (0 : Word) (hbase + 12)
  ...
  have hd1 := bytesRegion_sd_off_within .x16 .x0 revertDescBase (0 : Word) (hbase + 16)
  ...
  have hd2 := bytesRegion_sd_off_within .x16 .x0 revertDescBase (0 : Word) (hbase + 20)
  ...
  have hd3 := bytesRegion_sd_off_within .x16 .x0 revertDescBase (0 : Word) (hbase + 24)
  ...
  have s1 := addi_spec_gen_within .x19 .x16 x19o revertDescBase (72 : BitVec 12)
    (hbase + 28) (by nofun)
  rw [show revertDescBase + signExtend12 (72 : BitVec 12)
        = revertDescBase + BitVec.ofNat 64 (8 * (9 + 0)) from by decide] at s1
  ...
  have s1f := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x21 : Reg) ↦ᵣ x21o) ** bytesRegion revertDescBase R0)
    (by pcFreeR) s1e
  -- idx 8: li x21, 22
  ...
  have s2f := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x16 : Reg) ↦ᵣ revertDescBase) **
      ((.x19 : Reg) ↦ᵣ (revertDescBase + BitVec.ofNat 64 (8 * (9 + 0)))) **
      bytesRegion revertDescBase R0) (by pcFreeR) s2e
  -- idx 9..13: the zeroing loop (returnZeroLoop, embedded via ofProg_mono_sub)
  have hloop := returnZeroLoop_spec_within (hbase + 36) revertDescBase R0 9 22 0
  ...
  have hloopf := cpsTripleWithin_frameR ((.x16 : Reg) ↦ᵣ revertDescBase) (by pcFreeR) hloopE
  ...
  refine cpsTripleWithin_mono_nSteps (by omega)
    (cpsTripleWithin_weaken (fun _ hp => by xperm_chunked hp) (fun _ hq => by
      simp only [revertDescZeroed]
      xperm_chunked hq) c6)

/-- The size clamped to the 176-byte descriptor-body window (unsigned min). -/
def revertClamp (v : Word) : Word := if BitVec.ult (176 : Word) v then (176 : Word) else v

/-- The size clamped to the first-32-byte prefix window (unsigned min). -/
def revertClamp32 (v : Word) : Word := if BitVec.ult (32 : Word) v then (32 : Word) else v

/-- Round-trip: a 64-bit word equals `ofNat` of its own `toNat`. -/
theorem revert_word_ofNat_toNat (w : Word) : BitVec.ofNat 64 w.toNat = w := by
  apply BitVec.eq_of_toNat_eq
  rw [BitVec.toNat_ofNat, Nat.mod_eq_of_lt w.isLt]

/-- `revertClamp` never exceeds 176. -/
theorem revertClamp_toNat_le (v : Word) : (revertClamp v).toNat ≤ 176 := by
  unfold revertClamp
  split
  · decide
  · next h =>
...
/-- `revertClamp32` never exceeds 32. -/
theorem revertClamp32_toNat_le (v : Word) : (revertClamp32 v).toNat ≤ 32 := by
  unfold revertClamp32
  split
  · decide
  · next h =>
      simp only [BitVec.ult, decide_eq_true_eq,
...
theorem revertDescBase_valid (k : Nat) (hk : k < 256) :
    isValidByteAccess (revertDescBase + BitVec.ofNat 64 k) = true := by
  have h : (revertDescBase + BitVec.ofNat 64 k).toNat = 2684420096 + k := by
    rw [BitVec.toNat_add, show (revertDescBase).toNat = 2684420096 from by decide,
...
/-- **Size-clamp-to-176 segment** (`hbase+56 → hbase+72`).  `x21 := x15`,
    ...
    At label3, `x21 = min(x15,176) = revertClamp x15` and `x22 = 176`. -/
theorem revert_seg_clamp176 (hbase : Word) (x15v x21o x22o : Word) :
    cpsTripleWithin 4 (hbase + 56) (hbase + 72)
      ...
      (((.x15 : Reg) ↦ᵣ x15v) ** ((.x21 : Reg) ↦ᵣ revertClamp x15v) **
  ...
  · -- size > 176: bgeu NOT taken; execute mv x21, x22 (idx 17)
    ...
    refine cpsTripleWithin_mono_nSteps (by omega)
      (cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp) (fun _ hq => by
        simp only [revertClamp, if_pos h]; xperm_hyp hq) c2)
  · -- size ≤ 176: bgeu taken; skip mv, land at label3
    ...
    refine cpsTripleWithin_mono_nSteps (by omega)
      (cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp) (fun _ hq => by
        simp only [revertClamp, if_neg h]; xperm_hyp hq) c1)

/-- **Size/clamped stores + `evm_memory → descriptor+72` copy segment**
    (`hbase+72 → hbase+128`).  Stores `size@+64` and `clamped@+248`, points
...
theorem revert_seg_copy1 (hbase evmMemBase : Word)
    ...
    (hOff : x14v.toNat + (revertClamp x15v).toNat ≤ memBytes.length)
    (hlaMem : (hbase + (80 : Word)) + ((hiMem.zeroExtend 32 : BitVec 32) <<< 12).signExtend 64
                + signExtend12 loMem = evmMemBase) :
    cpsTripleWithin (7 * (revertClamp x15v).toNat + 8) (hbase + 72) (hbase + 128)
      (TAILCR hbase)
      (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x14 : Reg) ↦ᵣ x14v) ** ((.x15 : Reg) ↦ᵣ x15v) **
        ((.x16 : Reg) ↦ᵣ revertDescBase) ** ((.x17 : Reg) ↦ᵣ x17o) **
        ((.x19 : Reg) ↦ᵣ x19o) ** ((.x21 : Reg) ↦ᵣ revertClamp x15v) **
        ((.x22 : Reg) ↦ᵣ x22o) ** ((.x23 : Reg) ↦ᵣ x23o) **
        bytesRegion revertDescBase (revertDescZeroed descInit) **
        bytesRegion evmMemBase memBytes)
      (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x14 : Reg) ↦ᵣ x14v) ** ((.x15 : Reg) ↦ᵣ x15v) **
        ((.x16 : Reg) ↦ᵣ revertDescBase) **
        ((.x17 : Reg) ↦ᵣ (evmMemBase + BitVec.ofNat 64 (x14v.toNat + 0 + (revertClamp x15v).toNat))) **
        ((.x19 : Reg) ↦ᵣ (revertDescBase + BitVec.ofNat 64 (72 + 0 + (revertClamp x15v).toNat))) **
        ((.x21 : Reg) ↦ᵣ revertClamp x15v) ** ((.x22 : Reg) ↦ᵣ (0 : Word)) ** regOwn .x23 **
        bytesRegion revertDescBase
          (copyIntoRegion
            (setBytes (setBytes (revertDescZeroed descInit) 64 (dwordBytes x15v))
              248 (dwordBytes (revertClamp x15v))) memBytes 72 x14v.toNat (revertClamp x15v).toNat) **
        bytesRegion evmMemBase memBytes) := by
  have hReg256 : (revertDescZeroed descInit).length = 256 := by
    simp only [revertDescZeroed, zeroDwords_length, hDescLen]
  have hclamp := revertClamp_toNat_le x15v
  -- idx 18: sd x16, size, 64(x16)
  have d0 := bytesRegion_sd_off_within .x16 .x15 revertDescBase x15v (hbase + 72)
    (revertDescZeroed descInit) 8 (64 : BitVec 12) (by decide) (by rw [hReg256]; omega)
  ...
  have d0f := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x14 : Reg) ↦ᵣ x14v) ** ((.x17 : Reg) ↦ᵣ x17o) **
      ((.x19 : Reg) ↦ᵣ x19o) ** ((.x21 : Reg) ↦ᵣ revertClamp x15v) ** ((.x22 : Reg) ↦ᵣ x22o) **
      ((.x23 : Reg) ↦ᵣ x23o) ** bytesRegion evmMemBase memBytes) (by pcFreeR) d0e
  -- idx 19: sd x16, clamped, 248(x16)
  have d1 := bytesRegion_sd_off_within .x16 .x21 revertDescBase (revertClamp x15v) (hbase + 76)
    (setBytes (revertDescZeroed descInit) 64 (dwordBytes x15v)) 31 (248 : BitVec 12) (by decide)
  ...
  have a0f := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x14 : Reg) ↦ᵣ x14v) ** ((.x15 : Reg) ↦ᵣ x15v) **
      ((.x16 : Reg) ↦ᵣ revertDescBase) ** ((.x19 : Reg) ↦ᵣ x19o) **
      ((.x21 : Reg) ↦ᵣ revertClamp x15v) ** ((.x22 : Reg) ↦ᵣ x22o) ** ((.x23 : Reg) ↦ᵣ x23o) **
      bytesRegion revertDescBase
        (setBytes (setBytes (revertDescZeroed descInit) 64 (dwordBytes x15v)) 248
          (dwordBytes (revertClamp x15v))) ** bytesRegion evmMemBase memBytes) (by pcFreeR) a0e
  ...
  have a1f := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x14 : Reg) ↦ᵣ x14v) ** ((.x15 : Reg) ↦ᵣ x15v) **
      ...
      bytesRegion revertDescBase
        (setBytes (setBytes (revertDescZeroed descInit) 64 (dwordBytes x15v)) 248
          (dwordBytes (revertClamp x15v))) ** bytesRegion evmMemBase memBytes) (by pcFreeR) a1e
  ...
  have a2f := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x15 : Reg) ↦ᵣ x15v) ** ((.x16 : Reg) ↦ᵣ revertDescBase) **
      ((.x19 : Reg) ↦ᵣ x19o) ** ((.x21 : Reg) ↦ᵣ revertClamp x15v) ** ((.x22 : Reg) ↦ᵣ x22o) **
      ((.x23 : Reg) ↦ᵣ x23o) **
      bytesRegion revertDescBase
        (setBytes (setBytes (revertDescZeroed descInit) 64 (dwordBytes x15v)) 248
          (dwordBytes (revertClamp x15v))) ** bytesRegion evmMemBase memBytes) (by pcFreeR) a2e
  -- idx 23: addi x19, x16, 72  (→ descBase + 72)
  have a3 := addi_spec_gen_within .x19 .x16 x19o revertDescBase (72 : BitVec 12) (hbase + 92) (by nofun)
  rw [show revertDescBase + signExtend12 (72 : BitVec 12)
        = revertDescBase + BitVec.ofNat 64 (72 + 0) from by decide,
  ...
  have a3f := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x14 : Reg) ↦ᵣ x14v) ** ((.x15 : Reg) ↦ᵣ x15v) **
      ((.x17 : Reg) ↦ᵣ (evmMemBase + x14v)) ** ((.x21 : Reg) ↦ᵣ revertClamp x15v) **
      ((.x22 : Reg) ↦ᵣ x22o) ** ((.x23 : Reg) ↦ᵣ x23o) **
      bytesRegion revertDescBase
        (setBytes (setBytes (revertDescZeroed descInit) 64 (dwordBytes x15v)) 248
          (dwordBytes (revertClamp x15v))) ** bytesRegion evmMemBase memBytes) (by pcFreeR) a3e
  -- idx 24: mv x22, x21  (→ clamped)
  have a4 := mv_spec_within .x22 .x21 (revertClamp x15v) x22o (hbase + 96) (by nofun)
  ...
  have a4f := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x14 : Reg) ↦ᵣ x14v) ** ((.x15 : Reg) ↦ᵣ x15v) **
      ((.x16 : Reg) ↦ᵣ revertDescBase) ** ((.x17 : Reg) ↦ᵣ (evmMemBase + x14v)) **
      ((.x19 : Reg) ↦ᵣ (revertDescBase + BitVec.ofNat 64 (72 + 0))) ** ((.x23 : Reg) ↦ᵣ x23o) **
      bytesRegion revertDescBase
        (setBytes (setBytes (revertDescZeroed descInit) 64 (dwordBytes x15v)) 248
          (dwordBytes (revertClamp x15v))) ** bytesRegion evmMemBase memBytes) (by pcFreeR) a4e
  -- idx 25..31: the copy loop
  have hStoresLen : (setBytes (setBytes (revertDescZeroed descInit) 64 (dwordBytes x15v)) 248
      (dwordBytes (revertClamp x15v))).length = 256 := by
    rw [length_setBytes, length_setBytes, hReg256]
  have hloop := returnCopyLoop_spec_within (hbase + 100) evmMemBase revertDescBase memBytes
    (setBytes (setBytes (revertDescZeroed descInit) 64 (dwordBytes x15v)) 248
      (dwordBytes (revertClamp x15v))) x14v.toNat 72 (revertClamp x15v).toNat 0 x23o
    hSrcAlign (by decide) (by simpa using hOff) (by rw [hStoresLen]; omega) hSrcOver
    (by rw [hStoresLen]; decide)
    hSrcValid (fun k hk => revertDescBase_valid k (by rw [hStoresLen] at hk; exact hk))
  rw [show copyIntoRegion
        (setBytes (setBytes (revertDescZeroed descInit) 64 (dwordBytes x15v)) 248
          (dwordBytes (revertClamp x15v))) memBytes 72 x14v.toNat 0
        = setBytes (setBytes (revertDescZeroed descInit) 64 (dwordBytes x15v)) 248
          (dwordBytes (revertClamp x15v)) from rfl,
      show BitVec.ofNat 64 (revertClamp x15v).toNat = revertClamp x15v from revert_word_ofNat_toNat _,
      show evmMemBase + BitVec.ofNat 64 (x14v.toNat + 0) = evmMemBase + x14v from by
        rw [Nat.add_zero, revert_word_ofNat_toNat],
      show (0 : Nat) + (revertClamp x15v).toNat = (revertClamp x15v).toNat from Nat.zero_add _,
  ...
  have hloopf := cpsTripleWithin_frameR
    (((.x14 : Reg) ↦ᵣ x14v) ** ((.x15 : Reg) ↦ᵣ x15v) ** ((.x16 : Reg) ↦ᵣ revertDescBase) **
      ((.x21 : Reg) ↦ᵣ revertClamp x15v)) (by pcFreeR) hloopE
  -- compose
...
/-- **Second `la x17, evm_memory` + clamp-to-32 segment**
    (`hbase+128 → hbase+156`).  Re-points `x17` at `evm_memory+offset`, then
    `x22 := min(x15,32) = revertClamp32 x15`, `x21 := 32`, via `bgeu x21,x22,6f`.
...
theorem revert_seg_clamp32 (hbase evmMemBase : Word)
    ...
    cpsTripleWithin 7 (hbase + 128) (hbase + 156)
      ...
      (((.x14 : Reg) ↦ᵣ x14v) ** ((.x15 : Reg) ↦ᵣ x15v) **
        ...
        ((.x22 : Reg) ↦ᵣ revertClamp32 x15v)) := by
  -- idx 32: auipc x17
  ...
  · -- size > 32: bgeu NOT taken; mv x22, x21 (idx 38)
    ...
    refine cpsTripleWithin_mono_nSteps (by omega)
      (cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp) (fun _ hq => by
        simp only [revertClamp32, if_pos h]; xperm_hyp hq) c2)
  · -- size ≤ 32: bgeu taken; land at label6
    ...
    refine cpsTripleWithin_mono_nSteps (by omega)
      (cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp) (fun _ hq => by
        simp only [revertClamp32, if_neg h]; xperm_hyp hq) c1)
...
theorem revert_seg_copy2 (hbase evmMemBase : Word)
    ...
    (hSrcValid : ∀ k, k < memBytes.length →
      isValidByteAccess (evmMemBase + BitVec.ofNat 64 k) = true)
    (hOff32 : x14v.toNat + (revertClamp32 x15v).toNat ≤ memBytes.length) :
    cpsTripleWithin (7 * (revertClamp32 x15v).toNat + 2) (hbase + 156) (hbase + 188)
      (TAILCR hbase)
      ((((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x16 : Reg) ↦ᵣ revertDescBase) **
        ((.x17 : Reg) ↦ᵣ (evmMemBase + x14v)) ** ((.x19 : Reg) ↦ᵣ x19o) **
        ((.x22 : Reg) ↦ᵣ revertClamp32 x15v) **
        bytesRegion revertDescBase descBytes ** bytesRegion evmMemBase memBytes) ** regOwn .x23)
      (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x16 : Reg) ↦ᵣ revertDescBase) **
        ((.x17 : Reg) ↦ᵣ (evmMemBase + BitVec.ofNat 64 (x14v.toNat + 0 + (revertClamp32 x15v).toNat))) **
        ((.x19 : Reg) ↦ᵣ (revertDescBase + BitVec.ofNat 64 (revertClamp32 x15v).toNat)) **
        ((.x22 : Reg) ↦ᵣ (0 : Word)) ** regOwn .x23 **
        bytesRegion revertDescBase
          (copyIntoRegion descBytes memBytes 0 x14v.toNat (revertClamp32 x15v).toNat) **
        bytesRegion evmMemBase memBytes) := by
  refine cpsTripleWithin_of_forall_regIs_to_regOwn (fun x23o => ?_)
  -- idx 39: mv x19, x16
  have m0 := mv_spec_within .x19 .x16 revertDescBase x19o (hbase + 156) (by nofun)
  ...
  have m0f := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x17 : Reg) ↦ᵣ (evmMemBase + x14v)) **
      ((.x22 : Reg) ↦ᵣ revertClamp32 x15v) ** ((.x23 : Reg) ↦ᵣ x23o) **
      bytesRegion revertDescBase descBytes ** bytesRegion evmMemBase memBytes) (by pcFreeR) m0e
  -- idx 40..46: copy loop 2 (dest +0)
  have hloop := returnCopyLoop_spec_within (hbase + 160) evmMemBase revertDescBase memBytes
    descBytes x14v.toNat 0 (revertClamp32 x15v).toNat 0 x23o
    hSrcAlign (by decide) (by simpa using hOff32)
    (by have := revertClamp32_toNat_le x15v; rw [hDescLen]; omega) hSrcOver
    (by rw [hDescLen]; decide)
    hSrcValid (fun k hk => revertDescBase_valid k (by rw [hDescLen] at hk; exact hk))
  rw [show copyIntoRegion descBytes memBytes 0 x14v.toNat 0 = descBytes from rfl,
      show BitVec.ofNat 64 (revertClamp32 x15v).toNat = revertClamp32 x15v from revert_word_ofNat_toNat _,
      show evmMemBase + BitVec.ofNat 64 (x14v.toNat + 0) = evmMemBase + x14v from by
        rw [Nat.add_zero, revert_word_ofNat_toNat],
      show revertDescBase + BitVec.ofNat 64 (0 + 0) = revertDescBase from by decide,
      show (0 : Nat) + (revertClamp32 x15v).toNat = (revertClamp32 x15v).toNat from Nat.zero_add _,
...
/-- **Kind store + rollback + halt core segment** (`hbase+188 → resume &&& ~~~1`).
    Stores `kind = 2 @ +32`, restores the rollback-tracked log lengths through
    the `x20` environment pointer, then runs the shared `dispatchHaltRet 2`
    halt core. -/
theorem revert_seg_kind_rollback_halt (hbase flag resume env : Word)
    (x17o v5 v6 v1 f0 persistentLen persistentOld transientOld eventLen eventOld : Word)
    (descBytes : List (BitVec 8))
    (hDescLen : descBytes.length = 256)
    ...
    cpsTripleWithin 14 (hbase + 188) (resume &&& ~~~1)
      (TAILCR hbase)
      (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x16 : Reg) ↦ᵣ revertDescBase) **
        ((.x17 : Reg) ↦ᵣ x17o) ** ((.x20 : Reg) ↦ᵣ env) ** ((.x5 : Reg) ↦ᵣ v5) **
        ((.x6 : Reg) ↦ᵣ v6) ** ((.x1 : Reg) ↦ᵣ v1) ** (flag ↦ₘ f0) **
        ((env + signExtend12 (456 : BitVec 12)) ↦ₘ persistentLen) **
        ((env + signExtend12 (448 : BitVec 12)) ↦ₘ persistentOld) **
        ((env + signExtend12 (464 : BitVec 12)) ↦ₘ transientOld) **
        ((env + signExtend12 (480 : BitVec 12)) ↦ₘ eventLen) **
        ((env + signExtend12 (472 : BitVec 12)) ↦ₘ eventOld) **
        bytesRegion revertDescBase descBytes)
      (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x16 : Reg) ↦ᵣ revertDescBase) **
        ((.x17 : Reg) ↦ᵣ eventLen) ** ((.x20 : Reg) ↦ᵣ env) ** ((.x5 : Reg) ↦ᵣ (2 : Word)) **
        ((.x6 : Reg) ↦ᵣ flag) ** ((.x1 : Reg) ↦ᵣ resume) ** (flag ↦ₘ (2 : Word)) **
        ((env + signExtend12 (456 : BitVec 12)) ↦ₘ persistentLen) **
        ((env + signExtend12 (448 : BitVec 12)) ↦ₘ persistentLen) **
        ((env + signExtend12 (464 : BitVec 12)) ↦ₘ (0 : Word)) **
        ((env + signExtend12 (480 : BitVec 12)) ↦ₘ eventLen) **
        ((env + signExtend12 (472 : BitVec 12)) ↦ₘ eventLen) **
        bytesRegion revertDescBase (setBytes descBytes 32 (dwordBytes (2 : Word)))) := by
  -- idx 47: li x17, 2
  ...
  have k0f := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x16 : Reg) ↦ᵣ revertDescBase) **
      ((.x20 : Reg) ↦ᵣ env) ** ((.x5 : Reg) ↦ᵣ v5) ** ((.x6 : Reg) ↦ᵣ v6) **
      ((.x1 : Reg) ↦ᵣ v1) ** (flag ↦ₘ f0) **
      ((env + signExtend12 (456 : BitVec 12)) ↦ₘ persistentLen) **
      ((env + signExtend12 (448 : BitVec 12)) ↦ₘ persistentOld) **
      ((env + signExtend12 (464 : BitVec 12)) ↦ₘ transientOld) **
      ((env + signExtend12 (480 : BitVec 12)) ↦ₘ eventLen) **
      ((env + signExtend12 (472 : BitVec 12)) ↦ₘ eventOld) **
      bytesRegion revertDescBase descBytes) (by pcFreeR) k0e
  -- idx 48: sd x16, kind, 32(x16)
  have k1 := bytesRegion_sd_off_within .x16 .x17 revertDescBase (2 : Word) (hbase + 192)
  ...
  have k1f := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x20 : Reg) ↦ᵣ env) ** ((.x5 : Reg) ↦ᵣ v5) **
      ((.x6 : Reg) ↦ᵣ v6) ** ((.x1 : Reg) ↦ᵣ v1) ** (flag ↦ₘ f0) **
      ((env + signExtend12 (456 : BitVec 12)) ↦ₘ persistentLen) **
      ((env + signExtend12 (448 : BitVec 12)) ↦ₘ persistentOld) **
      ((env + signExtend12 (464 : BitVec 12)) ↦ₘ transientOld) **
      ...
      ((env + signExtend12 (472 : BitVec 12)) ↦ₘ eventOld))
    (by pcFreeR) k1e
  -- idx 49: ld x17, 456(x20)
  have r0 := ld_spec_within .x17 .x20 env (2 : Word) persistentLen 456 (hbase + 196) (by nofun)
  ...
  have r0f := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x16 : Reg) ↦ᵣ revertDescBase) **
      ((.x5 : Reg) ↦ᵣ v5) ** ((.x6 : Reg) ↦ᵣ v6) ** ((.x1 : Reg) ↦ᵣ v1) **
      (flag ↦ₘ f0) ** ((env + signExtend12 (448 : BitVec 12)) ↦ₘ persistentOld) **
      ((env + signExtend12 (464 : BitVec 12)) ↦ₘ transientOld) **
      ((env + signExtend12 (480 : BitVec 12)) ↦ₘ eventLen) **
      ((env + signExtend12 (472 : BitVec 12)) ↦ₘ eventOld) **
      bytesRegion revertDescBase (setBytes descBytes 32 (dwordBytes (2 : Word)))) (by pcFreeR) r0e
  -- idx 50: sd x17, 448(x20)
  have r1 := sd_spec_within .x20 .x17 env persistentLen persistentOld 448 (hbase + 200)
  ...
  have r1f := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x16 : Reg) ↦ᵣ revertDescBase) **
      ((.x5 : Reg) ↦ᵣ v5) ** ((.x6 : Reg) ↦ᵣ v6) ** ((.x1 : Reg) ↦ᵣ v1) **
      (flag ↦ₘ f0) ** ((env + signExtend12 (456 : BitVec 12)) ↦ₘ persistentLen) **
      ((env + signExtend12 (464 : BitVec 12)) ↦ₘ transientOld) **
      ((env + signExtend12 (480 : BitVec 12)) ↦ₘ eventLen) **
      ((env + signExtend12 (472 : BitVec 12)) ↦ₘ eventOld) **
      bytesRegion revertDescBase (setBytes descBytes 32 (dwordBytes (2 : Word)))) (by pcFreeR) r1e
  -- idx 51: sd x0, 464(x20)
  have r2 := sd_spec_within .x20 .x0 env (0 : Word) transientOld 464 (hbase + 204)
  ...
  have r2f := cpsTripleWithin_frameR
    (((.x16 : Reg) ↦ᵣ revertDescBase) ** ((.x17 : Reg) ↦ᵣ persistentLen) **
      ((.x5 : Reg) ↦ᵣ v5) ** ((.x6 : Reg) ↦ᵣ v6) ** ((.x1 : Reg) ↦ᵣ v1) **
      (flag ↦ₘ f0) ** ((env + signExtend12 (456 : BitVec 12)) ↦ₘ persistentLen) **
      ((env + signExtend12 (448 : BitVec 12)) ↦ₘ persistentLen) **
      ((env + signExtend12 (480 : BitVec 12)) ↦ₘ eventLen) **
      ((env + signExtend12 (472 : BitVec 12)) ↦ₘ eventOld) **
      bytesRegion revertDescBase (setBytes descBytes 32 (dwordBytes (2 : Word)))) (by pcFreeR) r2e
  -- idx 52: ld x17, 480(x20)
  have r3 := ld_spec_within .x17 .x20 env persistentLen eventLen 480 (hbase + 208) (by nofun)
  ...
  have r3f := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x16 : Reg) ↦ᵣ revertDescBase) **
      ...
      ((env + signExtend12 (464 : BitVec 12)) ↦ₘ (0 : Word)) **
      ((env + signExtend12 (472 : BitVec 12)) ↦ₘ eventOld) **
      bytesRegion revertDescBase (setBytes descBytes 32 (dwordBytes (2 : Word)))) (by pcFreeR) r3e
  -- idx 53: sd x17, 472(x20)
  have r4 := sd_spec_within .x20 .x17 env eventLen eventOld 472 (hbase + 212)
  ...
  have r4f := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x16 : Reg) ↦ᵣ revertDescBase) **
      ...
      ((env + signExtend12 (464 : BitVec 12)) ↦ₘ (0 : Word)) **
      ((env + signExtend12 (480 : BitVec 12)) ↦ₘ eventLen) **
      bytesRegion revertDescBase (setBytes descBytes 32 (dwordBytes (2 : Word)))) (by pcFreeR) r4e
  ...
  have hhf := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x16 : Reg) ↦ᵣ revertDescBase) **
      ((.x17 : Reg) ↦ᵣ eventLen) ** ((.x20 : Reg) ↦ᵣ env) **
      ((env + signExtend12 (456 : BitVec 12)) ↦ₘ persistentLen) **
      ((env + signExtend12 (448 : BitVec 12)) ↦ₘ persistentLen) **
      ((env + signExtend12 (464 : BitVec 12)) ↦ₘ (0 : Word)) **
      ((env + signExtend12 (480 : BitVec 12)) ↦ₘ eventLen) **
      ((env + signExtend12 (472 : BitVec 12)) ↦ₘ eventLen) **
      bytesRegion revertDescBase (setBytes descBytes 32 (dwordBytes (2 : Word)))) (by pcFreeR) hhE
...
/-- **The verified REVERT (0xfd) return-data window + rollback + halt core.**
    ...
    to `+72`, the first `min(size,32)` bytes copied to `+0`, `kind = 2@+32`),
    rolls back the tracked log lengths, and halts via the shared
    `dispatchHaltRet 2` core (`evm_halt_flag := 2`, `x1 := resume`, `ret` to
    `resume &&& ~~~1`).

    The memory-gas `preBody` (`.exit_outofgas`) is framed OUT (decision-1 TCB);
    the `la` immediates stay as reconstruction hypotheses (`hlaMem`/`hlaMem2`/
    `hla2`/`hla1`), the shared deferred byte-check. -/
theorem evm_revert_stack_spec_within
    (hbase p evmMemBase flag resume env : Word)
    (off size x1o x5o x6o x14o x15o x16o x17o x19o x21o x22o x23o f0 : Word)
    (persistentLen persistentOld transientOld eventLen eventOld : Word)
    ...
    (hSrcValid : ∀ k, k < memBytes.length →
      isValidByteAccess (evmMemBase + BitVec.ofNat 64 k) = true)
    (hOff : off.toNat + (revertClamp size).toNat ≤ memBytes.length)
    (hOff32 : off.toNat + (revertClamp32 size).toNat ≤ memBytes.length)
    (hlaMem : (hbase + (80 : Word)) + ((hiMem.zeroExtend 32 : BitVec 32) <<< 12).signExtend 64
                + signExtend12 loMem = evmMemBase)
    (hlaMem2 : (hbase + (128 : Word)) + ((hiMem2.zeroExtend 32 : BitVec 32) <<< 12).signExtend 64
                + signExtend12 loMem2 = evmMemBase)
    ...
    (hla1 : (hbase + 216 + 16) + ((hi1.zeroExtend 32 : BitVec 32) <<< 12).signExtend 64
              + signExtend12 lo1 = resume) :
    cpsTripleWithin (155 + 7 * (revertClamp size).toNat + 7 * (revertClamp32 size).toNat)
      hbase (resume &&& ~~~1)
      (TAILCR hbase)
      ...
      (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x1 : Reg) ↦ᵣ resume) ** ((.x5 : Reg) ↦ᵣ (2 : Word)) **
        ...
        ((.x15 : Reg) ↦ᵣ size) ** ((.x16 : Reg) ↦ᵣ revertDescBase) ** ((.x17 : Reg) ↦ᵣ eventLen) **
        ((.x19 : Reg) ↦ᵣ (revertDescBase + BitVec.ofNat 64 (revertClamp32 size).toNat)) **
        ((.x20 : Reg) ↦ᵣ env) **
        ...
        bytesRegion revertDescBase
          (setBytes
            (copyIntoRegion
              (copyIntoRegion
                (setBytes (setBytes (revertDescZeroed descInit) 64 (dwordBytes size)) 248
                  (dwordBytes (revertClamp size))) memBytes 72 off.toNat (revertClamp size).toNat)
              memBytes 0 off.toNat (revertClamp32 size).toNat) 32 (dwordBytes (2 : Word))) **
        bytesRegion evmMemBase memBytes) := by
  -- length lemmas for the intermediate descriptor images
  have hRc1len : (copyIntoRegion
      (setBytes (setBytes (revertDescZeroed descInit) 64 (dwordBytes size)) 248
        (dwordBytes (revertClamp size))) memBytes 72 off.toNat (revertClamp size).toNat).length = 256 := by
    simp only [copyIntoRegion_length, length_setBytes, revertDescZeroed, zeroDwords_length, hDescLen]
  have hRc2len : (copyIntoRegion
      (copyIntoRegion
        (setBytes (setBytes (revertDescZeroed descInit) 64 (dwordBytes size)) 248
          (dwordBytes (revertClamp size))) memBytes 72 off.toNat (revertClamp size).toNat)
      memBytes 0 off.toNat (revertClamp32 size).toNat).length = 256 := by
    simp only [copyIntoRegion_length, length_setBytes, revertDescZeroed, zeroDwords_length, hDescLen]
  -- Segment 1: prologue (hbase → hbase+8)
  have S1 := revert_seg_prologue hiSCM loSCM hiLen loLen hiRd loRd hiMem loMem hiMem2 loMem2
    hi2 lo2 hi1 lo1 hbase p off size x14o x15o
  have S1f := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ x1o) ** ((.x5 : Reg) ↦ᵣ x5o) ** ((.x6 : Reg) ↦ᵣ x6o) **
      ((.x16 : Reg) ↦ᵣ x16o) ** ((.x17 : Reg) ↦ᵣ x17o) ** ((.x19 : Reg) ↦ᵣ x19o) **
      ((.x20 : Reg) ↦ᵣ env) ** ((.x21 : Reg) ↦ᵣ x21o) ** ((.x22 : Reg) ↦ᵣ x22o) **
      ...
      ((env + signExtend12 (480 : BitVec 12)) ↦ₘ eventLen) **
      ((env + signExtend12 (472 : BitVec 12)) ↦ₘ eventOld) **
      bytesRegion revertDescBase descInit ** bytesRegion evmMemBase memBytes) (by pcFreeR) S1
  -- Segment 2: header + zero loop (hbase+8 → hbase+56)
  have S2 := revert_seg_header hiSCM loSCM hiLen loLen hiRd loRd hiMem loMem hiMem2 loMem2
    hi2 lo2 hi1 lo1 hbase x16o x19o x21o descInit hDescLen
  have S2f := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ x1o) ** ((.x5 : Reg) ↦ᵣ x5o) ** ((.x6 : Reg) ↦ᵣ x6o) **
      ((.x12 : Reg) ↦ᵣ p) ** ((.x14 : Reg) ↦ᵣ off) ** ((.x15 : Reg) ↦ᵣ size) **
      ((.x17 : Reg) ↦ᵣ x17o) ** ((.x20 : Reg) ↦ᵣ env) **
      ((.x22 : Reg) ↦ᵣ x22o) ** ((.x23 : Reg) ↦ᵣ x23o) **
      ((p + signExtend12 0) ↦ₘ off) ** ((p + signExtend12 32) ↦ₘ size) **
      (flag ↦ₘ f0) **
      ((env + signExtend12 (456 : BitVec 12)) ↦ₘ persistentLen) **
  ...
  have S3 := revert_seg_clamp176 hiSCM loSCM hiLen loLen hiRd loRd hiMem loMem hiMem2 loMem2
  ...
  have S4 := revert_seg_copy1 hiSCM loSCM hiLen loLen hiRd loRd hiMem loMem hiMem2 loMem2
    hi2 lo2 hi1 lo1 hbase evmMemBase off size x17o
    (revertDescBase + BitVec.ofNat 64 (8 * (9 + 0 + 22))) (176 : Word) x23o descInit memBytes
  ...
  have S4f := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ x1o) ** ((.x5 : Reg) ↦ᵣ x5o) ** ((.x6 : Reg) ↦ᵣ x6o) **
      ...
      ((.x20 : Reg) ↦ᵣ env) ** (flag ↦ₘ f0) **
      ((env + signExtend12 (456 : BitVec 12)) ↦ₘ persistentLen) **
      ((env + signExtend12 (448 : BitVec 12)) ↦ₘ persistentOld) **
      ((env + signExtend12 (464 : BitVec 12)) ↦ₘ transientOld) **
      ((env + signExtend12 (480 : BitVec 12)) ↦ₘ eventLen) **
      ((env + signExtend12 (472 : BitVec 12)) ↦ₘ eventOld)) (by pcFreeR) S4
  -- Segment 5: second la + clamp to 32 (hbase+128 → hbase+156)
  have S5 := revert_seg_clamp32 hiSCM loSCM hiLen loLen hiRd loRd hiMem loMem hiMem2 loMem2
    hi2 lo2 hi1 lo1 hbase evmMemBase off size
    (evmMemBase + BitVec.ofNat 64 (off.toNat + 0 + (revertClamp size).toNat))
    (revertClamp size) (0 : Word) hlaMem2
  have S5f := cpsTripleWithin_frameR
    (((.x0 : Reg) ↦ᵣ (0 : Word)) ** ((.x1 : Reg) ↦ᵣ x1o) ** ((.x5 : Reg) ↦ᵣ x5o) **
      ((.x6 : Reg) ↦ᵣ x6o) ** ((.x12 : Reg) ↦ᵣ p) ** ((.x16 : Reg) ↦ᵣ revertDescBase) **
      ((.x19 : Reg) ↦ᵣ (revertDescBase + BitVec.ofNat 64 (72 + 0 + (revertClamp size).toNat))) **
      ((.x20 : Reg) ↦ᵣ env) **
  ...
  have S6 := revert_seg_copy2 hiSCM loSCM hiLen loLen hiRd loRd hiMem loMem hiMem2 loMem2
    hi2 lo2 hi1 lo1 hbase evmMemBase off size
    (revertDescBase + BitVec.ofNat 64 (72 + 0 + (revertClamp size).toNat))
    (copyIntoRegion
      (setBytes (setBytes (revertDescZeroed descInit) 64 (dwordBytes size)) 248
        (dwordBytes (revertClamp size))) memBytes 72 off.toNat (revertClamp size).toNat)
  ...
  have S6f := cpsTripleWithin_frameR
    (((.x1 : Reg) ↦ᵣ x1o) ** ((.x5 : Reg) ↦ᵣ x5o) ** ((.x6 : Reg) ↦ᵣ x6o) **
      ...
      ((p + signExtend12 32) ↦ₘ size) ** ((.x20 : Reg) ↦ᵣ env) ** (flag ↦ₘ f0) **
      ((env + signExtend12 (456 : BitVec 12)) ↦ₘ persistentLen) **
      ((env + signExtend12 (448 : BitVec 12)) ↦ₘ persistentOld) **
      ((env + signExtend12 (464 : BitVec 12)) ↦ₘ transientOld) **
      ((env + signExtend12 (480 : BitVec 12)) ↦ₘ eventLen) **
      ((env + signExtend12 (472 : BitVec 12)) ↦ₘ eventOld)) (by pcFreeR) S6
  -- Segment 7: kind store + rollback + halt core (hbase+188 → resume &&& ~~~1)
  have S7 := revert_seg_kind_rollback_halt hiSCM loSCM hiLen loLen hiRd loRd hiMem loMem hiMem2 loMem2
    hi2 lo2 hi1 lo1 hbase flag resume env
    (evmMemBase + BitVec.ofNat 64 (off.toNat + 0 + (revertClamp32 size).toNat))
    x5o x6o x1o f0 persistentLen persistentOld transientOld eventLen eventOld
    (copyIntoRegion
      (copyIntoRegion
        (setBytes (setBytes (revertDescZeroed descInit) 64 (dwordBytes size)) 248
          (dwordBytes (revertClamp size))) memBytes 72 off.toNat (revertClamp size).toNat)
      memBytes 0 off.toNat (revertClamp32 size).toNat) hRc2len hla2 hla1
...
/-- **Cover lemma for the REVERT `.conditional` registry entry**
    ...
    (`revertClamp 5 = revertClamp32 5 = 5`), so the window copies the return data
    unclamped — the spec is not a vacuous or degenerate statement.
    `decide`-checked. -/
theorem revert_precondition_reachable :
    revertClamp (5 : Word) = 5 ∧ revertClamp32 (5 : Word) = 5 := by decide
