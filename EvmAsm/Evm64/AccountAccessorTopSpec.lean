/-
  ...
  Top-level success-path `cpsTripleWithin` triples for the migrated account
  field-accessor bodies (`EvmAsm/Codegen/Programs/AccountFieldExtract.lean`):
    ...
    * `account_extract_nonce_spec_within` — `accountExtractNonce_prog`
      (23 instructions, entry `GuestAddrs.account_extract_nonce`): same shape,
      with `a.nonce < 2^64` (EIP-2681); the u64 output cell holds `a.nonce`.
...
def extractNonceBase : Word := 0x8001c654#64
/-- Guest entry of `account_extract_balance`. -/
def extractBalanceBase : Word := 0x8001c5c8#64
...
private theorem aen_sub : ∀ a i, accountExtractNonceCode a = some i →
    accountExtractNonceFullCode a = some i :=
  CodeReq.union_mono_left

private theorem aen_wi_sub : ∀ a i, (rlp_walk_init_code walkInitBase) a = some i →
    accountExtractNonceFullCode a = some i :=
  CodeReq.mono_union_right aen_wi_disjoint CodeReq.union_mono_left

private theorem aen_wn_sub : ∀ a i, (rlp_walk_next_code walkNextBase) a = some i →
    accountExtractNonceFullCode a = some i :=
  CodeReq.mono_union_right aen_wn_disjoint
    (CodeReq.mono_union_right wi_wn_disjoint CodeReq.union_mono_left)

private theorem aen_cu64_sub : ∀ a i, (rlp_content_to_u64_code contentU64Base) a = some i →
    accountExtractNonceFullCode a = some i :=
...
private theorem ownify1 {n : Nat} {e x : Word} {cr : CodeReq} {P Q : Assertion} {r : Reg}
