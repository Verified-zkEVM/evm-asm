...
theorem divK_loop_body_n4_max_unified_spec
    ...
    cpsBranch (base + loopBodyOff) (sharedDivModCode base)
      ((.x12 ↦ᵣ sp) ** (.x1 ↦ᵣ j) **
       ...
       ((u_base + signExtend12 4064) ↦ₘ u_top) **
       (q_addr ↦ₘ q_old))
      (base + loopBodyOff) (loopBodyUnifiedPostN4 borrow_zero sp j q_hat v0 v1 v2 v3 u0 u1 u2 u3 u_top)
      (base + denormOff) (loopBodyUnifiedPostN4 borrow_zero sp j q_hat v0 v1 v2 v3 u0 u1 u2 u3 u_top) := by
  intro u_base q_hat q_addr ms ab hcarry hborrow
  cases borrow_zero
  · -- false (addback+BEQ path)
    rw [if_neg (by decide)] at hborrow
    exact divK_loop_body_n4_max_addback_spec
      sp j j_old v5_old v6_old v7_old v10_old v11_old v2_old
      v0 v1 v2 v3 u0 u1 u2 u3 u_top q_old base hbltu (hcarry (by decide)) hborrow
  · -- true (skip path)
    rw [if_pos rfl] at hborrow
    exact divK_loop_body_n4_max_skip_spec
      sp j j_old v5_old v6_old v7_old v10_old v11_old v2_old
...
theorem divK_loop_body_n4_call_unified_spec
    ...
    cpsBranch (base + loopBodyOff) (sharedDivModCode base)
      ((.x12 ↦ᵣ sp) ** (.x1 ↦ᵣ j) **
       ...
       (sp + signExtend12 3944 ↦ₘ scratch_un0))
      (base + loopBodyOff)
      (loopBodyUnifiedPostN4 borrow_zero sp j q_hat v0 v1 v2 v3 u0 u1 u2 u3 u_top **
       (sp + signExtend12 3968 ↦ₘ (base + 516)) **
       (sp + signExtend12 3960 ↦ₘ v3) **
       (sp + signExtend12 3952 ↦ₘ d_lo) **
       (sp + signExtend12 3944 ↦ₘ div_un0))
