...
/-- `fullModN4CallAddbackShift0PostV5` minus the `regOwn .x1` atom. -/
def fullModN4CallAddbackShift0PostV5NoX1 (sp base a0 a1 a2 a3 b0 b1 b2 b3 scratchMem : Word) : Assertion :=
  let qHat := divKTrialCallV5QHat (0 : Word) a3 b3
  let ms := mulsubN4 qHat b0 b1 b2 b3 a0 a1 a2 a3
  let c3 := ms.2.2.2.2
  let carry := addbackN4_carry ms.1 ms.2.1 ms.2.2.1 ms.2.2.2.1 b0 b1 b2 b3
...
/-- Exact-x1 twin of `evm_mod_n4_full_call_addback_shift0_spec_v5_noNop`. -/
theorem evm_mod_n4_full_call_addback_shift0_spec_v5_noNop_exact_x1 (sp base : Word)
    (a0 a1 a2 a3 b0 b1 b2 b3 v2 v5 v6 v7 v10 v11Old raVal x9In : Word)
    (q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7 nMem shiftMem jMem : Word)
    (retMem dMem dloMem scratchUn0 scratchMem : Word)
    (hbnz : b0 ||| b1 ||| b2 ||| b3 ≠ 0)
    ...
    cpsTripleWithin ((((8 + 21 + 24 + 4) + 13) + 234) + 12) base (base + nopOff) (modCode_noNop_v5 base)
      ((.x12 ↦ᵣ sp) ** (.x5 ↦ᵣ v5) ** (.x10 ↦ᵣ v10) ** (.x0 ↦ᵣ (0 : Word)) **
       ...
       (sp + signExtend12 3952 ↦ₘ dloMem) ** (sp + signExtend12 3944 ↦ₘ scratchUn0) **
       (sp + signExtend12 3936 ↦ₘ scratchMem) ** (.x1 ↦ᵣ raVal))
      (fullModN4CallAddbackShift0PostV5NoX1 sp base a0 a1 a2 a3 b0 b1 b2 b3 scratchMem **
       (.x1 ↦ᵣ raVal)) := by
  have hA := evm_mod_n4_preloop_call_addback_shift0_spec_v5_noNop_exact_x1 sp base
    a0 a1 a2 a3 b0 b1 b2 b3 v2 v5 v6 v7 v10 v11Old raVal x9In
    q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7 nMem shiftMem jMem
  ...
  exact cpsTripleWithin_mono_nSteps (by decide) <| cpsTripleWithin_weaken
    (fun h hp => by xperm_hyp hp)
    (fun h hq => by delta fullModN4CallAddbackShift0PostV5NoX1; xperm_hyp hq)
    hFull
