/-
  ...
  between div and mod code, so each piece re-proves over `modCode_noNop_v5` by
  swapping the per-instruction code-subsumption lemmas `*_sub_divCode_noNop_v5`
  → `*_sub_modCode_noNop_v5` (PhaseBV5Mod) and the phase bricks div→`_mod`.

  n=3 divisor-width detection: b3=0, b2≠0, writing nMem=3; the `bne x7` (b2)
...
theorem evm_mod_phaseAB_n3_clz_spec_within_v5_noNop (sp base : Word)
  ...
  have hAf := cpsTripleWithin_frameR
    ((.x6 ↦ᵣ v6) ** (.x7 ↦ᵣ v7) **
     ...
     ((sp + signExtend12 4000) ↦ₘ u7) ** ((sp + signExtend12 3984) ↦ₘ nMem))
    (by pcFree) hA
  have hB := evm_mod_phaseB_n3_spec_within_v5_noNop sp base b1 b2 b3
  ...
  have hCLZ := divK_clz_spec_within_v5_noNop_mod b2 b1 b2 base
...
theorem evm_mod_n3_to_normB_spec_within_v5_noNop (sp base : Word)
  ...
  have hABCLZ := evm_mod_phaseAB_n3_clz_spec_within_v5_noNop sp base b0 b1 b2 b3
    v5 v6 v7 v10 q0 q1 q2 q3 u5 u6 u7 nMem hbnz hb3z hb2nz
  have hABCLZf := cpsTripleWithin_frameR
    ((.x2 ↦ᵣ (clzResult b2).2 >>> (63 : Nat)) **
     ((sp + signExtend12 3992) ↦ₘ shiftMem))
    (by pcFree) hABCLZ
  have hC2 := divK_phaseC2_ntaken_spec_within_v5_noNop_mod sp shift ((clzResult b2).2 >>> (63 : Nat))
...
/-- n=3 preloop entry-to-loopSetup over `modCode_noNop_v5`. base → base+loopBodyOff. -/
theorem evm_mod_n3_to_loopSetup_spec_within_v5_noNop (sp base : Word)
  ...
  have hNB := evm_mod_n3_to_normB_spec_within_v5_noNop sp base b0 b1 b2 b3 v5 v6 v7 v10
  ...
  have hLS := divK_loopSetup_ntaken_spec_within_v5_noNop_mod sp (3 : Word)
...
/-- n=3 v5/no-NOP entry→loopSetup with the loop's x1+scratch frame already in
    place.  frameR + weaken around `evm_mod_n3_to_loopSetup_spec_within_v5_noNop`. -/
theorem evm_mod_n3_to_loopSetup_spec_within_v5_noNop_exact_x1_scratch_frame
    (sp base : Word)
    ...
    cpsTripleWithin (8 + 21 + 24 + 4 + 21 + 21 + 4) base (base + loopBodyOff)
      ...
      (loopSetupPost sp (3 : Word) (clzResult b2).1 a0 a1 a2 a3 b0 b1 b2 b3 **
       ((.x11 ↦ᵣ v11Old) ** ((sp + signExtend12 3976) ↦ₘ jMem) **
        ...
        (.x1 ↦ᵣ raVal))) := by
  have hPre :=
    evm_mod_n3_to_loopSetup_spec_within_v5_noNop sp base a0 a1 a2 a3 b0 b1 b2 b3
