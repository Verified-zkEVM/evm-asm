/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN4V5FullExactX1

  Exact-x1 twins of the n=4 v5 denorm/epilogue and full-path compositions
  (`base → nopOff` over `divCode_noNop_v5`), all four branches: call-skip /
  call-addback-beq × shift≠0 / shift=0.  Mirrors of
  `evm_div_n4_call_skip_denorm_epilogue_spec_v5_noNop` +
  `evm_div_n4_full_call_skip_spec_v5_noNop` (FullPathN4V5NoNop{Denorm,Full}CallSkip),
  their addback analogs, and the shift=0 full paths
  (FullPathN4V5FullShift0Call{Skip,Addback}), with the concrete caller `x1`
  (= `raVal`) framed through instead of `regOwn .x1`.
...
/-- `fullDivN4CallSkipPostV5` minus the `regOwn .x1` atom. -/
def fullDivN4CallSkipPostV5NoX1 (sp base a0 a1 a2 a3 b0 b1 b2 b3 scratchMem : Word) : Assertion :=
...
/-- `fullDivN4CallAddbackBeqPostV5` minus the `regOwn .x1` atom. -/
def fullDivN4CallAddbackBeqPostV5NoX1 (sp base a0 a1 a2 a3 b0 b1 b2 b3 scratchMem : Word) : Assertion :=
  let shift := (clzResult b3).1
  let antiShift := signExtend12 (0 : BitVec 12) - shift
  let b3' := (b3 <<< (shift.toNat % 64)) ||| (b2 >>> (antiShift.toNat % 64))
  let b2' := (b2 <<< (shift.toNat % 64)) ||| (b1 >>> (antiShift.toNat % 64))
...
/-- Exact-x1 twin of `evm_div_n4_call_skip_denorm_epilogue_spec_v5_noNop`. -/
theorem evm_div_n4_call_skip_denorm_epilogue_spec_v5_noNop_exact_x1 (sp base : Word)
  ...
  exact cpsTripleWithin_weaken
    (fun h hp => by
      ...
      xperm_hyp hp)
    (fun h hq => by
      delta fullDivN4CallSkipPostV5NoX1
      rw [sepConj_assoc'] at hq
      xperm_hyp hq)
    hBF

/-- Exact-x1 twin of `evm_div_n4_call_addback_beq_denorm_epilogue_spec_v5_noNop`. -/
theorem evm_div_n4_call_addback_beq_denorm_epilogue_spec_v5_noNop_exact_x1 (sp base : Word)
    (a0 a1 a2 a3 b0 b1 b2 b3 scratchMem raVal : Word)
    (hshift_nz : (clzResult b3).1 ≠ 0) :
    cpsTripleWithin (2 + 23 + 10) (base + denormOff) (base + nopOff) (divCode_noNop_v5 base)
  ...
  exact cpsTripleWithin_weaken
    (fun h hp => by
      ...
      xperm_hyp hp)
    (fun h hq => by
      delta fullDivN4CallAddbackBeqPostV5NoX1
      rw [sepConj_assoc'] at hq
      xperm_hyp hq)
    hBF
...
theorem evm_div_n4_full_call_skip_spec_v5_noNop_exact_x1 (sp base : Word)
  ...
  have hB := evm_div_n4_call_skip_denorm_epilogue_spec_v5_noNop_exact_x1 sp base
...
theorem evm_div_n4_full_call_addback_spec_v5_noNop_exact_x1 (sp base : Word)
  ...
  have hB := evm_div_n4_call_addback_beq_denorm_epilogue_spec_v5_noNop_exact_x1 sp base
    a0 a1 a2 a3 b0 b1 b2 b3 scratchMem raVal hshift_nz
  have hFull := cpsTripleWithin_seq_perm_same_cr
    (fun h hp => by xperm_hyp hp) hA hB
  exact cpsTripleWithin_mono_nSteps (by decide) hFull

/-- `fullDivN4CallSkipShift0PostV5` minus the `regOwn .x1` atom. -/
def fullDivN4CallSkipShift0PostV5NoX1 (sp base a0 a1 a2 a3 b0 b1 b2 b3 scratchMem : Word) : Assertion :=
...
/-- `fullDivN4CallAddbackShift0PostV5` minus the `regOwn .x1` atom. -/
def fullDivN4CallAddbackShift0PostV5NoX1 (sp base a0 a1 a2 a3 b0 b1 b2 b3 scratchMem : Word) : Assertion :=
  let qHat := divKTrialCallV5QHat (0 : Word) a3 b3
  let ms := mulsubN4 qHat b0 b1 b2 b3 a0 a1 a2 a3
  let c3 := ms.2.2.2.2
  let carry := addbackN4_carry ms.1 ms.2.1 ms.2.2.1 ms.2.2.2.1 b0 b1 b2 b3
...
theorem evm_div_n4_full_call_skip_shift0_spec_v5_noNop_exact_x1 (sp base : Word)
  ...
  exact cpsTripleWithin_mono_nSteps (by decide) <| cpsTripleWithin_weaken
    ...
    (fun h hq => by delta fullDivN4CallSkipShift0PostV5NoX1; xperm_hyp hq)
    hFull
...
theorem evm_div_n4_full_call_addback_shift0_spec_v5_noNop_exact_x1 (sp base : Word)
  ...
  have hFull := cpsTripleWithin_seq_perm_same_cr
    (fun h hp => by
      simp only [preloopCallAddbackShift0PostN4V5NoX1, loopBodyN4CallAddbackBeqJ0PostV5NoX1,
  ...
  exact cpsTripleWithin_mono_nSteps (by decide) <| cpsTripleWithin_weaken
    (fun h hp => by xperm_hyp hp)
    (fun h hq => by delta fullDivN4CallAddbackShift0PostV5NoX1; xperm_hyp hq)
    hFull
