/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN3V5PathShift0Mod

  v5 n=3 shift=0 LOOP + PATH (base → denormOff) over `modCode_noNop_v5`.  MOD mirror
  of `FullPathN3V5LoopShift0` + `FullPathN3V5PathShift0`: the shift=0 loop applies
  the already-lifted modCode unified dispatcher
  (`divK_loop_n3_unified_from_source_exact_loopIterScratch_v5_noNop_modCode_borrowCarry`);
  the path composes the shift=0 preloop (`evm_mod_n3_to_loopSetup_shift0_spec_v5_noNop`)
  ;; that loop via the op-agnostic loop-entry bridge.
-/
...
/-- v5 n=3 shift=0 loop body (loopBodyOff → denormOff) over `modCode_noNop_v5`. -/
theorem divK_loop_n3_shift0_param_v5_noNop_modCode (bltu_1 bltu_0 : Bool)
    (sp base : Word)
    ...
    cpsTripleWithin 468 (base + loopBodyOff) (base + denormOff)
      ...
      (loopN3UnifiedPostV5NoX1 bltu_1 bltu_0 sp base
        ...
        retMem dMem dloMem scratchUn0 scratchMem **
        (.x1 ↦ᵣ raVal)) := by
  apply divK_loop_n3_unified_from_source_exact_loopIterScratch_v5_noNop_modCode_borrowCarry
...
/-- Full n=3 shift=0 path, `base → denormOff`, over `modCode_noNop_v5`. -/
theorem evm_mod_n3_to_denorm_shift0_param_v5_noNop (bltu_1 bltu_0 : Bool)
    (sp base : Word)
  ...
  have hPre := evm_mod_n3_to_loopSetup_shift0_spec_v5_noNop sp base a0 a1 a2 a3 b0 b1 b2 b3
  ...
  have hPref := cpsTripleWithin_frameR
    ((.x11 ↦ᵣ v11Old) ** ((sp + signExtend12 3976) ↦ₘ jMem) **
     ...
     ((sp + signExtend12 3936) ↦ₘ scratchMem) ** (.x1 ↦ᵣ raVal))
    (by pcFree) hPre
  have hLoop := divK_loop_n3_shift0_param_v5_noNop_modCode bltu_1 bltu_0 sp base jMem (3 : Word)
