/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN4V5ToDenormShift0CallSkipMod

  n=4 v5 MOD shift=0 preloop + call+skip loop body, `base → denormOff` over
  `modCode_noNop_v5`.  MOD mirror of `evm_div_n4_preloop_call_skip_shift0_spec_v5_noNop`
  (FullPathN4V5ToDenormShift0CallSkip): composes the n=4 shift=0 MOD preloop
  (`evm_mod_n4_to_loopSetup_shift0_spec_v5_noNop`, base→loopBodyOff) with the
  GENERIC call+skip loop body over `modCode_noNop_v5`
  (`divK_loop_body_n4_call_skip_j0_norm_v5_noNop_modCode`, loopBodyOff→denormOff)
  ...
  `b3 ≥ 2^63` already.  The loop-body post `preloopCallSkipShift0PostN4V5` is
...
/-- n=4 v5 MOD shift=0 pre-loop + call+skip loop body over `modCode_noNop_v5`:
    base → denormOff.  MOD mirror of `evm_div_n4_preloop_call_skip_shift0_spec_v5_noNop`. -/
theorem evm_mod_n4_preloop_call_skip_shift0_spec_v5_noNop (sp base : Word)
    ...
    cpsTripleWithin (((8 + 21 + 24 + 4) + 13) + 158) base (base + denormOff) (modCode_noNop_v5 base)
