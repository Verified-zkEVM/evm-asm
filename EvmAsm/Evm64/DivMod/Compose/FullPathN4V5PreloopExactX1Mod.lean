/-
  EvmAsm.Evm64.DivMod.Compose.FullPathN4V5PreloopExactX1Mod

  MOD mirror of `FullPathN4V5PreloopExactX1`: the four x1-preserving n=4
  exact-X1 preloop specs (call-skip/addback × shift≠0/shift=0) over
  `modCode_noNop_v5`, composing the x9In/x2In-generic MOD n4 preloop leaf with
  the MOD exact-X1 loop-body wrappers.  The exit posts
  (`preloopCall{Skip,Addback}{,Shift0}PostN4V5NoX1`) are op-agnostic (loop exit is
  shared div/mod up to `denormOff`) — reused from the DIV file.  Toward
...
theorem evm_mod_n4_preloop_call_skip_spec_v5_noNop_exact_x1 (sp base : Word)
  ...
  have hPreF := cpsTripleWithin_frameR
    ((.x11 ↦ᵣ v11Old) ** ((sp + signExtend12 3976) ↦ₘ jMem) **
     ...
     (sp + signExtend12 3936 ↦ₘ scratchMem) ** (.x1 ↦ᵣ raVal))
    (by pcFree) hPre
  have hLoop := divK_loop_body_n4_call_skip_j0_norm_v5_noNop_exact_x1_modCode sp base
...
/-- Exact-x1 twin of `evm_mod_n4_preloop_call_addback_spec_v5_noNop`. -/
theorem evm_mod_n4_preloop_call_addback_spec_v5_noNop_exact_x1 (sp base : Word)
    (a0 a1 a2 a3 b0 b1 b2 b3 v5 v6 v7 v10 v11Old raVal x9In x2In : Word)
    (q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7 nMem shiftMem jMem : Word)
    (retMem dMem dloMem scratchUn0 scratchMem : Word)
    (hbnz : b0 ||| b1 ||| b2 ||| b3 ≠ 0)
  ...
  have hPreF := cpsTripleWithin_frameR
    ((.x11 ↦ᵣ v11Old) ** ((sp + signExtend12 3976) ↦ₘ jMem) **
     ...
     (sp + signExtend12 3936 ↦ₘ scratchMem) ** (.x1 ↦ᵣ raVal))
    (by pcFree) hPre
  have hLoop := divK_loop_body_n4_call_addback_j0_beq_norm_v5_noNop_exact_x1_modCode sp base
    jMem (4 : Word) shift u0 (a0 >>> (antiShift.toNat % 64)) v11Old antiShift
    b0' b1' b2' b3' u0 u1 u2 u3 u4 (0 : Word) raVal
...
/-- Exact-x1 twin of `evm_mod_n4_preloop_call_skip_shift0_spec_v5_noNop`. -/
theorem evm_mod_n4_preloop_call_skip_shift0_spec_v5_noNop_exact_x1 (sp base : Word)
...
/-- Exact-x1 twin of `evm_mod_n4_preloop_call_addback_shift0_spec_v5_noNop`. -/
theorem evm_mod_n4_preloop_call_addback_shift0_spec_v5_noNop_exact_x1 (sp base : Word)
    (a0 a1 a2 a3 b0 b1 b2 b3 v2 v5 v6 v7 v10 v11Old raVal x9In : Word)
    (q0 q1 q2 q3 u0Old u1Old u2Old u3Old u4Old u5 u6 u7 nMem shiftMem jMem : Word)
    (retMem dMem dloMem scratchUn0 scratchMem : Word)
    (hbnz : b0 ||| b1 ||| b2 ||| b3 ≠ 0)
