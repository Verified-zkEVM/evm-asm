/-
  ...
  and the dividend is copied verbatim.  Reuses the merged n=3 MOD phaseAB-clz
  (`evm_mod_phaseAB_n3_clz_spec_within_v5_noNop`) + the shift=0 phase-C2/copyAU MOD
...
theorem evm_mod_phaseAB_n3_clz_c2taken_spec_v5_noNop (sp base : Word)
  ...
  have hABCLZf := cpsTripleWithin_frameR
    ...
    (by pcFree) hABCLZ
  have hC2 := divK_phaseC2_taken_spec_within_v5_noNop_mod sp (clzResult b2).1
...
/-- Full n=3 shift=0 preloop, `base → loopBodyOff`, over `modCode_noNop_v5`. -/
theorem evm_mod_n3_to_loopSetup_shift0_spec_v5_noNop (sp base : Word)
