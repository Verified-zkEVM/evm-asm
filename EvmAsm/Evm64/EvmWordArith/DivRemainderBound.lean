...
theorem norm_euclidean_correct {a_val b_val qNat r_norm : Nat} (s : Nat)
    (heq : a_val * 2^s = qNat * (b_val * 2^s) + r_norm)
    (hlt : r_norm < b_val * 2^s) :
    qNat = a_val / b_val ∧ r_norm / 2^s = a_val % b_val := by
  -- Convert to the form expected by norm_euclidean_bridge
  have heq' : a_val * 2^s = b_val * 2^s * qNat + r_norm := by linarith
  exact norm_euclidean_bridge a_val b_val qNat r_norm s heq' hlt
