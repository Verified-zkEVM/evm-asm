/-
  ...
  This program is the la-FREE scan body of the `h_TLOAD` dispatcher handler
  ...
  `li x14, 0xa0830000` pseudo-instruction becomes its exact GNU-as expansion
  `lui x14, 0xa ; addiw x14, x14, 131 ; slli x14, x14, 16` (verified against
  ...
  Layout (47 instructions = 188 bytes), `envReg = x20`:
     ...
     +0   LD   x15 envReg 464     ; x15 = transient log_length n
     +4   BEQ  x15 x0 +168        ; empty log → zero arm (+172)
     +8   LUI  x14 0xa            ; \
     +12  ADDIW x14 x14 131       ;  } x14 = 0xa0830000 (transient log base)
     +16  SLLI x14 x14 16         ; /
...
/-- First 7 instructions of `evm_tload`: load the log length, exit to the zero
    arm when it is 0, materialize the transient log base
    (`lui`/`addiw`/`slli` — the exact GNU-as `li 0xa0830000` expansion), and
    compute the one-past-end scan pointer `x14 = base + 128 * n`. -/
def evm_tload_head (envReg : Reg) : Program :=
  LD .x15 envReg (BitVec.ofNat 12 transientLogLengthOff) ;;
  BEQ .x15 .x0 (BitVec.ofNat 13 168) ;;
  LUI .x14 (BitVec.ofNat 20 0xa) ;;
  ADDIW .x14 .x14 (BitVec.ofNat 12 131) ;;
...
def evm_tload_cmp (envReg : Reg) : Program :=
...
def evm_tload_loop (envReg : Reg) : Program :=
  evm_tload_cmp envReg ++ evm_tload_copy ++ evm_tload_tail

/-- Parameterized RISC-V program implementing the `TLOAD` reverse scan.
...
def evm_tload (envReg : Reg) : Program :=
  evm_tload_head envReg ++ evm_tload_loop envReg
...
abbrev evm_tload_code (envReg : Reg) (base : Word) : CodeReq :=
  CodeReq.ofProg base (evm_tload envReg)

abbrev evm_tload_loop_code (base : Word) : CodeReq :=
  CodeReq.ofProg base (evm_tload_loop .x20)

abbrev evm_tload_cmp_code (base : Word) : CodeReq :=
  CodeReq.ofProg base (evm_tload_cmp .x20)

abbrev evm_tload_copy_code (base : Word) : CodeReq :=
  CodeReq.ofProg base evm_tload_copy

abbrev evm_tload_tail_code (base : Word) : CodeReq :=
  CodeReq.ofProg base evm_tload_tail
...
theorem evm_tload_split (envReg : Reg) :
    evm_tload envReg = evm_tload_head envReg ++ evm_tload_loop envReg := rfl

theorem evm_tload_loop_split (envReg : Reg) :
    evm_tload_loop envReg = evm_tload_cmp envReg ++ evm_tload_copy ++ evm_tload_tail := rfl
