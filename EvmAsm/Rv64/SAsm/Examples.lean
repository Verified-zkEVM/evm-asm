...
private def demoLoop : Stmt :=
  ...
  .«while» "count" (.bltu .x5 .x6) 10
      (fun i rf => rf.get .x5 = BitVec.ofNat 64 i)
      (.block "step" [.ADDI .x5 .x5 1]) ;;;
  .assert "counted" (fun rf => rf.get .x5 = 10) ;;;
...
#guard (Stmt.assert "mid" (fun rf => rf.get .x5 = 0)).size = 0
#guard (Stmt.assert "mid" (fun rf => rf.get .x5 = 0)).flatten 0x1000 = []
