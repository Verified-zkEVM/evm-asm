/-
  EvmAsm.Rv64.RLP.ContentToU256BeStrict

  A verified RISC-V leaf subroutine that is a **drop-in replacement** for the
  (currently unverified) codegen guest function `rlp_content_to_u256_be_strict`
  ...
  `rlp_content_to_u256_be_strict` is a frameless leaf function: it is reached by
  `jal ra, rlp_content_to_u256_be_strict` and returns with `ret` (`JALR x0, ra, 0`).
  ...
  * On **failure** (`status 2` or `3`) the routine returns nonzero and the
  ...
  `rlp_content_to_u256_be_strict_prog`. **All four behavioral cases** are proved as
...
`rlp_content_to_u256_be_strict` (26 instructions). Register map: `a0=x10`, `a1=x11`,
...
```
-/
def rlp_content_to_u256_be_strict_prog : List Instr :=
  [ .SD .x12 .x0 0,           -- 0
    .SD .x12 .x0 8,           -- 1
    .SD .x12 .x0 16,          -- 2
    .SD .x12 .x0 24,          -- 3
...
theorem rlp_content_to_u256_be_strict_prog_length :
    rlp_content_to_u256_be_strict_prog.length = 26 := rfl

/-- The drop-in body as a `CodeReq` rooted at `base`. -/
abbrev rlp_content_to_u256_be_strict_code (base : Word) : CodeReq :=
  CodeReq.ofProg base rlp_content_to_u256_be_strict_prog
...
-/
theorem rlp_content_to_u256_be_strict_too_long_spec_within
    (base contentPtr len outPtr t0Old raVal : Word)
    (h_too_long : BitVec.ult (32 : Word) len) :
    cpsTripleWithin 8 base (raVal &&& ~~~1) (rlp_content_to_u256_be_strict_code base)
  ...
  have hA : cpsTripleWithin 5 base (base + 20) (rlp_content_to_u256_be_strict_code base)
  ...
  have hBr_framed := cpsBranchWithin_frameR
    ...
    (by pcFree) hBr_raw
  have hBr_ext := cpsBranchWithin_extend_code (cr' := rlp_content_to_u256_be_strict_code base)
    (fun a i h => by
      ...
      · next heq =>
        ...
        subst h
        exact CodeReq.ofProg_lookup_addr base rlp_content_to_u256_be_strict_prog 5 (base + 20)
          (by rw [rlp_content_to_u256_be_strict_prog_length]; norm_num)
          (by rw [rlp_content_to_u256_be_strict_prog_length]; norm_num)
          (by rfl)
  ...
  have hC : cpsTripleWithin 2 (base + 96) (raVal &&& ~~~1) (rlp_content_to_u256_be_strict_code base)
...
-/
theorem rlp_content_to_u256_be_strict_noncanonical_spec_within
    ...
    cpsTripleWithin 11 base (raVal &&& ~~~1) (rlp_content_to_u256_be_strict_code base)
  ...
  have hmono5 : ∀ a i, CodeReq.singleton (base + 20) (.BLTU .x5 .x11 (76 : BitVec 13)) a = some i
      → rlp_content_to_u256_be_strict_code base a = some i :=
    CodeReq.singleton_mono (CodeReq.ofProg_lookup_addr base rlp_content_to_u256_be_strict_prog 5 (base + 20)
      (by rw [rlp_content_to_u256_be_strict_prog_length]; norm_num)
      (by rw [rlp_content_to_u256_be_strict_prog_length]; norm_num) (by rfl))
  ...
  have hmono6 : ∀ a i, CodeReq.singleton (base + 24) (.BEQ .x11 .x0 (56 : BitVec 13)) a = some i
      → rlp_content_to_u256_be_strict_code base a = some i :=
    CodeReq.singleton_mono (CodeReq.ofProg_lookup_addr base rlp_content_to_u256_be_strict_prog 6 (base + 24)
      (by rw [rlp_content_to_u256_be_strict_prog_length]; norm_num)
      (by rw [rlp_content_to_u256_be_strict_prog_length]; norm_num) (by rfl))
  ...
  have hmono7 : ∀ a i, CodeReq.singleton (base + 28) (.LBU .x6 .x10 0) a = some i
      → rlp_content_to_u256_be_strict_code base a = some i :=
    CodeReq.singleton_mono (CodeReq.ofProg_lookup_addr base rlp_content_to_u256_be_strict_prog 7 (base + 28)
      (by rw [rlp_content_to_u256_be_strict_prog_length]; norm_num)
      (by rw [rlp_content_to_u256_be_strict_prog_length]; norm_num) (by rfl))
  ...
  have hmono8 : ∀ a i, CodeReq.singleton (base + 32) (.BEQ .x6 .x0 (56 : BitVec 13)) a = some i
      → rlp_content_to_u256_be_strict_code base a = some i :=
    CodeReq.singleton_mono (CodeReq.ofProg_lookup_addr base rlp_content_to_u256_be_strict_prog 8 (base + 32)
      (by rw [rlp_content_to_u256_be_strict_prog_length]; norm_num)
      (by rw [rlp_content_to_u256_be_strict_prog_length]; norm_num) (by rfl))
  ...
  simp only [signExtend12_0] at hRet
  have hE : cpsTripleWithin 2 (base + 88) (raVal &&& ~~~1) (rlp_content_to_u256_be_strict_code base)
      ((.x10 ↦ᵣ (srcBase + BitVec.ofNat 64 srcOff)) ** (.x1 ↦ᵣ raVal))
...
theorem cu256_strict_copy_body_spec_within
    ...
    cpsTripleWithin 5 (base + 56) (base + 76) (rlp_content_to_u256_be_strict_code base)
...
def copyNStrict (dst src : List (BitVec 8)) (di si : Nat) : Nat → List (BitVec 8)
  | 0 => dst
  | k + 1 => copyNStrict (dst.set di (getByteAt src si)) src (di + 1) (si + 1) k

theorem copyNStrict_zero (dst src : List (BitVec 8)) (di si : Nat) :
    copyNStrict dst src di si 0 = dst := rfl

theorem copyNStrict_succ (dst src : List (BitVec 8)) (di si k : Nat) :
    copyNStrict dst src di si (k + 1)
      = copyNStrict (dst.set di (getByteAt src si)) src (di + 1) (si + 1) k := rfl

theorem copyNStrict_length (dst src : List (BitVec 8)) (di si n : Nat) :
    (copyNStrict dst src di si n).length = dst.length := by
  induction n generalizing dst di si with
  | zero => rfl
  | succ k ih => rw [copyNStrict_succ, ih, List.length_set]

/-- `copyNStrict` splits: copying `n` in-range bytes overwrites exactly the window
    `[di, di + n)` of `dst` with `src[si, si + n)`.

    Lives here beside `copyNStrict` rather than in a caller: both
...
theorem copyNStrict_eq_append (dst src : List (BitVec 8)) (di si n : Nat)
    (hd : di + n ≤ dst.length) (hs : si + n ≤ src.length) :
    copyNStrict dst src di si n
...
theorem bytesRegion_replicate32_zero_strict (base : Word) :
...
theorem cu256_strict_loop_spec_within
    ...
    cpsTripleWithin (7 * n + 1) (base + 52) (base + 80) (rlp_content_to_u256_be_strict_code base)
      ...
      ((.x28 ↦ᵣ (0 : Word)) ** (.x7 ↦ᵣ (srcBase + BitVec.ofNat 64 (si + n))) **
       ...
       bytesRegion srcBase srcBytes ** bytesRegion dstBase (copyNStrict dstBytes srcBytes di si n)) := by
  have hmono : ∀ a i, CodeReq.singleton (base + 52) (.BEQ .x28 .x0 (28 : BitVec 13)) a = some i
      → rlp_content_to_u256_be_strict_code base a = some i :=
    CodeReq.singleton_mono (CodeReq.ofProg_lookup_addr base rlp_content_to_u256_be_strict_prog 13 (base + 52)
      (by rw [rlp_content_to_u256_be_strict_prog_length]; norm_num)
      (by rw [rlp_content_to_u256_be_strict_prog_length]; norm_num) (by rfl))
  ...
  | zero =>
    ...
    · rw [show (0#64 : Word) = 0 from by decide] at hq
      simp only [Nat.add_zero, copyNStrict_zero]
  ...
  | succ k ih =>
    ...
    have hA1' : cpsTripleWithin 1 (base + 52) (base + 56) (rlp_content_to_u256_be_strict_code base)
    ...
    have body := cu256_strict_copy_body_spec_within base srcBase dstBase x29Old (BitVec.ofNat 64 (k + 1))
    ...
    have hjal_mono : ∀ a i, CodeReq.singleton (base + 76) (.JAL .x0 (-24 : BitVec 21)) a = some i
        → rlp_content_to_u256_be_strict_code base a = some i :=
      CodeReq.singleton_mono (CodeReq.ofProg_lookup_addr base rlp_content_to_u256_be_strict_prog 19 (base + 76)
        (by rw [rlp_content_to_u256_be_strict_prog_length]; norm_num)
        (by rw [rlp_content_to_u256_be_strict_prog_length]; norm_num) (by rfl))
    have hjal_ext := cpsTripleWithin_extend_code hjal_mono hjal
    have hjal_S : cpsTripleWithin 1 (base + 76) (base + 52) (rlp_content_to_u256_be_strict_code base)
    ...
    rw [show 7 * (k + 1) + 1 = 1 + 5 + 1 + (7 * k + 1) from by ring,
        ...
        show di + (k + 1) = (di + 1) + k from by omega,
        show copyNStrict dstBytes srcBytes di si (k + 1)
           = copyNStrict (dstBytes.set di (srcBytes[si]'hsi0)) srcBytes (di + 1) (si + 1) k from by
          rw [copyNStrict_succ, ← hbyte]]
...
/--
**`rlp_content_to_u256_be_strict` — success path (`0 < len ≤ 32`, canonical).**
...
`copyNStrict (replicate 32 0) srcBytes (32-len) srcOff len` — `(32-len)` zero bytes
...
-/
theorem rlp_content_to_u256_be_strict_success_spec_within
    ...
    cpsTripleWithin (7 * len + 16) base (raVal &&& ~~~1) (rlp_content_to_u256_be_strict_code base)
      ((.x10 ↦ᵣ (srcBase + BitVec.ofNat 64 srcOff)) ** (.x11 ↦ᵣ BitVec.ofNat 64 len) **
      ...
      ((.x10 ↦ᵣ (0 : Word)) ** (.x11 ↦ᵣ BitVec.ofNat 64 len) ** (.x12 ↦ᵣ outPtr) **
       ...
       bytesRegion outPtr (copyNStrict (List.replicate 32 (0 : BitVec 8)) srcBytes (32 - len) srcOff len)) := by
  have hnlt : ¬ (BitVec.ult (32 : Word) (BitVec.ofNat 64 len) = true) := by
    simp only [BitVec.ult, decide_eq_true_eq, BitVec.toNat_ofNat,
      show BitVec.toNat (32 : Word) = 32 from by decide,
      Nat.mod_eq_of_lt (show len < 2 ^ 64 by omega)]
  ...
  simp only [signExtend12_0, signExtend12_8, signExtend12_16, signExtend12_24] at hSD0 hSD1 hSD2 hSD3
  have hPA0 : cpsTripleWithin 5 base (base + 20) (rlp_content_to_u256_be_strict_code base)
    ...
    rw [bytesRegion_replicate32_zero_strict]
    runBlock hSD0 hSD1 hSD2 hSD3 hLI
  ...
  have hPD : cpsTripleWithin 4 (base + 36) (base + 52) (rlp_content_to_u256_be_strict_code base)
  ...
  have hloop := cu256_strict_loop_spec_within base srcBase outPtr x29Old srcBytes
  ...
  simp only [signExtend12_0] at hRet
  have hPE : cpsTripleWithin 2 (base + 80) (raVal &&& ~~~1) (rlp_content_to_u256_be_strict_code base)
      ((.x10 ↦ᵣ (srcBase + BitVec.ofNat 64 srcOff)) ** (.x1 ↦ᵣ raVal))
  ...
  exact cpsTripleWithin_weaken
    (fun h hp => by simp only [memOwnU256Strict] at hp; xperm_hyp hp)
    (fun h hp => by
      have hp' := sepConj_mono_right
        (sepConj_mono (fun _ x => x) (sepConj_mono (fun _ x => x)
          (sepConj_mono (regIs_implies_regOwn .x5) (sepConj_mono (regIs_implies_regOwn .x6)
...
/--
**`rlp_content_to_u256_be_strict` — empty-content (canonical zero) path (`len = 0`).**
...
-/
theorem rlp_content_to_u256_be_strict_empty_spec_within
    (base srcBase outPtr raVal x5Old x6Old x7Old x28Old x29Old : Word) (srcOff : Nat) :
    cpsTripleWithin 9 base (raVal &&& ~~~1) (rlp_content_to_u256_be_strict_code base)
  ...
  have hPA0 : cpsTripleWithin 5 base (base + 20) (rlp_content_to_u256_be_strict_code base)
      ...
      ((.x12 ↦ᵣ outPtr) ** (.x0 ↦ᵣ (0 : Word)) ** (.x5 ↦ᵣ (32 : Word)) **
       bytesRegion outPtr (List.replicate 32 (0 : BitVec 8))) := by
    rw [bytesRegion_replicate32_zero_strict]; runBlock hSD0 hSD1 hSD2 hSD3 hLI
  have hPA := cpsTripleWithin_frameR
    ((.x10 ↦ᵣ (srcBase + BitVec.ofNat 64 srcOff)) ** (.x11 ↦ᵣ (0 : Word)) **
...
/--
**Unified spec for `rlp_content_to_u256_be_strict`.**
  ...
  · -- too-long (status 2)
    ...
    have ht := cpsTripleWithin_frameR
      ((.x6 ↦ᵣ x6Old) ** (.x7 ↦ᵣ x7Old) ** (.x28 ↦ᵣ x28Old) ** (.x29 ↦ᵣ x29Old) **
       bytesRegion srcBase srcBytes)
      (by pcFree)
      (rlp_content_to_u256_be_strict_too_long_spec_within base (srcBase + BitVec.ofNat 64 srcOff)
  ...
  · by_cases h0 : len = 0
    · -- empty / canonical zero (status 0)
      ...
      have he := cpsTripleWithin_frameR (bytesRegion srcBase srcBytes) (by pcFree)
        (rlp_content_to_u256_be_strict_empty_spec_within base srcBase outPtr raVal x5Old x6Old x7Old
    ...
    · by_cases hc : getByteAt srcBytes srcOff = 0
      · -- non-canonical (status 3)
        ...
        have hnc := cpsTripleWithin_frameR
          ...
          (by pcFree)
          (rlp_content_to_u256_be_strict_noncanonical_spec_within base srcBase outPtr raVal x5Old x6Old
      ...
      · -- success (status 0)
        ...
        have hs := rlp_content_to_u256_be_strict_success_spec_within base srcBase outPtr raVal x5Old x6Old
...
example : rlp_content_to_u256_be_strict_prog.length = 26 := rfl
example : (CodeReq.ofProg (0 : Word) rlp_content_to_u256_be_strict_prog) 20 =
    some (.BLTU .x5 .x11 (76 : BitVec 13)) := by decide
example : (CodeReq.ofProg (0 : Word) rlp_content_to_u256_be_strict_prog) 32 =
    some (.BEQ .x6 .x0 (56 : BitVec 13)) := by decide
example : (CodeReq.ofProg (0 : Word) rlp_content_to_u256_be_strict_prog) 88 =
    some (.LI .x10 (3 : Word)) := by decide
example : (CodeReq.ofProg (0 : Word) rlp_content_to_u256_be_strict_prog) 96 =
