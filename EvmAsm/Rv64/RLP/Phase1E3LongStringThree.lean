/-
  EvmAsm.Rv64.RLP.Phase1E3LongStringThree

  EL.3 full Phase 1 → Phase 3 → Phase 2 path for the long-string prefix
  `0xBA` (length-of-length = 3).

  Composes the full Phase 1 e3 path specialized to prefix `0xBA`, the Phase 3
  long-string entry (`lenLen = 3`, `len_acc = 0`, pointer advanced past the
...
/-- Concrete `0xBA` long-string flat-decode path (`lenLen = 3`).
...
theorem rlp_phase1_e3_0xBA_three_byte_length_spec_within
  ...
  intro ptr b1 b2 b3
  have hv5_lo :
      ¬ BitVec.ult (0xBA : Word) ((0 : Word) + signExtend12 (0x80 : BitVec 12)) := by
    decide
  have hv5_mid :
      ¬ BitVec.ult (0xBA : Word) ((0 : Word) + signExtend12 (0xB8 : BitVec 12)) := by
    decide
  have hv5_hi :
      BitVec.ult (0xBA : Word) ((0 : Word) + signExtend12 (0xC0 : BitVec 12)) := by
    decide
  have prefixSpec := rlp_phase1_e3_full_path_spec'_within
    (0xBA : Word) v10 v11Old v13 v14Old off1 off2 off3 base e3_target
    htarget hv5_lo hv5_mid hv5_hi hd_phase3
  have h_lenLen :
      (0xBA : Word) + signExtend12 (-(0xB7 : BitVec 12)) = (3 : Word) := by
    decide
  ...
  have loop := rlp_phase2_long_loop_three_byte_spec_within
    ...
    (e3_target + 12) back halign1 halign2 halign3 hvalid1 hvalid2 hvalid3 hback
  have loop' : cpsTripleWithin 18 (e3_target + 12) ((e3_target + 12) + 24)
      (CodeReq.ofProg (e3_target + 12) (rlp_phase2_long_loop_body_prog back))
      ((.x0 ↦ᵣ (0 : Word)) ** (.x5 ↦ᵣ (0xBA : Word)) **
        (.x10 ↦ᵣ ((0 : Word) + signExtend12 (0xC0 : BitVec 12))) **
        (.x11 ↦ᵣ (0 : Word)) ** (.x12 ↦ᵣ v12Old) **
