/-
  EvmAsm.Rv64.RLP.Phase1E3LongStringFour

  EL.3 full Phase 1 → Phase 3 → Phase 2 path for the long-string prefix
  `0xBB` (length-of-length = 4).

  Composes the full Phase 1 e3 path specialized to prefix `0xBB`, the Phase 3
  long-string entry (`lenLen = 4`, `len_acc = 0`, pointer advanced past the
...
/-- Concrete `0xBB` long-string flat-decode path (`lenLen = 4`).
...
theorem rlp_phase1_e3_0xBB_four_byte_length_spec_within
  ...
  have hv5_lo :
      ¬ BitVec.ult (0xBB : Word) ((0 : Word) + signExtend12 (0x80 : BitVec 12)) := by
    decide
  have hv5_mid :
      ¬ BitVec.ult (0xBB : Word) ((0 : Word) + signExtend12 (0xB8 : BitVec 12)) := by
    decide
  have hv5_hi :
      BitVec.ult (0xBB : Word) ((0 : Word) + signExtend12 (0xC0 : BitVec 12)) := by
    decide
  have prefixSpec := rlp_phase1_e3_full_path_spec'_within
    (0xBB : Word) v10 v11Old v13 v14Old off1 off2 off3 base e3_target
    htarget hv5_lo hv5_mid hv5_hi hd_phase3
  have h_lenLen :
      (0xBB : Word) + signExtend12 (-(0xB7 : BitVec 12)) = (4 : Word) := by
    decide
  ...
  have loop := rlp_phase2_long_loop_four_byte_spec_within
    ...
    (e3_target + 12) back halign1 halign2 halign3 halign4
    hvalid1 hvalid2 hvalid3 hvalid4 hback
  have loop' : cpsTripleWithin 24 (e3_target + 12) ((e3_target + 12) + 24)
      (CodeReq.ofProg (e3_target + 12) (rlp_phase2_long_loop_body_prog back))
      ((.x0 ↦ᵣ (0 : Word)) ** (.x5 ↦ᵣ (0xBB : Word)) **
        (.x10 ↦ᵣ ((0 : Word) + signExtend12 (0xC0 : BitVec 12))) **
        (.x11 ↦ᵣ (0 : Word)) ** (.x12 ↦ᵣ v12Old) **
