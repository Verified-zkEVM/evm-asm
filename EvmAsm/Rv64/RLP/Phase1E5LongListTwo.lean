/-
  EvmAsm.Rv64.RLP.Phase1E5LongListTwo

  EL.3 full Phase 1 → Phase 3 → Phase 2 path for the long-list prefix
  `0xF9` (length-of-length = 2).

  Composes the full Phase 1 e5 path specialized to prefix `0xF9`, the Phase 3
  long-list entry (`lenLen = 2`, `len_acc = 0`, pointer advanced past the
  prefix), and the two-byte Phase 2 long-form length loop.

  The postcondition reuses `rlp_phase2_long_loop_two_byte_post` (instantiated
  ...
  (`Phase1E3LongStringOne.lean`) for `lenLen = 2`.
-/
...
/-- Concrete `0xF9` long-list flat-decode path (`lenLen = 2`).

    Leaves `x11` holding the big-endian length over the two length bytes at
    `v13 + 1 .. v13 + 2` (per `rlp_phase2_long_loop_two_byte_post`),
    `x13 = (v13 + 1) + 2` pointing at the payload, `x14 = 0`, `x12` the last
    length byte, and `x5`/`x10`/`x0`/the source doubleword preserved. -/
theorem rlp_phase1_e5_0xF9_two_byte_length_spec_within
    ...
    cpsTripleWithin 23 base ((base + 44) + 24)
      ...
      ((.x5 ↦ᵣ (0xF9 : Word)) **
        ...
        rlp_phase2_long_loop_two_byte_post (0 : Word) ptr b1 b2
          wordVal dwordAddr) := by
  intro ptr b1 b2
  have hv5_lo :
      ¬ BitVec.ult (0xF9 : Word) ((0 : Word) + signExtend12 (0x80 : BitVec 12)) := by
    decide
  have hv5_2 :
      ¬ BitVec.ult (0xF9 : Word) ((0 : Word) + signExtend12 (0xB8 : BitVec 12)) := by
    decide
  have hv5_3 :
      ¬ BitVec.ult (0xF9 : Word) ((0 : Word) + signExtend12 (0xC0 : BitVec 12)) := by
    decide
  have hv5_hi :
      ¬ BitVec.ult (0xF9 : Word) ((0 : Word) + signExtend12 (0xF8 : BitVec 12)) := by
    decide
  have prefixSpec := rlp_phase1_e5_full_path_spec'_within
    (0xF9 : Word) v10 v11Old v13 v14Old off1 off2 off3 off4 base
    hv5_lo hv5_2 hv5_3 hv5_hi hd_phase3
  have h_lenLen :
      (0xF9 : Word) + signExtend12 (-(0xF7 : BitVec 12)) = (2 : Word) := by
    decide
  ...
  have prefix' : cpsTripleWithin 11 base (base + 44)
      ...
      ((.x0 ↦ᵣ (0 : Word)) ** (.x5 ↦ᵣ (0xF9 : Word)) ** (.x10 ↦ᵣ v10) **
        (.x11 ↦ᵣ v11Old) ** (.x12 ↦ᵣ v12Old) ** (.x13 ↦ᵣ v13) **
        (.x14 ↦ᵣ v14Old) ** (dwordAddr ↦ₘ wordVal))
      ((.x0 ↦ᵣ (0 : Word)) ** (.x5 ↦ᵣ (0xF9 : Word)) **
        (.x10 ↦ᵣ ((0 : Word) + signExtend12 (0xF8 : BitVec 12))) **
        (.x11 ↦ᵣ (0 : Word)) ** (.x12 ↦ᵣ v12Old) **
        (.x13 ↦ᵣ (v13 + signExtend12 (1 : BitVec 12))) **
        (.x14 ↦ᵣ (2 : Word)) ** (dwordAddr ↦ₘ wordVal)) :=
    cpsTripleWithin_weaken
      (fun _ hp => by xperm_hyp hp)
      (fun _ hp => by xperm_hyp hp)
      (cpsTripleWithin_frameR
        ((.x12 ↦ᵣ v12Old) ** (dwordAddr ↦ₘ wordVal)) (by pcFree) prefixSpec)
  have loop := rlp_phase2_long_loop_two_byte_spec_within
    (0 : Word) ptr v12Old wordVal dwordAddr
    (base + 44) back halign1 halign2 hvalid1 hvalid2 hback
  have loop' : cpsTripleWithin 12 (base + 44) ((base + 44) + 24)
      (CodeReq.ofProg (base + 44) (rlp_phase2_long_loop_body_prog back))
      ((.x0 ↦ᵣ (0 : Word)) ** (.x5 ↦ᵣ (0xF9 : Word)) **
        (.x10 ↦ᵣ ((0 : Word) + signExtend12 (0xF8 : BitVec 12))) **
        (.x11 ↦ᵣ (0 : Word)) ** (.x12 ↦ᵣ v12Old) **
