/-
  EvmAsm.Rv64.RLP.Phase1E3LongStringSeven

  EL.3 full Phase 1 → Phase 3 → Phase 2 path for the long-string prefix
  `0xBE` (length-of-length = 7).

  Composes the full Phase 1 e3 path specialized to prefix `0xBE`, the Phase 3
  long-string entry (`lenLen = 7`, `len_acc = 0`, pointer advanced past the
...
/-- Concrete `0xBE` long-string flat-decode path (`lenLen = 7`).
...
theorem rlp_phase1_e3_0xBE_seven_byte_length_spec_within
  ...
  have hv5_lo :
      ¬ BitVec.ult (0xBE : Word) ((0 : Word) + signExtend12 (0x80 : BitVec 12)) := by
    decide
  have hv5_mid :
      ¬ BitVec.ult (0xBE : Word) ((0 : Word) + signExtend12 (0xB8 : BitVec 12)) := by
    decide
  have hv5_hi :
      BitVec.ult (0xBE : Word) ((0 : Word) + signExtend12 (0xC0 : BitVec 12)) := by
    decide
  have prefixSpec := rlp_phase1_e3_full_path_spec'_within
    (0xBE : Word) v10 v11Old v13 v14Old off1 off2 off3 base e3_target
    htarget hv5_lo hv5_mid hv5_hi hd_phase3
  have h_lenLen :
      (0xBE : Word) + signExtend12 (-(0xB7 : BitVec 12)) = (7 : Word) := by
    decide
  ...
  have loop := rlp_phase2_long_loop_seven_byte_spec_within
    ...
    (e3_target + 12) back halign1 halign2 halign3 halign4 halign5 halign6 halign7
    hvalid1 hvalid2 hvalid3 hvalid4 hvalid5 hvalid6 hvalid7 hback
  have loop' : cpsTripleWithin 42 (e3_target + 12) ((e3_target + 12) + 24)
      (CodeReq.ofProg (e3_target + 12) (rlp_phase2_long_loop_body_prog back))
      ((.x0 ↦ᵣ (0 : Word)) ** (.x5 ↦ᵣ (0xBE : Word)) **
        (.x10 ↦ᵣ ((0 : Word) + signExtend12 (0xC0 : BitVec 12))) **
        (.x11 ↦ᵣ (0 : Word)) ** (.x12 ↦ᵣ v12Old) **
