/-
  EvmAsm.Rv64.RLP.ByteCopyIter

  One iteration of the byte-array copy loop: read src byte `off+k` (`LBU` from the
  source `bytesRegion`), store it to dst byte `k` (`SB` into the destination
  `bytesRegion`), and advance the src pointer, dst pointer, and counter. The
  straight-line core (5 instructions) of the counted copy loop that decodes 20-byte
  address / 32-byte hash RLP fields into the output struct.
  ...
  doesn't touch — no explicit disjointness hypothesis is needed.
      ...
      base       LBU  x12, x13, 0      ; x12 = src[off+k]
      base+4     SB   x12, x14, 0      ; dst[k] := x12
...
/-- **One copy iteration.** Reads `srcBytes[off+k]` and writes it to `dst` byte `k`,
    advancing the src pointer to `off+k+1`, the dst pointer to `k+1`, and decrementing
    the counter. The source region is unchanged; the destination region's byte `k`
    becomes `srcBytes[off+k]`. -/
theorem rlp_copy_iter_spec_within
    (srcBase dstBase : Word) (srcBytes dstBytes : List (BitVec 8))
    (off k : Nat) (cnt v12Old : Word) (base : Word)
    (hsalign : srcBase.toNat % 8 = 0) (hdalign : dstBase.toNat % 8 = 0)
    (hsrc : off + k < srcBytes.length) (hdst : k < dstBytes.length)
    (hsover : srcBase.toNat + (off + k) < 2 ^ 64) (hdover : dstBase.toNat + k < 2 ^ 64)
    (hsvalid : isValidByteAccess (srcBase + BitVec.ofNat 64 (off + k)) = true)
    (hdvalid : isValidByteAccess (dstBase + BitVec.ofNat 64 k) = true) :
    cpsTripleWithin 5 base (base + 20)
      ...
      ((.x12 ↦ᵣ v12Old) ** (.x13 ↦ᵣ (srcBase + BitVec.ofNat 64 (off + k))) **
       (.x14 ↦ᵣ (dstBase + BitVec.ofNat 64 k)) ** (.x15 ↦ᵣ cnt) **
       bytesRegion srcBase srcBytes ** bytesRegion dstBase dstBytes)
      ((.x12 ↦ᵣ (srcBytes[off + k]'hsrc).zeroExtend 64) **
       (.x13 ↦ᵣ (srcBase + BitVec.ofNat 64 (off + k + 1))) **
       (.x14 ↦ᵣ (dstBase + BitVec.ofNat 64 (k + 1))) **
       (.x15 ↦ᵣ (cnt + signExtend12 (-1 : BitVec 12))) **
       bytesRegion srcBase srcBytes **
       bytesRegion dstBase (dstBytes.set k (srcBytes[off + k]'hsrc))) := by
  -- (1) LBU x12, x13, 0 : x12 := src[off+k]; frame the dst region + x14, x15.
  have lbu := bytesRegion_lbu_within .x12 .x13 srcBase v12Old base srcBytes (off + k)
  ...
  have s_lbu : cpsTripleWithin 1 base (base + 4)
      ...
      ((.x12 ↦ᵣ v12Old) ** (.x13 ↦ᵣ (srcBase + BitVec.ofNat 64 (off + k))) **
       ...
       bytesRegion srcBase srcBytes ** bytesRegion dstBase dstBytes)
      ((.x12 ↦ᵣ (srcBytes[off + k]'hsrc).zeroExtend 64) **
       (.x13 ↦ᵣ (srcBase + BitVec.ofNat 64 (off + k))) **
       (.x14 ↦ᵣ (dstBase + BitVec.ofNat 64 k)) ** (.x15 ↦ᵣ cnt) **
       bytesRegion srcBase srcBytes ** bytesRegion dstBase dstBytes) :=
    cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp) (fun _ hp => by xperm_hyp hp)
      (cpsTripleWithin_frameR
        ((.x14 ↦ᵣ (dstBase + BitVec.ofNat 64 k)) ** (.x15 ↦ᵣ cnt) ** bytesRegion dstBase dstBytes)
        (by pcfree_region) lbu)
  -- (2) SB x12, x14, 0 : dst[k] := x12; frame the src region + x13, x15.
  have hbtrunc : ((srcBytes[off + k]'hsrc).zeroExtend 64).truncate 8 = srcBytes[off + k]'hsrc := by
    simp
  have sb := bytesRegion_sb_within .x14 .x12 dstBase ((srcBytes[off + k]'hsrc).zeroExtend 64)
    (base + 4) dstBytes k hdalign hdst hdover hdvalid
  rw [hbtrunc] at sb
  have s_sb : cpsTripleWithin 1 (base + 4) (base + 4 + 4)
      ...
      ((.x12 ↦ᵣ (srcBytes[off + k]'hsrc).zeroExtend 64) **
       (.x13 ↦ᵣ (srcBase + BitVec.ofNat 64 (off + k))) **
       (.x14 ↦ᵣ (dstBase + BitVec.ofNat 64 k)) ** (.x15 ↦ᵣ cnt) **
       bytesRegion srcBase srcBytes ** bytesRegion dstBase (dstBytes.set k (srcBytes[off + k]'hsrc))) :=
    cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp) (fun _ hp => by xperm_hyp hp)
      (cpsTripleWithin_frameR
        ((.x13 ↦ᵣ (srcBase + BitVec.ofNat 64 (off + k))) ** (.x15 ↦ᵣ cnt) **
  ...
  have addi13 := addi_spec_gen_same_within .x13 (srcBase + BitVec.ofNat 64 (off + k)) 1 (base + 8)
    (by nofun)
  rw [show (srcBase + BitVec.ofNat 64 (off + k)) + signExtend12 (1 : BitVec 12)
      = srcBase + BitVec.ofNat 64 (off + k + 1) from by
        rw [show signExtend12 (1 : BitVec 12) = (1 : Word) from by decide]; bv_omega] at addi13
  -- (4) ADDI x14, x14, 1 : dst ptr += 1.
  have addi14 := addi_spec_gen_same_within .x14 (dstBase + BitVec.ofNat 64 k) 1 (base + 12)
    (by nofun)
  rw [show (dstBase + BitVec.ofNat 64 k) + signExtend12 (1 : BitVec 12)
      = dstBase + BitVec.ofNat 64 (k + 1) from by
  ...
  have s4 : cpsTripleWithin 1 (base + 12) (base + 12 + 4)
      ...
      ((.x12 ↦ᵣ (srcBytes[off + k]'hsrc).zeroExtend 64) **
       (.x13 ↦ᵣ (srcBase + BitVec.ofNat 64 (off + k + 1))) **
       (.x14 ↦ᵣ (dstBase + BitVec.ofNat 64 (k + 1))) ** (.x15 ↦ᵣ cnt) **
       bytesRegion srcBase srcBytes ** bytesRegion dstBase (dstBytes.set k (srcBytes[off + k]'hsrc))) :=
    cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp) (fun _ hp => by xperm_hyp hp)
      (cpsTripleWithin_frameR
        ((.x12 ↦ᵣ (srcBytes[off + k]'hsrc).zeroExtend 64) **
         (.x13 ↦ᵣ (srcBase + BitVec.ofNat 64 (off + k + 1))) ** (.x15 ↦ᵣ cnt) **
         bytesRegion srcBase srcBytes ** bytesRegion dstBase (dstBytes.set k (srcBytes[off + k]'hsrc)))
