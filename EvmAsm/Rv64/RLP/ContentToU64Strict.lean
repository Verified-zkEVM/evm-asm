/-
  EvmAsm.Rv64.RLP.ContentToU64Strict

  A verified RISC-V leaf subroutine that is a **canonical-strict drop-in
  replacement** for the codegen guest function `rlp_content_to_u64_strict` emitted by
  ...
  Frameless leaf: reached by `jal ra, rlp_content_to_u64_strict`, returns via `ret`.
  ...
  * `a1` (`x11`) — **status**: `0` ok / `2` too long (`len > 8`) / `3`
  ...
  Lays out the 22-instruction canonical-strict body `rlp_content_to_u64_strict_prog`.
...
`rlp_content_to_u64_strict` (22 instructions). Register map: `a0=x10`, `a1=x11`,
...
```
-/
def rlp_content_to_u64_strict_prog : List Instr :=
  [ .LI .x5 (8 : Word),              -- 0
...
theorem rlp_content_to_u64_strict_prog_length :
    rlp_content_to_u64_strict_prog.length = 22 := rfl

/-- The drop-in body as a `CodeReq` rooted at `base`. -/
abbrev rlp_content_to_u64_strict_code (base : Word) : CodeReq :=
  CodeReq.ofProg base rlp_content_to_u64_strict_prog

/--
**`rlp_content_to_u64_strict` — content-too-long failure path.**
...
`ra &&& ~~~1`.
-/
theorem rlp_content_to_u64_strict_too_long_spec_within
    (base contentPtr len t0Old raVal : Word)
    (h_too_long : BitVec.ult (8 : Word) len) :
    cpsTripleWithin 5 base (raVal &&& ~~~1) (rlp_content_to_u64_strict_code base)
  ...
  have hmono1 : ∀ a i, CodeReq.singleton (base + 4) (.BLTU .x5 .x11 (72 : BitVec 13)) a = some i
      → rlp_content_to_u64_strict_code base a = some i :=
    CodeReq.singleton_mono (CodeReq.ofProg_lookup_addr base rlp_content_to_u64_strict_prog 1 (base + 4)
      (by rw [rlp_content_to_u64_strict_prog_length]; norm_num)
      (by rw [rlp_content_to_u64_strict_prog_length]; norm_num) (by rfl))
  ...
  have hC : cpsTripleWithin 3 (base + 76) (raVal &&& ~~~1) (rlp_content_to_u64_strict_code base)
  ...
  have hA : cpsTripleWithin 1 base (base + 4) (rlp_content_to_u64_strict_code base)
...
/--
**`rlp_content_to_u64_strict` — non-canonical failure path.**
...
-/
theorem rlp_content_to_u64_strict_noncanonical_spec_within
    ...
    cpsTripleWithin 10 base (raVal &&& ~~~1) (rlp_content_to_u64_strict_code base)
  ...
  have hC : cpsTripleWithin 3 (base + 8) (base + 20) (rlp_content_to_u64_strict_code base)
  ...
  have hmono5 : ∀ a i, CodeReq.singleton (base + 20) (.BEQ .x7 .x0 (40 : BitVec 13)) a = some i
      → rlp_content_to_u64_strict_code base a = some i :=
    CodeReq.singleton_mono (CodeReq.ofProg_lookup_addr base rlp_content_to_u64_strict_prog 5 (base + 20)
      (by rw [rlp_content_to_u64_strict_prog_length]; norm_num)
      (by rw [rlp_content_to_u64_strict_prog_length]; norm_num) (by rfl))
  ...
  have hmono6 : ∀ a i, CodeReq.singleton (base + 24) (.LBU .x28 .x6 0) a = some i
      → rlp_content_to_u64_strict_code base a = some i :=
    CodeReq.singleton_mono (CodeReq.ofProg_lookup_addr base rlp_content_to_u64_strict_prog 6 (base + 24)
      (by rw [rlp_content_to_u64_strict_prog_length]; norm_num)
      (by rw [rlp_content_to_u64_strict_prog_length]; norm_num) (by rfl))
  ...
  have hmono7 : ∀ a i, CodeReq.singleton (base + 28) (.BEQ .x28 .x0 (40 : BitVec 13)) a = some i
      → rlp_content_to_u64_strict_code base a = some i :=
    CodeReq.singleton_mono (CodeReq.ofProg_lookup_addr base rlp_content_to_u64_strict_prog 7 (base + 28)
      (by rw [rlp_content_to_u64_strict_prog_length]; norm_num)
      (by rw [rlp_content_to_u64_strict_prog_length]; norm_num) (by rfl))
  ...
  simp only [signExtend12_0] at hRet
  have hG : cpsTripleWithin 2 (base + 68) (raVal &&& ~~~1) (rlp_content_to_u64_strict_code base)
...
theorem nat_lor_mul256_strict (x y : Nat) (hy : y < 256) : (x * 256) ||| y = x * 256 + y := by
...
theorem cu64_strict_step (a : BitVec 64) (b : BitVec 8) (ha : a.toNat < 2 ^ 56) :
  ...
  rw [hsh, hb]
  exact nat_lor_mul256_strict a.toNat b.toNat b.isLt
...
theorem cu64_strict_body_spec_within
    ...
    cpsTripleWithin 5 (base + 36) (base + 56) (rlp_content_to_u64_strict_code base)
...
/-- **The accumulation loop (idx 8..14), `base+32 → base+60`, by induction on the
    ...
    `u64` never overflows. -/
theorem cu64_strict_loop_spec_within
    ...
    cpsTripleWithin (7 * n + 1) (base + 32) (base + 60) (rlp_content_to_u64_strict_code base)
  ...
  have hmono : ∀ a i, CodeReq.singleton (base + 32) (.BEQ .x7 .x0 (28 : BitVec 13)) a = some i
      → rlp_content_to_u64_strict_code base a = some i :=
    CodeReq.singleton_mono (CodeReq.ofProg_lookup_addr base rlp_content_to_u64_strict_prog 8 (base + 32)
      (by rw [rlp_content_to_u64_strict_prog_length]; norm_num)
      (by rw [rlp_content_to_u64_strict_prog_length]; norm_num) (by rfl))
  ...
  | succ k ih =>
    ...
    have body := cu64_strict_body_spec_within base srcBase (BitVec.ofNat 64 (Nat.fromBytesBE pre)) x28Old
    ...
    have hacc : ((BitVec.ofNat 64 (Nat.fromBytesBE pre) <<< (8 : Nat)) ||| BitVec.setWidth 64 (srcBytes[si]'hsi0))
      ...
      rw [cu64_strict_step _ _ (by rw [hx10tn]; exact hprelt), hx10tn, BitVec.toNat_ofNat,
    ...
    have hjal_mono : ∀ a i, CodeReq.singleton (base + 56) (.JAL .x0 (-24 : BitVec 21)) a = some i
        → rlp_content_to_u64_strict_code base a = some i :=
      CodeReq.singleton_mono (CodeReq.ofProg_lookup_addr base rlp_content_to_u64_strict_prog 14 (base + 56)
        (by rw [rlp_content_to_u64_strict_prog_length]; norm_num)
        (by rw [rlp_content_to_u64_strict_prog_length]; norm_num) (by rfl))
    have hjal_ext := cpsTripleWithin_extend_code hjal_mono hjal
    have hjal_S : cpsTripleWithin 1 (base + 56) (base + 32) (rlp_content_to_u64_strict_code base)
...
/--
**`rlp_content_to_u64_strict` — success path (`0 < len ≤ 8`, canonical).**
...
-/
theorem rlp_content_to_u64_strict_success_spec_within
    (base srcBase raVal t0Old x6Old t2Old t3Old : Word) (srcBytes : List (BitVec 8)) (srcOff len : Nat)
    (hlen0 : 0 < len) (hlen8 : len ≤ 8)
    (hsalign : srcBase.toNat % 8 = 0) (hsoff : srcOff < srcBytes.length)
    ...
    cpsTripleWithin (7 * len + 11) base (raVal &&& ~~~1) (rlp_content_to_u64_strict_code base)
      ((.x10 ↦ᵣ (srcBase + BitVec.ofNat 64 srcOff)) ** (.x11 ↦ᵣ BitVec.ofNat 64 len) **
        (.x5 ↦ᵣ t0Old) ** (.x6 ↦ᵣ x6Old) ** (.x7 ↦ᵣ t2Old) **
        (.x28 ↦ᵣ t3Old) ** (.x0 ↦ᵣ (0 : Word)) ** (.x1 ↦ᵣ raVal) ** bytesRegion srcBase srcBytes)
  ...
  have hloop := cu64_strict_loop_spec_within base srcBase ((srcBytes[srcOff]'hsoff).zeroExtend 64) srcBytes []
  ...
  simp only [signExtend12_0] at hRet
  have hG : cpsTripleWithin 2 (base + 60) (raVal &&& ~~~1) (rlp_content_to_u64_strict_code base)
      ((.x11 ↦ᵣ BitVec.ofNat 64 len) ** (.x1 ↦ᵣ raVal))
...
/--
**`rlp_content_to_u64_strict` — empty / canonical-zero path (`len = 0`).**
...
preserved.
-/
theorem rlp_content_to_u64_strict_empty_spec_within
    (base srcBase raVal t0Old x6Old t2Old t3Old : Word) (srcOff : Nat) :
    cpsTripleWithin 8 base (raVal &&& ~~~1) (rlp_content_to_u64_strict_code base)
...
/--
**Unified spec for `rlp_content_to_u64_strict`.**
...
-/
theorem rlp_content_to_u64_strict_spec_within
    (base srcBase raVal t0Old x6Old t2Old t3Old : Word) (srcBytes : List (BitVec 8)) (srcOff len : Nat)
    (hlen64 : len < 2 ^ 64)
    (hsalign : srcBase.toNat % 8 = 0)
    (hslen : srcOff + len ≤ srcBytes.length)
  ...
  · -- too-long (status 2)
    ...
    have ht := cpsTripleWithin_frameR
      ((.x6 ↦ᵣ x6Old) ** (.x7 ↦ᵣ t2Old) ** (.x28 ↦ᵣ t3Old) **
       bytesRegion srcBase srcBytes)
      (by pcFree)
      (rlp_content_to_u64_strict_too_long_spec_within base (srcBase + BitVec.ofNat 64 srcOff)
  ...
  · by_cases h0 : len = 0
    · -- empty (status 0)
      ...
      have he := cpsTripleWithin_frameR (bytesRegion srcBase srcBytes) (by pcFree)
        (rlp_content_to_u64_strict_empty_spec_within base srcBase raVal t0Old x6Old t2Old t3Old srcOff)
      refine cpsTripleWithin_mono_nSteps (by omega)
    ...
    · by_cases hc : getByteAt srcBytes srcOff = 0
      · -- non-canonical (status 3)
        ...
        have hnc := rlp_content_to_u64_strict_noncanonical_spec_within base srcBase raVal t0Old x6Old t2Old t3Old
      ...
      · -- success (status 0)
        ...
        have hs := rlp_content_to_u64_strict_success_spec_within base srcBase raVal t0Old x6Old t2Old t3Old
...
example : rlp_content_to_u64_strict_prog.length = 22 := rfl
example : (CodeReq.ofProg (0 : Word) rlp_content_to_u64_strict_prog) 4 =
    some (.BLTU .x5 .x11 (72 : BitVec 13)) := by decide
example : (CodeReq.ofProg (0 : Word) rlp_content_to_u64_strict_prog) 76 =
