...
/-- **`regOwn`-re-entry scalar field decode-and-store into region.** As
    `unified_scalar_field_decode_and_store_region` but the four clobbered scratch registers
    (`x5, x10, x11, x12`) are owned abstractly (`regOwn`) in the precondition — callable after a
    prior field has run. -/
theorem unified_scalar_field_decode_and_store_region_at_regOwn
  ...
  · refine cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp) (fun _ h => h)
    ...
    exact cpsTripleWithin_weaken (fun _ hp => by xperm_hyp hp) (fun _ h => h)
      (unified_scalar_field_decode_and_store_region base regionBase rOut outBase fieldImm bs O data
        tail outBytes di0 v5 v10 v11 v12 v14Old v15Old hlen1 hlen8 halign hover hwin hdrop hdalign
        hdst hdov hdval hImm hcode).1
  · exact (unified_scalar_field_decode_and_store_region base regionBase rOut outBase fieldImm bs O
      data tail outBytes di0 0 0 0 0 v14Old v15Old hlen1 hlen8 halign hover hwin hdrop hdalign
