...
14. Activate GitHub merge queue on the already-present `merge_group` trigger; batch + auto-eject. **Write the merge-queue design note *first*** documenting the TAP test-and-bisect pattern (on batch failure, bisect the batch to eject the offending PR rather than re-running the full ~490-script ziskemu suite per PR) before flipping the queue on, so the team understands eviction behavior and avoids friction. The former design note was removed as stale on 2026-08-14 and must be rewritten before activation. **Extends `build.yml`.**
...
- **Do NOT make CI gates that exceed the Lean build budget.** A full clean build + ~490 ziskemu scripts is heavy; serial per-PR gating bottlenecks to ~4 merges/hour. Use the **merge queue with batching + auto-bisect** (TAP, documented in the merge-queue design note before activation), not serial re-runs. Run the heavy fuzz/dual-path suites nightly + on `merge_group`, lighter checks on PR push. Keep checks cheap/cached.
