...
| 4 | `mpt_node_kind` ✅ **LANDED** whole-routine `.proven` `mpt_node_kind_spec_within` (#11799 dep; callees `rlp_list_count_items` + `rlp_list_nth_item`; 4 fixture in-edges; caller `EvmAsm/Codegen/Programs/Mpt.lean:229`) | the node-shape dispatch of `_decode_witness_node` (`EvmAsm/Stateless/SpecRef/IncrementalMpt.lean:182`) — 17-item branch vs 2-item leaf/extension, that dispatch only | RLP node bytes | input: `bytesRegion`; post operational `MptNodeKindResult` (machineOnly); `kindTag` under WF via `MptNodeKindWire.mptNodeKindResult_eq_kindTag` (#12027; success arms `kind < 3`) |
...
| 9 | `header_extract_number` (all callees verified: `rlp_field_to_u64`, `rlp_list_nth_item`, `rlp_content_to_u64`; caller `EvmAsm/Codegen/Programs/ParentHashAtBlockNumber.lean:101`) | the `number` field of `_decode_header` (`EvmAsm/Stateless/SpecRef/Stateless.lean:158`) — one field of it, not the whole decode | header RLP region | `bytesRegion`; `rlpItemRegionFrom` (`EvmAsm/Codegen/Programs/RlpEncodeBytesComposeSAsm.lean:860`) gives the `RLPItem`-typed phrasing |
