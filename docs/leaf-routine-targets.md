...
| 4 | `mpt_node_kind` ✅ **LANDED** whole-routine `.proven` `mpt_node_kind_spec_within` (#11799 dep; callees `rlp_list_count_items` + `rlp_list_nth_item`; 4 fixture in-edges; caller `EvmAsm/Codegen/Programs/Mpt.lean:229`) | the node-shape dispatch of `_decode_witness_node` (`EvmAsm/Stateless/SpecRef/IncrementalMpt.lean:182`) — 17-item branch vs 2-item leaf/extension, that dispatch only | RLP node bytes | input: `bytesRegion`; post operational `MptNodeKindResult` (machineOnly); pure `kindTag` under WF via `mptNodeKindGuest_eq_kindTag` |
