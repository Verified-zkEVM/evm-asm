...
The stateless-guest pipeline below shows where each stage stands. Legend:
🟩 done · 🟢 shipped & runnable (unverified) · 🟨 in progress · 🟥 blocked · ⬜ not started.
...
flowchart TB
    ...
    classDef todo  fill:#546e7a,stroke:#2c3a40,color:#fff;

    IN["read_input — IO interface<br/>(#2 ✅ done)"]:::done
    RLP["RLP-decode block + witness<br/>(#3 🟡 RV64 decoder phases 1–3)"]:::prog
    LOOP["fetch / decode / dispatch loop<br/>M5b shipped · 154 / 256 bytes wired<br/>(#4)"]:::ship

    subgraph H["opcode handlers — runnable, unverified (#5)"]
      direction LR
      STK["stack / arith / bitwise<br/>shift / memory / env"]:::ship
      STO["SLOAD · SSTORE<br/>TLOAD · TSTORE<br/>(log + journal)"]:::ship
      CF["child-frame descent<br/>CALL* · CREATE / CREATE2<br/>+ init-code interp"]:::ship
      PC["precompiles (#6)<br/>RIPEMD160 · ecrecover<br/>alt_bn128 add/mul/pairing"]:::ship
    end

    RCPT["receipts<br/>EIP-7708 logs · per-tx windows<br/>bloom / RLP · consensus verdict"]:::ship
    VERD["block verdict<br/>sender recovery · requests-hash<br/>(deposit / wd / consolidation)"]:::ship
    MPT["MPT pre-state witness verify<br/>(#7 ⬜ not started)"]:::todo
    ROOT["verified post-state root<br/>(#8 🟥 blocked on #4–#7)"]:::block
    OUT["write_output + halt<br/>(#2 / #9 ✅ done)"]:::done

    IN --> RLP --> LOOP --> H
    H --> RCPT --> VERD --> ROOT --> OUT
    MPT -. trusted pre-state .-> LOOP
    MPT --> ROOT
```
