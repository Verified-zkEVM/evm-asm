...
| Receipts + verdict | EIP-7708 logs · bloom/RLP · sender recovery · requests-hash | 🟢 runnable |
| Scale | 936 programs · 691 ziskemu round-trips · 300+ cases | — |
| Lowering proofs | 6 registry invariants · 13 / 87 handler specs | 🟨 partial |
| RLP input decoder | block + witness decode | 🟨 in progress |
...
The repo runs at very high velocity — **~9,700 commits since 2026-05-29** (~3,400 conventional
`feat`/`fix`), in fine-grained increments. By commit scope, the window's effort concentrates in
three areas (excluding ~535 mechanical `PROGRESS.md` regenerations):

| Focus | Scopes (commit counts) | What it is |
|---|---|---|
| **Stateless block-validator guest (codegen)** — *the dominant theme* | verdict 112 · stateless 138 · precompile 90 · mpt 47 · call-frame 35 · create 34 · requests 22 · receipts 19 · bal 10 · gas/env/dispatch/blockhash | Assembling the RV64 guest that re-executes a block and checks it against the EIP-7928 Block Access List + witness, producing a pass/fail **verdict**: per-tx sender recovery, exec-vs-BAL storage/balance/nonce/code consistency, `requests_hash` derivation, receipts-consensus checks, and real precompiles. |
| **DIV/MOD/EXP arithmetic verification** | divmod 453 · exp 39 · mulmod 4 | Pushing the conditional/partial arithmetic opcodes toward full-domain, unconditional Hoare triples (the §1 🔶/🟡 tiers). |
| **EEST conformance harness** | eest 317 (feat+fix) | Wiring the Ethereum Execution Spec Tests as stateless fixtures and shaking out the guest against them (the `--reverse` sweep, false-reject fixes). |

**In one line:** the last fortnight has been overwhelmingly about **standing up the stateless
block-validator guest** — moving the codegen track from "runs opcodes" to "re-executes a whole
block and renders a BAL/witness-checked verdict" — running in parallel with the long-tail
**DIV/MOD full-domain proof** effort and **EEST** shakedown. (Only **2 PRs open** at report time,
both in these themes: a receipts false-reject fix and a `u256_lt` stateless leaf — the repo merges
to `main` near-continuously.)
