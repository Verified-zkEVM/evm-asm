...
2. **Image-`CodeReq` coverage is a soundness-of-statement issue, not a
   completeness one.** `guestImageCodeReq` pins **35.39%** of `.text`
   (121500 of 343356 bytes; `scripts/guest_image_coverage.py` — re-measure). A `cr` that
