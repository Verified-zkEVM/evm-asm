...
gone; all 51 `*_sail_equiv` lemmas pass; axioms = the 3 classical ones; full
...
**Start with P4** if you want proof progress without standing up the Sail toolchain —
it's ~1 session, pure Lean, and produces the single auditable "our step *is* RISC-V"
object on the correct (vendored) model. **Do P3** when you can run the Sail C simulator /
`riscv-tests` — it's infra-heavy (executable-Lean generation + a CI job) but it's the
*essential* mitigation: GO so far means "builds + type-checks", NOT "proven correct
...
- **Deliverable:** `EvmAsm/Rv64/SailEquiv/StepSim.lean` —
  ...
  (model review §5.5).
- **Verify:** builds; `#print axioms step_execute_sail_sim` = 3 classical axioms only.
  (Pattern proven this session: `lake env lean` on a scratch file that `#print axioms`.)
...
- **Push still pending.** `feat/sail-zkvm-integration` has unpushed commits. Auto-mode
  has denied `git push` before — the user runs it:
  `git push origin feat/sail-zkvm-integration`.
