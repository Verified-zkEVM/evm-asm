...
`ByteList[1024]` boundary (1023 and 1024 bytes). Five adversarial inputs must
have `successful_validation = 0`: a forged witness child hash, malformed node
RLP, 1025-byte node, forged BAL account post-balance, and forged BAL storage
post-value. The generator independently asserts execution-specs accepts the
controls and rejects every forgery; the runner checks full output only for the
accepted controls because a rejected output's request root is not observable.
