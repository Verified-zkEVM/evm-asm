...
| Terminating | STOP, INVALID, RETURN, REVERT | 7 / 7 / tail / tail | ✅ STOP + INVALID proved (`evm_stop_stack_spec_within` / `evm_invalid_stack_spec_within`); RETURN + REVERT full standalone tails are `.conditional` (`evm_return_stack_spec_within` / `evm_revert_stack_spec_within`) from the framed post-gas entry through the descriptor window to `dispatchHaltRet 2`; REVERT also covers rollback length-cell restores |
...
- **Full-tail compositions**: DONE for RETURN and REVERT.
  - RETURN: `Terminating.evm_return_stack_spec_within` covers the standalone
    `depthAware=false` descriptor window, including the `system_call_mode=0`
    capture-block skip, `kind=1@+32`, and `dispatchHaltRet 2`; registry tier is
    `.conditional` with `return_precondition_reachable`.
  - REVERT: `Terminating.evm_revert_stack_spec_within` covers the standalone
    `depthAware=false` descriptor window, `kind=2@+32`, the rollback length-cell
    restores via `x20` (`env+456→448`, `env+464:=0`, `env+480→472`), and
    `dispatchHaltRet 2`; registry tier is `.conditional` with
    `revert_precondition_reachable`.
  - The memory-gas `preBody` (OOG branch) stays OUTSIDE both triples as the
    framed TCB entry boundary.
