...
- Still deferred (unchanged): the kernel-checked *binding* of `cycleBound` to
  the witness theorem's literal — see the Phase-1 R-C4 follow-up below. This
  backfill is a hand-read of each conclusion, so the binding gate is what would
  make it drift-proof.
...
- **Separation-logic state-assertion vocabulary** (layout-faithful Assertions
  ...
  for the 128-byte execution log and the canonical block `storage_writes` map
...
- **Progress-registry semantics — agent-steering rollout** (`EvmAsm/Progress.lean`,
  ...
  - **Phase 4 (this work) — review throughput & merge safety (P3):** rations the
    ...
    - **D2 (step 14) — merge-queue design doc FIRST.** ⚠️ The design note
      The former merge-queue design note was REMOVED as stale on 2026-08-14;
      rewrite it before activation. It documented the GitHub native merge queue with
