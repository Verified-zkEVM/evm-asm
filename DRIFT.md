...
| 4 | EVM interpreter loop on the decoded block | 🟡 blocked | no simulation bridge from dispatched handlers to the SpecRef interpreter — #11801 is the one-opcode `h_ADD` pilot for that bridge, `stage_system_call` has no machine post yet; #11578 rescoped off derive_* shims (NOT leaves) to `execution_requests_hash` validation-accept prefix (landed domainRestricted); hash half + stage_system_call still residual | 2026-08-11 |
