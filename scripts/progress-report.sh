...
# --------------------------------------------------------------------
# Pins — submodule SHAs, toolchain
#
# GH #12683: the generating commit SHA and date are deliberately NOT rendered.
# They made every regeneration rewrite the same header line, so any two
# concurrent PRs that regenerated PROGRESS.md conflicted on it regardless of
# whether their registry changes overlapped. Nothing consumed the value — it
# had exactly two references, the emitter and the drift gate's own ignore
# filter — and the file's git history answers "which commit produced this"
# more reliably than a self-reported SHA.
# --------------------------------------------------------------------
...
case "$MODE" in
  ...
  --check)
    ...
    # GH #12683: this comparison is now BYTE-EXACT. It used to need
    # `-I '^> Snapshot:'` because the header carried the generating commit's
    # SHA, which changes on every commit — so without the filter the gate
    # fired on every PR by construction. That line has been removed from the
    # emitter, which both removes the need for the filter and (the actual
    # motivation) stops PROGRESS.md from being a GUARANTEED merge conflict:
    # every concurrent regenerating PR rewrote the same line 3, so disjoint
    # registry-row changes still collided. Three PRs hit that in one session.
    #
    # ⚠️ Do not reintroduce an ignore pattern here. An ignored line is a place
    # a hand-edit can hide from the drift gate; byte-exact is strictly
    # stronger. Readers wanting "which commit produced this" have the file's
    # own git history, which is more accurate than a self-reported SHA.
