#!/usr/bin/env bash
#
# progress-report.sh — render the progress dashboard from the
# kernel-checked registry in EvmAsm/Progress.lean plus grep-derived
# numeric facts.
#
# Modes:
#   scripts/progress-report.sh --write            # render to ./PROGRESS.md
#   scripts/progress-report.sh --write <path>     # render to <path>
#
# The output is a GENERATED ARTIFACT and is NOT committed (#12683): a
# generated file in the tree conflicted on every concurrent PR that
# regenerated it. `PROGRESS.md` is therefore in .gitignore, and the
# `--check` drift mode (plus its CI entry point check-progress.sh) is
# gone — with nothing committed there is nothing to compare against.
# Consumers regenerate instead:
...
# decided. Note for whoever does it: the relative Markdown links below
# (`[foo](EvmAsm/foo.lean)`) resolve inside the repo view but NOT in a
# Discussion body, so that step needs to absolutize them.
#
# Design notes:
...
# Default target keeps the historical path for local use; the nightly passes
# an explicit temp path so nothing is written into the checkout.
OUT="${2:-PROGRESS.md}"
...
> registry in [\`EvmAsm/Progress.lean\`](EvmAsm/Progress.lean). This file is a **generated artifact
> and is not committed** (#12683) — regenerate it with \`scripts/progress-report.sh --write\`
> rather than editing it, and never cite a line number in it.
...
| Build CI guardrails | \`check-no-warnings.sh\`, \`check-unimported.sh\`, \`check-file-size.sh\`, \`check-drift.sh\`, \`check-axioms.sh\`, \`check-conformance-floor.sh\` |
...
# `mv` across filesystems (e.g. $RUNNER_TEMP on a different mount) is a
# copy+unlink in coreutils, so this is safe for an arbitrary --write target.
mv "$TMP" "$OUT"
trap - EXIT
echo "Wrote $OUT"
