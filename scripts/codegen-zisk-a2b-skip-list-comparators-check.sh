...
with open(sys.argv[1], \"wb\") as f:
    f.write(struct.pack(\"<Q\", len(bal))); f.write(struct.pack(\"<Q\", len(rows))); f.write(struct.pack(\"<Q\", len(skips)))
    f.write(skip); f.write(txidx); f.write(log); f.write(bal)
    total = 24 + len(skip) + len(txidx) + len(log) + len(bal); f.write(bytes((-total) % 8))
" "$in_file"
