#!/usr/bin/env bash
# Nonzero leaf-group-prefix singleton-collapse KAT for sd13v.
set -euo pipefail
cd "$(dirname "$0")/.."
...
empty = Bytes(b'')
# The two descriptors share their first nibble (0), making s5=1 before the
# deleted old child (0) and inserted singleton child (2) diverge.
old_rlp = b'\xe3\xa1\x20' + b'\0' * 32 + b'\x80'
old = LeafNode(Bytes([0] * 64), empty)
survivor_key = bytes([0, 2]) + b'\0' * 62
survivor = LeafNode(Bytes(survivor_key), Bytes(b'\x02'))
...
print('PASS: bounded builder preserves a nonzero leaf-group prefix after singleton collapse')
PY
