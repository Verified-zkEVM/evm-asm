...
# run_case <name> <halt_kind> <gas_left> <state_left> <refund> <state_used>
run_case() {
  local name="$1" halt="$2" gas="$3" state_left="$4" refund="$5" state_used="$6"
  ...
  python3 -c "
import struct, sys
vals = [$halt, $gas, $state_left, $refund, $state_used]
...
" "$in_file"
  ...
  python3 - "$out_file" "$name" "$halt" "$gas" "$state_left" "$refund" "$state_used" <<'PY'
import struct, sys
out_file, name = sys.argv[1], sys.argv[2]
halt, gas, state_left, refund, state_used = map(int, sys.argv[3:])
data = open(out_file, "rb").read()
actual = struct.unpack("<QQQ", data[:24])
if halt in (0, 1, 5):
    expected = (gas + state_left, refund, 1)
elif halt == 2:
    expected = (gas + state_left + state_used, 0, 0)
else:
    expected = (state_left + state_used, 0, 0)
ok = actual == expected
print(
...
FAILED=0
run_case "stop_success"       0 1000 70 9 555 || FAILED=1
run_case "return_success"     1 2222 33 4 777 || FAILED=1
run_case "selfdestruct_ok"    5 1234 66 8 111 || FAILED=1
run_case "revert_restores"    2 1000 70 9 55  || FAILED=1
run_case "invalid_burns_gas"  3 1000 70 9 55  || FAILED=1
run_case "outofgas_burns"     6 4444 12 3 34  || FAILED=1
