...
#     the `.bss` section-start is PER UNIT (guest 0xa0b70000, probe/test units
...
check ".data base"       "00000000a0b00000" "$DATA_BASE"
check ".bss base"        "00000000a0b70000" "$BSS_BASE"
...
ok = (data_end <= 0xa0b70000 and bss_end < 0xbf980000 and bss_end < 0xc0000000
      and sgd_base == bss_end and sgd_end < 0xbf980000)
print(f"  {'OK  ' if ok else 'DRIFT'} .data end 0x{data_end:x} <= .bss base 0xa0b70000")
...
# predicate. The ratchet also requires scripts/bal-class-a-notes.md and counts
# its explicit rationale bullets.
