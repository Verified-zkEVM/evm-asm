eip7702_authorization_extract_signature:
  ...
  bnez a2, .Lta77es_fail
  mv s5, a0                   # cursor
  mv s6, a1                   # end
  mv a0, s5; mv a1, s6; jal ra, rlp_walk_next; bnez a1, .Lta77es_fail; mv s5, a0
  mv a0, s5; mv a1, s6; jal ra, rlp_walk_next; bnez a1, .Lta77es_fail; mv s5, a0
  mv a0, s5; mv a1, s6; jal ra, rlp_walk_next; bnez a1, .Lta77es_fail; mv s5, a0
  # ---- Signature field 0: y_parity/v (canonical uint <= 8 bytes) -> u64 ----
  mv a0, s5; mv a1, s6; jal ra, rlp_walk_next; bnez a1, .Lta77es_fail
  sub t0, a0, a2; mv s7, a0; mv a0, t0; mv a1, a2
  jal ra, rlp_content_to_u64_strict
  bnez a1, .Lta77es_size
  sd a0, 0(s2); mv s5, s7
