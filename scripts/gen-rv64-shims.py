...
# The revision the original (pre-shim) module sources are read from, for their
# import blocks.  `main` is correct: the shims replace files that still exist
# there unchanged, and this must NOT drift to the working tree, or regenerating
# a second time would read a shim's own imports back in.
...
def original_prelude(rel: str) -> tuple[list[str], bool]:
    ...
    if proc.returncode != 0:
        raise SystemExit(f"gen-rv64-shims: cannot read {BASE_REV}:{path}")
...
def render(rel: str, names: list[str]) -> str:
    ...
    # Splice the dependency import in after the last existing import, so it sits
    # inside the module header rather than after `@[expose] public section`.
...
def main() -> int:
    ...
    for rel, names in sorted(shims.items()):
        ...
        text = render(rel, sorted(set(names)))
        if args.check:
